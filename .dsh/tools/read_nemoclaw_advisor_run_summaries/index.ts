/**
 * Read bounded Markdown summaries from retained PR Review Advisor artifacts for one GitHub Actions run. Requires Bash, base64, mktemp, stat, and Info-ZIP on Linux.
 */
export default async function read_nemoclaw_advisor_run_summaries(input: {
  ...
  if (!Number.isSafeInteger(maxCharacters) || maxCharacters < 1000 || maxCharacters > 100000)
    throw new Error("maxSummaryCharacters must be an integer from 1000 through 100000");
  const listed = await tools.run_github_cli({
    workdir: input.workdir,
    args: [
      "api",
      `repos/${repo}/actions/runs/${input.runId}/artifacts?per_page=100`,
      "--jq",
      '{artifacts:[.artifacts[]|select(.name|startswith("pr-review-specialist-"))|{id,name,expired,size:.size_in_bytes}]}',
    ],
    timeoutMs: 60000,
  });
  let payload;
  ...
  const all = Array.isArray(payload.artifacts) ? payload.artifacts : [];
...
base64 -w 0 "\$tmp/summary"`;
    ...
    if (result.kind !== "foreground")
      throw new Error("The artifact reader must run in the foreground");
    ...
    const clipped = decoded.length > maxCharacters;
  ...
  return {
    ...
    truncated: all.length > selected.length || summaries.some((item) => item.truncated),
    summaries,
    failures,
  };
}
