...
The finished OpenClaw workflow obtains its approved native inference MSIX from
the authenticated draft release `native-inference-inputs-2026-09-16` in the
same repository. CI requires the exact asset name, uploaded state, size, and
GitHub-reported SHA-256 before downloading it with the job's read-only token.
The ordinary intake then verifies the pinned package identity, architecture,
source revision, and bounded CUDA file set. The admitted receipt participates
in the runtime namespace and is retained as build evidence; the source MSIX is
not copied into the downloadable preview artifact set.
