...
async function main(): Promise<void> {
  ...
  const adapterIndex = process.argv.indexOf("--python-temp-adapter-sha256");
  const adapterSha256 = adapterIndex < 0 ? null : process.argv[adapterIndex + 1];
  if (
    (tempControl === null && adapterSha256 !== null) ||
    (tempControl !== null && !/^[a-f0-9]{64}$/u.test(adapterSha256 ?? ""))
  )
    throw new Error("The Python temp control requires an exact adapter SHA-256.");
      ...
      JSON.stringify({
        mxc: {
          command: [
            ...
            resultPath,
            nonce,
            ...(tempControl === null ? [] : [tempControl, adapterSha256!]),
          ],
          cwd: shareRoot,
          windows_ui: true,
        },
