...
import { requireAdapterSha256, tempControlPassed } from "./probe-python-temp-workload.mts";
...
test("the contained control receives the authoritative reviewed adapter hash", () => {
  assert.equal(requireAdapterSha256(COMPONENT_ADAPTER_SHA256), COMPONENT_ADAPTER_SHA256);
  assert.throws(() => requireAdapterSha256(undefined), /exact adapter SHA-256/u);
  assert.throws(() => requireAdapterSha256("A".repeat(64)), /exact adapter SHA-256/u);
});
