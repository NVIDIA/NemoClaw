...
export function requireAdapterSha256(value: string | undefined): string {
  if (typeof value !== "string" || !/^[a-f0-9]{64}$/u.test(value))
    throw new Error("The contained Python temp control requires an exact adapter SHA-256.");
  return value;
}
...
async function main() {
  const [runtime, output, nonce, control, adapterSha256Value] = process.argv.slice(2);
  if (
  ...
  const adapterSha256 = requireAdapterSha256(adapterSha256Value);
  ...
  for (const [id, relative] of [
    ...
    const result = await command(
      ...
      ["-I", "-c", pythonTempSource(), nonce, control, adapterSha256],
      process.env,
      process.cwd(),
    );
  ...
  const result = {
    ...
    nonce,
    control,
    expectedAdapterSha256: adapterSha256,
