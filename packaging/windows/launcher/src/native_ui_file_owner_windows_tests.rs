...
fn junction(link: &Path, target: &Path) {
    ...
    let mut command = std::process::Command::new(Path::new(&system).join("System32\\cmd.exe"));
    command
        .args(["/d", "/c", "mklink", "/J"])
        .arg(link)
        .arg(target)
    ...
    let result = crate::runtime_host::spawn_command(&mut command)
        .unwrap()
        .wait_with_output()
        .unwrap();
    assert!(
...
fn assert_rejects_reopened_permissions(arguments: &[&str]) {
    ...
    drop(fixture.owner.take());
    let system = std::env::var("SystemRoot").unwrap();
    let mut command = std::process::Command::new(Path::new(&system).join("System32\\icacls.exe"));
    command
        .arg(&fixture.root)
        .args(arguments)
        .creation_flags(0x0800_0000)
