...
struct Identity {std::wstring root,sid;DWORD session=0;};
Identity identity(){
  ...
  require(wcsnlen(buffer.data(),buffer.size())<buffer.size(),"private-root-bound");result.root=buffer.data();
  require(ProcessIdToSessionId(GetCurrentProcessId(),&result.session)!=0,"session-query");
  require(result.root.ends_with(L"\\"+result.sid),"private-root-sid");return result;
}
...
std::string identityFields(const Identity& id){return "\"pid\":"+std::to_string(GetCurrentProcessId())+",\"session\":"+std::to_string(id.session)+",\"sid\":"+quote(utf8(id.sid))+",\"privateRoot\":"+quote(utf8(id.root))+",\"rawProbeUnshimmed\":true";}
...
int wmain(int argc,wchar_t** argv){
  try{
    ...
    Api api;Identity id=identity();jobProof(workerPid,executorPid);std::wstring gd=absolute(id.root,globalLeaf(key)),sd=id.session?absolute(id.root,sessionLeaf(id.session,key)):gd;
    ...
    for(int count=0;count<3;++count){
      ...
      std::wstring foreign=absolute(other,globalLeaf(key));Handle fd,fe,fs;NTSTATUS ds=openDir(api,foreign,fd);std::wstring ep=absolute(foreign,eventLeaf),mp=absolute(foreign,sectionLeaf);Name en(ep),mn(mp);NTSTATUS ev=api.openEvent(&fe.value,EVENT_ALL_ACCESS,&en.attributes),se=api.openSection(&fs.value,SECTION_MAP_READ,&mn.attributes);
      std::wstring original=L"\\BaseNamedObjects\\msys-2.0S5-"+key;Name originalName(original,nullptr,&security,0x80);Handle glob;NTSTATUS globalStatus=api.createDirectory(&glob.value,0x2000f,&originalName.attributes);
      line("{\"kind\":\"denials\","+identityFields(id)+",\"foreignRoot\":"+quote(utf8(other))+",\"foreignDirectory\":"+status(ds)+",\"foreignEvent\":"+status(ev)+",\"foreignSection\":"+status(se)+",\"originalGlobalCreate\":"+status(globalStatus)+"}");
      constexpr auto denied=static_cast<NTSTATUS>(0xc0000022u);require(ds==denied&&ev==denied&&se==denied&&globalStatus==denied,"isolation-denial-failed");checked=true;
    }
