...
$process=[Diagnostics.Process]::Start($start);$failure=$null
try{if(-not $process.WaitForExit(110000)){throw 'The raw probe compiler exceeded its bound.'};if($process.ExitCode -ne 0){throw 'The unshimmed native object probe did not compile.'}}
catch{$failure=$_}finally{try{if(-not $process.HasExited){$process.Kill($true);if(-not $process.WaitForExit(5000)){throw 'Raw probe compiler did not stop.'}}}catch{if($null -eq $failure){$failure=$_}else{Write-Warning 'Raw probe compiler cleanup also failed.'}};$process.Dispose()}
