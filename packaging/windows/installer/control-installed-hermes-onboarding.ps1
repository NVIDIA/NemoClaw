...
$configFile = Join-Path $settings 'agents\hermes\native-windows.json'
...
$result = [ordered]@{ schemaVersion=1; classification='installed-hermes-native-onboarding'; passed=$false;
    nativeWpf=$false; source='installed-launcher-onboard'; successfulConfigurationSaves=0; rejectedEmptyIntegrationSave=$false;
    tavilyLiveLookup='not-tested-user-waiver'; appReceivedCiFlag=$false; keysLogged=$false; cleanupClosed=$false }
$primary = $null; $window = $null; $phase = 'launch'; $watch = [Diagnostics.Stopwatch]::StartNew()
$info = New-PreviewProcess $launcher @('--onboard','--agent','hermes','--wait')
...
try {
    ...
    $phase = 'Hermes-provider'
    $choice = Wait-InstalledElement $window 'AgentHermes'
    ...
    if ($saved.agent -cne 'hermes' -or $saved.profile -cne 'personal' -or $saved.inference -cne 'nvidia' -or $saved.endpoint -cne 'https://integrate.api.nvidia.com/v1' -or $saved.model -cne $Model -or $saved.credentialStored -cne $true) { throw 'The native saved configuration differs from the actual choices.' }
    if ([IO.File]::ReadAllText((Join-Path $settings 'active-agent.txt')).Trim() -cne 'hermes') { throw 'Native onboarding did not remember Hermes.' }
    $result.successfulConfigurationSaves = 1
    $result['rememberedAgent'] = 'hermes'; $result['provider'] = 'nvidia'; $result['searchRequested'] = $false
