...
#[test]
fn bundle_managed_first_install_reaches_verified_selection() {
    let mut store = Store::old();
    store.selected = None;
    let next = identity('e');
    begin(
...
#[test]
fn published_0_1_4_descriptor_is_preserved_until_replacement_is_selected() {
    let published = RuntimeIdentity {
        runtime_id: "67a963c0b49d32bce7b7c2bc7975dd4cac179fcb34cc33af996229c0a33a2446".into(),
        manifest_sha256: "80fe905a3ce888fd365a087cf8fe0e7cce5f0a39eb219327e717a55843aa3964".into(),
        source_revision: "7935e0ccee812d365fd58d32e7291fd3286a98bf".into(),
        node_sha256: "97cce5301a815d2dce07ac5bfd1e6039eae88185ec1d10ae4f8cb712f1732878".into(),
        node_version: "22.23.2".into(),
    };
    let mut store = Store::old();
    store.selected = Some(published.clone());
    let next = identity('e');
    begin(
        &mut store,
        &next.runtime_id,
    ...
    .unwrap();
    assert_eq!(store.journal.as_ref().unwrap().previous, Some(published));
    assert!(store.selected.is_none());
    verify(&mut store, &next.runtime_id, &next).unwrap();
    commit(&mut store, &next.runtime_id, false).unwrap();
    assert_eq!(store.selected, Some(next));
