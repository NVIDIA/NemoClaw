...
fs.writeFileSync(
  driver,
  `const cp=require("node:child_process"),fs=require("node:fs"),Module=require("node:module");const loaded=new Set();const originalLoad=Module._load;Module._load=function(request,...rest){const value=originalLoad.call(this,request,...rest);if(request==="undici")loaded.add(value);return value;};for(const name of ["spawn","spawnSync","execFile","execFileSync"]){const original=cp[name];cp[name]=function(...args){if(/(?:npm|npx|pnpm|yarn|pip)(?:\\b|[-.])/i.test(JSON.stringify(args.slice(0,2)))){fs.writeFileSync(${javascriptStringLiteral(path.join(output, "package-manager-attempt"))},name);throw Error("Unexpected package-manager invocation");}return original.apply(this,args);};}Module.syncBuiltinESMExports();process.on("exit",()=>fs.writeFileSync(${javascriptStringLiteral(path.join(output, "undici-instances.json"))},JSON.stringify({instances:loaded.size})));const entry=${javascriptStringLiteral(entry)};process.argv=[process.execPath,entry,"agent","--local","--agent","main","--session-id",${javascriptStringLiteral(nonce)},"--message",${javascriptStringLiteral("Run the controlled test tools then reply " + marker)},"--json"];require(entry).runCli(process.argv).catch(error=>{console.error(error);process.exitCode=1;});`,
);
