...
# Windows PowerShell 5.1 emits a JSON array as one pipeline object. Preserve
# that shape explicitly on newer PowerShell too, then validate its elements.
$parseOptions = @{ InputObject = (Get-Content -LiteralPath $InputPath -Raw) }
if ((Get-Command ConvertFrom-Json).Parameters.ContainsKey('NoEnumerate')) { $parseOptions.NoEnumerate = $true }
$paths = ConvertFrom-Json @parseOptions
if($paths -isnot [Array] -or $paths.Count -lt 1 -or $paths.Count -gt 32 -or (Test-Path -LiteralPath $OutputPath)){throw 'ACL snapshot paths/output are invalid.'}
$rows = @()
