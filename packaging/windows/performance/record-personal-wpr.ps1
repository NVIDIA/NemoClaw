...
if(-not(Test-Path -LiteralPath $requestPath -PathType Leaf) -or (Get-Item -LiteralPath $requestPath).Length -gt 16384){throw 'The trace request is absent or exceeds its bound.'}
...
$state=$null;$traceRecord=$null;$failure=$null;$readyWritten=$false;$toolPresent=Test-Path -LiteralPath (Get-WindowsPerformanceRecorderPath) -PathType Leaf
$record=[ordered]@{schemaVersion=1;classification='personal-primary-wpr-owner';sourceRevision=$request.sourceRevision;nonce=$request.nonce;requestSha256=(Get-FileHash -LiteralPath $requestPath -Algorithm SHA256).Hash.ToLowerInvariant();measuredPolicySha256=$request.policySha256;primaryLaunchedByRecorder=$false;debuggerAttached=$false;partialPrefixOnly=$true;maximumRecordingSeconds=45;maximumObservedRecordingBytes=256MB;recordingStopped=$false;trace=$null;error=$null}
try{
 ...
 }elseif(-not $toolPresent){$record.recordingStopped=$true}
 if(-not $readyWritten){Save-OwnedJson @{sourceRevision=$request.sourceRevision;nonce=$request.nonce;started=$false;error=$record.error} $ready}
