...
test("installed Hermes query modes bind both held owners and remain host or guest scoped", () => {
  ...
  for (const agentId of ["hermes", "pi"] as const) {
    ...
    const agentRuntimeRoot = "C:\\NemoClawRtxSparkPreviewState-S-1-5-21-1-2-3-1001-hermes";
    let stateHeld = true;
...
test("Hermes turn-only worker also preserves the held canonical tool environment", () => {
  ...
  const tools = nativeHermesToolEnvironment(heldHermesRuntime(), "C:\\Windows");
  const context: any = {
    process: { env: tools.environment, execPath: "C:\\shared-worker\\node.exe" },
    hermesHome: "C:\\state\\.hermes",
    home: "C:\\state",
    python: tools.python,
    required: () => tools.node,
...
test("download-on-setup choices preserve model identity without external inference credentials", async () => {
  for (const model of NATIVE_LOCAL_MODELS.models) {
    ...
    await configureNativeFromStdin(
      ...
      {
        inspectModel: async (_root, _launcher, selected) => {
    ...
    const submitted = {
      agent: "openclaw",
      inference: "local",
      options: { model: model.id, localModel: model.id },
    };
    assert.equal(normalizeOnboardingConfiguration(submitted, false).model, model.id);
    for (const mismatch of [
      { model: "foreign" },
      { endpoint: "http://127.0.0.1:8000/v1" },
      { credential: "unexpected-key" },
    ])
      assert.throws(() =>
        normalizeOnboardingConfiguration(
          { ...submitted, options: { ...submitted.options, ...mismatch } },
          false,
        ),
      );
  }
