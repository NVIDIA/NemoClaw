...
Require(!selected.BundledLocalInferenceAvailable);
var withEngine = Encoding.UTF8.GetBytes(Encoding.UTF8.GetString(Record()).Replace("\"schemaVersion\":1", "\"schemaVersion\":1,\"prebuiltLocalModelAvailable\":false,\"bundledLocalInferenceAvailable\":true", StringComparison.Ordinal));
Require(NativeRuntimeAvailability.Parse(withEngine).BundledLocalInferenceAvailable && !NativeRuntimeAvailability.Parse(withEngine).PrebuiltLocalModelAvailable);
Reject(Encoding.UTF8.GetBytes(Encoding.UTF8.GetString(withEngine).Replace("\"bundledLocalInferenceAvailable\":true", "\"bundledLocalInferenceAvailable\":\"true\"", StringComparison.Ordinal)));
Reject(Encoding.UTF8.GetBytes(Encoding.UTF8.GetString(withEngine).Replace("\"prebuiltLocalModelAvailable\":false,", "", StringComparison.Ordinal)));
Console.WriteLine("14 availability controls passed; includes legacy and bundled-engine presentation, no Windows UI or qualification claim.");
