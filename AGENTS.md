...
- Run `npm run test:changed` from a checkout with `origin/main`.
  After affected source tests pass, the command rejects new JavaScript files, growth in `src/lib/onboard.ts`, current test-file line-budget violations, and added `if` statements in changed tests.
