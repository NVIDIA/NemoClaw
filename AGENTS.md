...
- Run `npm run test:changed` from a checkout with `origin/main`. After affected source tests pass, it rejects new JavaScript files, growth in `src/lib/onboard.ts`, current test-file line-budget violations, and added `if` statements in changed tests. The trusted PR check also rejects test-file budget limits that increase from the base commit. Use `npm run test:watch` for focused source-test feedback. Add only concrete opaque-input mappings to `test/helpers/vitest-watch-triggers.ts` when the import graph cannot see a YAML, Python, shell, generated, or workflow dependency.
