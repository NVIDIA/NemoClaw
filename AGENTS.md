...
- Use `npm run test:changed` for affected CLI, plugin, and E2E-support tests plus the codebase growth contract test. Use `npm run test:watch` for focused source-test feedback. Add only concrete opaque-input mappings to `test/helpers/vitest-watch-triggers.ts` when the import graph cannot see a YAML, Python, shell, generated, or workflow dependency.
