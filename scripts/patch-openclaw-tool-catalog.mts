...
function patchNativeLlamacppCatalogCompat(distDir: string): {
  ...
  if (source.includes(NATIVE_LLAMACPP_MARKER)) {
    if (source.includes(NATIVE_DIRECT_TOOL_PATTERN)) {
      throw new Error(`${target}: native llama.cpp marker is present but original target remains`);
    }
    if (!source.includes(NATIVE_DIRECT_TOOL_REPLACEMENT)) {
      throw new Error(`${target}: native llama.cpp compatibility patch shape is incomplete`);
    }
    return { patched: false, file: target };
  }

  const count = countOccurrences(source, NATIVE_DIRECT_TOOL_PATTERN);
  if (count !== 1) {
    throw new Error(`${target}: expected exactly one native direct-tool target, found ${count}`);
  }
  const text = source.replace(NATIVE_DIRECT_TOOL_PATTERN, NATIVE_DIRECT_TOOL_REPLACEMENT);
  if (!text.includes(NATIVE_LLAMACPP_MARKER) || text.includes(NATIVE_DIRECT_TOOL_PATTERN)) {
