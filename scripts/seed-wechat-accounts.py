...
# the pre-installed extension files in place. generate-openclaw-config.ts
...
def _patch_openclaw_config(account_id: str) -> None:
    ...
    if not cfg_path.exists():
        # generate-openclaw-config.ts runs before us and is responsible for
...
def main() -> int:
    ...
    # every build from generate-openclaw-config.ts.
    if not account_id:
        return 0
