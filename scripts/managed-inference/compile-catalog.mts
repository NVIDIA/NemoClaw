...
const GENERATED_CATALOG_DOCUMENT_PATH = "src/lib/inference/serving/generated/catalog.json";
const LEGACY_GENERATED_CATALOG_PATHS = [
...
export function renderManagedInferenceCatalogDocument(
...
export function compileManagedInferenceCatalogSources(
  ...
  return {
    catalog,
    document: renderManagedInferenceCatalogDocument(catalog),
  };
}
...
export function writeManagedInferenceCatalog(
  repositoryRoot = REPOSITORY_ROOT,
): CompiledManagedInferenceCatalogResult {
  const compiled = compileManagedInferenceCatalog(repositoryRoot);
  const outputPath = path.join(repositoryRoot, GENERATED_CATALOG_DOCUMENT_PATH);
  fs.mkdirSync(path.dirname(outputPath), { recursive: true });
  fs.writeFileSync(outputPath, compiled.document, "utf8");
  for (const legacyPath of LEGACY_GENERATED_CATALOG_PATHS) {
    fs.rmSync(path.join(repositoryRoot, legacyPath), { force: true });
  }
  return compiled;
}
...
function main(): void {
  ...
  if (process.argv.includes("--write")) {
    ...
    compileManagedInferenceCatalog();
  }
}
