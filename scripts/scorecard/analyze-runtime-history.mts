...
export function formatRuntimeHistory(
  ...
  for (const current of currentRows.slice(0, 10)) {
    ...
    const priorMedian = median(medians);
    lines.push(
      `| ${escapeCell(current.target)} | ${escapeCell(current.scenario)} | ${priorRows.length} | ${seconds(current.medianMs)} | ${seconds(priorMedian)} | ${seconds(percentile(medians, 0.95))} | ${formatDelta(current.medianMs, priorMedian)} | ${formatPassRate(priorRows)} | ${formatOutcome(current)} |`,
    );
  }
  return `${lines.join("\n")}\n`;
}
