...
export const INJECTED_REMOVE_HELPER = [
  ...
  '\tconst supportedTargets = [report.workspaceDir, CONFIG_DIR].map((rootDir) => { const skillsDir = path.join(rootDir, "skills"); return { rootDir, skillsDir, targetDir: path.join(skillsDir, slug) }; });',
  "\tlet selected = resolveSkillStatusEntry(report.skills, slug);",
  "\tfor (const target of supportedTargets) {",
  "\t\tlet rootStat; let skillsStat; let rootRealPath; let skillsRealPath;",
  '\t\ttry { rootStat = await fs.lstat(target.rootDir); skillsStat = await fs.lstat(target.skillsDir); rootRealPath = await fs.realpath(target.rootDir); skillsRealPath = await fs.realpath(target.skillsDir); } catch (error) { if (error && typeof error === "object" && error.code === "ENOENT") continue; throw error; }',
  '\t\tif (!rootStat.isDirectory() || rootStat.isSymbolicLink() || !skillsStat.isDirectory() || skillsStat.isSymbolicLink() || path.dirname(skillsRealPath) !== rootRealPath || path.basename(skillsRealPath) !== "skills") throw new Error("Refusing to use a symbolic-link or escaped skill root for native removal.");',
  "\t\ttarget.skillsRealPath = skillsRealPath; target.skillsDev = skillsStat.dev; target.skillsIno = skillsStat.ino;",
  "\t}",
  "\t{",
  "\t\tconst recoveries = [];",
  "\t\tfor (const target of supportedTargets) {",
  "\t\t\tif (!target.skillsRealPath) continue;",
  "\t\t\tlet entries;",
  ...
  "\t\t\t\tconst quarantineDir = path.dirname(candidate);",
  "\t\t\t\tlet candidateStat;",
  '\t\t\t\ttry { candidateStat = await fs.lstat(candidate); } catch (error) { if (!(error && typeof error === "object" && error.code === "ENOENT")) throw error; try { await fs.rmdir(quarantineDir); } catch (cleanupError) { if (!(cleanupError && typeof cleanupError === "object" && cleanupError.code === "ENOENT")) throw new Error(`Interrupted skill removal directory requires inspection: ${quarantineDir}: ${String(cleanupError)}`); } continue; }',
  "\t\t\t\tif (!candidateStat.isDirectory() || candidateStat.isSymbolicLink()) throw new Error(`Interrupted skill removal target requires inspection: ${candidate}`);",
  "\t\t\t\trecoveries.push({ ...target, candidate, quarantineDir });",
  "\t\t\t}",
  "\t\t}",
  ...
  "\tconst target = supportedTargets.find((entry) => entry.skillsRealPath && path.resolve(path.dirname(filePath)) === path.resolve(entry.targetDir));",
  "\tif (!target) {",
  '\t\treturn { status: "unmanaged", slug, source: selected.source, filePath };',
  "\t}",
  "\tconst { skillsDir, skillsRealPath, targetDir } = target;",
  "\tlet targetStat;",
  "\tlet currentSkillsStat;",
  "\tlet currentSkillsRealPath;",
  "\tlet targetRealPath;",
  "\ttry {",
  "\t\ttargetStat = await fs.lstat(targetDir);",
  "\t\tcurrentSkillsStat = await fs.lstat(skillsDir);",
  "\t\tcurrentSkillsRealPath = await fs.realpath(skillsDir);",
  ...
  "\t\tthrow error;",
  "\t}",
  "\tif (!targetStat.isDirectory() || targetStat.isSymbolicLink() || !currentSkillsStat.isDirectory() || currentSkillsStat.isSymbolicLink() || currentSkillsStat.dev !== target.skillsDev || currentSkillsStat.ino !== target.skillsIno || currentSkillsRealPath !== skillsRealPath || path.dirname(targetRealPath) !== skillsRealPath || path.basename(targetRealPath) !== slug) {",
  ...
  "\t\t}",
  "\t} finally {",
  '\t\ttry { await fs.rmdir(quarantineDir); } catch (error) { if (!(error && typeof error === "object" && error.code === "ENOENT")) throw new Error(`Skill removal left a quarantine directory that requires inspection: ${quarantineDir}: ${String(error)}`); }',
  "\t}",
  "\tawait untrackClawHubSkill(report.workspaceDir, slug);",
