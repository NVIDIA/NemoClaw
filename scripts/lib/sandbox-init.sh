...
# ── Capability dropping ──────────────────────────────────────────
# OpenShell full enforcement clears the agent child's bounding set before
# launch. Verify the kernel state first so managed launches need no capsh
# re-execution or fallback warning. Do not infer enforcement from runtime env.
#
