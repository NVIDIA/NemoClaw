...
if [ "$(id -u)" -ne 0 ]; then
  ...
  prepare_start_log /tmp/gateway.log "" 644

  # Separate log for auto-pair in non-root mode as well.
  prepare_start_log /tmp/auto-pair.log "" 600
...
prepare_start_log /tmp/gateway.log gateway:gateway 644

# Separate log for auto-pair so sandbox user can write to it
prepare_start_log /tmp/auto-pair.log sandbox:sandbox 600
