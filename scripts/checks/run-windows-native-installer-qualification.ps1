...
function ConvertTo-AlternateWindowsPathCasing {
    param([Parameter(Mandatory)][string]$Path)

    if ($Path -cnotmatch '^[A-Za-z]:\\') {
        Fail-Qualification 'The path-casing regression fixture requires a drive-qualified Windows path.'
    }
    $driveLetter = $Path.Substring(0, 1)
    $alternateDriveLetter = if ($driveLetter -ceq $driveLetter.ToUpperInvariant()) {
        $driveLetter.ToLowerInvariant()
    } else {
        $driveLetter.ToUpperInvariant()
    }
    return $alternateDriveLetter + $Path.Substring(1)
}
...
try {
    ...
    Write-JsonFile -Path $recoveryAuthorityPath -Value ([pscustomobject]@{
        receiptVersion = 1
        classification = 'qualification-only'
        installRoot = ConvertTo-AlternateWindowsPathCasing -Path $installRoot
        openshell = [pscustomobject]@{
            repository = $script:CanonicalOpenShellRepository
            pullRequest = $script:TrustedOpenShellPullRequest
            revision = $script:TrustedOpenShellRevision
        ...
        versionRoot = ConvertTo-AlternateWindowsPathCasing -Path $repairedVersionRoot
        backupRoot = ConvertTo-AlternateWindowsPathCasing -Path $recoveryBackupRoot
        failedReplacementRoot = ConvertTo-AlternateWindowsPathCasing -Path $recoveryReplacementRoot
    ...
    $caseVariantReceipt = Get-Content -LiteralPath $installReceiptPath -Raw | ConvertFrom-Json
    $caseVariantReceipt.installRoot = ConvertTo-AlternateWindowsPathCasing -Path $caseVariantReceipt.installRoot
    $caseVariantReceipt.versionRoot = ConvertTo-AlternateWindowsPathCasing -Path $caseVariantReceipt.versionRoot
    Write-JsonFile -Path $installReceiptPath -Value $caseVariantReceipt
