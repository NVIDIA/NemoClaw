...
for ((attempt = 1; attempt <= max_attempts; attempt += 1)); do
  ...
  if [ "$attempt" -eq "$max_attempts" ]; then
    echo "::error::GHCR anonymous exact-digest pull outcome=exhausted attempt=$attempt/$max_attempts failure=anonymous-unavailable" >&2
    exit "$status"
  fi

  delay="${retry_delays[$((attempt - 1))]}"
  echo "::warning::GHCR anonymous exact-digest pull outcome=transient-external attempt=$attempt/$max_attempts failure=anonymous-unavailable retry-in=${delay}s" >&2
  sleep "$delay"
done
