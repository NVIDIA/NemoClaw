...
import { encodeManagedStartupProfile, MANAGED_STARTUP_PROFILE_SCHEMA_VERSION, } from "../../src/lib/onboard/managed-startup/profile.ts";
const AGENTS = new Set(["openclaw", "hermes", "langchain-deepagents-code"]);
...
export const MANAGED_STARTUP_E2E_NO_PROXY = ["localhost", "127.0.0.1", ".example.test"];
...
-----END CERTIFICATE-----
`;
export function managedStartupE2eProfile(agent, changed = false, withCorporateCa = false, withoutHostProxy = false) {
...
function main(argv) {
    if (argv.length === 1 && argv[0] === "--corporate-ca-b64") {
        process.stdout.write(`${Buffer.from(MANAGED_STARTUP_E2E_CORPORATE_CA_PEM, "utf8").toString("base64")}\n`);
        return;
    }
    const agentIndex = argv.indexOf("--agent");
    if (agentIndex < 0)
        throw new Error("--agent is required");
    const unexpected = argv.filter((value, index) => index !== agentIndex &&
        index !== agentIndex + 1 &&
        value !== "--changed" &&
        value !== "--corporate-ca" &&
    ...
    process.stdout.write(`${encodeManagedStartupProfile(managedStartupE2eProfile(agent, argv.includes("--changed"), argv.includes("--corporate-ca"), argv.includes("--without-host-proxy")))}\n`);
}
...
//# sourceMappingURL=generate-managed-startup-profile-fixture.mjs.map