...
const TRUSTED_OPENSHELL_RELEASES: readonly OpenShellReleaseTrust[] = [
  ...
  },
  {
    ...
    // template after installer tests moved under test/install. The fifth authorizes a future
    // installer template that selects the MUSL sandbox archive published by the development
    // channel while preserving the stable GNU release pin; the checked-in installer still uses
    // the fourth template until the dependent behavior change lands.
