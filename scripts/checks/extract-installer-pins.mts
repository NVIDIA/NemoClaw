...
const TRUSTED_OPENSHELL_RELEASES: readonly OpenShellReleaseTrust[] = [
  ...
  },
  {
    ...
    // The first template came from the downstream 0.0.106 pin. The strings-preflight digest
    // authorizes fail-before-download validation for the next stack layer. The repair digest
    // authorizes reuse when an existing formula has an invalid checksum or its release formula is
    // unavailable. Homebrew owns that source state, so NemoClaw cannot correct it there; the
    // installer verifies the trusted release formula before reuse.
    // installer-homebrew-formula-reuse-trust.test.ts and installer-hash-check.test.ts lock the
    // repair template and trust transition. Remove the repair digest when supported Homebrew
    // installs no longer need this path.
    installerTemplateSha256: [
