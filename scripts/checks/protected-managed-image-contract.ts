...
export type ProtectedManagedImageAgent = (typeof PROTECTED_MANAGED_IMAGE_AGENTS)[number];
...
export type ProtectedManagedImageContract = {
  readonly agent: ProtectedManagedImageAgent;
...
export type ProtectedManagedImageActivation = {
  readonly agents: readonly ProtectedManagedImageAgent[];
...
function parseEntry(
  ...
  if (
    ...
    !PROTECTED_MANAGED_IMAGE_AGENTS.includes(entry.agent as ProtectedManagedImageAgent)
  ) {
  ...
  return {
    agent: entry.agent as ProtectedManagedImageAgent,
...
export function parseProtectedManagedImageEvidence(
  ...
  const seenAgents = new Set<ProtectedManagedImageAgent>();
  const directRuns = evidence.directRuns.map((value): ProtectedManagedImageDirectRun => {
    ...
    const contract =
      typeof directRun.agent === "string"
        ? contractByAgent.get(directRun.agent as ProtectedManagedImageAgent)
        : undefined;
    if (
      !contract ||
