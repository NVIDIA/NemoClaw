...
usage() {
  echo "usage: $0 --output <json> --revision <sha> --cohort <id>" >&2
  exit 2
}
...
build_agent() {
  ...
  local alias_raw="$work_dir/${agent}-base-alias.raw"
  ...
  [[ "$base_digest" =~ ^sha256:[a-f0-9]{64}$ ]] || {
    echo "ERROR: ${agent} base did not resolve to one linux/amd64 digest" >&2
    exit 1
  }
  local base_reference="${base_repository}@${base_digest}"
  ...
  jq -e \
    ...
    --arg revision "$revision" '
      ...
      .[0].Config.Labels["io.nvidia.nemoclaw.managed-image.platform"] == "linux/amd64" and
