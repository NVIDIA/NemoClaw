...
const MANAGED_AGENTS = new Set<ProtectedManagedImageAgent>([
  "openclaw",
  "hermes",
...
const MANAGED_AGENT_BASE_POLICIES: Record<ProtectedManagedImageAgent, readonly string[]> = {
...
export function parseManagedImageOpenShellE2eInputs(argv: readonly string[]): Inputs {
  ...
  if (!MANAGED_AGENTS.has(agentValue as ProtectedManagedImageAgent)) {
  ...
  return {
    agent: agentValue as ProtectedManagedImageAgent,
    image,
    sandbox,
...
export function managedImageOpenShellBasePolicyPath(agent: ProtectedManagedImageAgent): string {
  return path.resolve(
...
function managedConfigPath(agent: ProtectedManagedImageAgent): string {
  switch (agent) {
    case "openclaw":
