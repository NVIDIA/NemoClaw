...
export const INJECTED_DIAGNOSTIC_HELPER = [
  ...
  "const NEMOCLAW_MTD_SECRET_RE = /\\b(?:authorization|proxy-authorization|cookie|set-cookie|api[-_]?key|token|secret|password)\\b[\"']?\\s*[:=]?\\s*[\"']?[^\\s\"']+/gi;",
  "const NEMOCLAW_MTD_BEARER_RE = /\\bBearer\\s+\\S+/gi;",
  "const NEMOCLAW_MTD_PRINTABLE_RE = /^[\\x20-\\x7e]*$/;",
  "function nemoClawMtdRedact(value) {",
  '\tif (typeof value !== "string" || value.length === 0) return undefined;',
  '\treturn value.replace(NEMOCLAW_MTD_SESSION_RE, "$1 <REDACTED>").replace(NEMOCLAW_MTD_BEARER_RE, "Bearer <REDACTED>").replace(NEMOCLAW_MTD_SECRET_RE, (match) => match.split(/[:=\\s]/)[0] + " <REDACTED>").slice(0, 240);',
  "}",
  ...
  "\treturn chain;",
  "}",
  "function nemoClawMtdPhase(chain, status, streamStarted) {",
  ...
  '\tif (status !== undefined) return "response_headers";',
  '\treturn "request";',
  "}",
  ...
  "\t\treturn nemoClawMtdRedact(text.slice(0, 2048));",
  "\t} catch {",
  "\t\treturn undefined;",
  "\t}",
  "}",
  ...
  "}",
  "function nemoClawManagedTransportFetch(inner, serverName, serverUrl) {",
  ...
  '\t\t\t\troute: proxy ? "trusted_env_proxy" : "direct",',
  ...
  '\t\t\t\terror_body: await nemoClawMtdErrorBody(response, headers["content-type"])',
  "\t\t\t});",
  "\t\t\treturn response;",
  "\t\t} catch (error) {",
  ...
  "\t\t\t\tproxy: proxy ? nemoClawMtdEndpoint(proxy) : undefined,",
  "\t\t\t\ttarget: nemoClawMtdEndpoint(serverUrl),",
  "\t\t\t\tphase: nemoClawMtdPhase(chain, undefined, false),",
  "\t\t\t\telapsed_ms: Date.now() - startedAt,",
