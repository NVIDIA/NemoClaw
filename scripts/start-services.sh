...
#   DISCORD_BOT_TOKEN=...  ./scripts/start-services.sh         # start all
...
do_start() {
  ...
  if [ -z "${TELEGRAM_BOT_TOKEN:-}" ] && [ -z "${DISCORD_BOT_TOKEN:-}" ]; then
    warn "Neither TELEGRAM_BOT_TOKEN nor DISCORD_BOT_TOKEN is set — no bridge will start."
    warn "Set at least one token to enable messaging."
  fi
