...
const STATE_TRANSACTION_HELPER = [
  ...
  '\treturn Boolean(operator && operator.role === "operator" && Array.isArray(operator.scopes) && operator.scopes.length > 0 && operator.scopes.every((scope) => typeof scope === "string" && scope));',
  "}",
  ...
  "\t\t!nemoclawStatesEqual(pairedTokenAfter.scopes, authTokenAfter.scopes)",
  ...
  '\tif (!identity || identity.deviceId !== deviceId || !beforePairedToken || !beforeAuthToken || !afterPairedToken || identity.deviceToken !== beforePairedToken.token || identity.deviceToken !== beforeAuthToken.token) throw new Error("stored device auth changed before NemoClaw self-approval publication");',
