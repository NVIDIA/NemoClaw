...
function findPatchedFetchGuard(distDirectory) {
...
function isSsrfBlock(error) {
  return /blocked|private|loopback|special-use/i.test(String(error));
}

async function assertTargetBlocked(fetchWithSsrFGuard, label, url) {
  let fetchCalls = 0;
  let lookupCalls = 0;
...
async function verifyGoogleCertProxyPath(fetchWithSsrFGuard) {
  let fetchCalls = 0;
  let lookupCalls = 0;
  let dispatcherName = "";
...
async function verifyRedirectToPrivateIsBlocked(fetchWithSsrFGuard) {
  let fetchCalls = 0;
  let lookupCalls = 0;
  await assert.rejects(
    () =>
...
export async function verifyOpenClawFetchGuardRuntime(distDirectory) {
  ...
  const module = await import(pathToFileURL(fetchGuardFile).href);
  ...
  const environment = new Map(PROXY_ENV_KEYS.map((key) => [key, process.env[key]]));
  ...
  try {
    const dispatcherName = await verifyGoogleCertProxyPath(fetchWithSsrFGuard);
    for (const [label, url] of BLOCKED_TARGETS) {
      await assertTargetBlocked(fetchWithSsrFGuard, label, url);
    }
    await verifyRedirectToPrivateIsBlocked(fetchWithSsrFGuard);
    return {
