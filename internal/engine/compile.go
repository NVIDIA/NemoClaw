...
func Targets(d config.Document, generations map[string]string) []Target {
	...
	providerType := "" // Empty preserves the original OpenAI state format.
	...
	return []Target{
		...
		{"provider", "nemoclaw_provider.inference", oshell.Row{"workspace": w, "name": p.Name, "owner": owner, "generation": generations["provider"], "endpoint": d.InferenceEndpoint(), "credential_env": credential, "provider_type": providerType}},
