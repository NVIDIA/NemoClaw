...
For the current OpenShell 0.0.106 pin, the same scenario verifies the exact release-source blobs for the Docker, Podman, and VM drivers and requires each driver to remove `OPENSHELL_GATEWAY_TLS_SERVER_NAME` after merging user environment.
It also verifies the exact upstream regression blobs that inject a hostile server-name value through each driver's sandbox specification and require the value to be absent from the resulting supervisor or guest environment.
...
It uses host networking and read-only Docker socket access, so hosts with glibc 2.39 or newer remain preferred for the current OpenShell 0.0.106 pin.
