...
Regression tests: the integrity-pin base and plugin-install suites, `test/security/fetch-guard-patch-regression.test.ts`, and `test/messaging-build-applier.test.ts` exercise script suppression and the fixed postinstall command at the execution boundaries.
