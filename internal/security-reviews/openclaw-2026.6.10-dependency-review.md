...
Regression tests: the integrity-pin base and plugin-install suites, `test/fetch-guard-patch-regression.test.ts`, and `test/runtime/messaging/messaging-build-applier.test.ts` exercise script suppression and the fixed postinstall command at the execution boundaries.
