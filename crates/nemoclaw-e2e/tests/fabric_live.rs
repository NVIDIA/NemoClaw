...
async fn fabric_native_access_and_reconciliation_preserve_the_hosted_runtime() {
    ...
    if document.agent_harness(agent).unwrap().kind == "openclaw" {
        exec(
            &client,
            &binding,
            [
    ...
    let response = if document.agent_harness(agent).unwrap().kind == "openclaw" {
        let setting = exec(
            &client,
            &binding,
        ...
        exec(&client, &binding, ["/opt/fabric/bin/python", "-c", "import sys,asyncio,json; sys.path.insert(0,'/opt/nemoclaw'); from fabric import configuration; from nemo_fabric import Fabric,FabricConfig; c=configuration(sys.argv[1]) if sys.argv[2]=='deepagents' else configuration(sys.argv[1],sys.argv[2]); c['runtime']['artifacts']='/sandbox/sdk-smoke'; print(json.dumps(asyncio.run(Fabric().run(FabricConfig.model_validate(c),input='Reply with exactly the word FOUR.',base_dir='/sandbox')).to_mapping()))", &agent.name, &document.agent_harness(agent).unwrap().kind].map(String::from).to_vec()).await
    };
    fs::write(directory.join("native-response.json"), &response).unwrap();
    assert!(
        confirmed_reply(&document.agent_harness(agent).unwrap().kind, &response),
    ...
    save(
        "proof.json",
        &json!({"passed":true,"deployment":document.metadata.uid,"harness":document.agent_harness(agent).unwrap().kind,"resourceBindings":before,"managedRuntimeBindings":managed_before,"runtimeId":hosted,"unchangedApply":true,"exportReapply":true,"nativeResponse":true,"hostedRuntimePreserved":true,"destroyed":true}),
    );
}
