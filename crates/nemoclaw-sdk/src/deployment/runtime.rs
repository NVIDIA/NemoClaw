...
impl Deployment {
    pub(super) async fn runtime_stage(
        ...
        let mut checked = tokio::select! {()=cancel.cancelled()=>return Err(Error::Cancelled),result=preflight(&self.engines,document,&record.generations,&targets,&bindings)=>result?};
