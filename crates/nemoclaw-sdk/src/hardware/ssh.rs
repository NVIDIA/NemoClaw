...
mod tests {
    ...
    use crate::services::{
        ServiceDefinition,
        installers::vllm::{
            HardwareProfile, ServiceHardware,
            hardware_capacity::{check_capacity, serving_memory},
        },
    };
    #[test]
    fn unified_memory_requires_observed_compute_capability_independently_of_vram() {
        ...
        check_capacity(service, &observation.capacity, true, 0, 0).unwrap();
        assert!(observation.capacity.gpu_memory.is_none());
        assert_eq!(
            serving_memory(service, &observation.capacity).unwrap(),
        ...
        let observation = decode(&serde_json::to_vec(&value).unwrap()).unwrap();
        assert!(check_capacity(service, &observation.capacity, true, 0, 0).is_err());
    ...
    fn arm64_blackwell_requires_observed_hbm_instead_of_host_ram() {
        ...
        .unwrap();
        let ServiceDefinition::Vllm(service) = doc.spec.services.get_mut("qwen").unwrap() else {
            panic!("expected vllm")
        };
        service.hardware = Some(ServiceHardware::Profile {
            profile: HardwareProfile::Gb300,
            architecture: None,
            min_gpu_memory_bytes: None,
        });
        assert!(check_capacity(service, &capacity, true, 0, 0).is_err());
        assert!(serving_memory(service, &capacity).is_err());
        for missing in [
