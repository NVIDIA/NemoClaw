...
- **Partial `/proc` visibility on Linux.** When `/proc/net/tcp` is readable but `/proc/net/tcp6` is not, the probe classifies IPv4 listeners only and does not fall back to `lsof`, so a non-loopback IPv6-only listener can pass. The `lsof` fallback runs only when `/proc/net/tcp` itself is unreadable.
...
- **Connect-time recovery restart.** `ensureOllamaAuthProxy` respawns the proxy from persisted state and does not read the status file, so a bind refusal during recovery reports a generic restart failure. The backend-specific remediation renders on the onboarding start path.
...
**Enforced by:** `test/ollama-auth-proxy-bind-probe.test.ts` covers the `/proc` and `lsof` loopback classifiers (accepts full 127.0.0.0/8 including IPv4-mapped IPv6, refuses wildcard and LAN-scope, refuses the lsof `*` token) and pins `EXIT_BACKEND_NOT_LOOPBACK = 2`; the host CLI acts on the structured status reason, which the following tests cover. `src/lib/inference/ollama/proxy-status.test.ts` covers the backend-specific remediation rendering: `OLLAMA_HOST` guidance only when no compatible endpoint was selected, and endpoint-and-port guidance otherwise, including an endpoint on the Ollama port and an IPv6 loopback endpoint. `test/ollama-proxy-startup.test.ts` covers the real startup path (port conflict, spawn failure, slow bind, IPv6-only listener, reclaim of a prior NemoClaw proxy) and proves `noAuthProxy()` keeps the refusal and the endpoint remediation for a compatible endpoint, including one on the Ollama port.
