...
- **Compensating controls:**
  ...
  3. The file is owned by the sandbox user (not root) and atomically replaced. Focused coverage in [`test/langchain-deepagents-code-proxy-runtime-contract.test.ts`](test/langchain-deepagents-code-proxy-runtime-contract.test.ts) and the live login-shell contract in [`test/e2e/e2e-cloud-experimental/checks/07-deepagents-code-headless-inference.sh`](test/e2e/e2e-cloud-experimental/checks/07-deepagents-code-headless-inference.sh) require a regular, non-symlink file with `sandbox-uid:444` metadata.
