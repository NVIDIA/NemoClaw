...
- **Compensating controls:**
  1. The file is credential-free by construction. `prepare_runtime_env` writes normalized proxy config, inherited trust-store paths, and the non-secret `LANGSMITH_TRACING` toggle. It does not persist LangSmith project variables or API keys.
  2. A regression test in [`test/langchain-deepagents-code-image.test.ts`](test/langchain-deepagents-code-image.test.ts) injects token-shaped values through both LangSmith project variables, scans the emitted env file against canonical token shapes, and fails CI if any secret-shaped value is present.
