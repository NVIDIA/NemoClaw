...
The recipe provides three sandboxed agent harness options: **OpenClaw** (default), **Hermes**, or **Deep Agents Code**; see [`AGENT-SELECTION.md`](AGENT-SELECTION.md). It also provides three GPU inference runtime options: **Ollama** (default), **vLLM**, or **NVIDIA NIM**, selected with `inference.runtime`; see [Inference runtimes](#inference-runtimes). All nine agent × runtime pairings render through the same 1 GPU → 1 pod → local OpenAI-compatible `/v1` server pattern, so metrics-proxy, HPA, and Envoy are unaffected by either choice. Not every pairing is equally documented or exercised upstream; see [Agent and runtime support](#agent-and-runtime-support) before selecting a combination.
