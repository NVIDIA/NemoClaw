...
# Defaults avoid overload → 502/503 → 0% GPU → retry spikes (see README-gpu.md).
...
if [[ "${SCALE_UP_OK}" -eq 1 ]]; then
  hpa_common_log "Load test complete: scaled to ${SCALE_UP_TARGET}/${TARGET_PODS} GPU replicas"
else
