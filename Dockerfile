...
# Base64-encoded JSON list of Slack channel IDs to allowlist
# (e.g. ["C012AB3CD","C987ZY6XW"]). When set, only messages from these channels
# are accepted; groupPolicy stays "allowlist". Default: empty list (block all).
...
ENV NEMOCLAW_MODEL=${NEMOCLAW_MODEL} \
    ...
    NEMOCLAW_SLACK_ALLOWED_CHANNELS_B64=${NEMOCLAW_SLACK_ALLOWED_CHANNELS_B64} \
...
_slack_allowed_channels = json.loads(base64.b64decode(os.environ.get('NEMOCLAW_SLACK_ALLOWED_CHANNELS_B64', 'W10=') or 'W10=').decode('utf-8')); \
...
_ch_cfg['slack']['accounts']['default'].update({'appToken': 'openshell:resolve:env:SLACK_APP_TOKEN'}) if 'slack' in _ch_cfg else None; \
_ch_cfg['discord'].update({'groupPolicy': 'allowlist', 'guilds': _discord_guilds}) if 'discord' in _ch_cfg and _discord_guilds else None; \
_ch_cfg['slack']['accounts']['default'].update({'channels': {ch_id: {'enabled': True} for ch_id in _slack_allowed_channels}}) if 'slack' in _ch_cfg and _slack_allowed_channels else None; \
