...
# Materialize every lock-selected archive into an immutable seed. A protected
# build may inject a previously exported, exact-lock seed; an ordinary cold
# build downloads the graph once. The helper verifies registry origin, target
# platform, size, and SHA-512 for every archive before the cache stage consumes
# the result with networking disabled.
FROM node:22-trixie-slim@sha256:db8a96a63e5264607ada2d206758876ebbed6a12be2ada7517793cbfb0c2a29c AS openclaw-managed-messaging-npm-archives
ARG TARGETARCH
COPY agents/openclaw/managed-image-messaging-runtime/package-lock.json /opt/managed-image-messaging-runtime/package-lock.json
COPY agents/openclaw/managed-image-messaging-runtime/npm-cache-seed/ /opt/imported-messaging-npm-cache-seed/
COPY scripts/checks/materialize-locked-npm-cache-seed.mts /opt/nemoclaw-build-tools/checks/materialize-locked-npm-cache-seed.mts
...
# group, so use gateway-owned, group-readable modes only when that split-user
# runtime marker is present. Same-UID OpenShell sandboxes retain OpenClaw's
# private modes. The patch leaves generic credential and identity store
...
# The gateway-state directory is owned by the gateway identity and only
# group-readable, so the sandbox can observe pairing convergence without
# modifying the gateway's authoritative authentication database.
