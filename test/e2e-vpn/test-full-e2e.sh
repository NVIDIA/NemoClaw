...
# VPN NVIDIA inference. Runs install.sh --non-interactive which handles
...
#   NVIDIA_API_KEY                         — required for VPN NVIDIA inference inference
#
# Usage:
#   NEMOCLAW_NON_INTERACTIVE=1 NEMOCLAW_ACCEPT_THIRD_PARTY_SOFTWARE=1 NVIDIA_API_KEY=nvapi-... bash test/e2e-vpn/test-full-e2e.sh
#
...
# shellcheck source=test/e2e-vpn/lib/openclaw-json.sh
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib/openclaw-json.sh"
# shellcheck source=test/e2e-vpn/lib/ci-compatible-inference.sh
...
# shellcheck source=test/e2e-vpn/lib/sandbox-teardown.sh
...
if inf_check=$(openshell inference get 2>&1); then
  ...
  elif grep -qi "compatible-endpoint" <<<"$inf_check_plain"; then
    pass "Inference configured via onboard"
  else
    fail "Inference not configured — onboard did not set up compatible-endpoint provider"
  fi
else
...
# ── Test 4a: Direct VPN NVIDIA inference ──
...
if $pong_ok; then
  pass "[ROUTING] inference.local: OpenShell routed curl to VPN NVIDIA inference and returned PONG"
  info "Routing path proven: sandbox curl → DNS forwarder → gateway proxy → VPN NVIDIA inference (does not exercise openclaw HTTP client; see Phase 4c)"
else
...
if [ "${NEMOCLAW_E2E_SECURITY_POSTURE:-}" = "1" ]; then
  # shellcheck source=test/e2e-vpn/lib/security-posture-assertions.sh
