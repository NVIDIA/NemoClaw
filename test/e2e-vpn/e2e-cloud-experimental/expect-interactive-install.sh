...
#
# Thin wrapper: real logic lives in test/e2e-vpn/test-e2e-cloud-experimental.sh (Phase 3 expect branch).
#
...
#   NVIDIA_API_KEY=nvapi-... bash test/e2e-vpn/e2e-cloud-experimental/expect-interactive-install.sh
#
# Offline expect-only smoke:
#   DEMO_FAKE_ONLY=1 bash test/e2e-vpn/e2e-cloud-experimental/expect-interactive-install.sh
#
...
if [[ "${DEMO_FAKE_ONLY:-0}" == "1" ]]; then
  exec bash test/e2e-vpn/test-e2e-cloud-experimental.sh
fi
