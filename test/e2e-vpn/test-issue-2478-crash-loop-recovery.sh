...
#   NVIDIA_API_KEY                         — legacy fallback for NVIDIA_API_KEY
#
# Usage:
#   NEMOCLAW_NON_INTERACTIVE=1 \
#   NEMOCLAW_ACCEPT_THIRD_PARTY_SOFTWARE=1 \
#     bash test/e2e-vpn/test-issue-2478-crash-loop-recovery.sh
...
# shellcheck source=test/e2e-vpn/lib/openai-compatible-api-proof.sh
...
gateway_serves_inference() {
  ...
  while [ "$elapsed" -lt "$timeout" ]; do
    ...
    # of provider-specific shapes (VPN NVIDIA inference vs. local Ollama).
    case "$out" in
...
if [ "$USE_COMPAT_MOCK" = "1" ]; then
  ...
  if [ -z "${NVIDIA_API_KEY:-}" ] && [ -n "${NVIDIA_API_KEY:-}" ]; then
