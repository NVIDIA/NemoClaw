...
#     NVIDIA_API_KEY=nvapi-... bash test/e2e-vpn/test-messaging-providers.sh
#
...
# shellcheck source=test/e2e-vpn/lib/discord-rest-policy-proof.sh
...
# shellcheck source=test/e2e-vpn/lib/telegram-api-proof.sh
...
if [ -z "${NVIDIA_API_KEY:-}" ] && [ -n "${NVIDIA_API_KEY:-}" ]; then
  ...
  info "Using legacy NVIDIA_API_KEY as fallback for NVIDIA_API_KEY"
fi
...
# hard-gated, version-pinned size proof is intentionally not part of this
# VPN-only shell tree.
