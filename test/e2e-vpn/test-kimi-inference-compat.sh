...
#
# Live path:
#   - uses the public VPN NVIDIA inference provider with moonshotai/kimi-k2.6
...
#   NVIDIA_API_KEY                   - public VPN NVIDIA inference key (nvapi-*)
...
#     bash test/e2e-vpn/test-kimi-inference-compat.sh
...
ensure_public_nvidia_api_key() {
  if [ -n "${NVIDIA_API_KEY:-}" ] && [[ "${NVIDIA_API_KEY}" == nvapi-* ]]; then
    # NemoClaw's VPN NVIDIA inference provider still reads NVIDIA_API_KEY.
...
run_kimi_onboard() {
  ...
  if use_kimi_mock; then
    ...
    unset NVIDIA_API_KEY NVIDIA_API_KEY OPENAI_API_KEY ANTHROPIC_API_KEY GEMINI_API_KEY
  else
    export NEMOCLAW_PROVIDER=custom
    ...
    if ! ensure_public_nvidia_api_key; then
      fail "K1: NVIDIA_API_KEY must be a public VPN NVIDIA inference nvapi-* key"
      summary
    fi
  fi
  ...
  if [ "$onboard_exit" -eq 0 ]; then
    if use_kimi_mock; then
      ...
      pass "K1: onboard completed for VPN NVIDIA Kimi sandbox"
    fi
  else
...
check_inference_route() {
  ...
  if [ "$rc" -eq 0 ] && echo "$response" | grep -q "$KIMI_MODEL"; then
    if use_kimi_mock; then
      ...
      pass "K3: sandbox inference.local models route reaches VPN NVIDIA Kimi"
    fi
  else
...
check_upstream_observed_agent_traffic() {
  if ! use_kimi_mock; then
    ...
    if [ "$rc" -eq 0 ] && echo "$route" | grep -q "compatible-endpoint" && echo "$route" | grep -q "$KIMI_MODEL"; then
      pass "K6: OpenShell route is VPN NVIDIA Kimi"
    else
      fail "K6: OpenShell route is not VPN NVIDIA Kimi (${route:0:400})"
    fi
    return
  fi
...
if use_kimi_mock; then
  ...
  info "Mode: live public VPN NVIDIA inference via compatible-endpoint"
fi
...
if use_kimi_mock; then
  ...
  pass "K0: public VPN NVIDIA inference key is available for Kimi"
else
  fail "K0: NVIDIA_API_KEY must be a public VPN NVIDIA inference nvapi-* key"
  summary
fi
