...
# shellcheck source=test/e2e-vpn/lib/inference-switch-retry.sh
. "${E2E_DIR}/lib/inference-switch-retry.sh"
# shellcheck source=test/e2e-vpn/lib/anthropic-switch-provider.sh
...
if nemoclaw_e2e_using_compatible_inference; then
  ...
  SWITCH_PROVIDER="${NEMOCLAW_SWITCH_PROVIDER:-compatible-endpoint}"
...
# shellcheck source=test/e2e-vpn/lib/install-path-refresh.sh
