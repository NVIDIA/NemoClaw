...
describe("repo-confined advisor read-only tools", () => {
  ...
  it("reports ordinary read ranges and file size (#9949)", async () => {
    ...
    const canonicalPath = fs.realpathSync(path.join(workspace, "ranges.txt"));

    expect(observations).toEqual([
      { path: canonicalPath, offset: 1, endOffset: 2, fileSize: 14, reachesEnd: false },
      { path: canonicalPath, offset: 3, endOffset: null, fileSize: 14, reachesEnd: true },
    ]);
  });
