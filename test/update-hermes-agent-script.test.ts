...
const CURRENT_INSTALLED_DOCKERFILE = [
  ...
  "COPY src/lib/actions/sandbox/openshell-child-visible-credentials.v0.0.82.json /usr/local/lib/nemoclaw/openshell-child-visible-credentials.v0.0.82.json",
...
`,
  ...
  it("refuses installed copies that predate the transactional MCP boundary", () => {
    ...
    const preMcpDockerfile = CURRENT_INSTALLED_DOCKERFILE.replace(
      /^(?:COPY (?:agents\/hermes\/(?:build-mcp-digest|mcp-config-transaction)\.py|src\/lib\/actions\/sandbox\/openshell-child-visible-credentials\.v0\.0\.82\.json) .*|RUN mcp_digest=.*build-hermes-mcp-digest\.py.*)\n/gm,
      "",
    );
    fs.mkdirSync(path.dirname(installedDockerfile), { recursive: true });
    ...
    try {
      ...
      expect(run.stdout).toContain("marker openshell-child-visible-credentials.v0.0.82.json");
