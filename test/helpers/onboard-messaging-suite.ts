...
export type OnboardMessagingGroup = "configuration" | "lifecycle" | "provider";
...
`;
          ...
          assert.ok(
            discordProvider,
            "expected my-assistant-discord-bridge provider create command",
          );
          ...
          assert.ok(
            telegramProvider,
            "expected my-assistant-telegram-bridge provider create command",
          );
          ...
          assert.ok(
            payload.messagingPlanEnv,
            "expected serialized messaging plan in host process env",
          );
      ...
      it(
        ...
        async () => {
          ...
          try {
            ...
            const onboardPath = JSON.stringify(path.join(repoRoot, "src", "lib", "onboard.ts"));
            const agentDefsPath = JSON.stringify(
...
`;
          ...
          assert.equal(
            Object.values(deniedPayload.providerRevisions).filter((revision) => revision === 2)
              .length,
            0,
          );
          ...
          assert.equal(
            (JSON.stringify([payload, deniedPayload]) + combinedOutput).includes(
...
`;
      ...
      it(
        ...
        async () => {
          const tmpDir = fs.mkdtempSync(
            path.join(os.tmpdir(), "nemoclaw-onboard-reuse-providers-"),
          );
          const fakeBin = path.join(tmpDir, "bin");
...
`;
          ...
          assert.ok(
            payload.commands.every(
              (entry: CommandEntry) => !entry.command.includes("sandbox create"),
            ),
            "should NOT recreate sandbox when providers already exist in gateway",
          );
          assert.ok(
            payload.commands.every(
              (entry: CommandEntry) => !entry.command.includes("sandbox delete"),
            ),
