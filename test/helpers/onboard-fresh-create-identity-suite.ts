...
import { encodeMessagingPlan, makeMessagingPlan } from "./messaging-plan-fixtures";
import { writeOkOpenshell } from "./onboard-openshell-fixture";
import { type CommandEntry, onboardScriptMocksPath } from "./onboard-split-context";
...
const freshCreateIdentityCases = [
  {
...
`;
        ...
        const assertRecoveryTuple = (record: Record<string, unknown>) => {
          ...
          assert.equal(
            ...
            payload.recoveryRegistryEntry.lifecycleGeneration,
          );
        };
        ...
        const assertCancellationRecovery = () => {
          ...
          for (const { messages, mode: reentryMode } of reentryCases) {
            ...
            assert.deepEqual(
              reentryPayload.retainedRecoveryRecords,
              payload.retainedRecoveryRecords,
            );
          }
        };
