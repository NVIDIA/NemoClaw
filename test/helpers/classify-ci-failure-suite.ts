  ...
  describe.skipIf(process.platform !== "linux")("CI failure classifier process", () => {
    if (group === "logs") {
      ...
      test.each([
        ...
        expect(result.stderr).toContain(
          "ask the user to correct authentication, authorization, SSO",
        );
    ...
    if (group === "artifacts") {
      ...
      test("reports bounded malformed result evidence while retaining valid artifact failures", () => {
        ...
        expect(
          value.artifact.malformedResultPaths.every((path: string) => path.length <= 1_000),
        ).toBe(true);
      ...
      test("does not return or use a credential-shaped artifact signal", () => {
        ...
        expect(value.findings).not.toContainEqual(
          expect.objectContaining({ type: "process-signal" }),
        );
        expect(result.stdout).not.toContain(secret);
      });
    }
    ...
    if (group === "process-lifecycle") {
      ...
      test("reports a redacted fixed-root removal command when cancellation cleanup fails", async () => {
        ...
        rmSync(`/tmp/nemoclaw-ci-classifier-${uid}/${generatedName}`, {
          recursive: true,
          force: true,
        });
      });
