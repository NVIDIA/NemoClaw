...
function restoreGnuSandboxTemplate(source: string): string {
  ...
  const restored = source.replace(devMusl, grouped);
  expect(restored, "development MUSL sandbox asset selection").not.toBe(source);
  return restored;
}
...
describe("installer Homebrew formula reuse trust", () => {
  const groupedInstallTemplate = restoreGnuSandboxTemplate(INSTALLER_SOURCE);
  const previousTemplate = restoreFlatInstallTestPaths(groupedInstallTemplate);
  ...
  const templates = [
    ...
    ["grouped install tests", TRUSTED_V00106_TEMPLATE_DIGESTS[3], groupedInstallTemplate],
    ["dev MUSL sandbox", TRUSTED_V00106_TEMPLATE_DIGESTS[4], INSTALLER_SOURCE],
  ] as const;
