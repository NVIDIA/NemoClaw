...
describe("installer Homebrew formula reuse trust", () => {
  const groupedInstallTestsTemplate = restoreHistoricalLinuxSandboxAssets(INSTALLER_SOURCE);
  const previousTemplate = restoreFlatInstallTestPaths(groupedInstallTestsTemplate);
  ...
  const templates = [
    ...
    ["grouped install tests", TRUSTED_V00106_TEMPLATE_DIGESTS[3], groupedInstallTestsTemplate],
