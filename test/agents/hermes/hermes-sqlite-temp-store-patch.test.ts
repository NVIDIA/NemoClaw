...
const databasePragmas = 'apply_database_pragmas(self._conn, db_label="state.db")';
const tempStore = '                self._conn.execute("PRAGMA temp_store=MEMORY")';
const foreignKeys = '                self._conn.execute("PRAGMA foreign_keys=ON")';
const unpatchedConnection = `${databasePragmas}\n${foreignKeys}`;
const legacyTempStoreConnection = `${databasePragmas}\n${tempStore}\n${foreignKeys}`;
...
# How long SessionDB fixture state remains valid.
...
`,
  ...
  it.each([
    ...
    ["partial", moduleSource(`${databasePragmas}\n${tempStore}`)],
