...
function runHermesArchiveLayer(
  ...
  fs.writeFileSync(
    ...
    [
      ...
      '      mkdir -p "$target_root/hermes_cli" "$target_root/tools" "$target_root/plugins/memory" "$target_root/plugins/cron_providers" "$target_root/plugins/platforms/a2a" "$target_root/providers"',
      ...
      '      printf "%s\\n" "nemoclaw_sanitized_installer_env" "_NEMOCLAW_HINDSIGHT_REQUIREMENTS" > "$target_root/tools/lazy_deps.py"',
      ...
      '      printf "%s\\n" "name: a2a" > "$target_root/plugins/platforms/a2a/plugin.yaml"',
      '      printf "%s\\n" "def register(ctx):" "    pass" > "$target_root/plugins/platforms/a2a/__init__.py"',
      "    fi",
      "  fi",
      "}",
