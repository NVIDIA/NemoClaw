...
type CrashBoundary =
  ...
  | "adapter-mismatch"
  | "authority-external"
  | "gateway-authority-refusal"
  | "authority-missing"
  | "attach-race"
  | "race"
  | "late-race"
  | "preupdate-observation-forbidden"
  | "restart-provider-authority-refusal"
  | "";
...
const externalAuthority =
  ...
  crashAfter === "gateway-authority-refusal" ||
...
const replacementProviderId = "22222222-3333-4444-8555-666666666666";
...
gatewayRuntime.recoverNamedGatewayRuntime = async () => {
  if (crashAfter === "gateway-authority-refusal") mark("gateway-selected");
  return {
    recovered: true,
    attempted: false,
...
providerCommands.runOpenshellProviderCommand = (args) => {
  ...
  if (args[0] === "provider" && args[1] === "get") {
    ...
    return marked("provider")
      ? { status: 0, stdout: "Id: " + (marked("foreign-provider") ? foreignProviderId : marked("replacement-provider") ? replacementProviderId : providerId) + "\nType: nemoclaw-mcp-v1\nResource version: " + providerVersion() + "\nCredential keys: FAKE_MCP_SECRET\n", stderr: "" }
  ...
  if (args[0] === "provider" && (args[1] === "create" || args[1] === "update")) {
    ...
    if (args[1] === "create") {
      ...
      if (crashAfter === "restart-provider-authority-refusal") mark("replacement-provider");
    }
    if (args[1] === "update") {
...
const inspectPolicyAuthority = (options) => {
  ...
  if (crashAfter === "gateway-authority-refusal" && marked("gateway-selected")) {
    throw new mcpPolicy.McpPolicyAuthorityRefusalError(
      "OpenShell policy authority changed during gateway selection",
    );
  }
  if (crashAfter === "restart-provider-authority-refusal" && marked("replacement-provider")) {
    throw new mcpPolicy.McpPolicyAuthorityRefusalError(
      "OpenShell policy authority changed after provider recreation",
    );
  }
...
describe("MCP add crash consistency", () => {
  ...
  it("refuses before the first manifest write when authority changes during gateway selection (#9833)", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-mcp-gateway-authority-"));
    try {
      initializeSandboxRegistry(home, "externally-managed", "external-generation");

      const refused = runAddProcess(home, "gateway-authority-refusal");

      expect(refused.status, `${refused.stdout}\n${refused.stderr}`).toBe(2);
      expect(refused.stderr).toContain("authority changed during gateway selection");
      expect(
        JSON.parse(fs.readFileSync(path.join(home, ".nemoclaw", "sandboxes.json"), "utf8"))
          .sandboxes["crash-test"].mcp,
      ).toBeUndefined();
  ...
  it("journals a recreated provider identity before reporting authority drift (#9833)", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-mcp-restart-authority-"));
    try {
      initializeSandboxRegistry(home, "externally-managed", "external-generation");
      const added = runAddProcess(home, "authority-external");
      expect(added.status, `${added.stdout}\n${added.stderr}`).toBe(0);
      fs.rmSync(path.join(home, "provider.marker"));
      fs.rmSync(path.join(home, "attached.marker"));
      fs.rmSync(path.join(home, "adapter.marker"));

      const refused = runAddProcess(home, "restart-provider-authority-refusal", true, "restart");

      expect(refused.status, `${refused.stdout}\n${refused.stderr}`).toBe(2);
      expect(refused.stderr).toContain("authority changed after provider recreation");
      ...
      const recovered = runAddProcess(home, "authority-external", true, "restart");
