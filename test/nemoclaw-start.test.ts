...
describe("nemoclaw-start gateway token export (#1114)", () => {
  ...
  it("reads, exports, prints, and shell-escapes the gateway token without touching rc files", () => {
    ...
    expect(envFile).toContain("_nemoclaw_intended_gateway_token='tok'\\''en'");
  ...
  it("writes the gateway port and URL into the runtime shell env (#3256)", () => {
    ...
    expect(envFile).toContain("_nemoclaw_intended_gateway_token='token'");
  });
  it("writes OpenClaw state env for connect-shell pairing approval (#3730)", () => {
    ...
    expect(envFile.indexOf("export OPENCLAW_STATE_DIR=")).toBeLessThan(
      envFile.indexOf("_nemoclaw_intended_gateway_token='"),
    );
  });
  ...
  it("generates a gateway token before writing the runtime shell env (#3256)", () => {
    ...
    expect(envFile).toContain(
      `_nemoclaw_intended_gateway_token='${configAfter.gateway.auth.token}'`,
    );
...
describe("direct-root entrypoint composition under CAP_DAC_OVERRIDE drop", () => {
  ...
  it("runs the helper chain end-to-end against a simulated root entrypoint", () => {
    ...
    try {
      ...
      expect(proxyEnv).toMatch(/_nemoclaw_intended_gateway_token='[A-Za-z0-9_-]{20,}'/);
      ...
      expect(proxyEnv).toContain(
        `_nemoclaw_intended_gateway_token='${updatedConfig.gateway.auth.token}'`,
      );
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
