...
describe("nemoclaw-start non-root fallback", () => {
  ...
  it("sources the runtime env before explicit one-shot commands (#5324)", () => {
    const src = fs.readFileSync(START_SCRIPT, "utf-8");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-oneshot-env-"));
    try {
      const runtimeEnvFile = path.join(tmpDir, "proxy-env.sh");
      fs.writeFileSync(
        runtimeEnvFile,
        [
          "export NEMOCLAW_OPENCLAW_GATEWAY_URL='ws://10.200.0.2:18789'",
      ...
      const script = [
        ...
        extractShellFunctionFromSource(src, "run_oneshot_command"),
        "normalize_mutable_config_perms() { :; }",
        `_RUNTIME_SHELL_ENV_FILE=${JSON.stringify(runtimeEnvFile)}`,
        "export OPENCLAW_GATEWAY_URL='ws://10.200.0.2:18789'",
        'run_oneshot_command bash -c \'printf "PUBLIC=%s\\nPRIVATE=%s\\nPORT=%s\\n" "${OPENCLAW_GATEWAY_URL-unset}" "${NEMOCLAW_OPENCLAW_GATEWAY_URL-unset}" "${OPENCLAW_GATEWAY_PORT-unset}"\'',
      ].join("\n");

      const result = spawnSync("bash", ["-c", script], { encoding: "utf-8", timeout: 5000 });

      expect(result.status, result.stderr || result.stdout).toBe(0);
      expect(result.stdout).toContain("PUBLIC=unset");
      expect(result.stdout).toContain("PRIVATE=ws://10.200.0.2:18789");
      expect(result.stdout).toContain("PORT=18789");
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
...
describe("nemoclaw-start gateway token export (#1114)", () => {
  ...
  it("writes the gateway port and private gateway URL into the runtime shell env (#3256)", () => {
    const { result, envFile } = runGatewayTokenHarness(
...
describe("nemoclaw-start auto-pair client whitelisting (#117)", () => {
  ...
  it("approves only whitelisted local-state clients and leaves rejected requests pending", () => {
    ...
    try {
      ...
      expect(Object.keys(pending)).toEqual(["reject-me"]);
      expect([...Object.keys(paired)].sort()).toEqual(["browser-1", "webchat-1"]);
      expect(JSON.stringify(paired)).not.toContain("operator.admin");
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
...
describe("nemoclaw-start auto-pair slow-mode keepalive (#4263)", () => {
  ...
  function writePairingState(
    prefix: string,
    pending: Record<string, unknown>,
    paired: Record<string, unknown>,
  ): { tmpDir: string; stateDir: string; devicesDir: string } {
    ...
    return { tmpDir, stateDir, devicesDir };
  }

  it("approves existing CLI write upgrades but rejects first-time CLI requests", () => {
    const { tmpDir, stateDir, devicesDir } = writePairingState(
      "nemoclaw-auto-pair-cli-upgrade-",
      {
        "late-cli": {
          requestId: "late-cli",
          deviceId: "cli-1",
    ...
    try {
      ...
      expect(run.stdout).toContain(
        "[auto-pair] rejected first-time cli pairing client=cli mode=cli",
      );
      const pending = JSON.parse(fs.readFileSync(path.join(devicesDir, "pending.json"), "utf-8"));
      const paired = JSON.parse(fs.readFileSync(path.join(devicesDir, "paired.json"), "utf-8"));
      expect(Object.keys(pending)).toEqual(["first-cli"]);
  ...
  it("removes stale CLI pairing-only devices and clears the matching local token", () => {
    const { tmpDir, stateDir, devicesDir } = writePairingState(
      "nemoclaw-auto-pair-prune-cli-",
      {},
      {
        "cli-1": {
          deviceId: "cli-1",
    ...
    try {
      ...
      expect(run.status).toBe(0);
      expect(run.stdout).toContain("[auto-pair] removed stale cli pairing-only devices=1");
  ...
  it("transitions to slow-mode even while rejected pending requests are sticky", () => {
    const { tmpDir, stateDir } = writePairingState(
      "nemoclaw-auto-pair-sticky-",
      {
        "evil-stuck": {
