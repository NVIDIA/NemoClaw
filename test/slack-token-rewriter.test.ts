...
describe("slack-token-rewriter: request bodies", () => {
  ...
  it("does not strip when request options override a Slack URL to a non-Slack target", () => {
    const mod = loadRewriter();
    const original = "token=xoxb-OPENSHELL-RESOLVE-ENV-SLACK_BOT_TOKEN";
    const expected = "token=openshell:resolve:env:SLACK_BOT_TOKEN";
    const req = mod.http.request(new URL("http://slack.com/api/auth.test"), {
      hostname: "127.0.0.1",
      path: "/probe",
      headers: {
        Authorization: "Bearer xoxb-OPENSHELL-RESOLVE-ENV-SLACK_BOT_TOKEN",
  ...
  it("does not strip based on request option fields that Node ignores", () => {
    const mod = loadRewriter();
    const original = "token=xoxb-OPENSHELL-RESOLVE-ENV-SLACK_BOT_TOKEN";
    const expected = "token=openshell:resolve:env:SLACK_BOT_TOKEN";
    const req = mod.http.request({
      href: "http://slack.com/api/auth.test",
      hostname: "127.0.0.1",
  ...
  it("strips when request options override a non-Slack URL to Slack Web API", () => {
    const mod = loadRewriter();
    const original = "token=xoxb-OPENSHELL-RESOLVE-ENV-SLACK_BOT_TOKEN";
    const req = mod.http.request(new URL("http://127.0.0.1/probe"), {
      hostname: "slack.com",
      path: "/api/auth.test",
      headers: {
        Authorization: "Bearer xoxb-OPENSHELL-RESOLVE-ENV-SLACK_BOT_TOKEN",
  ...
  it("strips redundant auth.test body token with revision-scoped canonical Authorization", () => {
    process.env.SLACK_BOT_TOKEN = "openshell:resolve:env:v12_SLACK_BOT_TOKEN";
    try {
      const mod = loadRewriter();
      const original = "token=xoxb-OPENSHELL-RESOLVE-ENV-SLACK_BOT_TOKEN";
      ...
      expect(req.getHeader("authorization")).toBe(
        "Bearer openshell:resolve:env:v12_SLACK_BOT_TOKEN",
      );
      expect(req.calls[0].args[0]).toBe("");
      expect(req.getHeader("content-length")).toBe("0");
