...
`,
  ...
  it("migrates only allowlisted legacy credentials through mocked gateway registration (#7617)", async () => {
    ...
    const credentialStore =
      require("../src/lib/credentials/store") as typeof import("../src/lib/credentials/store.js");
    try {
      await withProcessEnv(
        {
          HOME: tmpDir,
        ...
        async () => {
          ...
          credentialStore.removeLegacyCredentialsFile();
          assert.equal(
            fs.existsSync(legacyFile),
            false,
