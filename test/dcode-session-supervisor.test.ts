...
describe.runIf(canRun)("managed DCode session supervisor", () => {
  ...
  it("bounds cleanup when the relay closes its PTY without sending a signal (#6720)", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-dcode-stdin-disconnect-"));
    const pidFile = path.join(dir, "processes.pid");
    const session = path.join(dir, "session.py");
    const harness = path.join(dir, "harness.py");
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      ...
      [
        ...
        `supervisor = subprocess.Popen([sys.executable, ${JSON.stringify(supervisor)}, sys.executable, ${JSON.stringify(session)}], stdin=slave_fd)`,
        "os.close(slave_fd)",
        "deadline = time.monotonic() + 5",
        "while not pid_file.exists() and time.monotonic() < deadline:",
        "    time.sleep(0.05)",
