...
# Apply a network policy preset.
# Uses the direct CLI form (`policy-add <preset> --yes`) which works in both
# interactive and non-interactive (NEMOCLAW_NON_INTERACTIVE=1) modes.
apply_preset() {
  local preset_name="$1"
  log "  Applying preset '$preset_name' via policy-add --yes..."
  local exit_code=0
  set +e
