...
SKILL_ROOT="/sandbox/.openclaw/workspace/skills"
...
local_skill_dir="$(mktemp -d)"
cp "$payload_source" "${local_skill_dir}/SKILL.md"
ssh_config="$(mktemp)"
known_hosts="$(mktemp)"
...
set +e
install_out=$(node "$NEMOCLAW_CLI_BIN" "$SANDBOX_NAME" skill install "$local_skill_dir" 2>&1)
install_rc=$?
set -e
[ "$install_rc" -eq 0 ] \
  || die "nemoclaw skill install failed (exit ${install_rc}): ${install_out:0:300}"
...
# supervisor generates an ephemeral SSH host key. Pin that key before reading
# the native workspace result back for verification.
set +e
...
query_out=$(
  $TIMEOUT_CMD ssh -F "$ssh_config" \
    ...
    -o LogLevel=ERROR \
    "$ssh_host" \
    "sh -s -- '$remote_skill_file'" <"$remote_script" 2>&1
)
query_rc=$?
set -e

[ "$query_rc" -eq 0 ] || die "remote query failed (exit ${query_rc}): ${query_out:0:300}"
echo "$query_out" | grep -q "QUERY_PATH=${remote_skill_file}" || die "did not find query path marker"

ok "skill installed natively and queryable at ${remote_skill_file}"
printf '%s\n' "$query_out"
