...
const DOCKER_NETWORK_NAME_PATTERN = /^[a-zA-Z0-9][a-zA-Z0-9_.-]{0,127}$/u;
...
async function inspectOpenShellBridgeAddress(
  ...
  const networkName =
    nonEmpty(env.OPENSHELL_DOCKER_NETWORK_NAME) ??
    nonEmpty(process.env.OPENSHELL_DOCKER_NETWORK_NAME) ??
    DEFAULT_OPENSHELL_DOCKER_NETWORK;
  if (!DOCKER_NETWORK_NAME_PATTERN.test(networkName)) {
  ...
  if (
    ...
    gateways.length !== 1 ||
    !isPrivateIPv4(gateways[0]!)
  ) {
    throw new Error("OpenShell Docker network must expose one private IPv4 bridge gateway");
  }
  return gateways[0]!;
}
...
async function requireFakeApiDockerTopology(
  ...
  const publicationNetworkInspect = await runHost(
    host,
    "docker",
    ["network", "inspect", options.publicationNetwork],
    {
      artifactName: `inspect-fake-${options.kind}-api-publication-network`,
      env: options.env,
      redactionValues: options.redactionValues,
      timeoutMs: 30_000,
    },
  );
  expectExitZero(publicationNetworkInspect, `inspect fake ${options.kind} API publication network`);

  let containers: unknown;
  let networks: unknown;
  let publicationNetworks: unknown;
  try {
    containers = JSON.parse(containerInspect.stdout);
    networks = JSON.parse(networkInspect.stdout);
    publicationNetworks = JSON.parse(publicationNetworkInspect.stdout);
  } catch {
  ...
  const publicationNetworkRecord =
    Array.isArray(publicationNetworks) && publicationNetworks.length === 1
      ? publicationNetworks[0]
      : undefined;
  ...
  const publicationNetworkDriver =
    publicationNetworkRecord !== null &&
    typeof publicationNetworkRecord === "object" &&
    typeof (publicationNetworkRecord as { Driver?: unknown }).Driver === "string"
      ? (publicationNetworkRecord as { Driver: string }).Driver
      : undefined;
  const publicationNetworkInternal =
    publicationNetworkRecord !== null &&
    typeof publicationNetworkRecord === "object" &&
    (publicationNetworkRecord as { Internal?: unknown }).Internal === true;
  const publicationNetworkOptions =
    publicationNetworkRecord !== null &&
    typeof publicationNetworkRecord === "object" &&
    (publicationNetworkRecord as { Options?: unknown }).Options !== null &&
    typeof (publicationNetworkRecord as { Options?: unknown }).Options === "object"
      ? ((publicationNetworkRecord as { Options: Record<string, unknown> }).Options ?? {})
      : {};
  if (
    api === undefined ||
    proxy === undefined ||
    ...
    !networkInternal ||
    publicationNetworkDriver !== "bridge" ||
    publicationNetworkInternal ||
    publicationNetworkOptions["com.docker.network.bridge.enable_ip_masquerade"] !== "false" ||
    JSON.stringify(apiNetworks) !== JSON.stringify([options.network]) ||
    JSON.stringify(proxyNetworks) !==
      JSON.stringify([options.network, options.publicationNetwork].sort()) ||
...
export async function startFakeDockerApi(
  ...
  const proxyCreate = await runHost(
    host,
    "docker",
    [
      "create",
      ...
      "--network",
      publicationNetwork,
      ...proxyPorts.flatMap((port) => ["-p", `${openshellBridgeAddress}:0:${String(port)}`]),
      "--read-only",
      "--cap-drop",
      "ALL",
      "--security-opt",
  ...
  const proxyStart = await runHost(host, "docker", ["start", proxyContainer], {
