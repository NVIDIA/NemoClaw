...
export async function registerRetainedSandboxPolicyRestore(
  ...
  cleanup.add(`restore retained sandbox policy ${options.sandboxName}`, async () => {
    ...
    const verified = await sandbox.openshell(["policy", "get", "--base", options.sandboxName], {
...
export async function startFakeDockerApi(
  ...
  cleanup(`remove ${credentialDir}`, async () => {
    await fs.promises.rm(credentialDir, { recursive: true, force: true });
  });
...
export async function applyCredentialBoundFakePolicy(options: {
  ...
  const binaries = [...new Set(options.binaries ?? ["/usr/local/bin/node", "/usr/bin/node"])];
