...
test(
  ...
  const apiListenerBeforeSecond = hasHermesApi
    ? await host.inspectOpenShellForwardListener(String(apiPortBeforeSecond), SANDBOX_A, {
        artifactName: "phase-2-api-listener-before-second-onboard", env: commandEnv(),
      }) : null;
  ...
  const apiListenerAfterSecond = hasHermesApi
    ? await host.inspectOpenShellForwardListener(
        String(sandboxARegistryAfterSecond?.hermesApiPort), SANDBOX_A, {
          artifactName: "phase-3-api-listener-after-second-onboard", env: commandEnv(),
        }) : null;
  expect(sandboxARegistryAfterSecond?.hermesApiPort).toBe(apiPortBeforeSecond);
  expect(apiListenerBeforeSecond?.valid ?? false, apiListenerBeforeSecond?.output).toBe(hasHermesApi);
  expect(apiListenerAfterSecond?.valid ?? false, apiListenerAfterSecond?.output).toBe(hasHermesApi);
