...
export const HERMES_MCP_OVERSIZE_BODY_MARKER =
  "<response body omitted: exceeds diagnostic file bound>";
...
export function assertHermesMcpHttpResponse(
  ...
  if (status === null) {
    throw new Error("hermes real MCP tool call did not report one bounded HTTP status");
  }
  if (status < 200 || status >= 300) {
    const body = sanitizedPreview(result.stdout, explicitRedactionValues);
    throw new Error(
      `hermes real MCP tool call failed: HTTP ${status}; sanitized response body: ${body}`,
    );
  }
...
export function assertAuthenticatedMcpToolCallOutcome(options: {
  ...
  if (calls.length !== options.callsBefore + 1) {
    throw new Error(
      `real MCP tool call must issue exactly one tools/call request (observed delta=${calls.length - options.callsBefore})`,
    );
  }
  const call = calls.at(-1);
  if (call?.path !== "/mcp") {
    throw new Error("real MCP tool call did not reach the managed /mcp endpoint");
  }
  if (call.auth !== `Bearer ${options.expectedSecret}`) {
    throw new Error("real MCP tool call did not use the expected resolved bearer credential");
  }
  if (call.auth.includes("openshell:resolve:env")) {
    throw new Error("real MCP tool call forwarded an unresolved credential placeholder");
  }
}
