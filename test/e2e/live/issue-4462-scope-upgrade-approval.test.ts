...
if [ -n "\${OPENCLAW_GATEWAY_URL:-}" ]; then
  echo "PUBLIC_GATEWAY_URL_LEAK=\${OPENCLAW_GATEWAY_URL}" >&2
  exit 4
fi
...
if existing is None:
    raise SystemExit('scope approval requires an existing paired operator device')
else:
    ...
    existing_scopes=canonical_scopes(existing, ('scopes','approvedScopes'), 'existing paired')
    expected=existing_scopes | requested
    if not {'operator.read','operator.write'}.intersection(requested) or expected == existing_scopes:
  ...
  async ({ artifacts, cleanup: cleanupRegistry, host, sandbox, secrets, skip }) => {
    ...
    await artifacts.writeJson("target.json", {
      ...
      contracts: [
        ...
        "proxy env exposes the private gateway alias and contains the devices approve guard",
