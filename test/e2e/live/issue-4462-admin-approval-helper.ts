...
export function adminApprovalConnectScript(
  ...
  return [
    ...
    'agent_stdout="$(mktemp)"',
    'agent_stderr="$(mktemp)"',
    'trap \'rm -f -- "$devices_json" "$devices_err" "$approve_output" "$cron_output" "$agent_stdout" "$agent_stderr"\' EXIT',
    ...
    'agent_parser="$(mktemp)"',
    'trap \'rm -f -- "$devices_json" "$devices_err" "$approve_output" "$cron_output" "$agent_stdout" "$agent_stderr" "$agent_parser"\' EXIT',
