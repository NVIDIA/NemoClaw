...
import {
  type AuthenticatedMcpDiscoveryTarget,
...
async function assertAdapterDnsRebindingDenied(
  ...
  const survivingDiscoveryOffset = options.survivingMcp?.server.requests.length;
  ...
  if (options.survivingMcp && survivingDiscoveryOffset !== undefined) {
    await assertAuthenticatedMcpDiscovery(options.survivingMcp.server, {
      requestOffset: survivingDiscoveryOffset,
      expectedSecret: options.survivingMcp.expectedSecret,
      label: options.survivingMcp.label,
    });
  }
}
