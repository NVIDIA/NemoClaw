...
test(
  ...
  async ({ artifacts, cleanup, host, inference, lifecycle, progress, sandbox }) => {
    ...
    const profilesAfterRecovery = await execPiShell(
      sandbox,
      trustedSandboxShellScript(
        "set -eu; /usr/bin/env -u NEMOCLAW_E2E_PI_PROFILE bash -lc 'test \"$NEMOCLAW_E2E_PI_PROFILE\" = preserved'; /usr/bin/env -u NEMOCLAW_E2E_PI_PROFILE bash -ic 'test \"$NEMOCLAW_E2E_PI_PROFILE\" = preserved'; sha256sum /sandbox/.bashrc /sandbox/.profile",
      ),
