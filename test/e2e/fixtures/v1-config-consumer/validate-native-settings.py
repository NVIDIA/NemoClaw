...
def openclaw_evidence(adapter, settings, expected):
    ...
    agent = native["agents"]["entries"][expected["agentName"]]
    defaults = native["agents"]["defaults"]
    source = {
        "contextWindow": model["contextWindow"],
        "maxTokens": model["maxTokens"],
        "reasoning": model["reasoning"],
        "reasoningEffort": agent.get("thinkingDefault", defaults.get("thinkingDefault", "default")),
    ...
    default_agent = native_defaults["agents"]["entries"][expected["agentName"]]
    agent_defaults = native_defaults["agents"]["defaults"]
    target_defaults = {
        "contextWindow": default_model["contextWindow"],
        "maxTokens": default_model["maxTokens"],
        "reasoning": default_model["reasoning"],
        "reasoningEffort": default_agent.get(
            "thinkingDefault", agent_defaults.get("thinkingDefault", "default")
        ),
        "timeoutSeconds": agent_defaults["timeoutSeconds"],
        "heartbeatEvery": agent_defaults.get("heartbeat", {}).get("every"),
