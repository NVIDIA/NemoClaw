...
// CLI cases own separate output files; keep their child-process overlap bounded.
vi.setConfig({ maxConcurrency: 3 });
afterAll(() => rmSync(PLANNER_COMPILE_CACHE, { force: true, recursive: true }));

type PlannerCliResult = { status: number; stderr: string; stdout: string };
...
describe("E2E workflow plan", () => {
  ...
  it.concurrent("maps launchable-smoke to bootstrap-install-smoke when checkout_sha is set", async () => {
    ...
    try {
      const result = await runPlannerCli(["--ci-output"], {
        ...process.env,
        GITHUB_OUTPUT: output,
        GITHUB_STEP_SUMMARY: summary,
        INFERENCE_MODE: "mock",
  ...
  it.concurrent("plans active jobs while checking retired controller selectors (#7616)", async () => {
    const directory = mkdtempSync(path.join(tmpdir(), "nemoclaw-workflow-plan-cli-"));
    const output = path.join(directory, "github-output");
    const summary = path.join(directory, "summary.md");
  ...
  it.concurrent.each(RETIRED_CONTROLLER_SELECTOR_IDS)(
  ...
  it.concurrent("emits an empty matrix for retired free-standing rebuild selectors (#7615)", async () => {
    const directory = mkdtempSync(path.join(tmpdir(), "nemoclaw-workflow-plan-cli-"));
    const output = path.join(directory, "github-output");
    const summary = path.join(directory, "summary.md");
    const plan: ReturnType<typeof buildE2eWorkflowPlan> = {
  ...
  it.concurrent("rejects the retired bootstrap job outside a PR controller checkout", async () => {
    const directory = mkdtempSync(path.join(tmpdir(), "nemoclaw-workflow-plan-cli-"));
    try {
      const result = await runPlannerCli(["--ci-output"], {
        ...process.env,
...
`);
  ...
  it.concurrent("reports CLI failures as workflow annotations", async () => {
    const result = await runPlannerCli([
      "--jobs",
      "hermes-e2e",
      "--targets",
      "definitely-unknown-e2e-target",
    ]);
  ...
  it.concurrent("writes CI outputs from the selector environment through the CLI", async () => {
    const testId = firstId(discoverCredentialFreeTests(), "credential-free test");
    const directory = mkdtempSync(path.join(tmpdir(), "nemoclaw-workflow-plan-cli-"));
    const output = path.join(directory, "github-output");
    const summary = path.join(directory, "summary.md");
  ...
  it.concurrent("writes controller-selected jobs and targets through the CI-output path (#7031)", async () => {
    const directory = mkdtempSync(path.join(tmpdir(), "nemoclaw-workflow-plan-cli-"));
    const output = path.join(directory, "github-output");
    const summary = path.join(directory, "summary.md");
