...
describe("fixture redaction entry point", () => {
  ...
  it.each([
    ...
    Number.POSITIVE_INFINITY,
    Number.MAX_SAFE_INTEGER + 1,
  ])("rejects invalid post-redaction capture limit %s before spawning a child or writing artifacts", async (postRedactionCaptureLimitBytes) => {
      ...
      await expect(
        probe.run(
          trustedShellCommand({
            command: "bash",
            args: ["-lc", 'printf spawned >"$SPAWN_MARKER"'],
            reason: "prove invalid post-redaction limits fail before child execution",
          }),
          {
