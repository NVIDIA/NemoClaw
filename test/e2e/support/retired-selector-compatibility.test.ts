...
describe("retired E2E selector compatibility", () => {
  ...
  it("runs each ordinary replacement project once and emits bound signals (#7616)", () => {
    ...
    try {
      ...
      expect(commands).toEqual([
        "npx vitest run --project cli src/lib/actions/sandbox/rebuild-finalization.test.ts src/lib/actions/sandbox/rebuild-flow-helpers.test.ts src/lib/actions/upgrade-sandboxes-recovery.test.ts src/lib/sandbox/version.test.ts src/lib/security/credential-filter-secret-patterns.test.ts",
  ...
  it("runs and reports both rebuild replacements selected through targets (#7615)", () => {
    ...
    try {
      ...
      expect(commands).toEqual([
        "npx vitest run --project cli src/lib/actions/sandbox/rebuild-finalization.test.ts src/lib/actions/sandbox/rebuild-flow-helpers.test.ts src/lib/actions/upgrade-sandboxes-recovery.test.ts src/lib/sandbox/version.test.ts",
