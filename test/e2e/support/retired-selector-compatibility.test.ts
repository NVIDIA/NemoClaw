  ...
  it("runs each ordinary replacement project once and emits bound signals (#7616)", () => {
    ...
    try {
      ...
      expect(commands).toEqual([
        ...
        "npx vitest run --project integration test/cli/list-share-live-inference.test.ts test/credentials/credential-migration-reconciliation.test.ts test/credentials/credentials.test.ts test/runtime/gateway/gateway-drift-preflight.test.ts test/runtime/gateway/gateway-health-honest.test.ts test/rebuild-stale-recovery.test.ts",
