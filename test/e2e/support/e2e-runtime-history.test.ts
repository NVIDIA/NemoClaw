...
describe("E2E rolling runtime history", () => {
  ...
  it("prioritizes current failures and historically flaky tests over slow stable tests", () => {
    const failed = runtimeRow({
      target: "failed-fast",
      scenario: "failed fast",
      medianMs: 10_000,
      p95Ms: 10_000,
      maxMs: 10_000,
      passedRuns: 0,
    ...
    const stable = runtimeRow({
      target: "stable-slow",
      scenario: "stable slow",
      medianMs: 300_000,
      p95Ms: 300_000,
      maxMs: 300_000,
    });
    const prior = [
      createRuntimeSummary(1, "2026-07-20T00:00:00.000Z", [
        runtimeRow({
          target: "failed-fast",
          scenario: "failed fast",
          medianMs: 10_000,
    ...
    const markdown = formatRuntimeHistory([stable, flaky, failed], prior);

    expect(markdown.indexOf("| failed-fast |")).toBeLessThan(markdown.indexOf("| flaky-fast |"));
    expect(markdown.indexOf("| flaky-fast |")).toBeLessThan(markdown.indexOf("| stable-slow |"));
  });
