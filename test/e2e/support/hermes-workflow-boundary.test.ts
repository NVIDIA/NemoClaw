...
describe("Hermes E2E workflow boundary", () => {
  ...
  it("restores daemon content, mode, UID, GID, and runtime before cleanup (#6110)", () => {
    ...
    try {
      ...
      const result = spawnSync(
        "bash",
        [DOCKER_FIXTURE_PATH, "restore", stateDir, daemonJson, artifactDir],
        {
          cwd: process.cwd(),
          encoding: "utf8",
          env: {
      ...
      expect(restoredStat.uid).toBe(daemonStat.uid);
      expect(restoredStat.gid).toBe(daemonStat.gid);
      expect(fs.readFileSync(sudoLog, "utf8")).toContain(
        `chown ${daemonStat.uid}:${daemonStat.gid} ${daemonJson}`,
      );
      expect(
        fs.readFileSync(path.join(artifactDir, "docker-default-runtime-restored.txt"), "utf8"),
      ).toBe("nvidia\n");
    } finally {
      fs.rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("removes the private snapshot when daemon restoration cannot be proven (#6110)", () => {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(stateDir, "daemon.json.metadata"),
        `600 ${daemonStat.uid} ${daemonStat.gid}\n`,
        { mode: 0o600 },
      );
      fs.writeFileSync(path.join(stateDir, "default-runtime.original"), "nvidia\n", {
        mode: 0o600,
