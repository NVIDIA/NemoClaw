...
import {
  ...
  message,
  PROCESS_EXIT_WAIT,
  runBaselineMutationFixture,
  runEvidenceFixture,
  type FixtureMode,
  type SessionRecords,
} from "./launch-agent-turn-fixtures";
...
export function registerLaunchAgentTurnTests(group: LaunchAgentTurnGroup): void {
  ...
  function runLaunchSessionFixture(
    mode: FixtureMode,
    terminalCopy: "absent" | "ansi" | "reordered",
  ) {
  ...
  registerTestGroup(group, "evidence", () => {
    ...
    it.each([[], ["-g", "fixture-gateway"]].map((gatewayArgs) => [gatewayArgs] as const))(
      ...
      (gatewayArgs) => {
        ...
        expect(fixture.malformed.stderr).toContain(
          '"reason":"openshell_launch_invocation_invalid"',
        );
        ...
        expect(fixture.duplicate.stderr).toContain(
          '"reason":"openshell_launch_intercept_duplicate"',
        );
    ...
    it.runIf(process.platform === "linux")(
      ...
      async () => {
        ...
        calls.slice(1).forEach((call) => {
          ...
          expect(call.env?.NEMOCLAW_LAUNCH_RUNTIME_ENV_SCRIPT).toBe(
            OPENCLAW_LAUNCH_RUNTIME_ENV_SCRIPT,
          );
        });
      },
