...
describe("Pi qualification event oracle", () => {
  it.each(["pi-agent-qualification-amd64", "pi-agent-qualification-arm64"] as const)(
    "keeps every Pi image source in the %s ownership boundary",
    (targetId) => {
  ...
  it("selects both architecture qualifications for a copied Pi image source", () => {
    const targetIds = catalogueTargetsForChangedFiles([
      "nemoclaw-blueprint/scripts/nemotron-inference-fix.js",
    ]).map((target) => target.id);

    expect(targetIds).toEqual(["pi-agent-qualification-amd64", "pi-agent-qualification-arm64"]);
  });

  it("selects only the matching architecture for an architecture-specific receipt", () => {
    expect(
      catalogueTargetsForChangedFiles(["ci/pi-agent-qualification-v1-linux-amd64.json"]).map(
        (target) => target.id,
      ),
    ).toEqual(["pi-agent-qualification-amd64"]);
    expect(
      catalogueTargetsForChangedFiles(["ci/pi-agent-qualification-v1-linux-arm64.json"]).map(
        (target) => target.id,
      ),
    ).toEqual(["pi-agent-qualification-arm64"]);
  });

  it("selects both architecture qualifications when the shared environment helper changes", () => {
    const targetIds = catalogueTargetsForChangedFiles([
