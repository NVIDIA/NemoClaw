...
describe("Hermes MCP HTTP failure diagnostics", () => {
  it("sends one authenticated request per chat probe execution without retrying (#8697)", () => {
  ...
  it("redacts sensitive values before limiting text and binary response evidence (#8697)", async () => {
    const resultToken = "fixture-result-token";
    const script = buildHermesMcpChatProbeScript('{"messages":[]}', resultToken);
      ...
      const runBody = async (
        ...
        return probe.run(
          trustedShellCommand({
            ...
            args: ["-c", script],
            reason:
              "verify that ShellProbe redacts sensitive values before limiting response evidence",
          }),
          {
            artifactName,
            env: deterministicEnv({
  ...
  it("rejects HTTP 500 with bounded, redacted response evidence (#8697)", () => {
    ...
    ).toThrowError(/HTTP 500; redacted response body:/u);
    ...
    try {
      ...
      expect([...message.split("redacted response body: ")[1]!].length).toBeLessThanOrEqual(
        HERMES_MCP_FAILURE_PREVIEW_CHARS,
      );
    }
  });

  it("requires a result token and exactly one authenticated tools/call after HTTP 2xx (#8697)", () => {
    const request = {
  ...
  it("redacts transport evidence and rejects missing or duplicate HTTP status markers (#8697)", () => {
    const secret = "fixture-transport-secret";
    for (const result of [
      { exitCode: 7, signal: null, stdout: secret, stderr: "transport detail" },
      { exitCode: null, signal: "SIGKILL" as const, stdout: secret, stderr: "" },
    ]) {
      try {
        assertHermesMcpHttpResponse(result, [secret]);
        throw new Error("expected transport failure");
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        expect(message).toMatch(/transport failed \((?:exit=7|signal=SIGKILL)\)/u);
        expect(message).toContain("[REDACTED]");
