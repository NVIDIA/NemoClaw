...
| `openclaw-plugin-runtime-exdev.onboard-pairing` | `openclaw-plugin-runtime-exdev` | One attempt | None. | If fresh onboarding reports that canonical CLI device pairing did not appear, the test attempts to record structured failure diagnostics and always records `failed-no-retry` evidence. It does not automatically resume an ambiguously mutated onboarding session. | `openclaw-plugin-exdev-onboard-retry.json` |
| `openclaw-plugin-runtime-exdev.recreate-pairing` | `openclaw-plugin-runtime-exdev` | One attempt | None. | If recreation reports the same pairing failure, the test attempts to record distinct structured diagnostics and always records `failed-no-retry` evidence. A later attempt requires a new E2E dispatch after failure classification. | `openclaw-weather-plugin-recreate-retry.json` |
