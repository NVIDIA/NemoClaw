...
| `openclaw-plugin-runtime-exdev.onboard-pairing` | `openclaw-plugin-runtime-exdev` | Two attempts | Fresh onboarding reports that canonical CLI device pairing did not appear. | The retry uses `onboard --resume` only when OpenShell reports the sandbox as Ready, the registry resolves its current runtime identity, and the saved in-progress OpenClaw session is paused at `post_verify` for the same sandbox, gateway, and custom Dockerfile. The saved session retains mutation authority. Other errors stop without retry. | `openclaw-plugin-exdev-onboard-retry.json` |
