...
The retired nightly caller no longer runs. Host-side Vitest controllers now own
the standing staging Launchable lifecycle for the full scenario and issue #9880.
They share typed creation, access, identity, evidence, and cleanup fixtures while
the trusted workflows retain only authorization, pinned Brev setup, credential
scope, test invocation, and artifact upload. They do not restore source copying,
source installation, the legacy suite selector, or scheduled Brev coverage.
