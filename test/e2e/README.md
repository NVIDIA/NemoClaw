...
Gate initialization, prerequisite-CI handling, and protected approval share one
...
`tools/e2e/pr-e2e-gate.mts`—automatically dispatches those selected jobs.
...
trusted controller boundary leaves the required check in progress with
`E2E reviewer authorization required to run E2E`. No selected job or target
runs and no repository secret is exposed. The same controller run starts `Approve
...
synchronization controller cancels active child runs and closes required
...
matching pending required-check state, compatible trusted controller commit, and
final live revision. It updates the required check to `Running <count> E2E check(s)`
and dispatches the selected jobs and targets in one workflow run. The child
workflow receives the controller-owned required-check ID. Before checking
...
failure before dispatch restores the authorization title and leaves the
required check in progress. A maintainer can then launch a fresh first-attempt
...
targets in the required-check summary and the
...
matching pending required check, and that the controller commit is either
still `main` or
...
evidence for every selected item completes the required check successfully. Failed,
