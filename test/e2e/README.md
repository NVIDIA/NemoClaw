...
Treat it as passing evidence only when the `E2E` workflow concludes with `success` for the recorded PR number, head SHA, base SHA, and trusted workflow SHA.
A changed head or base invalidates the evidence and requires a new manual run.
