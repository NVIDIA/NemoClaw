...
fails the controller workflow and fails the required check closed.
...
reviewer to approve the exact head SHA, base SHA, and risk plan before
credentialed execution begins. If no job or target is selected, the required
check passes without an E2E run.
...
trusted controller boundary leaves `E2E / PR Gate` in progress with `E2E
reviewer authorization required to run E2E`. No selected job or target runs and
no repository secret is exposed. The same controller run starts `Approve
...
matching pending required-check state, compatible trusted controller commit,
and final live revision. It updates the required check to
...
authorization title and leaves the required check in progress. A maintainer can then
...
controller completes the required check as
...
the old exact-diff required check as cancelled when the PR revision moved
...
the prior coordinator remains active long enough to close its check.
