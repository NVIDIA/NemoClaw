...
discovery and lifecycle scope in two independent matrix jobs. Each job assembles one candidate
...
check. It rejects a dirty NemoClaw checkout and requires expected
...
the operator's host-preparation declaration. It observes whether the test
...
| `NEMOCLAW_E2E_EXPECTED_SHA` | 40-character NemoClaw checkout revision |
...
| `NEMOCLAW_WINDOWS_MXC_OPENSHELL_VERSION` | OpenShell package version |
| `NEMOCLAW_WINDOWS_MXC_OPENSHELL_REVISION` | 40-character OpenShell source revision |
...
A `managed-image` receipt instead requires a digest that matches the
registered sandbox image tag, a non-empty publication cohort, and a
...
the root-start and sandbox-phase limits. The installer must emit the local
