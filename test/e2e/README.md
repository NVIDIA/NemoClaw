...
If the OpenClaw second-server or post-rebuild tool call fails, the test attempts to write reduced MCP
request evidence and run one bounded `mcp status --tools --json` probe before cleanup. The probe
receives no host MCP secret. The test stops waiting for diagnostics after 65 seconds; this deadline
does not cancel an unfinished artifact write. Failed or late diagnostics can leave evidence unavailable.
Diagnostic failures do not replace the original failure, and the test does not retry the call.
Trusted-private relay cleanup attempts to retain the last 50 connection events, capped at 8 KiB.
Those events contain connection outcomes and byte counts, not request payloads or credentials.
When the trusted-private status probe fails, the test also attempts to capture the last two minutes
of OpenShell logs, limited to 200 lines and 32 KiB, with the test credentials redacted.
