...
maintainer or administrator to approve the PR SHA through `approve-e2e` before
credentialed execution begins. If no job or target is selected, coordination
passes without an E2E run and the native required job mirrors that success.
...
A repository maintainer or administrator chooses **Run workflow** on `main`, selects
...
progress with `Maintainer approval required to run fork E2E`. Non-secret
