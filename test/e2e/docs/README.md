...
- `.github/workflows/platform-vitest-main.yaml` runs the full Vitest suite in
  ...
  `macos-e2e.yaml` and `wsl-e2e.yaml` call focused E2E targets directly for
  platform coverage. Repository-hosted targets, including `whatsapp-qr-compact`
  and `ollama-auth-proxy`, are selected through `.github/workflows/e2e.yaml`.
