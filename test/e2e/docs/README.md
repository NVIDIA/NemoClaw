...
`dcode-rebuild-invalid-credential` reserves a 130-minute base budget for
onboarding, its credential-rotation lifecycle, state validation, and ordered
cloud checks. With config export refusal, its default test timeout is 132
minutes and its job ceiling is 152 minutes. `NEMOCLAW_TEST_TIMEOUT`, in
milliseconds, can raise but cannot
