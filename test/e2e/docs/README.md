...
- `.github/workflows/e2e.yaml` runs selected or all supported
  ...
  validates a plan hash that binds each risk signal to the workflow revision,
  inference mode, and optional hardware selections. The workflow boundary requires
  every selected job shard to upload its evidence artifact.
