...
describe("assertSafeName", () => {
  ...
  it("rejects names with semicolons (command injection)", () => {
    // assertSafeName calls process.exit(1) — we test that indirectly
    // by checking it throws or calls process.exit. We mock process.exit.
...
describe("runCaptureArgv", () => {
  ...
  it("does NOT expand shell metacharacters (injection proof)", () => {
    // This is the core regression test for issue #325 / command injection.
    // OLD pattern: run(`echo "${value}"`) with value="$(id)" would execute id.
    // NEW pattern: runCaptureArgv never invokes bash, so $(id) is literal.
    const malicious = "$(id)";
    const out = runCaptureArgv("echo", [malicious]);
    assert.ok(
      out.includes("$(id)"),
      `Expected literal '$(id)' in output, got: "${out}"`
    );
    assert.ok(
      !out.includes("uid="),
  ...
  it("does NOT expand semicolons as command separators", () => {
    ...
    const out = runCaptureArgv("echo", [malicious]);
    assert.ok(
      out.includes("safe; echo INJECTED"),
  ...
  it("does NOT expand backtick command substitution", () => {
    ...
    const out = runCaptureArgv("echo", [malicious]);
    assert.ok(
      out.includes("`echo PWNED`"),
      "Backtick must be treated as literal in argv"
    );
...
describe("runArgv", () => {
  ...
  it("runs a command without shell expansion", () => {
    // Use a command with stdio piped so we don't pollute test output
...
describe("runner opts.env does not drop inherited environment", () => {
  it("runCaptureArgv preserves PATH from process.env even when opts.env is passed", () => {
    ...
    // The fix: extract env from opts before spreading: `const { env: extra, ...rest } = opts`.
    const result = runCaptureArgv("printenv", ["PATH"], {
      env: { CUSTOM_TEST_VAR: "sentinel" },
    });
    assert.ok(
      result.length > 0,
      "PATH must be preserved when opts.env is passed — got empty string, which means PATH was dropped"
    );
    assert.ok(
      result.includes("/"),
      `PATH should contain path separators, got: "${result}"`
    );
  });
