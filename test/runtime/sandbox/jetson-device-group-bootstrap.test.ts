...
suite("Jetson device-group bootstrap", () => {
  ...
  it("preflights every group record before account mutation (#7610)", () => {
    const run = runBootstrap(["--device-group-gids", "44,110", "--", SUPERVISOR], {
      environment: { TEST_GETENT_MALFORMED_GID: "110" },
    });
