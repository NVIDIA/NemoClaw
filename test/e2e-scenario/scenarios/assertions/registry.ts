...
function suiteGroup(
  ...
  return group({
    ...
    phase,
    steps,
    description: `Converted suite ${suiteId}.`,
  });
}
...
const smokeSteps = [
  shellStep({
    id: "runtime.smoke.cli-available",
    phase: "runtime",
    ref: "test/e2e-scenario/validation_suites/smoke/00-cli-available.sh",
  }),
  shellStep({
  ...
  shellStep({
    id: "runtime.smoke.sandbox-listed",
    phase: "runtime",
    ref: "test/e2e-scenario/validation_suites/smoke/02-sandbox-listed.sh",
  }),
  shellStep({
    id: "runtime.smoke.sandbox-shell",
    phase: "runtime",
    ref: "test/e2e-scenario/validation_suites/smoke/03-sandbox-shell.sh",
    reliability: { timeoutSeconds: 30 },
  }),
];
...
const baselineOnboardingSteps = [
  shellStep({
    id: "baseline.cli-and-openshell",
    phase: "runtime",
    ref: "test/e2e-scenario/validation_suites/baseline-onboarding/00-cli-and-openshell.sh",
  }),
  shellStep({
    id: "baseline.sandbox-state",
    phase: "runtime",
    ref: "test/e2e-scenario/validation_suites/baseline-onboarding/01-sandbox-state.sh",
  }),
  shellStep({
    id: "baseline.route-and-smoke",
    phase: "runtime",
    ref: "test/e2e-scenario/validation_suites/baseline-onboarding/02-route-and-smoke.sh",
  }),
];

const onboardingStateSteps = [
  shellStep({
    id: "onboarding.state.registry",
    phase: "runtime",
    ref: "test/e2e-scenario/validation_suites/onboarding/state/00-registry-provider-model-policies.sh",
  }),
  shellStep({
    id: "onboarding.state.session",
    phase: "runtime",
    ref: "test/e2e-scenario/validation_suites/onboarding/state/01-session-provider-model-policies.sh",
  }),
];
...
export const validationSuiteGroups: AssertionGroup[] = [
  ...
  suiteGroup("platform-macos", [
    shellStep({
      id: "platform.macos.smoke",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/platform/macos/00-macos-smoke.sh",
    }),
  ]),
  suiteGroup("platform-wsl", [
    shellStep({
      id: "platform.wsl.smoke",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/platform/wsl/00-wsl-smoke.sh",
    }),
  ]),
  ...
  suiteGroup("ollama-auth-proxy", [
    ...ollamaProxySteps,
    shellStep({
      id: "runtime.ollama-auth-proxy.auth-enforcement",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/inference/ollama-auth-proxy/01-auth-enforcement.sh",
    }),
  ]),
  ...
  suiteGroup("model-router", [
    shellStep({
      id: "runtime.model-router.healthy-endpoint",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/inference/model-router/00-healthy-endpoint.sh",
    }),
    shellStep({
      id: "runtime.model-router.provider-routed-completion",
      phase: "runtime",
  ...
  suiteGroup("kimi-compatibility", [
    shellStep({
      ...
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/inference/kimi-compatibility/00-plugin-wiring.sh",
      reliability: { timeoutSeconds: 30, retry: { attempts: 2, on: ["model-toolcall-transient"] } },
    }),
    shellStep({
      id: "runtime.kimi.compatible-models-route",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/inference/kimi-compatibility/01-kimi-compatible-models-route.sh",
      reliability: { timeoutSeconds: 30, retry: { attempts: 2, on: ["model-toolcall-transient"] } },
    }),
  ]),
  ...
  suiteGroup("messaging-telegram", [
    shellStep({
      id: "messaging.telegram.injection-safety",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/messaging/telegram/00-telegram-injection-safety.sh",
      reliability: { timeoutSeconds: 30, retry: { attempts: 2, on: ["external-tunnel"] } },
    }),
    shellStep({
      id: "messaging.telegram.injection-payload-classes",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/messaging/telegram/01-telegram-injection-payload-classes.sh",
      reliability: { timeoutSeconds: 30, retry: { attempts: 2, on: ["external-tunnel"] } },
    }),
  ]),
  suiteGroup("messaging-discord", [
    shellStep({
      id: "messaging.discord.gateway-path",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/messaging/discord/00-discord-gateway-path.sh",
      reliability: { timeoutSeconds: 30, retry: { attempts: 2, on: ["external-tunnel"] } },
    }),
  ]),
  suiteGroup("messaging-slack", [
    shellStep({
      id: "messaging.slack.provider-state",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/messaging/slack/00-slack-provider-state.sh",
      reliability: { timeoutSeconds: 30, retry: { attempts: 2, on: ["external-tunnel"] } },
    }),
  ]),
  suiteGroup("messaging-token-rotation", [
    shellStep({
      id: "messaging.token-rotation",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/messaging/token-rotation/00-provider-rotation-isolated.sh",
    }),
  ]),
  suiteGroup("sandbox-lifecycle", [
    shellStep({
      id: "lifecycle.sandbox.gateway-health",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/sandbox/lifecycle/00-gateway-health.sh",
    }),
    shellStep({
      id: "lifecycle.sandbox.gateway-recovery",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/sandbox/lifecycle/01-gateway-recovery.sh",
    }),
  ]),
  suiteGroup("sandbox-operations", [
    shellStep({
      id: "lifecycle.sandbox.list-and-status",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/sandbox/operations/00-list-and-status.sh",
    }),
    shellStep({
      id: "lifecycle.sandbox.logs-and-exec",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/sandbox/operations/01-logs-and-exec.sh",
    }),
  ]),
  suiteGroup("snapshot", [
    shellStep({
      id: "lifecycle.snapshot.create-list-restore",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/sandbox/snapshot/00-create-list-restore.sh",
    }),
  ]),
  suiteGroup("snapshot-lifecycle", [
    shellStep({
      id: "lifecycle.snapshot.create-list-restore",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/sandbox/snapshot/00-create-list-restore.sh",
  ...
  suiteGroup("rebuild", [
    shellStep({
      id: "lifecycle.rebuild.state-preserved",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/rebuild_upgrade/00-state-preserved.sh",
      reliability: { timeoutSeconds: 120, retry: { attempts: 2, on: ["runner-infra"] } },
    }),
    shellStep({
      id: "lifecycle.rebuild.agent-version-upgraded",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/rebuild_upgrade/01-agent-version-upgraded.sh",
      reliability: { timeoutSeconds: 120, retry: { attempts: 2, on: ["runner-infra"] } },
    }),
    shellStep({
      id: "lifecycle.rebuild.post-rebuild-inference",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/rebuild_upgrade/02-post-rebuild-inference.sh",
      reliability: { timeoutSeconds: 120, retry: { attempts: 2, on: ["runner-infra"] } },
    }),
  ]),
  suiteGroup("upgrade", [
    shellStep({
      id: "lifecycle.upgrade.policy-config-preserved",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/rebuild_upgrade/03-policy-config-preserved.sh",
      reliability: { timeoutSeconds: 120, retry: { attempts: 2, on: ["wrong-installed-ref"] } },
    }),
    shellStep({
      id: "lifecycle.upgrade.survivor-reachable",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/rebuild_upgrade/04-upgrade-survivor-reachable.sh",
      reliability: { timeoutSeconds: 120, retry: { attempts: 2, on: ["wrong-installed-ref"] } },
    }),
  ]),
  ...
  suiteGroup("hermes-specific", [
    shellStep({
      id: "runtime.hermes.health",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/hermes/00-hermes-health.sh",
      reliability: { timeoutSeconds: 30, retry: { attempts: 2, on: ["gateway-transient"] } },
    }),
    shellStep({
      id: "runtime.hermes.history-writable",
      phase: "runtime",
      ref: "test/e2e-scenario/validation_suites/hermes/01-history-writable.sh",
      reliability: { timeoutSeconds: 90, retry: { attempts: 2, on: ["gateway-transient"] } },
    }),
  ]),
];
...
export function assertionGroupForOnboardingAssertion(
  assertionId: string,
): AssertionGroup | undefined {
...
export function validateAssertionGroups(groups: AssertionGroup[], repoRoot: string): void {
  for (const group of groups) {
    ...
    for (const step of group.steps) {
      ...
      if (
        (step.reliability?.retry?.attempts ?? 1) > 1 &&
        (step.reliability?.retry?.on.length ?? 0) === 0
      ) {
      ...
      if (step.implementation.kind === "shell") {
        ...
        if (!fs.existsSync(scriptPath) && !fs.existsSync(cwdScriptPath)) {
          throw new Error(
            `Assertion step ${step.id} references missing script ${step.implementation.ref}`,
          );
        }
      }
    }
