...
`,
  ...
  it("requires issue-2478 job-specific secret boundary and selector coverage", () => {
    ...
    const runVitest = job.steps?.find(
      (step) => step.name === "Run issue #2478 crash-loop recovery live Vitest test",
    );
    if (runVitest) {
      runVitest.env = {
        NVIDIA_INFERENCE_API_KEY: "${{ secrets.NVIDIA_INFERENCE_API_KEY }}",
    ...
    try {
      expect(validateE2eVitestScenariosWorkflowBoundary(workflowPath)).toEqual(
        expect.arrayContaining([
          ...
          "issue-2478-crash-loop-recovery-vitest step 'Run issue #2478 crash-loop recovery live Vitest test' env must not include NVIDIA_INFERENCE_API_KEY",
          "issue-2478-crash-loop-recovery-vitest Vitest step env must not include NVIDIA_INFERENCE_API_KEY",
