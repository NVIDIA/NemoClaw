...
import { repairSelectionInput, requireFixture } from "../../helpers/pr-review-advisor-repair.ts";
...
describe("PR Review Advisor repair artifact binding", () => {
  it("keeps one-shot identity fixed for the approved head, run, and findings (#10791)", () => {
    const original = selectRepairAttempt(parseSelectionInput(repairSelectionInput()));
  ...
  it("validates every bounded artifact file and rejects symlink substitution (#10791)", () => {
    ...
    const selectedInterest = requireFixture(
      ...
      buildAdvisorFindingLedger({
        ...
        input:
          interest === selectedInterest
            ...
            : {
                findings: [],
                noFindingsReason: `No ${interest} blocker remains.`,
              },
      }),
    );
    const selectedLedger = ledgers.find(({ interest }) => interest === selectedInterest);
    const selectedFindingId = requireFixture(selectedLedger?.findings[0]?.id, "selected finding");
