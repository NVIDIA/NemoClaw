...
describe("Hermes managed-tool gateway broker", () => {
  ...
  it("refreshes via header, replaces upstream auth, normalizes responses, and rotates OpenShell storage", async () => {
    ...
    const portalRequests: Array<{ body: string; refreshHeader?: string }> = [];
    ...
    try {
      ...
      const denied = await fetch(`http://127.0.0.1:${brokerPort}/firecrawl/v1/scrape`, {
        headers: { Authorization: "Bearer wrong-refresh" },
      });
      ...
      expect(portalRequests[0]?.refreshHeader).toBe("refresh-1");
      expect(new URLSearchParams(portalRequests[0]?.body).get("refresh_token")).toBeNull();
      expect(new URLSearchParams(portalRequests[0]?.body).get("grant_type")).toBe(
        "refresh_token",
      );
      ...
      const checks = [
        ["/browser-use/browsers", { "X-Browser-Use-API-Key": "refresh-2" }, "browser"],
        ["/fal-queue/fal-ai/test", { Authorization: "Key refresh-2" }, "fal"],
        ["/openai-audio/v1/audio/speech", { "openai-api-key": "refresh-2" }, "audio"],
        ["/modal/sandboxes", { Authorization: "Bearer refresh-2" }, "modal"],
      ] as const;
