...
EOF
  ...
  describe("validate_config_symlinks", () => {
    ...
    it("passes when symlinks point to expected targets", () => {
      ...
      // Use realpath to resolve macOS /var → /private/var symlinks so
      // the expected path matches what readlink -f returns inside bash.
      const resolvedConfig = execFileSync("realpath", [join(workDir, "config")], {
        encoding: "utf-8",
      }).trim();
      const resolvedData = execFileSync("realpath", [join(workDir, "data")], {
        encoding: "utf-8",
      }).trim();

      runWithLib(`
        validate_config_symlinks ${JSON.stringify(resolvedConfig)} ${JSON.stringify(resolvedData)}
        echo "SYMLINKS_OK"
      `);
    });
  ...
  describe("configure_messaging_channels", () => {
    ...
    it("logs active channels when tokens are present", () => {
      // Since configure_messaging_channels writes to stderr, we need to capture it
      const script = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        `source ${JSON.stringify(SANDBOX_INIT)}`,
      ...
      const tmpFile = join(tmpdir(), `sandbox-init-channels-${process.pid}.sh`);
      try {
        writeFileSync(tmpFile, script, { mode: 0o700 });
        const result = execFileSync("bash", [tmpFile], {
          encoding: "utf-8",
        ...
        // This shouldn't fail, but if it does we want the stderr
        const stderr = (e.stderr || "").toString();
        expect(stderr).toContain("telegram");
        expect(stderr).toContain("slack");
        expect(stderr).not.toContain("discord");
      }
    });
  });
