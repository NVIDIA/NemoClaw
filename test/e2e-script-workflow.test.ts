...
describe("E2E reusable workflow contract", () => {
  ...
  it("runs credential migration on the former shell runner through Vitest artifacts", () => {
    ...
    expect(runStep?.env?.NEMOCLAW_MODEL).toBe("nvidia/nemotron-3-super-v3");
    expect(runStep?.env?.NEMOCLAW_COMPAT_MODEL).toBe("nvidia/nemotron-3-super-v3");
  ...
  it("routes reusable hosted inference jobs through the hosted custom endpoint", () => {
    ...
    expect(exportStep?.run).toContain("NEMOCLAW_MODEL=nvidia/nemotron-3-super-v3");
    expect(exportStep?.run).toContain("NEMOCLAW_COMPAT_MODEL=nvidia/nemotron-3-super-v3");
  ...
  it("keeps rebuild fixture registry inference aligned with hosted custom inference", () => {
    ...
    for (const fixture of rebuildFixtures) {
      ...
      expect(body, fixture).toContain("nvidia/nemotron-3-super-v3");
  ...
  it("routes direct hosted-secret jobs through the hosted custom inference endpoint", () => {
    ...
    for (const { jobName, step } of directSecretSteps) {
      ...
      expect(step.env?.NEMOCLAW_MODEL, jobName).toBe("nvidia/nemotron-3-super-v3");
      expect(step.env?.NEMOCLAW_COMPAT_MODEL, jobName).toBe("nvidia/nemotron-3-super-v3");
