...
if (crashAfter === "credential-projection-coalesced") {
  ...
  process.env.NEMOCLAW_MCP_PROVIDER_SYNC_TIMEOUT_SECONDS = "4";
}
...
if (initializeSandbox && !registry.getSandbox("crash-test")) {
  registry.registerSandbox(
    externalAuthority
      ...
      : ${JSON.stringify(managedSandboxEntry("crash-test"))},
  );
}
...
function initializeSandboxRegistry(
  ...
  const sandboxEntry =
    policyAuthority === "nemoclaw-managed"
      ? managedSandboxEntry("crash-test", "openclaw", {
  ...
  const result = spawnSync(
    ...
    [
      "-e",
      `process.env.HOME = ${JSON.stringify(home)}; const registry = require("./src/lib/state/registry.js"); if (${JSON.stringify(replace)}) registry.removeSandbox("crash-test"); registry.registerSandbox(${JSON.stringify(sandboxEntry)});`,
    ],
    {
      cwd: process.cwd(),
      encoding: "utf8",
...
describe("MCP add crash consistency", () => {
  it("commits one bridge at the stable credential revision and rejects one duplicate (#9764)", async () => {
    ...
    try {
      ...
      const credentialObservations = fs
        .readFileSync(path.join(home, "credential-observation-log.marker"), "utf8")
        .split("\n")
        .filter(Boolean);
      expect(credentialObservations[0]).toBe("v4067750153477477214");
      expect(credentialObservations.slice(1)).toHaveLength(6);
      expect(
        credentialObservations
          .slice(1)
          .every((revision) => revision === "v4067750153477477215"),
      ).toBe(true);
