...
(async () => {
  ...
  const sandboxName = await createSandbox(null, "gpt-5.4", "nvidia-prod", null, "my-assistant", null, ["telegram"], ${customDockerfileArg});
...
(async () => {
  ...
  const sandboxName = await createSandbox(null, "gpt-5.4", "nvidia-prod", null, "my-assistant", null, ["whatsapp"], ${customDockerfileArg});
  console.log(JSON.stringify({ sandboxName, commands, registerCalls }));
})().catch((error) => {
  console.error(error);
  process.exit(1);
