...
describe("complete managed-image publication workflow", () => {
  ...
  it("builds and exercises every shipped agent from an exact PR image before merge (#7744)", () => {
    ...
    const installDependencies = step(prBuilder, "Install managed-image harness dependencies");
    const build = step(prBuilder, "Build and push PR managed image to isolated registry");
    expect(installDependencies.run).toBe("npm ci --ignore-scripts");
    expect(steps.indexOf(installDependencies)).toBeLessThan(steps.indexOf(build));
    ...
    const policyBoundary = step(prBuilder, "Build managed-image OpenShell policy boundary");
    const directExercise = step(prBuilder, "Exercise managed startup root stdin and hold");
    expect(policyBoundary.run).toBe("npm run build:policy-boundary");
    expect(steps.indexOf(policyBoundary)).toBeGreaterThan(steps.indexOf(build));
    expect(steps.indexOf(policyBoundary)).toBeLessThan(steps.indexOf(directExercise));
    expect(directExercise.run).toContain(
      "run-managed-image-direct-e2e.ts",
    );
    expect(step(prBuilder, "Exercise exact PR image through real OpenShell").run).toContain(
      "run-managed-image-openshell-e2e.ts",
    );
    const failureDiagnostics = step(
      prBuilder,
      "Export sanitized managed-image OpenShell failure diagnostics",
    );
    expect(failureDiagnostics.run).toContain("export-managed-image-failure-diagnostics.ts");
    expect(steps.indexOf(installDependencies)).toBeLessThan(steps.indexOf(failureDiagnostics));
    expect(
