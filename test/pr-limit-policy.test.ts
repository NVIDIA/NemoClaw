...
it("keeps contributor guidance aligned with the enforced maintainer exemption", () => {
  ...
  const e2eFixLoop = read(".agents/skills/nemoclaw-maintainer-fix-e2e-failures/SKILL.md");
  ...
  expect(documentedLimit(e2eFixLoop)).toBe(enforcedLimit);
  ...
  expect(e2eFixLoop).toContain("git show origin/main:.github/workflows/pr-limit.yaml");
  expect(e2eFixLoop).toContain("For a non-exempt author");
});
