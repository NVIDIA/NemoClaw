  ...
  it("ignores PATH-shadowed python3 at cold start so the validator cannot be bypassed", () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        // list rather than resolving `python3` via `$PATH`. The shadow bin
        // installed above is intentionally NOT on the list, so a compromised
        // PATH cannot redirect the validator to a no-op interpreter.
        "_HERMES_BOUNDARY_TIMEOUT=()",
        "_HERMES_PYTHON=\"\"",
        "for _candidate in /opt/hermes/.venv/bin/python3 /usr/local/bin/python3 /usr/bin/python3; do",
        '  if [ -x "$_candidate" ]; then _HERMES_PYTHON="$_candidate"; break; fi',
        "done",
        "unset _candidate",
        extractShellFunctionFromSource(src, "validate_hermes_env_secret_boundary"),
        `HERMES_DIR=${shellQuote(hermesHome)}`,
