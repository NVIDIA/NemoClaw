...
describe("policies", () => {
  describe("listPresets", () => {
    it("returns all 23 presets", () => {
      const presets = policies.listPresets();
      expect(presets.length).toBe(23);
    });
    ...
    it("includes the DuckDuckGo web-search preset", () => {
      const presets = policies.listPresets();
      expect(presets.map((preset) => preset.name)).toContain("duckduckgo");
    });
