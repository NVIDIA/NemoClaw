...
`,
  ...
  describe("issue 4224: spawnSync openshell ENOENT in non-interactive shells", () => {
    ...
    // A mocked process.exit still runs finally cleanup, so record same-process
    // mkdtempSync calls at exit instead of counting shared $TMPDIR state.
    it("applyPreset does not create temp dirs before the openshell resolvability check", () => {
      ...
      let policyTempDirsCreated = 0;
      let policyTempDirsCreatedAtExit = -1;

      const resolveSpy = vi
        .spyOn(resolveOpenshellModule, "resolveOpenshell")
        .mockReturnValue(null);
      const mkdtempSpy = vi.spyOn(fs, "mkdtempSync").mockImplementation((prefix, options) => {
        if (prefix === policyTempPrefix) {
          policyTempDirsCreated += 1;
        }
      ...
      const exitSpy = vi.spyOn(process, "exit").mockImplementation(((_code?: number) => {
        policyTempDirsCreatedAtExit = policyTempDirsCreated;
        throw new Error("__test_exit__");
      }) as never);
      ...
      try {
        ...
        expect(exitSpy).toHaveBeenCalledWith(1);
        expect(policyTempDirsCreatedAtExit).toBe(0);
      } finally {
        resolveSpy.mockRestore();
