...
export function configureMissingHermesForwardCapture(
  ...
  vi.spyOn(forwardService, "launchForwardService").mockImplementation(
    ((target: { localPort: number; sandboxName: string; gatewayName: string }) => {
      (harness.runOpenshellSpy as unknown as (...args: unknown[]) => unknown)(
        [
          "forward",
          "start",
          "--background",
          String(target.localPort),
          target.sandboxName,
          "--gateway",
          target.gatewayName,
        ],
