...
describe("sandbox connect scope-upgrade approval on recover/probe (#4504)", () => {
  ...
  it(
    "approve timeout matches the watcher, list exceeds OpenClaw's deadline, and both stay within the outer cap",
    testTimeoutOptions(20_000),
    () => {
      const { tmpDir, stateFile, sandboxName } = setupFixture(
        {
      ...
      // list budget exceeds OpenClaw 2026.7.1's own 10s gateway-call deadline.
      expect(CONNECT_AUTO_PAIR_APPROVE_TIMEOUT_S).toBe(10);
      expect(CONNECT_AUTO_PAIR_LIST_TIMEOUT_S).toBe(15);
