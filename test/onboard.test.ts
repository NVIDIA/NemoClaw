...
describe("onboard helpers", () => {
  ...
  it("patches the Hermes Dockerfile with runtime token env keys", () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "nemoclaw-onboard-dockerfile-hermes-runtime-"),
    );
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
        ...
        "ARG NEMOCLAW_HERMES_RUNTIME_TOKEN_ENVS_B64=W10=",
        "ARG NEMOCLAW_BUILD_ID=default",
      ].join("\n"),
    );
    ...
    try {
      ...
      const line = patched
        .split("\n")
        .find((l) => l.startsWith("ARG NEMOCLAW_HERMES_RUNTIME_TOKEN_ENVS_B64="));
      assert.ok(line, "expected Hermes runtime token build arg");
      const decoded = JSON.parse(Buffer.from(line.split("=")[1], "base64").toString("utf8"));
      assert.deepEqual(decoded, ["DISCORD_BOT_TOKEN"]);
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
...
`;
  ...
  it(
    "passes Hermes Discord tokens as runtime env instead of provider placeholders",
    { timeout: 60_000 },
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "nemoclaw-onboard-hermes-discord-runtime-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
      const scriptPath = path.join(tmpDir, "hermes-discord-runtime.js");
      const onboardPath = JSON.stringify(path.join(repoRoot, "dist", "lib", "onboard.js"));
      const runnerPath = JSON.stringify(path.join(repoRoot, "dist", "lib", "runner.js"));
      const registryPath = JSON.stringify(path.join(repoRoot, "dist", "lib", "state", "registry.js"));
      const preflightPath = JSON.stringify(path.join(repoRoot, "dist", "lib", "preflight.js"));
...
runner.runCapture = (command) => {
  if (_n(command).includes("sandbox get hermes-agent")) return "";
  if (_n(command).includes("sandbox list")) return "hermes-agent Ready";
  if (_n(command).includes("sandbox exec -n hermes-agent -- curl -sf http://localhost:18642/")) return "ok";
  if (_n(command).includes("forward list")) return "hermes-agent 127.0.0.1 18642 12345 running";
  return "";
};
registry.registerSandbox = () => true;
registry.updateSandbox = () => true;
...
childProcess.spawn = (...args) => {
  ...
  commands.push(entry);
  process.nextTick(() => {
    child.stdout.emit("data", Buffer.from("Created sandbox: hermes-agent\n"));
    child.emit("close", 0);
  });
  return child;
};
...
`;
      ...
      const providerMutations = payload.commands.filter((entry: CommandEntry) =>
        /\bprovider (create|update)\b/.test(entry.command),
      );
      assert.ok(
        providerMutations.every(
          (entry: CommandEntry) => !entry.command.includes("hermes-agent-discord-bridge"),
        ),
        "Hermes Discord should not create an OpenShell placeholder provider",
      );
      ...
      assert.doesNotMatch(createCommand.command, /--provider hermes-agent-discord-bridge/);
      assert.match(createCommand.command, /DISCORD_BOT_TOKEN=test-discord-token-value/);
      assert.ok(createCommand.env, "expected sandbox create environment");
      assert.equal(
        createCommand.env.DISCORD_BOT_TOKEN,
        undefined,
        "host Discord token must not leak into spawn environment",
      );

      const runtimeLine = createCommand.dockerfileContent
        ?.split("\n")
        .find((line: string) =>
          line.startsWith("ARG NEMOCLAW_HERMES_RUNTIME_TOKEN_ENVS_B64="),
        );
      assert.ok(runtimeLine, "expected Hermes runtime token build arg");
      const runtimeKeys = JSON.parse(Buffer.from(runtimeLine.split("=")[1], "base64").toString());
      assert.deepEqual(runtimeKeys, ["DISCORD_BOT_TOKEN"]);
    },
  );
