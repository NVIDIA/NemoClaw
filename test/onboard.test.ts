...
`;
  ...
  it("regression #2007: cleans up stale sandbox-owned dashboard forwards on other ports", () => {
    const source = fs.readFileSync(
      path.join(import.meta.dirname, "..", "src", "lib", "onboard.ts"),
      "utf-8",
    );

    // ensureDashboardForward must iterate occupied forwards and stop any
    // owned by this sandbox on ports other than the one we're about to bind.
    // This subsumes the original default-18789 cleanup from #2073.
    assert.match(
      source,
      /const occupied = getOccupiedPorts\(existingForwards\);[\s\S]*for \(const \[port, owner\] of occupied\.entries\(\)\)[\s\S]*owner === sandboxName[\s\S]*Number\(port\) !== actualPort[\s\S]*runOpenshell\(\["forward", "stop", port\]/,
    );
    // Guard against regressions to the hardcoded literal.
    assert.doesNotMatch(source, /const DEFAULT_SANDBOX_PORT = "18789"/);

    // The shared port constant must remain exported (used by sandbox image build).
