...
type DirectRunStubResult = {
  status: number | null;
  stdout?: string;
  stderr?: string;
};
type DirectSetupHarnessOptions = {
  runOpenshell?: (
    args: string[],
...
function createDirectSetupInferenceHarness(options: DirectSetupHarnessOptions = {}) {
  ...
  const setupInference = createSetupInference({
    ...
    upsertProvider: (
      ...
      onboardProviderHelpers.upsertProvider(name, type, credentialEnv, baseUrl, env, runOpenshell),
