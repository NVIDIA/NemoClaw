...
interface PortableOverrideOptions {
  profile?: string;
  dockerHost?: string;
  existingStorage?: string | ((home: string) => string);
  runroot?: string;
  runs?: number;
...
function runPortableOverride(options: PortableOverrideOptions = {}): {
  ...
  if (options.existingStorage !== undefined) {
    const existingStorage =
      typeof options.existingStorage === "function"
        ? options.existingStorage(home)
        : options.existingStorage;
    const storagePath = path.join(home, ".config", "containers", "storage.conf");
    fs.mkdirSync(path.dirname(storagePath), { recursive: true });
    fs.writeFileSync(storagePath, existingStorage, { mode: 0o600 });
  }
  ...
  const snippet = `
    ...
    podman() {
      ...
      case "$*" in
        ...
        *Store.RunRoot*) printf '%s\\n' "\${NEMOCLAW_TEST_RUNROOT:-$NEMOCLAW_TEST_EXPECTED_RUNROOT}" ;;
  ...
  const result = spawnSync("bash", ["-c", snippet], {
    ...
    env: {
      ...
      NEMOCLAW_TEST_EXPECTED_RUNROOT: (options.xdgRuntimeDir ?? "/run/user/1001").startsWith("/")
        ? `${options.xdgRuntimeDir ?? "/run/user/1001"}/containers`
        : "/run/user/4242/containers",
      XDG_RUNTIME_DIR: options.xdgRuntimeDir ?? "/run/user/1001",
    },
  });
  return {
    home,
...
describe("installer portable profile runtime override", () => {
  it("configures and verifies persistent Podman storage before installer preflight", () => {
    ...
    expect(result.stderr).toContain("SYSTEMCTL=--user stop podman.socket");
    expect(result.stderr).toContain("SYSTEMCTL=--user stop podman.service");
    ...
    expect(result.stderr).not.toContain("try-restart");
...
`,
    ...
    const stopSocket = trace.indexOf("SYSTEMCTL=--user stop podman.socket");
    const stopService = trace.indexOf("SYSTEMCTL=--user stop podman.service");
    const configureStorage = trace.indexOf("INFO=Configured persistent rootless Podman storage");
    const enableSocket = trace.indexOf("SYSTEMCTL=--user enable --now podman.socket");
    const enableRestart = trace.indexOf("SYSTEMCTL=--user enable podman-restart.service");
    const verifyStorage = trace.indexOf("PODMAN=info --format {{.Store.GraphRoot}}");
    expect(stopSocket).toBeLessThan(stopService);
    expect(stopService).toBeLessThan(configureStorage);
    expect(configureStorage).toBeLessThan(enableSocket);
    expect(enableSocket).toBeLessThan(enableRestart);
    expect(enableRestart).toBeLessThan(verifyStorage);
    expect(trace.match(/^PODMAN=.*$/gmu)).toEqual([
  ...
  it("falls back to the current user runtime directory when XDG_RUNTIME_DIR is not absolute", () => {
    const { home, result } = runPortableOverride({ xdgRuntimeDir: "" });
    expect(result.status).toBe(0);
    expect(
  ...
  it("replaces the obsolete NemoClaw-managed persistent RunRoot configuration", () => {
    const obsoleteStorage = (home: string) => `# NEMOCLAW_MANAGED_PORTABLE_STORAGE=1
[storage]
graphroot = "${home}/.local/share/containers/storage"
...
`;
    ...
    expect(
      fs.readFileSync(path.join(home, ".config", "containers", "storage.conf"), "utf8"),
    ).toContain('runroot = "/run/user/1001/containers"');
  });
