...
import { createDockerGpuInspectFixture } from "../../src/lib/onboard/__test-helpers__/docker-gpu-patch-fixtures";
...
function dockerCaptureFixture() {
  ...
  inspect.Id = OLD_CONTAINER_ID;
  return vi.fn((args: readonly string[]) => {
    if (args[0] === "ps") return `${OLD_CONTAINER_ID}\n`;
    if (args[0] === "inspect") return JSON.stringify([inspect]);
    return "";
  });
}

function dockerRunFixture() {
  return vi.fn((args: readonly string[]) => {
    if (args[0] === "ps") return { status: 0, stdout: `${NEW_CONTAINER_ID}\n` };
