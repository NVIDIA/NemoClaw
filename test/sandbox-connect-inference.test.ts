...
describe("sandbox connect scope-upgrade approval on recover/probe (#4504)", () => {
  ...
  it(
    ...
    () => {
      ...
      // List call keeps a tight 2s budget; approve call matches the watcher's
      // RUN_TIMEOUT_SECS = 10s.
      const listTimeout = 2;
      expect(script).toContain("[OPENCLAW, 'devices', 'list', '--json']");
      expect(script).toMatch(
        /\[OPENCLAW, 'devices', 'list', '--json'\][\s\S]*?timeout=2,/,
      );
      const approveTimeoutMatch = script.match(
        /'devices', 'approve', request_id, '--json'\][\s\S]*?timeout=(\d+),/,
      );
      expect(approveTimeoutMatch).not.toBeNull();
      const approveTimeout = Number(approveTimeoutMatch?.[1]);
      expect(approveTimeout).toBe(10);

      // MAX_APPROVALS is rendered into the script as a literal.
      const maxApprovalsMatch = script.match(/MAX_APPROVALS = (\d+)/);
      expect(maxApprovalsMatch).not.toBeNull();
      const maxApprovals = Number(maxApprovalsMatch?.[1]);

      // Read the real outer cap from source rather than hardcoding it, so this
      // invariant tracks any future change to CONNECT_AUTO_PAIR_TIMEOUT_MS.
      const connectSource = fs.readFileSync(
        path.join(import.meta.dirname, "..", "src", "lib", "actions", "sandbox", "connect.ts"),
        "utf-8",
      );
      const outerCapMatch = connectSource.match(
        /CONNECT_AUTO_PAIR_TIMEOUT_MS\s*=\s*([\d_]+)/,
      );
      expect(outerCapMatch).not.toBeNull();
      const outerCapSeconds = Number(outerCapMatch?.[1].replace(/_/g, "")) / 1000;

      // Budget invariant: the inner worst case (list + approveTimeout × MAX_APPROVALS)
      ...
      const innerBudgetSeconds = listTimeout + approveTimeout * maxApprovals;
      expect(innerBudgetSeconds).toBeLessThan(outerCapSeconds);
    },
  );
});
