...
suite("Jetson device-group bootstrap", () => {
  ...
  ])("rejects $title before account mutation (#8099)", ({ args, error }) => {
  ...
  it("rejects a missing sandbox account before group mutation (#8099)", () => {
  ...
  it("rejects a non-root caller before group mutation (#8099)", () => {
    const run = runBootstrap(["--device-group-gids", "44", "--", SUPERVISOR], {
  ...
  it("rejects a malformed existing group before account mutation (#8099)", () => {
    const run = runBootstrap(["--device-group-gids", "44", "--", SUPERVISOR], {
  ...
  it("stops before supervisor handoff when membership verification fails (#8099)", () => {
