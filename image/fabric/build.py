...
def main():
    ...
    if harness == "openclaw":
        for name in ("openclaw_adapter.py", "openclaw_features.py", "openclaw.fabric-adapter.json", "interfaces.py", "BRAVE-NOTICE.md", "BRAVE-LICENSE"):
            shutil.copyfile(ROOT / "image/fabric" / name, BUILD / name)
    if harness == "pi":
    ...
    (BUILD / "provenance.json").write_text(json.dumps({
        ...
        **({"openclaw_image": "ghcr.io/openclaw/openclaw@sha256:cc596b846506a5f4cfcee111394a2725f375f01cca2ebb492a161fd1b747f101",
            ...
            "brave_plugin": "@openclaw/brave-plugin@2026.9.4",
            "brave_plugin_sha256": "2b403a4b2d2be7a793235a458e2d376f38588fd913f41b92e208e8421a2a1617",
        ...
        **({"hermes_revision": HERMES_REVISION, "hermes_source_sha256": HERMES_HASH,
            "local_adapter_sources": {name: hashlib.sha256((ROOT / "image/fabric" / name).read_bytes()).hexdigest()
                for name in ("hermes_adapter.py", "hermes.fabric-adapter.json", "interfaces.py")},
