...
def gateway_settings(inference):
    ...
    return {
        "mode": "local",
        "bind": "lan" if settings.get("bind") == "0.0.0.0" else "loopback",
        "port": port,
        "auth": {"mode": "token", "token": "${NEMOCLAW_INTERFACE_TOKEN}"},
        "controlUi": {
            "enabled": True,
...
if __name__ == "__main__":
    ...
    args = sys.argv[1:]
    if args != ["devices", "list"] and not (
        len(args) == 3
        and args[:2] == ["devices", "approve"]
        and re.fullmatch("[A-Za-z0-9][A-Za-z0-9_-]*", args[2])
    ):
        raise SystemExit("usage: interfaces.py devices list | devices approve REQUEST_ID")
    home = Path("/sandbox/.openclaw")
    env = dict(
        os.environ,
        NEMOCLAW_INTERFACE_TOKEN=token(home),
        OPENCLAW_GATEWAY_TOKEN=token(home),
        OPENCLAW_CONFIG_PATH=str(home / "openclaw.json"),
        OPENCLAW_STATE_DIR=str(home),
        OPENCLAW_HOME="/sandbox",
    )
    raise SystemExit(
        subprocess.run(["/usr/local/bin/node", "/app/openclaw.mjs", *args], env=env).returncode
    )
