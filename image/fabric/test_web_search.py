...
class NativeSearchStartup(unittest.IsolatedAsyncioTestCase):
    async def test_gateway_resolves_placeholder_and_preserves_search_grants(self):
        options = {'api':'openai-completions','tuning':{},'agents':[
            ...
            'webSearch':{'provider':'brave','agentRefs':['main'],'credential':{'env':'SEARCH_KEY'}}}
        with tempfile.TemporaryDirectory(dir='/sandbox') as directory, patch.object(adapter,'ROOT',Path(directory)), patch.dict(os.environ,{'BRAVE_API_KEY':'fixture-placeholder'}):
            ...
            try:
                ...
                self.assertNotIn('fixture-placeholder',(Path(directory)/'openclaw.json').read_text())
            finally:
                await runtime.stop()
