...
// Exercises the OpenClaw tool factory and catalog from the image pinned in Dockerfile.openclaw.
import assert from 'node:assert/strict';
import fs from 'node:fs/promises';
import {execFileSync} from 'node:child_process';
import {createOpenClawCodingTools} from '/app/dist/agent-tools-CNTtT1Sj.mjs';
import {a as applyToolSearchCatalog, v as createToolSearchCatalogRef} from '/app/dist/local-model-lean-Ct7p3GNL.mjs';
...
for (const disclosure of ['direct', 'progressive']) {
  const options = {api: 'openai-completions', tuning: {}, agents: [
    {name: 'primary', tools: {disclosure}}, {name: 'reader', tools: {allow: ['read']}},
  ]};
    ...
    if (disclosure === 'direct') {
      ...
      if (agentId === 'reader') {
        ...
        for (const denied of ['exec', 'write', 'edit', 'sessions_spawn']) {
          await assert.rejects(() => call.execute('denied', {id: denied, args: {command: 'false'}}));
        }
      }
    }
  }
