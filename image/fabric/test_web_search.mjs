...
const options = {api:'openai-completions',tuning:{},agents:[{name:'main'},
  {name:'reader',tools:{allow:['read']}},{name:'writer'}],
...
for (const agentId of ['main','reader','writer']) {
  const tools = createOpenClawCodingTools({config,agentId,sessionKey:`agent:${agentId}:fixture`,workspaceDir:'/sandbox/workspace',cwd:'/sandbox/workspace'});
...
const server=http.createServer((request,response)=>{
  ...
  response.end(JSON.stringify({web:{results:[{title:'Owned fixture',url:'https://example.com/fixture',description:'Search response fixture'}]}}));
});
...
try {
  ...
  const snapshot=await prepareSecretsRuntimeSnapshot({config,env:process.env,includeAuthStoreRefs:false});
  assert.equal(snapshot.config.plugins.entries.brave.config.webSearch.apiKey,'fixture-placeholder');
  const tool=createBraveProvider().createTool({config:snapshot.config,searchConfig:snapshot.config.tools.web.search});
