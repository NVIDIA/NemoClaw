...
// Executes the tool factory from the OpenClaw image pinned in Dockerfile.openclaw.
...
const options = {api: 'openai-completions', tuning: {}, agents: [
  {name: 'primary'}, {name: 'reader', tools: {allow: ['read']}},
  {name: 'reviewer', tools: {allow: ['read']}},
]};
const config = JSON.parse(execFileSync('/opt/fabric/bin/python', ['-c',
...
for (const agentId of ['reader', 'primary', 'reviewer']) {
  const tools = createOpenClawCodingTools({config, agentId, sessionKey: `agent:${agentId}:fixture`,
    workspaceDir: '/sandbox/workspace', cwd: '/sandbox/workspace'});
  ...
  if (agentId === 'primary') {
    ...
    for (const denied of ['write', 'edit', 'exec', 'apply_patch', 'sessions_spawn', 'tool_search', 'tool_call']) {
      assert.equal(tools.find(tool => tool.name === denied), undefined);
    }
  }
}
assert.equal(await fs.readFile(sentinel, 'utf8'), 'owned-read-fixture');
console.log('Native OpenClaw: three agents, isolated tool catalogs, read execution, no write/exec/delegation tools.');
