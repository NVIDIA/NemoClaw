...
class ExecutionConfiguration(unittest.TestCase):
    ...
    def test_execution_and_default_drift_never_overwrite_native_state(self):
        for execution in [None, {'timeoutSeconds': 900}, {'heartbeatEvery': '30m'}, {'heartbeatEvery': '0m'}]:
            ...
            with tempfile.TemporaryDirectory() as directory, patch.object(adapter, 'ROOT', Path(directory)):
                ...
                self.assertEqual(original['models']['providers']['openshell']['timeoutSeconds'], timeout)
                self.assertEqual(configuration('main', 'openclaw', inference=options)['runtime']['timeout_seconds'], timeout + 60)
                if execution and 'heartbeatEvery' in execution:
                    self.assertEqual(defaults['heartbeat'], {'every': execution['heartbeatEvery'], 'isolatedSession': True})
                else:
                    self.assertNotIn('heartbeat', defaults)
                self.assertTrue(adapter.configuration_matches('main', options))
                for key, changed in [('timeoutSeconds', timeout + 1), ('heartbeat', {'every': '1s'})]:
...
class ExecutionInvocation(unittest.IsolatedAsyncioTestCase):
    async def test_configured_timeout_reaches_agent_request_and_rpc_wait(self):
        for execution in [None, {'timeoutSeconds': 900}]:
            ...
            timeouts = []
            rpc = runtime.rpc
            async def recording_rpc(method, params, timeout=280):
                timeouts.append(timeout)
                return await rpc(method, params, timeout)
            runtime.rpc = recording_rpc
            with patch.object(adapter, 'configuration_matches', return_value=True):
                await runtime.invoke(SimpleNamespace(input='hello'), SimpleNamespace(runtime_id='runtime-test', invocation_id='turn-one'))
...
@unittest.skipUnless(os.environ.get('NEMOCLAW_TEST_NATIVE_EXECUTION') == '1',
...
'''
        for execution, expected in [(None, [600000, 1800000]),
            ...
            result = subprocess.run([adapter.NODE, '--input-type=module', '-e', resolver],
                                    input=json.dumps(native), text=True, capture_output=True, check=True, timeout=15)
