...
export async function loadConfiguredModel(selected: AgentModelConfig, credentials: CredentialStore) {
  ...
  try {
    ...
    await writeFile(modelsPath, JSON.stringify({ providers: {
      [selected.provider]: {
        ...
        ...(metadata === undefined ? {} : { models: [{
          ...metadata as object,
          // The deployment owns identity and routing, not the opaque object.
          id: selected.model, baseUrl: selected.base_url,
        }] }),
      },
    } }), { mode: 0o600 });
    ...
    if (!model) throw new LifecycleError('pi_model_unknown',
