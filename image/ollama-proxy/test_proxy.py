...
from proxy import serve, load_key
    ...
    def test_auth_streaming_model_pin_and_external_ownership(self):
        ...
        class Upstream(BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                self.wfile.write(json.dumps({'models':[{'name':'qwen3:4b','digest':digest[0],'size':100}]}).encode())
            def do_POST(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory)
            settings={'upstream':f'http://127.0.0.1:{upstream.server_port}/v1','model':'qwen3:4b','digest':'a'*64}
            ...
            try:
                ...
                self.assertEqual(status,200); self.assertIn(b'data: [DONE]',body)
                self.assertEqual([call for call in calls if call[0]=='POST'],[('POST','/v1/chat/completions',None)])
                digest[0]='b'*64
            ...
            with self.assertRaises(RuntimeError): load_key(root)
