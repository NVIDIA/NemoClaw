...
def load_key(root):
    ...
    with os.fdopen(fd) as source:
        ...
        if (not stat.S_ISREG(info.st_mode) or info.st_mode & 0o777 != 0o600
                or info.st_uid != os.geteuid() or info.st_nlink != 1 or info.st_size != 64):
    ...
    for name in ('tcp', 'tcp6'):
        for line in Path('/proc/net', name).read_text().splitlines()[1:]:
            ...
            if fields[3] != '0A' or int(number, 16) != port:
                continue
            raw = b''.join(bytes.fromhex(address[i:i+8])[::-1] for i in range(0, len(address), 8))
...
def serve(settings, root, host, port):
    ...
    if (upstream.scheme != 'http' or upstream.path != '/v1' or upstream.query or upstream.fragment
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def dispatch(self):
            ...
            if len(auth) != 1 or not hmac.compare_digest(auth[0].encode(), ('Bearer '+key).encode()):
            ...
            if self.command == 'POST':
                try:
                    ...
                    if not 0 < size <= 4 << 20: return self.reply(413)
            ...
            if self.command == 'GET':
                return self.reply(200, json.dumps({'object':'list','data':[
                    {'id':settings['model'],'object':'model','owned_by':'ollama'}]}).encode())
            ...
            try:
                ...
                response = conn.getresponse()
                if 300 <= response.status < 400: return self.reply(502)
...
if __name__ == '__main__':
    try:
        ...
        raise SystemExit('Ollama proxy startup failed; retained resources require inspection') from None
