...
import { canonicalJson, sha256 } from "../advisors/canonical-json.mts";
import { buildRiskPlan, riskPlanRequiredJobIds } from "../advisors/risk-plan.mts";
import { discoverCredentialFreeTests } from "../e2e/credential-free-tests.mts";
...
import type { AdvisorFindingLedger } from "./finding-ledger.mts";
...
function credentialBearingRepairPathJob(headSha: string, changedPaths: readonly string[]) {
  ...
  return (
    riskPlanRequiredJobIds(buildRiskPlan({ headSha, changedFiles: changedPaths })).find(
      (job) => !credentialFree.has(job),
    ) ?? null
  );
}
