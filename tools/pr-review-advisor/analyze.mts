...
When the PR's summary states a correctness guarantee (fail-closed check, locality invariant, ordering constraint, capacity gate), identify the guarantee and enumerate the specific ways it could be silently bypassed: alternate code branches (e.g. cache-hit paths that skip the check), combined input states (e.g. multiple env vars set simultaneously with documented precedence that differs from implementation), and external system contract assumptions (e.g. what "unix://" actually proves about daemon locality). For each bypass path, verify against the diff whether it is closed, explicitly opted out under an authorized contract/non-goal, or unaddressed; unauthorized opt-outs remain unaddressed. When the implementation makes assumptions about an external system's behavior (env var precedence, API semantics, filesystem guarantees), verify the assumption against upstream documentation using read-only tools, not just internal code consistency.
