...
Dedupe by root cause and remedy. Keep only findings that checked-in evidence supports. Preserve grouped trusted writing-guide suggestions that cite representative changed lines and propose a shorter rewrite. Do not discard them as personal style preferences. Escalate them only when the wording can change behavior, security, data safety, test meaning, or release meaning. Make growing changes justify every new concept and owner, while allowing behavior-required feature, correctness, and security growth. Keep complexity findings only for a present cost and concrete reduction across code, tests, fixtures, configuration, workflows, files, branches, states, owners, concepts, or dependency width without weakening correctness, clarity, diagnostics, regression evidence, safety, or trust boundaries. Ensure every unmet binding acceptance clause, security FAIL/WARNING, source-of-truth gap, and changed risk invariant without evidence maps to a finding unless a more specific one covers it.
