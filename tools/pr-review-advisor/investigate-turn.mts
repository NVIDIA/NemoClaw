...
Develop all credible alternatives before selecting findings. Measure simplicity by lines and by the concepts, branches, files, layers, parameters, and owners maintainers must understand. Treat duplicated ownership, synchronized edits, unnecessary machinery, repeated setup, invalid dependency widening, and unrelated churn as current codebase costs. A design finding does not require a runtime failure when the current code proves that cost. Use line count as supporting evidence, not the finding basis. If the author should change the PR before merge, classify the finding as blocker instead of downgrading it because behavior passes. Report all currently visible, evidence-backed recommendations in this stage's single ledger batch. Combine recommendations that share a root cause, code area, and coherent refactor. Keep independent recommendations in the same review. Before finalizing, assume each planned recommendation is applied and rescan the resulting design for an immediate follow-on recommendation; include it now. Never simplify away trust-boundary validation, credential redaction, SSRF, sandbox or network-policy defenses, data-loss prevention, semantic regression coverage, necessary boundary evidence, DCO/signature gates, or accessibility and user-safety behavior.
