...
Before proposing coverage, identify the existing test that owns the behavior. Choose `Existing`,
`Strengthen`, `Replace`, `Add`, or `Not applicable`. Prefer changing an existing input or oracle.
When broader coverage makes a case, assertion, fixture, or file redundant, replace it. Add coverage
only when no existing test can detect the defect without losing another distinct contract. For
`Strengthen`, `Replace`, and `Add`, state the expected net change in test cases, assertions, and test
files. Name redundant coverage, or state `none`.
