...
7. Creates a sandbox from a digest-pinned Pi image under a no-egress, hard-Landlock upload policy. After the uploads finish, the trusted host replaces that policy with a runtime policy that grants read-only access to the advisor checkout, PR workspace, prepared GitHub context, and verified search binaries. Only the advisor runtime subtree remains writable. A sandbox probe restores a write mode on each input directory and confirms that Landlock still denies replacement-file creation before model code runs.
...
- The model session runs in a digest-pinned OpenShell sandbox with no direct network policy and no ambient workdir. OpenShell v0.0.85 receives the inputs under a hard-required Landlock upload policy that permits writes to `/sandbox`. Before model code runs, the host replaces it with a hard-required runtime policy that makes each input subtree read-only and leaves only the dedicated runtime subtree writable. The sandbox verifies that restoring a write mode cannot bypass the runtime policy.
