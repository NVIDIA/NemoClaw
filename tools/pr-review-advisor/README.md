...
7. Creates a sandbox from a digest-pinned Pi image under the same no-egress, hard-Landlock upload boundary used by the conflict fixer. Before model code runs, a trusted bootstrap removes write modes from the advisor checkout, PR workspace, prepared GitHub context, and verified search binaries, then probes that ordinary writes fail while the advisor runtime subtree remains writable. This mode sealing guards against accidental mutation; it is not a separate Landlock immutability boundary.
...
- The model session runs in a digest-pinned OpenShell sandbox under a hard-required Landlock policy with no direct network policy and no ambient workdir. OpenShell v0.0.85 requires `/sandbox` to remain policy-writable for its post-create uploads, so a trusted bootstrap seals all uploaded input trees with read-only file modes and verifies them before the Advisor starts. That seal prevents ordinary accidental writes but is not kernel-enforced immutability. The model-facing Advisor tools remain repository-confined and read-only; generated configuration and artifacts use the dedicated runtime subtree.
