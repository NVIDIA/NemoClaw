...
17. Uses a separate publisher job with no model credential or untrusted worktree.
    ...
    For terminology, it can show PR-SHA-bound decisions that only the second-opinion lane selected and cases where the lanes assigned different dispositions to the same term and changed location.
...
- `pr-review-advisor-terminology-ledger.json` — the canonical PR-SHA-bound terminology receipt, including decisions that reference a trusted trace, refreshed after every settled turn.
...
impact, recommendation, trace ID, and PR-SHA-bound source. The dispositions are `established`,
...
both lanes completed with PR-SHA-bound decisions. These differences remain advisory and do not
