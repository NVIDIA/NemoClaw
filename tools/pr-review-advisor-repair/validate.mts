...
function parseGitObject(
  ...
  return { mode: match[1]!, type: source === "tree" ? match[2]! : "blob" };
}
...
export function validateRepairLocally(input: {
  ...
  for (const [command, args] of dependencyCommands) {
    try {
      const result = runner(repository, command, args, env);
      commands.push(result);
      if (result.exitCode !== 0) {
        throw new ValidationSequenceError(
          "trusted dependency preparation returned failure",
          commands,
        );
      }
    } catch (error) {
      if (error instanceof ValidationCommandError) commands.push(error.command);
      if (error instanceof ValidationSequenceError) throw error;
      throw new ValidationSequenceError(
        error instanceof Error ? error.message : "trusted dependency preparation failed",
        commands,
      );
    }
    const status = String(
  ...
  for (const [command, args] of repositoryCommands) {
    try {
      const result = runner(repository, command, args, env);
      commands.push(result);
      if (result.exitCode !== 0) {
        throw new ValidationSequenceError("trusted validation command returned failure", commands);
      }
    } catch (error) {
      if (error instanceof ValidationCommandError) commands.push(error.command);
      if (error instanceof ValidationSequenceError) throw error;
      throw new ValidationSequenceError(
        error instanceof Error ? error.message : "trusted validation command failed",
        commands,
      );
    }
    const status = String(
