...
import { gitIsolationEnvironment, HERMETIC_GIT_ARGS } from "../advisors/hermetic-git.mts";
import { createGitHubTree } from "../pr-merge-conflict-fixer/publish.mts";
import {
...
export async function ensureVerifiedRepairCommit(input: {
  ...
  if (input.liveHeadSha === input.receipt.sourceHeadSha) {
    await createGitHubTree({
      baseSha: input.receipt.sourceHeadSha,
      finalTree: input.candidateTreeSha,
      headSha: input.receipt.sourceHeadSha,
      repository: input.repository,
      repositoryName: CANONICAL_REPOSITORY,
      request: async (method, apiPath, body) =>
        input.request(apiPath.slice(1), input.token, { method, body }),
    });
