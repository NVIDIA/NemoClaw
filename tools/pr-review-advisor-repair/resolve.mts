...
import { resolverModelConfiguration } from "../pr-merge-conflict-fixer/resolve.mts";
import {
...
export function assertFirstRepairRun(env: NodeJS.ProcessEnv): void {
  if (env.GITHUB_RUN_ATTEMPT !== "1") {
    throw new RepairContractError("repair generation is disabled on workflow reruns");
  }
}
...
export function resolverGitEnvironment(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const name of ["LANG", "LC_ALL", "PATH", "TZ"]) {
    ...
    if (value && safeEnvironmentValue(value)) clean[name] = value;
  }
  return {
    ...clean,
...
export const repairModelConfiguration = (): string => resolverModelConfiguration();
...
export function downloadRepairCandidate(
  ...
  downloadOpenShellPath(
    ...
    {
      name,
      source: "/sandbox/output/proposal.json",
      destination: `${proposalDirectory}/`,
    },
    tools,
  );
}
...
async function main(): Promise<void> {
  ...
  switch (command) {
    case "prepare":
      ...
      return;
    case "lifecycle":
      assertFirstRepairRun(process.env);
