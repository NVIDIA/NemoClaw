...
parser.add_argument('--hermes-relay', action='store_true', help='Exercise in-process Hermes Relay tracing')
parser.add_argument('--interfaces', action='store_true', help='Exercise the native dashboard with sandbox-local authentication')
...
cmd = ['docker', 'run', '--name', name, '--rm', '--runtime=runc', '--network', 'none', *(['--user', '0', '--cap-add', 'NET_ADMIN'] if h == 'openclaw' else []),
       ...
       '-e', f'FABRIC_HERMES_DASHBOARD={args.hermes_dashboard}', '-e', f'FABRIC_HERMES_TUI={args.hermes_tui}', '-e', f'FABRIC_TEST_INTERFACES={int(args.interfaces)}', '-e', f'FABRIC_INFERENCE_API={args.inference_api or ""}', '-e', f'FABRIC_PI_CATALOG={int(args.pi_catalog)}', '-e', 'HOME=/sandbox', '-e', 'TMPDIR=/sandbox/tmp', '-e', 'ADAPTER_PYTHON=/opt/fabric/bin/python',
       '-e', f'FABRIC_HERMES_RELAY={int(args.hermes_relay)}',
