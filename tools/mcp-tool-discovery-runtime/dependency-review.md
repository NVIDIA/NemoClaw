...
The installer applies the existing public corporate CA build argument to npm TLS when present. It also uses IPv4-first DNS resolution, a four-socket registry connection budget, and bounded fetch timeouts for connection-constrained self-hosted builders. The shared locked-install helper gives both the OpenClaw plugin builder and the reviewed MCP runtime one retry for npm's exact internal exit-handler failure, preferring the cache populated by the first attempt; all lockfile, registry-signature, type, bundle, and production-audit gates still run after a successful install and remain fail closed.
