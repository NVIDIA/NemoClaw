...
- `MCP-AUDIT-4` — Live advisory and Sigstore TUF queries in the image build can prevent a released version from reproducing the same image after external metadata or network availability changes. Surface: `install-reviewed-runtime.sh`. Resolution: move both advisory enforcement and registry-signature verification for this exact lock to the trusted reviewed npm audit CI check. The image build remains fail-closed on the committed lock and SHA-512 archive integrity, then runs the runtime test, typecheck, and exact bundle gates without live advisory or Sigstore TUF lookups. The protected imported-cache rebuild supplies the complete locked npm cache seed and runs with `--network none`; ordinary builds retain bounded access to lock-pinned registry archives. The helper enters recovery only when the initial `npm ci` output contains `Exit handler never called!`. It then retries `npm ci --offline` while filling at most eight uniquely lock-pinned missing archives. Each archive receives at most four attempts for the listed transient network errors. Other initial install failures, non-`ENOTCACHED` offline failures, ambiguous lock entries, integrity failures, and non-transient fetch failures stop immediately. Validation: `test/mcp-tool-discovery-image-contract.test.ts`.

## 2026-08-05 build audit boundary update

Issue #8253 showed that image-build audits made sandbox creation depend on current registry and Sigstore TUF data instead of only the committed build inputs. The shared installer no longer runs live advisory or registry-signature queries. It installs the exact lock with lifecycle scripts disabled from integrity-pinned archives, runs the runtime tests and typecheck, and verifies the exact bundle inputs.
...
Every agent image installs this committed graph with `npm ci --ignore-scripts` through the same reviewed installer, verifies every archive against the lock's SHA-512 integrity, typechecks the package against the real SDK types, and produces a single Node.js ESM bundle. The complete lock-derived Linux x64 graph and its Linux arm64 esbuild binary are committed so trusted protected multi-architecture and GPU rebuilds remain fully offline even when the workflow controller predates seed-artifact import support. The hosted protected build also exports the target-specific seed into its exact-run cache artifact for newer controllers, and protected rebuilds validate every archive before disabling BuildKit networking. The trusted reviewed npm audit CI gate verifies registry signatures for that exact lock before main or tag production image publication and before mutable cohort promotion. Only the bundle, its checked package manifest, and a deterministic notice containing the license text for every package in esbuild's actual input graph are copied into the final image; the build dependency tree is discarded with the builder stage. This preserves the official SDK implementation and its license obligations while avoiding a large production layer composed of thousands of small package files.
The installer applies the existing public corporate CA build argument to npm TLS when present. It also uses IPv4-first DNS resolution, a four-socket registry connection budget, and bounded fetch timeouts for connection-constrained self-hosted builders. After npm's exact internal exit-handler failure, the shared locked-install helper completes the OpenClaw plugin builder, its production dependency layer, and the reviewed MCP runtime from cache. If an offline retry identifies one missing registry archive, the helper verifies that exact URL and integrity entry in `package-lock.json`, fetches only that archive, and retries offline; the cache-completion loop is capped at eight archives and fails closed on ambiguous or unpinned input. Each archive fetch disables npm's internal retry loop and instead permits four short, bounded attempts only for transient network error codes. Integrity and other non-network failures remain fail-closed. The type and bundle gates still run after a successful install, while the trusted reviewed npm audit CI gate owns exact-lock registry-signature verification.
...
The image build requires lock-pinned archive integrity, the case-variant SSE session test, a root-owned non-writable bundled runtime, and an executable invalid-input contract check before it can complete. The reviewed npm audit CI check verifies registry signatures and separately evaluates the locked production graph against the repository's advisory policy. The production base-image publication workflow requires that check before base-image platform-digest uploads, mutable base-image tags, or managed-image promotion for the same commit.
