...
- `MCP-AUDIT-4` — A dependency-only remediation could weaken the image build's fail-closed audit boundary. Surface: `install-reviewed-runtime.sh`. Resolution: retain `npm audit --omit=dev --audit-level=low` and bind it with the reviewed manifest and lock in the image contract test. The installer retries only npm's exact internal `Exit handler never called!` failure once; every other install failure remains immediate, and the signature, test, typecheck, bundle, and production-audit gates still run after a successful retry. Validation: `test/mcp-tool-discovery-image-contract.test.ts`.
