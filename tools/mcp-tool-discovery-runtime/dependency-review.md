...
The installer applies the existing public corporate CA build argument to npm TLS when present. It also uses IPv4-first DNS resolution, a four-socket registry connection budget, and bounded fetch timeouts for connection-constrained self-hosted builders. The shared locked-install helper gives the OpenClaw plugin builder, its production dependency layer, and the reviewed MCP runtime one cache-only offline retry for npm's exact internal exit-handler failure. An incomplete cache fails immediately; all lockfile, registry-signature, type, bundle, and production-audit gates still run after a successful install and remain fail closed.
