...
- `MCP-AUDIT-4` — A live advisory query in the image build can prevent a released version from creating the same image after npm publishes a new advisory. Surface: `install-reviewed-runtime.sh`. Resolution: keep registry signature verification in the image build and move production advisory enforcement for this exact lock to the reviewed npm audit CI check. The helper enters recovery only when the initial `npm ci` output contains `Exit handler never called!`. It then retries `npm ci --offline` while filling at most eight uniquely lock-pinned missing archives. Each archive receives at most four attempts for the listed transient network errors. Other initial install failures, non-`ENOTCACHED` offline failures, ambiguous lock entries, integrity failures, and non-transient fetch failures stop immediately. The signature, test, typecheck, and bundle gates still run after installation succeeds. Validation: `test/mcp-tool-discovery-image-contract.test.ts`.
...
The image build requires npm registry signatures to be verified, the case-variant SSE session test, a root-owned non-writable bundled runtime, and an executable invalid-input contract check before the image can complete. The reviewed npm audit CI check separately evaluates the locked production graph against the repository's advisory policy and gates base-image platform-digest uploads, mutable base-image tags, and managed-image publication for the same commit.
