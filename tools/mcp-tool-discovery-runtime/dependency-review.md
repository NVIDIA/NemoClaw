...
The image build requires verified npm registry signatures, the case-variant SSE session test, a root-owned non-writable bundled runtime, and an executable invalid-input contract check before the image can complete. The reviewed npm audit CI check separately evaluates the locked production graph against the repository's advisory policy.
