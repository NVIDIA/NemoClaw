...
export const TRUSTED_HERMES_SWAP_SCRIPT = [
  ...
  "  /usr/bin/sudo -n /usr/sbin/swapon --show --bytes --noheadings --output SIZE |",
  ...
  '    if active_swap_names="$(/usr/bin/sudo -n /usr/sbin/swapon --show --noheadings --raw --output NAME 2>/dev/null)"; then',
  "      fixed_swap_active=0",
  ...
  "",
  'active_swap_names="$(/usr/bin/sudo -n /usr/sbin/swapon --show --noheadings --raw --output NAME)"',
  ...
  "  fi",
  'done <<< "${active_swap_names}"',
  "if (( provisioned_swap_active != 1 )); then",
  '  fail "new swap file is not active"',
  "fi",
  'active_swap_bytes="$(read_active_swap_bytes)"',
  'active_swap_bytes="${active_swap_bytes:-0}"',
  'if [[ ! "${active_swap_bytes}" =~ ^[0-9]+$ || "${active_swap_bytes}" -lt "${required_swap_bytes}" ]]; then',
  '  fail "unable to verify the required active swap capacity"',
  "fi",
  "",
  "trap - EXIT",
