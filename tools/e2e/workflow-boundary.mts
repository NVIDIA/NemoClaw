...
function requireCanonicalDockerHubAuthRun(
  ...
  const authKeys = Object.keys(authStep).sort();
  if (
    authKeys.length !== 3 ||
    authKeys[0] !== "env" ||
    authKeys[1] !== "name" ||
    ...
    errors.push("canonical Docker Hub auth step must contain exactly name, uses, and env");
  }
