...
async function updateRunningCheck(
  ...
  await githubApi(`repos/${context.repository}/check-runs/${context.checkRunId}`, token, {
    ...
    body: {
      ...
      output: {
        title: `Running ${selectionCount} E2E ${selectionCount === 1 ? "check" : "checks"}`,
        summary: `Risk plan ${options.planHash} selected jobs: ${options.jobs.join(", ") || "none"}; targets: ${options.targets.join(", ") || "none"}.`,
      },
    },
    userAgent: USER_AGENT,
  });
