...
import { manualTargets } from "./target-definitions/manual.mts";
...
const profileTargets: readonly E2eCatalogueTarget[] = [
...
validateE2eTargetCatalogue(profileTargets);
...
export type SharedE2eTarget = E2eExecutionMetadata & {
  id: string;
  file: string;
...
const sharedTargets: SharedE2eTarget[] = [
  {
...
export interface ManualE2eTarget {
  id: string;
...
export type E2eInventoryTarget =
  ...
  | { id: string; route: "manual"; definition: ManualE2eTarget };

export function buildExecutionInventory(
  targets: readonly E2eInventoryTarget[],
): Map<string, E2eInventoryTarget> {
  const entries = new Map<string, E2eInventoryTarget>();
  for (const entry of targets) {
    ...
    if (entry.route === "external-workflow" || entry.route === "manual") {
      if (entry.definition.tests.length === 0)
        throw new Error(`Execution target ${entry.id} requires test files`);
      for (const test of entry.definition.tests) {
        if (!/^test\/[a-zA-Z0-9_/-]+\.test\.ts$/.test(test.file))
          throw new Error(`Execution target ${entry.id} has an invalid test path: ${test.file}`);
      }
    }
...
const executionInventory = buildExecutionInventory([
  ...
  ...profileTargets.map((definition) => ({
    id: definition.id,
    route: "profile" as const,
    definition,
  })),
  ...canonicalTargets().map((definition) => ({
    id: definition.id,
...
export function sharedTarget(id: string): SharedE2eTarget {
  ...
  if (entry?.route !== "shared")
    throw new Error(`Credential-free test ${id} requires execution coverage metadata`);
  return entry.definition;
}

export function sharedTargetRows(): Pick<SharedE2eTarget, "id" | "file" | "project">[] {
  return listExecutionTargets().flatMap((entry) =>
    entry.route === "shared"
      ? [{ id: entry.id, file: entry.definition.file, project: entry.definition.project }]
      : [],
  );
}

export function reconcileSharedTargetDiscovery(
  discovered: readonly Pick<SharedE2eTarget, "id" | "file" | "project">[],
  expected = sharedTargetRows(),
): typeof expected {
  const found = new Map(discovered.map((row) => [row.id, row]));
  if (found.size !== discovered.length)
    throw new Error("Duplicate discovered shared E2E target IDs");
  for (const row of discovered) {
    if (!expected.some((candidate) => candidate.id === row.id))
      throw new Error(
        `Discovered shared E2E target ${row.id} has no inventory disposition: ${row.file}`,
      );
  }
  for (const row of expected) {
    if (!isDeepStrictEqual(found.get(row.id), row))
      throw new Error(
        `Shared E2E target ${row.id} does not match discovered file and project: ${row.file}`,
      );
  }
  return expected;
}

export const E2E_TARGET_CATALOGUE = profileTargets.map(({ id }) => {
  const entry = executionInventory.get(id)!;
  if (entry.route !== "profile") throw new Error(`Expected profile route for ${id}`);
  return entry.definition;
});
...
export function listTargets(): TargetDefinition[] {
  return listExecutionTargets().flatMap((entry) =>
    entry.route === "typed" ? [entry.definition] : [],
  );
}

export function getTarget(id: string): TargetDefinition | undefined {
  const entry = executionInventory.get(id);
  return entry?.route === "typed" ? entry.definition : undefined;
}
...
export function requireTargets(ids: string[]): TargetDefinition[] {
  return ids.map((id) => {
    ...
    if (!found)
      throw new Error(
        `Unknown target '${id}'. Available targets: ${listTargets()
