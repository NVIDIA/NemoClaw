...
type LivePullRequest = {
  base: { ref: string; repo: { full_name: string; node_id?: string } };
  head: { ref: string; repo: { full_name: string } | null };
  draft: boolean;
  state: string;
};

type LiveRef = { object: { sha: string } };
...
}): { finalTree: string; repository: string } {
...
async function publishValidatedTree(input: {
  ...
  if (result.updateRefs?.clientMutationId !== commitSha)
...
async function main(): Promise<void> {
  ...
  const commitSha = await publishResolution({
    ...
    patchPath: path.join(artifactDirectory, "resolution.patch"),
    repositoryName,
