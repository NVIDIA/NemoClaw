...
BOUNDARY_MANIFEST_NAME = "openshell-child-visible-credentials.v0.0.82.json"
...
def _load_credential_boundary_manifest() -> dict[str, object]:
    ...
    # whyNotSourceFix: OpenShell v0.0.82 has no machine-readable child-env contract.
    ...
    if (
        ...
        or manifest.get("openshellVersion") != "0.0.82"
    ):
...
def _validate_payload(action: str, payload: dict[str, object]) -> None:
    ...
    if action == "add" and hostname in host_aliases:
        raise ValueError(
            "Authenticated MCP OpenShell host aliases are unavailable with OpenShell v0.0.82"
        )
...
def _assert_non_root_lifecycle_identity() -> None:
    ...
    # whyNotSourceFix: OpenShell 0.0.82 supports both topologies but exposes no
