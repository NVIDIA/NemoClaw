...
def _validate_payload(action: str, payload: dict[str, object]) -> None:
    ...
    if action == "add" and revisioned_authorization_match is not None:
        credential_name = revisioned_authorization_match.group(1)
...
def _managed_candidate_matches(
    ...
    actual_match = (
        REVISIONED_ENV_PLACEHOLDER_RE.fullmatch(actual_authorization)
        if isinstance(actual_authorization, str)
        else None
    )
    return (
        expected_match is not None
        and actual_match is not None
