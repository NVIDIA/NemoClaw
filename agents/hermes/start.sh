...
verify_hermes_config_integrity() {
  # cap_dac_override is dropped early; root cannot read sandbox-owned 640 files.
  # Verify using the sandbox user while anchoring against the root-owned hash.
  if [ "$(id -u)" -eq 0 ]; then
    gosu sandbox bash -c "source \"${_SANDBOX_INIT}\"; verify_config_integrity \"${HERMES_IMMUTABLE}\" \"${HERMES_HASH_FILE}\""
  else
...
deploy_config_to_writable() {
  if [ -L "${HERMES_WRITABLE}" ]; then
    echo "[SECURITY] refusing writable Hermes home symlink: ${HERMES_WRITABLE}" >&2
    return 1
  fi

  mkdir -p "${HERMES_WRITABLE}"

  # When running as root, use gosu to write as sandbox user (owner of .hermes-data).
  if [ "$(id -u)" -eq 0 ]; then
    chown sandbox:sandbox "${HERMES_WRITABLE}"
    chmod 770 "${HERMES_WRITABLE}" 2>/dev/null || true
...
rewrite_rc_marker_block() {
  ...
  tmp="$(mktemp "/tmp/.${base}.tmp.XXXXXX")" || return 1
  ...
  if ! tee "$rc_file" >/dev/null <"$tmp" 2>/dev/null; then
    if [ "$(id -u)" -eq 0 ] \
      && gosu sandbox bash -c "chmod u+w \"${rc_file}\" 2>/dev/null || true; cat \"${tmp}\" > \"${rc_file}\"" 2>/dev/null; then
      :
    else
      rm -f "$tmp"
      return 1
...
validate_hermes_symlinks() {
  validate_config_symlinks /sandbox/.hermes /sandbox/.hermes/runtime
}
