...
async function ensureInferenceCredential(loaded, refreshToken, options = {}) {
  ...
  // Newer Portal credentials can invoke inference directly. Preserve the
  // agent-key fallback for older or malformed credentials.
  ...
  updateOpenshellInferenceProvider(loaded.state, credential, inferenceBaseUrl);
  const nextState = {
    ...loaded.state,
...
async function handleControlRequest(req, res) {
  ...
  if (req.url === "/credentials/register") {
    ...
    try {
      await ensureInferenceCredential(loaded, refreshToken);
    } catch (error) {
      restoreRuntimeCredential();
      throw error;
    }
