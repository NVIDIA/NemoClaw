...
- Advisory review date: 2026-09-02.
- Advisory result: `1` moderate, `0` high, and `0` critical vulnerabilities across the resolved production dependency graph. The configured high-severity gate passes without an exception.
...
- Security override: `fast-uri@3.1.7` (`sha512-dOvZVzjdZdz7phd9v6jCbwxrBW3fK6n8Rc0CtdmM4bumzMnxywBYhuph6J819RRw/ku+rLbelwfMunktuzVVHg==`) replaces the previously reviewed `3.1.5` resolution. It remains outside the earlier `GHSA-v2hh-gcrm-f6hx` and `GHSA-7p8r-x3mc-p8w7` ranges. `GHSA-5jgf-p345-68v8`, `GHSA-f65p-4m7j-42xc`, `GHSA-fph4-wmhf-6fwf`, and `GHSA-jqff-g426-hqxp` affect `3.1.5`. Version `3.1.6` is the first patched release; `3.1.7` adds the adjacent port and IP-literal validation fixes. The replacement remains within Ajv's declared `^3.0.1` range and preserves the v3 package boundary. Remove the override only when the declared graph resolves to a reviewed release outside every recorded affected range.
