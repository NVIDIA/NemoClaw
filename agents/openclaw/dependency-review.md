...
- Advisory result: `1` moderate, `0` high, and `0` critical findings across the resolved production dependency graph. The configured high-severity gate passed, and npm registry signature verification succeeded.
...
- Security override: `fast-uri@3.1.6` (`sha512-7Ical1vFEMr0onbVzEDIreM22I4khW+fzyQPwvAFWBp1iwdshSZRsL4jjRvPG9JP1uiqMHRto+YU6R2/CzDz5Q==`) replaces Ajv's vulnerable `3.1.3` resolution and the earlier `3.1.4` and `3.1.5` remediations. In addition to `GHSA-v2hh-gcrm-f6hx` and `GHSA-7p8r-x3mc-p8w7`, `GHSA-5jgf-p345-68v8`, `GHSA-f65p-4m7j-42xc`, `GHSA-fph4-wmhf-6fwf`, and `GHSA-jqff-g426-hqxp` affect `3.1.5`. The replacement remains within Ajv's declared `^3.0.1` range, has no dependencies, keeps the BSD-3-Clause license, and preserves the reviewed v3 API boundary. Remove the override when the declared graph resolves to a reviewed release outside all affected ranges.
