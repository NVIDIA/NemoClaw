...
- `GHSA-frvp-7c67-39w9` and `GHSA-9mqv-5hh9-4cgg` remediation: the locked `@modelcontextprotocol/sdk@1.29.0` still requests vulnerable `@hono/node-server@^1.19.9`. The former `2.0.5` override became vulnerable when the latter advisory was published through `2.0.9`, so the runtime manifest now pins the first release outside both affected ranges, `2.0.10`. Node `>=20` remains inside the image's Node `>=22.19.0` contract; real ESM and CommonJS Streamable HTTP transport construction/start/close probes plus `mcporter --version` validate the major override. Remove it when the SDK's declared range resolves to a reviewed patched release.
