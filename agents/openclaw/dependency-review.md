...
- `GHSA-frvp-7c67-39w9` and `GHSA-9mqv-5hh9-4cgg` remediation: the locked `@modelcontextprotocol/sdk@1.29.0` still requests vulnerable `@hono/node-server@^1.19.9`. The former `2.0.5` override became vulnerable when the latter advisory was published through `2.0.9`, so the runtime manifest now pins reviewed `2.0.11` (`sha512-bjD221KPLoJTWUwso1J6fGKiTXEUFedG/s0visavY4zakFPkeGURMRNly+FhBHs7T8Dz4qHaZIMX9ZoJHSJtKA==`), outside both affected ranges. Node `>=20` remains inside the image's Node `>=22.19.0` contract; real ESM and CommonJS Streamable HTTP transport construction/start/close probes plus `mcporter --version` validate the major override. Remove it when the SDK's declared range resolves to a reviewed patched release.
- `GHSA-v2hh-gcrm-f6hx` remediation: the same SDK's locked AJV graph requests `fast-uri@^3.0.1`; the runtime manifest pins the first release outside the affected `3.0.0` through `3.1.3` range, `3.1.4`. Remove the override when the declared graph resolves to a reviewed patched release.
