...
function parseRuntimeIdentityProfile(
  ...
  if ((policy.flow ?? "oauth2-refresh-token") === "oauth2-refresh-token") {
    if (
...
export async function prepareRuntimeIdentity(
  ...
  const isRefreshFlow = (profilePolicy.flow ?? "oauth2-refresh-token") === "oauth2-refresh-token";
  const refreshToken =
  ...
  if (!isRefreshFlow) {
    const subjectToken =
      config.subject_token_env === undefined
        ? undefined
        : requiredEnvironmentValue(config, config.subject_token_env, env);
    const tokenUrl = await requireOktaOBOConfig(config, profilePolicy, deps);
    if (!subjectToken || !clientSecret || !config.audience || !config.scopes) {
      throw new Error("Runtime identity token exchange configuration is invalid");
    }
