...
They prove that a child launched after rotation receives a different generation-scoped placeholder and that the protected resource receives both bearers.
...
- Credential rotation gives a new child a new generation-scoped placeholder, and `GET /v1.0/me` succeeds with the rotated credential.
...
After rotation, launch a new child process to receive the new generation-scoped placeholder.
