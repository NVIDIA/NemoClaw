...
`TC-INF-13` additionally uses the Entra v2 token-path shape, `ENTRA_ACCESS_TOKEN`, and the exact Microsoft Graph `GET /v1.0/me` credential-delivery rule. It proves that a `GET /v1.0/users` request is rejected before its bearer reaches the fixture.
