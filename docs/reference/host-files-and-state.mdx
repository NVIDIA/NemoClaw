...
| `<selected-state-root>/onboard-failures/` | Owner-only, bounded sandbox-create failure bundles containing a summary plus identity-bound gateway and rootfs-console evidence when available. These runtime artifacts are not automatically redacted; review them before sharing. | Delete an individual bundle only after you no longer need its failure evidence. |
<AgentOnly variant="hermes">
