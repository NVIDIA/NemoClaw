...
| `<selected-state-root>/onboard-failures/` | Owner-only failure diagnostics. Sandbox-create bundles contain a summary plus fixed structural failure signatures derived from at most 1 MiB of identity-bound gateway-log tail and 256 KiB of confined rootfs-console tail; raw runtime output is omitted. State-directory evidence is limited to 200 entries. Gateway and console truncation are recorded in `summary.txt` and reported in failure output. After writing a sandbox-create bundle, NemoClaw preserves that current bundle plus the nine newest prior timestamped entries and warns when pruning cannot complete. Other diagnostic writers do not continuously enforce that limit. | Delete an individual bundle only after you no longer need its failure evidence. |
<AgentOnly variant="hermes">
