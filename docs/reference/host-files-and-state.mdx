...
| `~/.nemoclaw/gateways/<port>/automatic-gateway-port.pending` | Owner-only identity for an installer-selected alternate port whose onboarding has started but not completed. When `NEMOCLAW_GATEWAY_PORT` is unset, the installer and CLI use this port so interrupted onboarding can resume or be cleaned up in the same state root. Successful installer-driven onboarding promotes it to `automatic-gateway-port`. | No. Do not remove it manually. To target another gateway, set `NEMOCLAW_GATEWAY_PORT` explicitly; to discard the interrupted attempt, use its reported resume, fresh-onboarding, or uninstall guidance. |
