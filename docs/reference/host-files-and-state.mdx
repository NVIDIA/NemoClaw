...
| `~/.nemoclaw/gateways/<port>/automatic-gateway-port.pending` | Owner-only identity for an installer-selected alternate port whose onboarding is deferred or incomplete. When `NEMOCLAW_GATEWAY_PORT` is unset, the installer and CLI use this port so later or resumed onboarding and cleanup stay in the same state root. Successful installer-driven onboarding promotes it to `automatic-gateway-port`. | No. Do not remove it manually. To target another gateway, set `NEMOCLAW_GATEWAY_PORT` explicitly; to discard the pending attempt, use its reported resume, fresh-onboarding, or uninstall guidance. |
