...
Earlier releases can store these files under `~/.nemoclaw/gateways/<port>/` when the managed pair was created from a non-default gateway port.
Current releases discover and validate that existing location without moving or duplicating its ownership state.
Do not move these files manually.
