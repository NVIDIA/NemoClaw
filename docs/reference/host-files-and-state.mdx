...
| `~/.nemoclaw/snapshots/` | Copies of host `~/.openclaw` state created by blueprint migration and rollback flows. NemoClaw excludes known authentication-state files and strips recognized credential fields from `openclaw.json`. | Only after you no longer need the corresponding rollback or restore point. The host CLI does not expose the direct runner's retention actions. |
