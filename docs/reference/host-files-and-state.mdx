...
| `<selected-state-root>/onboard-failures/` | Owner-only, bounded sandbox-create failure bundles containing a summary plus redacted, identity-bound gateway and rootfs-console evidence when available. NemoClaw retains the ten newest bundles and warns when pruning cannot complete. Review runtime diagnostics before sharing. | Delete an individual bundle only after you no longer need its failure evidence. |
<AgentOnly variant="hermes">
