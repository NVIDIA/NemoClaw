...
| `~/.nemoclaw/dual-station-vllm-api-key` | Owner-only host-global bearer API key shared by authenticated NemoClaw-managed vLLM profiles. | No while any managed vLLM runtime uses it. Destroying the last Local vLLM sandbox removes it with the authenticated single-host container unless `--keep-vllm` or `NEMOCLAW_KEEP_VLLM=1` keeps that container. Full uninstall removes it only after matching runtime cleanup succeeds and no sibling gateway remains. |
