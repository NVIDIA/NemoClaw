...
| `~/.nemoclaw/dual-station-vllm-api-key` | Owner-only host-global bearer API key shared by authenticated NemoClaw-managed vLLM profiles. | No while any managed vLLM runtime uses it. Destroying the last Local vLLM sandbox removes it with the authenticated single-host container, or alone when that container is already absent, unless `--keep-vllm` or `NEMOCLAW_KEEP_VLLM=1` keeps that container. Full uninstall removes it only after matching runtime cleanup succeeds and no sibling gateway remains. |
...
| `~/.nemoclaw/host-local-vllm-pending-retirement.json` | Owner-only, credential-free record of the Local vLLM sandbox name whose `destroy` removed the last registry row before the managed `nemoclaw-vllm` container was retired. | No while the record exists. A `destroy` retry for that sandbox name reads it, retires the container after the cause is resolved, and removes the record. The record is also removed when the retry finds another Local vLLM sandbox or receives `--keep-vllm`. Full uninstall removes it with the other host-global state. |
