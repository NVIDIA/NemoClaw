...
| `~/.nemoclaw/onboard-session.json` | Active onboarding session or rebuild journal. Interrupted rebuilds for other sandboxes can be retained separately. | Do not delete it to resolve an interrupted rebuild. For an ordinary failed onboarding attempt, prefer `$$nemoclaw onboard --fresh`. |
| `~/.nemoclaw/.onboard-rebuild-<sandbox>.json` | Retained rebuild session for one sandbox. NemoClaw restores it to the active session when that sandbox rebuilds. For a non-default gateway port, it resides under `~/.nemoclaw/gateways/<port>/`. | No. Rerun `$$nemoclaw <sandbox> rebuild` with the same settings and preserve the recovery files. |
