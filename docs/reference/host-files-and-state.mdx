...
| `~/.nemoclaw/rebuild-backups/` | Internal whole-native-state handoffs written by `backup-all` and rebuild flows. | Only after you no longer need rebuild recovery. |
...
- [Transfer State Manually](../manage-sandboxes/state-and-backups/transfer-state-manually) before removing state you may need later.
