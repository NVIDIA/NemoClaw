...
Current releases leave existing migration snapshots under `~/.nemoclaw/snapshots/` unchanged.
The direct blueprint runner retains these action arguments to inspect or remove the legacy copies:
...
Both deletion commands are irreversible.
They do not modify a running sandbox, but they remove the selected legacy copies.

Snapshot deletion requires `python3` on a POSIX host.
Listing works on native Windows, but deletion does not; use WSL to delete legacy snapshots on Windows.
