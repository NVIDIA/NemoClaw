...
Uninstall works on one gateway port: it removes the selected state root and leaves every other `~/.nemoclaw/gateways/<port>/` root and its gateway in place.
It names the environments it left behind and prints the command that removes each one.
Pass `--all-gateway-ports`, or set `NEMOCLAW_UNINSTALL_ALL_GATEWAY_PORTS=1`, to remove every gateway port and the shared host state in one run.
