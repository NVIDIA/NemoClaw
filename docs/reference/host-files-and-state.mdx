...
A `sandboxes.json` file that is present but does not contain valid JSON stops the commands that read or write the sandbox registry, such as `$$nemoclaw list` and `$$nemoclaw onboard`.
NemoClaw reports the file path and the recovery commands instead of reading the file as an empty registry.
No later command then replaces your sandbox records with empty state.
