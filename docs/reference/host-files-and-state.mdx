...
| `<selected-state-root>/onboard-failures/` | Owner-only, bounded sandbox-create failure bundles containing a summary plus redacted, identity-bound gateway and rootfs-console evidence when available. Review runtime diagnostics before sharing. | Delete an individual bundle only after you no longer need its failure evidence. |
<AgentOnly variant="hermes">
