...
| `~/.nemoclaw/ollama-proxy-token` | Host-global auth token used by the host-side Ollama auth proxy. Every gateway port on the host uses this one file. | Yes, but re-run onboarding for every gateway port that uses Local Ollama afterward, because a new token invalidates the shared proxy for all of them. |
