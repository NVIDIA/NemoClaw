...
| `~/.nemoclaw/gateways/<port>/automatic-gateway-port.pending` | Owner-only identity for an installer-selected alternate port whose onboarding is deferred or incomplete. When `NEMOCLAW_GATEWAY_PORT` is unset, the installer and CLI use this port so later or resumed onboarding and cleanup stay in the same state root. Successful installer-driven or direct CLI onboarding promotes it to `automatic-gateway-port`. | No. Do not remove it manually. Fresh onboarding retains this automatic port and promotes it on success. To discard the pending environment, follow its reported gateway-scoped uninstall guidance; set `NEMOCLAW_GATEWAY_PORT` explicitly only when you need to target another gateway. |
| `~/.nemoclaw/gateways/<port>/automatic-gateway-port` | Owner-only completed identity for an installer-selected alternate port. When `NEMOCLAW_GATEWAY_PORT` is unset, later installer and CLI processes restore this port instead of using `8080`. | No. Do not remove it manually. Set `NEMOCLAW_GATEWAY_PORT` explicitly when you need to target another gateway. Uninstalling this gateway removes the marker with its port-scoped state. |
