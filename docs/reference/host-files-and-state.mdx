...
| `<selected-state-root>/onboard-failures/` | Owner-only failure diagnostics. Sandbox-create bundles contain a summary plus redacted, identity-bound evidence when available: up to 1 MiB of gateway-log tail, 256 KiB of rootfs-console tail, and 200 state-directory entries. Gateway and console truncation are recorded in `summary.txt` and reported in failure output. After writing a sandbox-create bundle, NemoClaw preserves that current bundle plus the nine newest prior timestamped entries and warns when pruning cannot complete. Other diagnostic writers do not continuously enforce that limit. Review runtime diagnostics before sharing. | Delete an individual bundle only after you no longer need its failure evidence. |
<AgentOnly variant="hermes">
