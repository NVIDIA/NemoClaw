...
Portable cleanup instead performs exact receipt-owned durable retirement, preserves model stores, Podman images, and shared OpenShell resources, and retains `~/.nemoclaw/portable-uninstall-retirement.json` even with `--destroy-user-data` so a retry or later completed onboarding can finish recovery.
