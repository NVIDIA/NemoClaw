...
| `~/.nemoclaw/dual-spark-vllm-runtime.json` | Owner-only host-global dual-DGX Spark cleanup receipt. It contains no serving API key and binds the exact peer, plan, and container identities. | No while the managed pair exists. Full uninstall removes it after both exact containers are removed. |
