...
| External telemetry and observability export | Supported with caveats | OpenClaw can emit conversation traces through its diagnostics plugin, and LangChain Deep Agents Code can explicitly opt into bounded-content OTLP/HTTP traces with `--observability`. Both paths require an operator-run host collector. NemoClaw does not manage the collector, remote exporter credentials, fleet-wide routing, metrics or logs export, or authenticated tenant identity. Treat exported prompts, responses, tool inputs, and tool results as sensitive application data. Refer to [OpenClaw Conversation OTEL Diagnostics](commands#openclaw-conversation-otel-diagnostics). Broader observability adapter work remains tracked in [#3915](https://github.com/NVIDIA/NemoClaw/issues/3915). |
</AgentOnly>
<AgentOnly variant="hermes">
| External telemetry and observability export | Supported with caveats | OpenClaw can emit conversation traces through its diagnostics plugin, and LangChain Deep Agents Code can explicitly opt into bounded-content OTLP/HTTP traces with `--observability`. Both paths require an operator-run host collector. NemoClaw does not manage the collector, remote exporter credentials, fleet-wide routing, metrics or logs export, or authenticated tenant identity. Treat exported prompts, responses, tool inputs, and tool results as sensitive application data. Broader observability adapter work remains tracked in [#3915](https://github.com/NVIDIA/NemoClaw/issues/3915). |
</AgentOnly>
...
| Upgrades and lifecycle | Supported with caveats | Upgrade NemoClaw, then run `$$nemoclaw <sandbox> rebuild` to recreate the sandbox with the current image while backing up and restoring state. OpenClaw restores its complete credential-sanitized native configuration. Do not update the agent runtime in place inside the sandbox because the agent is image-pinned. Version attachment flexibility remains tracked in [#2217](https://github.com/NVIDIA/NemoClaw/issues/2217). |
...
<AgentOnly variant="openclaw">
| Native OpenClaw configuration | OpenClaw owns `openclaw.json` after first launch. Rebuild restores the sanitized configuration captured immediately before rebuild; snapshot restore reapplies the sanitized configuration stored in the selected snapshot. | Use `openclaw configure`, `openclaw config set`, or `openclaw config unset` inside the sandbox. Use NemoClaw host commands when a change also needs an OpenShell credential binding, network policy, or inference route. Refer to [Runtime Configuration Controls](../manage-sandboxes/configure-sandboxes/runtime-controls). |
</AgentOnly>
<AgentOnly variant="hermes,deepagents">
| In-sandbox config edits do not persist | Direct edits to agent config inside the running sandbox are not a durable replacement for the documented host workflow. | Use the documented host command or rerun onboarding for the setting you need to change. Refer to [Troubleshooting](troubleshooting). |
</AgentOnly>
...
| Runtime-owned capability and resource limits | OpenShell owns capabilities for managed processes. Entrypoint resource limits remain best effort when the runtime blocks them. | Keep OpenShell enforcement enabled. For direct containers, pass `--cap-drop=ALL` and `--ulimit` at the container runtime. Refer to [Process Controls](../security/security-controls/process-controls). |
