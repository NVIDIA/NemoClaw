...
| Agent config restore varies by runtime | OpenClaw restores its credential-sanitized native config as a whole file, so noncredential edits can survive rebuilds. Hermes regenerates its config, and managed Deep Agents restores only allowlisted values. | Use supported host commands for managed settings so sandbox config, OpenShell state, and registry metadata stay aligned. Refer to [Create and Restore Snapshots](../manage-sandboxes/state-and-backups/create-and-restore-snapshots). |
