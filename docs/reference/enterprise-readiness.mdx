...
| Credential isolation | Supported | Inference credentials stay on the host and never enter the sandbox; the agent reaches models through `inference.local`. CLI output redaction and backup sanitization add defense in depth, but NemoClaw does not scan or block agent writes to persistent memory or workspace files. Refer to [Credential Storage](../security/credential-storage) and [Security Best Practices](../security/best-practices). |
