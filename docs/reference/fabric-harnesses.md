...
Hermes uses the local API/dashboard adapter by default; experimental [Relay tracing](../agents.md#hermes-relay-tracing) keeps that adapter when `interfaces` is explicit or Tavily search is attached, otherwise selecting an upstream adapter without those interfaces.
