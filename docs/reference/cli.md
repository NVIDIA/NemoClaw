...
The composed mode then resolves credentials, runs the ordinary SDK plan, prints its JSON preview on stderr, and asks `Apply this plan?` separately from accepting the authored YAML.
...
If credential fulfillment, plan, or apply fails, retain the published YAML and state directory for standalone diagnosis and recovery.
