...
To recover from that legacy-state check, rerun interactive onboarding with `nemohermes onboard --fresh --name <sandbox-name> --agent <recorded-agent>`, select the sandbox's recorded provider and model, enable Brave Search again, and reuse the existing sandbox when prompted so NemoClaw records a matching session; then retry rebuild.
