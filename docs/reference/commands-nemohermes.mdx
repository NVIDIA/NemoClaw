...
`nemohermes stop` remains as a deprecated legacy full stop. It stops the tunnel services and also releases the managed host gateway port. Use `nemohermes tunnel stop` when the shared gateway should remain available.
