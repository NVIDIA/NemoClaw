...
Setting `NEMOCLAW_INSTANCE` to a non-default value before `nemohermes onboard` routes the NemoClaw home directory, credentials store, rebuild backups (and the snapshot tree underneath), local inference adapter state (including the Ollama auth-proxy token), and gateway binding (name, state directory, compatibility container) to that instance.
