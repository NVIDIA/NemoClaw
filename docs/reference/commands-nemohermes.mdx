...
```bash
nemohermes onboard --agent langchain-deepagents-code --observability
```
...
```bash
nemohermes my-dcode rebuild --observability --yes
nemohermes my-dcode rebuild --no-observability --yes
```
...
```bash
nemohermes my-assistant rebuild [--yes|-y|--force] [--verbose|-v] [--tool-disclosure <progressive|direct>] [--observability|--no-observability]
```
