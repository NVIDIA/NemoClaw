...
| `host.platform.dgx_spark` | DGX Spark identity from a product name or trusted FastOS marker on ARM64 with an available NVIDIA GPU. |
...
NemoClaw can also identify an OEM DGX Spark host from `/etc/fastos-release` when the product name does not identify the host.
The marker must be a regular file of 1 through 4,096 bytes, owned by UID and GID 0, not a symbolic link, and not writable by group or other users.
It must contain exactly one `NAME="DGX SPARK FASTOS"` line and no NUL or carriage-return byte.
NemoClaw parses the marker as inert text and does not pin its `VERSION` value.
A matching marker classifies the host as DGX Spark; the normal ARM64 and available NVIDIA GPU requirements still apply.
