...
| `host.platform.dgx_spark` | DGX Spark identity. |
| `host.platform.dgx_station` | Qualified DGX Station GB300 identity, hardware, and software profile. |
...
The `host.platform.dgx_station` capability is `present` only when the product identity, NVIDIA display-class PCI device, and software profile all qualify.
