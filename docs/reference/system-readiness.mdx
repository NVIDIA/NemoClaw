...
| `host.platform.dgx_station_software` | A qualified software profile or the validation-only factory-runtime profile. |
...
NemoClaw uses validation-only factory-runtime handling when a trusted, complete marker has the Station GB300 platform but an unrecognized version or build.
The report remains supported and includes the `host.platform.dgx_station_software_unrecognized` warning.
Invalid marker metadata, malformed or partial contents, or a conflicting DGX platform produce an `unqualified` qualification and the blocking `host.platform.dgx_station_unqualified` finding.
