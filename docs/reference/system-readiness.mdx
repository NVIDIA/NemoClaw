...
| `host.platform.n1x_wsl` | Windows N1x product identity qualified through WSL on Linux ARM64 with an available NVIDIA GPU and a matching selected-provider CUDA proof. |
...
Managed llama.cpp admission also requires `host.platform.wsl_gpu_passthrough` to be `present` and remains Docker Desktop-only.
