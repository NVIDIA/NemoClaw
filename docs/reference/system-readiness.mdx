...
For an OEM host whose DMI product name does not identify DGX Spark, a trusted `/etc/fastos-release` marker with exactly one `NAME="DGX SPARK FASTOS"` line identifies the host as DGX Spark.
The marker must be a regular, non-symbolic file of 1 through 4,096 bytes, owned by UID and GID 0, and not writable by group or other users.
This detection does not establish hardware qualification for an OEM system.
