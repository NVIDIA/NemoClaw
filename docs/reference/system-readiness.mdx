...
The `host.platform.dgx_station` capability is `present` only when the host runs Linux ARM64 on Ubuntu 24.04, reports an effective NVIDIA GPU, and the product identity, exact GB300 PCI device (`0x31c2` or `0x31c3`), and software profile all qualify.
NemoClaw reads DGX release metadata only from a bounded, root-owned, root-group marker that is not writable by group or other users.
A known mismatch produces an `unqualified` qualification and the blocking `host.platform.dgx_station_unqualified` finding.
Missing or unreadable qualification evidence produces an `unknown` qualification and the blocking `host.platform.dgx_station_inconclusive` finding.
