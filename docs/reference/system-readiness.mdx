...
`DOCKER_CONTEXT` overrides `DOCKER_HOST`, matching the Docker CLI, and NemoClaw never replaces a selected context with a discovered fallback socket.
When `DOCKER_CONTEXT` names a context, NemoClaw resolves that context to its endpoint, so readiness, onboarding, and every later Docker command measure the daemon you selected.
...
On Linux and WSL, the report reads the bounded `ID`, `VERSION_ID`, and `PRETTY_NAME` fields from `/etc/os-release` into `host.os.distribution`, `host.os.version`, and `host.os.pretty_name`. Ubuntu 24.04 is the host-level qualified release. Another distribution or release produces the `host.os.release_unqualified` warning, while missing or malformed release evidence produces `host.os.release_inconclusive`. These warnings do not claim that the host is unusable; they identify a release outside the tested host-level onboarding boundary before installation changes the system.
