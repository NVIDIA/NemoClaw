...
| `host.platform.dgx_spark` | DGX Spark identity on ARM64 with an available NVIDIA GPU. The identity can use a trusted DGX Spark FastOS marker when DMI does not identify the host. |
...
DGX Spark qualification accepts either a DMI product name that identifies DGX Spark or a trusted FastOS marker.
For an OEM ARM64 GB10 host whose DMI product name does not identify DGX Spark, `/etc/fastos-release` must be a regular file of 1 through 4,096 bytes that is owned by UID and GID 0, is not a symbolic link, grants no group or world write access, contains no NUL or carriage-return byte, and contains exactly one `NAME="DGX SPARK FASTOS"` line.
NemoClaw parses the marker as inert text and does not pin its `VERSION` value.
A matching marker identifies the host as DGX Spark, not N1x.
The host still requires ARM64 and an available NVIDIA GPU for the `host.platform.dgx_spark` capability to be `present`.
