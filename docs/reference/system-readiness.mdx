...
| `host.platform.n1x_wsl` | A proof-backed accepted N1x WSL GPU identity qualified through WSL on Linux ARM64 with reachable Docker Desktop. The accepted identities are listed below. |
...
It is `qualified` only for Linux ARM64 WSL with reachable Docker Desktop and the same successful GPU proof.
The capability is `present` only when that qualification passes.
Managed llama.cpp admission also requires `host.platform.wsl_gpu_passthrough` to be `present`, so selection cannot reuse an unproved GPU name.
