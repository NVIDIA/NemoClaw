...
When `DOCKER_CONTEXT` is unset, the report rejects an unsupported explicit `DOCKER_HOST` before it uses Docker daemon evidence.
...
`DOCKER_HOST` overrides `DOCKER_CONTEXT`, matching Docker CLI behavior when both variables are set.
When `DOCKER_HOST` is unset and `DOCKER_CONTEXT` names a context, NemoClaw resolves that context to its endpoint, so readiness, onboarding, and every later Docker command measure the daemon you selected.
A context that resolves to an absolute local `unix://` socket becomes the selected `DOCKER_HOST`.
A context that does not resolve, or that names any other endpoint, is reported through `host.docker.host_invalid` and the `invalid_docker_host` advisory instead of falling back to the default Docker socket.

When neither variable is set, NemoClaw first probes the Docker CLI's selected authority, including its `DOCKER_CONFIG` directory when set, without a host override.
If that authority completes with an unreachable result, NemoClaw probes a bounded set of local Unix sockets and accepts only an endpoint with valid server-version evidence.
On Linux, the set includes the rootless Docker socket at `/run/user/<UID>/docker.sock`.
If the probe times out or cannot start, NemoClaw keeps the selected authority. `host probe` reports `host.docker.probe_inconclusive`; onboarding reports the `docker_probe_inconclusive` advisory with a command to verify that authority.
When it selects a fallback socket, NemoClaw clears the ambient `DOCKER_CONTEXT` so later Docker commands use the selected `DOCKER_HOST`.
...
When two reachable fallbacks identify as different engines, onboarding reports the blocking `docker_authority_conflict` advisory.
The advisory names both sockets and their engines and provides a `DOCKER_HOST` command for the Docker socket.
A reachable Podman endpoint remains classified as Podman and does not satisfy the standard Docker runtime requirement.
For native rootless Podman on a qualified Linux host, set `NEMOCLAW_GATEWAY_RUNTIME=podman` before onboarding.
Review the [native Podman requirements](./platform-support#deployment-paths) before selecting that path.
...
| `host.platform.identity_consistent` | Firmware fields do not identify conflicting NVIDIA platform families or Station generations. |
...
| `host.platform.wsl_runtime_available` | Reachable Docker Desktop or a qualification-backed provider that owns WSL runtime readiness. |
...
| `host.platform.wsl_gpu_passthrough` | A bounded CUDA proof from the selected Docker or provider-owned runtime. |
| `host.platform.n1x_wsl` | A proof-backed accepted N1x WSL GPU identity qualified through WSL on Linux ARM64 with an available NVIDIA GPU and a matching selected-provider CUDA proof. The accepted identities are listed below. |
| `host.platform.dgx_spark` | DGX Spark identity from firmware or a trusted FastOS marker on ARM64 with an available NVIDIA GPU. |
...
| `host.platform.dgx_station_hardware` | Station GB300 firmware identity and an exact NVIDIA GB300 display-class PCI device. |
| `host.platform.dgx_station_software` | A qualified Station software profile. |
| `host.platform.dgx_station_runtime` | Linux ARM64, Ubuntu 24.04, and an effective NVIDIA GPU. |
| `host.platform.dgx_station` | The combined Station GB300 hardware, software-handling, and runtime result. |
...
When firmware does not identify DGX Spark, NemoClaw can use a trusted `/etc/fastos-release` marker.
The marker must be a regular file of 1 through 4,096 bytes, owned by UID and GID 0, and not a symbolic link.
It must grant no group or world write access and contain no NUL or carriage-return byte.
Its only `NAME=` line must be `NAME="DGX SPARK FASTOS"`.
A matching marker identifies DGX Spark; the ARM64 and available NVIDIA GPU requirements still apply.
This identity check does not establish hardware qualification for an OEM system.

The report adds `host.platform.wsl`, `host.platform.n1x_wsl`, `host.platform.dgx_spark`, or `host.platform.n1x` qualifications when those platforms apply.
For Station, it adds separate hardware, software, runtime, and combined qualifications.
For WSL, Docker Desktop and a qualification-backed provider that owns host readiness are `qualified`, native Docker is `unqualified`, and an unrecognized reachable runtime is `unknown`.
The `host.platform.wsl_gpu_passthrough` capability is `present` only after the selected provider's bounded CUDA proof passes.
...
The `host.platform.n1x_wsl` qualification applies when the bounded CUDA proof validates one GPU whose normalized NVIDIA identity is either `NVIDIA RTX Spark N1X` or `NVIDIA RTX Spark N1X (6144-core Blackwell RTX GPU)`.
It is `qualified` only for Linux ARM64 WSL with an available NVIDIA GPU and a successful proof whose provider identity matches Docker Desktop or the selected provider-owned runtime.
The capability is `present` only when that qualification passes.
Managed llama.cpp admission also requires `host.platform.wsl_gpu_passthrough` to be `present`, remains Docker Desktop-only, and cannot reuse an unproved GPU name.
Windows-host Ollama also remains Docker Desktop-only; the provider-owned Podman path applies to WSL-local Ollama.
The Windows chassis model is bounded diagnostic evidence only and does not establish N1x WSL identity.

DGX Station qualification separates hardware identity from software and runtime handling.
The hardware capability requires a bounded firmware product, product-family, board, or device-tree value with separate `Station` and `GB300` tokens.
It also requires a display-class PCI device with NVIDIA vendor ID `0x10de` and GB300 device ID `0x31c2` or `0x31c3`.
Conflicting recognized firmware fields make `host.platform.identity_consistent` absent and block platform support.
The runtime capability requires Linux ARM64, Ubuntu 24.04, and an effective NVIDIA GPU.
...
An unrecognized software profile leaves `host.platform.dgx_station_hardware` present but makes the software and combined Station qualifications unqualified.
Invalid marker metadata, malformed or partial contents, an unrecognized version or build, or a conflicting DGX platform produce an `unqualified` qualification and the blocking `host.platform.dgx_station_unqualified` finding.
...
When available, `host.platform.identity` includes bounded firmware, CPU count, host memory, GPU count and memory, OS, release-name, build, OTA, and software-profile evidence.
These capacity and display values are diagnostic evidence and do not replace the firmware and PCI hardware requirements.
...
After `host.platform.n1x` is `present`, accepting Express lets onboarding waive only that finding.
Declining or disabling Express can apply the same waiver when no provider is preselected or the explicit provider is recognized and is not Local NVIDIA NIM.
...
Neither finding can be waived by onboarding intent.
...
No blocking finding or unknown required capability is waived.
...
On Linux and WSL, the report reads at most 4,096 bytes from `/etc/os-release`, with the standard `/usr/lib/os-release` fallback.
It projects the `ID`, `VERSION_ID`, and `PRETTY_NAME` fields into `host.os.distribution`, `host.os.version`, and `host.os.pretty_name`.
Ubuntu 24.04 is the host-level qualified release.
Another distribution or release produces the `host.os.release_unqualified` warning, while missing, malformed, or oversized release evidence produces `host.os.release_inconclusive`.
These warnings do not claim that the host is unusable; they identify a release outside the tested host-level onboarding boundary before installation changes the system.
