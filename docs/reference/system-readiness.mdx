...
Immediately before source deletion, the rebuild journal requires the complete gateway authority to match that preflight handoff, apart from the one managed change from a package-managed service to the standalone gateway, and carries the journaled authority into replacement onboarding.
