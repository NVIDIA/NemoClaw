...
DGX Spark identity can come from the host product name or a trusted `/etc/fastos-release` marker.
The marker is a regular file of 1 through 4,096 bytes that is owned by UID and GID 0, is not a symbolic link, and grants no group or world write access.
It must contain exactly one `NAME="DGX SPARK FASTOS"` line.
This marker lets NemoClaw recognize a DGX Spark when the firmware product name does not identify it.
Invalid, ambiguous, unreadable, or untrusted markers do not identify the host as a DGX Spark.
