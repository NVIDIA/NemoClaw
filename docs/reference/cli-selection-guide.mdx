...
It starts the OpenShell gateway when needed, registers providers, resolves the selected agent's immutable managed release image to an exact digest, applies NemoClaw policy choices, and creates the sandbox with a secret-free startup profile.
...
For an agent setting that requires sandbox recreation, rerun onboarding so the managed startup configuration is recreated consistently:
