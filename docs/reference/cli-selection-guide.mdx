...
The command runs as the sandbox user with `HOME=/sandbox` and uses the NemoClaw-provisioned sandbox environment, including agent configuration, inference routing, and policy state.

```console
$ nemoclaw my-assistant exec -- cat /tmp/gateway.log
```

Use `openshell sandbox exec` only when you intentionally need the raw OpenShell execution path and want to bypass NemoClaw's sandbox registry and wrapper behavior.
