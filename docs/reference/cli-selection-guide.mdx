...
- Mount or manually copy sandbox state:
...
OpenShell preserves the native sandbox home through stop, start, and reconnect. Use `$$nemoclaw <name> share mount` for direct workspace editing or `download` for an independent host copy.
...
- [Persist Sandbox State](../manage-sandboxes/state-and-backups/create-and-restore-snapshots) for OpenShell persistence and whole-home rebuild transfer.
