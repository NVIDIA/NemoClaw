...
Use `$$nemoclaw <name> policy get` and `openshell policy set --policy <file> --wait <name>` only when you need to edit and replace the complete base policy.
Replace every `[STRIPPED_BY_MIGRATION]` marker before you apply the file.
