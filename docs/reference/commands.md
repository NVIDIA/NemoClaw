...
|----------|--------|--------|
| `NEMOCLAW_PROVIDER` | provider key (e.g. `nvidia`, `openai`, `anthropic`, `ollama`, `vllm`, `compatible`, `routed`) | Selects the inference provider in non-interactive onboarding. Must match one of the keys the wizard would prompt for. |
