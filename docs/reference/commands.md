...
When `nemoclaw onboard` detects an NVIDIA GPU on the host (`nvidia-smi` succeeds), it automatically enables GPU passthrough at both the gateway and sandbox level.
No flag is required — `nvidia-smi` and CUDA workloads work inside the sandbox out of the box.
...
If a gateway already exists without GPU passthrough, onboarding will exit with guidance to destroy and re-onboard.
To add GPU to an existing sandbox, use `--recreate-sandbox`.
