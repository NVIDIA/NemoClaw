...
During normal resolution, NemoClaw tries the release-version Hermes base before the exact published digest declared by the final Hermes Dockerfile.
The selected candidate must also pass any active OpenShell ABI requirement, and a validated result can be recorded for warm-hint reuse.
The final Hermes image accepts only resolver-validated official Hermes base images, the official digest tracked by its Dockerfile, or a repository-built local base, so an otherwise reachable or ABI-compatible image is not sufficient.
...
NemoClaw validates the requested image for the required MCP runtime, and the final Hermes image accepts only resolver-validated official Hermes base tags or digests, the tracked official digest, or a repository-built local base.
