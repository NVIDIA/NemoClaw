...
On supported `amd64` Docker hosts, stock onboarding prefers a complete immutable release image for the selected shipped agent.
...
If catalog or registry resolution fails, Docker falls back to the trusted canonical Dockerfile recipe.
An explicit `--from <Dockerfile>` request also remains a local Dockerfile build.
...
The base-image resolution controls below apply to the trusted Dockerfile fallback and explicit custom Dockerfile workflows.
