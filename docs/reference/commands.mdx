...
The `low`, `medium`, and `high` values apply only to the `compatible-endpoint` provider.
...
On another API family, NemoClaw removes an inherited `reasoning_effort` field and records the endpoint-default state.
...
Switching to another provider or API family clears the recorded effort.
...
| `NEMOCLAW_REASONING_EFFORT` | `low`, `medium`, `high`, or `default` | Applies only to the `compatible-endpoint` provider. A `low`, `medium`, or `high` value writes `params.extra_body.reasoning_effort` when the selected API is `openai-completions`. Another API family omits the field. Unset leaves the endpoint's own default in place, and `default` clears a recorded effort. On `compatible-endpoint`, any other value fails before NemoClaw changes the provider, policies, sandbox, or registry. Another provider clears the variable without reading it. |
