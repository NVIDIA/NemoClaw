...
The command, the command gathers system information, Docker state, gateway logs, and sandbox status into a summary or tarball.
...
The command, the name must match a registered sandbox.
...
Successful managed cleanup recursively removes that directory and all contents; `--keep-openshell` preserve it.
