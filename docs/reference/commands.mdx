...
| `--config <K=V>` | Typed non-secret provider configuration. Supported: `OPENAI_BASE_URL=<http(s)://host/path>` with `--type openai`. URLs with credentials, query parameters, or fragments are rejected. Repeatable |
