...
View sandbox logs. Use `--follow` to stream output in real time. Use `--tail <lines>` or `-n <lines>` to limit the number of returned lines. Use `--since <duration>` to show recent logs only, such as `5m`, `1h`, or `30s`.

<AgentOnly variant="openclaw,hermes">
The command reads both agent gateway output and OpenShell audit events, so policy denials appear alongside the gateway log stream. If one log source is unavailable, NemoClaw prints a warning and keeps reading the remaining source.
</AgentOnly>
<AgentOnly variant="deepagents">
