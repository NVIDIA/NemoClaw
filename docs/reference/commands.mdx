...
The line also reports cumulative duration and attempt count for each launch-readiness observation: `readiness.sandbox-identity`, `readiness.policy-get`, `readiness.inference-get`, `readiness.gateway-health`, `readiness.forward-health`, and `readiness.inference-route`.
It reports the first failed observation, the first readiness decision, and the first fallback decision when available.
