...
- Hermes: `/sandbox/.hermes/platforms/<channel>/` (for example `/sandbox/.hermes/platforms/whatsapp/`).
  For WhatsApp, NemoClaw also clears `/sandbox/.hermes/dashboard-home/platforms/whatsapp/session/`.
...
For a Hermes WhatsApp sandbox, `--channel whatsapp` probes only whether the default gateway session path and dashboard-home session path contain `creds.json`.
