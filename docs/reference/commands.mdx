...
Back up all registered running sandboxes to `~/.nemoclaw/rebuild-backups/` for the default instance, or `~/.nemoclaw-<instance>/rebuild-backups/` when `NEMOCLAW_INSTANCE` is set to a non-default value (see the Environment Variables section).
...
The command writes the snapshot to `~/.nemoclaw/rebuild-backups/<name>/` for the default instance, or `~/.nemoclaw-<instance>/rebuild-backups/<name>/` when `NEMOCLAW_INSTANCE` is set to a non-default value.
...
| `NEMOCLAW_INSTANCE` | nemoclaw | NemoClaw instance identity for multi-instance hosts. Scopes the migrated host-side surfaces — state root (`~/.nemoclaw` for the default instance, `~/.nemoclaw-<instance>` otherwise), credentials store, rebuild backups, local inference adapter state, shields audit and timer state, and the OpenShell gateway binding. Registry, onboard session, messaging, mount roots, uninstall plan, and `config-io` host-root healing still share `~/.nemoclaw` until follow-up PRs migrate them. |
...
`NEMOCLAW_INSTANCE` must be 1–32 lower-case alphanumerics with internal `-` (for example `agent-a`, `tenant1`).
Hyphen-separated segments may not be purely numeric so the instance name cannot collide with the gateway port suffix.
Without this rule, `agent-a` plus `NEMOCLAW_GATEWAY_PORT=8081` and `agent-a-8081` plus the default port would produce the same composed name.

The default instance name is the historical singleton identity NemoClaw has always used.
Leaving `NEMOCLAW_INSTANCE` unset, setting it to the empty string, or setting it explicitly to that default value all resolve to the same identity.
The state root then stays at `~/.nemoclaw` and the OpenShell gateway keeps its bare historical name (`nemoclaw-<port>` for non-default ports, with no suffix on the default port).
Existing single-instance deployments therefore observe no change after upgrading.
When the bare default gateway is already in use, varying `NEMOCLAW_GATEWAY_PORT` alone produces a `nemoclaw-<port>` gateway that segregates from the bare default without requiring `NEMOCLAW_INSTANCE`.
The port suffix is the implicit fallback identity for the default instance.

Setting `NEMOCLAW_INSTANCE` to a non-default value before `$$nemoclaw onboard` routes the NemoClaw home directory, credentials store, rebuild backups (and the snapshot tree underneath), local inference adapter state (including the Ollama auth-proxy token), shields audit and timer state, and gateway binding (name, state directory, compatibility container) to that instance.
Combine `NEMOCLAW_INSTANCE` with `NEMOCLAW_GATEWAY_PORT` to compose the instance and port suffixes (`nemoclaw-<instance>-<port>`).
Modules that still read `~/.nemoclaw` directly continue to share state across instances until follow-up PRs migrate them.
These remaining shared surfaces are the sandbox registry, onboard session state, messaging configuration, host-side mount roots, the uninstall plan, and the `config-io` host-root permission healer.
