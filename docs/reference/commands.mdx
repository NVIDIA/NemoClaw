...
<AgentOnly variant="hermes">
For an active Portable Hermes sandbox with an `ollama-local` provider, `connect --probe-only` can resume only the stopped Ollama runner named by the published receipt.
For a descriptor-backed provider, it verifies the existing inference route and does not require or recover an Ollama receipt.
