...
Routed onboarding acquires the selected gateway route lock first, then the host-wide lifecycle lock for each Model Router port.
...
Model Router destruction takes the gateway route lock, then the host-wide Model Router port lock, and then tries the onboarding session lock without waiting.
...
It leaves the Model Router process and replacement session unchanged.
...
Non-Model Router cleanup uses a non-blocking session update.
If onboarding owns the lock or a same-name replacement has a different identity, `destroy` leaves the replacement session unchanged.
