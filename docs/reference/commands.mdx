...
Deploy a skill directory to a running sandbox. The command validates the `SKILL.md` frontmatter, which requires a `name` field, then follows the selected agent's installation contract. OpenClaw and Deep Agents create a bounded secure snapshot; Hermes uploads the selected files directly.
...
OpenClaw plugins are a different kind of extension. To install an OpenClaw plugin, refer to [Install OpenClaw Plugins](../manage-sandboxes/install-openclaw-plugins). For a skill, NemoClaw creates an exact host snapshot, transfers it into a private staging directory, and invokes the pinned native OpenClaw workspace installer for the NemoClaw-managed primary `main` agent. That primary agent must use the default `/sandbox/.openclaw/workspace` directory. The command checks this support boundary and refuses a custom `main` workspace before it creates sandbox staging or invokes native publication. OpenClaw owns publication into `/sandbox/.openclaw/workspace/skills/<name>`, source metadata, precedence, and activation. NemoClaw verifies the installed digest plus native `skills list`, `skills info`, and `skills check` results before reporting success.

NemoClaw stores ownership evidence in protected host state bound to the exact live sandbox identity. An exact-content retry verifies an unchanged skill only after a completed install receipt. An interrupted fresh installation retains a pending receipt, but the retry refuses any existing target because sandbox content cannot prove that the uncertain operation created it. If native publication succeeds but final host receipt persistence fails, NemoClaw reports that the target may have been installed without finalized ownership evidence. Inspect `/sandbox/.openclaw/workspace/skills/<name>` and confirm its provenance before removal; a retry cannot adopt an existing target until that ownership state is reconciled. The same-content retry can proceed when the ambiguous target is absent. When rebuild restores the workspace into a replacement sandbox, it durably transfers validated skill ownership evidence to the replacement identity before retiring the source evidence. Cross-sandbox snapshot restore instead clones workspace content without transferring host ownership evidence; restored skills are treated as user-owned at the destination, so inspect and remove an existing restored target before installing that name through NemoClaw. Changed-content updates are refused because pinned OpenClaw 2026.7.1 has no compare-and-swap contract that can atomically bind the verified current content to a forced replacement. It also refuses installation when either former `/sandbox/.openclaw/skills/<name>` or `$HOME/.openclaw/skills/<name>` path exists, preventing old and native destinations from becoming competing authorities. Inspect the legacy and workspace paths and migrate legacy or changed content manually before retrying. Start a new OpenClaw session to load the installed skill; the command does not edit `sessions.json`.
...
For OpenClaw, exact-content retries are verified but changed-content updates are refused until the native installer provides an atomic conditional-update contract. Hermes updates its generic sandbox skill path in place. Chat history is preserved. Deep Agents supports only fresh-name installs because its active skill directory is shared with agent-authored content. Follow the agent-specific activation guidance above after installation.
...
Automatic OpenClaw skill removal is unavailable in this capability slice because pinned OpenClaw 2026.7.1 has no public native skill removal command. The command refuses before changing files. Use `$$nemoclaw <name> connect` to inspect `/sandbox/.openclaw/workspace/skills/<skill>` plus the legacy `/sandbox/.openclaw/skills/<skill>` and `$HOME/.openclaw/skills/<skill>` paths, and remove or migrate content manually only after confirming its provenance.
