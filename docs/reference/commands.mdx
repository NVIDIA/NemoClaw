...
When no sibling gateways remain, uninstall removes every other entry under `~/.nemoclaw/` (gateway source, runtime state, the Ollama auth proxy PID file, etc.).
When sibling gateways remain, it removes only the selected gateway's resources and port-scoped state while preserving shared host resources.
