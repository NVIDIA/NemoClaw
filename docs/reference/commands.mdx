...
The command exits nonzero before running checks, writes one JSON error document to standard output, and writes the conflict to standard error.

| Flag | Description |
| --- | --- |
| `--json` | Emit the global report as JSON. Cannot be combined with `--text`. |
| `--text` | Emit the global human-readable report explicitly. Cannot be combined with `--json`. |
