...
Replace the complete denied-tool list for one managed MCP server without changing its endpoint, credential, address pins, provider, or agent adapter.
For a legacy public registration without recorded address pins, the command first resolves and records the current validated pins.
...
NemoClaw removes the current generated policy before it records and applies the replacement intent.
If replacement policy activation fails, the route remains blocked.
Run `$$nemoclaw <name> mcp restart <server>` to converge to the recorded intent.
