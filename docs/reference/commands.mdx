...
Set `NEMOCLAW_HERMES_API_PORT=<N>` before you onboard a new sandbox to require a specific port.
...
Rerunning onboarding with the variable and without `--recreate-sandbox` moves the host forward to `<N>` while the relay still listens on the recorded port, and requests to `<N>` fail.
