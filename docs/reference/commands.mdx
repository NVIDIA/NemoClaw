...
Before starting the adapter, a trusted launcher must open two owner-only regular files and map only these descriptors into the child process:
...
The trusted launcher must not place credential source paths or values in arguments or environment variables.
It must open source files without following symbolic links and must intentionally inherit only credential descriptors `3` and `4` across this exec.
...
After the trusted launcher has validated and mapped both descriptors, it execs this command with no credential paths or values in its arguments or environment:
