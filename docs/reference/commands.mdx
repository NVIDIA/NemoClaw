...
When the same `status` run recovers a managed gateway, it retries the route and inference request together up to three total attempts, with a two-second delay after each failed attempt.
