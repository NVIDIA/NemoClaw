...
`--install-tag` flag. | | `NEMOCLAW_DISABLE_TELEMETRY` | exactly `1` | Stops the post-success
installer telemetry attempt before production configuration lookup, event construction, or network
access. Production delivery remains disabled until the client ID, schemas, and endpoints are
provisioned. | | `NEMOCLAW_ENABLE_LOCAL_MODEL_PROFILE` | `1` to enable | Enables the fixed
