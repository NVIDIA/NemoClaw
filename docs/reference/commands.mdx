...
If the `stdout` JSON marks the turn as incomplete or abandoned, the wrapper exits with status `1` even when the envelope reports success.
These markers are an `incomplete_turn` error kind, an `abandoned` liveness state, or `replayInvalid`.
