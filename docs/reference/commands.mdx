...
Follow the [retained-sandbox recovery procedure](#recover-a-retained-sandbox); its OpenShell-absence and immutable runtime-identity requirements also apply to APF creation. For Docker-backed APF creation, its Docker container-identity requirement also applies.
