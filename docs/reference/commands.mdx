...
<AgentOnly variant="hermes">
  Portable Hermes `connect --probe-only` can also print credential-free currentness,
  container-inspection, and lifecycle-recovery timing lines.
  These diagnostics report elapsed stages and counts for receipt, socket, executable,
  container, health, and recovery checks.
  They do not change recovery behavior or the command exit status.
</AgentOnly>
...
After registering the channel, NemoClaw asks whether to rebuild immediately. Running `add` for an already-configured channel overwrites the stored credentials where applicable. The operation is idempotent. NemoClaw validates every existing OpenShell channel profile against its checked-in provider contract before reuse. This contract includes the provider type, credential keys, endpoints, binaries, inference capability, and refresh configuration when applicable. A missing, malformed, conflicting, or incompatible profile stops the operation before reuse. Hermes Discord uses its dedicated static provider type because its policy binds the Discord API and Gateway endpoints.

For a refresh-based bridge, NemoClaw preserves an exact existing provider's working credential until the gateway mints a replacement.
If refresh configuration fails, NemoClaw attempts to delete only bridge providers created by that channel-add attempt.
It preserves existing providers.
If deletion fails, the error names the provider and gives an exact gateway-scoped `openshell provider delete` command.
Do not rely on outbound channel replies until you resolve the reported refresh failure.
