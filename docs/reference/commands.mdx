...
Lease acceptance also requires a secure, independently writable OS per-user runtime authority.
On Linux, NemoClaw uses `/run/user/<numeric-uid>`; on macOS, it asks the OS for the user's temporary root.
...
That user also needs a secure, independently writable OS per-user runtime authority: `/run/user/<numeric-uid>` on Linux, or the user temporary root derived from the OS on macOS.
