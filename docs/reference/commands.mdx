...
If the sandbox is running an older Hermes version than this NemoClaw release expects, `status` and `connect` add an `Update` line pointing at `nemohermes <name> rebuild` to pick up the newer version. The rebuild reuses the existing sandbox name and transfers the complete native home while OpenShell keeps credential stores under its own lifecycle.
...
<Warning>
OpenShell sandbox deletion removes the native home/workspace storage. Download any files you need before destroying the sandbox; do not rely on deleted OpenShell storage as a backup.
...
For QR-paired channels (today: WhatsApp), NemoClaw destructively clears the in-sandbox session directories before the rebuild so the complete native-home transfer does not restore the removed auth blob and let the channel reconnect:
...
| `--force` | Skip the confirmation prompt. It does not bypass an unavailable MCP source or allow rebuild to continue when the complete native home/workspace cannot be transferred, when the selected remote provider (other than Hermes Provider) no longer exposes the recorded credential key, when that credential is expired, or when its expiration metadata cannot be verified. A present credential key without an expiration entry is treated as non-expiring. |
...
Follow [Continue an Interrupted Replacement](../manage-sandboxes/operate-sandboxes/recover-and-rebuild-sandboxes#continue-an-interrupted-replacement) for the retained-backup recovery procedure.
...
After restore, the command starts the rebuilt Hermes gateway with the complete source native home, including its `/sandbox/.hermes` config and unknown agent-written paths.
...
| Entry | What it holds |
| --- | --- |
| `rebuild-backups/` | Private whole-workspace handoffs used by rebuild, recreation, and strict pre-upgrade `$$nemoclaw backup-all`. |
