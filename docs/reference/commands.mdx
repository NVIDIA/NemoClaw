...
The default output includes a label and a warning. When the dashboard forward is bound on all interfaces, the default output also says so on its own line. Pass `--quiet` or `-q` to print only the URL to stdout so scripts can capture it:
...
Print the browser dashboard URL for a running Hermes sandbox. Hermes manages dashboard sessions itself, so this command prints a plain URL without an OpenClaw `#token=` fragment. The built-in dashboard is forwarded on port `18789` by default. When the dashboard forward is bound on all interfaces, the default output says so on its own line; `--quiet` prints only the URL.
