...
Run `sandbox cua target health <name>` or `sandbox cua task status <name>` to record an independent observation.
Cancel the exact observed active task first, then run `sandbox cua target detach <name>` or `sandbox cua target destroy <name>` to prove cleanup.
...
The most recent 16 terminal results and their content-addressed evidence references remain available through `cua task result` and `cua task status` after a normal CLI reconnect.
