...
When a single server is named, status also runs a wire-level credential-resolution probe: an MCP `initialize` sent from inside the sandbox with the `openshell:resolve:env:KEY` placeholder header, reported as `provider.credentialResolution` in JSON and a `credential resolution:` line in text output.
A `FAILED` verdict means OpenShell returned provider metadata as healthy but never rewrote the placeholder on egress, so agent runtimes receive the same auth failure and skip the server.
Ambiguous rejections (HTTP 400), endpoint outages (HTTP 5xx), policy denials, and unreachable sandboxes report as `unknown` rather than blaming the credential rewrite.
