...
NemoClaw stores ownership evidence in protected host state bound to the exact live sandbox identity. It updates an existing workspace skill only when that evidence matches the current workspace digest; otherwise it refuses the collision without replacing the skill. It also refuses installation when the former `/sandbox/.openclaw/skills/<name>` path exists, preventing old and native destinations from becoming competing authorities. Inspect both paths and migrate legacy content manually before retrying. If publication finishes but verification is interrupted, retry with the same local content so NemoClaw can reconcile the pending operation. Start a new OpenClaw session to load the installed or updated skill; the command does not edit `sessions.json`.
...
For OpenClaw, an existing workspace skill is updated only when protected provenance proves that NemoClaw installed the unchanged current content. Hermes updates its generic sandbox skill path in place. Chat history is preserved. Deep Agents supports only fresh-name installs because its active skill directory is shared with agent-authored content. Follow the agent-specific activation guidance above after installation.
