...
When the container runs and OpenShell reports `Stopped`, `start` starts the sandbox through OpenShell to repair the phase. A running sandbox in another phase skips the container start and still runs the gateway and forward health checks. A paused container is unpaused. If the container was removed entirely, `start` fails and points you to `$$nemoclaw <name> rebuild`.

For a sandbox that uses the Docker driver, `docker_unreachable` means `docker info` did not return valid server data.
The sandbox state cannot be verified until Docker access succeeds.
Run `docker info` to inspect the error before choosing a remedy.
If Docker is stopped, follow [Docker is not running](troubleshooting#docker-is-not-running).
If `docker info` reports a permission-denied error on Linux, follow [Docker permission denied on Linux](troubleshooting#docker-permission-denied-on-linux).
For any other failure, correct the error from `docker info`, such as an invalid context or TLS configuration.
Make `docker info` succeed and report server data, then retry `$$nemoclaw <name> start`.
