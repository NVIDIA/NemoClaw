...
<AgentOnly variant="openclaw">
  Portable lifecycle recovery emits one credential-free `Portable lifecycle timing:` line.
  ...
  It also emits one credential-free `Portable OpenClaw gateway startup timing:` line.
  When accepted launch-readiness evidence lets `connect --probe-only` or `recover` skip lifecycle work, NemoClaw emits each receipt once.
  ...
  These diagnostics do not change recovery behaviour or command exit status.
</AgentOnly>
