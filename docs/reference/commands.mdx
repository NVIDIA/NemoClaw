...
On Linux, if installed OpenShell lifecycle commands cannot retire the default gateway on port `8080`, the installer checks a verified active `nemoclaw-openshell-gateway.service` before a verified NemoClaw-managed gateway PID file.
