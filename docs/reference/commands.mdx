...
### Use Sandbox-First Commands

Sandbox commands require a sandbox name before the action.
For example, run `$$nemoclaw <name> doctor`, not `$$nemoclaw doctor`.

If you omit the name, NemoClaw identifies the action, prints the required command form, and exits without registry recovery.
It lists registered sandboxes when available.
If setup is pending, wait for onboarding to finish or run `$$nemoclaw onboard --resume` after an interrupted run.
...
A registered sandbox can have the same name as an action.
Use the sandbox-first form to address it, for example `$$nemoclaw doctor status`.
