...
Deploy a skill directory to a running sandbox. The command validates the `SKILL.md` frontmatter, which requires a `name` field, creates a bounded secure snapshot, and stages that snapshot privately for the selected agent's native lifecycle command. NemoClaw does not publish into an agent skill directory itself.
...
OpenClaw plugins are a different kind of extension. To install an OpenClaw plugin, refer to [Install OpenClaw Plugins](../manage-sandboxes/install-openclaw-plugins). For a skill, NemoClaw creates an exact host snapshot and transfers it into a private staging directory. It then invokes the pinned native OpenClaw installer in intentional replacement mode for agent `main`. The native installer resolves the active workspace target, replaces a same-name workspace or ClawHub skill through its staged backup and rollback flow, writes its source metadata, and determines precedence and activation. NemoClaw verifies the installed digest plus native `skills list`, `skills info`, and `skills check` results before reporting success. The returned native target must be a canonical `workspace/skills/<name>` or `workspace-*/skills/<name>` path inside `/sandbox/.openclaw`.

The native `main` agent workspace is authoritative. Current configurations normally resolve it to `/sandbox/.openclaw/workspace`; existing configurations that resolve it to a canonical named workspace such as `/sandbox/.openclaw/workspace-main` remain supported. Fresh installs, exact-content retries, and changed-content updates all use that same native replacement path; NemoClaw does not select the workspace, decide whether an existing skill is owned, or keep a parallel list of installed skills. Rebuild and destroy likewise have no NemoClaw skill receipt or inventory to transfer or clean up. Start a new OpenClaw session to load the installed skill; the command does not edit `sessions.json`.

</AgentOnly>
<AgentOnly variant="hermes">

Hermes plugins are different from skills. For a skill, NemoClaw passes the private staged snapshot to the pinned `hermes skills import-local` command. Hermes parses the staged skill, runs its native skill scanner, rejects a project or external same-name skill that would remain active, publishes or replaces profile state with rollback, records the install in its own Skills Hub lock, clears its loader cache, and verifies the active result before returning success. NemoClaw then verifies the published content digest. It does not keep a Hermes skill list or write `/sandbox/.hermes/skills` directly. Start a new Hermes chat session after an install or update; a gateway restart is not required.

A Hermes sandbox image created before the native import capability was added refuses installation with rebuild-and-retry guidance. Rebuilding preserves Hermes' agent-owned skill state; NemoClaw does not fall back to direct publication.

</AgentOnly>
<AgentOnly variant="deepagents">

For Deep Agents Code, NemoClaw passes the private staged snapshot to the pinned `dcode skills import` command for agent `agent`. DCode validates the staged skill, resolves its user skill directory, refuses a project same-name skill that would remain active, and publishes or replaces its own user state with backup and rollback. It then re-runs DCode skill discovery and returns the resolved active path; NemoClaw verifies the content digest at that returned path. NemoClaw does not keep a DCode skill list or write `/sandbox/.deepagents/agent/skills` directly. The managed `dcode` launchers discover newly imported skills on the next session without accepting executable hook configuration. Installation does not enable project hooks or unmanaged MCP files.

A Deep Agents Code sandbox image created before the native import capability was added refuses installation with rebuild-and-retry guidance. Rebuilding preserves DCode's agent-owned skill state; NemoClaw does not fall back to direct publication.
...
Before creating a private sandbox stage, the command reconciles abandoned `.nemoclaw-skill-stage*` directories left by an uncatchable interruption. It removes only same-user, mode-`0700`, regular-file directory trees inside the selected agent state root. If a candidate cannot be proven safe or removed, installation stops and reports the exact path for operator inspection instead of accumulating another stage.

The native publisher is bounded to 90 seconds inside the 120-second SSH operation. OpenClaw's three native post-publish checks each have a 5-second bound, leaving time for digest verification and private-stage cleanup. A publisher or verifier timeout terminates the affected process and directs you to run `$$nemoclaw <name> skill list` before retrying. This prevents a retry from deleting staging content that an earlier native operation could still be reading.

OpenClaw, Hermes, and Deep Agents Code snapshot installation is limited to 1,024 selected files and 64 MiB of total file content. The command refuses larger inputs before transfer or installation begins.

For OpenClaw, Hermes, and Deep Agents Code, fresh installs and updates always delegate target resolution, publication, and lifecycle metadata to the pinned native agent command, then verify the resulting native state and content digest. Chat history is preserved. Follow the agent-specific activation guidance above after installation. Agents without a native skill lifecycle are rejected before sandbox skill state changes.

### `$$nemoclaw <name> skill list`

List skills from the selected agent's own state.

Native listing has a 30-second sandbox command bound. If the selected agent's state cannot be inspected within that bound, the command exits nonzero and directs you to retry after the sandbox becomes reachable.

<AgentOnly variant="openclaw">

This is a thin pass-through to `openclaw skills list` for the primary `main` agent inside the sandbox. NemoClaw does not merge, cache, or maintain a second skill list. Supported native output and filtering arguments are forwarded. A bare `--` separator and agent overrides are refused because the lifecycle commands are bound to the sandbox's primary agent state.
...
This is a thin pass-through to `hermes skills list` inside the sandbox. Supported native filtering and output arguments are forwarded, while a bare `--` separator is refused. NemoClaw does not merge, cache, or maintain a second Hermes skill list.

```bash
nemohermes my-assistant skill list
```

</AgentOnly>
<AgentOnly variant="deepagents">

This is a thin pass-through to `dcode skills list` for DCode's primary `agent` state inside the sandbox. Supported native filtering and output arguments are forwarded. A bare `--` separator and agent overrides are refused because the lifecycle commands are bound to that primary state. NemoClaw does not merge, cache, or maintain a second DCode skill list.

```bash
nemo-deepagents my-assistant skill list
```
...
This is a pass-through to the pinned `openclaw skills remove <skill> --agent main` capability. The native command reads OpenClaw's resolved skill status and removes only the exact active workspace target or the exact active native global target under `/sandbox/.openclaw/skills`. It clears OpenClaw's own ClawHub tracking and reads native state again. Bundled, managed, project, and other external entries are refused. If removing one entry reveals a same-name fallback from the other native location, the command reports that remaining active entry and exits nonzero; run the same command again to remove that newly resolved entry. This is also the supported cutover path for skills installed by older NemoClaw releases. A sandbox image created before this capability was added fails the non-mutating capability probe with rebuild guidance; rebuilding preserves both skill locations. NemoClaw does not consult or update a host-side skill list or ownership receipt.

</AgentOnly>
<AgentOnly variant="hermes">

This is a pass-through to `hermes skills uninstall <skill> --yes`. Hermes resolves the entry from its own Skills Hub lock, removes the corresponding skill path, updates its lock, and invalidates its skill cache. An untracked, builtin, project, or external skill is refused by Hermes instead of being deleted through a host-side path guess. Start a new Hermes chat session for the removal to take effect. A gateway restart is not required.

</AgentOnly>
<AgentOnly variant="deepagents">

This is a pass-through to `dcode skills remove-imported <skill> --agent agent --json`. DCode resolves the skill from its own precedence-aware state and removes it only when the exact active user skill carries matching DCode-native import provenance. Agent-authored, project, builtin, untracked, and mismatched skills are refused. DCode owns that per-skill lifecycle metadata and its transactional removal quarantine; NemoClaw does not choose a directory or keep removal ownership metadata. A sandbox image created before this provenance-bound native removal command was added fails a non-mutating capability probe with rebuild guidance.
