...
| `NEMOCLAW_CUA_BROWSER_ENDPOINT` | loopback HTTP URL with an explicit port | Selects the browser service that NemoClaw projects through `host.openshell.internal`. Credentials, query strings, and fragments are rejected. |
| `NEMOCLAW_CUA_COMPUTER_ENDPOINT` | loopback HTTP URL with an explicit port | Selects the computer service that NemoClaw projects through `host.openshell.internal`. |
| `NEMOCLAW_CUA_TERMINAL_ENDPOINT` | loopback HTTP URL with an explicit port | Selects the terminal service that NemoClaw projects through `host.openshell.internal`. |
| `NEMOCLAW_CUA_FIXTURE_ENDPOINT` | loopback HTTP URL with an explicit port | Selects the synthetic fixture service that NemoClaw projects through `host.openshell.internal`. |

Set all four endpoint variables together during onboarding, sandbox creation, and rebuild. Later `start` and `connect` operations use the recorded relay state when these variables are absent. If supplied for an existing sandbox, all four values must match its recorded state. NemoClaw requires distinct service ports from `1024` through `65535`. Each URL must use `localhost`, `127.0.0.1`, or `[::1]` and must not contain credentials, a query string, or a fragment. The three tool-service URLs must use `/` because NVLumina constructs their routes from the projected host and ports. NemoClaw writes `base_host`, `computer_use_port`, `browser_use_port`, and `terminal_use_port` under `[tool_servers]` in `/app/config/config.toml`. Set `NEMOCLAW_CUA_FIXTURE_ENDPOINT` for the fixture policy only; NemoClaw does not write it under `[tool_servers]`. The fixture URL may use a non-root path, and the generated policy permits `GET` and `POST` at that path and its descendants. The generated policy permits the NemoCUA Python clients to use only `GET` and `POST` at each selected service path and its descendants. Keep each host service bound to loopback; the scenario must configure its service package to listen on `127.0.0.1` or `[::1]` before onboarding. NemoClaw does not convert a wildcard-bound service into a loopback-only service. NemoClaw owns the sandbox bridge relay and OpenShell policy.
