...
Pass `--json` for machine-readable output that includes a `schemaVersion`, the display-selected `defaultSandbox`, the point-in-time `readyDefaultSandbox` for bare `$$nemoclaw connect`, recovery metadata, and the sandbox inventory.
...
The JSON `defaultSandbox` and the default marker in text output honor the same environment override order as host-level status and tunnel commands: `NEMOCLAW_SANDBOX_NAME`, then `NEMOCLAW_SANDBOX`, then `SANDBOX_NAME`, then the registry default.
The JSON-only `readyDefaultSandbox` ignores those display overrides and reports the non-pending sandbox that a bare `$$nemoclaw connect` would select from the recovered registry state.
It is `null` when no registered sandbox is ready.
Bare connect reads the registry again, so a later registry change can produce a different target.
...
For terminal-agent sandboxes, the success line includes the agent and sandbox names so readiness automation can confirm that a bare connect reached the expected registry-selected sandbox.
