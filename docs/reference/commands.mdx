...
| `NEMOCLAW_COMPATIBLE_AUTH_MODE` | `api-key` or `none` | Selects authentication for a custom OpenAI-compatible endpoint in non-interactive onboarding. `none` is accepted only for exact loopback URLs. |
