...
If the shared gateway path is empty, NemoClaw reads only `platforms.whatsapp.extra.session_path` from `/sandbox/.hermes/config.yaml`.
NemoClaw accepts an absolute configured path under `/sandbox/.hermes` that has no `.` or `..` segment.
