...
<Warning title="Recover a Deferred Provider Refresh Failure">
If provider attachment fails after NemoClaw verifies the created sandbox, it preserves the exact identity and policy checkpoint and does not publish the completed sandbox registry entry.
The live OpenShell sandbox can still be `Ready`; the diagnostic reports its observed state and whether its durable identity still matches the checkpoint.
The provider-refresh record stores the current phase and attached provider names, not credentials.
The recovery command attempts to wipe manifest-defined agent state, destroys the exact checkpointed sandbox, and removes its registry entry.
The `--force` option suppresses confirmation:

```bash
$$nemoclaw <sandbox> destroy --force
```

Run the command only for the sandbox named by that diagnostic.
It revalidates the checkpoint and live identity during destruction and refuses to destroy a same-name replacement.
After the command confirms destruction, retry onboarding.
</Warning>
...
| `NEMOCLAW_EXTRA_PLACEHOLDER_KEYS` | whitespace- or comma-separated list of upper-snake env keys | Adds operator-supplied credentials to the matching canonical OpenShell provider so per-profile credentials such as `TELEGRAM_BOT_TOKEN_AGENT_A` use the same policy-bound, out-of-process placeholder injection as the canonical channel token instead of being baked into each Hermes profile `.env` as raw text. Refer to [Extra placeholder keys](#extra-placeholder-keys) for the entry shape and validation rules. |
...
For each entry, NemoClaw adds the credential to the canonical channel or web-search provider selected for the sandbox. This co-locates the extension with the canonical provider's explicit policy binding instead of creating an unbound generic provider.
OpenShell resolves the named credential placeholder to the operator-supplied value at the bound egress boundary.
...
The corresponding canonical channel or web-search provider must be part of the selected sandbox plan; otherwise NemoClaw warns and ignores the extension.

If a referenced env is unset at onboard time, NemoClaw does not add that credential key to the canonical provider, so no placeholder is attached to the OpenShell gateway and no Hermes profile can resolve it.
