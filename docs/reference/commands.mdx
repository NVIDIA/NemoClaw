...
When a bring-your-own-key web search provider (Tavily Search or Firecrawl Search) is selected, it replaces `nous-web` as the Hermes web search and extract backend while the other selected Nous tools remain enabled.
...
Hermes supports Tavily Search and Firecrawl Search through NemoClaw onboarding and does not support Brave Search.
...
To enable Firecrawl in non-interactive mode, set the provider and matching key.
...
When the selector is unset, NemoClaw enables Tavily when `TAVILY_API_KEY` is available, or Firecrawl when only `FIRECRAWL_API_KEY` is available, and ignores `BRAVE_API_KEY` for Hermes.
An explicit Tavily or Firecrawl selection with no matching key exits before sandbox creation.
A web search key that fails validation prints a warning, disables web search for that run, and lets the rest of onboarding continue.
Changing or disabling the web search provider recreates the sandbox because the Hermes backend, environment placeholder, and credential attachment are part of the image.
If you also select the Nous-managed web gateway through Nous Portal OAuth, the selected bring-your-own-key provider replaces `nous-web` while other selected Nous tools remain enabled.
...
| Flag | Description |
|------|-------------|
| `--type <TYPE>` | Provider type (e.g. `tavily`, `firecrawl`, `nvidia`, `openai`, `anthropic`, `generic`) |
...
| Variable | Format | Effect |
|----------|--------|--------|
| `NEMOCLAW_WEB_SEARCH_PROVIDER` | `tavily`, `firecrawl`, or `none` | Selects Tavily Search or Firecrawl Search in non-interactive onboarding or disables web search explicitly. When unset, `TAVILY_API_KEY` implicitly selects Tavily, or `FIRECRAWL_API_KEY` implicitly selects Firecrawl. |
...
| `FIRECRAWL_API_KEY` | Firecrawl Search API key | Supplies and implicitly selects Firecrawl Search when no web search provider is set and no Tavily key is present. NemoClaw validates the key and stores it in OpenShell rather than the sandbox. |
...
Each entry must extend a canonical channel envKey with a non-empty `_<suffix>` (for example `TELEGRAM_BOT_TOKEN_AGENT_A`); the canonical envKeys are `TELEGRAM_BOT_TOKEN`, `DISCORD_BOT_TOKEN`, `SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`, `WECHAT_BOT_TOKEN`, `BRAVE_API_KEY`, `TAVILY_API_KEY`, and `FIRECRAWL_API_KEY`.
