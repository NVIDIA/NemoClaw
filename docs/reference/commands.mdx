...
Supported sandboxes use an immutable managed image, a NemoClaw-owned gateway, and an explicit representable policy.
They can use hosted external inference, the native NVIDIA hosted provider with its default endpoint, or the verified managed local vLLM configuration.
The managed vLLM export is limited to its fixed managed serving identity and complete live runtime evidence.
