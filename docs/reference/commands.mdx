...
A `FAILED` verdict means the endpoint answered both requests identically, so OpenShell forwarded the placeholder verbatim instead of rewriting it on egress, and agent runtimes receive the same auth failure and skip the server.
An expired or revoked credential does not produce `FAILED`: it resolves on the wire and is rejected with a different status than the control, which reports as verified with a detail asking you to check the stored credential value.
Ambiguous differing rejections, endpoints that skip authentication, endpoint outages, policy denials, and unreachable sandboxes report as `unknown` rather than blaming the credential rewrite, and a persisted URL that fails the current authenticated-endpoint boundary is never probed.
