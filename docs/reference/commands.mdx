...
| `--keep-openshell` | Leave OpenShell binaries, gateway service daemons, and gateway service configuration in place, without disabling or removing the OpenShell gateway service or stopping the host `openshell-gateway` daemon. This includes NemoClaw's marked Linux `$XDG_CONFIG_HOME/systemd/user/nemoclaw-openshell-gateway.service` unit, or `~/.config/systemd/user/nemoclaw-openshell-gateway.service` when `XDG_CONFIG_HOME` is unset, upstream Linux package units, and the macOS Homebrew service. |
...
Default-port uninstall removes only NemoClaw's marked Linux gateway unit and NemoClaw-managed entries in `openshell/gateway.env`.
...
Only port `8080` uses the Linux systemd user service or macOS Homebrew service.
Gateways on custom ports run as detached processes and do not change the default gateway service.
...
| `NEMOCLAW_OPENSHELL_GATEWAY_BIN` | path | Advanced override for the `openshell-gateway` binary used by Docker-driver gateway startup when NemoClaw stages its Linux marked user service or uses a permitted standalone fallback. The default port's marked user service accepts only NemoClaw's trusted install paths (`~/.local/bin/openshell-gateway`, `/usr/local/bin/openshell-gateway`, or `/usr/bin/openshell-gateway`); another path fails service staging. The macOS Homebrew service uses the Homebrew formula's gateway binary. Defaults to the binary next to `openshell`, then common install paths. |
| `NEMOCLAW_OPENSHELL_SANDBOX_BIN` | path | Advanced override for the `openshell-sandbox` binary used by Linux Docker-driver gateway startup, including the marked user service environment and standalone fallback. The macOS Homebrew service uses the Homebrew formula's driver layout. Defaults to the binary next to `openshell`, then common install paths. |
