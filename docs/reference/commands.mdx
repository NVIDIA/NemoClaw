...
Because a delivered turn therefore always writes to one of the two streams, a dispatch that exits `0` with no bytes on either stream is reported as a failure: the wrapper prints the recovery guidance to `stderr` and exits with status `1` instead of reporting a successful turn.
Both captured paths pin the sandbox's owning gateway with an explicit `-g`, and neither forwards an interactive terminal on `stdin`; a genuine pipe or redirect is still passed through, so `printf 'ping' | nemoclaw my-assistant agent --agent main` keeps working.
...
If the stdout JSON marks the turn itself incomplete or abandoned — an `incomplete_turn` error kind, an `abandoned` liveness state, or `replayInvalid` — the wrapper exits `1` even when the envelope's own top-level status reports success, because a turn can run every tool successfully and still be abandoned before it produces a reply.
The JSON trace is still written to `stdout` first and unchanged, so the partial tool trace is preserved; the verdict, the markers that produced it, and verify-before-retry guidance go to `stderr`. Tool calls in a partial trace may have already applied side effects, so verify what the turn changed before retrying. An upstream non-zero exit status is passed through unchanged rather than relabelled.
