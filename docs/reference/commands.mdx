...
The JSON output includes at least `schemaVersion`, `name`, `found`, `agent`, `agentDisplayName`, `agentRuntime`, `dcodeAutoApprovalMode`, `model`, `provider`, `recordedRoute`, `liveRoute`, `routeDrift`, `phase`, `gatewayState`, `inferenceHealth`, `rpcIssue`, `hostGpuDetected`, `sandboxGpuEnabled`, `sandboxGpuMode`, `sandboxGpuDevice`, `openshellDriver`, `openshellVersion`, `policies`, `customPolicyTransition`, `baselineExclusions`, `baselineExclusionStates`, `baselineExclusionTransition`, `failureLayer`, `terminalRuntimeHealth`, `servingProcessHealth`, and `dockerPaused`.
`customPolicyTransition` is `null` when custom policy state is settled; otherwise it identifies the interrupted `apply` or `remove` preset that must be reconciled before protected mutations that could change or capture the same policy state.
Follow [Recover an Interrupted Change](../network-policy/configure-policies/create-custom-policy-presets#recover-an-interrupted-change) to complete the recorded operation.
...
`baselineExclusionTransition` is `null` when policy state is settled; otherwise it identifies the interrupted `exclude` or `restore` key that must be reconciled before protected mutations that could change or capture the same policy state.
While it remains, NemoClaw blocks managed MCP and messaging-channel policy changes, Shields down, rebuild, `backup-all`, snapshot creation and restore, and sandbox creation or onboarding.
Destroy is blocked while the sandbox is live or its state cannot be classified; if gateway-scoped checks prove it is already absent, destroy clears the exact pending journal and continues cleanup.
...
A pending custom or baseline policy transaction blocks destroy while the sandbox is live or its state cannot be classified.
If gateway-scoped checks prove that the sandbox is already absent, destroy clears the exact pending journal and continues local cleanup.
...
NemoClaw refuses ordinary `policy add`, `policy remove`, `policy exclude`, and `policy restore` mutations while Shields are down or a Shields transition is incomplete.
For either state, run `$$nemoclaw <name> shields up`, confirm `$$nemoclaw <name> shields status` reports `UP`, and retry the policy command.
If Shields state is corrupt, follow the reported repair guidance instead of changing policy.
Managed MCP and rebuild transactions use narrowly scoped internal authority for their own lifecycle-owned policy writes; generic policy commands cannot use that authority to bypass Shields.
...
Managed MCP-generated entries can appear in the shared policy listing, but ordinary `policy add` and `policy remove` commands refuse to replace or remove them.
Use `$$nemoclaw <name> mcp restart <server>` or `$$nemoclaw <name> mcp remove <server>` instead.
...
For custom presets, NemoClaw records a durable journal before it changes the gateway policy.
If the process stops or registry finalization fails before live and durable state agree, `policy list`, `status`, and `doctor` report the interrupted apply and block conflicting policy and sandbox lifecycle mutations.
Re-run the external `policy add` form as described in [Recover an Interrupted Change](../network-policy/configure-policies/create-custom-policy-presets#recover-an-interrupted-change).
...
`Custom policy repair required` means NemoClaw preserved a durable custom apply or remove journal.
For an apply, rerun the displayed external `policy add` form; for a remove, rerun the displayed exact removal command.
Follow [Recover an Interrupted Change](../network-policy/configure-policies/create-custom-policy-presets#recover-an-interrupted-change) before another policy or sandbox lifecycle mutation.
`repair required — interrupted exclude/restore; rebuild blocked` means NemoClaw preserved a durable baseline transaction journal after a crash or persistence failure; rerun the displayed exact policy command to reconcile it.
While either custom or baseline journal remains, NemoClaw blocks policy and sandbox lifecycle mutations that could change or capture the same state, including managed MCP, messaging-channel, Shields down, rebuild, backup, snapshot, and onboarding work.
Destroy also blocks a live or unclassified sandbox, while an exact gateway-scoped absence proof lets destroy clear the matching journal and continue cleanup.
...
For a custom preset, removal requires the live owned content to match the registry record exactly and refuses externally edited values for manual inspection.
Managed MCP-generated entries must be removed with `$$nemoclaw <name> mcp remove <server>`.
...
It refuses a pending custom or baseline policy transaction before opening a shields-down window, starting backup, or deleting the sandbox, and prints the applicable policy repair guidance.
...
If that sandbox has a pending custom or baseline policy transaction, `backup-all` refuses it before starting a stopped container, changing Shields, or copying state and reports the applicable repair guidance.
...
A pending custom or baseline policy transaction blocks snapshot creation before NemoClaw copies state.
...
A snapshot restore refuses a source with a pending custom or baseline policy transaction before it changes state.
A cross-sandbox restore also refuses to replace an existing destination with either pending transaction before it creates or deletes anything.
