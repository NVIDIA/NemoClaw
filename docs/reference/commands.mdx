...
The controller must attest two unchanged zero-supervisor scans with a stable PID 1 and report `SUPERVISOR_NOT_RUNNING`.
Only then can a local Docker-driver sandbox with the legacy keepalive startup enter transactional container recreation.
NemoClaw backs up manifest-defined state and stops the original through OpenShell before recreating it with a credential-free managed startup command.
It retains the previous container while it verifies replacement identity and health, restores state through the pinned replacement, and restarts and verifies the gateway.
Restoration runs as the sandbox user before the final OpenShell handoff.
The gateway restart must return an authenticated `ok` result and pass the managed settle check.
NemoClaw then stops the replacement through OpenShell and removes the previous container at the irreversible commit point.
It starts the replacement through OpenShell and verifies the final handoff, readiness, and container identity before restoring host forwards.
Failures in replacement health, state restoration, or gateway verification before commit trigger a rollback attempt.
After previous-container removal, handoff or readiness failures cannot roll back; the primary dashboard or API host forward remains stopped.
For state preservation, backup cleanup, and recovery guidance, refer to [Recover the Agent Runtime](../manage-sandboxes/operate-sandboxes/recover-and-rebuild-sandboxes#recover-the-agent-runtime).
...
For managed legacy keepalive recovery, state restoration and gateway verification precede the final OpenShell handoff.
`NEMOCLAW_GATEWAY_RECOVERY_WAIT_SECONDS` controls the additional OpenShell readiness wait after that handoff.
By then, the previous container has been removed, so a readiness timeout cannot trigger rollback and leaves the primary dashboard or API host forward stopped.
Follow the printed host recovery guidance.
