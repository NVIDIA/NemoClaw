...
`sessions rm` and `sessions download` are NemoClaw-side helpers: they read `sessions.json` over `openshell sandbox exec`, resolve the supplied `<sessionKey>` to its current `sessionId`, and act on every `<sessionId>.*` file in the agent's `sessions/` directory.
With no `<sessionKey>`, `sessions rm` wipes every `*.jsonl`, `*.jsonl.lock`, and reset archive under that agent's directory and resets `sessions.json` to `{}`; `sessions download` copies the whole directory (including `sessions.json` and any orphan files) to the host.
