...
Text output adds `Endpoint` for `compatible-endpoint` and `compatible-anthropic-endpoint` under that condition.
JSON output adds `endpointUrl` under the same condition.
...
When participating metadata is missing, invalid, conflicting, too long, has an invalid gateway binding or another path shape, or contains recognized credential material in any URL component, both formats retain the provider and model without exposing the endpoint URL.
The command also omits internal HTTPS-pin adapter routes because their upstream endpoint is intentionally not retained and the adapter URL cannot be reused as an upstream endpoint.
...
The state is `unavailable` when durable registry metadata cannot be read or found, `invalid` when recorded metadata cannot prove the live route, `conflicting` when eligible same-gateway rows disagree, `withheld` when the URL may expose a credential, and `adapter-managed` for an internal HTTPS-pin adapter route.
For `invalid` and `conflicting`, JSON also returns up to eight output-safe names in `affectedSandboxes`; text output prints the same names after `Affected`.
The recovery action tells you to repair the named registrations, align conflicting routes, use a credential-free root or `/v1` API base, or omit endpoint options for an adapter-managed same-provider model change.
