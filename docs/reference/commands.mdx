...
The detection requires `nvidia-smi` to succeed and, on hosts whose firmware does not classify as a known NVIDIA platform (DGX Spark, DGX Station, Jetson, or Tegra), additionally requires that the GPU name does not match the WSL2 d3d12/WDDM placeholder family (`JMJWOA-Generic-*`) and that either the host is not ARM64 Linux (the d3d12/WDDM shim ships on Windows-on-ARM only) or the NVIDIA kernel driver is bound (`/proc/driver/nvidia/` present), so that placeholder shims on non-NVIDIA hardware are not mistaken for real GPUs.
