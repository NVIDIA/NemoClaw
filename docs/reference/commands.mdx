...
For an external Hermes dashboard origin, set `CHAT_UI_URL` to an HTTPS URL before onboarding. Do not include credentials in the URL. NemoClaw retains the loopback OpenShell forward and permits only the configured hostname through Hermes dashboard host validation. This check does not authenticate a reverse proxy. Protect the proxy or other ingress before you expose the dashboard.
