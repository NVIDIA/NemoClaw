...
Bare `$$nemoclaw connect` (no sandbox name) connects to the default sandbox — the one set during onboarding or promoted with `$$nemoclaw use <name>`.
A registered sandbox literally named `connect` keeps the name-first reading, and when no default can be resolved the bare form exits non-zero with guidance instead of connecting.
