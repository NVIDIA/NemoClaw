...
For unversioned development checkouts, NemoClaw tries source-commit images, then the image tagged with the nearest reachable release version, and only uses `:latest` when no version tag is discoverable.
