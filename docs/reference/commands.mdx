...
NemoClaw labels every OpenClaw gateway line with `[gateway]`.
It places the label after a leading timestamp and does not add a second label to an already labeled line.
OpenShell audit output remains unchanged.
