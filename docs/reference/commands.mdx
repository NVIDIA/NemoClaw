...
On Linux, uninstall removes `~/.local/state/nemoclaw` unless you pass `--keep-openshell`.
...
`--keep-openshell` preserves it because the retained managed gateway service and process depend on that state.
