...
| `NEMOCLAW_MODEL_ROUTER_VENV` | directory path | Places the host-side Model Router virtual environment at this path. A relative path resolves from the current working directory. Before creating or replacing the environment, NemoClaw requires at least 3 GiB of free or reclaimable capacity on that filesystem. When the filesystem does not report capacity, onboarding prints the reason and continues. |
