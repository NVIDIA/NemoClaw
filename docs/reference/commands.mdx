...
`--reasoning-effort` accepts `low`, `medium`, `high`, or `default`, and applies only to the `compatible-endpoint` provider.
NemoClaw rejects any other value before it changes the OpenShell route, the agent config, or the host registry.
...
`--reasoning-effort` overrides `NEMOCLAW_REASONING_EFFORT`; when you omit the flag, `$$nemoclaw inference set` applies an exported `NEMOCLAW_REASONING_EFFORT`, and when neither is set the sandbox keeps the effort it already records.
Pass `default` to clear a recorded effort and return the endpoint to its own default.
...
| `NEMOCLAW_REASONING_EFFORT` | `low`, `medium`, `high`, or `default` | Writes `params.extra_body.reasoning_effort` on the model entry in the built OpenClaw config, which OpenClaw merges into the request body of a route that uses `openai-completions`. Applies only to the `compatible-endpoint` provider; unset leaves the endpoint's own default in place, and `default` clears an effort that a sandbox already records. On that provider, any other value fails before NemoClaw touches the provider, policies, the sandbox, or the registry; another provider clears the variable without reading it. |
