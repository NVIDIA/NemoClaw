...
On supported `amd64` and `arm64` Docker hosts, stock onboarding requires a complete immutable release image for the selected shipped agent.
...
On those Docker hosts, catalog or registry resolution failures stop stock onboarding before it creates a new sandbox or deletes and replaces an existing one.
Gateway startup and provider preparation can already have completed before managed-image resolution.
The Docker managed-image path never silently falls back to a Dockerfile.
...
An explicit `--from <Dockerfile>` request remains a separate local Dockerfile build.
NemoClaw shows a Dockerfile build-time estimate only after that explicit local-build path is known.
Stock managed-image launch does not show the estimate because it does not run a local Dockerfile build.

On Docker, managed-image onboarding binds startup to the exact OpenShell sandbox ID, Docker runtime ID, image digest, and startup-profile fingerprint.
It durably commits shared state only after the replacement runtime reports completion.
If cleanup cannot prove that a mutable sandbox name still identifies that exact sandbox ID, NemoClaw quiesces and retains the exact Docker runtime instead of issuing an unsafe name-only delete.
Follow [Managed bootstrap quiesced and retained a sandbox](troubleshooting#managed-bootstrap-quiesced-and-retained-a-sandbox) before removing the retained owner.
...
Native Podman remains unsupported in this release; the native-runtime follow-up in issue #7744 owns that enablement.
Use Docker Engine, Docker Desktop, or Colima.
...
Stop the NIM container, remove any sandbox-owned local Dockerfile image, and delete the sandbox.
Shared immutable managed images remain in Docker's cache for other sandboxes.
...
Register a messaging channel with the sandbox and rebuild so the replacement startup configuration includes the new channel.
...
Malformed JSON or invalid entries fail with a structured error.
For managed images, the trusted startup config generator refuses to start the agent with invalid configuration; an explicit custom Dockerfile runs the same generator during its local build.
...
- Allowed entry fields: `id`, `workspace`, `agentDir`, `tools`, `subagents`, `description`, `model`. Any other key fails configuration application (no implicit credential or env pass-through).
- Allowed `tools` fields: `profile`, `allow`, `deny`. Allowed per-agent `subagents` fields: `delegationMode`, `allowAgents`, `model`, `thinking`, `requireAgentId`. Any other nested key fails configuration application.
...
| `NEMOCLAW_SANDBOX_READY_TIMEOUT` | `180` | Wall-clock timeout for post-create readiness and OpenShell command re-registration after policy application, in seconds. Raise when managed image pull and startup, a local Dockerfile build and gateway upload, in-sandbox boot, or post-policy re-registration exceeds the default. When the post-create deadline expires, onboarding attempts exact-owner cleanup and prints the retry or retained-owner guidance. |
...
The post-create readiness wait removes the orphaned sandbox when exact-owner cleanup is safe so the next `$$nemoclaw onboard` starts clean.
If OpenShell can delete only by mutable name and NemoClaw cannot prove that name still identifies the exact managed bootstrap owner, it leaves the exact Docker runtime quiesced and prints retained-owner recovery guidance instead.
