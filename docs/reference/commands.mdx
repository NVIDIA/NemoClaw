...
If unlock already succeeded and a later rollback cannot re-lock because the config path is unsafe, the command restores the restrictive policy when it can, keeps the Shields down record until config protection is verified, and requires manual intervention.
