...
Before creating a private sandbox stage, the command checks for pre-existing `.nemoclaw-skill-stage*` paths left by an interruption or another sandbox process. A path name, owner, and mode cannot prove that NemoClaw created its contents, so the command never removes it automatically. Installation stops and reports the exact retained path for operator inspection and explicit cleanup before retrying.
...
Native listing has a 30-second sandbox command bound. If that bound expires, the command exits with status 124 and directs you to retry after the sandbox becomes reachable. Other native failures preserve the agent's diagnostics and report the native exit status without suggesting a reachability retry.
...
This is a pass-through to `dcode skills delete <skill> --agent agent --force --json`. DCode resolves the active skill from its own precedence-aware state and applies its native path and symlink checks before deletion. The named removal explicitly authorizes DCode to delete that native entry, including a user skill created by the agent rather than installed through NemoClaw. NemoClaw does not choose a directory, distinguish skill provenance, or keep removal ownership metadata.
