...
| `NEMOCLAW_OLLAMA_PROXY_SKIP_BIND_PROBE` | exactly `1` to enable | Starts the Ollama auth proxy even when Ollama answers on a non-loopback interface. The proxy otherwise refuses to start, because any host that can reach that interface reaches Ollama without the proxy's token. Binding Ollama to loopback is the supported fix; use this override only when you accept that exposure. Export the variable rather than setting it for one command: a later NemoClaw command that respawns the proxy without it refuses to start again. |
