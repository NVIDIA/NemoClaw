...
For Hermes Portable Ollama, `--fresh` replaces the onboarding checkpoint without automatically deleting a private unfinished gateway-provider transaction.
When the recorded journal is canonical and its sandbox, model, provider, credential name, and receipt target match the new run, NemoClaw retains its transaction identity and resumes its exact phase.
This prevents a new session identifier from stranding a provider created by the interrupted run or creating that provider twice.
If the journal is malformed or describes different intent, NemoClaw preserves it and exits with a redacted CLI diagnostic without a Node.js stack trace instead of mutating ambiguous authority.
...
Only `--fresh` also discards the saved onboarding checkpoint; the refresh environment variable affects base-image selection only.
...
Use `--fresh` to ignore any saved onboarding checkpoint and restart the wizard from scratch. This is useful after an interrupted `$$nemoclaw onboard` run when you want to discard that checkpoint instead of continuing it with `--resume`.

<AgentOnly variant="hermes">

Hermes Portable Ollama can recover a matching unfinished gateway-provider transaction independently of the checkpoint.
Refer to the [`--resume` and `--fresh` options](#--resume-and---fresh) for the recovery requirements.
