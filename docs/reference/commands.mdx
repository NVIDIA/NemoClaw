...
`--agent` accepts only `hermes` as a no-op alias and rejects any other value; the OpenClaw-only `--keep-transcript`, `--json`, and `--verbose` flags are not supported on a Hermes sandbox.
