...
When a release changes an excluded entry, first run `$$nemoclaw <name> policy restore <key> --force` to clear the stale record, preview the current scope with `$$nemoclaw <name> policy exclude <key> --dry-run`, and then explicitly exclude it again if you still accept the support impact.
When a release removes the entry, run `$$nemoclaw <name> policy restore <key> --force` only to clear the stale exclusion record; there is no replacement scope to review or re-approve.
...
When the current baseline still defines the entry, the command prints the egress it re-allows.
When the baseline no longer defines the entry, the command states that it will clear only the stale exclusion record.
...
| `--dry-run` | Preview the restored egress without applying changes |
