...
`$$nemoclaw <name> destroy --force` instead completes the destruction without scrubbing the retained volume's adapter entry.
It reports what it kept and still deletes the exact OpenShell providers, so the retained credential placeholder cannot authenticate; onboarding a sandbox with the same name may need to replace the stale MCP adapter config.
...
OpenClaw shields must be down for this config mutation; the managed Mcporter config is locked with the rest of the `workspace` state root.
`$$nemoclaw <name> mcp remove <server> --force` reports the refusal and continues with the host-side provider and policy cleanup instead.
