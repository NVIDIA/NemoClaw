...
If that policy cleanup fails, run the `$$nemoclaw <name> policy remove <channel>` command printed by the CLI.
...
Inspect the channel's gateway providers, restore the preset YAML, correct the gateway failure, and rerun `$$nemoclaw <name> channels add <channel>`.
