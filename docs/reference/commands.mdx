...
keywords: ["nemoclaw cli commands", "nemoclaw command reference", "nemoclaw quarantine", "nemo-deepagents commands", "dcode commands"]
content:
  type: "reference"
---
...
The `quarantine` value is `null` when no fence is active.
An active value contains the phase, exact fence target, bounded operation journal, and SHA-256 reason digest.
