...
When OpenShell rejects the native GPU flag before creation progress, NemoClaw saves redacted diagnostics and proves that the sandbox and its labeled Docker containers are absent without deleting the sandbox by name.
It retries only after that absence proof succeeds.
When a native route or readiness failure requires managed cleanup with exact identities, NemoClaw stops instead of deleting a sandbox by its mutable name or retrying.
...
NemoClaw verifies absence through two stable checks of the OpenShell sandbox list and labeled Docker containers.
If absence cannot be proven safe, onboarding stops and prints cleanup guidance instead.
