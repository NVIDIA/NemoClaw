...
When the sandbox is reachable and Telegram has one global mention mode, the Messaging section compares the stored setting with the rendered agent configuration.
The check reports `mention-only (1)` or `all-messages (0)` and warns when the expected and rendered modes differ.

<AgentOnly variant="openclaw">

For OpenClaw, the Telegram mention-mode check applies when `TELEGRAM_GROUP_POLICY=open`.
The command does not summarize one global mention mode when the policy is `allowlist` or `disabled`.
...
For Telegram configurations with one global mention mode, the output reports `Telegram group mention mode (TELEGRAM_REQUIRE_MENTION)` as `mention-only (1)` or `all-messages (0)`.
If the stored setting differs from the rendered mode, the comparison warns and displays both values.

<AgentOnly variant="openclaw">

For OpenClaw, this Telegram comparison applies when `TELEGRAM_GROUP_POLICY=open`.
The command does not summarize one global mention mode when the policy is `allowlist` or `disabled`.
