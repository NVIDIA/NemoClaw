...
Slack `--wait` applies only to OpenClaw sandboxes.
Other channel manifests return `readiness_not_supported` until they define equivalent readiness checks.
On timeout, the command exits nonzero with `readiness.state`, `readiness.category`, and `readiness.reason` set to `timeout`.
`readiness.retryable` mirrors the last observed channel state, while `readiness.lastObserved.category` and `readiness.lastObserved.reason` preserve the underlying network or runtime cause.
