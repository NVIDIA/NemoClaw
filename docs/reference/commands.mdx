...
The command confirms that the source is a file or directory before it downloads.
Symbolic links and other special source types are refused.
If the command cannot confirm the source type, it exits without downloading.
