...
When it integrates this launcher, the Gabby scenario calls the internal `launchVoiceGateway()` entry point with trusted source paths and runtime fields.
That launcher emits the following child-process contract with no credential paths or values in its arguments or environment.
