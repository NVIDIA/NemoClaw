...
Export a supported registered OpenClaw or Hermes sandbox as a secret-free `NemoClawConfig` document. The same command is available as `$$nemohermes config export <sandbox>`. The command reads the registry and current OpenShell state without changing the sandbox. It stops without writing output when required identity, policy, inference, or workload evidence is missing or inconsistent. If the source changes during observation, the command retries the complete observation once and stops without output if the retry also changes.
...
Hermes export also requires the default API configuration, a disabled dashboard, and no Hermes tool gateways.
...
OAuth authentication and other profiles fail with a diagnostic instead of producing an incomplete document.

<AgentOnly variant="hermes">

### `nemoclaw-acp`

Use `nemoclaw-acp` to connect an external Agent Client Protocol (ACP) client to a compatible managed Hermes sandbox.
The command starts an ACP session and writes protocol traffic only to standard output.
It writes lifecycle diagnostics to standard error.
It requires the supported OpenShell version and the system SSH client on Linux or macOS.

Specify the registered sandbox name.
If the name is registered on more than one gateway, specify its NemoClaw gateway name.

```bash
nemoclaw-acp --sandbox <sandbox-name> [--gateway <nemoclaw-gateway-name>] [--timeout <seconds>]
```

The command accepts a timeout from `1` through `86,400` seconds.
When the timeout expires, it ends the ACP session.

Before it starts the session, `nemoclaw-acp` recovers the selected gateway, verifies the live sandbox identity, and requires the sandbox to be ready.
It accepts only a current managed Hermes sandbox with its recorded immutable lifecycle identity.
It rejects custom-image, stale, missing, ambiguous, stopped, or incompatible targets before it opens the session.

```bash
nemoclaw-acp --sandbox my-hermes-sandbox
```

Use `--help` to print command usage.
Use `--version` to print the installed NemoClaw version.
...
NemoClaw can register one external host component during fresh Linux onboarding with a NemoClaw-managed Docker-driver gateway.
Create the declaration before onboarding at `~/.config/nemoclaw/external-component.json`.
Its owner must be the current user, and its mode must be `0600`.
The declaration and socket-parent directories must not be writable by group or other users.
Both declared Unix sockets must already exist, belong to the current user, and allow access only to that user.
...
`componentId` must contain 1 through 64 letters, numbers, periods, underscores, or hyphens and start with a letter or number.
Both socket paths must be distinct absolute paths.
NemoClaw rejects a declaration with another field, an unsupported schema version, an unsafe owner, mode, path, or socket.

After it verifies the declaration, NemoClaw configures the component as a fail-closed OpenShell interceptor.
The interceptor can modify or validate `CreateSandbox` operations and validate `UpdateConfig` operations.
NemoClaw supports this lifecycle only for a new explicitly named sandbox.
It rejects `--resume`, `--recreate-sandbox`, reuse, repair, providerless APF onboarding, and externally supervised gateways.

After sandbox creation and policy verification, NemoClaw sends one HTTP/1.1 `POST /v1/activate` request to `activationSocketPath`.
The JSON request identifies the component, gateway, sandbox immutable identity, lifecycle generation, and effective policy hash.
The component must return one bounded `200` response with a JSON body that repeats `schemaVersion`, `activationId`, `componentId`, `sandboxId`, and `policyHash`, and sets `result` to `activated` or `rejected`.
NemoClaw waits at most 30 seconds.

If activation is rejected, incomplete, or ambiguous, NemoClaw preserves the sandbox and records the activation evidence.
It does not report onboarding success or retry activation automatically.
...
For an active Portable Hermes receipt, `stop` targets only the receipt-owned Podman container without using Docker as a fallback.
It waits for Podman to report `exited` and OpenShell to report `Error` or `Stopped`.
...
For an active Portable Hermes receipt, `start` follows the Podman container, supervisor, authenticated health, and rollback contract documented for [`recover`](#$$nemoclaw-name-recover).
It then checks and repairs the recorded host forwards before reporting success.
