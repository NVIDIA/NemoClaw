...
The external Gabby integration selects each trusted source path and invokes the launcher.
The identity lifecycle from #8403 owns source-file creation, replacement, removal, and credential revocation, while the launcher owns each descriptor from opening through parent and child closure.
Closing the descriptors or stopping the gateway does not remove a source file or revoke its credential; rotation and removal must replace or delete the source and revoke the old value through that lifecycle owner.

The launcher is an internal library boundary for the external Gabby scenario, not another CLI command and not part of normal NemoClaw managed startup.
Gabby calls the shipped `runVoiceGatewayLaunch()` action with trusted source paths and runtime fields; package-contract coverage launches the real internal command through that production entry point and verifies descriptor cleanup and restart-based credential rotation.
