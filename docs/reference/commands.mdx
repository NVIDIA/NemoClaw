...
For a Docker-managed OpenClaw sandbox, the export can include one or more read-only secondary agents in runtime order.
They can share a hosted route or, on native Linux, a verified attached-Ollama route.
Each secondary agent has a unique name, uses only `tools.allow: [read]`, and shares the primary route, selected model, and retained route tuning.
