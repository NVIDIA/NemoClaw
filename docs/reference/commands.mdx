...
| `NEMOCLAW_EXPERIMENTAL_PROFILE` | `portable` | Hosted installer only. Selects the same hidden Hermes Portable onboarding path as `--experimental-profile portable` when set in the installer environment. Direct `nemohermes onboard` commands ignore this variable; pass `--experimental-profile portable` instead. Successful onboarding records that explicit selection in the receipt-owned Portable lifecycle. Later CLI processes use the receipt when the variable is absent; an explicit non-Portable command value is never overridden. |
