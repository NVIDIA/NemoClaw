...
To enable the experimental DuckDuckGo Web Search backend in non-interactive mode, set the experimental flag, the provider, and accept the third-party software notice (or pass `--yes-i-accept-third-party-software`):
...
`NEMOCLAW_WEB_SEARCH_PROVIDER=duckduckgo` selects the DuckDuckGo backend, which has no API key requirement.
The DuckDuckGo backend is unofficial and rate-limited; do not depend on it for production workloads.
In interactive mode, the wizard prompts for `[brave/duckduckgo]` only when `NEMOCLAW_EXPERIMENTAL=1` is set and `NEMOCLAW_WEB_SEARCH_PROVIDER` is unset; otherwise it defaults to Brave. Pre-setting `NEMOCLAW_WEB_SEARCH_PROVIDER=duckduckgo` (together with `NEMOCLAW_EXPERIMENTAL=1`) skips the provider prompt and selects DuckDuckGo directly in both interactive and non-interactive modes.
