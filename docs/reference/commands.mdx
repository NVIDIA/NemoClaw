...
After an OpenShell administrator verifies the live durable ID, resolves or removes that exact sandbox through an identity-bound procedure, and durably records the resolution, rerun the original onboarding command with the same required provider, model, agent, policy, and environment inputs, add `--fresh`, and use an available sandbox name.
