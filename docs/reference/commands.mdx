...
During initial onboarding, combine `--observability` with `--agent langchain-deepagents-code`.
...
For the complete collector setup, privacy boundary, policy recovery, verification, and a host-side LangSmith exporter example, refer to [Quickstart with LangChain Deep Agents Code](../../openclaw/get-started/quickstart-langchain-deepagents-code#export-traces-through-a-local-collector).
...
Pass `--observability` with `--agent langchain-deepagents-code` to enable backend-neutral runtime traces for Deep Agents Code.
...
For the complete receiver contract and a runnable LangSmith collector setup, refer to [Quickstart with LangChain Deep Agents Code](../../openclaw/get-started/quickstart-langchain-deepagents-code#export-traces-through-a-local-collector).
