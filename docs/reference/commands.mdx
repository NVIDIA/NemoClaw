...
NemoClaw reads the following environment variables to configure service ports, onboarding behavior, and lifecycle defaults. Set them before running `$$nemoclaw onboard` or any command that starts services. After surrounding whitespace is removed, each port must use canonical decimal notation with no leading zero and a value from 1024 through 65535, unless a variable's own description gives a narrower range.
