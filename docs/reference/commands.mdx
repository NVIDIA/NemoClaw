...
For OpenSSH egress, use a custom preset with the exact destination host and SSH port.
SSH traffic is not HTTP, so the endpoint must use L4 passthrough (`access: full` and `tls: skip`) and should stay scoped to the OpenSSH, Git, and netcat proxy binaries.
Existing sandboxes created before OpenSSH egress support must be rebuilt so the image has the OpenSSH client, CONNECT proxy helper, and password prompt wrapper.

```yaml
preset:
  name: company-ssh
  description: "OpenSSH access to company bastion"
...
For an internal host alias, add the alias to the sandbox and scope the policy to the resolved IP address.
The OpenSSH proxy helper uses `/etc/hosts` aliases for the proxy CONNECT target, so users can still type the alias while OpenShell evaluates the IP-scoped policy:

```console
$ nemoclaw my-assistant hosts-add bastion 10.0.0.5
```

```yaml
preset:
  name: company-ssh-private
  description: "OpenSSH access to private company bastion"

network_policies:
  company_ssh_private:
    name: company_ssh_private
    endpoints:
      - host: 10.0.0.5
        port: 22
        access: full
...
Built-in and custom policy presets applied to the old sandbox are reapplied to the new one so your egress rules survive the rebuild.
