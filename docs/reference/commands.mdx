...
If the Dashboard path has credentials and the gateway path is empty, the report recommends updating or rebuilding the sandbox.
It also tells you to pair again from Dashboard so credentials use `/sandbox/.hermes/platforms/whatsapp/session`, then rerun `channels status`.
