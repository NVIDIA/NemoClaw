...
| `NEMOCLAW_SANDBOX_READY_TIMEOUT` | `180` | Wall-clock timeout for the post-create readiness wait and for OpenShell re-registration after managed recovery recreation, in seconds. Raise when the sandbox image build, gateway upload, in-sandbox boot, or control-plane re-registration exceeds the default (typical on 70B+ models, first-time gateway uploads over slow links, or DGX Station / remote-VM first runs). When the deadline expires, onboarding deletes an orphaned sandbox and prints the retry hint; managed recovery leaves the primary dashboard or API host forward stopped. |
