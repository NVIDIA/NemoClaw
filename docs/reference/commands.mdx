...
For Hermes setups, uninstall verifies and stops the selected gateway's managed port-forward watcher and sandbox-scoped forward before deleting their state. If either cleanup cannot be confirmed, uninstall returns nonzero and preserves the watcher state so the cleanup can be retried without touching sibling gateways.
