...
Only when WeChat appears in neither the messaging plan nor the current OpenShell policy can cleanup be a no-op if the registry entry is missing, the provider does not support stopped-state cleanup, or no eligible stopped runtime resource exists.
