...
The build target is declared by `min_openclaw_version` in `nemoclaw-blueprint/blueprint.yaml` and mirrored by `OPENCLAW_VERSION` in the NemoClaw Dockerfiles.
