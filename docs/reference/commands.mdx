...
| `NEMOCLAW_EXTRA_AGENTS_JSON` | JSON array of OpenClaw secondary-agent entries | Adds secondary agents to `agents.list` in the built OpenClaw config. See [Extra OpenClaw agents](#extra-openclaw-agents) for the entry schema, path constraints, and validation rules. |
...
- `workspace` and `agentDir` must be absolute paths that resolve under `/sandbox/.openclaw/`.
