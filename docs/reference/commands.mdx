...
One exception applies when the supplied path is the selected agent's own managed Dockerfile (for example `agents/hermes/Dockerfile` in the NemoClaw checkout the CLI runs from), NemoClaw stages the repository root as the build context, exactly as the managed build does, because that Dockerfile copies repository-root paths.
