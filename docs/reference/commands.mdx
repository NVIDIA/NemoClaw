...
| `NEMOCLAW_ONBOARD_VALIDATION_TIMEOUT_SECONDS` | Probe-specific | Connection and total deadlines for applicable non-streaming OpenAI-compatible provider validation during onboard. Positive finite seconds round up and cap at `600`; the value only raises existing deadlines. It does not extend the fixed five-second streaming-event probe. |
