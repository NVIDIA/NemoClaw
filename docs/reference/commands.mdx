...
For a running Podman sandbox whose in-sandbox gateway has stopped, `recover` delegates to the registered runtime provider.
...
If the Podman container itself is stopped, run `$$nemoclaw <name> start` before `recover`.
