...
| Flag | Description |
|------|-------------|
| `--save-host <path>` | Copy each successful backup directory into `<path>/<sandbox>/<timestamp>/` after the snapshot is written. The destination is created if it does not exist. Use this before `nemoclaw uninstall` so the backups survive the `~/.nemoclaw/` wipe |
...
| `--save-host <path>` | Copy the snapshot directory into `<path>/<sandbox>/<timestamp>/` after creation. The destination is created if it does not exist |
...
When uninstall detects snapshots or rebuild backups under any of the protected paths inside `~/.nemoclaw/` (currently `~/.nemoclaw/rebuild-backups/` and `~/.nemoclaw/backups/`), live registered sandboxes in `~/.nemoclaw/sandboxes.json`, or running Docker containers matching the sandbox/gateway pattern, it prints the affected sandboxes, names the destructive surface, and refuses to proceed.
...
To keep your workspace state across uninstall, copy fresh backups into a path outside `~/.nemoclaw/` before re-attempting uninstall.
