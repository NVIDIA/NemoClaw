...
For a sandbox that uses the Docker driver, `docker_unreachable` means `docker info` did not return valid server data. It does not show that the sandbox container is missing. If Docker is stopped, follow [Docker is not running](troubleshooting#docker-is-not-running). If Docker is running, follow [Docker permission denied on Linux](troubleshooting#docker-permission-denied-on-linux). Make `docker info` succeed, then retry `$$nemoclaw <name> start`.
