...
It starts an unreachable gateway only when the recorded gateway name exactly matches the selected target.
A conflicting or ambiguous gateway identity, an authentication, schema, timeout, or other observation failure stops the adapter before gateway startup or session creation.
