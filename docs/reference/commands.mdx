...
If the OpenShell request is unreachable or times out, sandbox state remains unverified.
The request might have reached OpenShell before the connection failed.
This failure does not prove that Docker is stopped or the sandbox was removed.
Preserve the sandbox; do not rebuild, destroy, or re-onboard it to resolve a connection failure.
For a Docker-backed gateway, run `docker info` on the owning gateway's host to inspect the error before choosing a remedy.
...
For context or TLS errors, correct the reported Docker configuration.
Restore access to the owning OpenShell gateway, then run `$$nemoclaw <name> status` before retrying `start`.
