...
The detection requires `nvidia-smi` to succeed and, on hosts whose firmware does not classify as a known NVIDIA platform (DGX Spark, DGX Station, Jetson, or Tegra), additionally requires the NVIDIA kernel driver to be bound (`/proc/driver/nvidia/` present on Linux) and the GPU name to not match the WSL2 d3d12/WDDM placeholder family (`JMJWOA-Generic-*`), so that placeholder shims on non-NVIDIA hardware are not mistaken for real GPUs.
