...
`--sandbox` is required.
When more than one registered Hermes sandbox has that name, pass `--gateway <name>` to select its owning NemoClaw gateway.
...
It can start a gateway only when the adapter observes that the exact recorded selected gateway is unreachable.
