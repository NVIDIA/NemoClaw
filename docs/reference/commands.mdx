...
The `recover` command requires a standard Podman sandbox to be running and ready in OpenShell. It does not restart or replace the provider-owned container. If the sandbox is stopped, run `$$nemoclaw <name> start` first.
