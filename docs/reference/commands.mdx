...
When uninstall detects snapshots or rebuild backups under `~/.nemoclaw/rebuild-backups/`, it prints the affected sandboxes, names the destructive surface, and exits without removing anything.
The gate fires even with `--yes` so non-interactive automation cannot silently drop user data.
Run `nemoclaw backup-all` and copy `~/.nemoclaw/rebuild-backups/` to a safe path outside `~/.nemoclaw/` before re-running uninstall, or set `NEMOCLAW_UNINSTALL_DESTROY_USER_DATA=1` to acknowledge the loss and proceed.
Workspace files inside the sandbox container (`/sandbox/.openclaw/workspace/`) live in the container's writable layer and are destroyed with the container regardless of this gate.
Use `nemoclaw backup-all` first to capture container workspace contents into the host-side snapshot directory before copying it out.
