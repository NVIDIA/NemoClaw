...
For Model Router sandboxes, `destroy` keeps the process and recovery identity when it cannot prove that teardown is safe.
...
If `destroy` keeps the Model Router recovery identity:
