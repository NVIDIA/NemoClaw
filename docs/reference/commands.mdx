...
For native NVIDIA Endpoints inference, export supports `nvidia-prod` only with OpenShell's built-in NVIDIA profile, its default endpoint, and no provider configuration overrides.
...
With `--json` and file output, the version 1 result includes the source sandbox, output path, and document and specification digests.
...
Before reserving an inference route, onboarding releases an abandoned route-only reservation from an earlier session while holding the onboarding lock.
It reports that release and preserves reservations that remain owned by another active operation.
