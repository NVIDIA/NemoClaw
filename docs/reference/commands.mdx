...
If managed catalog or registry resolution is unavailable during rebuild, the Docker workflow can use the trusted canonical Dockerfile fallback.
It does not substitute an unpinned managed `:latest` image.
