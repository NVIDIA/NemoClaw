...
When the live OpenShell gateway inference route differs from the route recorded for the target sandbox, `connect` warns and re-points the shared route to that sandbox's recorded provider and model.
This switch lets registered sandboxes with different valid routes time-share the gateway.
Incomplete route metadata and invalid gateway bindings remain hard errors.
...
Connecting to a sandbox re-points the route to that sandbox's recorded provider and model.
