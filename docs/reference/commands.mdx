...
When a release changes an excluded entry, first run `policy restore <key> --force` to clear the stale record, preview the current scope with `policy exclude <key> --dry-run`, and then explicitly exclude it again if you still accept the support impact.
When a release removes the entry, run `policy restore <key> --force` only to clear the stale exclusion record; there is no replacement scope to review or re-approve.
...
The command prints the egress the restore re-allows, then asks for the same explicit acknowledgement as `policy exclude`.
Use `--force` or `--yes` for non-interactive acknowledgement, or `--dry-run` to preview the egress that would be restored.
