...
With no `<sessionKey>`, `sessions download` copies the whole directory (including `sessions.json` and any orphan files) to the host.
...
```bash
nemoclaw <name> sessions reset <agent> <session> [--reason new|reset]
```
