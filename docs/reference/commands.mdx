...
| Variable | Format | Effect |
|----------|--------|--------|
| `NEMOCLAW_YES` | `1` to enable | Auto-accepts confirmation prompts (`--yes` equivalent) including in helpers like the Ollama proxy auth setup, but does not auto-answer the interactive managed-vLLM storage prompt. Express and other non-interactive setup paths print the managed-vLLM storage warning and continue for now. |
