...
The v0 command projects verified source state into a pre-release v1alpha1 YAML shape.
It does not validate the document with the unreleased v1 parser.
For managed OpenClaw and Hermes sandboxes, `image: null` is a temporary placeholder until the v1 release defines the target image value.
Each exported sandbox contains exactly one inline `harness` and one singular `agent` with one inline `inference` definition.

A hosted single-agent export has this structure:
...
The exporter emits one primary agent for a single-agent OpenClaw sandbox.
It rejects an OpenClaw sandbox that has retained secondary agents because the current v1 contract requires one agent per sandbox.
