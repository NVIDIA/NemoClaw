...
Host-side removal is the supported path because it updates the messaging plan, OpenShell providers, network policy, and agent config together. An OpenClaw config change made inside the sandbox can survive snapshot restore, but it does not update those host-side resources. Hermes continues to regenerate its image-owned messaging config during rebuild.
