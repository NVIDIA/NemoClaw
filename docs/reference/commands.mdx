...
The dedicated vLLM onboarder rejects `NEMOCLAW_VLLM_MODEL` and `NEMOCLAW_VLLM_EXTRA_ARGS_JSON` before it installs vLLM.
`NEMOCLAW_VLLM_PORT` may select an unused host listener.
...
The command reports each profile's stable ID, display name, inference backend, model, topology, selection mode, support state, validation level and evidence, estimated downloads, and incompatibility reason.
...
| `NEMOCLAW_LLAMACPP_PORT` | 8081 | Existing and managed llama.cpp inference |
...
`NEMOCLAW_GATEWAY_PORT` also cannot overlap configured service, vLLM, llama.cpp, Ollama, Ollama proxy, Bedrock Runtime adapter, OpenRouter runtime adapter, or HTTPS Pin Runtime adapter ports, and cannot use reserved auto-allocation ranges or the default inference/proxy ports `8000`, `8081`, `11434`, `11435`, `11436`, `11437`, and `11438`.
Port `8081` is reserved for authenticated existing-server attachment and the managed llama.cpp runtime. The configured llama.cpp port cannot be assigned to another configurable NemoClaw service port.
Each runtime adapter port must be distinct from the gateway, dashboard, vLLM, llama.cpp, Ollama, Ollama proxy, dashboard allocation range, and other runtime adapter ports.
...
| `NEMOCLAW_VLLM_MODEL` | catalog alias, served name, or Hugging Face model ID | Selects a model through compatible managed-inference presets and recipes and remains authoritative during DGX Station installer setup. The packaged catalog defines the aliases, including `qwen3.6-27b`, `qwen3.6-35b-a3b-nvfp4`, `muse-glimmer-30b`, `nemotron-3-nano-4b`, `nemotron-3.5-lightning-30b`, `deepseek-v4-flash`, `deepseek-v4-flash-0731`, `nemotron-3-ultra-550b-a55b`, and `deepseek-r1-distill-70b`. NemoClaw checks host architecture, GPU compute capability and memory capacity, topology, and pinned runtime compatibility before it selects a recipe. The Muse Glimmer and Nemotron 3.5 Lightning generic Linux amd64 profiles remain Experimental after bounded Brev verification. NemoClaw does not enable vision or DFlash speculative decoding for Muse Glimmer. Station Express selects `nemotron-3-ultra-550b-a55b`; a qualified reciprocal pair uses the distributed topology, while no qualifying pair retains the single-Station Ultra topology. Outside Station Express, unset uses the per-platform profile default. Gated models (for example, `deepseek-r1-distill-70b`) require `HF_TOKEN` or `HUGGING_FACE_HUB_TOKEN`. |
