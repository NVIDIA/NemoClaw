...
The endpoint is omitted (and `endpointUrl` is `null`) when the registry records none, when no configured provider owns it, and for the default sandbox when the live gateway route has drifted to a different provider than the stored one, because the stored endpoint no longer describes the route in use.
...
<AgentOnly variant="hermes">
Experimental portable Hermes reports its portable lifecycle phase instead and does not probe an inference route.
</AgentOnly>
