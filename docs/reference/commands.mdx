...
<Warning title="Endpoint Evidence Required">
  OpenShell provider metadata currently exposes configuration key names, but not an independently
  read endpoint value. Config export does not treat its registry endpoint as independent evidence, so
  current live exports stop with `missing-provenance` and do not write output.
</Warning>

When independent endpoint evidence is available, write a file on Linux:

```bash
$$nemoclaw config export my-assistant --output nemoclaw.yaml
```

Under the same condition, use `--output -` to write YAML to standard output on another supported host or in a pipeline:
...
When the required endpoint evidence is available, version 1 exports managed OpenClaw sandboxes that use hosted external inference, an immutable managed image, a NemoClaw-owned gateway, and an explicit representable policy. Other profiles fail with a diagnostic instead of producing an incomplete document.
