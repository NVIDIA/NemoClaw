...
For a sandbox that uses the Docker driver, `docker_unreachable` means NemoClaw could not reach the Docker daemon; it does not show that the sandbox container is missing. Start Docker, confirm that `docker info` succeeds, and retry `$$nemoclaw <name> start`. Refer to [Docker is not running](troubleshooting#docker-is-not-running) for recovery steps.
