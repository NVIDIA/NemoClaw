...
Full single-gateway Podman uninstall for the portable experimental profile uses exact receipt-owned durable retirement.
It preserves shared OpenShell providers, the gateway service and process, binaries, configuration, model stores, and Podman images under the [portable authority, retry, and preservation contract](../manage-sandboxes/operate-sandboxes/uninstall-nemoclaw).
