...
| `--config <K=V>` | Typed non-secret provider configuration. Supported: `OPENAI_BASE_URL=<http(s)://public-host/path>` with `--type openai`. URLs with credentials, query parameters, fragments, loopback, link-local, private, internal, or private DNS destinations are rejected. Configure trusted private inference endpoints through onboarding so NemoClaw can preserve their trust and address pins. Repeatable |
