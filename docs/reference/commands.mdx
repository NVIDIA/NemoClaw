...
If NemoClaw obtained a durable identity fingerprint but could not save recovery evidence, rerun `$$nemoclaw <sandbox-name> destroy --yes`. If recovery remains blocked, preserve the terminal output and ask an OpenShell administrator for identity-bound recovery.
