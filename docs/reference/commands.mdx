...
| `NEMOCLAW_EXPERIMENTAL_PROFILE` | `portable` | Selects the same hidden Hermes Portable onboarding path as `--experimental-profile portable`. Successful onboarding records this explicit selection in the receipt-owned Portable lifecycle. Later CLI processes use that receipt when the variable is absent; an explicit non-Portable command value is never overridden. |
