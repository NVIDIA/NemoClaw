...
Before the review, the wizard collects all available choices, including the provider and model, sandbox name, resource profile, custom CPU and RAM values, sandbox GPU setting, and policy tier. It requests provider credentials only after you select **Apply configuration**.
...
OpenShell `0.0.99` evaluates that policy before replacing the attached provider placeholder in the allowed request header.
...
OpenShell `0.0.99` mutates providers by name, so do not concurrently replace a managed provider through another OpenShell client during this command.
