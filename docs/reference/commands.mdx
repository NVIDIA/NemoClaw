...
That line is labeled `Inference (configured):` and reports the route recorded in the registry; this command runs no inference health probe, so the line never reports reachability.
It appends the upstream endpoint as `Inference (configured): <provider> / <model> (<endpoint>)` when the registry records one, and reports the same value as `endpointUrl` on each JSON sandbox row.
Credentials embedded in that endpoint are redacted before it is reported.
The endpoint is omitted (and `endpointUrl` is `null`) when the registry records none, and for the default sandbox when the live gateway route has drifted to a different provider than the stored one, because the stored endpoint no longer describes the route in use.
For a live `Inference: healthy (<endpoint>)` reachability result, run `$$nemoclaw <name> status`, which probes the sandbox's inference route and reports the probe endpoint rather than the configured upstream.
