...
When participating metadata is missing, invalid, conflicting, too long, has an invalid gateway binding, or contains recognized credential material in any URL component, both formats retain the provider and model without adding endpoint details.
...
If the registry cannot be read for a compatible custom route, the command exits non-zero.
Known corruption and permission errors retain their safe recovery steps and affected path.
Unclassified errors use a generic message without exposing registry contents or error details.
That message directs a direct lookup to `$$nemoclaw status` and a sandbox-first lookup to `$$nemoclaw <name> status`.
