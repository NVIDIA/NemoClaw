...
Before the final-sandbox gateway decision, `destroy` confirms through `openshell sandbox list` that no live sandbox remains on the gateway.
A listed sandbox counts as live unless it is in the `Error` or `Failed` phase and NemoClaw confirms that its Docker container is gone.
...
If the deleted sandbox is still listed after that wait, if a different live sandbox remains, or if the list command fails, `destroy` leaves the gateway running, names the live sandboxes or the failed command, and exits 0.
When you passed `--cleanup-gateway`, the message states that the flag was not applied.
