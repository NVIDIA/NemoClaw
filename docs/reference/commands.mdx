...
Each Hermes profile `.env` references the matching `${KEY}` placeholder, so no raw token lands on disk.

Entries are split on whitespace and commas, must match `^[A-Z][A-Z0-9_]{0,127}$`, and cannot collide with a canonical channel envKey (`TELEGRAM_BOT_TOKEN`, `DISCORD_BOT_TOKEN`, `SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`, `WECHAT_BOT_TOKEN`, `BRAVE_API_KEY`).
...
If a referenced env is unset at onboard time, the matching provider row is skipped and the placeholder is not registered with the OpenShell gateway.
