...
For an ambiguous or incomplete identity, stop the gateway and restore the exact generated `openshell-gateway.toml` and complete `jwt/` directory from a trusted host-level backup.
...
Restore them as the owning user, keep both directories private, and do not grant group or other access to the configuration or JWT files.
