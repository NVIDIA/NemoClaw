...
To commit the replacement, NemoClaw stops it, removes the rollback backup, waits for OpenShell to retire the previous lifecycle record, starts the replacement as the final container lifecycle event, and verifies OpenShell supervisor readiness again within the same handoff deadline.
