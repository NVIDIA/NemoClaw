...
It also restores supported operator-managed configuration that you set with `nemohermes <name> config set`.
The completion report lists restored and dropped configuration keys.
If it cannot capture, restore, or verify those values, rebuild exits nonzero and retains recovery state.
