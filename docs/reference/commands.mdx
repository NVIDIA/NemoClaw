...
`NEMOCLAW_INSTANCE` must be 1–32 lower-case alphanumerics with internal `-` (for example `agent-a`, `tenant1`). The default instance keeps the original `~/.nemoclaw` state root and `nemoclaw` gateway name verbatim, so existing single-instance deployments observe no change. Setting `NEMOCLAW_INSTANCE` to a non-default value before `$$nemoclaw onboard` scopes the credentials directory, rebuild backups, and gateway binding to that instance; combine with `NEMOCLAW_GATEWAY_PORT` to compose the instance and port suffixes (`nemoclaw-<instance>-<port>`). Full per-instance segregation of registry, snapshots, and messaging is tracked separately as follow-up.
