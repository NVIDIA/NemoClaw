...
For a Docker-managed OpenClaw sandbox with hosted inference or fixed managed vLLM, the export can include one or more secondary agents in runtime order.
Each secondary agent has a unique name, shares the primary inference route, and uses only `tools.allow: [read]`.
For managed vLLM, every agent shares the verified serving identity, route tuning, and `contextWindow: 65536`.
The export rejects the complete document when any secondary agent has a model that differs from the primary model, unsupported tools, execution, interfaces, authentication, observability, a primary-agent override, or a default subagent setting.
