...
If the device is not settled and has no pending request, NemoClaw runs one bounded request producer on the owning gateway.
It waits for the same-device request, approves at most once, and verifies the final settled state.
Portable onboarding accepts one already-pending canonical write upgrade, avoids a duplicate producer, and still requires strict same-device settlement before completion.
...
Resume skips their secret prompts when the same session recorded a successful OpenShell provider registration and OpenShell still reports the expected name, type, and credential keys.
...
The selected menu entry determines the recipe even when `NEMOCLAW_LLAMACPP_RECIPE` names another recipe.
...
Managed settle confirmation treats `SUPERVISOR_BUSY` as inconclusive within its configured window.
...
For an active Portable Hermes receipt, `start` revalidates and recovers the receipt-owned Podman container and manifest-owned Hermes startup command without using Docker as a fallback.
A `pending` or `configuring` receipt instead directs you to resume Portable onboarding.
...
You can supply hosts through `NEMOCLAW_TRUSTED_PRIVATE_HOSTS` instead, and NemoClaw combines the variable with command options.
NemoClaw resolves each matching host and adds generated `allowed_ips` pins to an in-memory copy of the preset.
...
Pass a repeatable `--trusted-private-host <exact-host-or-ip>` option to admit a RFC1918, CGNAT, or IPv6 unique local destination for the current command.
...
`--force` may remove a modified same-name agent adapter entry, but provider deletion still requires the recorded ID and credential key plus an accepted managed provider type, and policy deletion still requires owned content.
...
NemoClaw removes only a volume with ownership labels; inspection or removal failure exits nonzero and preserves registry state for retry.
</Warning>
</AgentOnly>
