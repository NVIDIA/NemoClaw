...
For an ambiguous or incomplete identity, stop the gateway and restore the exact generated `openshell-gateway.toml` and complete `jwt/` directory from a host-level backup captured from that gateway's exact state directory before the failure and kept under the owning user's exclusive access.
