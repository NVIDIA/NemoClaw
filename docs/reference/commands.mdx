...
`NEMOCLAW_INSTANCE` must be 1–32 lower-case alphanumerics with internal `-` (for example `agent-a`, `tenant1`). Hyphen-separated segments may not be purely numeric: this rules out collisions with the gateway port suffix (without the rule, `agent-a` plus `NEMOCLAW_GATEWAY_PORT=8081` and `agent-a-8081` plus the default port would produce the same composed name).

The default instance name is the historical singleton identity NemoClaw has always used. Leaving `NEMOCLAW_INSTANCE` unset, setting it to the empty string, or setting it explicitly to that default value all resolve to the same identity: the state root stays at `~/.nemoclaw` and the OpenShell gateway keeps its bare historical name (`nemoclaw-<port>` for non-default ports; no suffix on the default port). Existing single-instance deployments therefore observe no change after upgrading. When the bare default gateway is already in use, varying `NEMOCLAW_GATEWAY_PORT` alone produces a `nemoclaw-<port>` gateway that segregates from the bare default without requiring `NEMOCLAW_INSTANCE` — the port suffix is the implicit fallback identity for the default instance.

Setting `NEMOCLAW_INSTANCE` to a non-default value before `$$nemoclaw onboard` routes the NemoClaw home directory, credentials store, rebuild backups (and the snapshot tree underneath), local inference adapter state including the Ollama auth-proxy token, and gateway binding (name, state directory, compatibility container) to that instance; combine with `NEMOCLAW_GATEWAY_PORT` to compose the instance and port suffixes (`nemoclaw-<instance>-<port>`). Modules that still read `~/.nemoclaw` directly — the sandbox registry, onboard session state, messaging configuration, host-side mount roots, the uninstall plan, and the `config-io` host-root permission healer — continue to share state across instances; follow-up PRs migrate those surfaces.
