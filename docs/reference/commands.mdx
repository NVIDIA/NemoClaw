...
For a Hermes WhatsApp sandbox, `--channel whatsapp` checks only the shared gateway session path at `/sandbox/.hermes/platforms/whatsapp/session/creds.json`.
If the file is missing, pair with `hermes whatsapp` inside the sandbox and rerun `$$nemoclaw <sandbox> channels status --channel whatsapp`.
