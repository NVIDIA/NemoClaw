...
The `toolDiscovery` JSON field contains `ok`, `count`, `tools`, `truncated`, and `commandStatus`, plus a redacted `detail` on failure or a bounded partial result. A discovery failure also reports `failedStage` as `preflight`, `runtime`, `initialization`, or `tool-discovery`, and `failureClass` as `precondition`, `runtime`, `connection`, `authentication`, `protocol`, or `tool-operation`. A structured runtime failure keeps `ok` false when `commandStatus` is `0`, and a non-zero command status is always a failure. These names are the server's point-in-time advertised tools, not an attestation of the exact tools visible to the model after agent filters, progressive disclosure, or session state. An older sandbox image without the shared client or with an incompatible discovery protocol reports that the sandbox must be rebuilt.
