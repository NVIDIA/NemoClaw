...
OpenClaw plugins are a different kind of extension. To install an OpenClaw plugin, refer to [Install OpenClaw Plugins](../manage-sandboxes/install-openclaw-plugins). For a skill, NemoClaw creates an exact host snapshot and transfers it into a private staging directory. It then invokes the pinned native OpenClaw installer in intentional replacement mode for agent `main`. The native installer resolves the active workspace target, replaces a same-name workspace or ClawHub skill through its staged backup and rollback flow, writes its source metadata, and determines precedence and activation. NemoClaw verifies the installed digest plus native `skills list`, `skills info`, and `skills check` results before reporting success.

The primary `main` agent must use the default `/sandbox/.openclaw/workspace` directory. The command checks this support boundary and refuses a custom `main` workspace before native publication. Fresh installs, exact-content retries, and changed-content updates all use that same native replacement path; NemoClaw does not decide whether an existing skill is owned or keep a parallel list of installed skills. Rebuild and destroy likewise have no NemoClaw skill receipt or inventory to transfer or clean up. Start a new OpenClaw session to load the installed skill; the command does not edit `sessions.json`.
