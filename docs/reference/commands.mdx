...
<AgentOnly variant="openclaw">
| `NEMOCLAW_SKIP_GOOGLECHAT_TUNNEL` | `1` to enable | Skips the interactive Google Chat webhook-tunnel derivation and Google Cloud Console confirmation during onboard, accepting a pre-supplied `GOOGLECHAT_AUDIENCE` instead. Use only in hermetic test environments; unlike the Slack and Telegram probes this skips a required operator step, so the channel receives no inbound events unless you configured the Console endpoint out of band. The audience is still shape-validated. |
</AgentOnly>
