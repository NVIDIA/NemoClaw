...
Before the final-sandbox gateway decision, `destroy` confirms through `openshell sandbox list` that no sandbox remains on the gateway.
...
If any sandbox is still listed after that wait, or the list command fails, `destroy` leaves the gateway running, names the listed sandboxes or the failed command, and exits 0.
When you passed `--cleanup-gateway`, the message states that the flag was not applied.
After `openshell sandbox list` reports no sandboxes, run `openshell gateway remove <gateway-name>` to remove the gateway.
