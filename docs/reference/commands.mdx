...
| `NEMOCLAW_HF_DOWNLOAD_STALL_TIMEOUT` | `600` (10 minutes) | How long a Hugging Face model pre-download may run with zero new output before onboard aborts it as stalled. Accepts a positive finite number of seconds up to the Node.js timer limit (about 24.8 days). Blank, invalid, non-positive, sub-millisecond, or larger values use the default. Unlike `NEMOCLAW_OLLAMA_PULL_TIMEOUT`, this is an inactivity timeout, not a total wall-clock cap: a slow but progressing multi-hundred-GB transfer is not affected, only a complete stall is. Raise it on very high-latency links where output can legitimately pause this long between progress updates. |
