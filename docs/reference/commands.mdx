...
| `NEMOCLAW_SANDBOX_READY_TIMEOUT` | `180` | Shared wall-clock deadline for post-create publication, durable identity settlement, and executable readiness. Raise it only when OpenShell needs longer after the create command returns to publish the sandbox, settle its durable identity, or reach executable `Ready`. A post-create readiness failure preserves the sandbox for the identity-bound retained-sandbox recovery procedure above. |
