...
The OpenShell exec endpoint rejects any argument that contains a newline or carriage return, so multi-line commands (for example a `bash` heredoc) cannot be passed through `exec`. NemoClaw detects this before dispatch, names the offending argument position, and exits with status `2` rather than surfacing the lower-level OpenShell `InvalidArgument` error. Join the statements with semicolons (`exec -- bash -lc "cmd1; cmd2"`) or run a script file inside the sandbox (`exec -- bash <script-path>`) instead.
