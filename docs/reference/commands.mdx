...
When ordinary `connect` finds that its recorded `ollama-local` route is unavailable, it uses the same recovery path before it opens an interactive session.
The `recover` command also uses that path.
...
<AgentOnly variant="hermes">
For Portable Hermes, `connect`, `connect --probe-only`, and `recover` can resume a stopped published Ollama runner only for the recorded `ollama-local` provider.
Ordinary `connect` starts recovery only when its recorded inference route is unavailable.
For another configured provider, `connect --probe-only` and `recover` verify the recorded inference route and required forwards without Ollama recovery.
...
The `recover` command requires a Podman sandbox to be running and ready in OpenShell before it delegates an in-sandbox gateway failure to the registered runtime provider.
The provider restarts the exact managed container without changing its identity.
If provider recovery fails, NemoClaw reports the provider diagnostic and stops before it restores host-side forwards.
If the Podman container itself is stopped, the command cannot reach provider recovery through the readiness gate; run `$$nemoclaw <name> start` before `recover`.

For providers that use the managed gateway controller, `recover` sends an authenticated lifecycle request through registry-scoped privileged direct-container control.
...
If `SUPERVISOR_NOT_RUNNING` remains after the bounded retries, NemoClaw leaves the container unchanged and directs you to rebuild or re-onboard.
If the gateway is already running, the command does not force-restart it; it can still re-evaluate supported safety checks and check or recover host-side forwards, and it exits zero only when those forward checks succeed.
...
It allows 30 seconds for forward verification and 60 seconds for the complete inspection, start, and verification operation.
...
| `NEMOCLAW_GATEWAY_RECOVERY_WAIT_SECONDS` | `30`, `90`, or `120`, depending on the recovery phase | Wall-clock timeout for OpenShell command re-registration after policy application, plus gateway health and OpenShell readiness during managed or provider recovery. A valid finite, nonnegative value overrides the internal budget for the current recovery phase. |
