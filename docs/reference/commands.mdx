...
An ARM64 Linux host that is native or Docker Desktop-backed WSL clears a `JMJWOA-Generic-*` name only when one bounded `docker run --gpus all nvcr.io/nvidia/k8s/cuda-sample:vectoradd-cuda12.5.0` workload succeeds, which may first pull that image; other non-firmware-vouched hosts still fail closed.
