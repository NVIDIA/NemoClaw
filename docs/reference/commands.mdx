...
If a provider command fails without confirming that no change occurred, provider state may be partial; rerun onboarding before using the route or retrying `inference set`.
If NemoClaw cannot restore the previous selection, resolve the reported restore failure, then rerun onboarding.
