...
Run the printed selector command.
Do not retry the restore or delete the destination by name while the selector reports a sandbox or OpenShell cannot confirm the result.
If the selector and destination name are both absent, rerun the original restore command.
