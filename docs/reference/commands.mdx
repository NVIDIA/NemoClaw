...
NemoClaw requires four distinct service ports. It appends their sandbox-facing URLs to `/app/config/config.toml` and limits the NemoCUA Python clients to those ports. Keep each host service bound to loopback; NemoClaw owns the sandbox bridge mapping and OpenShell policy.
