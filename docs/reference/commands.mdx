...
Supported sandboxes use hosted external inference, including native NVIDIA hosted inference, an immutable managed image, a NemoClaw-owned gateway, and an explicit representable policy.
...
For an active Portable Hermes receipt, `stop` targets only the exact receipt-owned Podman container and waits for its terminal stopped state and for OpenShell to report `Error` or `Stopped`.
It does not use Docker as a fallback.
