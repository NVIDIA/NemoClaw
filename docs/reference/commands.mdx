...
Routed onboarding and Model Router destruction use the same host-wide lifecycle lock for each Model Router port.
Onboarding holds the lock through sandbox registry publication.
Destruction holds the lock through the peer check, process stop, and session update.
