...
| `--force` | Add the channel when another sandbox uses the same messaging credential or the conflict check cannot complete. For Slack, this flag also overrides a same-gateway conflict with different credentials. Slack does not offer an interactive conflict override. |
