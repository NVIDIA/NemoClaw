...
For normal turns, the wrapper inherits the remote command's exit code and streams whatever `openclaw agent` emits without adding host-side buffering. When the top-level OpenClaw `--json` output flag is present, the wrapper uses a captured no-TTY path with a 64 MiB buffer so `stdout` stays parseable JSON; raw `stderr` is forwarded, and failed-tool or untrusted-child provenance found in the stdout JSON is appended to `stderr`. Literal `--json` values consumed by flags such as `-m` or `--reply-channel`, or arguments after `--`, stay on the normal passthrough path.
