...
For an active Portable Hermes receipt, `connect --probe-only` can resume the exact published Ollama runtime when it is stopped.
It verifies an already-running runtime without starting it.
The recovery uses the active receipt, registry row, published runtime receipt, and committed gateway state.
It does not select another runtime or provider, or change the registry row or gateway provider generation.
If recovery or the final route verification fails after a start, NemoClaw restores the exact runtime to its prior stopped state before it reports the failure.
Interactive `connect` and `launch` verify the published runtime but do not resume it.
