...
On a detected DGX Station, the installer also accepts these mutually exclusive express-install selections:

| Installer flag | Effect |
|---|---|
| `--station-deepseek` | Select DeepSeek V4 Flash while retaining the Station express flow. |
| `--station-experimental-single-user` | Explicitly opt in to the reclaimed-v1/offload90 Nemotron Ultra profile. The canonical Station recipe remains the default when this flag is absent. |

Pass installer arguments after `bash -s --`:

```bash
curl -fsSL https://www.nvidia.com/nemoclaw.sh | bash -s -- --station-experimental-single-user
```

The experimental flag requires the exact qualified local Docker image ID `sha256:e44830d3f0197850e5a63ca895b13a047f358ff4aea15b1655abf61391bb883e` to be preloaded.
No pullable repository manifest is currently proven for that image, so fresh installs fail closed instead of substituting another vLLM runtime.
Refer to [Set Up vLLM](../inference/local-inference/set-up-vllm#opt-in-to-the-experimental-single-user-station-profile) for the exact runtime contract and image check.
...
| `NEMOCLAW_VLLM_PROFILE` | profile name | Selects an explicit managed-vLLM runtime profile. `experimental-single-user` is restricted to DGX Station and selects the reclaimed-v1/offload90 recipe; leaving this variable unset preserves the existing per-model and per-platform behavior. Prefer the installer flag `--station-experimental-single-user`, which validates conflicting selections and sets the matching model and served name. |
...
| `NEMOCLAW_IGNORE_VLLM_DISK_SPACE` | exactly `1` | Allows managed vLLM to continue when verified Docker image-storage or registered model-cache capacity is insufficient; other values do not bypass either check. Inconclusive checks report diagnostics and continue automatically. |
<AgentOnly variant="openclaw">
