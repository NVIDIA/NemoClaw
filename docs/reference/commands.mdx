...
If users want to type a short alias such as `ssh user@bastion`, add it with `nemoclaw my-assistant hosts-add bastion 10.0.0.5`; the SSH proxy resolves `/etc/hosts` aliases before dialing, so the policy can still stay scoped to the resolved IP address.
