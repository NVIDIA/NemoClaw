...
| `NEMOCLAW_DISABLE_SUPERVISOR_RELAUNCH` | `1` to enable | Skips the automatic in-sandbox supervisor relaunch during `$$nemoclaw <name> recover` when the supervisor is gone, for example after a container restart. Use only as a troubleshooting escape hatch; recovery then falls back to the rebuild or re-onboard guidance. |
