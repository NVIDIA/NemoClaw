...
The reason is stored in the receipt, so it must not contain credentials, prompts, messages, or workspace content.
...
Receipt fields and diagnostics are bounded and redacted, but you must still keep the reason and idempotency key secret-free.
...
It preserves the quarantine receipt before it clears the fence.
A failure to preserve the pre-release receipt or changed target authority stops release and leaves the fence active.
...
If the final receipt update fails after fence clearance, release exits `2`, and the sandbox remains stopped.
...
An active value contains the phase, exact fence target, bounded operation journal, and secret-free reason.
