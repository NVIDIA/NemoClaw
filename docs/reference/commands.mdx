...
Docker and Podman gateways cannot reuse one state directory. To switch drivers, use the applicable `$$nemoclaw uninstall` path that removes the existing gateway state, then retry onboarding. To run both drivers concurrently, select an unused gateway port with `NEMOCLAW_GATEWAY_PORT=<port>`; if needed, also set `NEMOCLAW_OPENSHELL_GATEWAY_STATE_DIR=<path>`.
