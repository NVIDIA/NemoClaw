...
The export contains credential environment-variable references, not credential values. File output uses mode `0600`. Without `--force`, an existing destination is preserved and the command fails. The command refuses symbolic links and other non-regular output types. With `--force`, it verifies that the existing destination is a regular file before replacement.

With `--json` and a file output, the command returns a version 1 result with the source sandbox, output path, and document and specification digests. Standard output contains only YAML when `--output -` is used.
