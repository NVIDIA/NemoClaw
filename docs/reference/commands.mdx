...
Use `--resume` to continue a resumable onboarding session with the provider, model, sandbox name, agent, observability choice, Deep Agents auto-approval capability, and custom Dockerfile path recorded by the original run.
...
#### `--dcode-auto-approval <disabled|thread-opt-in>`

Configure whether a managed Deep Agents Code sandbox permits explicit thread-scoped auto-approval.
The default `disabled` mode keeps the TUI auto-approval choice and `dcode -y` unavailable.
The `thread-opt-in` mode makes those explicit activation paths available, but it does not turn auto-approval on for a thread by itself.

```bash
$$nemoclaw onboard --dcode-auto-approval thread-opt-in
nemoclaw onboard --agent langchain-deepagents-code --dcode-auto-approval thread-opt-in
```

When a thread opts in, tool calls, including shell commands, can run without further confirmation until a thread boundary resets the state.
A new `dcode` process, `/clear`, `/force-clear`, a thread switch or resume, and an agent switch each return to manual approval.
The TUI shows the active-state indicator and warning for the current thread.
The host-side `$$nemoclaw <name> status` output reports the configured capability instead of the state of a live TUI thread.

The positive mode applies only to the built-in managed `langchain-deepagents-code` image.
NemoClaw rejects `thread-opt-in` for OpenClaw, Hermes, and custom `--from` images.
Pass `--dcode-auto-approval disabled` when you need to revoke the capability or clear recorded Deep Agents state before switching agents.
Changing the mode recreates the sandbox because the mode lives in a root-owned image file, and ambient runtime environment values cannot enable it.

```bash
$$nemoclaw my-dcode rebuild --dcode-auto-approval disabled --yes
```

The capability does not bypass OpenShell policy, credential isolation, managed inference, managed MCP validation, or other managed runtime controls.
It also does not change the separate `dcode -n` headless boundary.
...
| `--dcode-auto-approval <disabled\|thread-opt-in>` | Change the managed Deep Agents Code thread auto-approval capability. Enabling prints a warning, and either value requires sandbox recreation. |
