...
If an extension environment variable is unset during onboarding, NemoClaw omits that extension from the provider credential set.
NemoClaw creates or reuses the canonical provider when its primary credential is available.
No placeholder exists for the unset extension.
Export the extension credential before onboarding when that profile requires it.
