...
The build target comes from `OPENCLAW_VERSION` in the NemoClaw Dockerfiles; `min_openclaw_version` in `nemoclaw-blueprint/blueprint.yaml` remains the lower-bound compatibility floor for the pinned blueprint image artifact.
