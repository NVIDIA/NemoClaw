...
For any registered-sandbox upgrade that you prepare manually, use the current prepared NemoClaw CLI to require strict backup and retire all applicable registered legacy forwards before you retire the old gateway.
...
Set `NEMOCLAW_OPENSHELL_UPGRADE_PREPARED=1` only after both commands succeed and the backup command reports a complete result.
...
When `recover` repairs a stopped built-in OpenClaw or Hermes gateway, it repeats the managed-controller probe only while a bounded startup, discovery, lease-contention, or pinned-container restart transition remains unconfirmed. Any other result ends recovery.
...
During restore, the command keeps the replacement OpenClaw gateway stopped while it restores state and writes host-managed messaging and MCP configuration.
After those offline write steps, it runs `openclaw doctor --fix` while the gateway remains held down.
It then releases that maintenance window for one native OpenClaw start.
...
State replacement runs under the per-sandbox mutation lock.

<AgentOnly variant="openclaw">
For OpenClaw, an in-place restore stops the sandbox before it changes state, installs the complete credential-sanitized native `openclaw.json` file without a selective merge or config hash, and runs `openclaw doctor --fix` while the gateway remains held down.
This interrupts active agent work.
It then releases the window for one native OpenClaw start.
If restore fails after maintenance begins, the CLI reports whether the sandbox is stopped or its running state could not be verified.
For a verified stopped sandbox, run `$$nemoclaw <name> start`.
For an unverified running state, run `$$nemoclaw <name> stop`, then `$$nemoclaw <name> start`.
Verify that the sandbox is ready, then retry the same snapshot restore command.
</AgentOnly>

In-place restore does not change the OpenShell policy.
A cross-sandbox clone reads the source live policy and hands that document to destination creation.
...
<AgentOnly variant="openclaw">
After NemoClaw creates the destination, it stops the sandbox and holds its OpenClaw gateway down while it restores state and runs `openclaw doctor --fix`.
It then releases the maintenance window for one native OpenClaw start.
If restoration or doctor fails, NemoClaw attempts to stop the destination and reports whether that stop was verified.
After correcting the reported failure, retry with a new clone name.
The incomplete clone remains registered; inspect it before deletion or replacement with `--force`.
</AgentOnly>
...
If NemoClaw cannot confirm the native OpenClaw config batch, the OpenShell route and registry remain committed, but `inference set` exits with status `1`.
Rerun the same `inference set` command to converge the config.
A rebuild restores the previous native `openclaw.json`, so it does not repair this failure.
...
It does not run sandbox, inference, messaging, agent-version, or agent-service checks.
