...
Let onboarding create the directory when possible. Its containing directory must be owned by the current user or root and must not be group- or world-writable. If the state directory already exists, it must be an owner-controlled, non-symbolic-link directory with mode `0700`.
If onboarding stops immediately after reserving that directory, uninstall removes the marker-only reservation only while the selected gateway port is free; any gateway configuration, runtime marker, PID file, or live listener makes cleanup fail closed and preserve the directory. When a listener occupies the port, inspect and stop that listener, then rerun uninstall after the port is free.
