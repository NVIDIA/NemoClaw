...
For an active Portable Hermes receipt, `nemohermes <name> connect --probe-only` revalidates the recorded published Ollama runtime and inference route.
If the exact recorded runtime is stopped, it resumes that runtime and verifies the route before it restores missing manifest-declared host forwards.
The command does not select, install, or pull a model.
If it cannot prove the recorded runtime, registry, or private publication authority, it exits nonzero without using another runtime.
...
The flag wins, then the environment variables in that order.
NemoClaw checks the sandbox on its registered owning gateway.
An unknown sandbox, an invalid gateway binding, a denied OpenShell observation, or a sandbox absent from its owning gateway exits nonzero without writing a tarball.
The error identifies the name and the source environment variable when applicable.
Without an explicit name, `$$nemoclaw debug` selects the registered default sandbox, or the first registered sandbox when no default exists.
It exits nonzero when that selected sandbox is stale or its gateway binding is invalid.
