...
For hosted OpenClaw inference, export preserves each context-window, maximum-token, reasoning, and reasoning-effort setting as a route override when omitting it would change the target's effective behavior.
...
For OpenClaw, it retains an `interfaces.dashboard` marker because an absent target dashboard disables the interface.
For the managed default port and loopback bind, the marker includes `port: 18789`; otherwise, it includes each retained port or bind value that differs from the target default.
...
For supported settings, export compares the source behavior with the v1 target behavior when that setting is absent.
It omits a setting only when absence gives the target the same effective behavior.
Hermes export fails when Hermes tool gateways are enabled.
It preserves Hermes dashboard and browser terminal UI behavior against the target defaults.
A disabled dashboard or disabled browser terminal UI remains explicit because absence enables it in the target.
It also preserves nondefault dashboard or internal ports and a nondefault allocated API port under `interfaces`.
