...
Before starting the adapter, the bounded VoiceClaw launcher opens two owner-only regular files without following symbolic links and maps only these credential descriptors into the child process:
...
Gabby calls `launchVoiceGateway()` with trusted source paths and runtime fields; package-contract coverage launches the real internal command through that entry point and verifies descriptor cleanup and restart-based credential rotation.
