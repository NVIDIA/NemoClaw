...
`--reasoning-effort` accepts `low`, `medium`, `high`, or `default`, and sends the value as `reasoning_effort` in the request body of a model routed through `openai-completions`.
Omit the flag to keep the recorded effort, or pass `default` to clear it and return the endpoint to its own default.
...
| `NEMOCLAW_REASONING_EFFORT` | `low`, `medium`, `high`, or `default` | Sends `reasoning_effort` in the request body of a model routed through `openai-completions`. Unset, or `default`, leaves the endpoint's own default in place. Any other value fails before NemoClaw touches the provider, policies, the sandbox, or the registry. |
