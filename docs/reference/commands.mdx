...
Deploy a skill directory to a running sandbox. The command validates the `SKILL.md` frontmatter, which requires a `name` field. It snapshots selected non-dot regular files while preserving their subdirectory structure, then follows the selected agent's installation contract.
...
Secure snapshot installation is limited to 1,024 selected files and 64 MiB of total file content. The command refuses larger inputs before contacting the sandbox.
