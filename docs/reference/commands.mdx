...
| `--force` | Add any supported channel when another sandbox uses the same messaging credential, a required conflict check cannot complete, or a channel-owned check requires abort. This flag is the only conflict override. Examples include Slack on a shared gateway and Microsoft Teams on a shared webhook port. |
