...
| `NEMOCLAW_INSTANCE` | `nemoclaw` | NemoClaw instance identity for multi-instance hosts. Scopes the state root (`~/.nemoclaw` for the default `nemoclaw` instance, `~/.nemoclaw-<instance>` otherwise) and the OpenShell gateway name so two instances on the same host stay segregated. |
...
The default instance name is `nemoclaw` — the same identity NemoClaw has used historically. Leaving `NEMOCLAW_INSTANCE` unset, setting it to the empty string, or setting it to `nemoclaw` explicitly all resolve to the same identity: the state root stays at `~/.nemoclaw` and the gateway name stays at `nemoclaw`. Existing single-instance deployments therefore observe no change after upgrading. When the bare `nemoclaw` gateway is already in use, varying `NEMOCLAW_GATEWAY_PORT` alone produces a `nemoclaw-<port>` gateway that segregates from the bare default without requiring `NEMOCLAW_INSTANCE` — the port suffix is the implicit fallback identity for the default instance.

Setting `NEMOCLAW_INSTANCE` to a non-default value before `$$nemoclaw onboard` routes the NemoClaw home directory, credentials store, rebuild backups, local inference adapter state, and gateway binding (name, state directory, compatibility container) to that instance; combine with `NEMOCLAW_GATEWAY_PORT` to compose the instance and port suffixes (`nemoclaw-<instance>-<port>`). Modules that still read `~/.nemoclaw` directly — the sandbox registry, onboard session state, snapshot trees, and messaging configuration — continue to share state across instances; follow-up PRs migrate those surfaces.
