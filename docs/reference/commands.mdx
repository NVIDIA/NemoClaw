...
| `NEMOCLAW_SANDBOX_READY_TIMEOUT` | `180` | Wall-clock timeout for OpenShell readiness checks after the sandbox creation stream ends. Image build and gateway upload finish before this budget starts. Raise it when in-sandbox boot or OpenShell registration exceeds the default. Ordinary readiness failures print a lifecycle receipt. |
...
An ordinary post-create readiness failure also prints a lifecycle receipt with `readiness_gate`, `readiness_reason`, `create_stream_status`, `timeout_seconds`, and `terminal_resolution`.
NemoClaw prints the retry command only after it deletes the sandbox or confirms that the sandbox is already absent.
If deletion fails, NemoClaw marks the sandbox as retained and prints a manual cleanup command without a retry command.
Docker GPU compatibility cleanup records a deferred terminal resolution and omits the generic retry command.
