...
Use this option to create a new sandbox whose policy is supplied during OpenShell creation.
...
- Do not combine the option with `--experimental-profile portable`.
...
If post-create verification fails, NemoClaw preserves the incomplete sandbox because automatic deletion would use its mutable name.
Do not destroy that sandbox by name.
Retain the reported sandbox name and durable identity fingerprint for comparison only.
