...
For a same-provider model change, omit the endpoint options so NemoClaw reuses the endpoint recorded during onboarding for the target sandbox.
Missing or `inference set` provenance and every explicitly supplied different URL remain subject to the full address validation above.
