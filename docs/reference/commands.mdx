...
When participating metadata is missing, invalid, conflicting, too long, has an invalid gateway binding or another path shape, or contains recognized credential material in any URL component, both formats retain the provider and model without adding endpoint details.
...
To switch only the model for an existing adapter route, omit the endpoint option so NemoClaw reuses the recorded route.
If the live route is readable but optional endpoint registry metadata is not, both formats retain the live provider and model and omit the endpoint without exposing registry details.
Run `$$nemoclaw status` after a direct lookup or `$$nemoclaw <name> status` after a sandbox-first lookup when an expected endpoint is absent.
