...
After native Podman onboarding fails before NemoClaw records provider ownership, keep `NEMOCLAW_GATEWAY_RUNTIME=podman` set when you run `destroy`.
When the sandbox registry or gateway state records a runtime provider, `destroy` uses that provider instead of the current selector.
If both locations record a provider, they must agree.
Resolve a reported provider mismatch before you rerun `destroy`.
