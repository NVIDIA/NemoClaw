...
That caller must create, replace, and remove each source file and revoke old credential values, while the launcher owns each descriptor from opening through parent and child closure.
