...
| `NEMOCLAW_OLLAMA_NO_AUTOSTART` | `1` to enable | Disables the wizard's auto-start of a local Ollama daemon (equivalent to passing `--no-ollama-autostart`). When set and Ollama is not running on `localhost:11434`, the `nemoclaw onboard` Local Ollama path prints a warning and falls back to the default Ollama model instead of spawning `ollama serve`. |
