...
For Hermes Portable Ollama, `--fresh` replaces the onboarding checkpoint without deleting a private unfinished gateway-provider transaction.
When the journal is canonical and its sandbox, model, provider, credential name, and receipt target match, NemoClaw keeps the transaction identity and resumes its exact phase.
If the journal is malformed or describes different intent, NemoClaw preserves it and exits with a redacted diagnostic instead of mutating ambiguous authority.
