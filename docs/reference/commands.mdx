...
On supported `amd64` and `arm64` hosts, stock onboarding requires a complete immutable release image for the selected shipped agent.
...
If catalog or registry resolution fails, stock onboarding stops before mutation.
NemoClaw never silently falls back from a managed image to a Dockerfile.
