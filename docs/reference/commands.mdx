...
<AgentOnly variant="openclaw">
  ...
  When accepted launch-readiness evidence lets `connect --probe-only`, `recover`, or the readiness probe performed by `start` skip additional lifecycle work, NemoClaw attempts to emit each receipt once.
