...
On a host that matches an explicit experimental hardware preset, the provider menu offers **NVIDIA Nemotron with managed llama.cpp**.
...
For prerequisites, external traffic, verification, and recovery, refer to [Set Up llama.cpp](../inference/local-inference/set-up-llama-cpp#install-managed-llamacpp-on-an-nvidia-gpu-host).
