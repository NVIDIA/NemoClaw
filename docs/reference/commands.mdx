...
Select the `build`, `cloud`, or `routed` NVIDIA hosted provider.
...
If a credential is present, the installer runs normal validated onboarding.
An invalid credential fails validation and does not select deferred onboarding.

After the runtime process supplies `NVIDIA_INFERENCE_API_KEY`, run the onboarding command for the selected agent:
...
|----------|--------|--------|
| `NEMOCLAW_DEFER_ONBOARDING` | `1` to enable | Hosted installer only. Selects the same behavior as `--defer-onboarding` for Hermes or LangChain Deep Agents Code with NVIDIA hosted inference. When the required credential is absent and no sandbox is registered, the installer installs NemoClaw without creating a provider, sandbox, or completed onboarding state. |
