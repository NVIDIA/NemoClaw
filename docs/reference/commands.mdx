...
<AgentOnly variant="hermes">
  Portable Hermes lifecycle recovery also emits one credential-free `Hermes Portable lifecycle recovery timing:` line.
  It reports qualification, container start, currentness, exec readiness, authenticated health, startup, rollback, action, count, total, and result fields.
