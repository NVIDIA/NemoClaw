...
For a detailed channel with a status probe, `--json` returns the `schemaVersion`, `sandbox`, `channel`, and `report` envelope.
A paused channel retains that envelope even when NemoClaw skips its live probe.
