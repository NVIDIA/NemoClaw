...
It then prints the gateway start guidance for your host.
That guidance names `$$nemoclaw onboard` where NemoClaw starts the gateway, and the owning deployment plus `openshell gateway select <gateway>` where it does not.
