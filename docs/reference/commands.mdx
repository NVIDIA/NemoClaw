...
OpenClaw and Deep Agents snapshot installation is limited to 1,024 selected files and 64 MiB of total file content. The command refuses larger inputs before transfer or installation begins.
...
This is a pass-through to the pinned `openclaw skills remove <skill> --agent main` capability. The native command reads OpenClaw's resolved skill status, removes the skill only when that active entry is the exact `main` workspace target, clears OpenClaw's own ClawHub tracking for that target, and reads native state again. It refuses to delete a bundled, managed, or otherwise non-workspace active skill. If removing the workspace entry reveals a same-name fallback from another OpenClaw source, the command reports that remaining active entry and exits nonzero. NemoClaw does not consult or update a host-side skill list or ownership receipt.
