...
For unversioned development checkouts, NemoClaw tries the image tagged with the nearest reachable release version before source-commit images, and only uses `:latest` when no version tag is discoverable.
