...
Combine this flag with `--agent langchain-deepagents-code`; NemoClaw rejects it for OpenClaw and Hermes sandboxes.
...
Resuming an interrupted session with a conflicting explicit choice fails instead of changing the trace boundary mid-session.
The Restricted tier suppresses the preset, so the sandbox cannot reach the collector through this opt-in on that tier.

Deep Agents Code sends metadata-only, headerless OTLP/HTTP protobuf traces to the fixed local endpoint `http://host.openshell.internal:4318/v1/traces`.
The sandbox cannot configure a remote endpoint, exporter headers, credentials, or a backend.
For the collector contract, privacy fields, policy caveats, and a host-side LangSmith exporter example, refer to [Quickstart with LangChain Deep Agents Code](../../openclaw/get-started/quickstart-langchain-deepagents-code#export-traces-through-a-local-collector).
...
A rebuild also preserves the recorded Deep Agents Code observability choice and the matching local OTLP policy state.
...
Pass `--observability` with `--agent langchain-deepagents-code` to enable metadata-only runtime traces for Deep Agents Code.
...
The sandbox sends headerless OTLP/HTTP protobuf requests only to `http://host.openshell.internal:4318/v1/traces`.
...
The collector must validate and redact received attributes before remote forwarding because the local policy applies to the managed Python interpreter and does not provide authenticated tenant identity.
For the complete receiver contract and a LangSmith collector fragment, refer to [Quickstart with LangChain Deep Agents Code](../../openclaw/get-started/quickstart-langchain-deepagents-code#export-traces-through-a-local-collector).
