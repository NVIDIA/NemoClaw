...
If OpenShell rejects the native `--gpu` option before creation progress, NemoClaw does not delete the sandbox by name.
It saves redacted diagnostics and proves that the sandbox and its labeled containers are absent before retrying.
For another eligible failure, NemoClaw retries only after cleanup proves that the sandbox and its labeled containers are absent.
If absence or cleanup cannot be proven safe, onboarding stops and prints cleanup guidance instead.
