...
| `NEMOCLAW_AUTO_PAIR_DEADLINE_SECS` | positive number of seconds up to `1000000000000` | Sets the total lifetime of the in-sandbox OpenClaw auto-pair watcher. Defaults to `28800` seconds (8 hours). Each poll and sleep remains inside this deadline. |
| `NEMOCLAW_AUTO_PAIR_FAST_DEADLINE_SECS` | positive number of seconds up to `1000000000000` | Retains a legacy auto-pair setting in the managed runtime environment. The current watcher does not read this setting, so changing it has no effect and no default applies. |
| `NEMOCLAW_AUTO_PAIR_RUN_TIMEOUT_SECS` | positive number of seconds up to `2147483` | Sets the timeout for each OpenClaw CLI command that the auto-pair watcher runs. Defaults to `10` seconds. |
| `NEMOCLAW_AUTO_PAIR_SLOW_INTERVAL_SECS` | positive number of seconds up to `1000000000` | Sets the in-sandbox OpenClaw auto-pair watcher cadence after the canonical local CLI device reaches the exact baseline scopes and has no same-device pending request. Defaults to `5` seconds. Before that settlement, the watcher polls every `1` second. Raise only on load-sensitive gateways. |
| `NEMOCLAW_AUTO_PAIR_FAST_REENTRY_POLLS` | positive integer up to `9007199254740991` | Sets how many fast polls run after the watcher observes a fresh allowlisted scope-upgrade request. Defaults to `5`; set lower only when you need to reduce gateway polling. |
| `NEMOCLAW_AUTO_PAIR_FAST_REENTRY_INTERVAL_SECS` | positive number of seconds up to `1000000000` | Sets the fast-reentry interval after a fresh allowlisted scope-upgrade request. Defaults to `1`. |
...
NemoClaw rejects `Infinity`, `NaN`, unparseable text, and values outside the ranges of these six auto-pair controls before it creates the sandbox. The managed image entrypoint enforces the same ranges. A direct process environment assignment that bypasses the managed entrypoint uses the documented default when the watcher cannot use the value.
