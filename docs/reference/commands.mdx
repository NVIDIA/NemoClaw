...
When uninstall detects other gateway-port environments, it names each one, gives the `NEMOCLAW_GATEWAY_PORT=<port>` command that removes it, and points at the whole-host sweep.
...
The sweep enumerates the gateway state roots under `~/.nemoclaw/gateways/`, confirms once against the resulting port list, then uninstalls each other port before the port `NEMOCLAW_GATEWAY_PORT` selects.
