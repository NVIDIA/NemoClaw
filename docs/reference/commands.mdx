...
For every other provider, it verifies the recorded inference route and required forwards without attempting Ollama recovery.
The `recover` command uses the same provider-specific inference path, but ordinary `connect` does not.
Follow [Recover Portable Published Ollama](../manage-sandboxes/operate-sandboxes/recover-and-rebuild-sandboxes#recover-portable-published-ollama) for `ollama-local` authority checks, route proof, rollback, and failure handling.
</AgentOnly>
