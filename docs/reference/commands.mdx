...
NemoClaw requires four distinct service ports from `1024` through `65535`. Each URL must use `localhost`, `127.0.0.1`, or `[::1]` and must not contain credentials, a query string, or a fragment. NemoClaw appends the sandbox-facing URLs to `/app/config/config.toml` and limits the NemoCUA Python clients to those ports. Keep each host service bound to loopback; NemoClaw owns the sandbox bridge mapping and OpenShell policy.
