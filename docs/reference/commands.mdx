...
#### `--from-image <repository>@<digest>`

Onboard a published OpenClaw or Hermes image with the Docker runtime without building a Dockerfile.
Supply an immutable SHA-256 digest with `--from-image` or `NEMOCLAW_FROM_IMAGE`.
In non-interactive mode, supply a sandbox name with `--name` or `NEMOCLAW_SANDBOX_NAME`.
The image publisher owns its content and compatibility with your NemoClaw release.

```bash
$$nemoclaw onboard --name my-agent --from-image <repository>@sha256:<digest>
```

The image must match the runtime platform and declare a non-root user, `/sandbox` working directory, and startup command.
It must contain `NEMOCLAW_TOOL_DISCLOSURE=progressive` or `NEMOCLAW_TOOL_DISCLOSURE=direct`.
Onboarding adopts that mode when you have not selected one; an explicit conflicting mode fails before creation.
Configure registry credentials through Docker before onboarding a private image.

Do not combine `--from-image` with `--from` or the Portable profile.
Resume retains the recorded digest; changing it requires `--fresh --recreate-sandbox`.
Rebuild consumes the same digest without building an image.
Cleanup retains the published image, and upgrades leave it pinned and publisher-managed.
