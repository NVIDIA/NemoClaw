...
| `--force` | Add the channel when another sandbox uses the same messaging credential, a required conflict check cannot complete, or a channel-owned check requires abort. Conflicts that require abort do not offer an interactive override. Examples include Slack on a shared gateway and Microsoft Teams on a shared webhook port. |
