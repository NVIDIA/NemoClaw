...
NemoClaw invokes `openclaw sessions` via `openshell sandbox exec` and forwards OpenClaw flags verbatim, but filters default list output so legacy internal `nemoclaw-onboard-warmup-*` sessions are hidden from user-facing output.
...
NemoClaw forwards every flag the in-sandbox CLI accepts (`--agent`, `--all-agents`, `--active`, `--limit`, `--json`, `--store`, `--verbose`) and filters the resulting default table or JSON so legacy internal `nemoclaw-onboard-warmup-*` sessions are hidden.
...
With no positional keys, the command exports every non-internal session for the agent; if only legacy internal warm-up sessions exist, the command reports that there are no sessions to bundle and writes no artifact.
Legacy internal `nemoclaw-onboard-warmup-*` sessions are excluded from export-all output, but passing an explicit warm-up session key still exports that session for debugging.
