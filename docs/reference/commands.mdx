...
Onboarding does not check that a pinned port is free, so when another listener holds it the sandbox gets no API forward and onboarding warns that it could not start the forward.
