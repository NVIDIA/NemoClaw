...
The detection requires `nvidia-smi` to succeed and, on hosts whose firmware does not classify as a known NVIDIA platform (DGX Spark, DGX Station, Jetson, or Tegra), additionally requires `nvidia-smi` to report a strict NVIDIA identity (a kernel-issued GPU UUID, a valid SM compute capability, and a board VBIOS version) so that WSL2 d3d12/WDDM placeholder shims on non-NVIDIA hardware are not mistaken for real GPUs.
