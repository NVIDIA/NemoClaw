...
Run `$$nemoclaw <sandbox-name> destroy` to complete identity-bound recovery.
Destroy proceeds only after it verifies one retained recovery record and the immutable Docker sandbox identity.
It removes only the qualified sandbox containers and clears the matching recovery record after verified cleanup.
...
Run `$$nemoclaw <sandbox-name> destroy` to complete cleanup through the retained immutable identity.
