...
<AgentOnly variant="hermes">
For an active Portable Hermes lifecycle, only `connect --probe-only` can resume the exact published receipt-owned Ollama runtime when that runtime is stopped.
It revalidates the lifecycle receipt, sandbox registry, provider, inference route, and private publication authority before it resumes the runtime.
It verifies an already-running runtime without starting another one.
After authenticated Hermes health and inference verification succeed, it restores any missing recorded host forwards.
Interactive `connect` and `launch` verify the recorded inference route but do not resume the runtime or change its state.
If probe-only recovery fails after it starts the runtime, NemoClaw restores the recorded stopped state or exits nonzero when it cannot prove that restoration.
Run `nemohermes <sandbox-name> doctor` to inspect Portable lifecycle authority before you retry.
</AgentOnly>
