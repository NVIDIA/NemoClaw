...
Use `nemoclaw-acp` to connect an external Agent Client Protocol (ACP) client to a managed Hermes sandbox.
The hosted installer installs this host-side executable with NemoClaw.
The current managed contract requires OpenShell 0.0.116, Hermes 0.20.6, and `agent-client-protocol` 0.9.0.
The host must provide the system SSH client at `/usr/bin/ssh`.
It supports Linux and macOS hosts, including Linux under WSL2 where NemoClaw supports Hermes and OpenShell.
Native Windows is not supported.

Configure the ACP client to start `nemoclaw-acp` as a standard input and output server.
The command reads ACP messages from standard input, reserves standard output for ACP messages, and writes lifecycle diagnostics to standard error.

```bash
nemoclaw-acp --sandbox <sandbox-name> [--gateway <nemoclaw-gateway-name>] [--timeout <seconds>]
```

| Option | Description |
| --- | --- |
| `--sandbox <name>` | Select the registered managed Hermes sandbox. This option is required. |
| `--gateway <name>` | Select the owning NemoClaw gateway when the sandbox name is registered on more than one gateway. |
| `--timeout <seconds>` | End the ACP session after a positive integer from `1` through `86,400` seconds. When omitted, the adapter sets no session deadline. |
| `--help`, `-h` | Print command usage. |
| `--version`, `-v` | Print the installed NemoClaw version. |

Before it starts the session, `nemoclaw-acp` revalidates the sandbox registration, recovers an unreachable recorded gateway, and verifies the live sandbox identity and runtime versions.
It accepts only a ready sandbox created from the managed Hermes image by the installed NemoClaw version.
It rejects a missing, ambiguous, stopped, custom-image, stale, or incompatible target before it starts `hermes-acp`.
Gateway identity, authentication, schema, timeout, and other observation failures remain blocked and do not authorize recovery.
Unset `OPENSHELL_GATEWAY_ENDPOINT` before starting the adapter so an ambient endpoint cannot replace the recorded gateway authority.
For a stale or incompatible managed sandbox, [update NemoClaw and the sandbox](../manage-sandboxes/operate-sandboxes/update-sandboxes), then retry the adapter.

For example, start the adapter for a uniquely registered sandbox:

```bash
nemoclaw-acp --sandbox my-hermes-sandbox
```

OpenShell retains custody of its credentials and the sandbox's inference credentials.
The adapter does not forward credential-bearing environment variables to the SSH process.
It requests one SSH configuration for the selected gateway and sandbox, validates the fixed SSH target, writes the configuration to a mode `0600` temporary file, and gives only that file path to the system SSH client.
After success, failure, cancellation, timeout, or client disconnect, the adapter attempts to remove the private `nemoclaw-acp-ssh-*` directory under the host temporary directory.
Removal is best effort.
If the operating system refuses removal, the directory can remain and the command does not report its exact path.
Remote diagnostic content does not enter the ACP stream or pass through to the client.

The adapter preserves the remote `hermes-acp` exit status unless it is `255`, which identifies an SSH transport failure.
The adapter itself uses these exit statuses:

| Status | Result |
| --- | --- |
| `1` | The adapter could not launch safely, the system SSH client could not start, or the ACP client disconnected. |
| `2` | The command options are invalid. |
| `69` | The required host platform, OpenShell version, or `/usr/bin/ssh` is unavailable. |
| `78` | The managed Hermes or ACP SDK runtime is incompatible. |
| `124` | The 30-second compatibility probe or the configured session timeout expired. |
| `130` | The host sent `SIGINT`. |
| `143` | The host sent `SIGTERM`. |
| `255` | The OpenShell SSH transport failed. |

Read standard error to distinguish adapter-generated failures.
An expected client disconnect exits with status `1` without an additional diagnostic.
The remote process can return a status that also appears in this table.

Client disconnect, cancellation, timeout, and transport failure terminate the corresponding remote `hermes-acp` process.

If the OpenShell gateway restarts during a session, that session ends with a nonzero status.
Start `nemoclaw-acp` again to recover the recorded gateway and create a new ACP session.
After `nemohermes <sandbox-name> rebuild` completes, start a new adapter session so NemoClaw can verify the rebuilt sandbox's recorded identity.

The installer publishes a NemoClaw-managed `nemoclaw-acp` shim under `~/.local/bin` when the active npm prefix does not already provide the executable there.
If `~/.local/bin/nemoclaw-acp` already exists and is not a NemoClaw-managed shim, installation stops and leaves the existing entry unchanged.
Move or remove the conflicting entry, then rerun the installer.
...
A custom NVIDIA provider profile or endpoint override is not exportable.
...
Version 1 exports managed OpenClaw sandboxes and canonical managed Hermes sandboxes in the default workspace.
...
A managed OpenClaw sandbox can also use the verified managed local vLLM configuration.
The managed vLLM export runs only on Linux x86_64 and requires its fixed managed serving identity and complete live runtime evidence.
A managed OpenClaw sandbox can also export the verified local Ollama route on native Linux with the Docker driver.
The export records the external Ollama daemon port, the NemoClaw-managed authentication proxy port, and the `qwen3.5:9b` model digest.
It requires the managed proxy credential but never exports that credential or its value.
It fails unless the live daemon, proxy, model, and sandbox route match the retained configuration.
...
For hosted OpenClaw inference, the export preserves nondefault managed context-window, maximum-token, reasoning, and reasoning-effort settings as route overrides.
It preserves nondefault agent timeout and heartbeat settings under `execution`.
For a Docker-managed OpenClaw sandbox with hosted inference, the export can include one secondary agent that shares the primary route.
The secondary agent has a unique name and only `tools.allow: [read]`.
The export rejects a secondary agent with a different model, tools, execution settings, primary-agent override, or default subagent setting.
For OpenClaw, it preserves a nondefault managed dashboard port or bind address under `interfaces.dashboard`.
...
It omits settings that equal the managed defaults.
Hermes export fails when Hermes tool gateways are enabled.
It preserves an enabled Hermes dashboard, nondefault dashboard or internal ports, enabled browser terminal UI, and a nondefault allocated API port under `interfaces`.
A disabled dashboard and default API port are omitted.
...
The exported agent binds its API-key authentication to that provider reference.
...
- If OpenShell confirms the sandbox is absent, `destroy` selects one retained record from its registry generation, gateway, create-attempt evidence, and available identity fingerprint. Provider and Docker cleanup then apply their own immutable identity checks. `destroy` removes only qualified residual resources and clears the matching recovery record only after verified cleanup.
...
It preserves the current session's reservation, any row that is no longer route-only, and any row that has gained verified sandbox-create authority.
...
<AgentOnly variant="hermes">
Portable Hermes probes can also print credential-free `Hermes Portable currentness timing:`, `Hermes Portable inspection timing:`, and `Hermes Portable lifecycle recovery timing:` lines.
These diagnostics report durations and counts for authority, container, health, startup, and rollback checks without changing recovery behavior or exit status.
</AgentOnly>

<AgentOnly variant="openclaw">
  Portable lifecycle recovery attempts to emit at most one credential-free `Portable lifecycle timing:` line.
  ...
  It also attempts to emit at most one credential-free `Portable OpenClaw gateway startup timing:` line.
  When accepted launch-readiness evidence lets `connect --probe-only`, `launch`, or `recover` skip additional lifecycle work, NemoClaw attempts to emit each receipt once.
  The lifecycle receipt reports `0ms` for timing fields, `0` for attempt fields, `containerAction=reused`, `gatewayAction=reused`, `ollamaAction=not-applicable`, and `result=already-running`.
  The gateway receipt reports `0ms` for timing fields, `firstReadyAttempt=0`, `lastFailure=none`, and `diagnosticReadOutcome=not-applicable`.
  Receipt-output failures do not change recovery behavior or command exit status.
</AgentOnly>

Run it for health checks and scripted readiness probes; users continue to run only `$$nemoclaw launch <name>`.

<AgentOnly variant="hermes">
For Portable Hermes, `connect --probe-only` and `recover` can resume a stopped published Ollama runner only for the recorded `ollama-local` provider.
For another configured provider, they verify the recorded inference route and required forwards without Ollama recovery.
Refer to [Recover Portable Published Ollama](../manage-sandboxes/operate-sandboxes/recover-and-rebuild-sandboxes#recover-portable-published-ollama) for authority checks and rollback handling.
</AgentOnly>
...
- Status `1` with blank stdout and exactly one stderr line: `SUPERVISOR_NOT_RUNNING`, `SUPERVISOR_DISCOVERY_PENDING`, `GATEWAY_HEALTH_TIMEOUT`, or `SUPERVISOR_BUSY`. A typed host-side container-not-found result is sanitized to the exact `PRIVILEGED_CONTROL_UNAVAILABLE` line and is also retryable. The same line without that typed result is terminal.
...
Recovery counts typed container-not-found results separately from other retryable results.
It stops when either count reaches its limit: 21 container-not-found results or 11 other retryable results.
A mixed sequence permits at most 31 attempts.
At the default interval, the polling delay is at most 60 seconds for container discovery, 30 seconds for other startup results, or 90 seconds for a mixed sequence.
The recovery loop also has a 210-second deadline that includes controller execution and polling.
Recovery stops after 3 attempts return `SUPERVISOR_BUSY`.
The managed controller emits `SUPERVISOR_DISCOVERY_PENDING` only when an incomplete process-table scan during startup cannot yet prove either one exact supervisor or clean supervisor absence. That result delays recovery but cannot authorize container recreation or accept a supervisor identity; a later request must perform the complete identity proof again. Managed settle confirmation treats exact `SUPERVISOR_BUSY` and `SUPERVISOR_DISCOVERY_PENDING` results as inconclusive within its configured window. Status `137` and the Docker restart result remain terminal during that confirmation. The managed supervisor startup waiter accepts the three exact controller startup results, the typed container-not-found result, `SUPERVISOR_BUSY`, and the two container-transition results.
The final supervisor startup loop uses the same separate attempt allowances and its own 210-second deadline.
Unbound container IDs, reformatted Docker errors, status `137` with nonblank output, and other diagnostic results are terminal. NemoClaw treats `SUPERVISOR_UNAVAILABLE` as terminal because the managed controller uses it for integrity refusals, ambiguous discovery, and process-identity changes. It does not repeat the recovery action or treat the settle probe as inconclusive, and instead prints host-side restart and rebuild guidance. Other controller failures also stop immediately. Only an exact `SUPERVISOR_NOT_RUNNING` result that remains after the bounded startup retries can enter transactional legacy keepalive recreation. The pinned controller probe must then confirm the missing supervisor before recreation proceeds. If the gateway is already running, the command does not force-restart it; it can still re-evaluate supported safety checks and check or recover host-side forwards, and it exits zero only when those forward checks succeed. When the dashboard port is held by a listener that NemoClaw cannot attribute to the sandbox's own OpenShell forward, the command reports the port, leaves that listener running, does not restore a forward onto it, and exits nonzero. Legacy SSH forward records do not establish ownership. Stop the owning OpenShell gateway or service to release the listener, then run `recover` again. Use [`$$nemoclaw <name> gateway restart`](#$$nemoclaw-name-gateway-restart) when you deliberately need a running gateway to reload runtime configuration or plugins.
...
Hermes Portable forward repair reports the failed boundary and the affected port or operation when available.
Before mutation, it distinguishes an occupied port, a failed or invalid `openshell forward list` result, unresolved recorded ports, and an inconclusive reachability check.
After mutation begins, it distinguishes a failed `start` or `stop` operation from a settlement timeout.
NemoClaw does not stop an unproved owner and does not publish launch-readiness evidence after any of these failures.
...
For an active Portable Hermes receipt, `recover` checks the saved OpenShell phase and the receipt-owned Podman container state before mutation.
When OpenShell reports `Stopped`, recovery calls OpenShell start, including when the container is already running.
A saved `Error` phase, or a saved `Ready` phase with a container that is not running, stops recovery before mutation.
The error identifies the mismatch and asks you to preserve startup diagnostics and resolve the saved state through OpenShell.
Recovery does not reset OpenShell storage or start the container directly through Podman.
If recovery started a stopped container, it launches the receipt-owned `nemoclaw-start` command once and waits for authenticated Hermes health.
For a container that was already running, recovery preserves the workload and waits for authenticated health after any saved-phase reconciliation.
If a container or OpenShell identity check, reconnect check, or health check later fails after recovery started a previously stopped container, NemoClaw uses OpenShell to stop it.
It then requires Podman to report the receipt-owned container as `exited` and OpenShell to report `Error` or `Stopped`.
...
Hermes owns its native `mcp_servers` configuration. A direct MCP change is part of that snapshot and remains authoritative; no host-side MCP registry comparison gates recovery.
...
The native `mcp_servers` content in that snapshot remains authoritative, including direct edits; the supervisor does not reconcile it against host-side MCP state.
...
The command can fail at these layers: unsupported agent, privileged control unavailable, supervisor not running, secret-boundary refusal, unsafe config path, config hash mismatch when transaction metadata cannot be validated, relaunch quarantined, launch failure, health timeout, or forward recovery failure. `relaunch quarantined` means the in-sandbox supervisor stopped attempting relaunch after repeated process or health failures or an unrecoverable startup-layout refusal. Inspect the reported cause before choosing a repair; another recovery request does not retry a stopped relaunch. For ordinary process or health failures, correct the cause, then run `$$nemoclaw <name> stop` and `$$nemoclaw <name> start` to reset the supervisor. Rebuild only if the sandbox still cannot start.
...
For an active Portable Hermes receipt, `stop` uses OpenShell to stop only the receipt-owned sandbox.
It does not fall back to Docker.
It waits for the receipt-owned Podman container to report `exited` and OpenShell to report `Error` or `Stopped`.
If either state does not settle, the command exits nonzero.
...
For an active Portable Hermes receipt, `start` uses OpenShell to start only the receipt-owned sandbox.
It then verifies the receipt-owned Podman container, supervisor, authenticated health, and rollback contract documented for [`recover`](#$$nemoclaw-name-recover).
It checks and repairs the recorded host forwards before reporting success.
It does not fall back to Docker.
A `pending` or `configuring` receipt instead directs you to resume Portable onboarding.
...
For Portable Hermes, text `status` reports `Saved Portable lifecycle phase: pending`, `configuring`, or `active` from the retained receipt.
It also prints `Runtime and agent health: not probed`; the saved phase does not establish live health.
...
For a non-Portable sandbox, status reports a clean `phase: "Stopped"` only when the persisted
...
The command fails closed when it cannot safely identify the source entry. `--force` permits removal of a modified same-name agent entry, but it does not widen policy or provider authority and never deletes a provider. Legacy Mcporter or `.nemoclaw-mcp.json` entries must be converted with `mcp migrate --apply` before ordinary removal.
...
| `--force` | Skip the confirmation prompt. It does not bypass an unavailable MCP source or allow rebuild to continue when a manifest-declared state directory or state file cannot be fully backed up. |
...
After restore, the command restores Deep Agents manifest-defined state, regenerates `/sandbox/.deepagents/config.toml`, and writes the agent-native `/sandbox/.deepagents/.mcp.json` from the bounded source handoff while preserving unrelated native entries. Before changing the sandbox, rebuild verifies that the recorded `inference.local` route is still reachable and that the target provider, model, reasoning settings, web search selection, base image, current agent MCP source, and current live OpenShell policy can be captured. If those checks fail after backup, NemoClaw restores the previous MCP state and keeps the existing sandbox intact. Use rebuild after a failed Deep Agents version check, after enabling Tavily Search, or as the apply step for legacy MCP migration.
...
That route line is labeled `Inference (configured):` to distinguish it from the live inference result from `$$nemoclaw <name> status`.
...
For non-Portable sandboxes, the command gathers system information, Docker state, gateway logs, and sandbox status into a summary or tarball.
...
<AgentOnly variant="hermes">
For Portable Hermes, select the sandbox explicitly to collect a redacted offline report from its retained receipt.
The report works when the registry and runtime are unavailable.
It contains only the report schema version, sandbox name, agent, saved lifecycle phase, and runtime and agent health values of `not-probed`.
With `--output`, the archive contains only `portable-lifecycle.json`.
It excludes raw receipts, credentials, environment data, private paths, and runtime logs.
It preserves an existing output file if archive creation fails.
`--quick` produces the same minimal report.
A malformed selected receipt stops collection before an archive is created.
While Portable authority remains on the host, NemoClaw blocks non-Portable diagnostic collection.
</AgentOnly>

An explicit name can come from `--sandbox`, `NEMOCLAW_SANDBOX_NAME`, `NEMOCLAW_SANDBOX`, or `SANDBOX_NAME`, in that precedence order.
For non-Portable sandboxes, the name must match a registered sandbox.
...
If the reported `onboard.lock` remains after onboarding ended abruptly, confirm that no NemoClaw onboarding process is active in any environment that shares the state root, remove only the reported lock, and rerun uninstall.
