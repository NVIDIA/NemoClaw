...
| `/nemoclaw shields [status]` | Show read-only shields posture; reports explicit `DOWN` state, otherwise treats unverifiable host-side lockdown as `NOT CONFIGURED` and points to `$$nemoclaw <name> shields status` |
...
Use host-side `$$nemoclaw <sandbox> shields up|down` commands to change shields posture.
