...
This is a thin pass-through to `openclaw skills list --agent main` inside the sandbox. NemoClaw does not merge, cache, or maintain a second skill list. Native output and filter flags such as `--json`, `--eligible`, and `--verbose` are forwarded verbatim. An `--agent` override is refused because the lifecycle commands are bound to the sandbox's primary `main` agent.
...
This is a thin pass-through to `dcode skills list --agent agent` inside the sandbox. Native flags such as `--json` and `--project` are forwarded. NemoClaw does not merge, cache, or maintain a second DCode skill list. An `--agent` override is refused because the lifecycle commands are bound to DCode's primary `agent` state.
