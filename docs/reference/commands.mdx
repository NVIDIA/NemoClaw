...
NemoClaw stores ownership evidence in protected host state bound to the exact live sandbox identity. An exact-content retry verifies an unchanged skill only after a completed install receipt. An interrupted fresh installation retains a pending receipt, but the retry refuses any existing target because sandbox content cannot prove that the uncertain operation created it. Inspect the target and remove it only after confirming its provenance; the same-content retry can proceed when the ambiguous target is absent. When rebuild restores the workspace into a replacement sandbox, it durably transfers validated skill ownership evidence to the replacement identity before retiring the source evidence. Cross-sandbox snapshot restore instead clones workspace content without transferring host ownership evidence; restored skills are treated as user-owned at the destination, so inspect and remove an existing restored target before installing that name through NemoClaw. Changed-content updates are refused because pinned OpenClaw 2026.7.1 has no compare-and-swap contract that can atomically bind the verified current content to a forced replacement. It also refuses installation when the former `/sandbox/.openclaw/skills/<name>` path exists, preventing old and native destinations from becoming competing authorities. Inspect both paths and migrate legacy or changed content manually before retrying. Start a new OpenClaw session to load the installed skill; the command does not edit `sessions.json`.
