...
NemoClaw keeps the native entry while it checks the current live policy. When that policy binds a provider, NemoClaw detaches it only if the provider identity, type, and credential key match the binding. It then verifies that fresh sandbox processes no longer receive the credential, removes the generated policy if present, and finally removes the named agent-native entry. If detachment, credential absence, or policy removal cannot be proved, the command exits nonzero and retains the native entry for an explicit retry. The workspace provider and its stored credential are deliberately preserved because current OpenShell provider deletion is name-based. The command reports an identified retained provider so an operator can inspect or remove it separately after confirming it is unused.
...
| Flag | Description |
| --- | --- |
| `--force` | Permit removal of a modified same-name native agent entry after verified policy and credential cleanup; never bypass those checks or delete the workspace provider |
