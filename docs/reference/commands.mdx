...
A create-attempt label can help an OpenShell administrator identify and remove the exact sandbox, but it does not let NemoClaw retire the record without immutable identity authority.
If neither a fingerprint nor a label is available, preserve the terminal output and ask the administrator to identify the exact sandbox from gateway or controller evidence.
If NemoClaw reports that it could not save recovery evidence, rerun `$$nemoclaw <sandbox-name> destroy --yes`.
NemoClaw reconstructs the recovery record only when the registry preserves a matching `verified-create` checkpoint and lifecycle authority.
If reconstruction fails, the authority conflicts, or immutable identity evidence is missing, preserve the terminal output and follow the same escalation.
