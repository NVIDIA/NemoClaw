...
Automatic OpenClaw skill removal is unavailable in this capability slice because pinned OpenClaw 2026.7.1 has no public native skill removal command. The command refuses before changing files. Use `$$nemoclaw <name> connect` to inspect both `/sandbox/.openclaw/workspace/skills/<skill>` and the legacy `/sandbox/.openclaw/skills/<skill>` path, and remove or migrate content manually only after confirming its provenance.
