...
For Hermes, warm-hint and candidate validation also reruns a container probe for the MCP SDK and native Streamable HTTP integration.
...
Deep Agents teardown does not require the managed launcher marker from the old image, so an existing compatible MCP entry cannot block `mcp remove`, sandbox rebuild, or destroy merely because the image predates that probe.
A replacement image must pass the marker probe before post-rebuild MCP providers or policy are restored.
