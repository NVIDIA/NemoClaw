...
The inventory is host-wide; its live inference read queries the gateway that `NEMOCLAW_GATEWAY_PORT` selects.
...
Show the host-wide sandbox list and the status of host auxiliary services (for example cloudflared).
It summarizes registered sandboxes and host services, and reads the live inference route and gateway health from the gateway that `NEMOCLAW_GATEWAY_PORT` selects.
...
The sandbox-first grammar `$$nemoclaw <name> inference get` resolves the gateway recorded for that sandbox and reads that gateway's live route.
