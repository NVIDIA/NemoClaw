...
A rebuild preserves the recorded Deep Agents Code validation profile unless `--dcode-validation-profile` installs a replacement profile or explicitly disables it.
...
```bash
$$nemoclaw my-assistant rebuild [--yes|-y|--force] [--verbose|-v] [--tool-disclosure <progressive|direct>] [--dcode-auto-approval <disabled|thread-opt-in>] [--dcode-validation-profile <absolute-path|disabled>] [--observability|--no-observability]
```
...
| `--dcode-validation-profile <absolute-path\|disabled>` | Install a bounded managed headless validation-command profile from a normalized absolute host path, or pass `disabled` to remove the recorded profile; this option is accepted only for managed Deep Agents Code sandboxes. |
