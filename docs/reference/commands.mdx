...
If neither a durable identity fingerprint nor a create-attempt label is available, automatic recovery is unavailable. Preserve the terminal output and ask an OpenShell administrator to identify the exact sandbox from gateway or controller evidence.
