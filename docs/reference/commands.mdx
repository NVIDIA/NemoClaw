...
If the file is missing, `channels add` exits with a non-zero status before any token prompt, registry write, or rebuild prompt.
When the file is present, NemoClaw applies the preset to the sandbox before the rebuild so the bridge has egress to its upstream API.
