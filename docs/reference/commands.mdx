...
OpenShell `0.0.82` evaluates that policy before replacing the attached provider placeholder in the allowed request header.
...
OpenShell `0.0.82` mutates providers by name, so do not concurrently replace a managed provider through another OpenShell client during this command.
...
| `NEMOCLAW_GATEWAY_BIND_ADDRESS` | 127.0.0.1 | The OpenShell gateway uses this bind address; Docker-driver gateways on OpenShell 0.0.82 keep it on loopback while gateway JWT auth is active. |
<AgentOnly variant="openclaw,hermes">
...
`NEMOCLAW_GATEWAY_BIND_ADDRESS` accepts only `127.0.0.1` and `0.0.0.0`, but Docker-driver gateways on OpenShell 0.0.82 reject `0.0.0.0` while gateway JWT auth is active.
<AgentOnly variant="openclaw,hermes">
...
| `NEMOCLAW_IGNORE_VLLM_DISK_SPACE` | exactly `1` | Allows managed vLLM to continue when verified Docker image-storage or registered model-cache capacity is insufficient, or when registered model-cache capacity cannot be measured; other values do not bypass either check. An inconclusive Docker image-storage check reports diagnostics and retains its existing automatic-continue behavior. |
<AgentOnly variant="openclaw">
