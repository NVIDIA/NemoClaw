...
| `NEMOCLAW_REASONING_EFFORT` | `low`, `medium`, `high`, or `default` | Applies only to the `compatible-endpoint` provider. A `low`, `medium`, or `high` value writes `params.extra_body.reasoning_effort` when the selected API is `openai-completions`. Unset leaves the endpoint's own default in place. During `inference set`, NemoClaw rejects every explicit value, including `default`, before mutation unless the resulting route uses `compatible-endpoint` and `openai-completions`. Without an explicit effort input, switching to another provider or API family clears a recorded effort. During onboarding, another provider clears the variable without reading it. |
