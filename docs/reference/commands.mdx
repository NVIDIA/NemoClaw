...
| `NEMOCLAW_OPENSHELL_GATEWAY_BIN` | path | Advanced override for the `openshell-gateway` binary used by Linux Docker-driver startup. For the default port, the installer accepts only `~/.local/bin/openshell-gateway`, `/usr/local/bin/openshell-gateway`, or `/usr/bin/openshell-gateway`; another path fails service staging. The macOS Homebrew service uses the formula's binary. Defaults to the binary next to `openshell`, then common install paths. |
