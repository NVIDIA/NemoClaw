...
On ordinary native Linux Docker-driver hosts, NemoClaw automatically tries native OpenShell GPU injection first and keeps the Docker container-swap path as a one-time compatibility fallback.
If native sandbox creation or readiness has a trusted host-side GPU routing failure, or an explicit driver proof fails while the exact container's host configuration confirms that no GPU was attached, onboarding saves redacted diagnostics and removes the incomplete sandbox before it retries through the compatibility path.
Sandbox-reported CUDA output alone cannot authorize the broader compatibility envelope; that case fails closed and points to the existing explicit `NEMOCLAW_DOCKER_GPU_PATCH=1` control.
NemoClaw retries only after it verifies that no OpenShell-labeled container remains; if cleanup cannot be proven safe, onboarding stops and prints cleanup guidance instead.
...
Leave `NEMOCLAW_DOCKER_GPU_PATCH` unset or set it to `auto` for automatic GPU onboarding.
On ordinary native Linux, automatic mode tries native OpenShell GPU injection first and retries once through the compatibility path when safe.
Set `NEMOCLAW_DOCKER_GPU_PATCH=0` to use native OpenShell GPU injection without the compatibility fallback on ordinary native Linux or Jetson/Tegra.
Set `NEMOCLAW_DOCKER_GPU_PATCH=1`, or another legacy nonzero value, to use only the compatibility path on ordinary native Linux.
...
```bash
$$nemoclaw my-assistant rebuild [--yes|-y|--force] [--verbose|-v] [--tool-disclosure <progressive|direct>] [--observability|--no-observability]
```
...
| `NEMOCLAW_DOCKER_GPU_PATCH` | unset, `auto`, `0`, or a nonzero compatibility value | Controls internal Linux Docker-driver GPU routing without changing whether GPU passthrough is enabled. Unset or `auto` tries native OpenShell injection first with one safe compatibility fallback on ordinary native Linux, and uses the compatibility path directly on Docker Desktop WSL and Jetson/Tegra. `0` selects native-only injection on ordinary native Linux and Jetson/Tegra, but Docker Desktop WSL ignores it. `1` and other legacy nonzero values select compatibility-only routing. On Jetson/Tegra, use `0` only for troubleshooting because it bypasses the device-group propagation needed for CUDA. |
