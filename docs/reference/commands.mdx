...
|----------|-----------------|--------|
| `NEMOCLAW_LOG_LEVEL` | `error`, `warn`, `info`, or `debug` (case-insensitive; surrounding whitespace is ignored) | Sets the logging threshold. A valid value takes precedence over the other debug environment variables. An invalid, blank, or unset value falls through to the lower-priority selectors. |
