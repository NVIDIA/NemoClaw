...
| `NEMOCLAW_EXTRA_PLACEHOLDER_KEYS` | whitespace- or comma-separated list of upper-snake env keys | Adds credentials to matching canonical OpenShell providers. Refer to [Extra placeholder keys](#extra-placeholder-keys). |
...
NemoClaw adds each credential to its matching canonical provider, and OpenShell resolves it at that provider's policy-bound egress.
In the operator-owned profile `.env`, use an assignment such as `TELEGRAM_BOT_TOKEN_AGENT_A=openshell:resolve:env:TELEGRAM_BOT_TOKEN_AGENT_A`; never store the raw token.
...
NemoClaw skips an entry when its canonical provider is not selected or its credential is unavailable.
