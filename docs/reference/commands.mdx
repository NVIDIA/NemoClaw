...
For an active Portable Hermes receipt, `recover` uses OpenShell to start only the receipt-owned sandbox when its Podman container is stopped.
...
For an already-running container, recovery does not launch the startup command, does not stop the sandbox, and only waits for authenticated health.
If a container or OpenShell identity check, reconnect check, or health check later fails after recovery started the sandbox, NemoClaw uses OpenShell to stop it.
...
Stop managed local inference resources, remove the host-side Docker image built during onboard, and delete the sandbox. This removes the sandbox from the registry. For Ollama-backed sandboxes, `destroy` also asks Ollama to unload currently loaded models and clears stale auth proxy state on a best-effort basis. For a Local vLLM sandbox, `destroy` removes the NemoClaw-managed `nemoclaw-vllm` container and frees its GPU memory after the registry removal leaves no sandbox in any gateway state root with provider `vllm-local`. Pass `--keep-vllm` or set `NEMOCLAW_KEEP_VLLM=1` to keep that container running for a later sandbox. `destroy` preserves the container and reports why when another registered sandbox uses Local vLLM, when the container lacks the NemoClaw managed label, when the container carries distributed vLLM labels but no ownership receipt, when an authenticated container does not match its persisted API key and receipt, when a distributed vLLM receipt owns it, when NemoClaw cannot read every gateway sandbox registry to confirm that no sandbox still uses Local vLLM, or when Docker is unavailable. `destroy` records the pending retirement in `~/.nemoclaw/host-local-vllm-pending-retirement.json` when it leaves the container in place for any reason other than `--keep-vllm`, a remaining Local vLLM sandbox, or a distributed receipt, or when it stops before the retirement completes. Resolve the reported cause, then rerun `destroy` for the same sandbox name to retire the container. For Model Router sandboxes, `destroy` keeps the process and recovery identity when another sandbox uses the port or when session, process, or absence checks are inconclusive. It also preserves a replacement onboarding session when the captured session identity changed. If the captured session uses the destroyed sandbox name with another router port, `destroy` clears only the sandbox association and preserves that router's recovery identity. For lock order, same-port peer handling, and cleanup checks, refer to [Set Up Model Router](../inference/hosted-inference/set-up-model-router#router-and-sandbox-lifecycle-locks).
...
```bash
$$nemoclaw my-assistant destroy [--yes|-y|--force] [--cleanup-gateway|--no-cleanup-gateway] [--keep-vllm]
```
...
If the recorded host variable is exported, restart replaces the provider credential and waits for an attached credential handle. A legacy revision handle must advance; an OpenShell 0.0.116 stable handle can remain unchanged after provider identity and endpoint authorization are revalidated.
...
OpenShell 0.0.116 cannot bind that provider to an MCP endpoint, so restart and rebuild fail closed instead of activating it.
...
When `--sandbox` is supplied explicitly through the flag or one of `NEMOCLAW_SANDBOX_NAME`, `NEMOCLAW_SANDBOX`, or `SANDBOX_NAME`, the name must match a registered sandbox. The flag wins, then the environment variables in that order.
...
| `NEMOCLAW_KEEP_VLLM` | `1`, `true`, or `yes` to keep; `0`, `false`, or `no` to remove | Keeps the NemoClaw-managed `nemoclaw-vllm` container running when `$$nemoclaw <name> destroy` deletes the last registered sandbox that uses Local vLLM. By default, that destroy removes the container. Command-line `--keep-vllm` has the same effect. |
