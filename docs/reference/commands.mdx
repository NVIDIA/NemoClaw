...
`Infinity`, `NaN`, unparseable text, and values outside the ranges above are rejected for these auto-pair controls when the sandbox is created, so they never reach the sandbox. A sandbox started outside the NemoClaw CLI skips those checks; there, any value the watcher cannot use falls back to the documented default rather than stopping the watcher.
