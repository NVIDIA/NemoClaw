...
When same-gateway legacy sandbox records use the selected supported provider but omit its credential environment-variable name, rebuild derives the peer's missing credential environment-variable name for the compatibility check without changing the peer's registry entry.
Rebuild saves only the target sandbox's metadata.
