...
Supported provider names are `nvidia-prod`, `nvidia-nim`, `nvidia-router`, `openai-api`, `openrouter-api`, `anthropic-prod`, `compatible-anthropic-endpoint`, `gemini-api`, `compatible-endpoint`, `hermes-provider`, `ollama-local`, and `vllm-local`.
...
For a non-default port, uninstall can also remove a port-scoped state root after onboarding fails during preflight, before it creates a gateway or sandbox.
...
Uninstall preserves a recent malformed lock because another process can still be writing it; wait for the reported condition to settle, confirm that no onboarding process shares the state root, and retry.
