...
| `NEMOCLAW_REASONING_EFFORT` | `low`, `medium`, or `high` | Writes `params.extra_body.reasoning_effort` on the model entry in the built OpenClaw config, which OpenClaw merges into the request body of a route that uses `openai-completions`. Applies only to the `compatible-endpoint` provider; unset leaves the endpoint's own default in place. `default` is a `--reasoning-effort` value only; it is not a supported variable value. Any other value fails before NemoClaw touches the provider, policies, the sandbox, or the registry. |
