...
An `SSH sessions` line reports how many active SSH sessions the sandbox has, or `none`.
<AgentOnly variant="openclaw,hermes">
...
For each listed sandbox, the text output includes the configured inference provider and model plus the number of active SSH sessions.
