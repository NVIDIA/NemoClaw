...
The v0 command projects verified source state into the v1alpha1 YAML shape.
The v1 Rust parser remains authoritative for accepting that document; the v0 command does not carry a copy of the v1 schema or semantic validator.
...
Managed vLLM, local Ollama, and Podman exports are deferred. The command reports these sources as unsupported instead of producing a partial document.
...
For a Docker-managed OpenClaw sandbox with hosted inference, the export can include one or more read-only secondary agents in runtime order.
Each secondary agent has a unique name, uses only `tools.allow: [read]`, and shares the primary hosted route, selected model, and retained route tuning.
...
Force-restart the supported agent runtime. OpenClaw restarts its in-sandbox gateway process through the controller for the live container topology. Hermes first preflights its secret boundary, then OpenShell stops and starts the identity-verified sandbox before NemoClaw verifies gateway health and recovers host forwards. Use this after runtime configuration or plugin changes that the agent reads only at gateway startup, such as Hermes Langfuse plugin settings. Unlike `recover`, this command restarts a healthy runtime instead of exiting after the health probe.
...
For OpenClaw's direct root-entrypoint topology, PID 1 stops only the gateway child whose process ID and process start identity match the tracked child, applies the restart seal, and launches the replacement under the separate `gateway` UID.
For OpenClaw's OpenShell-managed topology, the installed root controller verifies a stable OpenShell to `nemoclaw-start` to gateway process shape, holds a root-only lifecycle lock, publishes one root-owned exit authorization bound to the exact gateway process ID, kernel start identity, and live controller identity, pidfd-targets the observed child, waits for the nonroot entrypoint supervisor to respawn it under the sandbox UID, and proves the replacement listener and HTTP health.
...
The OpenClaw authorization keeps an authenticated host-requested exit out of the crash budget while its exact root controller remains live.
...
For Hermes, the host runs a non-mutating gateway preflight that validates `/sandbox/.hermes/.env` against the secret-boundary guard, then holds the shared sandbox lifecycle lock while OpenShell stops and starts the registered live sandbox identity. After startup, NemoClaw verifies gateway health and recovers the dashboard, messaging, and manifest-declared forwards.
This path does not signal an in-sandbox gateway child or publish a controller exit authorization. OpenShell stop/start replaces the running supervisor instance, so the former controller crash-budget exemption is not part of Hermes restart behavior.
The new Hermes supervisor continuously repairs its gateway, API relay, dashboard, dashboard relay, and gateway log stream; four consecutive gateway health failures trigger child recovery, while five unexpected exits or failed replacement candidates within 60 seconds stop relaunch for that supervisor instance.
The restarted supervisor adopts one stable snapshot of mutable `config.yaml` and `.env` after the secret boundary passes.
...
Hermes host config writes retain their root-only mutation lock; sandbox stop/start is serialized by the host-side lifecycle lock.
...
<AgentOnly variant="openclaw,hermes">
  After the container stops, NemoClaw attempts to stop that sandbox's
