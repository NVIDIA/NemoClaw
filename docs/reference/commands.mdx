...
Recovery accepts at most 3 `SUPERVISOR_BUSY` results.
Status `137` and the Docker restart result share the 11-attempt managed-control budget.
Managed settle confirmation treats only exact `SUPERVISOR_BUSY` as inconclusive within its configured window.
