...
When recreation can consume a local Docker image and Docker reports that Buildx is missing or broken while validating the managed OpenClaw context generated by NemoClaw, NemoClaw prints a warning, retries that exact fingerprinted context once with Docker's legacy builder, and carries the verified builder and Docker endpoint configuration into recreation.
