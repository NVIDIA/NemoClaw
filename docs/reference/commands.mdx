...
#### Interactive Choice Review

During interactive onboarding, host checks are read-only, and credential-free choice collection does not change external resources before the review.
The available choice groups include the agent, inference provider and model, sandbox name, web search, messaging channels, managed tools, resources, and policy tier.
The selected agent and host capabilities determine which choices appear within those groups.
Onboarding does not request raw credentials during choice collection, store them in the draft, or show them in the review.
Custom endpoint URLs cannot contain userinfo, query parameters of any name, or fragments.
Before the review, onboarding revalidates seeded and resumed collecting drafts and recollects a rejected endpoint URL without displaying it.
Accepted drafts receive the same endpoint URL validation before materialization.

Type `b` or `back` during choice collection to return to the previous available choice prompt.
Type `exit` or `quit` at a choice prompt to exit onboarding before external resource changes begin.
Backspace continues to edit text in the current prompt.
You can repeat `back` until you reach the first choice.
At the first choice, another `b` or `back` keeps you there.
Hidden groups and groups that are not applicable are skipped.

Nested prompts use these Back transitions:

| Current prompt | Back destination |
|---|---|
| Agent | Agent, which is the first-choice floor |
| Inference provider | Agent |
| Model | Inference provider |
| Endpoint URL | Model |
| Authentication method | Endpoint URL when present; otherwise Model |
| Sandbox name | The last applicable Inference prompt: Authentication method, Endpoint URL, or Model |
| Web search | Sandbox name |
| Messaging channels | Web search |
| Managed tools | Messaging channels |
| Resource profile | Managed tools when available; otherwise the previous visible choice group |
| CPU | Resource profile |
| RAM | CPU |
| Sandbox GPU | RAM for a custom profile; otherwise Resource profile |
| Policy tier | Sandbox GPU |

When you select a custom resource profile, onboarding collects CPU, then RAM, then the sandbox GPU setting.
The review shows both overrides in the Resources line, for example, `Resources: custom (CPU: 25%, RAM: 25%), GPU: auto`.

After you reach the review, select **Edit a choice** to jump directly to any available group.
Onboarding returns immediately to the review when the saved later choices remain compatible.
If the edited value makes a dependent choice incompatible, onboarding clears or adjusts that choice and reopens only affected groups with missing answers.

These edits can invalidate dependent choices:

| Edited choice | Cleared or adjusted dependent choices |
|---|---|
| Agent | Managed tools and Policy tier; incompatible Inference or Web search choices; unsupported Messaging channels. If only the model is incompatible, onboarding keeps the provider and resets Model to its displayed default. |
| Inference provider and model | Managed tools and Policy tier |
| Web search | Policy tier |
| Messaging channels | Policy tier |
| Managed tools | Policy tier |

Editing Sandbox name, Resources, or Policy tier preserves the other draft choices.
Onboarding derives the sandbox and resource plans only after you select **Apply configuration**.
Before materialization, onboarding revalidates the accepted choices against the current agent, provider, web search, messaging, managed tool, resource profile, and policy menus.
This check also applies when `--resume` uses an accepted draft.
If an accepted choice is no longer available, onboarding stops before it changes provider, gateway, Docker, policy, or sandbox state.
Onboarding prints `Accepted onboarding choices are no longer available. Rerun onboarding to review the current choices.`
Run `$$nemoclaw onboard --fresh` to discard the accepted draft and review the current choices.

The review offers these actions:

- **Apply configuration** requests required credentials and starts the forward-only resource changes.
- **Edit a choice** opens the direct choice menu.
- **Exit onboarding** stops before external resource changes begin.

After you select **Apply configuration**, onboarding can change Docker, gateway, provider, policy, and sandbox state.
The `back` command and **Edit a choice** are not available during this forward-only phase.
If a resource change stops or you need to correct a completed configuration, follow the command-specific remediation that onboarding prints.
Use `$$nemoclaw onboard --resume` for a retained incomplete session.
Use the applicable `inference set`, `policy`, or `channels` command for a supported configuration change.
Use `$$nemoclaw onboard --fresh --name <sandbox-name> --recreate-sandbox` for a build-time choice that requires sandbox recreation.
#### `--profile <name>`
...
The wizard collects the provider and model choices before it requests the provider credential.
...
Before the review, the wizard collects all available choices, including the sandbox name, resource profile, custom CPU and RAM values, sandbox GPU setting, and policy tier.
<AgentOnly variant="openclaw,hermes">
This collection also includes optional web search, messaging channels, and any managed tools supported by the selected agent and provider.
</AgentOnly>
The **policy tier** choice controls the suggested network policy presets for the sandbox.
After review, onboarding applies the reviewed tier's suggestions without reopening the preset picker.
After you select **Apply configuration**, onboarding requests required credentials, registers providers, and creates or updates the selected resources.
Three tiers are available:

| Tier | Description |
|------|-------------|
| Restricted | No tier defaults. Web search or other integrations selected earlier can still add their required presets; disable those integrations during choice review for baseline-only access. |
...
After you apply the configuration, NemoClaw derives and records the finalized built-in preset selection from the reviewed tier and supported integrations without opening a preset picker.
...
If you exit a brand-new run during choice collection or review, NemoClaw stops before it changes external resources.
Existing live sandboxes remain unchanged.
...
| `NEMOCLAW_OLLAMA_NO_AUTOSTART` | `1` to enable | Skips eager Ollama auto-start while NemoClaw applies a reviewed Local Ollama choice (equivalent to passing `--no-ollama-autostart`). When set and Ollama is not running on `localhost:11434`, an agent that uses the legacy `16384`-token context floor, currently OpenClaw, prints a warning and selects the default fallback model instead of spawning `ollama serve`. An agent that requires a larger verified runtime context, currently Hermes at `64000` tokens, returns an error when the Ollama provider is already accepted or onboarding is non-interactive. The flag covers only this provider setup; later steps for the authentication proxy, validation, and model warm-up still expect a reachable Ollama. On Linux hosts with a systemd Ollama unit, the loopback-override path may still restart the daemon after review and before this gate runs. |
