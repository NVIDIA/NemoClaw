...
When an incomplete snapshot contains captured entries, NemoClaw retains them at the printed path for manual recovery, but excludes that snapshot from `snapshot list` and snapshot restore selection.
...
When removal fails, the command reports the retained snapshot path and exits nonzero.
The incomplete snapshot remains excluded from `snapshot list` and restore selection.
