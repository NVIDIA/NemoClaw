...
```bash
$$nemoclaw my-assistant rebuild [--yes|-y|--force] [--verbose|-v] [--tool-disclosure <progressive|direct>] [--dcode-auto-approval <disabled|thread-opt-in>] [--observability|--no-observability] [--retire-recovery <transaction-id>]
```
...
| `--retire-recovery <transaction-id>` | Retire an exact retained rebuild recovery transaction after independently confirming that its recorded sandbox is absent. Requires `--yes`; it removes only the transaction-bound policy handoff and recovery marker while preserving the backup. |
