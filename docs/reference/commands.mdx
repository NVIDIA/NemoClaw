...
After the add commits, NemoClaw sends a differential pair of wire-level MCP `initialize` requests from inside the sandbox — one with the placeholder header and one with an unresolvable control bearer — to verify that OpenShell resolves the credential on egress; an inconclusive result prints a warning without failing the add, and `--no-probe` skips this check.
