...
For a public entry, restart activates a complete newly resolved public address set.
If DNS returns one address already present in a recorded multi-address set, restart retains the complete recorded set.
If a legacy registration has one recorded pin and DNS still returns only one address, restart stops before mutation; remove and add the server again after DNS returns a complete address set.
