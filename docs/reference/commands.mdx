...
Set `NEMOCLAW_DISABLE_TELEMETRY` to the exact value `1` on the receiving `bash` command to stop the post-success installer telemetry attempt.

```bash
curl -fsSL https://www.nvidia.com/nemoclaw.sh | NEMOCLAW_DISABLE_TELEMETRY=1 bash
```

The telemetry client checks this control before it loads telemetry configuration, builds the event, or sends a telemetry request.
Production delivery remains disabled until the client ID, schemas, and endpoints are provisioned.
