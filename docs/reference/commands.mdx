...
Set `NEMOCLAW_HERMES_API_PORT=<N>` before onboarding to require a specific port.
The sandbox relay binds this port when the sandbox starts, so an existing sandbox keeps its recorded port until you rerun onboarding with `--recreate-sandbox`.
