...
The retry is still gated on a trusted host-side GPU routing failure, or an explicit driver proof plus exact-container host configuration confirming that no GPU was attached; onboarding then saves redacted diagnostics and removes the incomplete sandbox before retrying.
