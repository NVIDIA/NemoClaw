...
For a Docker-managed OpenClaw sandbox with hosted inference, the export can include one or more secondary agents in runtime order.
Each secondary agent has a unique name, shares the primary hosted route, and uses only `tools.allow: [read]`.
The export rejects the complete document when any secondary agent has a model that differs from the primary model, unsupported tools, execution, interfaces, observability, a primary-agent override, or a default subagent setting.
