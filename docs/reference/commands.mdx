...
| Entry | What it holds |
|---|---|
| `rebuild-backups/` | Host-side snapshots that `nemoclaw <name> snapshot create` and `nemoclaw backup-all` write, and that `nemoclaw <name> snapshot restore` reads back after you reinstall. |
...
| `sandboxes.json` | Host-side sandbox registry that maps sandbox names back to their persistence directories when you reinstall. |
...
| Non-interactive (`--yes`, `NEMOCLAW_NON_INTERACTIVE=1`, or non-TTY shell) | Preserves the entries and prints a short notice. |
| Any context with `NEMOCLAW_UNINSTALL_DESTROY_USER_DATA=1` | Skips the preserved-data prompt and removes everything under `~/.nemoclaw/`. |
