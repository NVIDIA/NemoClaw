...
### `$$nemoclaw <name> quarantine`

Fence and stop one qualified sandbox without deleting its evidence.
Use quarantine when you must prevent restart and preserve state for investigation.
Use [`$$nemoclaw <name> stop`](#$$nemoclaw-name-stop) for a routine, reversible resource stop.

```bash
$$nemoclaw my-assistant quarantine --reason "unexpected outbound activity" [--idempotency-key <key>] [--json]
```

The command requires `--reason <text>`.
NemoClaw stores only the SHA-256 digest of the reason in the fence and receipt, so retries can detect a changed reason without persisting the original text.
The reason still passes through your shell command line, so do not include credentials, prompts, messages, or workspace content.

Use `--idempotency-key <key>` to give retries a stable request identity.
The receipt stores only the SHA-256 digest of this value.
The key must be 1 to 256 bytes and must not contain control characters.
If you omit the flag, the command generates a key and writes it to stderr before it persists the fence or starts isolation.
This publication also occurs with `--json`, so stdout remains the structured result channel.
If the command cannot publish the generated key, it exits before mutation.
A supplied key is not printed again.

This release qualifies the default OpenClaw, Hermes, and LangChain Deep Agents Code manifests with the Docker runtime provider.
An unqualified agent or runtime provider fails before fence persistence and stop operations.

Before mutation, NemoClaw validates the exact runtime-provider authority, OpenShell gateway, lifecycle generation, and live sandbox identity.
It derives required access surfaces from the loaded agent manifest and sandbox registry.
Gateway agents stop their configured messaging channels, declared dashboard or API forwards, other declared forwards, and exact workload.
Terminal agents stop their declared forwards and exact workload.

NemoClaw synchronizes the durable restart fence and its parent directory before it attempts any stop.
The runtime provider consumes the exact prepared authority for one stop attempt.
NemoClaw then observes workload execution and OpenShell sandbox access independently from that stop result.
The command preserves the registry entry, workspace, snapshots, logs, credentials, and other evidence.

A complete quarantine exits `0`, prints the fence ID and receipt path, and leaves the fence active.
A partial quarantine exits `2`, leaves the fence active, and does not roll back completed stops.
Use the same reason and idempotency key to reconcile the same request.
A different idempotency key conflicts with the active fence.

While the fence is active, commands that can reactivate, replace, restore, or upgrade the sandbox fail closed after they acquire the lifecycle lock.
Status, non-fixing doctor inspection, and evidence collection remain available when they do not weaken the fence.
The existing `destroy` confirmation and exact-target cleanup contract remains available.

Pass `--json` to return the result, fence, operation journal, and receipt metadata as JSON.
The receipt path is under the selected gateway's NemoClaw state directory.
Receipts use private host permissions and reject symbolic links.
Receipt fields and diagnostics are bounded and redacted, and neither the reason nor idempotency key is stored in plaintext.

For the operator workflow, refer to [Quarantine a Sandbox](../manage-sandboxes/operate-sandboxes/run-sandboxes#quarantine-a-sandbox).

### `$$nemoclaw <name> quarantine release`

Release one exact quarantine fence without starting the sandbox.

```bash
$$nemoclaw my-assistant quarantine release --fence-id <fence-id> [--json]
```

The required `--fence-id <id>` value must match the active fence.
NemoClaw validates that the runtime-provider authority still matches the fenced target.
It then independently observes that the exact runtime remains stopped and OpenShell sandbox access remains unavailable before it writes either release receipt or clears the fence.
A failed, inconclusive, or interrupted observation, receipt write failure, or changed target authority stops release and leaves the fence active for retry.

A successful release exits `0` and leaves the sandbox stopped.
Run `$$nemoclaw my-assistant start` separately when you intend to resume execution.
If the final receipt update fails, release exits `2`, the fence remains active, and retrying the same release can finish safely.
Pass `--json` to return the release result and receipt metadata as JSON.
