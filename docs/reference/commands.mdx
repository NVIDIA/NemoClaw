...
`Infinity`, `NaN`, and unparseable text are rejected for these auto-pair controls when the sandbox is created. A value that parses but sits above the range falls back to the documented default inside the sandbox instead of stopping the watcher, so a sandbox started outside the NemoClaw CLI is never left with a timer it cannot use.
