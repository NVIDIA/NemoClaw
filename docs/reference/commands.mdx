...
Strict backup reports a confirmed stranded registry sandbox without creating a backup for it because both the selected gateway and the container provider prove that no sandbox runtime remains.
Strict backup counts that record as stranded and exits nonzero, so complete the cleanup or recreation action printed by `$$nemoclaw backup-all` and rerun the command before you retire the gateway:
...
Confirm that the strict backup summary reports `0 failed, 0 skipped` with no stranded count and that the `Legacy dashboard forwards:` summary appears before gateway destruction.
A confirmed stranded record has no backup to reuse; clear or recreate it before the strict command can succeed.
...
Before an OpenShell upgrade, the installer prepares the current release CLI and uses it to run `backup-all` in strict mode. Strict mode requires every registered sandbox that still has a runtime to produce a fresh backup and aborts before gateway changes if an ordinary sandbox is skipped or fails. When strict mode reports an ordinary skipped sandbox, start that sandbox or its container and rerun the installer or `$$nemoclaw backup-all`. A registry record that the selected gateway and container provider both prove has no remaining runtime is reported and counted as confirmed stranded, produces no backup, and fails strict mode because it was not backed up. Follow the reported cleanup or recreation action in [Resume a Manually Prepared Upgrade](../manage-sandboxes/operate-sandboxes/update-sandboxes#resume-a-manually-prepared-upgrade), then rerun the installer or `$$nemoclaw backup-all`.
