...
The wizard creates an OpenShell gateway, registers inference providers, and creates the sandbox.
For stock OpenClaw, Hermes, and LangChain Deep Agents Code onboarding on Docker, NemoClaw resolves the installed CLI release through one complete all-agent managed-image catalog.
It selects the chosen agent's exact platform digest, creates the sandbox without staging or building a Dockerfile, and applies the generated startup profile through the transactional managed bootstrap.
If the catalog is unavailable or does not match the release, platform, all-agent publication cohort, or managed-image contract, onboarding stops before sandbox creation instead of falling back to a Dockerfile build.
...
The following base-image resolution controls apply only to the legacy Dockerfile workload path.
Stock managed-image onboarding and rebuild select exact release-catalog digests and do not run local base-image discovery or builds.

When Docker exposes the required identity metadata, NemoClaw records the base-image resolution on images produced through the legacy managed Dockerfile path.
...
It then prompts for optional web search and messaging channels, creates and starts the sandbox, and asks for a **policy tier** that controls the default set of network policy presets applied to the sandbox.
</AgentOnly>
<AgentOnly variant="deepagents">
It then prompts for optional web search, creates and starts the sandbox, and asks for a **policy tier** that controls the default set of network policy presets applied to the sandbox.
</AgentOnly>
...
Changing or disabling the selected provider recreates the sandbox because the plugin configuration and credential attachment are part of the managed startup profile.
...
Changing or disabling Tavily recreates the sandbox because the Hermes backend, environment placeholder, and credential attachment are part of the managed startup profile.
...
Use `--agents <agents.yaml>` to declare secondary OpenClaw agents, `agents.defaults`, and main-agent overrides in a checked-in manifest that NemoClaw applies through the managed startup profile.
...
The wizard also accepts optional `SLACK_ALLOWED_USERS` and `SLACK_ALLOWED_CHANNELS` values so you can restrict Slack DMs, channel `@mention` users, and channel IDs before NemoClaw creates the sandbox.
...
NemoClaw applies those values through the managed startup profile as Discord guild workspace config so the bot can respond in the selected server, not just in DMs.
...
Build the sandbox image from a custom Dockerfile through the legacy Dockerfile workload path instead of selecting the release managed image.
This flag bypasses the stock managed-image catalog and startup-profile path for that sandbox workload.
