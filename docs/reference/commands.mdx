...
The generated policy is also binary-scoped: the selected agent's Node.js adapter is allowed, while an interactive `/usr/bin/curl` request to the same endpoint is denied. Use `mcp status <server>` for the supported credential-resolution check; a failed shell `curl` is not evidence that the managed MCP route is broken.
