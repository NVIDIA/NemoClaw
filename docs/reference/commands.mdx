...
Docker and Podman gateways cannot reuse one state directory. Before switching drivers, stop the existing gateway and its sandboxes (for example with `$$nemoclaw uninstall`, or by destroying the sandboxes that use it), or point the new run at an unused directory with `NEMOCLAW_OPENSHELL_GATEWAY_STATE_DIR=<path>`.
