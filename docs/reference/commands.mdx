...
This is a thin pass-through to `openclaw skills list` for the primary `main` agent inside the sandbox. NemoClaw does not merge, cache, or maintain a second skill list. Native output and filtering arguments are forwarded verbatim. An agent override is refused because the lifecycle commands are bound to the sandbox's primary agent state.
...
This is a thin pass-through to `hermes skills list` inside the sandbox. Native filtering and output arguments are forwarded. NemoClaw does not merge, cache, or maintain a second Hermes skill list.
...
This is a thin pass-through to `dcode skills list` for DCode's primary `agent` state inside the sandbox. Native filtering and output arguments are forwarded. NemoClaw does not merge, cache, or maintain a second DCode skill list. An agent override is refused because the lifecycle commands are bound to that primary state.
...
This is a pass-through to `dcode skills delete <skill> --agent agent --force --json`. DCode resolves the skill from its own precedence-aware state and applies its native path and symlink checks before deletion. NemoClaw does not choose a directory or keep removal ownership metadata.
