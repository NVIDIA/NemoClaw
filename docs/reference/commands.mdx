...
It does not run `openclaw doctor --fix` on the restored configuration.
...
<AgentOnly variant="openclaw">
For OpenClaw, restore stops the sandbox, installs the complete credential-sanitized native `openclaw.json` file without a selective merge or config hash, validates the restored state, and starts the sandbox.
