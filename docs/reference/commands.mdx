...
| `NEMOCLAW_INSTANCE` | `default` | NemoClaw instance identity for multi-instance hosts. Scopes the state root (`~/.nemoclaw` for the default instance, `~/.nemoclaw-<instance>` otherwise) and the OpenShell gateway name so two instances on the same host stay segregated. |
...
The default behaviour applies when `NEMOCLAW_INSTANCE` is unset or empty: the state root stays at `~/.nemoclaw` and the gateway name stays at `nemoclaw`. Existing single-instance deployments therefore observe no change after upgrading. When the bare `nemoclaw` gateway is already in use, varying `NEMOCLAW_GATEWAY_PORT` alone produces a `nemoclaw-<port>` gateway that segregates from the bare default without requiring `NEMOCLAW_INSTANCE` — the port suffix is the implicit fallback identity for the default instance.
