...
OpenClaw accepts `subagents.maxSpawnDepth` only on `agents.defaults.subagents`, never inside a per-agent `subagents` object. To set it, use the object payload shape and pass it under `defaults`:
