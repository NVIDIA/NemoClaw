...
When matching onboarding state records Brave Search as enabled, rebuild also probes that route; a legacy Brave-enabled entry without matching state fails closed.
...
After any backup and immediately before provider detachment or deletion, NemoClaw rechecks the target gateway and registry entry and repeats the inference and applicable Brave Search probes.
