...
Each sandbox row reports `agent` as the sandbox's agent name in both text and JSON output: `openclaw` for a sandbox with no other agent recorded, and `unknown` for a row recovered from the live gateway, which does not expose the agent.
...
Each JSON sandbox row reports the same `agent` value as `$$nemoclaw list` and `$$nemoclaw <name> status --json`, so `openclaw` is reported for a sandbox with no other agent recorded.
