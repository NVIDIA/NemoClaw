...
$$nemoclaw my-assistant mcp update github --refresh-public-pins
```
...
| `--refresh-public-pins` | Validate current public DNS answers and replace only the existing endpoint's `allowed_ips` |

Public-pin refresh requires matching current sources and existing exact public pins.
It preserves all other policy fields, including denied tools, methods, binary grants, and credential bindings.
It rejects private or special-use answers, address ranges, and conflicting sources before writing.
If the update is not confirmed, inspect status and the live policy before retrying.
Restart, rebuild, and restore do not refresh pins.
