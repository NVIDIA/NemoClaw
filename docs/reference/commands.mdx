...
For a LangChain Deep Agents Code sandbox that uses OpenRouter, the adapter returns HTTP `404` for `https://inference.local/v1/models`.
During final launch-readiness validation, `connect --probe-only` and `launch` send one inference request through the same route instead of treating that `404` as a failure.
The command accepts the route only when the request returns a structurally valid response.
If the request fails, `connect --probe-only` exits nonzero and identifies the inference request as the failed check.
