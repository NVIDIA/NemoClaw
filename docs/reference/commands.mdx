...
Before upload, NemoClaw snapshots at most 1,024 files and 64 MiB with no-follow reads. Files with names starting with `.` are skipped and listed. Unsafe paths, symlinks, and special files are rejected. Staging uses a private temporary directory. NemoClaw attempts cleanup after the add or an interrupted upload, retries once when post-add cleanup fails, and fails the command with the staging path if cleanup cannot complete.
