...
Raw web search and messaging credentials are never written to the onboarding session, so interactive resume can request a missing credential again while preserving the completed non-secret choices.
If a completed messaging choice lacks its credential during non-interactive resume, NemoClaw preserves the choice and exits; export the missing messaging credential environment variables, then run `$$nemoclaw onboard --resume` again.
