...
For a Docker-driver sandbox, `start` binds recovery to the immutable identity of the existing container.
It does not rebuild, replace, or destroy the container.

`start` returns success only after these checks pass:

- The authenticated managed gateway is healthy.
- OpenShell reports the sandbox in the `Ready` phase.
- Every required host forward is active.
- A final authenticated managed-gateway check passes after inference-route and pairing reconciliation.
...
Run the printed `$$nemoclaw <name> recover` command before you retry `start`.
