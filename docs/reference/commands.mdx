...
Each entry must declare `id` and `tools`; `workspace`, `agentDir`, and `subagents.maxSpawnDepth` are optional.
...
- `workspace` defaults to `/sandbox/.openclaw/workspace-<id>`; when set, must equal that exact value.
- `agentDir` defaults to `/sandbox/.openclaw/agents/<id>`; when set, must equal that exact value.
