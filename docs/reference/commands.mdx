...
| `NEMOCLAW_INSTALL_TAG` | release tag | For internal installer commands: the release tag to install. Defaults to `lkg` when unset. Overridden by the `--install-tag` flag. |
