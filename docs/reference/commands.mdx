...
`launch` returns the OpenClaw process exit code.
...
After an OpenClaw one-shot command exits, `exec` returns the remote command's exit code.

For a registered OpenClaw sandbox with a selected owning managed gateway, a successful direct `openclaw pairing approve googlechat <code>` command also restarts that gateway so the new sender allowlist applies to the next message. If restart fails after the approval commits, `exec` exits with status `1` and reports that the approval was not rolled back. Run `$$nemoclaw <name> gateway restart` before testing the next message. Without an owning managed gateway, NemoClaw does not attempt activation or print a managed restart command; unregistered and non-OpenClaw sandboxes also do not receive the automatic restart.
...
This host command is not available for OpenClaw because OpenClaw owns `openclaw.json` after first launch.
Connect to the sandbox and use `openclaw configure`, `openclaw config set`, or `openclaw config unset`.
Use `$$nemoclaw inference set`, credential commands, and policy commands when the change also requires host-side OpenShell state.
...
```bash
$$nemoclaw my-assistant config set --key model.default --value openai:nvidia/nemotron
$$nemoclaw my-assistant config set --key agent.max_turns --value 100 --restart
```
...
| `--restart` | Restart the Hermes gateway after writing |
...
For a stopped, non-paused Docker-driver container, `recover` starts the sandbox through OpenShell before it waits for readiness.
If the container already runs while OpenShell reports `Stopped`, `recover` still starts the sandbox through OpenShell to repair the phase.
If OpenShell cannot start a stopped container, recovery starts that container directly, then continues to the readiness check and reports the resulting failure.
...
For a Podman sandbox, `recover` requires the sandbox to be running and ready in OpenShell before it checks the native gateway.
If the Podman sandbox is stopped, run `$$nemoclaw <name> start` before `recover`.

After the sandbox is ready, `recover` checks the built-in agent's native health endpoint.
It does not restart an unhealthy or stopped built-in gateway.
Run `$$nemoclaw <name> gateway restart` to request a native agent restart.
If that command cannot run, stop and start the sandbox through OpenShell.
...
NemoClaw then verifies the agent health endpoint and repairs only the host-side OpenShell forwards.
...
| `--fix` | Approve pending allowlisted dashboard and CLI tool-scope upgrades. Mutually exclusive with `--json` |
...
After the remote command exits, `exec` preserves its exit code.

A successful direct `openclaw pairing approve googlechat <code>` command in a registered OpenClaw sandbox with a selected owning managed gateway restarts that gateway, so the new sender allowlist applies to the next message. If the approval commits but restart fails, `exec` exits with status `1` and reports that the approval was not rolled back. When an owning gateway was selected, it directs you to run `$$nemoclaw <name> gateway restart`. Without an owning managed gateway, NemoClaw does not attempt activation or print a managed restart command; unregistered and non-OpenClaw sandboxes do not receive the automatic restart.
...
Host-side removal is the supported path because it removes the OpenShell credential binding and policy with the channel configuration.
For OpenClaw, NemoClaw restores the complete credential-sanitized native config across rebuilds, but an in-sandbox channel removal cannot update those host resources.
For Hermes, managed startup regenerates `/sandbox/.hermes/.env` and the related config during rebuild.
...
Restore sandbox state from a snapshot. For an in-place restore, the sandbox must be running. If no selector is provided, the latest snapshot is used. Restore removes files added after the snapshot only from state directories selected for cleanup. It preserves directories that exist only in the target manifest or whose backup failed. State replacement runs under the per-sandbox mutation lock. For OpenClaw, restore installs the complete credential-sanitized native `openclaw.json` file without a selective merge or config hash. In-place restore does not change the OpenShell policy. A cross-sandbox clone reads the source live policy and hands that document to destination creation.
...
<AgentOnly variant="hermes">
| `NEMOCLAW_CONFIG_ACCEPT_NEW_PATH` | Exactly `"1"` to opt in (`true`, `yes`, `on` are not accepted) | Allows `nemohermes <name> config set` to write a dotpath that does not already exist in the sandbox config, without the interactive confirmation. Equivalent to passing `--config-accept-new-path`, and it takes precedence over `NEMOCLAW_NON_INTERACTIVE=1`. Without it, a run without a TTY refuses the write instead. |
</AgentOnly>
