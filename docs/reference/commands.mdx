...
The recovery action tells you to restore or repair registry metadata, align conflicting routes, use a credential-free root or `/v1` API base, or omit endpoint options for an adapter-managed same-provider model change.
These diagnostics never include a stored URL, registry path, or underlying registry error.
