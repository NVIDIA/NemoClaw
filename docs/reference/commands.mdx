...
Older Hermes sandbox images that predate the standalone validator are detected and fail closed: `recover` refuses the gateway recovery, names the sandbox, and explains that `/sandbox/.hermes/.env` could not be re-evaluated. Re-image the sandbox to a current Hermes build before retrying recovery.
