...
For a compatible custom provider, also show the persisted endpoint URL when the selected gateway's matching registry metadata is safe and unambiguous.
...
When participating metadata is missing, invalid, conflicting, or has an invalid gateway binding, text output reports a non-secret `Endpoint: unavailable` reason, up to five affected sandbox names, and recovery guidance.
JSON output adds a bounded `endpointDiagnostic` object with `reason`, `affectedSandboxNames`, `additionalAffectedSandboxCount`, and `recovery`; it never includes a rejected endpoint or gateway binding.
Run the reported status command before changing registry metadata.
For `invalid-gateway-binding`, restore known-good `gatewayName` and `gatewayPort` metadata from a trusted backup.
Do not copy a binding from another sandbox.
If no trusted backup is available, back up and remove the affected sandbox, then re-onboard it.
