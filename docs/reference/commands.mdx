...
The same preservation applies if you cancel at the policy-tier selector or either policy-preset selector after sandbox creation.
...
After an OpenShell administrator verifies the live durable ID, removes that exact sandbox through an identity-bound procedure, and confirms removal, repeat the original onboarding intent with `$$nemoclaw onboard --fresh --name <new-sandbox>`.
