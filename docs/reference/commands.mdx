...
| `sessions rm <agent> [<sessionKey>]` | Wipe a single session or the whole agent's sessions directory in the sandbox. |
...
The OpenClaw gateway should be idle for the target agent before running `sessions rm`; live writes against `sessions.json` race the gateway writer.
...
```console
$ nemoclaw <name> sessions rm <agent> [<session>]
```
...
The OpenClaw gateway should be idle for the target agent before running this command; live writes against `sessions.json` race the gateway writer.
