...
- Status `1` with blank stdout and exactly one stderr line: `SUPERVISOR_NOT_RUNNING`, `SUPERVISOR_DISCOVERY_PENDING`, `GATEWAY_HEALTH_TIMEOUT`, or `SUPERVISOR_BUSY`. A typed host-side direct-container discovery miss is sanitized to the exact `PRIVILEGED_CONTROL_UNAVAILABLE` line and is also retryable; the same untyped line from an in-container helper is terminal.
...
Recovery counts typed direct-container discovery misses separately from other retryable results.
It stops when either count reaches its limit: 21 unavailable results or 11 other retryable results.
A sequence containing both permits at most 31 total attempts. The complete managed-control loop has one 210-second deadline shared by controller execution and polling. At the default interval, attempts alone can add no more than 90 seconds of sleep, but each controller call consumes the same total deadline.
Recovery stops after 3 attempts return `SUPERVISOR_BUSY`.
The managed controller emits `SUPERVISOR_DISCOVERY_PENDING` only when an incomplete process-table scan during startup cannot yet prove either one exact supervisor or clean supervisor absence. That result delays recovery but cannot authorize container recreation or accept a supervisor identity; a later request must perform the complete identity proof again. Managed settle confirmation treats exact `SUPERVISOR_BUSY` and `SUPERVISOR_DISCOVERY_PENDING` results as inconclusive within its configured window. Status `137` and the Docker restart result remain terminal during that confirmation. The managed supervisor startup waiter accepts the three exact controller startup results, the typed direct-container discovery miss, `SUPERVISOR_BUSY`, and the two container-transition results.
It uses the same separate allowances for unavailable and other retryable results.
...
Hermes Portable forward repair reports an occupied recorded host port separately from unavailable or ambiguous `openshell forward list` state. Both conditions stop before forward mutation. An occupied-port diagnostic means another sandbox or listener owns a required port; an unavailable-state diagnostic means NemoClaw could not safely parse and prove one forward state. NemoClaw never stops an unproved owner automatically.
