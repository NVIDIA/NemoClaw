...
The export records the external Ollama daemon port, the NemoClaw-managed authentication proxy port, and the verified selected model name and SHA-256 digest.
...
For a sandbox that uses the Docker driver, `docker_unreachable` means `docker info` did not return valid server data. It does not show that the sandbox container is missing. If Docker is stopped, follow [Docker is not running](troubleshooting#docker-is-not-running). If `docker info` reports a permission-denied error on Linux, follow [Docker permission denied on Linux](troubleshooting#docker-permission-denied-on-linux). For any other failure, inspect and correct the reported `docker info` error, such as an invalid context or TLS configuration. Make `docker info` succeed, then retry `$$nemoclaw <name> start`.
