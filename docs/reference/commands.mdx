...
NemoClaw records the base-image resolution on each managed sandbox image.
During a warm recreate or rebuild, it validates the repository digest or local image identity, platform, and OpenShell ABI requirement through local Docker metadata before reusing it.
...
Normal resolution checks compatible images in Docker's local image store before attempting to pull a missing published candidate.
On Linux, NemoClaw can reuse or build an ABI-compatible local fallback when published candidates are unavailable or incompatible.
...
If NemoClaw cannot validate, pull, reuse, or build a compatible base image, the operation fails instead of using an unvalidated or stale image.
