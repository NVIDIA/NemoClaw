...
Supported sandboxes use hosted external inference, an immutable managed image, a NemoClaw-owned gateway, and an explicit representable policy.
...
NemoClaw keeps the native entry while it verifies the live policy binding and provider identity, type, and credential key. It removes the generated policy before detaching that exact provider, as OpenShell requires. It then verifies that no attached provider carries the MCP credential key and that fresh sandbox processes no longer receive it, before removing the native entry. The workspace provider and its stored credential remain available for explicit cleanup.

If cleanup cannot be proved, the command exits nonzero and retains the native entry. A retry after policy removal does not infer provider ownership from its name. If an attachment still carries the MCP credential key, inspect and detach that attachment explicitly before retrying, even if fresh processes no longer receive the credential.
...
| Flag | Description |
| --- | --- |
| `--force` | Permit removal of a modified same-name native agent entry after verified policy, attachment, and credential cleanup; never bypass those checks or delete the workspace provider |
