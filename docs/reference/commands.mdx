...
| `--fresh` | Reinstall the maintained build for a clean re-clone of `~/.nemoclaw/source`; useful to repair a broken install. Does not reset onboarding state. Runs only when the reinstall is provably not a version regression. |
| `--allow-downgrade` | Allow `--fresh` to reinstall the maintained build when NemoClaw cannot prove that doing so is not a downgrade. |
...
`--fresh` therefore runs only when NemoClaw can show the reinstall is not a version regression, which means the maintained build is either newer than the install or the same version.
NemoClaw resolves the maintained tag to a commit before comparison.
The installer uses that commit even if the tag changes during the command.
It reports the reason and exits non-zero in every other case: the install is newer than the maintained tag, the two versions cannot be ordered, or the maintained tag does not resolve to a version at all.
