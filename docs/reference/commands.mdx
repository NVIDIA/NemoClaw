...
For a non-interactive install, set `NEMOCLAW_CONFIRM_LEGACY_MANAGED_RECREATE=1` only after verifying every listed sandbox used a managed image.
...
| `NEMOCLAW_CONFIRM_LEGACY_MANAGED_RECREATE` | Exactly `1` to confirm | Confirms to the installer that every listed pre-fingerprint OpenClaw or Hermes sandbox used a NemoClaw-managed image, allowing recovery onto the current managed image. Set it only after verifying every listed sandbox. Recorded custom-image evidence remains blocked. |
