...
<AgentOnly variant="hermes">
<Warning>
For a NemoClaw-managed, non-portable Docker Hermes sandbox, uninstall removes its managed state volume even without `--destroy-user-data`.
...
`--yes` only acknowledges the global `Proceed?` confirmation prompt and still preserves the listed host-side entries.
...
Existing automation using `--yes` therefore retains those entries.
