...
| `NEMOCLAW_EXTRA_PLACEHOLDER_KEYS` | whitespace- or comma-separated list of upper-snake env keys | Adds operator-supplied credentials to the matching canonical OpenShell provider. Per-profile credentials such as `TELEGRAM_BOT_TOKEN_AGENT_A` use the canonical credential's policy-bound, out-of-process placeholder injection instead of raw text in each Hermes profile `.env`. Refer to [Extra placeholder keys](#extra-placeholder-keys) for the entry shape and validation rules. |
...
For each entry, NemoClaw adds the credential to the selected canonical channel or web search provider.
This keeps the extension under the canonical provider's explicit policy binding instead of creating an unbound generic provider.
OpenShell resolves the named credential placeholder to the operator-supplied value at the bound egress boundary.
The Hermes profile `.env` files are operator-owned: write `TELEGRAM_BOT_TOKEN_AGENT_A=openshell:resolve:env:TELEGRAM_BOT_TOKEN_AGENT_A` in the applicable profile `.env` and use the matching assignment for each configured credential.
Do not write a raw token.
...
The selected sandbox plan must include the corresponding canonical channel or web search provider.
Otherwise, NemoClaw warns and ignores the extension.

If neither the credential store nor the host environment has a value for an entry at onboarding time, NemoClaw omits the extension from provider registration.
The canonical provider does not receive the credential key, so the corresponding placeholder cannot resolve inside Hermes.
Add the credential before running `$$nemoclaw onboard` for that profile.
