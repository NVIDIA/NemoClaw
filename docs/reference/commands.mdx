...
OpenShell can keep listing the deleted sandbox while its runtime terminates, so `destroy` polls the list for up to 30 seconds in total until that row is gone.
...
After that message, confirm that `openshell sandbox list` reports no sandboxes.
