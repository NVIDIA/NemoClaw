...
Preserve the record as evidence, rerun the original onboarding command with the same required provider, model, agent, policy, and environment inputs, add `--fresh`, and use another available sandbox name.
