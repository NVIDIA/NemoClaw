...
`image/fabric/dependencies.lock` pins the third-party Linux ARM64 Python wheels and their hashes.
Fabric's three or four Python wheels per recipe are built from that verified source and receive hashes in the generated installation lock.
Pi additionally retains the verified TypeScript source, npm lockfiles, and installed package licenses in `/opt/fabric-source`.
Per-harness Python locks are alongside the default lock.
Installed wheel license metadata remains in `/opt/fabric`; source provenance and the installation-lock hash are in `/opt/nemoclaw/provenance.json`.
The verified source archive and extracted source remain under `.build/fabric` on the builder.
