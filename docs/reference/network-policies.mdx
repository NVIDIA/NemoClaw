...
| Personal | `personal-open-internet` | Lets every sandbox binary open TCP connections to public and private address ranges on destination ports `80` and `443`. The broad route replaces overlapping web endpoints while preserving non-web policy. Unspecified, loopback, and link-local ranges remain blocked. |
...
For a fresh onboarding run, the experimental portable profile selects the Personal tier in `suggested` mode when `NEMOCLAW_POLICY_PRESETS` is unset, blank, or contains only whitespace.
The resulting tier selection applies `personal-open-internet`.
...
`$$nemoclaw onboard --resume` does not inject the fresh-onboarding Personal selection.
