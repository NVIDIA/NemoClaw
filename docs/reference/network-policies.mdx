...
To replace the live policy with a complete raw policy file, start from the sandbox-authored policy and use `openshell policy set`:

```bash
openshell policy get <sandbox-name> --base | awk 'found { print } /^---$/ { found = 1 }' > current-policy.yaml
