...
When you reselect Balanced's suggested presets, NemoClaw replaces the previous built-in `brew` preset with `brew-balanced`.
Explicit `brew` selections retain its broader policy.
Resuming onboarding keeps the current `brew` selection rather than reselecting Balanced defaults.
Skipping optional setup preserves the applied `brew` policy.
Operator-owned `brew` policies are not replaced.
