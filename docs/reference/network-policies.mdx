...
LangChain Deep Agents Code sandboxes use `agents/langchain-deepagents-code/policy-additions.yaml` for the equivalent managed Python runtime boundary.
</Note>
...
The Restricted tier suppresses both agent-required local observability presets.
...
The sandbox sends headerless OTLP/HTTP protobuf traces to a collector that the operator runs on the host.
...
The metadata-only guarantees apply to managed `dcode` instrumentation, while arbitrary Python can encode data in span names, status descriptions, events, links, or allowed attribute values.
The collector must validate and redact received traces before forwarding them, and it must not treat span fields such as `service.name` as authenticated tenant identity.
For setup and privacy details, refer to [Deep Agents Code OTLP Traces](commands#deep-agents-code-otlp-traces).
