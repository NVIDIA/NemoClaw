...
<Note>
Hermes sandboxes use an agent-specific baseline policy in `agents/hermes/policy-additions.yaml` so Hermes runtime binaries can reach the service endpoints they need while keeping the same deny-by-default model. Messaging channel egress is not part of that baseline; Hermes channel presets live under `src/lib/messaging/channels/<channel>/policy/hermes.yaml` and are applied only when that channel or preset is selected.
</Note>
...
The baseline policy does not include messaging endpoints for Telegram, Discord, Slack, WeChat, WhatsApp, or Microsoft Teams.
...
| Open | `npm`, `pypi`, `huggingface`, `brew`, `brave when supported`, `weather`, `public-reference`, `slack`, `discord`, `telegram`, `teams`, `wechat` (experimental), `whatsapp` (experimental), `jira`, `outlook` | Broad access across third-party services including messaging, productivity, weather, and public-reference APIs. |
