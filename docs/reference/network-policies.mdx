...
<AgentOnly variant="openclaw">
`$$nemoclaw onboard --resume` reconciles the policy selection instead of skipping it while that channel's preset is still applied.
</AgentOnly>
<AgentOnly variant="hermes">
A completed `$$nemoclaw onboard --resume` keeps the recorded messaging selection when no replacement credential is supplied.
Run `$$nemoclaw <sandbox> channels remove <channel>` to remove a recorded channel and its matching policy preset.
</AgentOnly>
<AgentOnly variant="openclaw,hermes">
