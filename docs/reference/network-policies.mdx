...
The separate `raw.githubusercontent.com` route lets Deep Agents Code follow
GitHub file links and read repository source through its managed `fetch_url`
tool. Repository, ref, and file path segments vary by task, so the route covers
the host while limiting requests to read-only GET and HEAD methods and the
listed managed binaries.
