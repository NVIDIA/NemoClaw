...
`policy restore` previews the egress it re-allows and then requires the same explicit acknowledgement as `policy exclude` (`--force` or `--yes` in non-interactive use).
