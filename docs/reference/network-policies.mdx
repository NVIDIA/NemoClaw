...
The command prints every endpoint, method/path rule, and binary the exclusion removes, and requires explicit acknowledgement (`--force` in non-interactive use).
The exclusion is bound to the reviewed entry content, recorded in the sandbox registry, and replayed on rebuild.
...
An unreadable or third live state remains fail-closed for manual inspection instead of being guessed or overwritten.
...
The critical `managed_inference` entry cannot be excluded because it carries the required managed inference route. NemoClaw enforces this both when the command runs and whenever durable exclusion state is replayed.
...
If the release removed the entry entirely, `policy exclude <key>` fails with "Unknown baseline entry" because there is nothing left to exclude. Clear the stale record instead:
