...
| Balanced (default) | `npm`, `pypi`, `huggingface`, `brew`, web-search preset (when supported) | Full dev tooling and web search for agents that support web search. No messaging platform access. |
| Open | `npm`, `pypi`, `huggingface`, `brew`, web-search preset (when supported), `slack`, `discord`, `telegram`, `wechat` (experimental), `whatsapp` (experimental), `jira`, `outlook` | Broad access across third-party services including messaging and productivity. |
...
NemoClaw filters tier defaults by the active agent's supported integrations and the selected web-search provider.
The included web-search preset depends on the provider you picked during onboarding: `brave` for Brave Search (default) or `duckduckgo` for the experimental DuckDuckGo backend.
NemoClaw prunes the unused web-search preset from the tier defaults.
For example, Hermes onboarding omits the web-search preset because Hermes does not use NemoClaw's OpenClaw web-search configuration.
