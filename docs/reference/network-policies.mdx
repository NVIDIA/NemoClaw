...
OpenClaw can select `brave` or `tavily`, while Hermes can select `tavily` or `firecrawl`.
...
When Hermes uses a bring-your-own-key web search provider (Tavily or Firecrawl), NemoClaw removes `nous-web` from the effective managed-tool selection while preserving other selected Nous tool presets.
