...
The installer records a pending automatic port under its port-scoped state root before onboarding and promotes it after success.
Later installer and CLI processes restore either identity when `NEMOCLAW_GATEWAY_PORT` is unset, so interrupted onboarding returns to the same port-scoped recovery state.
