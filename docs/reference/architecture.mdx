...
| `~/.nemoclaw/gateways/<port>/` | Segregated host state root (its own registry, credentials, and snapshots) for a non-default `NEMOCLAW_GATEWAY_PORT`. The default gateway port uses the top-level `~/.nemoclaw/` location, so existing single-gateway hosts are unchanged. |
<AgentOnly variant="openclaw">
