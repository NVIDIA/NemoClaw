...
It runs the real blueprint runner against a real OpenShell gateway and sandbox, exchanges a refresh token through a public HTTPS OAuth endpoint, proves the sandbox sees only a stable placeholder, proves the protected resource receives successive versioned bearer tokens before and after rotation, checks secret-free plan, status, state, logs, and request ledgers, and verifies ownership-aware rollback.
...
During apply, the runner first inspects the target sandbox and stops before identity mutation unless it confirms existence or an explicit sandbox-not-found result.
...
Before attaching that runtime identity to the sandbox, the runner creates or reuses the configured inference provider and requires `openshell inference set` to verify the requested route.
This keeps the new credential attachment out of the sandbox until OpenShell accepts the requested inference route; after that verification succeeds, the runner attaches the runtime identity and applies any policy additions.
