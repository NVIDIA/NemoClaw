...
Before adoption, NemoClaw asks systemd for the unit's effective `ExecStart` and checks that binary's version against the blueprint range (`min_openshell_version` through `max_openshell_version`).
...
When NemoClaw declines the package unit, it reports the version once and selects an existing marked NemoClaw user service when available.
