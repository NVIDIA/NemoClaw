...
For tarball installs, the NemoClaw installer stages a marked user-level `nemoclaw-openshell-gateway.service` based on the upstream unit; onboarding validates, enables, restarts, and checks the selected service.
...
Gateways on custom ports remain detached and separate from the default service.
