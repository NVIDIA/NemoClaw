...
The deterministic fixture proves the complete Entra-shaped runtime path without storing a third-party tenant credential in pull-request CI; tenant acceptance remains an operator check against Microsoft Entra and Graph.
...
For tenant acceptance, apply the copied profile, confirm refresh status, and from a newly launched sandbox child call:
...
The child must see an `openshell:resolve:env:...` placeholder rather than the token.
The Graph request must return the signed-in delegated user, and a request to another Graph path must be denied by OpenShell policy.
Rotate the credential, launch a new child, repeat `/v1.0/me`, then roll back and confirm the attachment and owned provider are removed.
Do not save the Graph response, refresh token, access token, or client secret in CI artifacts.
