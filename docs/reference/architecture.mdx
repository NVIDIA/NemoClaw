...
For a reused sandbox and provider, it reads the live gateway route and preserves it only when the provider and model exactly match the blueprint; an absent or different route must pass `openshell inference set`.
