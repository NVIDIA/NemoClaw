...
During apply, the runner first inspects the requested provider name and stops if any same-name provider already exists.
...
Only an absent provider proceeds to profile import, or exact export comparison when that profile is already registered, followed by provider creation, gateway-managed OAuth refresh configuration, initial token rotation, and sandbox attachment.
