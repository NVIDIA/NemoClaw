...
<Warning>
  ...
  OpenShell 0.0.85's gateway-side OAuth refresh client enforces HTTPS but does not pin the address NemoClaw resolved, so only identity-platform-controlled DNS suffixes such as `okta.com` belong in this trust table.
