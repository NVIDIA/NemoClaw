...
The direct blueprint runner can opt in to a provider-neutral runtime identity component.
...
<Warning>
  The pinned OpenShell runtime resolves the OAuth token URL and provider endpoint hostnames again after NemoClaw validates them.
  Because that boundary cannot yet preserve both the validated peer and the original HTTPS identity, the direct runner rejects every DNS-backed runtime identity destination before profile import.
  The current `okta-runtime-v1` trust policy admits only `okta.com` hostnames, so the bundled profile is a reference contract and does not currently provide an activatable production Okta path.
  Remove this fail-closed restriction only when the OpenShell refresh and provider endpoint contracts atomically validate and connect to the same peer.
</Warning>
...
DNS-backed HTTPS destinations are also rejected because the pinned OpenShell runtime would resolve their hostnames again after validation.
...
During apply, the runner imports the profile or exports an existing profile and requires its parsed YAML content to match the requested profile.
It then creates or verifies the runtime-credential provider, configures gateway-managed OAuth refresh, rotates an initial access token, and attaches the provider to the sandbox.
The runner refuses to reuse a same-name provider unless its type, credential key, and absence of unrelated configuration match the requested binding.
The persisted run plan records whether that apply created the provider and its sandbox attachment.
...
If a later apply step fails, the runner detaches the provider when that apply attached it, deletes the provider when that apply created it, and removes a sandbox created by that apply.
...
The sandbox receives an opaque `OKTA_ACCESS_TOKEN` placeholder, and the OpenShell L7 proxy substitutes the current access token only for admitted HTTPS requests.
...
The same blueprint can optionally configure `components.policy.middlewares`.
Those entries use the OpenShell middleware API to extend the existing middleware chain with endpoint selection, ordering, and `fail_closed` behavior.
The runner rejects a requested middleware name that would replace an entry in the existing chain.
A blueprint can declare at most 10 middleware entries.
Middleware order values must be unique; omitted values default to `0`.
Middleware is optional for Okta access-token refresh and presentation.
Use it only when the deployment has a compatible middleware installed for additional request policy.
