...
The direct blueprint runner can opt in to an Okta runtime-identity component.
...
It is for operators extending a reviewed blueprint with a tenant-specific Okta profile.
...
Copy `nemoclaw-blueprint/provider-profiles/okta-runtime-v1.yaml` into the blueprint you operate.
Set its `token_url` to the tenant's authorization-server token endpoint, replace `api.example.okta.com` with the approved upstream API host, and add only the API scopes that the Okta authorization server grants to `refresh.scopes`.
