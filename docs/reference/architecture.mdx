...
Homebrew 6.x can return a pinned-formula untrusted-tap refusal from `brew info --json=v2 openshell` after `brew list --formula openshell` reports the formula installed.
...
Every other Homebrew or `launchctl` error stops onboarding.
