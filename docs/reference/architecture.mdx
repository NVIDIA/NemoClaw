...
If the selected systemd user service cannot be inspected, started, or made healthy, NemoClaw prints its `journalctl` command and attempts to stop a service that reached startup.
It attempts the standalone fallback even when the stop fails.
...
If the Homebrew service cannot be inspected, started, or made healthy, NemoClaw prints the formula log command and attempts to stop a service that reached startup.
It attempts the standalone fallback even when the stop fails.
The standalone gateway starts only after NemoClaw verifies exclusive ownership of the gateway port.
A host without Homebrew, or with Homebrew but no installed OpenShell formula, also uses the standalone macOS gateway.
An installed Homebrew formula whose identity cannot be verified or does not match the official tap remains a hard failure.
Systemd service trust validation and unsafe environment configuration also remain hard failures.
