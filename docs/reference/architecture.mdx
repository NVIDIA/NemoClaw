...
The managed startup-profile applicator turns NemoClaw onboarding choices into Hermes YAML and environment files when the sandbox starts, and the Hermes plugin manifest exposes NemoClaw tools and an `on_session_start` hook.
</AgentOnly>
<AgentOnly variant="deepagents">
...
The managed startup-profile applicator turns NemoClaw onboarding choices into `config.toml` when the sandbox starts, and the managed launchers enforce the supported credential, MCP, tracing, and sandbox boundaries before `dcode` starts.
</AgentOnly>
...
On supported Docker hosts, stock NemoClaw onboarding pulls the selected agent's immutable managed release image at an exact digest.
Those published images are built from the [`ghcr.io/nvidia/nemoclaw/sandbox-base`](https://github.com/NVIDIA/NemoClaw/pkgs/container/nemoclaw%2Fsandbox-base) base, but stock onboarding does not layer the runtime Dockerfile on the user's host.
An explicit `--from <Dockerfile>` request remains the separate local-build path.
</AgentOnly>
<AgentOnly variant="deepagents">
On supported Docker hosts, stock Deep Agents onboarding pulls the immutable managed release image at an exact digest.
That published image is built from the agent-specific `agents/langchain-deepagents-code/Dockerfile.base`, which installs Node, Python, shell tools, and the hash-locked `deepagents-code` package needed by the terminal harness.
Stock onboarding does not layer the managed Deep Agents runtime Dockerfile on the user's host; an explicit `--from <Dockerfile>` request remains the separate local-build path.
</AgentOnly>
<AgentOnly variant="openclaw">
...
| `SLACK_ALLOWED_CHANNELS` | Comma-separated Slack channel IDs where channel `@mention` events are enabled (e.g. `C012AB3CD,C987ZY6XW`). Stock onboarding records the value in the managed startup profile and materializes it when the sandbox starts; an explicit custom Dockerfile must apply it during its local build. Combine with `SLACK_ALLOWED_USERS` to restrict both channel and member. |
| `CHAT_UI_URL` | URL for the optional chat UI endpoint. |
| `NEMOCLAW_DISABLE_DEVICE_AUTH` | Sandbox-creation toggle that disables gateway device pairing when set to `1` before onboarding. Stock onboarding records it in the managed startup profile; an explicit custom Dockerfile must apply it during its local build. |
</AgentOnly>
<AgentOnly variant="hermes">
| `TELEGRAM_BOT_TOKEN` | Telegram bot token you provide before `$$nemoclaw onboard`. OpenShell stores it in a provider; the sandbox receives placeholders, not the raw secret. |
| `TELEGRAM_ALLOWED_IDS` | Comma-separated Telegram user or chat IDs for allowlists when onboarding applies channel restrictions. |
