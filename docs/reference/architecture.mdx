...
Hermes keeps its agent-owned image, plugin, config, entrypoint, and baseline policy additions under `agents/hermes/`.
Messaging channel egress is channel-owned: Hermes channel presets live under `src/lib/messaging/channels/<channel>/policy/hermes.yaml` and are applied only when that channel or preset is selected.
