...
If the sandbox exists, the runner inspects the configured inference provider before identity mutation.
It reuses that inference provider when inspection succeeds and permits creation only after an explicit provider-not-found result.
...
The persisted run plan records whether that apply created the provider, its sandbox attachment, and the sandbox itself.
