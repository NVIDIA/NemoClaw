...
It attempts the standalone fallback even when the stop fails.
The standalone gateway starts only after NemoClaw verifies exclusive ownership of the gateway port.
A host without Homebrew also uses the standalone macOS gateway.
Managed-service trust validation and unsafe environment configuration remain hard failures on both platforms.
