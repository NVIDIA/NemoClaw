...
Managed inference profiles are assembled from declarations under `managed-inference/models/`, `managed-inference/recipes/`, and `managed-inference/presets/`.
Model definitions own identity, immutable revisions, aliases, preparation steps, and API capabilities.
Recipes reference a model and define a runnable backend, image, resource floor, and execution contract.
Presets bind recipes to compatible hardware and carry selection priority, support state, and validation evidence.
The catalog compiler validates and joins those layers, rejects ambiguous defaults and missing references, and includes the result in the packaged runtime catalog.

Resolution filters out incompatible candidates before it compares priority.
General Linux presets provide a baseline, while higher-priority appliance presets can select optimized settings for DGX Spark, DGX Station, N1x, or another registered hardware target.
Adding a model that uses an existing execution contract does not require a model-specific TypeScript branch.
