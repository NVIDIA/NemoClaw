...
After onboarding succeeds, the installer records an automatically selected port under its port-scoped state root, and later CLI processes restore that selection when `NEMOCLAW_GATEWAY_PORT` is unset.
