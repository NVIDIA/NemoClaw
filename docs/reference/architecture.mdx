...
When Homebrew cannot confirm the formula identity, for example when it refuses to load the formula from a tap it has not marked trusted, onboarding warns and uses the standalone macOS gateway fallback.
