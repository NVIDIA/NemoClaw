...
The direct blueprint runner can opt in to an Okta or Microsoft Entra runtime-identity component.
...
It is for operators extending a reviewed blueprint with a tenant-specific identity-provider profile.
...
Copy the applicable template into the blueprint you operate.
For Okta, use `nemoclaw-blueprint/provider-profiles/okta-runtime-v1.yaml`, set `token_url` to the tenant's authorization-server token endpoint, replace `api.example.okta.com` with the approved upstream API host, and add only the API scopes that the Okta authorization server grants to `refresh.scopes`.
For Microsoft Entra, use `nemoclaw-blueprint/provider-profiles/entra-graph-runtime-v1.yaml`, replace `common` in the token URL with the approved tenant, and reduce the Microsoft Graph scope and path to the delegated permission the workload needs.
...
Configure exactly one reference per blueprint run under `components`:
...
identity:
  entra:
    profile_path: provider-profiles/acme-entra-graph-runtime.yaml
...
Obtain the refresh token through an authorized OAuth bootstrap flow for the same client and authorization server as the profile.
`openshell gateway login` authenticates a CLI user to the gateway and is not part of this runtime-credential flow.

During apply, the runner imports the profile, creates a runtime-credential provider, configures gateway-managed OAuth refresh, rotates an initial access token, and attaches the provider to the sandbox.
The refresh token and optional client secret are passed as secret material and stay in the gateway credential store.
The sandbox receives only the profile's opaque credential placeholder, `OKTA_ACCESS_TOKEN` for Okta or `MS_GRAPH_ACCESS_TOKEN` for Entra, and the OpenShell L7 proxy substitutes the current access token only for admitted HTTPS requests.
...
Middleware is optional for runtime access-token refresh and presentation.
Use it only when the deployment has a compatible middleware installed for additional controls, such as identity assertions or on-behalf-of exchange.
