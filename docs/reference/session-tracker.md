...
title:
  ...
  nav: "Session Tracker"
description:
  main: "Reference for the behavioral session tracker that detects multi-step exfiltration attacks by tracking three capability classes per agent session."
  agent: "Session tracker API reference covering trifecta detection (read_sensitive + ingested_untrusted + has_egress), risk classification, event storage, and exposure queries. Use when looking up session tracking behavior, capability classes, or the SessionStore API."
...
The session tracker hooks into the OpenClaw plugin lifecycle automatically when NemoClaw registers.
The integration module (`nemoclaw/src/security/session-integration.ts`) wires a `SessionStore` into two plugin hooks.

The `before_tool_call` hook classifies each tool invocation into a capability class.

| Capability | Tools matched | Condition |
|---|---|---|
| `read_sensitive` | `read_file`, `cat`, `head`, `tail`, `str_replace_editor`, `view_file` | Path argument matches a sensitive pattern (`/etc/passwd`, `.env`, `credentials`, `secret`, `token`, `.pem`, `.key`, `id_rsa`) |
| `ingested_untrusted` | `fetch`, `curl`, `wget`, `http_get`, `browser_navigate`, `web_fetch` | Always recorded when tool is invoked |
| `has_egress` | `http_post`, `http_put`, `send_email`, `send_message`, `upload_file` | Always recorded when tool is invoked |

The `session_end` hook removes the session from the store to prevent unbounded memory growth.

When a trifecta is first detected the plugin logs a warning through the host logger.
Callers who need a different response can instantiate their own `SessionStore` with a custom `onTrifecta` callback.

The classification rules are intentionally conservative.
False negatives are acceptable because the tracker is a detection aid, not a policy gate.
The NemoClaw team can extend the tool lists and path patterns as the agent tool catalog evolves.
...
The integration module calls this on `session_end` to clean up individual sessions.
