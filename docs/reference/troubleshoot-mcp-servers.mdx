...
If `mcp add` rejects a `http://` or local URL and requires HTTPS, this is the managed registration boundary, not a Hermes-specific limitation.
Managed `mcp add` enforces HTTPS, public DNS endpoints, and pinned egress policy the same way for OpenClaw, Hermes, and Deep Agents Code so OpenShell forwards the bearer credential over TLS.
...
For local development, expose the MCP server over HTTPS on a public DNS endpoint, then register it with `mcp add`.
