...
Restart validates and reloads the current provider-backed registration; it never consumes an exported replacement credential.
To replace the credential, follow the conservative remove and re-add procedure:

```bash
export <CREDENTIAL_ENV>='replacement-value'
$$nemoclaw <sandbox> mcp remove <server>
# Confirm no sandbox uses the retained provider named by remove, then delete it.
openshell provider delete <PROVIDER_NAME>
$$nemoclaw <sandbox> mcp add <server> --url <MCP_URL> --env <CREDENTIAL_ENV>
unset <CREDENTIAL_ENV>
```

Preserve the original denied-tool selectors and `--trusted-private-host` declaration on the re-add.
See [Rotate a Credential](../manage-sandboxes/mcp-servers/manage-mcp-servers#rotate-a-credential) for the complete provider-cleanup sequence.
...
If restart reports a missing provider, remove the native registration and re-add it with the same URL, credential environment name, denied-tool selectors, and trusted-private declaration. Restart does not recreate providers.
...
## Rebuild Cannot Prove a Credential Handle

If rebuild reports that it could not prove a credential handle, a fresh sandbox process did not expose the exact OpenShell credential identity required for adapter cleanup.
...
Export the replacement under the same environment-variable name, then remove and re-add the server so a fresh provider binding and agent placeholder are created:

```bash
export <CREDENTIAL_ENV>='replacement-value'
$$nemoclaw <sandbox> mcp remove <server>
$$nemoclaw <sandbox> mcp add <server> --url <HTTPS_MCP_URL> --env <CREDENTIAL_ENV>
unset <CREDENTIAL_ENV>
```

Run status again.
OpenShell can retain the same `s…` stable handle after a provider update; verify endpoint authorization with the named `mcp status` probe.
Retry rebuild only after `provider.credentialReady` and `adapter.registered` are both `true`.
If the re-add still cannot prove the credential handle, repair the reported OpenShell provider, policy, or adapter condition before retrying teardown.

<AgentOnly variant="hermes">

## Hermes Add Outcome Is Unknown

If Hermes reports an unknown outcome after losing its OpenShell exec relay, do not rerun `mcp add`.
Do not force-remove or remove and re-add the server.
NemoClaw proved neither committed state nor absence, so another mutation can conflict with the in-flight or committed transaction.
...
Preserve the reported diagnostic.
Repair any reported sandbox readiness, gateway health, provider, policy, attachment, or adapter problem before another MCP lifecycle command.
Keep the registration unchanged while status remains unavailable, conflicting, or incomplete.
</AgentOnly>

## Add Transaction Is Incomplete

For an interrupted `mcp add` that does not report an unknown outcome, inspect the native source and live status first.
If no native registration exists, rerun the original command with the same URL, environment-variable name, and any `--deny-tool` options.
If the native registration exists, remove it and then re-add it after re-exporting the credential.

To abandon the transaction, run `mcp remove <server> --force`.
NemoClaw disables the native agent entry first and cleans only live resources whose ownership it can prove. It preserves the workspace provider and reports any residual policy or attachment cleanup.

## Agent MCP Capability Is Missing

If add or restart reports that the OpenClaw native writer, Hermes transaction helper, or Deep Agents native MCP capability v3 is unavailable, rebuild the sandbox onto a current image before retrying.
When an older OpenClaw or Deep Agents sandbox has a legacy MCP file, preview `mcp migrate`, then use `mcp migrate --apply` when you intend to convert it. To delete one named legacy entry instead, run `mcp remove <server>`; NemoClaw removes it only when a matching committed registry row proves ownership and otherwise preserves it. A native definition that conflicts with the legacy definition stops migration before mutation.

<AgentOnly variant="deepagents">
Migration carries the legacy `.deepagents/.nemoclaw-mcp.json` entries through a normal rebuild into the native `/sandbox/.deepagents/.mcp.json` file.
Preview and apply the migration, then retry the managed MCP command:

```bash
nemo-deepagents <name> mcp migrate
nemo-deepagents <name> mcp migrate --apply
nemo-deepagents <name> mcp add <server-name> --url <url> --env <credential-env>
```

NemoClaw preserves unrelated entries in `/sandbox/.deepagents/.mcp.json` and mutates only the named managed server.
Use `nemo-deepagents <name> mcp ...` for servers that need OpenShell credential replacement and egress policy.
</AgentOnly>

<AgentOnly variant="hermes">
If Hermes reports that reconcile-finality capability version 1 is unavailable, the MCP mutation did not start.
Rebuild the sandbox onto a current image, then retry the original MCP command:

```bash
$$nemoclaw <sandbox> rebuild
```

Rebuild uses the older helper only to remove exact managed entries from the retiring sandbox.
The replacement helper must report capability version 1 before it restores those entries.
...
Read recent sandbox logs while you reproduce an MCP failure:
...
The command reads agent gateway output and OpenShell audit events, so policy denials appear beside gateway logs.
</AgentOnly>
<AgentOnly variant="deepagents">
The command reads OpenShell logs, including audit events, so policy denials appear with other OpenShell log entries.
</AgentOnly>
If NemoClaw cannot enable OpenShell audit logs, it prints a warning and the policy denial events can be missing.
...
An agent-native registration path, such as editing OpenClaw `openclaw.json` inside the sandbox, may accept a plain-HTTP or local URL, but it bypasses NemoClaw credential replacement and generated egress policy.
...
If `mcp status <server>` reports `public address pins: drift`, refresh only its live policy pins:

```bash
$$nemoclaw <sandbox> mcp update <server> --refresh-public-pins
$$nemoclaw <sandbox> mcp status <server> --tools
```

The refresh preserves all other policy fields, including denied tools, and retains the existing agent configuration and credentials.
Successful tool discovery verifies recovery.
Status, restart, rebuild, and restore preserve the live pins.
If refresh cannot confirm a write, inspect status and the live OpenShell policy before retrying.

If status reports `public address pins: rejected`, DNS resolved to a private or special-use address.
Investigate the destination change; refresh does not approve that address.

If status reports `public address pins: unresolved`, DNS is unavailable and the recorded pins remain unchanged.
Retry status after DNS recovers.

If status reports `private address pins: drift`, review the endpoint address change before updating access.
Status, restart, rebuild, and restore do not add new trusted-private addresses.
...
Cloudflare Quick Tunnels and named tunnels can rotate public edge addresses while keeping one hostname.
A managed MCP server can require another explicit pin refresh after each public address change.
