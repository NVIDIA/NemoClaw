...
NemoClaw rejects another value before sandbox creation.
...
Advance from `3000` to `5000`, and then to `10000`, only when the logs report a `tools/list` timeout.
Stop at the lowest value that completes every planned run without a `tools/list` timeout.
