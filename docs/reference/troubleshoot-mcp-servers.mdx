...
If the retry also fails, the sandbox log records a temporary MCP transport failure and states that credentials and configuration were not rejected.
...
A turn in which any server failed to start does not become the session's cached tool catalog.
NemoClaw contacts that server again on the next agent turn, so a brief upstream outage clears on its own and a repaired credential or policy takes effect without a new session.
A turn in which every server started is cached and reused as usual.
</AgentOnly>
