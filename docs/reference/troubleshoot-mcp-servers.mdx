...
When enabled, each successful request attempts to emit a `managed_transport_shadow` line group with the same safe operation, transport, timeout, and elapsed-time fields.
