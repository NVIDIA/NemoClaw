...
If `mcp add` is interrupted, inspect the native source and live status first. If no native registration exists, rerun the original command with the same URL, environment-variable name, and any `--deny-tool` options. If the native registration exists, remove it and then re-add it after re-exporting the credential.
...
When an older OpenClaw or Deep Agents sandbox has a legacy MCP file, preview `mcp migrate`, then use `mcp migrate --apply` to convert it. A native definition that conflicts with the legacy definition stops migration before mutation.
...
If `mcp status <server>` reports `public address pins: drift`, restart the server to validate, apply, and record the current public answers:
...
A managed MCP server can require another restart after each public address change.
