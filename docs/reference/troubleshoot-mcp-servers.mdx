...
When that request fails inside a sandbox, OpenClaw writes one `managed_transport_failure` line group to the sandbox log:
...
The sampler waits at most 250 ms, reads at most 2,048 response bytes, and emits at most 2,048 UTF-8 bytes after redaction.
...
`trace_id` is a local identifier created for each diagnostic.
It does not appear in OpenShell audit events, so correlation is by endpoint and time until OpenShell records a shared identifier.
</AgentOnly>
