...
When a remote Streamable HTTP MCP request fails inside a sandbox, NemoClaw writes one `managed_transport_failure` line group to the sandbox log:
...
[nemoclaw] operation=docs
[nemoclaw] route=trusted_env_proxy
[nemoclaw] proxy=127.0.0.1:3128
[nemoclaw] target=mcp.example.com:443
[nemoclaw] phase=response_headers
[nemoclaw] http_status=503
[nemoclaw] elapsed_ms=1512
[nemoclaw] server=envoy
[nemoclaw] x-envoy-response-flags=UF,URX
[nemoclaw] session_present=true
[nemoclaw] trace_id=<generated-id>
```

Read `phase` first. It states which step failed:
...
| `response_stream` | The response body failed after streaming began |

`route` states whether the managed proxy was used.
`server`, `via`, `x-request-id`, and the `x-envoy-*` fields are the proxy's own account of the failure and appear only when the proxy sends them.
...
Successful and streaming responses are unaffected.
...
`trace_id` identifies the application-side failure but does not yet appear in OpenShell audit events, so correlation is by endpoint and time until OpenShell records a shared identifier.
</AgentOnly>
