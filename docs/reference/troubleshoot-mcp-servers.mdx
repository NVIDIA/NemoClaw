...
<Warning>
Enabling shadow diagnostics records one timing event for every successful remote Streamable HTTP MCP request and increases log volume.
Each event includes the endpoint, configured server name, operation, and timeout metadata, but excludes tool arguments and successful response bodies.
...
When enabled, each successful request emits a `managed_transport_shadow` line group with the same safe operation, transport, timeout, and elapsed-time fields.
The diagnostic does not read the successful response body or change the returned response.

For `tools/list`, NemoClaw retains up to 64 successful elapsed-time samples per endpoint in the OpenClaw process.
...
NemoClaw creates it before the request starts, but does not add it to the request.
...
Cloudflare Quick Tunnels rotate public edge addresses and are not durable for exact address pinning.
