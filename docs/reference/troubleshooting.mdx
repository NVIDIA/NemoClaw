...
1. Check the managed OpenShell gateway service.
   ...
   If you set `NEMOCLAW_GATEWAY_PORT` to a custom port, skip this service check and continue with the NemoClaw recovery step using the same environment value.
   Only the default port `8080` uses a managed service.

   On Apple Silicon macOS with Homebrew, OpenShell uses the official formula and the gateway is a Homebrew service:
   ...
   Without Homebrew, continue with the NemoClaw recovery step below for the standalone gateway fallback.

   On Linux package installs, OpenShell can provide an upstream user service:
   ...
   On Linux tarball installs, NemoClaw installs a marked user `nemoclaw-openshell-gateway.service`.
   ...
   The systemd user manager starts it with your user session.
   NemoClaw does not enable lingering.
