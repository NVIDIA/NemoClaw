...
On WSL 2, NemoClaw automatically uses a temporary credential-free Docker configuration for public managed-image and probe pulls on the default Docker context when this helper is unavailable.
It removes the temporary configuration after the operation and does not modify your Docker configuration.
NemoClaw keeps your configured credentials for a non-default Docker context and an explicit custom Dockerfile because those paths can require a private registry.

If an explicit custom Dockerfile cannot pull its base image, restore the Docker Desktop session or credential-helper access before retrying.
Do not remove the `credsStore` entry unless you understand the effect on registries that use stored credentials.
