...
It also keeps eligible Jetson GPU device group memberships when OpenShell starts the nonroot sandbox user.
The compatibility policy permits read-only access to the NVIDIA runtime library directory at `/opt/nvidia/l4t-gpu-libs` when that path is present.
