...
If `$$nemoclaw uninstall` reports that the local uninstall script is missing, copy the versioned `uninstall.sh` URL that it prints, review it, run it, and then retry the installer:

```bash
curl -fsSL <versioned-uninstall-url> | bash
curl -fsSL https://www.nvidia.com/nemoclaw.sh | bash
```
