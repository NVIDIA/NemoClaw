...
If the listener, selected-state proof, or service identity does not match, NemoClaw prints no stop command and uses the standalone ownership checks.
When `OPENSHELL_DB_URL` is present, it proves selected-state ownership only when it matches the selected database URL, even if the namespace value differs.
