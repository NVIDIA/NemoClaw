...
The default installer and an explicit `NEMOCLAW_INSTALL_TAG=lkg` selection stop before replacing a newer installed NemoClaw release. Other explicit `NEMOCLAW_INSTALL_TAG` or `NEMOCLAW_INSTALL_REF` values can replace the installed release; use a version tag or full commit SHA when you need a fixed source.
