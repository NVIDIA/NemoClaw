...
On macOS, the port check also tries a privileged `lsof` probe without prompting for a password so root-owned listeners are detected before the sandbox build starts. For a new sandbox, NemoClaw reserves the selected loopback port through sandbox preparation and the image build. If another listener claims the port before NemoClaw binds the reservation, NemoClaw selects another port before changing sandbox resources. NemoClaw releases the reservation immediately before it starts the managed OpenShell service forward. If forwarding then fails, onboarding removes the new sandbox and tells you to resolve the reported error before retrying.
...
When the `gateway` check passed and only the `api` check failed, the API remains reachable inside the sandbox. Restore the managed host forward through NemoClaw:
...
An HTTP status of `200` or `401` means the managed host forward is reachable. Any other status, `000`, or connection failure requires fresh diagnostics. Run `$$nemoclaw <name> status`, then follow the reported gateway or forward diagnostic. Do not create a separate `openshell forward start` process for a NemoClaw-managed port.
