...
For OpenClaw direct GPU onboarding, the creation-time filesystem policy adds no Jetson entries unless `/dev/nvmap` is an existing, non-symlink character device.
