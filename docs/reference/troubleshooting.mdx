...
If native sandbox creation, readiness, or GPU verification reports a hard failure, NemoClaw captures redacted diagnostics, deletes the incomplete sandbox, verifies that no OpenShell-labeled Docker container remains, and retries exactly once through the compatibility path.
Before retrying, cleanup must pass two consecutive checks that the sandbox is absent from the OpenShell gateway list and that no Docker container with its OpenShell sandbox label remains.
