...
For `nemohermes <name> gateway restart`, NemoClaw runs that secret-boundary check as a non-mutating preflight before it asks the sandbox's recorded OpenShell gateway to stop and start the complete sandbox. It revalidates the registered and live sandbox identity around the lifecycle steps, then requires native gateway health before repairing host forwards. If the identity changes or OpenShell cannot safely complete either lifecycle step, the command exits nonzero and does not continue to health or forward recovery. Do not retry against a same-named replacement; inspect the recorded gateway and sandbox identity first.
