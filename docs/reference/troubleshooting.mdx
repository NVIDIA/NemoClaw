...
Do not rely on direct edits to `openclaw.json` for a provider change. A snapshot-backed rebuild can preserve the file edit, but the edit does not update the OpenShell route, host-managed credentials, or registry metadata. Use onboarding or a supported host-side inference command to keep those resources aligned.
...
Agent-local channel commands do not update NemoClaw's messaging plan, OpenShell providers, or network policy, so NemoClaw intercepts `openclaw channels` commands that mutate the config.
