...
The portable experimental profile requires the current user's systemd hierarchy to expose the `cpu` cgroup controller to `app.slice`.
NemoClaw checks this requirement before it writes portable configuration, activates services, starts the registry, builds an image, or creates a sandbox.
The credential-free preflight reads only `cgroup.controllers` files under `/sys/fs/cgroup`.

The preflight distinguishes missing, unreadable, malformed, and readable controller evidence.
Classify the file state before you select a recovery action:

```bash
uid="$(id -u)"
user_slice="/sys/fs/cgroup/user.slice/user-${uid}.slice"
user_manager="${user_slice}/user@${uid}.service"
...
An access policy can prevent the current user from testing whether a path exists.
For each `missing` or `read failed` result, ask an administrator to run `sudo stat -- <reported-path>`.
If `stat` finds the file but the current-user read failed, classify the file as unreadable.
If `stat` reports that the path does not exist, classify the file as missing.
An empty readable file is valid evidence that exposes no controllers.
A `malformed` result means the successful read contained a duplicate controller name or a token outside the lowercase controller-name grammar.

Do not print malformed bytes directly to a terminal.
Ask an administrator to inspect only the reported path and its cgroups v2 mount:
...
sudo od -An -tx1 -v -- "$reported_path"
findmnt --target "$reported_path" --output TARGET,FSTYPE,OPTIONS
```

The administrator must correct the cgroups v2 mount or kernel-provided evidence before you rerun the classification command.
Do not change systemd delegation, stop or start the user manager, or reboot the host to repair malformed evidence.
...
| Any controller file is malformed | The file was readable, but it did not contain unique, whitespace-separated lowercase controller names. | Stop before changing delegation or user-manager state. Ask an administrator to inspect the exact file and cgroups v2 mount integrity with the commands above. |
...
| `cpu` is unavailable to `user-UID.slice`; its file is missing or does not contain `cpu` | The system manager has not activated the controller at the affected user's ancestor slice. | Apply all three named systemd drop-ins below, then follow the interruption-aware activation procedure. |
| systemd did not delegate `cpu`; the user-manager file is missing or does not contain `cpu` | The current user's manager does not receive the `cpu` controller. | Apply all three named systemd drop-ins below, then follow the interruption-aware activation procedure. |
| `cpu` is unavailable to `app.slice`; its file is missing | The current user's `app.slice` hierarchy does not exist for this boot. | Apply all three named systemd drop-ins below, then follow the interruption-aware activation procedure. |
| `cpu` is unavailable to `app.slice`; its file is readable but does not contain `cpu` | The user manager has not activated the delegated controller for `app.slice`. | Apply all three named systemd drop-ins below, then follow the interruption-aware activation procedure. |

Do not use a boot, delegation, or service lifecycle action to correct an unreadable file.
Those actions do not restore access that filesystem permissions or an LSM policy denies.

The systemd changes require administrator access.
NemoClaw does not edit `/etc/systemd`, invoke `sudo`, remove the sandbox CPU limit, or continue with weaker resource isolation.

<Warning>
The `user@.service` template applies to every user manager on the host.
The `user-UID.slice` drop-in applies only to the affected numeric user ID.
The `app.slice` drop-in applies to every user manager on the host.
Applying or removing these drop-ins requires an interruption-aware lifecycle procedure later in this section.
Read its warning before you stop or start a user manager or reboot the host.
</Warning>
...
Use the three dedicated NemoClaw drop-in paths below.
The service drop-in delegates `cpu` to user managers.
The `user-UID.slice` drop-in requests the kernel default CPU weight of `100` at the affected user's system-manager ancestor.
The `app.slice` drop-in requests the kernel default CPU weight of `100`.
The two explicit weight settings activate the controller above and inside the user manager without changing its relative default weight.
If any file exists, stop and ask the administrator to inspect its ownership and content.
Do not replace any file.

```bash
uid="$(id -u)"
if [[ ! "$uid" =~ ^[0-9]+$ ]]; then
  printf 'Refusing invalid numeric user ID: %s\n' "$uid" >&2
  exit 1
fi
...
for drop_in_name in delegation user_slice app_slice; do
  printf 'Record for rollback: %s_drop_in_created=0\n' "$drop_in_name"
  printf 'Record for rollback: %s_drop_in_dir_created=0\n' "$drop_in_name"
done

for drop_in in "$delegation_drop_in" "$user_slice_drop_in" "$app_slice_drop_in"; do
...
create_drop_in() {
  ...
  if ! drop_in_tmp="$(sudo mktemp "${drop_in_dir}/.nemoclaw-cpu-controller.XXXXXX")"; then
    printf 'CPU controller drop-in temporary file creation failed: %s\n' "$drop_in" >&2
    return 1
  fi

  if ! printf '%s\n' "$@" | sudo dd of="$drop_in_tmp" status=none \
    || ! sudo chown root:root -- "$drop_in_tmp" \
    || ! sudo chmod 0644 -- "$drop_in_tmp"; then
    printf 'CPU controller drop-in creation failed: %s\n' "$drop_in" >&2
    if ! sudo rm -f -- "$drop_in_tmp"; then
      printf 'CPU controller drop-in temporary file removal failed: %s\n' "$drop_in_tmp" >&2
    fi
    return 1
  fi

  if ! drop_in_id="$(sudo stat -Lc '%d:%i' -- "$drop_in_tmp")" \
    || ! sudo ln -T -- "$drop_in_tmp" "$drop_in"; then
    printf 'CPU controller drop-in creation failed: %s\n' "$drop_in" >&2
    if ! sudo rm -f -- "$drop_in_tmp"; then
      printf 'CPU controller drop-in temporary file removal failed: %s\n' "$drop_in_tmp" >&2
    fi
...
If a creation command fails, do not reload systemd.
Use the final value printed for every `*_created` record and its matching `device:inode` record.
The initial `0` records make cleanup executable even when only the first file was published.
Ask the administrator to inspect all three drop-in paths and their directories.
Remove a `.nemoclaw-cpu-controller.*` temporary path only when the failed command reported that exact path and the administrator verifies its identity.

#### Clean Up a Partial Drop-In Creation

Run this command only when creation failed before any `systemctl daemon-reload`.
Copy the final creation records into every placeholder.
The command removes only objects recorded as created by this procedure and bound to their creation-time identity.
It preserves pre-existing objects, accepts recorded objects that are already absent, refuses identity or type drift, and attempts every cleanup action before it exits.
...
remove_created_drop_in() {
  ...
  if sudo test -L "$drop_in" || ! sudo test -f "$drop_in"; then
    printf 'Refusing unexpected CPU controller drop-in type: %s\n' "$drop_in" >&2
    return 1
  fi
  if [ "$(sudo stat -Lc '%d:%i' -- "$drop_in")" != "$expected_id" ]; then
...
remove_created_directory() {
  ...
  if ! sudo test -e "$drop_in_dir" && ! sudo test -L "$drop_in_dir"; then
    return 0
  fi
  if sudo test -L "$drop_in_dir" || ! sudo test -d "$drop_in_dir"; then
    printf 'Refusing unexpected CPU controller drop-in directory type: %s\n' \
      "$drop_in_dir" >&2
    return 1
  fi
  if [ "$(sudo stat -Lc '%d:%i' -- "$drop_in_dir")" != "$expected_id" ]; then
...
If the command exits nonzero, inspect every refusal and preserve the affected path.
After correcting a transient cleanup failure, rerun the complete command with the same final creation records.
Do not reload systemd after partial-creation cleanup.
...
sudo stat -Lc '%n %d:%i' -- \
  "$delegation_drop_in" "$user_slice_drop_in" "$app_slice_drop_in"
sudo cat -- "$delegation_drop_in"
sudo cat -- "$user_slice_drop_in"
sudo cat -- "$app_slice_drop_in"
```

Continue only when the command exits with status `0` and all three `device:inode` values match the creation command's rollback records.
The service file content must be:
...
Record all rollback lines from the creation command in the administrator's change record, including all three file creation records and all three directory creation records.
They identify the exact files and any directories created by this procedure and contain no credentials.
Rollback must use those recorded values.
If a required value is lost, do not use the removal command below; ask the administrator to inspect and remove the configuration through the host's normal change-management process.

The administrator's `sudo` policy can request authentication.
NemoClaw never receives that credential.

<Warning>
Before the next command, the affected user must save their work.
The administrator must identify and account for that user's host workloads, including rootless Podman containers and other user services.
Stopping the manager interrupts all services owned by that user.
Run the command from an independent administrator session that does not depend on the affected user's manager.
</Warning>
...
The stop must complete before `daemon-reload` runs.
Reloading while the instantiated unit is inactive lets systemd recalculate ancestor controller masks for the new delegation.
The final start makes the system manager apply the per-user slice weight and makes the user manager read the `app.slice` weight.
Do not replace this sequence with `systemctl restart`.

The immediate `systemctl start` can fail with `status=219/CGROUP` even after the inactive reload.
When the status or journal output reports that result, leave the three verified drop-ins in place and do not repeatedly start or restart the manager.
Have the affected user create a fresh login session later so logind can create a new user manager.
From the independent administrator session, verify that the later login recovered it:
...
Continue only when the command prints `active`.
After the affected user signs in through the recovered manager, start `app.slice` in that user's session:

```bash
systemctl --user start app.slice
```

If a later fresh login does not recover the manager, an administrator can use a reboot as the final recovery step.

<Warning>
Before rebooting, every host user must save their work, and the administrator must safely stop or account for all host workloads.
A reboot interrupts every service, container, and interactive session on the host.
</Warning>
...
After the host returns, have the affected user sign in and confirm that `user@UID.service` is active before starting `app.slice`.

Verify that systemd loaded all three NemoClaw drop-ins:
...
Continue only when the effective unit views list the NemoClaw service, per-user slice, and `app.slice` drop-in paths created above.

Verify the root hierarchy, affected user's ancestor slice, current user manager, and `app.slice`:
...
Continue only when all four lines end in `: cpu`.
This verification reads no credentials.
Then rerun portable onboarding:
...
Remove only the three named files and any drop-in directory that the creation record marks as created by this procedure.
Do not use `systemctl revert`.
Do not remove another `user@.service`, `user-UID.slice`, or `app.slice` drop-in or a pre-existing drop-in directory.

Confirm all three files and retrieve the `device:inode` values from the administrator's creation-time change record:
...
Continue only when all three paths, their content, and their current `device:inode` values match the creation-time record.
The removal command is retry-safe: an already absent recorded path is accepted, an identity mismatch is refused, and reload/start are attempted after every removal attempt so a partial failure does not leave the manager stopped.
...
remove_recorded_directory_if_created() {
  drop_in_dir="$1"
  created="$2"
  expected_id="$3"
...
remove_recorded_drop_in "$delegation_drop_in" "$expected_delegation_drop_in_id" ||
  cleanup_failed=1
remove_recorded_drop_in "$user_slice_drop_in" "$expected_user_slice_drop_in_id" ||
  cleanup_failed=1
remove_recorded_drop_in "$app_slice_drop_in" "$expected_app_slice_drop_in_id" ||
  cleanup_failed=1
remove_recorded_directory_if_created \
  "$(dirname "$delegation_drop_in")" \
  "$delegation_drop_in_dir_created" \
...
The inactive reload removes all three NemoClaw drop-ins from the instantiated units and recalculates ancestor controller masks before the manager starts.
Do not replace this sequence with `systemctl restart`.
If the command exits nonzero, inspect every reported failure.
After correcting it, rerun the complete removal command with the same creation-time identities; paths already removed by the earlier attempt are accepted.

The immediate rollback `systemctl start` can also fail with `status=219/CGROUP`.
The rollback command still attempts `daemon-reload` and `start` after every file-removal attempt.
When the status or journal output reports `219/CGROUP`, do not restore the removed files and do not repeatedly start or restart the manager.
Have the affected user create a fresh login session later, then verify from the independent administrator session that `user@UID.service` is active.

If a later fresh login does not recover the manager, an administrator can reboot as the final rollback recovery step.
...
After the affected user signs in through the recovered manager, verify that systemd no longer loads any NemoClaw drop-in:
...
The output must not list any of the three NemoClaw drop-ins.
Another administrator-owned drop-in can still configure `cpu`; do not remove it as part of this rollback.
Removing the NemoClaw drop-ins can make the portable CPU delegation preflight fail again.
