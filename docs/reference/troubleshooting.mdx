...
The completion output prints the selected value.
Run the printed export command in every later terminal session that targets this gateway.
...
Later `list`, `status`, `connect`, `stop`, `rebuild`, and `uninstall` commands use the state root selected by this variable.
A command without the export uses port `8080` instead.
