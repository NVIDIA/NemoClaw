...
Docker passes those group IDs through `--group-add`.
...
Before the replacement starts the OpenShell supervisor, NemoClaw runs a root-owned helper from the OpenClaw sandbox image.
...
Use `NEMOCLAW_SANDBOX_GPU=0` for CPU-only sandbox behavior while [issue #7610](https://github.com/NVIDIA/NemoClaw/issues/7610) remains open.
