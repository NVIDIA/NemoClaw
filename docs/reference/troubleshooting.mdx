...
If the global npm prefix is not writable, it writes a user-local shim to `~/.local/bin/nemoclaw` instead.
Installer verification checks this user-local shim independently of the active npm prefix.
If the shim is valid but your current shell does not resolve it, installation continues using the shim's absolute path and prints commands to refresh your `PATH`.
