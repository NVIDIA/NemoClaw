...
Do not copy, recreate, or edit a registry entry or lifecycle receipt by hand. If `doctor` reports that it cannot qualify the authority, preserve both files unchanged and stop. Manual edits can bind recovery to stale or replacement state.
