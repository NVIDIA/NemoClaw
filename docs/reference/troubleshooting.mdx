...
Destroying the final sandbox with `--cleanup-gateway` first tries to stop the packaged OpenShell gateway service, then reaps only host gateway processes whose ownership NemoClaw verifies. This does not prove that the port is free: a foreign, ambiguous, or other-user listener may remain. Verify the gateway port after cleanup. The service is stopped, not disabled or removed, and the next onboarding run starts it again. If the service cannot be stopped, `destroy` exits non-zero and prints the status command for the service.
