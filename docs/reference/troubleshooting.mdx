...
Do not delete a failed sandbox by mutable name. Follow the retained-sandbox recovery output and require NemoClaw to validate the durable sandbox identity before removal. If NemoClaw retains an exact validated replacement-container ID and prints a `docker rm -f <container-id>` command, use only that exact command after confirming that the pre-patch sandbox was not restored. Fix the NVIDIA Container Toolkit or CDI configuration reported in the diagnostics, then rerun onboarding. If you do not need GPU access inside the sandbox, rerun with `--no-sandbox-gpu`.
