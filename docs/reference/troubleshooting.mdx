...
When the Hermes config root is not locked, startup also stops when the `logs` directory tree contains a symbolic link, a hard-linked file, another unsafe entry, or changes during descriptor-anchored repair.
...
The gateway log names the reported state item and reports `Hermes pre-launch layout repair failed`.
