...
cleanup_retained_policy() {
  local retained_policy_dir retained_policy_basename policy_file
...
Do not resume or restart onboarding until `policy list` reports the live gateway policy.

If a policy mutation does not succeed, onboarding stops and:

- leaves earlier successful mutations applied and recorded;
- does not record the failed mutation in the sandbox registry;
- marks the session failed and resumable;
- reports one of the two results below.

After either result, run `policy list` before you resume or restart onboarding.
...
The failed mutation does not update the sandbox registry.
If the gateway applied an unconfirmed mutation, the affected preset appears as a disagreement.
...
Restore gateway access and run `policy list` again before you resume or restart onboarding.

After you know the live policy, follow [Previous onboarding session failed](#previous-onboarding-session-failed) to resume or restart onboarding.
