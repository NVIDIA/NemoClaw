...
### Local Dockerfile image push fails with out-of-memory errors

This failure applies only to explicit custom Dockerfile workflows.
Their locally built sandbox image can be approximately 2.4 GB compressed.
During local image push, the Docker daemon, k3s, and the OpenShell gateway run alongside the export pipeline, which buffers decompressed layers in memory.
On machines with less than 8 GB of RAM, this combined usage can trigger the OOM killer.
Stock managed-image onboarding pulls an exact immutable image and does not run this local export pipeline.
...
Install both packages, start Colima with enough resources for the sandbox and any explicit custom Dockerfile build, and verify Docker before onboarding:
...
Some corporate networks block outbound UDP port 53 to public DNS servers and force all host name resolution through DNS over TLS on TCP port 853.
Containers do not inherit the host's DNS-over-TLS configuration.
This can block sandbox startup and, on an explicit custom Dockerfile path, make dependency installation time out while resolving `registry.npmjs.org`.

NemoClaw's preflight runs a short `docker run --rm busybox nslookup nemoclaw-dns-probe-<random>.invalid` probe before it prepares the sandbox.
The fresh `.invalid` name should return NXDOMAIN through a working resolver, so cached answers cannot hide blocked DNS egress.
When the probe confirms a DNS failure, onboarding stops with platform-specific remediation before sandbox creation.
...
On macOS, the port check also tries a privileged `lsof` probe without prompting for a password so root-owned listeners are detected before sandbox creation starts.
...
Use Docker Desktop, Colima, or Docker Engine and rerun onboarding.
...
### Explicit custom Dockerfile build fails during OpenClaw plugin install

Complete managed release images already contain the reviewed supported-plugin dependency union and do not install those packages during stock onboarding.
An explicitly selected custom Dockerfile can install managed plugins for selected features such as web search or diagnostics.
...
Rerunning onboarding with a different provider recreates the sandbox because provider configuration and credential attachment are sandbox-creation inputs.
Managed images receive the provider configuration through a secret-free startup profile, while a local Dockerfile path applies the same configuration during its build.
...
### Sandbox startup or local Dockerfile build is slow (under-provisioned container runtime)

Default Colima ships with 2 vCPU and 2 GiB of memory, which is not enough headroom for a NemoClaw sandbox.
Stock onboarding on a supported `amd64` or `arm64` host requires an exact managed image and avoids a local Dockerfile build.
An explicit custom Dockerfile can still stall part-way through a local BuildKit build on an under-provisioned runtime.

Preflight inspects `docker info` for `NCPU` and `MemTotal` and prints a warning when the runtime falls below 4 vCPU or 8 GiB.
In interactive onboarding, the warning prompt defaults to abort, so pressing Enter stops the run before sandbox preparation reaches the likely failure point.
...
The configuration review shows a local build-time estimate only for an explicit custom Dockerfile.
Managed-image launch does not show a Dockerfile build estimate.
...
On systems with 8 GB RAM or less and no swap configured, managed-image pull and startup or an explicit custom Dockerfile build and upload can exhaust available memory and get killed by the Linux OOM killer (exit code 137).
...
### Managed bootstrap quiesced and retained a sandbox

For a managed release image on Docker, NemoClaw binds bootstrap cleanup to the exact OpenShell sandbox ID and full Docker runtime ID.
OpenShell deletion currently accepts a mutable sandbox name rather than an immutable ID.
If NemoClaw cannot prove that the name still identifies the same sandbox, it stops the exact Docker runtime and retains it instead of risking deletion of a concurrent same-name replacement.

The error includes the sandbox name, immutable sandbox ID, and Docker runtime ID.
Do not run the printed name-only delete until you confirm that no same-name recreation is in progress and `openshell sandbox get <name>` still reports the exact sandbox ID from the error.
Inspect the exact Docker runtime ID from the error and confirm that it is stopped.

```bash
openshell sandbox get <name>
docker inspect <runtime-id> --format '{{json .State}}'
```

If either identity differs or cannot be read, preserve the retained runtime and resolve the ownership conflict before continuing.
After the identities match and no concurrent recreation is active, coordinate the name-only removal and then resume onboarding:
...
The retained runtime protects rollback evidence and shared-state commit receipts.
Do not remove its Docker container directly unless the OpenShell owner has already been reconciled and you have separately preserved any required recovery evidence.
...
If the import does not occur, check onboarding output for a warning that the selected source was skipped, then inspect sandbox startup logs for the managed runtime merge result.
An explicit custom Dockerfile build can also log `baking corporate proxy CA from ...`.
...
NemoClaw ships `openclaw` in the immutable managed release image, so the CLI is image-pinned rather than updated in place inside a running sandbox.
...
The following cleanup message reports either that exact-owner cleanup succeeded or that NemoClaw quiesced and retained the exact runtime because name-only deletion was unsafe.
...
- The host or gateway is pulling and starting the managed image for the first time, or building and uploading an explicitly selected custom Dockerfile (cold caches, slow link).
- The selected model is large (70B+ parameters or 4-bit/8-bit quantisations that take time to memory-map).
- Onboarding runs on a remote VM where a managed image pull or explicit custom-image upload crosses a slow link (for example DGX Station first-run installer).
...
opt in.
If you onboarded an older release that selected `/v1/responses`, rerun onboarding so the replacement sandbox starts with Chat Completions:
...
It patches the running config but does not update the recorded startup configuration.
A fresh `$$nemoclaw onboard` regenerates that configuration for either the managed-image or local Dockerfile path.
...
NemoClaw-managed rebuilds can overwrite those edits, while onboarding keeps the immutable image, managed startup configuration, OpenShell route, and host-managed credentials aligned.
...
This is expected behavior.
`NEMOCLAW_DISABLE_DEVICE_AUTH` is a sandbox-creation setting.
Managed-image onboarding includes it in the secret-free startup profile, while an explicitly selected custom Dockerfile must apply it during the local build.
Changing or exporting it later does not rewrite `openclaw.json` inside an existing sandbox.

If you need a different device-auth setting, rerun onboarding so NemoClaw recreates the sandbox with the desired configuration.
...
This is expected.
The messaging channel list is materialized when the sandbox starts during `$$nemoclaw onboard` or `$$nemoclaw rebuild`.
Managed-image onboarding carries the selected channel names in the startup profile, while a local Dockerfile build uses its build arguments.
The startup path writes agent config such as `/sandbox/.openclaw/openclaw.json` for OpenClaw or `/sandbox/.hermes/.env` for Hermes.
...
Both offer to rebuild the sandbox so its startup configuration reflects the new channel set.
...
This is expected.
NemoClaw materializes the sandbox's OpenClaw configuration (`/sandbox/.openclaw/openclaw.json`) from host-side onboarding, rebuild, inference, policy, and messaging inputs.
...
NemoClaw materializes channel entries in `/sandbox/.openclaw/openclaw.json` when the sandbox starts.

As a result, commands that try to rewrite the managed config from inside the sandbox, including `openclaw doctor --fix`, cannot repair Discord, Telegram, or Slack channel entries in place.

If your Discord channel config is wrong, rerun onboarding so NemoClaw recreates the sandbox with the correct messaging selection.
...
Current NemoClaw rebuilds materialize Telegram in the account-based layout and set Telegram group chats to `groupPolicy: open`, which avoids the empty `groupAllowFrom` warning path for default group-chat access.
...
1. Managed config and provider wiring
...
1. Native Discord gateway path
   ...
   If logs show errors such as `getaddrinfo EAI_AGAIN gateway.discord.gg`, repeated reconnect loops, or a `400` response while probing the gateway path, the problem is usually in the native gateway/proxy path rather than in the managed config.
...
These are sandbox-startup settings.
Managed-image onboarding includes them in the startup profile, while a local Dockerfile path applies them through that build.
Changing them after onboarding requires rerunning `$$nemoclaw onboard` to recreate the sandbox.
...
### `k3s` cannot find an explicitly built custom image

After an explicit custom Dockerfile build, `k3s` inside the gateway container sometimes fails to pull the resulting image even though it exists on the host.
...
Managed-image resolution, pull, or startup failures; explicit custom Dockerfile build or upload failures; and TLS, provider, policy, dashboard, and inference failures stay on their existing error paths and do not trigger the GPU compatibility fallback.
...
Native Podman remains unsupported in this release; the native-runtime follow-up in issue #7744 owns that enablement.
Use Docker Engine, Docker Desktop, or Colima and rerun onboarding.
...
### Hermes reports that the managed-tool broker runtime changed

NemoClaw does not replace a healthy Hermes managed-tool broker when its executable hash changes.
The broker holds each sandbox's raw refresh credential only in process memory, so an automatic restart would simultaneously disconnect every managed-tool Hermes sandbox on the host.
The existing broker and its already registered sandboxes keep running; a new clone or reauthorization fails closed until the broker is recovered.

Use a planned reauthorization window:

1. Run `$$nemoclaw list --json` to enumerate the Hermes sandboxes, then record each one for which you selected any managed Nous tool during onboarding.
2. Stop the existing Hermes managed-tool broker through host process or service management. A host restart is the safe fallback when there is no process supervisor.
3. Run `nemohermes onboard --resume` for the first recorded sandbox and complete a fresh Nous Portal device-code authorization. This starts the current broker runtime.
4. Run `nemohermes onboard --resume` for every remaining recorded sandbox. Each authorization registers that sandbox's independent refresh credential with the same broker process.
5. Exercise one selected managed tool in every sandbox before ending the maintenance window.

For non-interactive recovery, export the matching sandbox's fresh `NEMOCLAW_HERMES_TOOL_GATEWAY_REFRESH_TOKEN` immediately before each `nemohermes onboard --resume --non-interactive` invocation, then unset it before moving to the next sandbox.
Do not reuse one sandbox's refresh credential for another.
