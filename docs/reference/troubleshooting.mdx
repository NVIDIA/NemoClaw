...
- `DOCKER_CONTEXT` selects the `default` context. When it is unset, `DOCKER_HOST` is unset and `docker context show` returns `default`. NemoClaw reads `DOCKER_HOST`, `DOCKER_CONTEXT`, and `DOCKER_CONFIG` from the environment of the `$$nemoclaw` command.
