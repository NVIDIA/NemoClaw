...
cleanup_retained_policy() {
  local platform_tmp_root platform_tmp_root_real retained_policy_dir retained_policy_real
  local retained_policy_basename retained_policy_parent retained_policy_expected_real policy_file
  ...
  if [[ -z $platform_tmp_root || $platform_tmp_root != /* ]]; then
    printf 'Validation failed for %q: the platform temporary directory is not absolute.\n' "$platform_tmp_root" >&2
    return 1
  fi
  if ! platform_tmp_root_real=$(node -e 'process.stdout.write(require("node:fs").realpathSync.native(process.argv[1]))' "$platform_tmp_root"); then
    printf 'Validation failed for %q: the platform temporary directory is not canonical.\n' "$platform_tmp_root" >&2
    return 1
  fi

  IFS= read -r -p 'Enter the exact temporary policy directory from the error: ' retained_policy_dir
  retained_policy_basename=${retained_policy_dir##*/}
  retained_policy_parent=${retained_policy_dir%/*}

  if [[ -z $retained_policy_dir || $retained_policy_basename != nemoclaw-policy-* || $retained_policy_basename == nemoclaw-policy- ]]; then
    printf 'Validation failed for %q: the basename must match nemoclaw-policy-* and include a suffix.\n' "$retained_policy_dir" >&2
    return 1
  fi
  if [[ $retained_policy_parent != "$platform_tmp_root" ]]; then
    printf 'Validation failed for %q: the path must be a direct child of the actual platform temporary directory %q.\n' "$retained_policy_dir" "$platform_tmp_root" >&2
    return 1
  fi
  if [[ ! -d $retained_policy_dir ]]; then
    printf 'Validation failed for %q: the path is not a directory.\n' "$retained_policy_dir" >&2
    return 1
  fi
  if [[ -L $retained_policy_dir ]]; then
    printf 'Validation failed for %q: the directory is a symbolic link.\n' "$retained_policy_dir" >&2
    return 1
  fi
  if [[ ! -O $retained_policy_dir ]]; then
    printf 'Validation failed for %q: the current user does not own the directory.\n' "$retained_policy_dir" >&2
    return 1
  fi
  if ! retained_policy_real=$(node -e 'process.stdout.write(require("node:fs").realpathSync.native(process.argv[1]))' "$retained_policy_dir"); then
    printf 'Validation failed for %q: the path cannot be resolved canonically.\n' "$retained_policy_dir" >&2
    return 1
  fi
  retained_policy_expected_real=$platform_tmp_root_real/$retained_policy_basename
  if [[ $retained_policy_real != "$retained_policy_expected_real" ]]; then
    printf 'Validation failed for %q: the canonical path is outside the actual platform temporary directory.\n' "$retained_policy_dir" >&2
    return 1
  fi

  policy_file=$retained_policy_dir/policy.yaml
  if [[ -e $policy_file || -L $policy_file ]]; then
    if ! rm -- "$policy_file"; then
      printf 'Cleanup failed for %q while removing policy.yaml.\n' "$retained_policy_dir" >&2
      return 1
    fi
  fi
  if ! rmdir -- "$retained_policy_dir"; then
    printf 'Cleanup failed for %q while removing the directory.\n' "$retained_policy_dir" >&2
    return 1
  fi
  if [[ -e $retained_policy_dir || -L $retained_policy_dir ]]; then
    printf 'Cleanup failed for %q: the path still exists.\n' "$retained_policy_dir" >&2
    return 1
  fi
  printf 'Removed retained policy directory %q.\n' "$retained_policy_dir"
}
cleanup_retained_policy
```

The procedure removes only `policy.yaml`, so `rmdir` stops if the directory contains anything else.
Continue only after the procedure reports that the retained policy directory is removed.
...
Fresh portable onboarding selects `personal-open-internet` when `NEMOCLAW_POLICY_PRESETS` is unset.
The gateway can refuse that default selection, so do not assume that a preset you added caused the refusal.
