...
### The Hermes gateway will not restart after `shields up`

The Hermes config root holds the agent's top-level runtime state, not only its configuration: `auth.json`, the drain request, and the temporary files that back every atomic `gateway_state.json` and `gateway.pid` replace are created directly in `/sandbox/.hermes`.
Lockdown therefore keeps that directory group-writable with the set-id and sticky bits and only changes its owner, so the gateway can still manage its own runtime files while the sticky bit stops the sandbox identity from unlinking or renaming the sealed root-owned configuration.
Inspect a locked root with:

```bash
nemohermes <name> exec -- stat -c '%a %U:%G' /sandbox/.hermes
```
...
The gateway cannot write its runtime state there, so every restart fails its health check and the supervisor quarantines relaunch until the sandbox is recreated.
`nemohermes <name> shields status` reports the stale posture as drift; re-run `nemohermes <name> shields up` to repair it, or rebuild the sandbox.
