...
On native or Docker Desktop-backed WSL ARM64 Linux hosts without firmware-confirmed NVIDIA platform metadata, onboarding clears a placeholder name with one bounded Docker CUDA workload before rejecting the GPU:

```
docker run --rm --gpus all nvcr.io/nvidia/k8s/cuda-sample:vectoradd-cuda12.5.0
```

The run may first pull the CUDA sample image and is bounded to 3 minutes.
Set `NEMOCLAW_WSL_GPU_PROOF_TIMEOUT_MS` to raise that bound on slow links.
A host whose GPU passes the workload keeps GPU passthrough; a host that fails or times out falls back to CPU.
