...
| systemd did not delegate `cpu`; the user-manager file is missing or does not contain `cpu` | The current user's manager does not receive the `cpu` controller. | Apply the named `user@.service` drop-in below. |
| `cpu` is unavailable to `app.slice`; its file is missing | The current user's `app.slice` hierarchy does not exist for this boot. | Start `app.slice`. If the file remains missing after you apply the delegation change, use the stop, reload, and start sequence below or reboot the host. |
| `cpu` is unavailable to `app.slice`; its file is readable but does not contain `cpu` | The running user manager has not made the delegated controller available to `app.slice`. | Use the stop, reload, and start sequence below or reboot the host after the delegation change. Then start `app.slice` and verify all three boundaries. |
...
The systemd delegation change requires administrator access.
NemoClaw does not edit `/etc/systemd/system`, invoke `sudo`, remove the sandbox CPU limit, or continue with weaker resource isolation.

<Warning>
The `user@.service` template applies to every user manager on the host.
Applying or removing this drop-in requires the administrator to stop the affected user's manager, reload systemd, and start the manager.
...
Use the dedicated NemoClaw drop-in path below.
If the file already exists, stop and ask the administrator to inspect its ownership and content.
...
sudo install -d -o root -g root -m 0755 -- "$(dirname "$drop_in")" &&
  printf '%s\n' '[Service]' 'Delegate=cpu memory pids' | sudo tee "$drop_in" >/dev/null &&
...
Verify that the persistent path is a root-owned regular file with mode `0644` and the required content:

```bash
if sudo test -L "$drop_in" \
  || ! sudo test -f "$drop_in" \
  || [ "$(sudo stat -Lc '%U:%G %a' -- "$drop_in")" != "root:root 644" ]; then
  printf 'Unexpected CPU delegation drop-in type, owner, or mode: %s\n' "$drop_in" >&2
  exit 1
fi
...
The final start creates the user manager with those updated masks.
...
#### Remove the CPU Delegation Drop-In

Remove only the named file that this procedure created.
Do not use `systemctl revert`, and do not remove another `user@.service` drop-in.

Confirm the exact file before removal:
...
Continue only when the path and content match this procedure.
From an administrator session that does not depend on the affected user's manager, stop the manager first.
Remove the file while the manager is inactive, then reload systemd and start the manager:
...
After the affected user signs in again, verify that systemd no longer loads this drop-in:

```bash
drop_in="/etc/systemd/system/user@.service.d/90-nemoclaw-cpu-delegation.conf"

if sudo test -e "$drop_in" || sudo test -L "$drop_in"; then
  printf 'CPU delegation drop-in remains after removal: %s\n' "$drop_in" >&2
  exit 1
fi
...
The output must not list `/etc/systemd/system/user@.service.d/90-nemoclaw-cpu-delegation.conf`.
Another administrator-owned drop-in can still delegate `cpu`; do not remove it as part of this rollback.
Removing the NemoClaw drop-in can make the portable CPU delegation preflight fail again.
