...
A stale hash never blocks shields: locking regenerates the record from the current `openclaw.json` bytes.
On current sandboxes, a truly absent `.config-hash` is also tolerated by `$$nemoclaw <name> shields up` from the default mutable state: the guard synthesizes the hash from the current `openclaw.json` under the frozen tree and the lock proceeds.
...
Run `shields up` first, or regenerate the hash as in the manual-edit workflow, before taking shields down.
...
To recover, upgrade the NemoClaw CLI first, restore the config from the quarantine copy while shields are down, and then rebuild so the sandbox picks up the current guard:
...
Rebuilding with an old CLI restages the old guard, so upgrade before rebuilding.
