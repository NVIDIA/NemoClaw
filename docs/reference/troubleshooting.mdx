...
If Hermes Portable host-forward recovery fails, use the diagnostic class before retrying:

- `A required recorded host port is occupied` means another sandbox or listener owns the recorded dashboard or API port, such as `8642`. NemoClaw leaves that owner and all forwards unchanged. Inspect `openshell forward list` and the host listener, then stop it only after confirming ownership.
- `could not read and prove one unambiguous OpenShell host-forward state` means the forward list was unavailable, malformed, or ambiguous. NemoClaw makes no forward changes. Restore a readable `openshell forward list` result before retrying.
- `could not establish the required recorded host forwards within the recovery bound` means a proven recovery operation did not settle. Inspect the recorded forward state before another probe or launch.
