...
Open the prepared shell. During setup, `connect` automatically settles pending
requests from the `cli`, `openclaw-cli`, and `openclaw-control-ui` clients when
the requested scopes are limited to pairing, read, and write and OpenClaw binds
the request to the authenticated device identity. Allowlisted client metadata
alone does not authorize approval for another device:
...
The `exec` command preserves the native command output and exit status. After
`exec` fails, NemoClaw does not inspect pending requests automatically. The
`connect` flow settles only allowlisted client identities requesting pairing,
read, and write scopes with an authoritative device-identity binding; requests
outside that bounded path remain available for manual review.
