...
When NemoClaw did not observe the exit, the diagnostic reports that fact and prints `pgrep -af openshell-gateway`.
Run that command and confirm that it lists no process before you move the state.
Moving the state directory while a gateway process still runs leaves that process without its state directory.
