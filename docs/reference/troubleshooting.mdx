...
NemoClaw aborts Slack onboarding, rebuild, and `channels add slack` when another sandbox uses the same Slack bot or app token.
The Slack check applies across gateways.
NemoClaw also aborts when another Slack sandbox uses the same gateway, even when the credentials differ.
Pass `--force` to `channels add slack` only when you explicitly accept either conflict risk.
Other credential-backed channels also abort credential conflicts without an interactive override.
Pass `--force` to `channels add <channel>` only when you explicitly accept the duplicate-consumer risk.
