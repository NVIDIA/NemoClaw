...
<Warning title="Preserve Interrupted Rebuild Recovery">
The resume and fresh-onboarding commands below apply to ordinary onboarding failures.
For an interrupted rebuild, do not manually delete `onboard-session.json` or `.onboard-rebuild-<sandbox>.json` files from the selected state directory.
