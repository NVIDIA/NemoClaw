...
When NemoClaw establishes that the gateway process stopped and an unused archive path is available, it also prints that archive path beside the selected state directory and the profile-specific onboarding command.
...
NemoClaw cannot always observe the gateway process exit because the start loop also reports this failure after its health poll budget expires.
Before NemoClaw offers the state move, its gateway runtime checks the recorded process and scans current gateway process identities for the runtime namespace tied to the selected state directory.
This scan also detects a replacement gateway that a service manager starts after the original process exits.

The state move prints only when this check establishes that no gateway process holds the directory.
When NemoClaw cannot establish that, it prints the host's managed-service stop command, then the profile-specific onboarding command on the following line.
On Linux, the stop command targets the detected OpenShell systemd user service.
On macOS, it targets the OpenShell Homebrew service.

For example, the printed stop command is one of these commands:
...
Stop the gateway, then run the onboarding command that NemoClaw prints on the next line.
