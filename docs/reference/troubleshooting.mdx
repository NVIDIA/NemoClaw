...
1. Check the managed OpenShell gateway service.
   ...
   If formula verification fails or Homebrew cannot grant or remove temporary trust, rerun `scripts/install-openshell.sh` and then rerun onboarding.
...
On Apple Silicon macOS with Homebrew, rerun `scripts/install-openshell.sh` to restore the pinned formula and temporary trust contract.
