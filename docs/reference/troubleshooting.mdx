...
NemoClaw allocates a port automatically only for a sandbox that has no registry row, so `nemohermes <name> destroy` followed by onboarding is the alternative.
`destroy` deletes the sandbox and its state.
