...
A gateway wedged this way shows up as `openclaw health` reporting `gateway_transport_error` — often `1006 abnormal closure (no close frame)` — while the container stays `running` with no restarts, and agent replies keep working through the embedded fallback route instead of the gateway.
