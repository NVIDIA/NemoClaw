...
Confirm the requesting device and the requested scopes.
Approve only the request that matches the failed command:

```bash
$$nemoclaw <name> exec -- openclaw devices approve <requestId>
```

Then rerun the original command unchanged.
NemoClaw preserves the native command output and exit status and does not inspect pending requests automatically.
