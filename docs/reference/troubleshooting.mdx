...
- In non-interactive mode (piped `curl | bash` with `NEMOCLAW_NON_INTERACTIVE=1`, CI, scripts), the installer refuses and exits with a non-zero status so a scripted re-run cannot loop.
  ...
  Interactive resume can request a web search or messaging credential again when it was available only to the interrupted process because raw credentials are not stored in the onboarding session.
  Non-interactive resume cannot prompt for a missing messaging credential: it preserves the completed channel choice, exits, and asks you to export the missing messaging credential environment variables before running `$$nemoclaw onboard --resume` again.
