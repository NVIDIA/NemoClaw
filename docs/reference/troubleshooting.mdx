...
Recovery targets each sandbox's own ports. A second Hermes sandbox on the same host receives the next free API port, so check `nemohermes <name> status` before assuming a missing `8642` row belongs to the sandbox you are debugging.
