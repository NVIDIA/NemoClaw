...
If your corporate CA lives only in the host system trust store, point the same variable at it explicitly (for example `NEMOCLAW_CORPORATE_CA_BUNDLE=/etc/ssl/certs/ca-certificates.crt`). NemoClaw does not auto-scan the host trust store, so it never bakes an unrelated bundle without an explicit path.
