...
Onboarding preflight prints this warning before the first image pull and then continues.
NemoClaw reads the config from `$DOCKER_CONFIG/config.json` when `DOCKER_CONFIG` is set, and from `~/.docker/config.json` otherwise.

To work around a failed pull, pull the image with an isolated Docker config, then resume:
...
Alternatively, temporarily remove the `credsStore` entry from `~/.docker/config.json`, then rerun `$$nemoclaw onboard`.
