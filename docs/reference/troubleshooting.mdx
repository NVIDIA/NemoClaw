...
If an `openshell-gateway` process still holds the port, the standalone gateway owns it, not the service. Release it with the gateway environment that started it:

```bash
NEMOCLAW_GATEWAY_PORT="${gateway_port}" $$nemoclaw uninstall
```
