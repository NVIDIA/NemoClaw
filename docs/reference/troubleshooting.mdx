...
### Portable Hermes receipt and registry authority do not match

Portable Hermes lifecycle commands require the current registry entry and the durable lifecycle receipt to identify the same sandbox, gateway, and lifecycle generation. If onboarding reports that it cannot match them, keep both files unchanged and inspect the sandbox:

```bash
nemohermes <sandbox-name> doctor
```

Do not copy, recreate, or edit a registry entry or lifecycle receipt by hand. Follow the identity-qualified recovery guidance from `doctor`. After that recovery succeeds, continue the retained session:

```bash
nemohermes onboard --resume
```

If `doctor` cannot qualify an existing recovery path, preserve the registry and receipt files and stop. Manual edits can bind recovery to stale or replacement state.
