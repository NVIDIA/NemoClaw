...
Onboarding endpoint validation for a local inference provider drops the proxy variables from the probe process instead, so a host proxy cannot answer for a loopback endpoint it has no route to.
