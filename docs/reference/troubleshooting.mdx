...
After onboarding succeeds, the completion path records the selected value under `~/.nemoclaw/gateways/<port>/automatic-gateway-port`.
Failed or cancelled onboarding does not retain the selection, so a retry rechecks candidate availability.
Later NemoClaw commands restore that recorded selection when `NEMOCLAW_GATEWAY_PORT` is unset.
