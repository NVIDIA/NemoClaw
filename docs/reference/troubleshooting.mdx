...
With that control set, NemoClaw retries exactly once through the compatibility path after one of these conditions:

- Sandbox creation rejects the native `--gpu` flag before progress. NemoClaw captures redacted diagnostics and proves that the sandbox and its labeled containers are absent. It does not delete the sandbox by name.
- The exact OpenShell-managed container labeled for the sandbox records a host runtime GPU-injection error.
- An explicit `nvidia-smi` driver proof fails while that container's immutable host configuration confirms that no GPU was attached.

For the host runtime and driver-proof conditions, NemoClaw retries only after cleanup proves that the sandbox and its labeled containers are absent.
If the managed runtime requires provider-owned cleanup, NemoClaw stops before the compatibility retry.
