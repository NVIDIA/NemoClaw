...
### Automatic GPU routing or compatibility patch failed

The automatic route depends on the host environment. Identify the matching path before applying the recovery guidance.

| Symptom | Route or stage | Recovery |
| --- | --- | --- |
| Native `--gpu` is rejected, host runtime evidence identifies GPU injection failure, or an explicit driver proof fails and host configuration confirms no GPU attachment | Ordinary Linux native attempt | In automatic mode, wait for the one safe compatibility retry. Use `NEMOCLAW_DOCKER_GPU_PATCH=1` to select compatibility before creation, or `=0` to require native-only behavior. |
| `Cleanup could not be proven safe` | Native-to-compatibility handoff | Run the printed sandbox deletion command, verify both the gateway row and labeled Docker containers are absent, then rerun onboarding. |
...
#### Ordinary native Linux: native with one safe fallback

Automatic GPU onboarding first uses native OpenShell GPU injection.
If sandbox creation rejects the native GPU flag before progress, the exact labeled container records a host runtime GPU-injection error, or an explicit `nvidia-smi` driver proof fails while that container's immutable host configuration confirms that no GPU was attached, NemoClaw captures redacted diagnostics, deletes the incomplete sandbox, verifies that no OpenShell-labeled Docker container remains, and retries exactly once through the compatibility path.
Free-form build/list text and sandbox-reported CUDA output do not authorize the broader retry. Without corroborating host evidence, onboarding fails closed and directs the operator to clean up and explicitly select compatibility with `NEMOCLAW_DOCKER_GPU_PATCH=1`.
Before that retry, NemoClaw warns that the legacy GPU compatibility envelope recreates the OpenShell-managed Docker container and may relax container confinement compared with native injection; set `NEMOCLAW_DOCKER_GPU_PATCH=0` to require native-only behavior instead.
Specifically, compatibility recreation adds `SYS_PTRACE`, adds `apparmor=unconfined` when the original container has no AppArmor option, and uses a compatibility policy that makes `/proc` writable for the NVIDIA runtime's process-name initialization. These broader settings are why onboarding warns before the swap and retains a native-only opt-out.
NemoClaw verifies cleanup with two stable checks (sandbox absent from gateway list and no labeled containers) before retrying through the compatibility path.
...
The bounds are intentionally fixed: allowing environment input to weaken or extend the cleanup proof would make a security gate deployment-dependent. On a host that cannot prove absence within the bound, onboarding fails closed; select compatibility from the outset with `NEMOCLAW_DOCKER_GPU_PATCH=1` instead of weakening the handoff proof.
...
Set `NEMOCLAW_DOCKER_GPU_PATCH=1`, or another legacy nonzero value, to use only the compatibility path for diagnostics or older host compatibility.

#### Docker Desktop WSL: compatibility only
...
#### Jetson/Tegra: compatibility only
