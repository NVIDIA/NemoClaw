...
List the pending requests through the native OpenClaw interface:

```bash
$$nemoclaw my-assistant connect
```

In the prepared shell, list the requests:
...
Then rerun the original command unchanged.
...
`connect` flow can settle only allowlisted pairing, read, and write upgrades;
requests outside that bounded path remain available for manual review.
