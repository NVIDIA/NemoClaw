...
Running `$$nemoclaw onboard` directly on a sandbox that already has a watcher also stops that watcher, and nothing replants it until the next full install or the next `destroy`/`onboard` cycle.
The sandbox keeps working, but its forward stops auto-recovering from a dropped listener until then.
...
Stop it only when that command reports your own user and the watcher script path from the diagnostic:
