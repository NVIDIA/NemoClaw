...
A later `--fresh` run reuses the transaction only when the canonical journal matches the sandbox, model, provider, credential name, and receipt target.
NemoClaw preserves malformed, mismatched, or changed state rather than guessing ownership.

Do not edit the journal or delete the provider manually.
To start an isolated attempt, choose a different sandbox name, an unused `NEMOCLAW_GATEWAY_PORT`, and a separate `NEMOCLAW_OPENSHELL_GATEWAY_STATE_DIR`.
Retire preserved resources only through the lifecycle authority that created them.
