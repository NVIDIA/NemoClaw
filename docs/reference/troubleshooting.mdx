...
### Portable Onboarding State Conflicts

An interrupted Hermes Portable Ollama run can leave a private gateway-provider journal before it publishes its inference receipt.
A later `--fresh` run has a new onboarding session identifier, but NemoClaw reuses the recorded transaction only when the complete canonical journal intent still matches the sandbox, model, provider, credential name, and receipt target.
NemoClaw reconciles the recorded phase with live authority, then creates, adopts, or deletes only the provider proven by that transaction.

If onboarding reports a gateway-provider journal or provider-authority conflict, the recorded state is malformed, belongs to different intent, or no longer matches the live provider.
NemoClaw preserves both sides rather than guessing ownership.
Do not edit the journal or delete the provider manually.
To start an isolated attempt without changing the preserved state, choose a different sandbox name, an unused `NEMOCLAW_GATEWAY_PORT`, and a separate `NEMOCLAW_OPENSHELL_GATEWAY_STATE_DIR`.
Retire the preserved resources only through the lifecycle authority that created them.
