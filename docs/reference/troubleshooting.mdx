...
sudo install -d -o root -g root -m 0755 -- "$(dirname "$delegation_drop_in")" || exit 1
sudo install -d -o root -g root -m 0755 -- "$(dirname "$app_slice_drop_in")" || exit 1
...
create_drop_in "$delegation_drop_in" '[Service]' 'Delegate=cpu memory pids' &&
  create_drop_in "$app_slice_drop_in" '[Slice]' 'CPUWeight=100'
```
...
Continue only when the command exits with status `0` and prints this output:
...
Remove only the two named files that this procedure created.
Do not use `systemctl revert`.
Do not remove another `user@.service` or `app.slice` drop-in.

Confirm both files before removal:
...
Continue only when both paths and their content match this procedure.
From an administrator session that does not depend on the affected user's manager, stop the manager first.
Remove both files while the manager is inactive, then reload systemd and start the manager:
...
app_slice_drop_in="/etc/systemd/user/app.slice.d/90-nemoclaw-cpu-controller.conf"
sudo systemctl stop "user@${uid}.service" &&
  sudo rm -- "$delegation_drop_in" "$app_slice_drop_in" &&
  sudo systemctl daemon-reload &&
  sudo systemctl start "user@${uid}.service"
```
