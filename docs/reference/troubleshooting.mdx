...
2. Conventional CA variables `REQUESTS_CA_BUNDLE`, `CURL_CA_BUNDLE`, then `SSL_CERT_FILE` (each **skipped silently** when it points at a missing or invalid file).
3. Host administrator anchor directories (skipped silently when absent or unusable).
