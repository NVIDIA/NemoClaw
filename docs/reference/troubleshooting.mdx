...
### Snapshot Creation Requires a Verified Python Interpreter

Snapshot creation stops when NemoClaw cannot resolve a verified `python3` interpreter.
...
NemoClaw appends the cleanup result when it removes the incomplete snapshot or cleanup fails.
...
It accepts only a verified executable at one of the fixed locations listed in [Prerequisites](../get-started/prerequisites), and it rejects a candidate that fails its ownership, permission, or executable checks.
Install or repair `python3` at one of those locations, then run the command again.
