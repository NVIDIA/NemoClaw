...
Use this recovery when global policy inspection or sandbox creation reports a TLS or certificate error during onboarding.
The OpenShell gateway certificate may have changed since the CLI last registered it.
This procedure also applies when global policy inspection fails before sandbox creation.
