...
If the startup log reports `HERMES_RUNTIME_PREPARATION_FAILED stage=<stage>`, the named environment, config, dependency, provider, or messaging preparation failed before a replacement gateway launched. The supervisor quarantines that nontransient failure immediately instead of repeating it. Correct the named input through its supported host-side configuration or credential flow, then restart the sandbox:
...
Use `$$nemoclaw <name> rebuild --yes` only when the input or sandbox state cannot be repaired safely. Repeated recovery requests do not retry the quarantined preparation.

When the relaunch quarantine instead comes from an integrity mismatch or an exhausted gateway crash budget, retrying the recovery command cannot clear it. Restore the registered configuration and refresh its integrity metadata in one transaction:
