...
Confirm that the generated Hermes configuration selects the web search backend you enabled.

```bash
$$nemoclaw <name> exec -- cat /sandbox/.hermes/config.yaml
```

The output should include a `web` mapping with `backend: tavily` or `backend: firecrawl`.
If it does not, rerun onboarding with `NEMOCLAW_WEB_SEARCH_PROVIDER=tavily` and `TAVILY_API_KEY`, or `NEMOCLAW_WEB_SEARCH_PROVIDER=firecrawl` and `FIRECRAWL_API_KEY`.
...
Use Tavily Search or Firecrawl Search through NemoClaw onboarding instead.
...
NemoClaw writes `web.backend: tavily` (or `firecrawl`), applies the matching `tavily` or `firecrawl` policy preset, and configures credential rewriting for Hermes (request-body for Tavily, `Authorization` header for Firecrawl).
If the same onboarding run selected the Nous-managed web gateway, the selected provider replaces `nous-web` while selected Nous image, audio, browser, and code tools remain enabled.
