...
NemoClaw removes the temporary policy file it submits whether or not the gateway accepts it.
...
NemoClaw could not confirm the result, for example, because the gateway connection ended before the result arrived:
