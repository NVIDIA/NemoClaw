...
1. Check the managed OpenShell gateway service.
   ...
   If the Homebrew service cannot be inspected, started, or made healthy during onboarding, NemoClaw prints this command for the formula logs:
   ...
   NemoClaw attempts to stop a service that reached startup and use the standalone fallback.
   The standalone gateway starts only after NemoClaw verifies exclusive ownership of the gateway port.
   If Homebrew is present but the `openshell` formula is missing, onboarding also uses the standalone fallback.
   Rerun the NemoClaw installer to restore the managed service for later onboarding runs.
   An installed `openshell` formula from another tap remains a hard failure; rerun the installer to restore the official formula.
   ...
   If the service cannot be inspected, started, or made healthy during onboarding, NemoClaw prints this command for its logs:
   ...
   For this service, NemoClaw prints the following log command after an onboarding failure:
   ...
   On Linux, NemoClaw attempts to stop a service that reached startup and use the standalone fallback.
   The standalone gateway starts only after NemoClaw verifies exclusive ownership of the gateway port.
