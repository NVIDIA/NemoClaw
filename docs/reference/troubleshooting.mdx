...
For an OpenClaw sandbox that uses an OpenAI-compatible endpoint, onboarding checks `inference.local` from inside the sandbox whether or not you configure messaging.
If that smoke check fails, fix the compatible-endpoint base URL, credentials, model, or network route before you retry onboarding.
