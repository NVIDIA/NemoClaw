...
The `invalid_docker_host` advisory means that `DOCKER_HOST` is set to an endpoint that NemoClaw cannot safely pass to the managed OpenShell gateway service.
