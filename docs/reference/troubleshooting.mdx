...
The onboarding policy step composes a policy document for the presets you selected and deselected, then submits it to the OpenShell gateway.
NemoClaw removes the temporary file that carries the composed policy whether or not the gateway accepts it.
...
```text
OpenShell rejected the policy for sandbox 'my-assistant' (exit 1): <OpenShell diagnostic>. The policy was not applied and re-applying it will be rejected again; change the preset selection instead.
```

The live policy is unchanged.
Resume reapplies the recorded selection, so it submits the same document and receives the same refusal.
...
This covers a connection that ended before the result arrived, an unreachable gateway, an elapsed deadline, and a rejected credential.
...
Because the failed step did not record the presets as applied, a preset that the gateway did apply appears as that disagreement.
