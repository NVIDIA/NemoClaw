...
NemoClaw's sandbox entrypoint intercepts `openclaw channels <add|remove>` and points to the host-side commands below.
...
The host command `$$nemoclaw <sandbox> config set` returns guidance to use the native OpenClaw command.
