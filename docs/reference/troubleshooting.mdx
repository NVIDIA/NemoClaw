...
Rerunning onboarding with `--recreate-sandbox` and no variable also allocates a free port, because a recreate that is not resuming an interrupted session removes the registry row first.
A recreate that resumes an interrupted session keeps the row, so the sandbox keeps `8642` unless you also set the variable.
