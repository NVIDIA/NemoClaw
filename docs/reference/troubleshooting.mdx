...
After installer-driven onboarding succeeds, it promotes that file to `automatic-gateway-port`.
Deferred, failed, cancelled, or later direct CLI onboarding keeps the pending identity so a later installer or CLI process restores the same port and reaches its onboarding or cleanup path instead of creating another environment.
