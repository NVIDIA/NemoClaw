...
Do not use `shields up` to replace corrupt state.
Inspect the reported state-file error, restore the state file from a trusted host backup, or run `$$nemoclaw <name> rebuild` before retrying.
