...
the requested scopes are limited to pairing, read, and write:
...
Confirm the requesting client, device, and requested scopes.
Approve only the request that matches the failed command:
...
read, and write scopes; requests outside that bounded path remain available for
manual review.
