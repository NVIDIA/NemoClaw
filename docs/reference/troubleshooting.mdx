...
When you run an `openclaw` command through `$$nemoclaw <name> exec -- ...` and it exits non-zero, NemoClaw asks the sandbox whether any device request is pending when the host can bound the probe process tree.
...
The automatic probe requires Linux and `/usr/bin/timeout`. On another host, review pending requests directly before approving one:
