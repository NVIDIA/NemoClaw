...
If the warning mentions degraded Landlock support, check the host kernel and container runtime.
Linux kernels older than 5.13 and some VM-backed Docker runtimes can run with reduced filesystem enforcement.
NemoClaw still relies on container mount configuration in that mode, but you should treat the host as lower assurance until Landlock is available.
