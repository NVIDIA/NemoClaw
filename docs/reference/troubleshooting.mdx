...
On an OpenClaw Jetson host, the installer applies persistent group read-write access when `/dev/nvmap` is a real character device.
If the device is absent, setup warns and continues without installing the rule.
...
If CUDA reports `NvRmMemInitNvmap failed with Permission denied` and `cuInit(0)=999`, rerun the NemoClaw installer to apply the permission setup:
