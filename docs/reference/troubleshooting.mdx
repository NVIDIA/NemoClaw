...
- The in-sandbox runtime needs more time to boot or load its selected model.
- OpenShell needs more time to publish the sandbox's `Ready` state.

<AgentOnly variant="openclaw,hermes">

For OpenClaw and Hermes, post-`Ready` sandbox identity or command checks can also exceed the budget.

</AgentOnly>

Image build and gateway upload finish before the readiness budget starts.
Their duration does not consume `NEMOCLAW_SANDBOX_READY_TIMEOUT`.
...
NemoClaw then tries to delete the partially created sandbox.
After successful deletion or confirmed absence, the output ends with `Retry: $$nemoclaw onboard`.
If deletion fails, the receipt marks the sandbox as retained.
NemoClaw then prints `openshell sandbox delete "<name>"` as manual cleanup and omits the retry command.
For Docker GPU compatibility cleanup, the receipt records `deferred_to_docker_gpu_patch` and omits the generic retry command.
