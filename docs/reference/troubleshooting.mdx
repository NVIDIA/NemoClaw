...
If a non-NemoClaw process is already bound to the dashboard port or the gateway port, identify the conflicting process and verify it is safe to stop:
...
If you see the error on an older release, identify the SSH process and verify that it is safe to stop:
