...
With that control set, if sandbox creation rejects the native GPU flag before progress, NemoClaw captures redacted diagnostics and verifies that the sandbox and its labeled Docker containers are absent without deleting the sandbox by name.
It retries exactly once through the compatibility path only after that proof succeeds.
If a native route or readiness failure requires managed cleanup with exact identities, NemoClaw stops instead of deleting a sandbox by its mutable name or retrying.
...
NemoClaw verifies absence with two stable checks: the sandbox is absent from the gateway list, and no OpenShell-managed Docker containers labeled for that sandbox remain.
...
If absence or container cleanup cannot be proven safe, onboarding stops before the retry and prints manual cleanup guidance.
