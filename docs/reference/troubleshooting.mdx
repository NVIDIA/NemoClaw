...
When you run `npm install` in a source checkout, NemoClaw first tries `npm link`.
If `npm link` fails, it creates a development shim at `~/.local/bin/nemoclaw`.
The installer also creates a user-local shim for each packaged CLI.
Verification checks the active CLI's shim at `~/.local/bin/$$nemoclaw` independently of the active npm prefix.
If that valid shim is not the command your current shell resolves, installation continues using its absolute path and prints commands to refresh your `PATH`.
