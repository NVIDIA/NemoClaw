...
If `destroy` prints `Shared NemoClaw gateway left running`, NemoClaw skipped gateway cleanup because `openshell sandbox list` still reported a sandbox after the post-delete wait, or because that command failed.
The message names the listed sandboxes or the failed command.
...
Replace `nemoclaw` with the gateway name from the message when the sandbox used a custom gateway port.
