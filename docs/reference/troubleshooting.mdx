...
An integrity failure can still report a missing or malformed metadata file, an unsafe path, or a raced snapshot.
...
If the error reports unsafe, missing, or malformed integrity state, rebuild from registered configuration:
