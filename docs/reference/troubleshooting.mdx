...
# Qualified rootless Podman
podman run --rm --device nvidia.com/gpu=all --entrypoint /bin/sh "$proof_image" -c "$proof_script"
```

A qualifying result includes `Test PASSED` and exactly one `NEMOCLAW_GPU_MEMORY_MIB=<total>, <free>` line. That line is the same-container capacity evidence used for N1x model selection.

The run is bounded to 3 minutes by default. Set `NEMOCLAW_WSL_GPU_PROOF_TIMEOUT_MS` to a positive integer number of milliseconds to change that bound; values above 15 minutes are capped at 15 minutes. A passing workload lets onboarding treat the detected GPU as eligible for GPU passthrough during that run. On Windows-on-Arm, only the documented N1x WSL path can use this proof for platform qualification; other GPU routes remain unsupported. A failed, interrupted, or timed-out workload leaves the GPU unproven and does not enable GPU passthrough. After any unsuccessful capture, NemoClaw checks for the proof container by its exact name and ownership label for up to 15 seconds, and removes only its immutable container ID if it appears. If an interrupted capture remains absent through that deadline, or removal fails, NemoClaw reports unresolved cleanup and names the exact resource to verify or remove before retrying. The names-only unified-memory fallback does not run this workload and rejects denylisted names. WSL uses Docker Desktop by default; a qualification-backed rootless Podman provider can run the same proof for WSL-local Ollama.
