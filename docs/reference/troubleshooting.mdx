...
sudo install -d -o root -g root -m 0755 -- "$(dirname "$delegation_drop_in")" &&
  sudo install -d -o root -g root -m 0755 -- "$(dirname "$app_slice_drop_in")" &&
  printf '%s\n' '[Service]' 'Delegate=cpu memory pids' \
    | sudo tee "$delegation_drop_in" >/dev/null &&
  sudo chown root:root "$delegation_drop_in" &&
  sudo chmod 0644 "$delegation_drop_in" &&
  printf '%s\n' '[Slice]' 'CPUWeight=100' | sudo tee "$app_slice_drop_in" >/dev/null &&
  sudo chown root:root "$app_slice_drop_in" &&
  sudo chmod 0644 "$app_slice_drop_in"
```

If a creation command fails, do not reload systemd.
Ask the administrator to inspect the named path and remove it only if this procedure created the partial file.
