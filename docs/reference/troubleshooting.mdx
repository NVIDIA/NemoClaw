...
Before onboarding begins, the installer records the selected value under `~/.nemoclaw/gateways/<port>/automatic-gateway-port.pending`.
After onboarding succeeds, it promotes that file to `automatic-gateway-port`.
Failed or cancelled onboarding keeps the pending identity so a new installer process restores the same port and reaches its existing onboarding session or cleanup path instead of creating another environment.
