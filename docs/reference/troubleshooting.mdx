...
For OpenClaw, onboarding checks `inference.local` from inside the ready sandbox for every OpenAI-compatible endpoint before policy selection.
This route probe accepts HTTP `200` through `499`, so it proves managed route access but not a successful model response.
When messaging is enabled, onboarding runs a separate model-response smoke check.
If either check fails, fix the reported route or model problem before testing the messaging channel again.
...
### Compatible endpoint onboarding reports `BROKEN 000`

OpenClaw compatible-endpoint onboarding can stop with:

```text
OpenShell Docker proxy child environment or gateway path is unavailable (BROKEN 000).
```

This result means the sandbox-side probe could not establish the managed OpenShell proxy or gateway path.
Verify that the OpenShell Docker gateway proxy is running and that sandbox exec children receive its HTTPS proxy and CA environment.
Do not add `inference.local` to `/etc/hosts` or `NO_PROXY` because either change bypasses the managed inference and credential boundary.
Do not treat this result as a direct DNS problem by changing Docker DNS, CoreDNS, or virtual machine networking.
After you restore the managed proxy route, run `$$nemoclaw onboard --resume`.
