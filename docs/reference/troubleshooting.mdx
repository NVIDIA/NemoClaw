...
The error identifies the hierarchy boundary that failed:

| Reported condition | Meaning | Recovery |
| --- | --- | --- |
| cgroups v2 is unavailable | `/sys/fs/cgroup/cgroup.controllers` is missing or unreadable. | Ask an administrator to boot a Linux host with a readable cgroups v2 mount. |
| The kernel hierarchy does not expose `cpu` | The root `cgroup.controllers` file does not contain the `cpu` controller. | Ask an administrator to enable the `cpu` controller in the host's kernel cgroup hierarchy. |
| systemd did not delegate `cpu` to the current user's manager | The user manager's `cgroup.controllers` file is missing, unreadable, or does not contain `cpu`. | Apply the `user@.service` delegation procedure below. |
| `cpu` is unavailable to the current user's `app.slice` | The `app.slice` `cgroup.controllers` file is missing, unreadable, or does not contain `cpu` for this boot. | Restart the current user's manager or reboot the host, then verify all three boundaries below. |
...
Restarting `user@<uid>.service` stops and starts the current user's systemd services, including rootless Podman and other user services.
Plan this interruption with the host administrator before applying the change.
</Warning>
...
Ask an administrator to open a systemd drop-in for the user-manager template:

```bash
sudo systemctl edit user@.service
```

Add this exact content, then save and close the editor:
...
Reload systemd and restart the affected user's manager:
...
`daemon-reload` makes the drop-in visible, but it does not update an existing user manager.
The `user@<uid>.service` restart or a host reboot is required before `app.slice` can receive the new controller delegation.
The administrator's `sudo` policy can request authentication, but NemoClaw never receives that credential.
