...
This failure occurs after an OpenShell downgrade.
...
You reach that state in one of two ways:

- You select an older release with `NEMOCLAW_INSTALL_TAG`.
...
The diagnosis prints in both cases. The state move prints only when NemoClaw establishes that no gateway process holds the directory.
When NemoClaw cannot establish that, it prints the stop command for the managed gateway service instead, followed by the onboarding command:
