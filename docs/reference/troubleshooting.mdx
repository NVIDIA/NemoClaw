...
- If `policy list` reports no disagreement for the failed mutation, compare the live preset set with the selection in the failed session.
  Resume only if the live policy still requires the failed addition or removal to match that selection.
