...
The Hermes Discord policy allows its virtual-environment Python interpreter and does not grant Node.js or `curl` Discord egress. A Node.js or `curl` failure is therefore not proof that the Discord preset is broken.
...
try:
    with urllib.request.urlopen("https://discord.com/api/v10/gateway", timeout=20) as response:
...
To check Discord CDN egress, use the same Python runtime:
...
try:
    with urllib.request.urlopen("https://cdn.discordapp.com/", timeout=20) as response:
        print(response.status)
except urllib.error.HTTPError as error:
    print(error.code)
PY
