...
A Hermes sandbox onboarded before the API port became per-sandbox carries no allocated port and keeps `8642`, so recreate it when a second Hermes sandbox needs its own API forward.
