...
The watchdog logs the cause of every not-serving probe, and its recovery bound depends on whether the gateway has ever served:
...
- Before the gateway has ever served, the watchdog waits out a longer boot grace window of 20 not-serving probes, roughly 10 minutes, because it cannot tell a slow boot from a gateway that came up unable to serve. Set `NEMOCLAW_GATEWAY_WATCHDOG_BOOT_GRACE_PROBES` to change this bound.

`NEMOCLAW_GATEWAY_WATCHDOG_BOOT_GRACE_PROBES` accepts a positive integer of up to nine digits, and so do `NEMOCLAW_GATEWAY_WATCHDOG_INTERVAL_SECONDS` and `NEMOCLAW_GATEWAY_WATCHDOG_REFUSED_THRESHOLD`.
...
Each line states the count and the bound it is counting toward, such as `(2/4 since the last serving response)` or `(7/20 since launch, having never served)`.
