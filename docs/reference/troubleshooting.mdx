...
### Automatic GPU fallback or compatibility patch failed

On ordinary native Linux Docker-driver hosts, automatic GPU onboarding first uses native OpenShell GPU injection.
...
Before that retry, NemoClaw warns that the legacy GPU compatibility envelope recreates the OpenShell-managed Docker container and may relax container confinement compared with native injection; set `NEMOCLAW_DOCKER_GPU_PATCH=0` on ordinary native Linux to require native-only behavior instead.
...
These fail-closed safety limits are internal constants (two stable absence checks, five maximum attempts, and a one-second interval) and are not configurable through environment variables.
...
On Docker Desktop WSL and Jetson/Tegra, automatic GPU onboarding uses the compatibility path directly, which creates the sandbox first and then recreates the OpenShell-managed Docker container with NVIDIA GPU flags.
Set `NEMOCLAW_DOCKER_GPU_PATCH=1`, or another legacy nonzero value, to use only the compatibility path on ordinary native Linux for diagnostics or older host compatibility.
If the compatibility attempt fails, onboarding leaves the failed sandbox and diagnostic bundle in place so you can inspect the OpenShell and Docker state.
