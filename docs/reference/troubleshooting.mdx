...
| --- | --- | --- |
| Native `--gpu` is rejected, GPU initialization fails during readiness, or the CUDA proof fails | Ordinary Linux native attempt | In automatic mode, wait for the one safe compatibility retry. Use `NEMOCLAW_DOCKER_GPU_PATCH=1` to select compatibility before creation, or `=0` to require native-only behavior. |
...
If sandbox creation is rejected for native GPU routing, readiness diagnostics identify GPU injection or device initialization as the cause, or the completed CUDA proof returns a hard failure, NemoClaw captures redacted diagnostics, deletes the incomplete sandbox, verifies that no OpenShell-labeled Docker container remains, and retries exactly once through the compatibility path.
