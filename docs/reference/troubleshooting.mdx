...
To move a sandbox that predates the change onto its own port, run `nemohermes <name> destroy` and onboard it again, because NemoClaw allocates a port only for a sandbox that has no registry row.
