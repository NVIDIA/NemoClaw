...
The preflight distinguishes a missing controller file from a read failure.
...
do
  if [ ! -e "$controllers" ]; then
    ...
    printf '%s: unreadable\n' "$controllers"
  elif contents="$(cat -- "$controllers")"; then
...
Applying or removing either drop-in requires the administrator to stop the affected user's manager, reload systemd, and start the manager.
The administrator can reboot the host instead of running that sequence.
Stopping the manager stops that user's systemd services, including rootless Podman and other user services.
Plan each interruption with the affected user and host administrator.
</Warning>
...
Use the two dedicated NemoClaw drop-in paths below.
The service drop-in delegates `cpu` to user managers.
The `app.slice` drop-in requests the kernel default CPU weight of `100`.
The explicit setting makes the user manager activate the delegated controller for `app.slice`.
If either file exists, stop and ask the administrator to inspect its ownership and content.
...
Ask the administrator to inspect both drop-in paths and their directories.
Remove either drop-in path or a `.nemoclaw-cpu-controller.*` temporary file only if this procedure created it.
...
Record all rollback lines from the creation command in the administrator's change record, including both file `device:inode` values and both directory creation records.
...
Run the following commands from an administrator session that does not depend on the affected user's manager:
...
The final start makes the user manager read the `app.slice` CPU weight and enable the delegated controller.
Do not replace this sequence with `systemctl restart`, which can fail with `219/CGROUP` before systemd recalculates the masks.
The sequence stops at the first failed command.
If a command fails after the manager stops, correct the failure and rerun the failed command and each remaining command.
...
Continue only when all three lines end in `: cpu`.
...
Remove only the two named files and any drop-in directory that the creation record marks as created by this procedure.
Do not use `systemctl revert`.
Do not remove another `user@.service` or `app.slice` drop-in or a pre-existing drop-in directory.

Confirm both files and retrieve the `device:inode` values from the administrator's creation-time change record:
...
Continue only when both paths, their content, and their current `device:inode` values match the creation-time record.
From an administrator session that does not depend on the affected user's manager, stop the manager first.
...
The inactive reload removes both NemoClaw drop-ins from the instantiated units and recalculates ancestor controller masks before the manager starts.
...
The output must not list either NemoClaw drop-in.
