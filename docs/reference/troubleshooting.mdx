...
2. Install and start a supported container runtime (Docker Desktop or Colima). Without a running runtime, the installer cannot connect to Docker.
   If you use Colima through Homebrew, install both Colima and the Docker CLI:
...
If `colima status` succeeds but `docker info` or `which docker` fails with a missing `docker` command, install the Docker CLI separately:
