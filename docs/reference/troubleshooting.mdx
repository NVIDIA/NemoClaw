...
On a supported local Docker-driver sandbox with the legacy keepalive startup, it can authorize a container-identity-pinned in-place supervisor launch.
NemoClaw requires managed gateway health, the settle check, and OpenShell readiness before it re-establishes the primary port forward.
...
If the error mentions `SUPERVISOR_NOT_RUNNING` and the trusted launch could not proceed, `SUPERVISOR_REBUILD_REQUIRED`, a missing `nemoclaw-gateway-control` helper, or a missing managed controller, the sandbox image may predate the current lifecycle contract.
