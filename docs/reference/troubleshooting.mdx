...
If a host previously ran an older Spark setup script, `/etc/docker/daemon.json` may still contain `default-cgroupns-mode: host`.
That stale daemon-wide setting can break CUDA initialization in GPU containers even when `nvidia-smi` works.
Current onboarding detects the setting, prints the daemon config path, and warns before continuing.
To remove it manually, back up the Docker daemon config, delete only `default-cgroupns-mode`, and restart Docker:
...
data.pop("default-cgroupns-mode", None)
