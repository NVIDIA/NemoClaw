...
NemoClaw applies the same fail-closed conflict policy to every supported messaging channel during onboarding, rebuild, and `channels add`.
It rejects reused channel credentials and channel-owned shared-resource conflicts without an interactive override.
Channel-specific examples include Slack gateway conflicts and Microsoft Teams webhook port conflicts.
