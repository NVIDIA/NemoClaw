...
The affected sandboxes use that route until another onboarding run or, when custom endpoint and API-family identity match, `$$nemoclaw <name> connect` warns and re-points its provider and model.
`connect` refuses different custom endpoints or API families because its provider/model-only route change cannot safely reconstruct that metadata.
...
The JSON status report exposes the same live and recorded routes in `routeDrift`.
...
Onboarding applies complete time-shared route changes; `connect` re-points only when any custom endpoint and API-family identity already match.
