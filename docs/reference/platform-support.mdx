...
| Web search backend | Tested with limitations | Onboarding supports Brave and Tavily for OpenClaw and Tavily for Hermes. Provider selection, agent configuration, and credential attachment are sandbox-creation inputs, so changing the provider recreates the sandbox. Stock onboarding carries the non-secret configuration in the managed startup profile; an explicit custom Dockerfile must apply it during its local build. OpenShell replaces resolver placeholders at egress, including JSON request-body rewriting for Hermes Tavily. Users supply the backend credential; NemoClaw does not bundle a key. |
...
| Item | Status | Why |
|------|--------|-----|
| Podman / other container runtimes | Unsupported | Native Podman remains unsupported in this release; the native-runtime follow-up in issue #7744 owns that enablement. Onboard surfaces an explicit unsupported-runtime error for Podman (`src/lib/onboard/fatal-runtime-preflight.ts` prints the rejection; `src/lib/onboard/preflight.ts` flags the unsupported runtime upstream). Only Docker Engine, Docker Desktop, and Colima are supported today. See issue #420 (closed). |
