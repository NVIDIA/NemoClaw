...
| Intel Mac (macOS x86_64) | Unsupported | The supported OpenShell Homebrew gateway service path is Apple Silicon only. OpenShell does not publish macOS x86_64 standalone gateway assets. Install hard-fails on x86_64 macOS. See issue #954 (closed). |
