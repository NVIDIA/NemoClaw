...
The DGX Station `experimental-single-user` managed-vLLM profile is available through the explicit installer flag `--station-experimental-single-user`; advanced direct onboarding may opt in with the matching `NEMOCLAW_VLLM_PROFILE` and `NEMOCLAW_VLLM_MODEL` selectors.
It does not change the default Station recipe or promote DGX Station beyond Deferred status.
The profile currently requires its exact qualified runtime image to be preloaded because no pullable repository manifest has been published and proven for that image; do not present it as a fresh-install or current-PR physical-hardware qualification.
