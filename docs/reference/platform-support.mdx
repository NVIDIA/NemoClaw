...
| State migration | Tested | Sandbox state migrates across rebuilds with credentials intentionally excluded. Hermes excludes `auth.json` and restores its SQLite session DB through the backup API. OpenClaw restores its credential-sanitized native config as a whole file; sanitization removes the top-level `gateway` object and credential fields. |
