...
| Windows WSL2 | Docker Desktop (WSL backend) | Tested with limitations | P1 | No | Requires WSL2 with Docker Desktop backend. Accepting Windows Express and its third-party software notice is the explicit opt-in to this Experimental surface. Windows Express and non-interactive onboarding with no requested or recorded provider, `NEMOCLAW_MODEL`, or managed-recipe override automatically select the managed llama.cpp Qwen 3.6 recipe when a bounded CUDA proof validates one GPU whose normalized identity is either `NVIDIA RTX Spark N1X` or `NVIDIA RTX Spark N1X (6144-core Blackwell RTX GPU)` on Linux ARM64 WSL, the local-default Docker authority is active, and the 48,000 MiB Docker and GPU memory, driver, NVIDIA integration, and Docker Desktop GPU-passthrough checks pass. The Windows chassis model does not participate in selection. Ordinary interactive onboarding can select another listed provider. Other Windows WSL Express installs use WSL-local Ollama. Windows-host Ollama remains available through manual onboarding when Docker Desktop WSL integration, the local `default` Docker context, no `DOCKER_HOST` override, and the required route protections are present. Dedicated N1x WSL CI and physical end-to-end validation are pending. |
...
| Local llama.cpp (managed) | Experimental | Local OpenAI-compatible | Uses repository-owned YAML recipes and exact GGUF verification. The selected runtime-provider boundary owns lifecycle; all currently qualified profiles require Docker. Qualified DGX Spark hosts offer NVIDIA Nemotron 3 Nano 30B-A3B. Qualifying Windows WSL N1x hosts use Qwen 3.6 35B-A3B during Windows Express or non-interactive onboarding with no requested or recorded provider, `NEMOCLAW_MODEL`, or managed-recipe override. The N1x recipe remains pending physical agent qualification. |
