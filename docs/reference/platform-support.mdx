...
| State persistence and rebuild transfer | Tested | OpenShell preserves the native agent home during routine lifecycle operations. Before destructive replacement, NemoClaw archives and integrity-checks the complete native home/workspace, then restores it into the replacement sandbox. |
...
| State migration | Tested | The complete native agent home/workspace migrates across rebuilds. OpenShell-owned credentials remain outside that tree and are not copied into it. |
