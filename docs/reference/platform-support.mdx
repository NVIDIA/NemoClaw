...
| Local llama.cpp (managed) | Experimental | Local OpenAI-compatible | Uses repository-owned YAML recipes, exact GGUF verification, and the selected runtime-provider lifecycle. Qualified DGX Spark hosts offer NVIDIA Nemotron 3 Nano 30B-A3B. Qualifying Windows WSL N1x hosts use Qwen 3.6 35B-A3B during Windows Express or non-interactive onboarding with no requested or recorded provider. The N1x recipe remains pending physical agent qualification. |
