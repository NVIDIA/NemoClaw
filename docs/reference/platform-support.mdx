...
| Windows WSL2 | Docker Desktop (WSL backend) | Tested with limitations | P1 | No | Requires WSL2 with Docker Desktop backend. Onboarding automatically selects the Experimental managed llama.cpp Qwen 3.6 recipe when a bounded CUDA proof validates one `NVIDIA RTX Spark N1X` GPU on Linux ARM64 WSL, the local-default Docker authority is active, and the 48,000 MiB Docker and GPU memory, driver, NVIDIA integration, and Docker Desktop GPU-passthrough checks pass. The Windows chassis model does not participate in selection. Other Windows WSL Express installs use WSL-local Ollama. Windows-host Ollama remains available through manual onboarding when Docker Desktop WSL integration, the local `default` Docker context, no `DOCKER_HOST` override, and the required route protections are present. Dedicated N1x WSL CI and physical end-to-end validation are pending. |
