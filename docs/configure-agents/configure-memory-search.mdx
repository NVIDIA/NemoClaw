...
```bash
nemoclaw my-assistant exec -- \
  openclaw config set models.providers.ollama-mem \
  '{"api":"ollama","baseUrl":"http://host.openshell.internal:8000","apiKey":"ollama-local","models":[{"id":"qwen3-embedding:0.6b","name":"Qwen3 Embedding 0.6B"}]}' \
  --strict-json \
  --merge
...
nemoclaw my-assistant exec -- \
  openclaw config set memory.search.enabled true --strict-json

nemoclaw my-assistant exec -- \
  openclaw config set memory.search.provider ollama-mem
...
OpenClaw owns the native configuration after first launch.
Restart and reconnect preserve the current native changes.
Rebuild restores the sanitized configuration captured immediately before rebuild, while snapshot restore reapplies the settings from the selected snapshot.
The `local-inference` OpenShell policy preset controls access to the private host bridge URL.
