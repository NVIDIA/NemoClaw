...
Stock onboarding carries the setting in the managed startup profile; an explicit custom Dockerfile applies it during its local build.
Direct `openclaw config set` changes are not the supported path for this host-managed default, and a rebuild overwrites direct file edits.
