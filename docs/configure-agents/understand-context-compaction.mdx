...
NemoClaw applies the safeguard setting from the managed startup profile when it creates or recreates a stock sandbox.
An explicit custom Dockerfile applies the setting during its local build.
Recreate the sandbox when you need to change this host-managed setting.
