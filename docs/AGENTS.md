...
- Keep dated entries directly under `docs/changelog/`. If the planned date already has a file, add the new H2 version section with the newest version first.
- Start a new dated file with the parser-safe MDX SPDX comment shown in `docs/CONTRIBUTING.md`, then add an exact H2 heading such as `## v0.0.83`. Do not use an HTML comment for the SPDX header.
- Keep the complete summary and detailed bullets in this one shared entry. Do not create separate OpenClaw, Hermes, or Deep Agents release-note pages.
