...
1. Run `$$nemoclaw <name> status` to confirm the active provider and endpoint.
   ...
   With Docker Desktop on WSL, treat the auth-proxy diagnostic as non-authoritative because the sandbox reaches host loopback directly; use the main `Inference` result and local-backend diagnostic.
   With native Docker on WSL, rerun onboarding when the active Ollama route uses the auth proxy and that diagnostic fails.
   On non-WSL hosts that front Ollama with the auth proxy, run `$$nemoclaw <name> connect` when the auth-proxy result is `unreachable` to restart the proxy with its persisted token.
   On those hosts, rerun onboarding when the auth-proxy result is `unauthorized` to rotate the rejected proxy token and refresh the route, and follow the diagnostic detail for any other failure.
