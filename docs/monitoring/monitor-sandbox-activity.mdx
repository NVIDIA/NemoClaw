...
1. Run `$$nemoclaw <name> status` to confirm the active provider and endpoint.
   ...
   On WSL, treat the auth-proxy diagnostic as non-authoritative and use the main `Inference` result and local-backend diagnostic.
   On other hosts that front Ollama with the auth proxy, run `$$nemoclaw <name> connect` when the auth-proxy result is `unreachable` to restart the proxy with its persisted token.
   On those hosts, rerun onboarding when the auth-proxy result is `unauthorized` to rotate the rejected proxy token and refresh the route.
   For any other auth-proxy failure on those hosts, follow the diagnostic detail.
