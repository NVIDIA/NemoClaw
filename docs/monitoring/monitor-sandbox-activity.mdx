...
1. Run `$$nemoclaw <name> status` to confirm the active provider and endpoint.
   ...
   If the auth-proxy result is `unreachable`, run `$$nemoclaw <name> connect` to restart the proxy with its persisted token.
   If it is `unauthorized`, rerun onboarding to rotate the rejected proxy token and refresh the route.
   For any other auth-proxy failure, follow the diagnostic detail.
