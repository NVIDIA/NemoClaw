...
- Managed MCP commands add, list, inspect with `status`, refresh credentials and registration with `restart`, and remove authenticated HTTPS Streamable HTTP servers for OpenClaw, Hermes, and experimental LangChain Deep Agents Code through native OpenShell policy enforcement and provider-backed credential replacement.
