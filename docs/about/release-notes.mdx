...
- Shared OpenShell gateways now enforce a single compatible inference route across every registered sandbox, including stopped sandboxes.
  ...
  As a migration requirement, switching to a custom compatible provider now requires both `--endpoint-url` and `--inference-api`, and incomplete legacy custom-route metadata fails closed until the sandbox is removed and re-onboarded with complete metadata.
