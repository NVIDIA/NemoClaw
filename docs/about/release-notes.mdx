...
- Day-two commands behave more predictably in automation and across agents.
  Non-terminal `exec` closes stdin unless `--stdin` explicitly forwards a pipe, session passthrough selects the sandbox's native agent binary, resumed Hermes one-shot turns append to the selected session, and `gc` finds orphaned locally prebuilt sandbox images.
