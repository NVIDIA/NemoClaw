...
NemoClaw pins the OpenClaw version inside the sandbox at build time via `OPENCLAW_VERSION` in the NemoClaw Dockerfiles; `min_openclaw_version` remains the lower-bound compatibility floor for the pinned blueprint image artifact.
