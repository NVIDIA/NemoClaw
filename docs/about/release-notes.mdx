...
- Deep Agents interactive sessions keep auto-approval disabled by default.
  Operators can enable the `thread-opt-in` capability during onboarding or a named transactional rebuild, after which each thread must still activate it through the TUI approval choice or `dcode -y`.
