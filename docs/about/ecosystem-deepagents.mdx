...
| Path | What it means |
|------|---------------|
| **NemoClaw path** | You adopt the reference stack. Stock onboarding resolves the Deep Agents managed release image to an exact digest, applies the current inference and runtime configuration through a secret-free startup profile, and uses the blueprint's policies and state contract. |
...
| Credential handling | You create OpenShell providers manually and decide how `dcode` receives endpoint credentials. | NemoClaw creates OpenShell providers during onboarding, points Deep Agents at `https://inference.local/v1`, keeps provider credentials on the host, and carries only non-secret route state in the managed startup profile. |
...
| Inference setup | You configure OpenShell inference routing and Deep Agents provider settings manually. | NemoClaw validates the selected provider from the host, configures the OpenShell route, and materializes `/sandbox/.deepagents/config.toml` from the managed startup profile for the OpenAI-compatible path. An explicit custom Dockerfile must apply equivalent configuration during its local build. |
...
| Blueprint versioning | No NemoClaw blueprint; your image tag is whatever you built locally. | NemoClaw verifies the blueprint and accepts the release-image catalog only when one publication cohort contains exact OpenClaw, Hermes, and Deep Agents image digests. |
