...
| Lifecycle state | Not included unless you build it. | OpenShell preserves the complete native home. NemoClaw transfers that complete tree during replacement while OpenShell keeps its credential stores outside the agent home. |
| Host readiness and operations | You inspect host prerequisites and operate OpenShell resources directly. | NemoClaw provides read-only host readiness reporting, sandbox status and logs, recovery guidance, rebuild, and uninstall workflows. |
