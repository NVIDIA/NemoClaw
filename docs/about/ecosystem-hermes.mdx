...
| Credential handling | You create OpenShell providers manually with `openshell provider create` and configure placeholder resolution at egress. | NemoClaw creates OpenShell providers during onboarding. Managed startup profiles are secret-free, and NemoClaw filters sensitive host environment variables from sandbox creation. Explicit custom Dockerfile builds retain the build-argument filtering boundary. |
...
| Inference setup | You configure OpenShell inference routing and Hermes `config.yaml` manually. | NemoClaw validates credentials from the host, configures the OpenShell route, and materializes model settings in `/sandbox/.hermes/config.yaml` from the managed startup profile. An explicit custom Dockerfile must apply equivalent configuration during its local build. Hermes Provider onboarding is available through `nemohermes`. |
| Channel messaging | OpenShell delivers channel tokens through its provider system and L7 proxy; you configure Hermes platform adapters manually. | NemoClaw automates supported channel setup during onboarding and materializes Hermes env/config with placeholder tokens from the managed startup profile. An explicit custom Dockerfile must apply equivalent configuration during its local build. OpenShell resolves the placeholders at egress. |
