...
| Operate the lifecycle | Reports host readiness, creates or updates sandboxes, reports status and logs, and rebuilds or recovers sandboxes while preserving the native agent home. |
...
| Repeatable lifecycle | Uses versioned, digest-verified blueprints while OpenShell preserves the complete native home and workspace across supported lifecycle operations. |
