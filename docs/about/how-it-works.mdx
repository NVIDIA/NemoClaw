...
description-agent: "Describes NemoClaw components, functional layers, lifecycle data flow, service interactions, external integrations, and protection layers. Use for architecture and sandbox-lifecycle questions."
...
It covers the host CLI, OpenShell gateway, agent integration layer, lifecycle state, managed Model Context Protocol (MCP) servers, and protection layers.
NemoClaw packages these components as a repeatable setup, but it does not replace OpenShell or the selected agent runtime.
...
NemoClaw separates host orchestration, OpenShell management, and sandbox execution into explicit functional layers.
...
: Rebuild, snapshot, and restore operations preserve only the state declared for the selected agent runtime. Each agent manifest and operation defines which credential-bearing files to exclude.
...
NemoClaw moves configuration and lifecycle state from host readiness checks through sandbox operation and supported recovery paths.
...
The OpenShell gateway mediates requests between the agent runtime and host-managed services.
...
Use these topics to continue from the architecture overview into agent-specific setup, detailed reference material, or policy configuration.
