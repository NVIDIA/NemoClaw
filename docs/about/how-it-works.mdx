...
The CA bundle and authentication file must be nonempty regular files, must not be symbolic links, and must be no larger than 1 MiB.
The CA bundle must contain only PEM CA certificates.
Both operations read the CA bundle, inspect only authentication file metadata, and never read the authentication file contents.
They do not create or remove either administrator-owned file.
...
Run the target-only plan before the health request.
The plan validates local input, fingerprints the CA bundle, and prints a sanitized JSON plan without connecting.
...
Run the external status command to make one public health request through the official OpenShell SDK.
...
The command validates authentication file metadata but does not read its contents or authenticate.
...
<Warning>
  ...
  A failed status request makes no remote change, so correct the endpoint, CA bundle, file metadata, gateway health, or release mismatch before you retry.
