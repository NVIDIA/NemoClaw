...
flowchart LR
    ...
    STATE["Managed state and artifacts<br/><small>registry, native home, logs, backups</small>"]:::state
    ...
    SANDBOX -.->|"persist native home"| STATE
...
| Managed state and artifacts | Preserve non-policy registry records and the complete OpenShell-provided native home/workspace during replacement; OpenShell alone stores sandbox policy and credentials. |
...
OpenShell-native state : Routine lifecycle operations keep the native home/workspace in OpenShell storage. Rebuild and recreation transfer that complete tree without a NemoClaw path allowlist; OpenShell-owned credential stores remain outside it.
...
NemoClaw operates the sandbox through host-side commands while OpenShell owns its persistent native home and workspace.
...
| Rebuild migration | Transfers the complete native home and workspace to a replacement sandbox; OpenShell-owned credential storage remains outside the transfer. |
...
Refer to [Recover and Rebuild Sandboxes](../manage-sandboxes/operate-sandboxes/recover-and-rebuild-sandboxes) and [Persist Sandbox State](../manage-sandboxes/state-and-backups/create-and-restore-snapshots) for lifecycle details.
