...
## Configure a Headless Validation Profile

Arbitrary shell execution remains disabled whether or not a validation profile is installed.
Without a profile, headless `dcode -n` has no command executor.
An operator can install one immutable profile with up to 32 exact validation commands during a transactional rebuild.
The profile applies only to headless `dcode -n` runs and does not change interactive approval behavior.

Create a JSON document that follows `schemas/dcode-validation-profile.schema.json` in the NemoClaw source tree.
Each command records one exact absolute executable and argument vector, one fixed working directory, allowed environment variable names, a timeout, a combined output limit, and an invocation limit.
The profile records environment variable names only; the runtime checks inherited values when it executes a command.
Loader, interpreter, package-manager, Git, OpenShell, network-trust, and other process-control environment names are not allowed.
`HOME` and `PATH` may appear in the profile for clarity, but the runtime always replaces them with fixed managed values instead of inheriting them.
The `sandboxName` must match the rebuild target.
Use a stable task identifier for `taskIdentity`.
Set `sourceIdentity` to `sha256:` followed by the SHA-256 digest of the ASCII string `git:<object-format>:<full-HEAD-object-id>`.
For example, a SHA-1 repository at commit `0123...cdef` uses the digest of `git:sha1:0123...cdef`, not the commit ID by itself.
Set `contentDigest` to the SHA-256 digest of the compact JSON object after removing `contentDigest` and recursively sorting object keys.

Install the profile from a normalized absolute host path.
The host file must be regular, must not be a symbolic link, and must be no larger than 64 KiB.

```bash
nemo-deepagents <sandbox-name> rebuild \
  --dcode-validation-profile /absolute/path/validation-profile.json \
  --yes
```

NemoClaw validates the profile before it changes the existing sandbox, bakes the canonical profile into a root-owned image file, and records the same profile in the sandbox registry.
Before every execution, the managed runtime starts a fail-closed mutation guard across the Git work tree, recomputes `sourceIdentity` from the working directory's Git `HEAD`, and rejects tracked changes and untracked files that Git does not ignore.
The guard remains active through process exit.
If the guard detects a file creation, write, metadata change, rename, or deletion during verification or execution, the runtime terminates the command and returns `source_identity_mismatch`.
The runtime also recomputes the identity after exit before it can report success.
The receipt records the initially verified identity separately.
It also rejects repository-local hooks, filesystem monitors, and external diff, filter, or merge drivers so source verification cannot invoke repository-configured programs.
It then admits only an exact profiled argument vector and executes the no-follow, descriptor-bound executable from the descriptor-bound working directory directly with `shell=False`.
Near matches, shell metacharacters, alternate executables, environment assignments, symbolic-link path escapes, excess output, timeouts, and exhausted invocation budgets fail closed.
Executable paths must identify root-owned regular files that are not symbolic links or writable by group or other users.
Working directories must exist without symbolic-link resolution and remain inside a declared root.
Invocation reservations use root-owned, sticky write-once claims and apply across concurrent and separate headless DCode processes for the installed profile digest.
The sandbox user can consume a remaining slot but cannot remove or replace a used slot; attempts to corrupt a slot fail closed.
If process creation fails, the runtime releases the reservation; a launched command consumes it regardless of its terminal result.
The child receives only the named non-credential environment values plus fixed `HOME` and `PATH` values.
OpenShell network, filesystem, process, and credential policies continue to apply.

The tool result is a bounded JSON receipt, not command output, with:

- Profile, declared source, verified source, argument, stdout, and stderr digests.
- The non-secret task identity, command identifier, and working directory from the validated profile.
- Terminal status, duration, byte counts, and exit code.

It never includes stdout, stderr, credentials, or environment values.
`verifiedSourceIdentity` is `null` when a request is rejected before Git source verification completes.
Use the host status command before execution to inspect the effective profile and digest in machine-readable form.

```bash
nemo-deepagents <sandbox-name> status --json
```

Later rebuilds retain the recorded profile.
Remove the capability explicitly with another transactional rebuild.

```bash
nemo-deepagents <sandbox-name> rebuild --dcode-validation-profile disabled --yes
```
