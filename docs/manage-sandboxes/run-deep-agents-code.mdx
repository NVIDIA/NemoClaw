...
- Deep Agents Model Context Protocol (MCP) commands and project MCP auto-loading
- ACP mode
- Native LangSmith tracing and ambient OpenTelemetry exporter configuration

Interactive sessions retain Deep Agents Code's native local behavior for shell allowlists, startup commands, interpreter and programmatic-tool settings, and user hooks.
These processes run as the sandbox user inside OpenShell and receive the supported secret-free runtime environment.
OpenShell preserves the complete native home, and managed rebuild transfers it without interpreting `hooks.json` or selected `config.toml` keys.
...
MCP servers registered through `nemo-deepagents <sandbox-name> mcp add` remain available through the agent-native `/sandbox/.deepagents/.mcp.json` configuration and OpenShell egress policy.
...
<a id="protect-the-managed-login-profile"></a>

### Edit Personal Shell Files

You can edit personal shell files under `/sandbox`, including `.bashrc`, `.profile`, and `.bash_profile`.
Ordinary login and interactive shells use your personal files.

Managed probes use `/etc/profile.d/nemoclaw-dcode.sh`, owned by `root:root` with mode `0444`.
The hook clears `BASH_ENV` and `ENV` and selects an image-owned home before Bash reads personal files.
The managed launcher restores `HOME=/sandbox` and reads proxy settings from image-owned files.
...
Automatic and unrestricted approval modes remain unavailable unless you enable the managed thread opt-in capability.
Interactive shell allowlists use Deep Agents Code's native approval behavior.
...
The `thread-opt-in` setting grants permission to use Deep Agents Code's native approval modes, but it does not activate one by itself.
Use `dcode -y` or **Enable Auto for this thread** for classifier-backed Auto mode.
Use the native `--yolo` or `/yolo` path for unrestricted approval after its warning and acknowledgement.
The TUI shows the upstream active-state indicator for the selected mode.

Deep Agents Code preserves its native approval state across `/clear`, `/force-clear`, thread resume or switch, and agent switch operations.
A new process starts from the native `[startup].mode` setting; NemoClaw forces Manual when the sandbox capability is `disabled`.
