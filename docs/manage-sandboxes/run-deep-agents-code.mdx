...
If the sandbox has legacy MCP configuration, preview it with `nemo-deepagents <sandbox-name> mcp migrate`.
Apply it with `nemo-deepagents <sandbox-name> mcp migrate --apply`; migration rebuilds the sandbox for the native MCP runtime.
Ordinary rebuild stops while legacy MCP state remains.
If the older runtime has no legacy MCP state, rebuild before adding native MCP servers.
