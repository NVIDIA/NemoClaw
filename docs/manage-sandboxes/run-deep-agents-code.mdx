...
- ACP mode, interpreter tool calling, and shell allow-list overrides
...
For authenticated MCP setup and credential rotation, see [Add an MCP Server](../mcp-servers/add-an-mcp-server) and [Manage MCP Servers](../mcp-servers/manage-mcp-servers).
