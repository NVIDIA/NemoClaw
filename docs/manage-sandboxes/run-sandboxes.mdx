...
<AgentOnly variant="openclaw,hermes">
After Docker reports the existing container as running, NemoClaw waits for OpenShell to report the sandbox in the `Ready` phase.
...
<AgentOnly variant="deepagents">
After Docker reports the existing container as running, NemoClaw waits for OpenShell to report the sandbox in the `Ready` phase, then verifies the managed terminal runtime before the command reports success.
</AgentOnly>
