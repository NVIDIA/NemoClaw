...
Sandbox-scoped commands that read, exec into, or inspect a sandbox, such as `$$nemoclaw <name> exec` and `$$nemoclaw <name> status`, address each sandbox through its recorded gateway binding, so the current shell's `NEMOCLAW_GATEWAY_PORT` does not redirect them. Lifecycle commands such as `$$nemoclaw <name> stop` still act on the gateway that `NEMOCLAW_GATEWAY_PORT` selects.
...
If you intentionally run separate OpenShell gateways on the same host, set a different `NEMOCLAW_GATEWAY_PORT` before each onboarding run.
NemoClaw isolates the gateway name and local state by port so one port-specific gateway does not replace another.
Sandbox-scoped commands that read, exec into, or inspect a sandbox, such as `nemo-deepagents <name> exec` and `nemo-deepagents <name> status`, address each sandbox through its recorded gateway binding, so the current shell's `NEMOCLAW_GATEWAY_PORT` does not redirect them. Lifecycle commands such as `nemo-deepagents <name> stop` still act on the gateway that `NEMOCLAW_GATEWAY_PORT` selects.
A non-default `NEMOCLAW_GATEWAY_PORT` also gets its own host state root at `~/.nemoclaw/gateways/<port>/`, with a separate sandbox registry, snapshots, and legacy credential-migration files, so gateway-scoped state stays segregated while shared host-level files remain under `~/.nemoclaw/`.
On first use after upgrading, NemoClaw moves legacy rows and related state only when their recorded gateway identity matches the selected port; ambiguous state is left untouched with remediation.
Provider credentials remain in the OpenShell gateway store.
The default port keeps the shared `~/.nemoclaw/` location.
...
The host-side dashboard forward listens on all interfaces (`0.0.0.0`) only when `NEMOCLAW_DASHBOARD_BIND=0.0.0.0` is set for a sandbox created with that opt-in, or when the host is WSL. `CHAT_UI_URL` does not change the bind. Other hosts may then reach the dashboard at this host's address, subject to the host firewall. A later forward recovery, such as `connect` or `recover` re-creating a missing forward, selects the bind the same way from its own environment. NemoClaw records the bind each time it starts a dashboard forward. `$$nemoclaw <name> dashboard-url` and `$$nemoclaw list` report that recorded bind, and `$$nemoclaw <name> status` omits its SSH port-forward guidance while the recorded bind is `0.0.0.0`. Leave `NEMOCLAW_DASHBOARD_BIND` unset to keep the forward on loopback.
