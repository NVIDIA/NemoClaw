...
A `CHAT_UI_URL` whose host is not loopback makes onboarding start the host-side dashboard forward on all interfaces (`0.0.0.0`). Other hosts may then reach the dashboard at this host's address, subject to the host firewall. A later forward recovery, such as `connect` or `recover` re-creating a missing forward, does not read `CHAT_UI_URL`. It re-creates the forward on all interfaces only when that environment sets `NEMOCLAW_DASHBOARD_BIND=0.0.0.0` for a sandbox prepared for remote exposure, or when the host is WSL. Otherwise it re-creates the forward on loopback. NemoClaw records the bind each time it starts a dashboard forward. `$$nemoclaw <name> dashboard-url` and `$$nemoclaw list` report that recorded bind, and `$$nemoclaw <name> status` omits its SSH port-forward guidance while the recorded bind is `0.0.0.0`. Leave `CHAT_UI_URL` unset, or give it a loopback host, to keep the forward on loopback.
