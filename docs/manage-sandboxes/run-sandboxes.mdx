...
For lower-level diagnostics, list legacy and user-managed OpenShell forwards separately.
NemoClaw verifies its managed service forwards through `nemoclaw <name> status` and `recover`.
