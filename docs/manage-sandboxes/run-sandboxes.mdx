...
Release validates the unchanged target authority and persists both the pre-release and final released receipts before it clears the fence.
It never starts or recovers the sandbox.
Run `$$nemoclaw my-assistant start` separately when you intend to resume execution.
If release cannot validate the target or persist either receipt, it returns a non-zero result and keeps the fence active for retry.
