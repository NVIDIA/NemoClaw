...
After Docker reports the existing container as running, NemoClaw runs the selected agent's readiness checks.
<AgentOnly variant="openclaw,hermes">
NemoClaw repairs and verifies the in-sandbox gateway and host forwards before the command reports success.
</AgentOnly>
<AgentOnly variant="deepagents">
NemoClaw verifies the managed terminal runtime before the command reports success.
</AgentOnly>
