...
A `CHAT_UI_URL` whose host is not loopback makes the host-side dashboard forward listen on all interfaces (`0.0.0.0`), at onboarding and at every later forward recovery, so other hosts may reach the dashboard at this host's address, subject to the host firewall. `$$nemoclaw <name> dashboard-url`, `$$nemoclaw list`, and `$$nemoclaw <name> status` report that bind. Leave `CHAT_UI_URL` unset, or give it a loopback host, to keep the forward on loopback.
