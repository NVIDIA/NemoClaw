...
Release validates the unchanged target authority and preserves the receipt before it clears the fence.
It never starts or recovers the sandbox.
Run `$$nemoclaw my-assistant start` separately when you intend to resume execution.
If release cannot validate the target or preserve the pre-release receipt, it returns a non-zero result and keeps the fence active.
