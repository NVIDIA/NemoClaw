...
A later onboarding run for another sandbox does not tear down the first sandbox's gateway or dashboard forward.
</AgentOnly>

<AgentOnly variant="openclaw">
If re-onboarding finds the registered dashboard port already bound, NemoClaw briefly stops and starts the same ready OpenClaw sandbox to verify its dashboard forward.
If the port remains bound after the sandbox stops, resolve the listener conflict, run `nemoclaw <sandbox-name> start`, then retry onboarding.
For other failures, follow the reported recovery steps.
</AgentOnly>
