...
<AgentOnly variant="openclaw">
Re-onboarding the same ready OpenClaw sandbox briefly stops and starts it to verify the registered dashboard forward.
If verification leaves the sandbox stopped, resolve the listener conflict.
Run `nemoclaw <sandbox-name> start`, then retry onboarding.
</AgentOnly>
