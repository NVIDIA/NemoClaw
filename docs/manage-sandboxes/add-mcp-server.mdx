...
For a public DNS hostname, one validated response must contain at least two public addresses so NemoClaw can pin the complete returned set.
...
Use an endpoint whose DNS lookup returns its complete public address set in one response.
...
DNS rotation beyond the complete set returned during public preflight remains unpinned.
