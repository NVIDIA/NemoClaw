...
Before an MCP add or restart mutation, NemoClaw requires the Hermes helper to report reconcile-finality capability version 1.
An add or restart with an older helper stops with rebuild guidance and leaves the provider, policy, attachment, and native configuration unchanged.
</AgentOnly>
<AgentOnly variant="openclaw">
OpenClaw stores MCP servers in `openclaw.json` under `mcp.servers`.
To continue using or updating an existing Mcporter entry, follow [Migrate an Existing Registration](manage-mcp-servers#migrate-an-existing-registration).
To remove an entry instead, `$$nemoclaw my-sandbox mcp remove <server>` supports direct removal when its committed registry row proves ownership and the removal preconditions are met.
Refer to [Remove a Server](manage-mcp-servers#remove-a-server).
</AgentOnly>

An interrupted add that does not report an unknown outcome is retried from the current agent and OpenShell sources.
A deterministic provider name lets the retry recognize a partially created provider without a host-side transaction marker.
The retry resumes only when the native entry, generated policy, provider identity and credential key, and sandbox attachment form an exact prefix of the requested add; conflicting or ambiguous resources remain unchanged and stop the command.

<AgentOnly variant="hermes">
If Hermes loses its OpenShell exec relay during the managed reload, NemoClaw first proves whether the transaction committed.
It returns success for proven committed state and follows normal rollback for proven absence.
If neither state can be proven, NemoClaw reports an unknown outcome without rollback or another mutation.
Do not retry, force-remove, or remove and re-add the server while the outcome is unknown.
Follow [Hermes Add Outcome Is Unknown](../../reference/troubleshoot-mcp-servers#hermes-add-outcome-is-unknown).
</AgentOnly>
...
One normalized endpoint URL can have only one credential-bound managed server definition in a sandbox.
If two definitions need different credentials, retain one definition for that URL or configure distinct endpoint URLs so OpenShell can select one credential unambiguously.
...
Cloudflare Quick Tunnels and other public endpoints can rotate their address sets.
After each change, follow [Change Endpoint Pins](manage-mcp-servers#change-endpoint-pins) to refresh the validated public pins explicitly.
