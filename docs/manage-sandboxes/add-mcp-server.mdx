...
The generated egress policy is binary-scoped. It permits the selected agent's adapter binaries to reach the MCP endpoint, but it does not permit `/usr/bin/curl`. A failed `curl` request from an interactive sandbox shell therefore does not show that the MCP route is broken. Use the managed status probe to verify policy readiness, in-sandbox endpoint reachability, and credential resolution:

```bash
$$nemoclaw my-sandbox mcp status github
```

The managed probe sends a bounded MCP `initialize` request from inside the sandbox through the selected agent adapter. Adapter binaries differ between OpenClaw, Hermes, and Deep Agents Code, so do not substitute a generic shell command for this check.
