...
Managed MCP also verifies that the selected host OpenShell gateway was launched from the current private configuration with `proxy_connect_by_hostname = false`.
An older running gateway without launch-bound configuration identity is refused before MCP mutation.
Rerun `$$nemoclaw onboard` to replace a NemoClaw-owned host gateway, or restart an externally managed gateway through its declared supervisor, then retry the MCP command.
For a public DNS hostname, preflight must return at least two public addresses.
NemoClaw pins the addresses from that response.
NemoClaw rejects a singleton public DNS response because it cannot distinguish a stable singleton from one member of a rotating answer set.
Explicitly trusted private hostnames use one DNS lookup and retain exact host-bound address pins.
A trusted private IPv4 literal requires no DNS lookup.
Preflight does not refresh `allowed_ips` during a live connection or replace address pins with hostname-only trust.
...
Until a successful `mcp restart` or rebuild restoration refreshes the public pins, any later DNS answer outside the addresses returned during public preflight remains unpinned and fails closed.
Cloudflare Quick Tunnels and other endpoints that return only one rotating address or later change beyond that set are not durable managed MCP deployments.
