...
Hermes stores the stable credential handle from the readiness check in the OpenShell resolver placeholder.
Registration, inspection, rollback, and lifecycle reconciliation preserve that handle while it remains observable.
</AgentOnly>
...
The generated egress policy is binary-scoped. It permits the selected agent's Node.js adapter to reach the MCP endpoint, but it does not permit `/usr/bin/curl`. A failed `curl` request from an interactive sandbox shell therefore does not show that the MCP route is broken. Use the managed status probe to verify both policy readiness and credential resolution:

```bash
$$nemoclaw my-sandbox mcp status github
```

To test only unauthenticated network reachability from inside the sandbox, use the allowed Node.js runtime and inspect the HTTP status without printing a response body:

```bash
node -e 'fetch("https://api.githubcopilot.com/mcp/").then(r => console.log(r.status)).catch(e => { console.error(e.message); process.exitCode = 1 })'
```

An authentication rejection can still prove that the Node.js request reached the endpoint. Only the managed status probe verifies that OpenShell resolved the stored credential.
...
NemoClaw records the denied-tool list and includes it in every generated policy comparison.
Restart and rebuild preserve the deny rules without a manual policy edit.
