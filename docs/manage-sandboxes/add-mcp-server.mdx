...
Hermes stores MCP servers in its native `config.yaml`. Registration and inspection preserve the credential revision from the readiness check in the OpenShell resolver placeholder without comparing it with host-side desired state.
</AgentOnly>
...
An interrupted add is retried from the current agent and OpenShell sources. A deterministic provider name lets the retry recognize a partially created provider without a host-side transaction marker.
...
An exported replacement updates it during restart. `mcp remove` detaches the provider, and sandbox destroy may leave the detached provider for explicit operator cleanup.
