...
If a public hostname's first DNS response contains one address, NemoClaw takes two more validated snapshots and pins the sorted union from all three responses.
If the first public response contains multiple addresses, NemoClaw uses that response without another lookup.
NemoClaw rejects a public preflight if either additional snapshot is unresolved or contains a private, special-use, or otherwise disallowed address.
Explicitly trusted private targets use one DNS lookup and retain exact host-bound address pins.
