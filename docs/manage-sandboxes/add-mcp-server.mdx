...
If a public hostname's first DNS response contains one address, NemoClaw takes two more validated responses and pins the sorted union from all three responses.
If the first public response contains multiple addresses, NemoClaw uses that response without another lookup.
NemoClaw rejects a public preflight if either additional response is unresolved or contains a private, special-use, or otherwise disallowed address.
...
These preflight lookups do not refresh `allowed_ips` during a live connection or replace address pins with hostname-only trust.
...
DNS rotation beyond the addresses observed during public preflight remains unpinned.
Cloudflare Quick Tunnels and other endpoints that change beyond that set are not durable managed MCP deployments.
