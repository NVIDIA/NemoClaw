...
NemoClaw persists only the variable name, writes a revision-scoped placeholder such as `openshell:resolve:env:v<revision>_KEY` into sandbox-side MCP config, and relies on OpenShell to resolve the placeholder at egress.
...
The sandbox configuration contains only a revision-scoped OpenShell credential placeholder, such as `openshell:resolve:env:v<revision>_LOCAL_MCP_TOKEN`.
