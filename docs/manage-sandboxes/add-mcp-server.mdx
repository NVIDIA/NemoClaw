...
The generated egress policy is binary-scoped.
It permits the selected agent's adapter binaries to reach the MCP endpoint, but it does not permit `/usr/bin/curl`.
