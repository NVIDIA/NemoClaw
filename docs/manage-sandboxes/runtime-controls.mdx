...
| Hermes plugin code, Langfuse settings, or other startup-only runtime config | Runtime after a supported host-side update and gateway restart | Use a custom Dockerfile for plugin code or use a supported host config command, then run `$$nemoclaw <name> gateway restart` |
