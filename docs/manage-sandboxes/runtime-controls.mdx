...
| --- | --- | --- |
| Inference provider | Runtime route update; configuration update only when the sandbox user can write `/sandbox/.openclaw/openclaw.json` | Run `$$nemoclaw inference set` |
| Inference model on the current provider | Runtime route update; configuration update only when the sandbox user can write `/sandbox/.openclaw/openclaw.json` | Run `$$nemoclaw inference set` |
...
| `agents.list` | Runtime after gateway restart; generated OpenClaw configuration disables hot reload | Prefer agent or NemoClaw commands that keep host and sandbox state aligned, then run `$$nemoclaw <name> gateway restart` |
...
If NemoClaw skips custom-image configuration synchronization, update the image so the sandbox user owns and can write the OpenClaw configuration, then recreate the sandbox.
Refer to [Switch Models](../../inference/manage-inference/switch-models) for the custom-image route and configuration behavior.
