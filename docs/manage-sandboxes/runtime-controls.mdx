...
During restoration, NemoClaw reconciles the current exact managed MCP entries into the saved policy.
A removed MCP server stays removed, while an unrelated surviving server keeps its recorded endpoint and address pins.

If the recorded owner exits before releasing its lock generation, NemoClaw leaves Shields down and records permanent containment.
New mutations remain blocked so an untracked descendant cannot change the sandbox after restoration.
