...
This table maps each commonly changed item to the layer that owns it and the command that changes it.
...
If a row above conflicts with what you observe, the runtime source of truth for Hermes is `/sandbox/.hermes/config.yaml` plus `/sandbox/.hermes/.env`.
