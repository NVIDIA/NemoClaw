...
OpenClaw owns its native gateway restart.
In supported Linux OpenShell sandboxes, restart replaces the Node process image while preserving its process ID, so changed plugin modules load without recreating the sandbox.
NemoClaw then verifies gateway health and checks or recovers host-side forwards.
Refer to [Recover and Rebuild Sandboxes](../operate-sandboxes/recover-and-rebuild-sandboxes) for the command and recovery guidance.
