...
Those cases, retained gates, and fail-closed state-directory errors keep new mutations blocked until you complete generation operator recovery.
...
For a fail-closed case, stop all NemoClaw processes for the sandbox, then follow the paths, identities, tokens, and removal order in the reported error.
