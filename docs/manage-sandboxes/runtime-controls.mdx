...
A live or ambiguous owner, a replaced or unrelated generation, conflicting artifacts, or a missing protected generation remains fail-closed.
Those conditions and retained gates keep new mutations blocked until the reported recovery completes.
Wait for recovery to finish before running any sandbox-scoped command, including a `channels` or `policy` command with `--dry-run`.
...
If the error instead reports invalid or conflicting artifacts, a replaced or unrelated lifecycle generation, or a missing protected generation, stop all NemoClaw processes for the sandbox and follow the paths, identities, tokens, and removal order in that error.
...
Then rerun `$$nemoclaw <name> shields status`.
