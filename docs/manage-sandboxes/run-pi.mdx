...
Rebuild validates the recorded workload authority, resolves the current release's exact Pi image, and preserves declared Pi state. It also replays the recorded context window, maximum output-token count, and reasoning flag.
...
After a NemoClaw update, inspect registered sandboxes, then rebuild Pi to consume a newer reviewed package and image.
...
Each Pi update requires a new exact package integrity, dependency review, image digest, and qualification result. Pi runtime self-update and package auto-update remain unsupported.
