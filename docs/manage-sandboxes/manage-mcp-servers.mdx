...
Before it changes an adapter, policy, or provider, NemoClaw captures the complete live policy for the replacement.
It refuses this MCP rebuild handoff when the policy contains a literal credential value.
Replace literal credentials with supported OpenShell credential bindings or resolver placeholders, then retry rebuild.
After this check, rebuild removes adapter entries and exact owned policies, then detaches providers before replacing the sandbox.
