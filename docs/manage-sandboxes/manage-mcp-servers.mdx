...
JSON output reports the same value and both address sets in `publicTarget` or `trustedPrivateTarget`.
Status never adds a new address to the policy.
For public drift, run `$$nemoclaw my-sandbox mcp restart <server>` to validate, apply, and record the current public answers.
For trusted-private drift, review the address change and follow [Change Endpoint Pins](#change-endpoint-pins).
