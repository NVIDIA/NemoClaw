...
`remove` detaches the exact managed provider and preserves the provider object. After confirming no sandbox uses that provider, delete it using the exact name reported by `remove`:
