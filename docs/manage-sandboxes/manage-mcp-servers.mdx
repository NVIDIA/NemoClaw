...
After upgrading, restart or rebuild restoration rejects an existing public MCP server when its current DNS response contains only one public address. The command stops before policy, provider, or adapter mutation because it cannot distinguish a stable singleton from one member of a rotating answer set.
