...
When DNS returns one address that is already in a multi-address recorded set, restart retains the complete recorded set instead of widening or shrinking the policy from an incomplete response. The success message reports that choice.
After a successful public restart, NemoClaw records the activated address set as the server's current pins.
...
### Recover a Legacy Public Single-Pin Registration

After upgrading, restart or rebuild restoration rejects a legacy public MCP server with only one recorded pin when its current DNS response also contains one public address. The command stops before policy, provider, or adapter mutation because it cannot establish a complete recorded answer set. A server with at least two valid recorded public pins can instead retain that set when the singleton response is one of those pins.

Record the server URL and credential-variable name. Remove the affected server, then add it again after the endpoint returns at least two public addresses in one DNS response:

```bash
export MCP_TOKEN='replace-with-secret-manager-value'
...
NemoClaw pins only the addresses from that response. Any later DNS answer outside those pins fails closed. If you are retiring the sandbox instead of recovering the server, `$$nemoclaw <sandbox> destroy` remains available.
