...
The diagnostic starts with `Unsafe managed Deep Agents MCP projection path:` and names a type such as `symbolic link`, `FIFO`, or `non-regular file`.
For this unsafe path-type result, the command exits with status `2`.
