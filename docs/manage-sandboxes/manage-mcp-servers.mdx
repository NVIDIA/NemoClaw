...
Policy deletion still requires owned content.
Force never claims an unowned or drifted provider or same-key live policy.

For an ordinary managed entry, NemoClaw cleans up the adapter, removes the owned policy, and then detaches its provider because OpenShell rejects detach while `credential_binding.provider` still references that provider.
