...
`mcp list`, `status`, `add`, `update`, `restart`, and `remove` use agent configuration and live OpenShell state.
If the sandbox registry is missing or corrupt, these commands require a verified gateway and live sandbox and do not recreate the registry.

<AgentOnly variant="openclaw">

Without a usable registry entry, OpenClaw configuration changes and gateway reloads require a local Docker gateway owned by NemoClaw in the `default` workspace.
NemoClaw must verify its scoped namespace, container identity, loaded configuration, and Linux process environment.
Unavailable or inconsistent evidence stops the command.
Registered sandboxes keep their existing execution path.
...
`mcp migrate` stops when the registry is corrupt because it cannot inspect committed legacy intent.
When the registry is absent, migration does not create it.

<AgentOnly variant="deepagents">

Deep Agents migration still requires a registered sandbox for its rebuild workflow.
...
Migration writes the native configuration atomically, reloads or rebuilds the agent, verifies native enumeration, fresh credential readiness, and authenticated tool discovery, and only then retires the legacy adapter or projection. OpenClaw must activate the new Tool Search catalog before legacy cleanup, and Deep Agents removes the migrated entry from `.deepagents/.nemoclaw-mcp.json` before reporting success.

If migration fails before legacy cleanup starts, it attempts to remove newly created native entries and retains the legacy configuration for retry. Rollback can leave native entries when their removal fails. Once legacy cleanup starts, migration preserves the verified native entries and any remaining legacy configuration, so a cleanup failure can leave a partially migrated state. The existing OpenShell policy, provider, and attachment remain available. Run the migration preview again and inspect the reported sources before retrying `mcp migrate --apply`.
...
Export the replacement under the same host environment name used by `mcp add`, then remove the native registration:
...
`remove` preserves the provider object for explicit cleanup.
...
Add the server again to create a fresh provider binding:
...
Running restart without a server name validates and reloads every managed server at its current credential revision. It does not rotate credentials.
...
Delete the retained provider using the [credential rotation cleanup](#rotate-a-credential) above, then re-add the server:
...
Removing the server removes its generated policy key, detaches the exactly identified provider, and verifies attachment and credential cleanup before deleting the agent-native entry. The provider itself is preserved for explicit cleanup. The re-add performs a new DNS preflight and records the reviewed exact address set.
...
`remove --force` may remove a modified same-name native entry after policy, attachment, and credential cleanup is proven. It never claims or deletes an ambiguous OpenShell provider.

Policy removal targets the deterministic current MCP policy key. Force never claims an unrelated policy or provider.

NemoClaw refuses conflicting native configuration and live policy before changing either source. It verifies the live policy binding, provider ID, type, and credential key, then removes the policy key before detaching that exact provider. OpenShell requires this order because a policy cannot reference a detached provider. NemoClaw verifies that no attachment carries the MCP credential key and that fresh sandbox processes no longer receive it before removing the native registration. The workspace provider is retained for explicit cleanup.

If policy, attachment, or credential cleanup cannot be proven, the command fails and retains the native registration for recovery, including with `--force`. A retry after policy removal does not infer provider ownership from its name. If an attachment still carries the MCP credential key, inspect and detach it explicitly before retrying `mcp remove`, even if fresh processes no longer receive the credential. NemoClaw never detaches the provider from another sandbox or silently reports that the provider was deleted.
...
An ordinary Deep Agents rebuild stops before deletion if legacy entries remain in agent configuration or the old registry.
This includes configurations that also contain native entries.
Preview the entries with `$$nemoclaw my-sandbox mcp migrate`, then apply them with `$$nemoclaw my-sandbox mcp migrate --apply`.
Explicit migration carries the validated legacy servers through rebuild into native `.deepagents/.mcp.json`.
Other native servers and unrelated top-level content remain unchanged.
If a rebuild is already incomplete, follow [Continue an Interrupted Replacement](../operate-sandboxes/recover-and-rebuild-sandboxes#continue-an-interrupted-replacement).
Use the printed `rebuild --yes` recovery command before starting migration again.
...
If MCP restoration remains incomplete after replacement, NemoClaw retains the rebuild journal and source-derived MCP handoff.
Fix the reported cause.
Run `$$nemoclaw my-sandbox rebuild --yes` to resume restoration into the accepted replacement sandbox.

## Destroy a Sandbox with MCP State

Destroy reads the current agent and OpenShell sources before deletion but does not scrub a separate MCP registry or write destroy markers.
OpenShell deletes the sandbox-scoped policy and attachments with the sandbox.
NemoClaw preserves workspace-level MCP providers and reports names available from source inspection.
If the source inventory is unavailable, destroy continues without reporting provider names.
Before explicit cleanup, verify each provider's identity and confirm that no sandbox uses it.
Do not infer ownership from a provider name.
An interrupted destroy is retried from current sandbox presence; no local MCP marker authorizes cleanup.
