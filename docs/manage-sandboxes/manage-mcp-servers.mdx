...
The `commandStatus` field records the sandbox discovery runtime's exit status. It is `null` when no runtime exit status exists, such as a failed precondition or an unreachable sandbox.
