...
If the path is a symbolic link or FIFO, the command prints `Unsafe managed Deep Agents MCP projection path: symbolic link` or `Unsafe managed Deep Agents MCP projection path: FIFO`, writes no status output, and exits with status `2`.
