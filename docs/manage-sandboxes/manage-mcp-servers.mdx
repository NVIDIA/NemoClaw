...
Before it reads `/sandbox/.deepagents/.nemoclaw-mcp.json`, Deep Agents Code v2 `mcp status` checks the path type.
It does not follow a final symbolic link or read from a FIFO.
If the path is a symbolic link, FIFO, or another non-regular file, the command names the path type, writes no status output, and exits with status `2`.
The diagnostics are `Unsafe managed Deep Agents MCP projection path: symbolic link`, `Unsafe managed Deep Agents MCP projection path: FIFO`, and `Unsafe managed Deep Agents MCP projection path: non-regular file`.
Other Deep Agents inspection failures can also exit with status `2`.
Use the standard error diagnostic to distinguish an unsafe path from another inspection failure.
Deep Agents Code v1 reads the legacy `.deepagents/.mcp.json` path and does not expose these type-specific diagnostics.
