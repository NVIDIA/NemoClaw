...
When status inspects a managed Deep Agents v2 MCP projection, it rejects a symbolic link, FIFO, or other non-regular file at `/sandbox/.deepagents/.nemoclaw-mcp.json`.
The command exits with status `2` and does not report a healthy server status.
</AgentOnly>
