...
`remove --force` may remove a modified same-name native entry after credential and policy cleanup is proven. It never claims or deletes an ambiguous OpenShell provider.

Policy removal targets the deterministic current MCP policy key. Force never claims an unrelated policy or provider.

NemoClaw refuses conflicting native configuration and live policy before changing either source. It detaches a provider only when the live policy binding, provider ID, type, and credential key agree, then verifies that fresh sandbox processes no longer receive the credential. It removes the policy key and finally the native registration. If the policy is already absent, it preserves providers and still requires credential absence before removing an authenticated native entry. The workspace provider is retained for explicit cleanup.

If credential or policy cleanup cannot be proven, the command fails and retains the native registration for recovery, including with `--force`. Inspect the reported policy or provider state, correct the cause, and retry `mcp remove`. It never detaches the provider from another sandbox or silently reports that the provider was deleted.
