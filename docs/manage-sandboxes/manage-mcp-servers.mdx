...
For Deep Agents, `mcp status` exits with status `2` when the managed projection path is a symbolic link, FIFO, or another non-regular file. It prints the unsafe type and no healthy server status.
