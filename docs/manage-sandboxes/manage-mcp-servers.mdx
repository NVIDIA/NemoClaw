...
The command changes only the generated OpenShell policy and persisted denied-tool intent.
It does not change the endpoint, credential, address pins, provider, or agent adapter.
For a legacy public registration without recorded address pins, it first resolves and records the current validated pins.
NemoClaw removes the current generated policy before it records and applies the replacement intent.
If replacement policy activation fails, the command reports the failure, preserves that intent, and leaves the MCP route blocked.
Run `$$nemoclaw my-sandbox mcp restart doordash` to retry policy activation.
