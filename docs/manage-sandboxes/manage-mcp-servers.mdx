...
The per-server `warnings` array reports unsupported stored boundaries, public and trusted-private DNS pin changes, and credential-resolution findings. The `env.missing` field lists recorded host variable names that are currently unset.
...
Status resolves public and trusted private endpoints without changing managed state.
Text output reports `public address pins:` or `private address pins:` with `match`, `drift`, or `unresolved`.
Public endpoints that resolve to private or special-use addresses report `rejected`.
NemoClaw reports `unresolved` when a public DNS lookup exceeds 2 seconds and leaves its live pins unchanged.
JSON output reports the same value, recorded pins, and validated current pins when available in `publicTarget` or `trustedPrivateTarget`.
Status never adds a new address to the policy.
For public or trusted-private drift, follow [Change Endpoint Pins](#change-endpoint-pins).
...
Use only one update mode per command: `--deny-tool`, `--clear-deny-tools`, or `--refresh-public-pins`.

## Change Endpoint Pins

When a public server changes addresses, refresh its existing exact pins explicitly:

```bash
$$nemoclaw my-sandbox mcp update github --refresh-public-pins
$$nemoclaw my-sandbox mcp status github --tools
```

The refresh validates the current DNS answers and replaces only `allowed_ips` on the existing MCP endpoint in the live OpenShell policy.
It preserves denied tools, methods, binary grants, credential bindings, and every other policy field.
Agent configuration, providers, and attachments remain unchanged, and NemoClaw retains no separate MCP pin record.
Status, restart, rebuild, and restore do not refresh these pins.

Refresh requires an existing public endpoint with exact public pins and matching agent and policy sources.
It rejects private or special-use DNS answers and policies with address ranges.
Correct a reported source conflict before retrying.
If the write cannot be confirmed, inspect `mcp status` and the live OpenShell policy before retrying.
Successful tool discovery verifies that the endpoint is reachable through its existing credential binding.
