...
Before it reads `/sandbox/.deepagents/.nemoclaw-mcp.json`, Deep Agents Code `mcp status` checks the path type without following a symbolic link or opening a FIFO.
