...
<AgentOnly variant="hermes">
When you stop Discord, NemoClaw retains its exact credential-bound provider because the stopped channel policy still references it.
The stopped Discord runtime does not start, and NemoClaw does not create a replacement provider from the available bot token.
</AgentOnly>
