...
If deletion fails after detachment, NemoClaw first attempts to restore the provider attachment so the configured channel can keep delivering.
If restoration also fails, the command names the provider and sandbox, warns that delivery may remain interrupted, and leaves the channel's credentials, registry entry, and policy in place for a retry.
