...
<AgentOnly variant="hermes">
For stopped Hermes Discord, a preserved credential-bound policy requires the validated static provider, so the rebuild retains and attaches only that provider without starting Discord or recreating its credentials.
