...
`channels remove googlechat` disables the channel, records removal as pending, removes its agent configuration and matching policy preset, and then detaches and deletes the `<sandbox>-googlechat-bridge` provider.
