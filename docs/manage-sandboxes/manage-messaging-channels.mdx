...
During recovery, this exception applies only when WeChat no longer appears in either the messaging plan or the current OpenShell policy.
NemoClaw then treats state cleanup as complete without inspecting a volume if the registry entry is missing, the driver is not Docker, or no eligible stopped container exists.
</AgentOnly>
