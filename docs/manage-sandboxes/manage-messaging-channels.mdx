...
Before NemoClaw reuses a bridge provider, it verifies the recorded provider binding and the checked-in OpenShell provider-profile boundary. If either check fails, NemoClaw stops before it changes the provider. If replacement-token minting fails for an existing refresh bridge, the provider retains its working credential and the command exits nonzero.
