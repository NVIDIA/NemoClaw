...
Slack conflicts exit nonzero without an interactive confirmation.
Pass `--force` to `channels add slack` only when you explicitly accept the duplicate-session risk.
...
For non-Slack channels, if NemoClaw has only legacy metadata and cannot compare credential hashes, it keeps the conservative warning.
Slack aborts when it cannot complete the conflict check unless you pass `--force` to `channels add slack`.
