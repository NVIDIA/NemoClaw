...
A registry or Microsoft Teams host-port conflict leaves the original sandbox intact.
...
An unverified required webhook forward makes the rebuild fail without deleting the replacement sandbox.
...
Preserve the registry and onboarding state for maintainer-assisted recovery if the check still fails.
