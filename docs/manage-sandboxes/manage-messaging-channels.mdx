...
NemoClaw applies the same fail-closed conflict policy to every supported messaging channel.
The policy checks for these conditions:

- Another registered sandbox uses the same channel credential.
- A channel-owned pre-enable check finds a shared resource conflict.
- NemoClaw cannot complete a required conflict check.

Onboarding, rebuild, and `channels add` exit nonzero when a required check reports a conflict.
Onboarding and rebuild do not have a conflict override.
Pass `--force` to `channels add <channel>` only when you explicitly accept the duplicate-consumer or shared-resource risk.

Channel manifests define the credential and resource checks for each channel.
The built-in checks include these channel-specific rules:

- Each active sandbox must use distinct credentials for a credential-backed channel.
- WeChat requires a distinct iLink `accountId` for each active sandbox.
- Slack checks both bot and Socket Mode app tokens across gateways.
- Slack also requires each active sandbox to use a different gateway.
- Microsoft Teams requires each active sandbox to use a different local webhook port.

If NemoClaw has only legacy metadata and cannot compare credential hashes, it treats the active channel as a conflict.
If onboarding or rebuild cannot read the messaging registry, the operation aborts.
If `channels add` cannot complete a required check, it aborts unless you pass `--force`.
