...
Slack sandboxes must also use different gateways.
...
If another sandbox already uses the same credential, onboarding, rebuild, and `channels add` exit nonzero without an interactive confirmation.
Channel-owned pre-enable conflicts that declare abort behavior also exit without an interactive confirmation.
Pass `--force` to `channels add <channel>` only when you explicitly accept the duplicate-consumer or shared-resource risk.
For Slack, NemoClaw checks both bot and Socket Mode app tokens across gateways.
It also rejects another Slack sandbox on the same gateway, even when the credentials differ.

For Microsoft Teams, NemoClaw also checks the local webhook port because active Teams sandboxes cannot share it.
If NemoClaw has only legacy metadata and cannot compare credential hashes, it treats the active channel as a conflict and aborts.
If onboarding or rebuild cannot read the messaging registry required for a conflict check, it aborts; neither operation has a conflict override.
If `channels add` cannot complete a required conflict check, it also aborts unless you pass `--force`.
