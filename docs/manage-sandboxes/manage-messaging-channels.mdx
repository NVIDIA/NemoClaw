...
If the channel plan is unavailable and NemoClaw cannot verify the live policy, `shields down` stops without changing the live policy.
...
Paste the displayed public HTTPS URL into the HTTP endpoint field in **Google Chat API** > **Configuration** > **Connection settings**.
Confirm the prompt, rebuild the sandbox, and retry `shields down`.
</AgentOnly>
