...
The WhatsApp adapter reads the paired session once while the gateway starts, and the rebuild restores that session afterwards.

In bot mode the sandbox replies to the listed senders and stops replying to the paired account's self-chat.
Bot mode rejects every sender when the allowlist is empty, so set both values together.
