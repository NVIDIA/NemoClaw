...
After the repair, `$$nemoclaw <sandbox> channels status --channel whatsapp` finds the default gateway session path empty and then checks the configured session path, so the split warning clears.
The report adds a `Session path override` line that states the gateway session check used the path set in `platforms.whatsapp.extra.session_path`.
Before removing the override, run `$$nemoclaw <sandbox> channels status --channel whatsapp` after dashboard pairing and gateway startup.
Confirm that the report no longer shows the `Session path override` line, which means the default gateway session path now holds the credentials.
