...
<AgentOnly variant="hermes">
Rebuild restore retires the reserved legacy `profiles/dashboard-home` directory while preserving the shared `platforms/whatsapp` session.
On an older running sandbox, use `$$nemoclaw <sandbox> channels remove whatsapp` to clear every compatibility session path before pairing again.
</AgentOnly>
