...
<AgentOnly variant="hermes">
Hermes can pair from dashboard-home while the gateway reads the default `platforms/whatsapp/session` path.
...
```bash
$$nemoclaw <sandbox> config set --key platforms.whatsapp.extra.session_path --value /sandbox/.hermes/dashboard-home/platforms/whatsapp/session --restart --config-accept-new-path
```
...
This is a bounded compatibility repair for Hermes dashboard pairing that writes session credentials under dashboard-home while the gateway still reads the default session path.
