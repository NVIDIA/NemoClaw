...
<AgentOnly variant="hermes">
Rebuild preserves the shared `platforms/whatsapp` session and native Hermes profiles, including an older `profiles/dashboard-home` profile.
Use `$$nemoclaw <sandbox> channels remove whatsapp` to clear every compatibility session path before pairing again.
</AgentOnly>
