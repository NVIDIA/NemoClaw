...
Current managed images migrate a safe legacy `profiles/dashboard-home` session into the shared native path during startup.
If `channels status` still reports credentials only under the legacy path, the sandbox is running an older image or migration could not complete.
...
`channels remove` clears `/sandbox/.hermes/platforms/whatsapp` and compatibility sessions left by older images at `/sandbox/.hermes/profiles/dashboard-home/platforms/whatsapp/session` and `/sandbox/.hermes/dashboard-home/platforms/whatsapp/session`.
...
<AgentOnly variant="hermes">
Rebuild preserves the shared `platforms/whatsapp` session and user-created native Hermes profiles.
An older `profiles/dashboard-home` session is migrated into the shared path during startup and the retired profile is removed.
Use `$$nemoclaw <sandbox> channels remove whatsapp` to clear every compatibility session path before pairing again when migration reports a conflict.
</AgentOnly>
