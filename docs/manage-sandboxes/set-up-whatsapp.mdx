...
If you paired an existing sandbox before this shared-path fix, `channels status` can report credentials only under the legacy `profiles/dashboard-home` path.
Update or rebuild the sandbox, then pair again from Dashboard so the credentials are written to `/sandbox/.hermes/platforms/whatsapp/session`.
Run `channels status` again to confirm that the gateway path contains the new credentials.
</AgentOnly>
