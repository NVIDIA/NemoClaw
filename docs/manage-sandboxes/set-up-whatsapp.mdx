...
This is a bounded compatibility repair for Hermes dashboard pairing that writes session credentials under `profiles/dashboard-home` while the gateway still reads the default session path.
