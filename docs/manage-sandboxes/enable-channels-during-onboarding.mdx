...
The environment is the only place a channel token is read from, so no separate credential removal is needed.
`$$nemoclaw credentials reset` takes an OpenShell provider name and does not change what onboarding detects.
