...
Clearing a channel's host environment variables and rerunning onboarding does not remove an active host-backed channel when every recorded credential binding still matches its OpenShell gateway provider.
...
If a recorded binding is missing or no longer matches the expected provider, onboarding disables the channel and drops the matching policy preset.
...
The command removes the durable channel selection, stored credentials, gateway provider, and matching network policy preset, then prompts you to rebuild the sandbox.
