...
Clearing a channel's host environment variables and rerunning onboarding does not remove an active host-backed channel when every recorded credential binding still matches its OpenShell gateway provider or its gateway-minted bridge provider remains exact.
