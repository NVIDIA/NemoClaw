...
description-agent: "Explains the interactive and scripted onboarding flows for selecting messaging channels, creating OpenShell bridge providers, preserving reusable channels, and explicitly removing a channel. Use when enabling or disabling channels during onboarding."
...
Clearing a channel's host environment variables does not remove an active host-backed channel when its recorded OpenShell gateway provider still matches the channel's credential contract.
...
Use `channels remove` for an explicit, durable removal:

```bash
$$nemoclaw my-assistant channels remove discord
```

The command removes the durable channel selection, stored credentials, gateway provider, and matching network policy preset.
Interactive use then prompts you to rebuild the sandbox.
Noninteractive use queues the rebuild; run `$$nemoclaw my-assistant rebuild` to apply it.
For an in-sandbox QR-paired channel, the command also clears its session state before teardown.
Refer to [Manage Messaging Channels](manage-messaging-channels) for channel-specific teardown effects and recovery guidance.
...
- [Choose Messaging Channels](messaging-channels) for provider requirements.
