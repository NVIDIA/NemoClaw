...
It collects the inputs the channel needs, validates or registers the matching OpenShell provider profile, records the channel in the sandbox registry, and asks whether to rebuild immediately.
...
It flags `gateway-providers` as residual because an in-flight update can leave the gateway with the new token.

For a refresh bridge such as Google Chat, NemoClaw reuses an existing provider only when its provider binding and checked-in provider-profile boundary match.
It does not replace the existing credential with a pending value before gateway token minting succeeds.
If minting fails, the command exits nonzero and leaves the existing provider credential usable.
