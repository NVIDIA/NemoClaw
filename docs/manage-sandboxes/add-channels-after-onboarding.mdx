...
<Warning>
  ...
  Replacement is allowed only when the provider is unattached or attached only to the selected
  sandbox. An attachment to another sandbox stops the operation.
</Warning>
...
Inspect the channel's gateway providers, restore the preset YAML, correct the gateway failure, and rerun the original command.
