...
It cannot guarantee the prior bridge provider state.
An incompatible provider can be removed or changed during replacement, and refresh material available only during the command cannot be restored.
NemoClaw flags `gateway-providers` as residual.
Inspect each named provider, restore the preset YAML, correct the gateway failure, and rerun the original command.
