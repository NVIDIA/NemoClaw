...
Before NemoClaw registers or reuses a channel bridge profile, it checks the complete checked-in OpenShell profile contract.
A missing, malformed, or conflicting profile stops the operation before provider reuse.
When a refresh bridge already matches, it keeps its working credential until the gateway mints a replacement.
If refresh fails, NemoClaw leaves the existing bridge available.
