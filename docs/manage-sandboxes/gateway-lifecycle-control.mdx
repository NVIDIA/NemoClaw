...
OpenClaw owns its native `gateway restart` operation.
In supported Linux OpenShell sandboxes, the native restart replaces the Node process image while preserving its process ID, so updated plugin modules load without recreating the sandbox.
NemoClaw then verifies gateway health and checks or recovers host-side forwards.
The recovery controllers described below apply to `recover`.
Refer to [Recover and Rebuild Sandboxes](../operate-sandboxes/recover-and-rebuild-sandboxes) for restart instructions.

</AgentOnly>

Built-in OpenClaw and Hermes images support two direct-container recovery topologies.
Hermes also uses these controllers for `gateway restart`.
...
For `recover` and Hermes `gateway restart`, the managed controller acquires the expected-exit lock before it inspects the supervisor or gateway.
...
For built-in OpenClaw and Hermes controllers, a successful `recover` response supplies the initial authenticated gateway-health proof.
Hermes `gateway restart` uses the same controller proof.
...
The host selects the matching controller automatically for `recover` and Hermes `gateway restart`.
