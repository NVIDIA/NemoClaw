...
The managed controller waits for an active expected-exit lock for the existing recovery timeout before it returns `SUPERVISOR_BUSY` for a concurrent lifecycle request.
