...
For controller-backed built-in recovery, a successful `recover` or `gateway restart` response supplies the initial authenticated gateway-health proof.
...
<AgentOnly variant="hermes">
After an authenticated gated recovery request, NemoClaw observes the replacement Hermes gateway through the sandbox health endpoint over OpenShell.
It uses the health timeout from the Hermes agent manifest and repeats the check after the shared settle window.
This gated recovery check does not require the retired NemoClaw managed gateway controller.
