...
---
Built-in OpenClaw images support two direct-container lifecycle topologies for `recover` and `gateway restart`.
...
Hermes configuration remains mutable. Before a forced restart, NemoClaw runs the Hermes gateway wrapper as a non-mutating preflight so the existing secret-boundary checks can reject unsafe configuration before any lifecycle mutation.
It then targets the OpenShell gateway recorded for the sandbox, revalidates the registered and live sandbox identity around each lifecycle step, and asks OpenShell to stop and start the complete sandbox. After OpenShell starts it, NemoClaw proves native gateway health and repairs the sandbox's declared host forwards.

The OpenShell stop and start RPCs currently identify a sandbox by mutable name rather than durable sandbox ID. NemoClaw must fail closed when it cannot bind those mutations to the verified live identity; an identity change is never authorization to operate on the replacement.

Before gateway launch, the supervisor validates the secret boundary and records one stable config snapshot in its transaction metadata.
The native `mcp_servers` content in that snapshot remains authoritative, including direct edits. No separate host-side MCP registry or intended/applied marker participates in restart eligibility.
...
For the built-in OpenClaw controller, a successful `recover` or `gateway restart` response supplies the initial authenticated gateway-health proof.
...
<AgentOnly variant="hermes">
For Hermes, OpenShell sandbox start is followed by a native gateway-health probe before forward recovery or success is reported.
The newly started nonroot Hermes supervisor continuously repairs the gateway, API relay, dashboard, dashboard relay, and gateway log stream.
...
Hermes host restart does not authorize or signal one gateway-child exit: OpenShell stops and starts the complete sandbox, which replaces the supervisor instance and resets that instance's crash budget.
