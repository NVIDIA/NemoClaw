...
For OpenClaw sandboxes, Teams setup also installs a runtime message-tool hint patch that tells the agent to mention users with `@[Display Name](AAD_OBJECT_ID)` so the OpenClaw Teams plugin can emit Teams-native mention entities even when optional skills are not loaded.
