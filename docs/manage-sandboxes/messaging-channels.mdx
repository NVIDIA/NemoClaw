...
OpenClaw owns `/sandbox/.openclaw/openclaw.json`, and NemoClaw restores the complete credential-sanitized native config across rebuilds.
OpenShell credential bindings and policy remain host-owned.
Use the host-side channel commands because an in-sandbox config change cannot create the matching OpenShell credential binding or policy.
</AgentOnly>
<AgentOnly variant="hermes">
Use host-side `$$nemoclaw <sandbox> channels` commands.
