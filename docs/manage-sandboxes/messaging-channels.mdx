...
Use host-side `$$nemoclaw <sandbox> channels` commands.
Use Hermes `/sethome` inside a chat to select its home channel.
A rebuild preserves the matching `*_HOME_CHANNEL`, `*_HOME_CHANNEL_NAME`, and `*_HOME_CHANNEL_THREAD_ID` routing assignments.
Do not edit `/sandbox/.hermes/.env` directly.
NemoClaw regenerates other messaging settings at image build time, and direct changes do not persist across rebuilds.
</AgentOnly>
