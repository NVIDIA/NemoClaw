...
| Microsoft Teams (experimental) | `MSTEAMS_APP_ID`, `MSTEAMS_APP_PASSWORD`, `MSTEAMS_TENANT_ID` | `TEAMS_ALLOWED_USERS` for direct-message Microsoft Entra ID object ID allowlisting, `TEAMS_GROUP_POLICY` and `TEAMS_GROUP_ALLOWED_USERS` for OpenClaw group and channel sender allowlisting, `MSTEAMS_PORT` for the local webhook port, `TEAMS_REQUIRE_MENTION` for OpenClaw group and channel mention mode |
...
Set `TEAMS_GROUP_POLICY=allowlist` and `TEAMS_GROUP_ALLOWED_USERS=<entra-object-id>[,<entra-object-id>]` to render `channels.msteams.groupPolicy="allowlist"` and `channels.msteams.groupAllowFrom` in `openclaw.json`.
If `TEAMS_GROUP_POLICY=allowlist` is set without `TEAMS_GROUP_ALLOWED_USERS`, NemoClaw keeps the rendered group policy open because OpenClaw treats an empty group allowlist as open.
...
export TEAMS_GROUP_POLICY=allowlist
export TEAMS_GROUP_ALLOWED_USERS=<your-entra-object-id>
...
If you need optional channel settings such as `TELEGRAM_ALLOWED_IDS`, `TELEGRAM_REQUIRE_MENTION`, `TELEGRAM_GROUP_POLICY`, `DISCORD_SERVER_ID`, `DISCORD_USER_ID`, `DISCORD_REQUIRE_MENTION`, `SLACK_ALLOWED_USERS`, `SLACK_ALLOWED_CHANNELS`, `TEAMS_ALLOWED_USERS`, `TEAMS_GROUP_POLICY`, `TEAMS_GROUP_ALLOWED_USERS`, `MSTEAMS_PORT`, or `TEAMS_REQUIRE_MENTION`, export them before the rebuild starts.
...
MSTEAMS_APP_ID=<your-teams-app-id> \
  ...
  TEAMS_GROUP_ALLOWED_USERS=<your-entra-object-id> \
  MSTEAMS_PORT=3978 \
  $$nemoclaw my-assistant channels add teams
```
