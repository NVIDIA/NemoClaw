...
Stock onboarding carries the selected channel configuration in the managed startup profile and materializes it when the sandbox starts.
An explicit custom Dockerfile applies the same configuration during its local build.
Runtime delivery remains under OpenShell control.
...
Do not run `openclaw channels add` or `openclaw channels remove` inside the sandbox because NemoClaw materializes `/sandbox/.openclaw/openclaw.json` from host-managed startup configuration, and direct changes inside the running container do not persist across rebuilds.
</AgentOnly>
<AgentOnly variant="hermes">
Use host-side `$$nemoclaw <sandbox> channels` commands.
Do not mutate messaging configuration directly inside the sandbox because NemoClaw materializes `/sandbox/.hermes/.env` and Hermes config from host-managed startup configuration, and direct changes inside the running container do not persist across rebuilds.
</AgentOnly>
