...
For OpenClaw sandboxes, Teams setup also installs a `msteams` agent skill that tells the agent to mention users with `@[Display Name](AAD_OBJECT_ID)` so the OpenClaw Teams extension can emit Teams-native mention entities.
