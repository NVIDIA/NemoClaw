...
<AgentOnly variant="openclaw">
When `TELEGRAM_GROUP_POLICY=open`, NemoClaw writes the selected mention mode explicitly so `TELEGRAM_REQUIRE_MENTION=0` does not fall back to OpenClaw's mention-only runtime default.
NemoClaw does not summarize one global mention mode when `TELEGRAM_GROUP_POLICY` is `allowlist` or `disabled` because group access is blocked or controlled per group before mention handling.
</AgentOnly>
<AgentOnly variant="hermes">
For Hermes, NemoClaw maps `TELEGRAM_REQUIRE_MENTION` to `telegram.require_mention`, and `channels status` and `doctor` compare the stored setting with that rendered value.
</AgentOnly>
...
For Telegram, use the channel status and doctor commands to verify the group mention mode against the rendered agent configuration:
...
When NemoClaw can reach the sandbox and read its rendered configuration, both commands report `Telegram group mention mode (TELEGRAM_REQUIRE_MENTION)` as `mention-only (1)` or `all-messages (0)`.
If the stored setting and rendered configuration differ, the check reports a warning with the expected and rendered modes.
Rebuild the sandbox with `$$nemoclaw my-assistant rebuild` to regenerate the channel configuration after you correct the setting.
