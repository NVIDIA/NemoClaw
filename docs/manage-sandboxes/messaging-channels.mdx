...
You can enable channels during `nemohermes onboard` or add them later with host-side `nemohermes <sandbox> channels` commands.
...
<AgentOnly variant="hermes">
If applying the preset fails, NemoClaw warns and tells you to re-apply manually with `nemohermes <sandbox> policy-add <channel>` after the rebuild.
</AgentOnly>
