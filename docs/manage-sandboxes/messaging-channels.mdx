...
For compatibility with older QA scripts, NemoClaw also accepts `TELEGRAM_AUTHORIZED_CHAT_IDS` and `TELEGRAM_CHAT_ID`, but new automation should use `TELEGRAM_ALLOWED_IDS`.
NemoClaw tracks these aliases in messaging compatibility metadata; maintainers should remove them manually after QA automation and public templates have stopped exporting them for at least one full release.
...
Set `MSTEAMS_APP_ID`, `MSTEAMS_APP_PASSWORD`, and `MSTEAMS_TENANT_ID`; `TEAMS_CLIENT_ID` and `TEAMS_TENANT_ID` are also accepted for the non-secret app ID and tenant ID.
