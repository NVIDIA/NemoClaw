...
Do not run `openclaw channels add` or `openclaw channels remove` inside the sandbox.
A direct change to `/sandbox/.openclaw/openclaw.json` can survive snapshot restore, but it does not update NemoClaw's messaging plan, OpenShell providers, or network policy.
</AgentOnly>
<AgentOnly variant="hermes">
Use host-side `$$nemoclaw <sandbox> channels` commands.
