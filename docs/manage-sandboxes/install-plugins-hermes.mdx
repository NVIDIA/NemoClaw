...
The dependency does not modify the root virtual environment, and the managed target is not declared in the generated `.env` file.
...
The sandbox user can modify this dependency tree, and gateway startup validates the supported client version before loading it.
The directory is outside the generated `.env`, `config.yaml`, and `.config-hash` files.
