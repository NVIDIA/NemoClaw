...
The image keeps the root virtual environment and sealed configuration unchanged.
...
The `local-memory` preset permits the Hermes Python runtime to call `host.openshell.internal` on port `8888`.
...
The first plugin load installs `hindsight-client==0.6.1` under `/sandbox/.hermes/lazy-packages`.
...
A rebuild recreates the lazy-install directory.
The first Hindsight plugin load after the rebuild reinstalls the supported client in that directory.
