...
<AgentOnly variant="deepagents">
  ...
  want to pick up a new base image without losing the complete native home, use `rebuild`
...
<AgentOnly variant="deepagents">
  The rebuild command transfers the complete native home, including `config.toml`, `.mcp.json`,
  hooks, skills, memories, histories, packages, and unknown files. It also passes the complete
  current OpenShell policy to replacement creation.
</AgentOnly>
...
When an incomplete rebuild transfer contains captured data, NemoClaw retains it at the printed path for manual recovery but never treats it as complete.
The retained archive may contain agent-written secrets.
...
Remove the incomplete host-side transfer only after you verify that the original sandbox or a later complete backup contains every required state item.
