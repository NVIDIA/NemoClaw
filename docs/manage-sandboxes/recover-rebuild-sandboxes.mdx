...
If the sandbox has only an inert legacy state record, create a trusted snapshot or backup and rebuild it before running further mutations. If NemoClaw reports an old timer, transition, policy snapshot, or runtime-provider recovery artifact, stop any older NemoClaw process or reboot the host before continuing. Onboard a current-image replacement under a new, unregistered sandbox name, then use the [manual transfer workflow](../state-and-backups/transfer-state-manually) to copy only trusted user data. NemoClaw leaves unattributed provider artifacts in place for recovery review while it creates that distinct replacement; operations on an existing sandbox remain blocked when a recovery artifact is attributable to it. Do not use cross-sandbox snapshot restore from the legacy sandbox because that flow preserves the source image. The current CLI deliberately does not interpret, resume, or delete ambiguous legacy recovery state.
