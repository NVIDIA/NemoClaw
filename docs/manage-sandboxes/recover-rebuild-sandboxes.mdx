...
When an incomplete backup contains captured entries, NemoClaw retains them at the printed path for manual recovery, but excludes that backup from `snapshot list` and snapshot restore selection.
Do not edit its manifest or try to make it selectable.
Repair access to the original sandbox state, then rerun the rebuild.
Remove the incomplete host backup only after you verify that the original sandbox or a later complete snapshot contains every required file.
