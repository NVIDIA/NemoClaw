...
For a remote inference provider, preflight checks the selected credential's non-secret expiration metadata from OpenShell.
An expired credential or expiration metadata that NemoClaw cannot verify stops rebuild before backup or deletion, including with `--force`.
