...
For a local Docker-driver sandbox whose container still uses the legacy keepalive startup, `recover` can restart the credential-free managed workload in the registered container.
NemoClaw keeps the container identity and preserves the existing writable-layer state, then requires managed gateway health and OpenShell `Ready` before starting the primary dashboard or API host forward.
A definitive managed-health failure stops immediately; if readiness does not complete within the configured budget, the forward stays stopped.
