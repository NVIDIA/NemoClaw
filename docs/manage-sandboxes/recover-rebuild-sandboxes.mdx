...
It reuses a messaging provider only when its exact type and credential keys match the recorded channel binding and its checked-in OpenShell provider-profile boundary.
