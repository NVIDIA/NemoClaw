...
It reuses a messaging provider only when its type and credential keys match the recorded channel binding.
...
A policy-required provider attachment is the narrow exception: when stopped Hermes Discord retains a credential-bound policy, rebuild attaches its validated static provider without starting Discord.
