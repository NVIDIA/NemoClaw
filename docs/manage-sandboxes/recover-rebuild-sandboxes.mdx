...
<AgentOnly variant="openclaw,hermes">
  ...
  a non-paused container that Docker still associates with the registered sandbox, and it leaves a
  running or paused container unchanged. If OpenShell cannot start the sandbox, recovery starts the
  container directly so it is running again, and `start` repairs the phase on the next run. If
  neither start succeeds, recovery continues to the OpenShell readiness check and reports the
  resulting failure.
</AgentOnly>
