...
If OpenShell reports the sandbox in `Phase: Error` but Docker no longer has its driver container,
run `$$nemoclaw <sandbox-name> rebuild --yes` to recreate the container while preserving workspace state.
If the sandbox is absent from the live OpenShell gateway, destroy the stale NemoClaw entry and
onboard a clean replacement. Restore a separately created snapshot afterward if one is available;
rebuild cannot recover a workspace or live OpenShell policy that no longer exists.
...
use `$$nemoclaw <sandbox-name> rebuild --yes` to recreate the container with workspace state
preserved. This recovery does not apply when the sandbox itself is absent from the live OpenShell
gateway; use the clean replacement flow described above for that case.
