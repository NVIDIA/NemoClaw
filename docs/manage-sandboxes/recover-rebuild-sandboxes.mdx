...
An explicit `DOCKER_HOST` remains the sole Docker daemon authority, so NemoClaw ignores conflicting `DOCKER_CONTEXT` and `DOCKER_CONFIG` values for the build.
Otherwise, an explicit custom Dockerfile continues to use your configured Docker credentials because its base image or build steps might require a private registry.
If that custom rebuild cannot reach the credential helper or private registry, restore access for the selected Docker daemon before retrying.
