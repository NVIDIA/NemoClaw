...
When an active Portable Hermes sandbox uses the `ollama-local` provider and a published receipt-owned Ollama runner, use one of these commands to recover it:
...
For another Portable Hermes provider, `recover` and `connect --probe-only` verify the recorded inference route and required forwards without requiring an Ollama receipt or starting an Ollama runner.
