...
Use `rebuild` to move a managed sandbox to the exact managed image for the installed NemoClaw release while preserving the state declared by the agent manifest.
Before mutation, NemoClaw verifies the sandbox's recorded runtime provider and platform, resolves one complete all-agent managed-image catalog, and binds the replacement to the selected agent's exact image digest.
The replacement preserves the provider-bound managed workload and applies a freshly rendered startup profile before NemoClaw restores the declared state.
If the provider authority, catalog, image contract, or startup profile fails validation, rebuild stops before deleting the original sandbox.
A sandbox created with `--from` remains on the legacy Dockerfile rebuild path and uses its recorded custom Dockerfile inputs.
