...
They do not restart, replace, or reap the shared host OpenShell gateway process.
...
Only the onboarding path and `nemoclaw <name> gateway restart` are permitted to reap or replace the shared host gateway.
</AgentOnly>
