...
<AgentOnly variant="openclaw">
  ...
  policy into sandbox recreation. For that one replacement create, NemoClaw adds filesystem paths
  and baseline network entries required by the current replacement image when they are absent from
  the live policy. Existing keys and same-name live entries win, so host changes are not overwritten.
...
For that one replacement create, NemoClaw adds filesystem paths and baseline network entries required by the current replacement image when they are absent from the live policy. Existing keys and same-name live entries win, so host changes are not overwritten. NemoClaw does not save a desired-policy copy or enforce that handoff after OpenShell creates the sandbox.
