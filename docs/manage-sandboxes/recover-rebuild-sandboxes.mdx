...
If status reports `sandbox_container_stopped`, a missing sandbox, or an unhealthy gateway, run the recover command instead of opening a shell.

```bash
$$nemoclaw <sandbox-name> recover
```

The command repairs a stopped in-sandbox gateway and re-establishes the dashboard port-forward in one step. It is idempotent and safe to script. If the gateway is already healthy, `recover` exits after the probe and does not restart it. If the host forward is already active, recovery accepts it only after OpenShell ownership is reconciled and the local endpoint is reachable. If a listener that NemoClaw cannot attribute to the sandbox's own OpenShell forward holds the port, `recover` reports it and leaves it running. It then exits non-zero and does not restore the forward. Legacy SSH forward records do not establish ownership. Stop the owning OpenShell gateway or service to release the listener before retrying recovery.
...
If `$$nemoclaw <name> status` reports `Phase: Stopped` after an intentional stop, run `$$nemoclaw <name> start`.
If status reports `sandbox_container_stopped`, a missing sandbox, or an unhealthy Hermes gateway, run the recover command instead of opening a shell.
...
After Portable `start` completes lifecycle recovery, it checks and repairs the recorded host forwards before reporting success.
Portable Hermes records the selected OpenShell gateway during onboarding.
Recovery and uninstall use that recorded gateway, including a non-default gateway selected through `NEMOCLAW_GATEWAY_PORT`.
If one required forward fails after recovery starts another, NemoClaw terminates only forwards started by that recovery attempt.
It preserves existing forwards and exits nonzero when it cannot prove cleanup.

When the saved OpenShell phase is `Stopped`, `recover` reconciles it through OpenShell even if the receipt-owned container is already running.
It does not mutate a saved `Error` state or a `Ready` state with a non-running container.
Preserve the startup diagnostics and resolve the reported OpenShell state before you retry.

### Recover Portable Published Ollama

When an active Portable Hermes sandbox uses `ollama-local` and a published receipt-owned Ollama runner, use one of these commands to recover it:
...
For another configured provider, `recover` and `connect --probe-only` verify the recorded inference route and check and repair required forwards without requiring or starting an Ollama runner.
...
Save both results.
The expected status line is `Saved Portable lifecycle phase: active`.
Status also prints `Runtime and agent health: not probed`.
...
Before recreation, NemoClaw backs up the state directories and files declared by the agent manifest, then stops the original sandbox through OpenShell.
It retains the previous container while it verifies the replacement identity and managed health, restores state, and restarts and verifies the gateway.
State restoration runs as the sandbox user through direct-container execution pinned to the replacement; it does not wait for OpenShell re-registration.

After these checks pass, NemoClaw stops the replacement through OpenShell and removes the previous container.
That removal commits the swap and makes automatic rollback unavailable.
NemoClaw then starts the replacement through OpenShell and requires the final handoff and readiness checks to pass before restoring host forwards.
Mounted state remains available, but a committed swap does not retain other writable-layer changes.

If replacement health, state restoration, or gateway verification fails before commit, NemoClaw attempts rollback and leaves the primary dashboard or API host forward stopped.
After the previous container is removed, a handoff or readiness failure leaves that forward stopped and requires the printed host recovery guidance.
NemoClaw removes the temporary state backup after a confirmed final handoff or rollback.
If it cannot confirm either outcome, it retains the backup.
If rollback is unconfirmed, inspect Docker state before retrying recovery.

After the final handoff, the additional OpenShell readiness wait defaults to 120 seconds.
...
Refer to the [probe timing reference](../../reference/commands#$$nemoclaw-name-connect) for observation counts, readiness decisions, and Portable Hermes diagnostics.
...
<AgentOnly variant="openclaw,hermes">
  Prepared backup recovery reuses its digest-verified pre-upgrade policy handoff at the deletion boundary.
  It does not recapture the live policy, and it stops before deletion if the handoff is missing or changed.
</AgentOnly>
...
<AgentOnly variant="deepagents">
  ...
  carries the current agent-native `.mcp.json` entries through a bounded in-memory handoff, and passes the complete current OpenShell policy to
...
<Warning title="Forced Rebuild Scope">
  ...
  When the sandbox has MCP servers, NemoClaw must read the agent-native MCP source before any destructive step. `--force` does not bypass an unavailable, unsafe, or conflicting source.
  NemoClaw rechecks that source-derived bridge snapshot immediately before deletion and stops if the source, target, policy, provider, or recorded gateway changed.
  ...
  NemoClaw preserves the bounded MCP handoff and rebuild-recovery record, does not attempt to stop NIM, and does not claim that the original sandbox is intact.
