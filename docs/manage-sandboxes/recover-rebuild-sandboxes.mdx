...
The restart command asks OpenClaw to perform its native gateway restart.
In supported Linux OpenShell sandboxes, the native restart replaces the Node process image while preserving its process ID, so updated plugin code loads without recreating the sandbox.
NemoClaw verifies listener and HTTP health, then checks or recovers host-side dashboard, messaging, and agent forwards.
