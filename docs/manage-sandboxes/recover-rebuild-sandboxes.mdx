...
also reconciles a sandbox that OpenShell still reports as `Stopped` instead of treating provider
state as independent lifecycle authority. NemoClaw does not directly restart or unpause the
underlying provider container for a standard sandbox. If OpenShell can no longer identify or start
the sandbox, destroy the stale NemoClaw entry and onboard a clean replacement.
