...
For a local Docker-driver sandbox whose registered container uses the legacy keepalive startup, `recover` can launch the managed supervisor in that container.
...
NemoClaw requires managed gateway health and uses `NEMOCLAW_SANDBOX_READY_TIMEOUT` (180 seconds by default) to prove OpenShell readiness before starting the primary dashboard or API host forward.
