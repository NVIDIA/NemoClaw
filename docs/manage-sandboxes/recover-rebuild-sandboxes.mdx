...
NemoClaw treats a journaled replacement as void when the sandbox registry row and the live same-name sandbox report the same OpenShell identity, and that identity is not the journaled replacement.
The rerun then reports `Discarded the void replacement journal for '<name>'; its source sandbox is still registered and live.` and records a fresh journal against that live source.
