...
`start` also repairs a sandbox that OpenShell still reports as `Stopped` while its container runs,
which can happen when something started the container outside OpenShell. It reports that it is
starting the sandbox through OpenShell rather than treating the running container as already
started.
...
<AgentOnly variant="openclaw,hermes">
  `recover` can start an existing stopped Docker-driver sandbox before it repairs the agent
  runtime. It starts the sandbox through OpenShell, which restarts the same container and also
  advances the sandbox out of the `Stopped` phase that the readiness check waits on. It starts only
  a non-paused container that Docker still associates with the registered sandbox. If that container
  is already running while OpenShell reports `Stopped`, `recover` still starts the sandbox through
  OpenShell to repair the phase. It leaves paused containers and running sandboxes in other phases
  unchanged. If OpenShell cannot start a stopped container, recovery starts the container directly
  so it is running again, and `start` repairs the phase on the next run. If neither start succeeds,
...
Ordinary `connect` can resume the runner when its recorded inference route is unavailable.
When `launch` runs its complete preflight, it uses the same recovery path before it opens a session.
...
If the native supervisor remains absent after bounded recovery, NemoClaw leaves the container unchanged and directs you to rebuild or re-onboard.
