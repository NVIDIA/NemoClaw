...
<AgentOnly variant="openclaw,hermes">
  Prepared legacy gateway recovery has one exception. When the recorded gateway lists the retained
  sandbox but cannot render its old provider-backed specification, NemoClaw reuses the
  digest-verified pre-upgrade policy handoff. It does not recapture the live policy at the deletion
  boundary, and it stops before deletion if the handoff is missing or changed.
</AgentOnly>
