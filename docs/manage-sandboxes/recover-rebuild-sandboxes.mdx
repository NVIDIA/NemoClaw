...
If NemoClaw reports an old timer, transition, policy snapshot, or runtime-provider recovery artifact attributed to the affected sandbox, stop every older NemoClaw process or reboot the host before continuing. Onboard a current-image replacement under a different, unregistered sandbox name—not the affected sandbox's name—then use the [manual transfer workflow](../state-and-backups/transfer-state-manually) to copy only trusted user data. Operations on the affected sandbox remain blocked while its recovery artifact remains.

If the retirement notice reports unattributed transition or provider recovery state, do not onboard any sandbox yet. Stop every older NemoClaw process or reboot the host, preserve the ambiguous artifacts for recovery review, and resolve their ownership outside NemoClaw before any further sandbox mutation. Because the artifact can belong to a deleted sandbox whose name may be reused, a different requested name does not make onboarding safe.
