...
<AgentOnly variant="openclaw">
  ...
  policy into sandbox recreation. For that one replacement create, NemoClaw adds only filesystem
  access paths required by the current replacement image when they are absent from the live policy.
  It does not add replacement network entries, save a desired-policy copy, or enforce that handoff
  after OpenShell creates the sandbox. When no host web-search key is staged, rebuild preflight
...
For that one replacement create, NemoClaw adds only filesystem access paths required by the current replacement image when they are absent from the live policy. It does not add replacement network entries, save a desired-policy copy, or enforce that handoff after OpenShell creates the sandbox.
