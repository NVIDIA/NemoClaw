...
<AgentOnly variant="openclaw">
Use `rebuild` to move a stock sandbox to the current managed OpenClaw image and startup profile without losing its managed workspace files.
For an explicit custom-image sandbox, use the same command after changing its Dockerfile or base image.
</AgentOnly>
<AgentOnly variant="hermes">
Use `rebuild` to move a stock sandbox to the current managed Hermes image and startup profile without losing its managed state files.
For an explicit custom-image sandbox, use the same command after changing its Dockerfile or base image.
</AgentOnly>
<AgentOnly variant="deepagents">
Use `rebuild` to move a stock sandbox to the current managed Deep Agents Code image and startup profile, or to enable Tavily Search, without losing manifest-defined Deep Agents state.
For an explicit custom-image sandbox, use the same command after changing its Dockerfile or base image.
</AgentOnly>
