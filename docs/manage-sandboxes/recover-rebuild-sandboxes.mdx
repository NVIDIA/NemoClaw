...
If NemoClaw reports an old timer, transition, policy snapshot, or runtime-provider recovery artifact, stop every older NemoClaw process or reboot the host before continuing. Onboard a current-image replacement under a different, unregistered sandbox name—not the affected sandbox's name—then use the [manual transfer workflow](../state-and-backups/transfer-state-manually) to copy only trusted user data. NemoClaw leaves unattributed provider artifacts in place for recovery review while it creates that distinct replacement; operations on an existing sandbox remain blocked when a recovery artifact is attributable to it.
