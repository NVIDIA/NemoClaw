...
run `$$nemoclaw <sandbox-name> status`. The status and connect recovery path can restore an
OpenShell-missing standard Docker sandbox only when its exact labeled Docker-driver container
still remains. If that check cannot identify or recover the sandbox, destroy the stale NemoClaw
entry and onboard a clean replacement. Restore a separately created snapshot afterward if one is
available; rebuild cannot recover a workspace or live OpenShell policy that no longer exists.
