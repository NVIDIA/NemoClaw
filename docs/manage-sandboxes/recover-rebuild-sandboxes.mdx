...
For these providers, an expired credential or expiration metadata that NemoClaw cannot verify stops rebuild before backup or deletion, including with `--force`.
A credential with no expiration or a future expiration continues through the remaining preflight checks.
