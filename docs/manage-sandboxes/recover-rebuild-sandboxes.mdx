...
Do not retry rebuild when NemoClaw reports that a retained rebuild policy handoff contains a literal credential.
The handoff cannot safely restore a sandbox that the interrupted rebuild already deleted.
Keep the backup for manual data recovery.
Retire the failed rebuild state, then onboard a fresh sandbox under a new name:
...
Do not discard the retained handoff before you retire the failed rebuild state.
</Warning>
