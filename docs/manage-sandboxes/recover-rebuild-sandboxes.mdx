...
For an active Portable Hermes receipt, recovery starts only the exact receipt-owned Podman container when it is stopped.
If authenticated health is not ready after that start, recovery launches the receipt-owned `nemoclaw-start` command once and waits for health.
...
A later identity, reconnect, or health failure rolls back only a container that recovery started.
