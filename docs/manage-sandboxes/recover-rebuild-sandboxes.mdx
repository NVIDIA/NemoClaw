...
The restart command first runs the Hermes gateway wrapper as a non-mutating secret-boundary preflight. It then revalidates the registered sandbox and its live identity, targets the sandbox's recorded OpenShell gateway, and asks OpenShell to stop and start the complete sandbox. NemoClaw rechecks native gateway health before it recovers dashboard, messaging, and agent forwards. An identity change, an OpenShell lifecycle failure, or failed health proof stops the operation before forward recovery.
...
Five unexpected gateway exits or failed replacement candidates within 60 seconds stop relaunch for the current supervisor instance. OpenShell sandbox stop and start reset the supervisor instance instead of authorizing an in-container gateway-child exit.
