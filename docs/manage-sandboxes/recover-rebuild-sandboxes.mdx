...
<AgentOnly variant="hermes">
For a registered Hermes sandbox without base-image resolution metadata or an explicit base-image override, rebuild requires the release-pinned immutable Hermes base image.
If NemoClaw cannot resolve and validate that image, rebuild stops before it changes the sandbox.
It does not build or reuse a local base image as a fallback.
</AgentOnly>
