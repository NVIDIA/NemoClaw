...
and `recover` reconcile a sandbox that OpenShell still reports as `Stopped`, including when its
provider container is already running. Neither command directly restarts or unpauses the provider
container. If the container is paused, follow the printed provider guidance instead.
