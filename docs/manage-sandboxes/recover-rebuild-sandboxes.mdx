...
Upgrade the NemoClaw CLI before rebuilding — rebuilding with an old CLI restages the old in-container guard — then restore the config from the quarantine copy or rebuild with `--yes` to recreate a known-good baseline.
Current sandboxes report quarantine filenames in the shields error output and tolerate a missing `.config-hash` on lock by synthesizing it from `openclaw.json`.
</AgentOnly>
