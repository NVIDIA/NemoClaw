...
The line also reports cumulative duration and attempt count for sandbox identity, policy, inference, gateway health, forward health, and inference route observations.
It identifies the first failed observation and the first readiness and fallback decisions when available.
It also reports the lifecycle and forward actions, the result, and the failed stage when the probe fails.
