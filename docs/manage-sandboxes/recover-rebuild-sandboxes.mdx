...
It also captures credential-free, non-managed values for supported Hermes dotpaths that host-side `config set` previously recorded.
After it writes the fresh managed route, rebuild merges and verifies those values.
The completion output lists the restored and dropped Hermes operator config keys.
Rebuild drops managed-route fields and values that contain credentials.
If it cannot capture or restore the operator configuration, rebuild exits nonzero and preserves the backup for recovery.
