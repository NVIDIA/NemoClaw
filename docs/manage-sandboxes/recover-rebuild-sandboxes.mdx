...
and `recover` reconcile a sandbox that OpenShell still reports as `Stopped`; neither command
directly restarts or unpauses the underlying provider container. Before destructive replacement,
run `$$nemoclaw <sandbox-name> status`. If the owning gateway reports the registered sandbox
missing, NemoClaw leaves any provider container untouched. Preserve any separate snapshot, use
`$$nemoclaw <sandbox-name> destroy --yes` to retire the stale registered sandbox when the command
can prove the OpenShell resource absent, then onboard a clean replacement. Restore the snapshot
afterward if one is available; rebuild cannot recover a workspace or live OpenShell policy that no
longer exists.
...
If rebuild keeps the source sandbox but cannot release it from backup maintenance, the warning names the retained sandbox and reports whether it verified that the sandbox stopped.
If stopping is not verified, do not assume that the sandbox is stopped or ready to use.
Preserve the retained sandbox and backup, and follow the recovery guidance printed with the warning.
