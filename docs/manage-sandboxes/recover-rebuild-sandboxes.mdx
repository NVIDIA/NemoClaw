...
Do not retry rebuild when a retained policy handoff contains a literal credential. Keep the backup and handoff for recovery. Restore or select the recorded gateway and confirm `openshell status` is healthy. Only then destroy the failed sandbox and confirm OpenShell reports it deleted:

```bash
$$nemoclaw <sandbox-name> destroy --force
```

If deletion is unconfirmed, preserve the recovery state and restore gateway access. After confirmed deletion, run `$$nemoclaw onboard --name <new-sandbox-name>`. Do not discard the retained handoff before retiring the failed state.
</Warning>
