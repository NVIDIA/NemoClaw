...
<AgentOnly variant="hermes">
The rebuild command preserves Hermes state, the complete current OpenShell policy, managed MCP configuration, and supported operator-managed configuration while recreating the container.
Before deletion, NemoClaw captures values set through `nemohermes <name> config set`, excluding NemoClaw-managed route fields and credential material.
After it restores state, NemoClaw merges the captured values into the replacement configuration and verifies them.
Rebuild reports each restored and dropped configuration key.
If capture, restore, or verification fails, rebuild exits nonzero and retains its recovery state.
