...
Before `rebuild` deletes the existing sandbox, NemoClaw records a replacement journal in the active onboarding session.
...
Rebuilding another sandbox can retain this session in `.onboard-rebuild-<sandbox>.json` under the selected NemoClaw state directory.
Rerunning the affected sandbox's rebuild restores its retained session automatically.
For a non-default gateway port, these files reside under `~/.nemoclaw/gateways/<port>/`.
Resume or clear an unfinished ordinary onboarding session before retrying retained rebuild recovery.
To discard that ordinary onboarding attempt and start over, run `$$nemoclaw onboard --fresh` as described in [Previous onboarding session failed](../../reference/troubleshooting#previous-onboarding-session-failed).

<Warning title="Preserve Rebuild Recovery Files">
Do not manually delete `onboard-session.json` or retained `.onboard-rebuild-<sandbox>.json` files to resolve an interrupted replacement.
Keep the recovery files and rerun the affected sandbox's rebuild with the same settings.
</Warning>
