...
It also restores supported non-gateway configuration values that you previously wrote with `config set`.
The rebuild result lists the restored keys and any keys it dropped.
If it cannot capture those values before replacement, rebuild stops before it deletes the existing sandbox.
If it cannot restore them after replacement, rebuild exits nonzero and retains recovery state.
