...
After a nonzero delete, NemoClaw queries the sandbox name on its exact recorded gateway: an explicit missing result converges as deleted, while a `Ready` or `Running` result triggers an attempt to restore prepared MCP state and any shields lockdown that rebuild temporarily opened.
That MCP and shields restoration can fail and is reported rather than presented as a successful rollback.
Any partial or unreachable result remains ambiguous: NemoClaw preserves the MCP ownership and rebuild-recovery records, does not attempt to stop NIM, skips the rebuild process's immediate shields relock, and does not claim that the original sandbox is intact.
