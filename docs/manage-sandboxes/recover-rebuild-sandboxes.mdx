...
If recovery launches a new forward, it accepts the forward after the OpenShell process owns the listener through the startup stability window without opening a connection to the endpoint.
...
If the host forward is already active, recovery accepts it only after OpenShell ownership is reconciled and the local endpoint is reachable.
For a standard Hermes sandbox, if recovery launches a new forward, it accepts the forward after the OpenShell process owns the listener through the startup stability window without opening a connection to the endpoint.
