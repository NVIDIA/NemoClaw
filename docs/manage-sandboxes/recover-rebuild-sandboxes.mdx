...
For every Hermes rebuild without a `NEMOCLAW_HERMES_SANDBOX_BASE_IMAGE_REF` override, NemoClaw disables new local base-image fallback.
It can reuse a recorded local image only when its resolution metadata and image identity remain valid.
Otherwise, rebuild requires the release-pinned immutable Hermes base image.
