...
It starts the sandbox through OpenShell rather than treating the running container as already started.
...
<AgentOnly variant="openclaw,hermes">
  `recover` can start an existing stopped Docker-driver sandbox before it checks the agent runtime.
  It first starts the sandbox through OpenShell, which restarts the same container and advances the sandbox out of the `Stopped` phase.
  ...
  It leaves paused containers and running sandboxes in other phases unchanged.
  If OpenShell cannot start a stopped container, recovery starts that container directly and reports any later readiness failure.
</AgentOnly>
...
The native agent owns the gateway and its child processes.
NemoClaw observes native gateway health and repairs host-side forwards without taking gateway process lifecycle authority.
For a stopped Docker-driver sandbox, NemoClaw can ask OpenShell to start it and can repair the provider container state through the registered runtime provider.
If the native supervisor remains absent, NemoClaw leaves the sandbox unchanged and directs you to rebuild or re-onboard.
...
Rebuild restores the complete credential-sanitized native `openclaw.json` file from the pre-rebuild snapshot.
NemoClaw does not compare that file with a config hash, seal, last-good copy, recovery anchor, baseline, or selected-key registry.
If the file cannot be restored safely, NemoClaw exits nonzero and does not report a successful rebuild.
...
<AgentOnly variant="openclaw">
  ...
  Rebuild also restores the complete credential-sanitized native `openclaw.json` file.
  Native OpenClaw configuration changes therefore survive sandbox replacement without a selective merge.
