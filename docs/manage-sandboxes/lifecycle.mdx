...
Before opening shields, starting backup, or deleting a live sandbox, NemoClaw selects the recorded gateway, validates the exact provider and model route with a small completion from the live sandbox, runs blocking Docker, GPU, CDI, DNS, and OpenShell checks, prepares recorded messaging attachments and policies, pins the resolved agent base image when applicable, and builds a temporary replacement image from the recorded configuration.
