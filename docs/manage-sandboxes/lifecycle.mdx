...
<Note>
`nemoclaw uninstall` preserves `~/.nemoclaw/rebuild-backups/` (host-side snapshots created by `nemoclaw <name> snapshot create` and `nemoclaw backup-all`) and `~/.nemoclaw/sandboxes.json` (the sandbox registry) by default.
