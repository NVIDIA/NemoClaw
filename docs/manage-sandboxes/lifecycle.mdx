...
It also shows the dashboard URL when NemoClaw records a dashboard port, marked `(bound on all interfaces)` when that dashboard forward listens on every interface, or `(bind not recorded)` when NemoClaw has no recorded bind for that forward.
</AgentOnly>
...
<AgentOnly variant="openclaw,hermes">
The log command reads both agent gateway output and OpenShell audit events, so policy denials appear beside gateway logs.
</AgentOnly>
<AgentOnly variant="deepagents">
The log command reads OpenShell logs, including audit events, so policy denials appear with other OpenShell log entries.
</AgentOnly>
