...
It also shows the dashboard URL when NemoClaw records a dashboard port, marked `(bound on all interfaces)` when that dashboard forward listens on every interface, or `(bind not recorded)` when the forward was created before NemoClaw recorded binds.
</AgentOnly>
