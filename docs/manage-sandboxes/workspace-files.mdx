...
| `/sandbox/.deepagents/skills/` | Upload and snapshot directory for skills installed with `nemoclaw <name> skill install`. Deep Agents Code does not read this path directly, so the command mirrors each skill into `agent/skills/`. |
| `/sandbox/.deepagents/agent/skills/` | Skills that Deep Agents Code loads at session start, including skills created by the built-in skill creator and mirrors written by `skill install`. Preserved by NemoClaw snapshots. |
