...
Rebuild restore treats `.hermes/profiles/` as native agent-owned state and preserves every named profile, including an older `.hermes/profiles/dashboard-home/`; the dashboard no longer reads that reserved profile.
