...
Current `skill install` operations publish through OpenClaw into `/sandbox/.openclaw/workspace/skills/`. A skill installed by an older NemoClaw release can remain in OpenClaw's native global directory at `/sandbox/.openclaw/skills/`; rebuilds preserve both agent-owned locations. `skill list` reports whichever entry OpenClaw resolves. `skill remove` removes that exact resolved workspace or global entry, and reports a same-name fallback if one becomes active. Run it again to remove the fallback. NemoClaw does not migrate these entries through a host registry or infer ownership from their paths.
