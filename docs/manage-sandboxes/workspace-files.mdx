...
| `/sandbox/.hermes/.env` | NemoClaw-generated environment and messaging placeholders, plus preserved home-channel routing. |
...
During rebuild, NemoClaw also preserves `.env` assignments whose names match `*_HOME_CHANNEL`, `*_HOME_CHANNEL_NAME`, or `*_HOME_CHANNEL_THREAD_ID`.
The complete `.env` file remains excluded, and a current generated assignment overrides a preserved assignment with the same name.
...
Use Hermes `/sethome` to select a home channel instead of editing `.env` directly.
Other direct edits to `/sandbox/.hermes/config.yaml` or `/sandbox/.hermes/.env` can be overwritten by rebuilds.
