...
| Podman | Any host | Unsupported because read-only host mounts have not been qualified for the Podman runtime provider. |
...
An unsupported provider reports its declared reason.
A supported provider on an unqualified host reports that the host platform is not qualified.
