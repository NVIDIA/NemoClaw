...
description-agent: "Explains OpenClaw workspace files, Hermes state, Deep Agents state, and read-only host mounts. Use when preparing to edit, mount, rebuild, or transfer sandbox state."
...
Those paths resolve to `/sandbox/USER.md` and `/sandbox/SOUL.md`. They are part of the OpenShell-provided native home and transfer with the rest of that tree during rebuild.
...
OpenShell handles persistence automatically for per-agent workspaces.
...
An unavoidable rebuild transfers the complete native home, including every `workspace-<name>/` directory and the default `workspace/`.
...
These settings and tools survive restarts and are included in the complete native-state transfer used by rebuilds and upgrades.
...
Before replacing the sandbox, NemoClaw transfers the complete native home and workspace into the replacement.

If NemoClaw cannot archive the complete native state, it reports the failure and stops before replacing the sandbox.
...
OpenShell removes the native home and workspace with the sandbox.

<Warning>
Back up your workspace files before running `$$nemoclaw <name> destroy`.
Refer to [Persist Sandbox State](create-and-restore-snapshots) for the supported boundaries.
</Warning>
...
- [Persist Sandbox State](create-and-restore-snapshots)
...
| `/sandbox/.hermes/kanban.db` | Default Hermes kanban board database. It persists as part of the complete native home. |
...
| `/sandbox/SOUL.md` | Durable top-level Hermes persona file preserved by OpenShell storage. |
...
Rebuilds and upgrades transfer the complete OpenShell-provided native home, including `SOUL.md`, the Web Dashboard profile under `.hermes/profiles/dashboard-home/`, the SQLite database behind `.hermes/state.db`, and the default kanban board in `.hermes/kanban.db`.
Named boards, attachments, worker logs, and scratch workspaces under `.hermes/kanban/` are included because they are under the native home. External directory or worktree targets are included only when they are also under the OpenShell-provided native home/workspace.
...
| `/sandbox/.deepagents/agent/skills/` | Canonical writable user-skill root declared by the Deep Agents Code integration. The agent owns discovery and activation; OpenShell preserves this agent state. |
| `/sandbox/.deepagents/hooks.json` | User-authored native hook configuration. It persists with the native home; review its executable commands before transferring it into another workspace. |
| `/sandbox/.deepagents/.mcp.json` | Agent-native MCP configuration. NemoClaw-managed entries contain OpenShell credential placeholders and are read directly during status, migration, and rebuild. The file persists as part of the complete native home. |
...
| `/sandbox/.deepagents/.env` | User-managed Deep Agents Code environment file. It persists as agent-written native-home state; do not place raw credentials here when an OpenShell credential binding is available. |
...
They persist when they live under your sandbox workspace because the entire native home/workspace is OpenShell-owned storage.

## Persistence Behavior

Deep Agents state lives in the sandbox's persistent state volume, not in the container image alone.
Normal restarts preserve that state.
Rebuilds and upgrades transfer the complete native home/workspace as one opaque archive.
NemoClaw does not apply a Deep Agents path or config-key allowlist during that transfer, so unknown files, `config.toml`, hooks, skills, memories, histories, packages, and project state all move together.
OpenShell-managed credential material remains under OpenShell ownership rather than entering the archive.
...
They transfer with the complete native home during rebuild. Copy them manually with `$$nemoclaw <name> download` before destroying the sandbox when you also need an independent backup.
...
Direct edits to `/sandbox/.deepagents/config.toml` persist through rebuild. A later explicit configuration command can still change fields it owns.
...
OpenShell preserves the complete native home, but NemoClaw does not maintain a skill inventory or infer which skills are visible or active. Use `$$nemoclaw <name> skill list` for the selected agent's native view.
...
- [Persist Sandbox State](create-and-restore-snapshots) explains OpenShell persistence and whole-home rebuild transfer.
