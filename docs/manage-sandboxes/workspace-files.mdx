...
NemoClaw reserves `main` for the primary agent. Current configurations use the default workspace; secondary named agents use suffixed workspaces. Existing configurations that resolve `main` to the legacy `workspace-main/` path remain supported by native agent commands and snapshots. To retire that compatibility path, move its user-owned files to `workspace/` and remove the explicit `main` workspace setting before deleting `workspace-main/`. For example, a Teams-integrated deployment can use the primary `main` agent plus per-user agents.

```text
/sandbox/.openclaw/
├── workspace/           # primary agent "main"
├── workspace-main/      # supported legacy primary-agent workspace
...
Files are per-agent.
Changes in `workspace/AGENTS.md` are not visible to `workspace-support/`.
...
Current `skill install` operations publish through OpenClaw into the native `main` agent workspace. That normally resolves to `/sandbox/.openclaw/workspace/skills/`; supported legacy configurations can resolve it to `/sandbox/.openclaw/workspace-main/skills/`. A skill installed by an older NemoClaw release can remain in OpenClaw's native global directory at `/sandbox/.openclaw/skills/`; rebuilds preserve all of these agent-owned locations. `skill list` reports whichever entry OpenClaw resolves. `skill remove` removes that exact resolved workspace or global entry, and reports a same-name fallback if one becomes active. Run it again to remove the fallback. NemoClaw does not migrate these entries through a host registry or infer ownership from their paths.
...
| `/sandbox/.deepagents/agent/skills/` | Agent-owned skills that Deep Agents Code loads at session start, including skills created by the built-in skill creator and skills imported by `$$nemoclaw <name> skill install`. Preserved by NemoClaw snapshots. |
...
A pre-cutover `/sandbox/.deepagents/skills/` upload directory was only a duplicate staging destination and was never loaded by Deep Agents Code; current rebuilds intentionally do not preserve or recreate it. The agent-owned `/sandbox/.deepagents/agent/skills/` copy remains the durable native state.
