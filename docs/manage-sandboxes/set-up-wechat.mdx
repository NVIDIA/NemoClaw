...
For stock onboarding, NemoClaw records the non-secret `WECHAT_ACCOUNT_ID`, `WECHAT_BASE_URL`, and `WECHAT_USER_ID` metadata in the managed startup profile and materializes it when the sandbox starts.
An explicit custom Dockerfile must apply equivalent metadata during its local build.
The bridge can then pre-seed per-account context tokens without another QR handshake.
