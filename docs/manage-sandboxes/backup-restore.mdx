...
For OpenClaw, NemoClaw restores the credential-sanitized `openclaw.json` captured in the snapshot as a whole file. Other agents continue to use any restore ownership rules declared by their current manifest. For Deep Agents targets, whole-file config replacement is limited to sandboxes created from a custom Dockerfile.
...
During rebuild or restore, NemoClaw restores the credential-sanitized `openclaw.json` as a whole file instead of merging selected keys into the replacement image's generated config. The snapshot remains authoritative for noncredential OpenClaw-owned provider, tool, plugin, hook, cron, and channel settings. Sanitization removes the top-level `gateway` object and credential fields, while preserving supported OpenShell resolver placeholders.

A rebuild does not merge new `openclaw.json` defaults from the replacement image into the restored file. Verify the config and plugin registrations after a rebuild when the new image changes those fields. Restore publishes the restored file as the last-good copy and refreshes `.config-hash`.
