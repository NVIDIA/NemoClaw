...
Preserve the error and the pending row for maintainer review.
Do not retry the restore, use `--force`, or delete the destination by name because no supported command can prove that the changed row still belongs to this restore.
