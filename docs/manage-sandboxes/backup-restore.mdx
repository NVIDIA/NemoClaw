...
<AgentOnly variant="openclaw">
  NemoClaw parses native `openclaw.json` as JSON5 and serializes it as standard JSON before it
  stores the sanitized snapshot copy. JSON5 comments and formatting do not survive snapshot
  creation, so a credential written only in a comment is not retained.
</AgentOnly>
...
For OpenClaw, NemoClaw restores the complete credential-sanitized native config file.
For other managed images, NemoClaw applies the current manifest's managed config merge rules and does not fall back to whole-file replacement.
...
<AgentOnly variant="openclaw">
An in-place OpenClaw restore stops the sandbox before it changes state.
This interrupts active agent work.
After restored state is present, NemoClaw runs `openclaw doctor --fix` while the gateway remains held down, then releases the maintenance window for one native OpenClaw start.
...
For a verified stopped sandbox, run `$$nemoclaw my-assistant start`.
For an unverified running state, run `$$nemoclaw my-assistant stop`, then `$$nemoclaw my-assistant start`.
...
In-place restore does not mutate the OpenShell policy.
Cross-sandbox clone reads the source live policy and uses it only as the destination creation handoff.
...
Snapshots preserve the complete credential-sanitized `openclaw.json` file.
Rebuild and restore replace the generated file with that native backup instead of reconstructing selected settings.
This preserves OpenClaw-owned providers, tools, plugins, hooks, cron jobs, and gateway settings.
...
Use `$$nemoclaw <sandbox> channels` commands to change messaging channels.
If the config cannot be restored safely, NemoClaw stops the restore instead of applying a partial fallback.
Restore does not change OpenShell policy, credential bindings, or the host-side inference route.
Reconcile those host resources when the restored configuration expects different values, then restart the gateway when OpenClaw requires it.
