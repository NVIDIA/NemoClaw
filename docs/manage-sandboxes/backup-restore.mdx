...
It omits a recognized lockfile when the file contains any of these values:
...
NemoClaw also preserves installed `node_modules/**/package.json` manifests byte for byte because dependency names can match credential field names.
