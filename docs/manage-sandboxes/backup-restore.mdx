...
After a rebuild restores `profiles`, NemoClaw preserves that native agent-owned directory unchanged. An older `profiles/dashboard-home` profile is inactive compatibility state, but rebuild does not delete it. If it contains legacy WhatsApp credentials, use `$$nemoclaw <sandbox> channels remove whatsapp` to clear the compatibility sessions before pairing again.
