...
The agent-native `.deepagents/.mcp.json` and `hooks.json` are excluded from ordinary file copying because MCP is carried through the bounded source handoff and NemoClaw disables executable Deep Agents Code hooks in the managed harness.
