...
A temporary OpenShell SSH configuration cleanup failure is separate from incomplete snapshot cleanup.
NemoClaw reports the retained temporary directory and, during snapshot creation, still attempts to remove the incomplete snapshot.
Remove the reported temporary SSH configuration directory before retrying snapshot creation or restore.
