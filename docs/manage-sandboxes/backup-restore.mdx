...
Before NemoClaw marks a snapshot complete, it strips recognized credential fields from copied JSON, YAML, and environment files.
...
If it cannot remove the unsafe file, snapshot creation deletes the incomplete backup and returns an error.
