...
It removes only named generated hash and runtime-state metadata.
Other safe regular files, including legacy `config.yaml` and `.env` files, merge only when the native destination is absent or byte-identical.
