...
The source sandbox does not have to be running.
When a sandbox can no longer be recovered, restore its snapshot into a replacement with `--to`: NemoClaw builds the replacement from the image recorded in the source registry entry.
Restore stops with guidance to run `$$nemoclaw onboard` when the stopped source records no image.
