...
Before it restores snapshot files, NemoClaw reconstructs the managed MCP projection from the host registry. If `/sandbox/.deepagents/.nemoclaw-mcp.json` is a directory, the restore leaves snapshot files unchanged and prints a recovery command. Run that command to move the directory to the printed recovery name, then rerun the same snapshot restore command.
