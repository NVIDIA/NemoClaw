...
It skips dependency lockfiles and installed dependency trees, whose package names match the credential field names and whose contents `npm install` recreates from the lockfile.
...
<AgentOnly variant="hermes">
A release published before this exemption scanned dependency trees as well.
