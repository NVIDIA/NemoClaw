...
After a rebuild restores `profiles`, NemoClaw removes only the retired reserved `profiles/dashboard-home` directory before it reports the restore as complete. If that cleanup fails, NemoClaw marks the restore incomplete instead of reporting success.
