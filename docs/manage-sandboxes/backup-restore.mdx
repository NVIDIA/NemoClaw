...
openshell sandbox download "$SANDBOX" /sandbox/.deepagents/.state/ "$BACKUP_DIR/state/"
...
openshell sandbox upload "$SANDBOX" "$BACKUP_DIR/state/" /sandbox/.deepagents/.state/
...
Use manual `openshell sandbox download` / `openshell sandbox upload` only when you need to inspect or transfer a specific state directory.
