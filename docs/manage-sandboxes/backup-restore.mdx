...
Stop all NemoClaw processes for the sandbox, then follow that guidance before you retry the snapshot.
...
When an eligible sandbox starts with Shields up, `backup-all` acquires the lifecycle lock and opens a 30-minute shields-down window.
A sandbox that starts with Shields down remains down.
`backup-all` reacquires the lock under that exact timer generation while it copies sandbox state.
...
NemoClaw attempts to restore the previous Shields posture before it processes the next sandbox, including when the backup fails.
