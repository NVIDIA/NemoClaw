...
After a restore that includes Hermes state databases, the CLI prints a reminder to run `$$nemoclaw <name> gateway restart` so the gateway picks up the restored databases.
</AgentOnly>
