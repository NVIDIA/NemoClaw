...
If OpenShell does not confirm whether it created the destination, NemoClaw prints a create-attempt label and keeps the route reservation. Run the printed selector command. Do not retry the restore while the selector reports a sandbox or OpenShell cannot confirm the result. If the selector and destination name are both absent, rerun the original restore command. NemoClaw removes only the matching retained reservation before it starts a new create attempt. If the selector identifies a sandbox, resolve that retained sandbox through its owning gateway before you retry.
