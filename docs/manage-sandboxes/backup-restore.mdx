...
Snapshot clone reads the source sandbox policy from OpenShell and passes that complete current OpenShell policy to destination creation through a private temporary handoff.
If the policy contains a literal credential value, NemoClaw stops before it writes the handoff or changes the destination.
Replace literal credentials with supported OpenShell credential bindings or resolver placeholders, then rerun the original restore command.
The snapshot manifest and registry contain no custom-preset copy, baseline-exclusion record, or desired-policy replay state.
...
If NemoClaw cannot securely remove the temporary clone policy after destination creation, it leaves the destination as a pending clone and does not restore snapshot state.
Inspect and remove the task-owned temporary policy file that the error identifies.
Then rerun the same restore command without `--force` so NemoClaw can reconcile the destination without deleting or recreating it.
