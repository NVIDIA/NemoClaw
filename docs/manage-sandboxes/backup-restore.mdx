...
Pick a backup directory **outside `~/.nemoclaw/`** so the data survives `nemoclaw uninstall` — anything inside `~/.nemoclaw/` (including `~/.nemoclaw/backups/`, which is gate-protected but still wiped once the uninstall gate is acknowledged) is removed by the uninstaller.
...
The script defaults to `~/nemoclaw-backups/` so backups survive `nemoclaw uninstall`.
Set `NEMOCLAW_BACKUP_BASE` to override the location (e.g. point it at an external mount).
