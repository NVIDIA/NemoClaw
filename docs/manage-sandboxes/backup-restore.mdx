...
If OpenShell does not confirm whether it created the destination, NemoClaw prints a create-attempt label.
For a clone restore that uses host-local inference, NemoClaw also keeps the route reservation.
...
For a host-local inference clone, NemoClaw removes only the matching retained route reservation before it starts a new create attempt.
