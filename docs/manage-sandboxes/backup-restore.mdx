...
<AgentOnly variant="hermes">
Hermes snapshots include `SOUL.md`, user-created named profiles under `.hermes/profiles/`, the SQLite database behind `.hermes/state.db`, and the default kanban board in `.hermes/kanban.db`.
The gateway, CLI, TUI, and Web Dashboard share the native `.hermes/config.yaml`; snapshots exclude that NemoClaw-managed file and do not retain the raw `.hermes/.env` file.
For rebuild recovery, NemoClaw preserves only the allowlisted non-secret home-channel assignments from `.hermes/.env`; other values are not retained.
Rebuild restore treats `.hermes/profiles/` as native agent-owned state and preserves user-created named profiles.
On startup, NemoClaw migrates an older reserved `.hermes/profiles/dashboard-home/` into the shared native home, including a legacy WhatsApp session, and removes the retired profile.
It removes named generated hash and runtime-state metadata.
It also removes a legacy `config.yaml` or `.env` only when its contents match the generated projection of the native files.
Other safe regular files merge only when the native destination is absent or byte-identical.
Unsafe links or conflicting native and legacy files stop startup without deleting either copy.
...
After a rebuild restores `profiles`, NemoClaw preserves user-created named profiles unchanged.
The reserved `profiles/dashboard-home` name is a one-time migration source rather than durable profile state.
Startup moves safe legacy files into the native Hermes home and retires that directory.
If migration reports a collision or unsafe path, inspect both copies from a trusted recovery environment; NemoClaw leaves them intact instead of choosing one.
