...
After a cross-sandbox restore creates the destination, NemoClaw establishes gateway pairing for the host CLI.
OpenClaw regenerates its device identity on demand, and NemoClaw also repairs CLI pairing on connect.
</AgentOnly>
<AgentOnly variant="hermes">
