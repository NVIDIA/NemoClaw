...
Each stock sandbox runs an immutable managed image that pins the selected agent version; an explicit custom image owns its own version pin.
NemoClaw does not update that agent runtime in place.
...
After updating the host CLI, run `$$nemoclaw <name> rebuild` to resolve the current managed image to an exact digest, apply a fresh startup profile, and preserve the agent state that NemoClaw manages.
