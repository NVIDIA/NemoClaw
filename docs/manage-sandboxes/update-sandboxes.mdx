...
<AgentOnly variant="openclaw,hermes">
  The registry must contain positive evidence that NemoClaw managed the sandbox image, unless the
  name pre-fingerprint confirmation below applies.
</AgentOnly>
If the replacement gateway reports a stale sandbox as Ready or Running, the installer reuses the
validated pre-upgrade backup. It does not attempt another backup from the replaced legacy runtime.
...
<AgentOnly variant="openclaw">
  OpenClaw's native plugin and package state is restored with the rest of its declared state;
  NemoClaw does not require separate plugin ownership metadata.
</AgentOnly>
...
<Warning>
  Destroying a sandbox can remove its persistent state. Preserve any required backups before
  cleanup. A local-only forced cleanup does not confirm that the OpenShell sandbox was deleted.
</Warning>
