...
When a legacy Hermes sandbox has no recorded base-image resolution metadata, `nemohermes <name> rebuild` requires the release-pinned immutable Hermes base image.
If NemoClaw cannot resolve and validate that image, rebuild stops before it changes the sandbox.
An explicit base-image override or recorded resolution metadata uses its existing rebuild path.
