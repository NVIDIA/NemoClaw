...
Before it prepares recovery or creates backups, the installer requires any existing OpenShell executable it will use to report a version.
Only after that validation does it prepare the current NemoClaw CLI without replacing OpenShell and require a fresh backup of every registered sandbox.
