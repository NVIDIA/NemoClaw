...
Use this path only after strict backup succeeds, every applicable registered legacy forward is retired, and the gateway is retired.
...
Do not set `NEMOCLAW_OPENSHELL_UPGRADE_PREPARED=1` unless both commands succeed and the backup command reports that strict backup and exact legacy-forward retirement completed.
...
Before native startup or post-upgrade doctor, NemoClaw removes the zero-byte `exec-approvals.json` placeholder that older images, including v0.0.123, created.
It removes only an unchanged, regular file with a single link.
Nonempty approval files remain unchanged for OpenClaw to process, including malformed files.
An unsafe file type or a changed file stops startup instead of deleting unverified state.
</AgentOnly>
