...
Before it retires an out-of-range gateway, the installer retires each registered legacy dashboard forward through its exact sandbox, bind, port, process, gateway, and workspace identity.
...
Use this path only after you back up every registered sandbox, retire each registered legacy dashboard forward, and retire the gateway.
Run the backup and forward-retirement step with the current prepared NemoClaw CLI before you retire the gateway:

```bash
$$nemoclaw backup-all --retire-legacy-forwards
```

Do not set `NEMOCLAW_OPENSHELL_UPGRADE_PREPARED=1` unless that command succeeds and reports that every registered sandbox backup and exact legacy-forward retirement completed.
If the command fails or cannot prove a retirement, resolve that failure while the current gateway and completed backups remain intact.
