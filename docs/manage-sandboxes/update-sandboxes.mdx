...
`upgrade-sandboxes --check` reads sandbox state without starting, recovering, or selecting a gateway.
When all registered sandboxes resolve to one recorded gateway, the command queries that gateway instead of the gateway for the current `NEMOCLAW_GATEWAY_PORT`.
