...
<AgentOnly variant="openclaw">
OpenClaw's native plugin and package state transfers with the complete native home; NemoClaw does not require separate plugin ownership metadata.
</AgentOnly>
...
Keep an independent host copy before a manual update when you need an extra recovery point.
...
Each rebuild destroys the old container and creates a new one while transferring the complete OpenShell-provided native home/workspace.
...
Paths outside the native home/workspace, such as packages installed into the container image's system directories, are not preserved. Packages and other files installed under the native home are preserved.

## Related Topics

- [Persist Sandbox State](../state-and-backups/create-and-restore-snapshots) for the preservation contract.
