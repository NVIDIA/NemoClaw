...
The default installer preserves a newer installed NemoClaw release. It also stops without changing the CLI when it cannot verify the maintained `lkg` release. To intentionally install an older release, select its immutable version tag with `NEMOCLAW_INSTALL_TAG`.
