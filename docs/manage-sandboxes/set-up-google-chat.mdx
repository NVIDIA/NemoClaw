...
After OpenClaw commits the sender to its owner allowlist, NemoClaw automatically restarts that selected gateway.
...
If gateway restart fails after the approval commits, `exec` exits with status `1` and reports that the approval was not rolled back.
For a selected owning gateway, it prints the managed recovery command; run `nemoclaw my-assistant gateway restart` before testing the next message.
