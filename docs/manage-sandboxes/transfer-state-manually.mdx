...
The retired reserved profile at `.hermes/profiles/dashboard-home/` is not active dashboard state, but rebuild preserves it with other native profiles. Omit it from a manual transfer unless you intentionally need its contents; use `$$nemoclaw <sandbox> channels remove whatsapp` to clear compatibility sessions from an existing sandbox.
