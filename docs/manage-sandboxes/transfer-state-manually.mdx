...
Restore safe Deep Agents files only, including the credential-free native `.mcp.json`, then run the managed rebuild or onboarding flow so NemoClaw regenerates route metadata and reconnects MCP entries to live OpenShell policy and provider state.
