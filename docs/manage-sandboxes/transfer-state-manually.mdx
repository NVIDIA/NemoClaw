...
keywords: ["nemoclaw download", "openshell sandbox upload", "manual sandbox backup", "workspace transfer"]
content:
  type: "how_to"
skill:
  priority: 30
...
$$nemoclaw "$SANDBOX" download /sandbox/.deepagents/skills/ "$BACKUP_DIR/skills/"
...
If a listed file or the `memory/` directory cannot download, the helper deletes the new timestamped backup and exits.
Check the reported sandbox path for a missing file, symbolic link, or unsupported file type.
