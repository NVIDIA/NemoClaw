...
Full uninstall removes a Docker container only when it can prove ownership through the selected gateway container name or an unambiguously registered sandbox container; it preserves a container that merely shares a `nemoclaw-` or `openshell-` name prefix or a NemoClaw image reference.
If Docker cannot inventory containers, uninstall exits nonzero and preserves the remaining uninstall state so a later run can retry.
