...
After NemoClaw deletes `containers.conf`, it deletes `~/.config/nemoclaw/portable/` only when that directory is empty.
It then deletes `~/.config/nemoclaw/` only when that directory is empty.
It preserves unrelated entries in either directory.
