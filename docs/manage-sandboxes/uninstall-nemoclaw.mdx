...
Portable cleanup preserves shared OpenShell provider and gateway registrations, the gateway service and process, and shared OpenShell binaries and configuration.
It also preserves unrelated OpenShell-registered sandboxes.
...
It unsets a current-user systemd manager selector only when its current value matches NemoClaw's recorded or derived portable projection.
