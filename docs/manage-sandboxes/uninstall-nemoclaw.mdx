...
<Warning>
The sweep applies the uninstall plan to every listed gateway port.
Review the port list before you confirm because `--yes` skips this confirmation.
...
The sweep confirms once, lists the ports it will remove, then uninstalls each port in turn and the currently selected port last.
