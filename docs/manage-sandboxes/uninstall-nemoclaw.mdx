...
A sandbox that OpenShell cannot reach or refuses to delete still leaves state behind, so uninstall keeps the selected gateway state for a retry and exits nonzero.
