...
Within each gateway pass, uninstall creates a fresh host-side backup for each eligible registered sandbox before it deletes that pass's OpenShell sandboxes.
...
Default uninstall backs up required Hermes state first.
...
The whole-host sweep restores each port's custom state directory from its sandbox registry or verified incomplete-create checkpoint.
An explicit `NEMOCLAW_OPENSHELL_GATEWAY_STATE_DIR` overrides the recorded directory only for the port selected by `NEMOCLAW_GATEWAY_PORT`.
If recorded directories are invalid or conflict, the sweep exits before any port cleanup.
Ports without recorded directories use their per-port defaults, except when the selected port has an explicit override.
For an older custom gateway whose directory was not recorded, rerun its failed pass with the original directory:
...
Before deleting a registered sandbox, the default path writes a fresh backup under that gateway's `rebuild-backups/` directory.
...
- [Transfer State Manually](../state-and-backups/transfer-state-manually) before removing state you may need later.
