...
If an interrupted dual-DGX Spark transaction leaves `<selected-state-root>/dual-spark-managed-serving.json.ssh-binding/` without a durable runtime receipt, full uninstall fails closed before cleanup or other mutation and preserves the claim for explicit recovery or removal.
