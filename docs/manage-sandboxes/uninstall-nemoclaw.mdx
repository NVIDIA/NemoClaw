...
Before each signal, uninstall revalidates the canonical PID, current user, stable process start identity, lifecycle generation, executable path, and launcher path.
...
It preserves the selected gateway's PID, bearer token, lifecycle state, and prepared uninstall journal for a retry.
