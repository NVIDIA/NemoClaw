...
During full uninstall, NemoClaw requires complete selected-gateway authority before it removes provider registrations, the gateway registration, `sandboxes.json`, or later local state.
It also requires two consecutive empty OpenShell sandbox inventories.
If the authority is incomplete or the inventory cannot be verified or does not remain empty, uninstall exits nonzero and preserves that downstream state for retry.
The OpenShell sandbox deletion can partially complete before verification fails.
Resolve the reported OpenShell authority or inventory error, inspect the selected gateway, and rerun the same uninstall command.
