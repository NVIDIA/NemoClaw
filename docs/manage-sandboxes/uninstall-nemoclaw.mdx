...
After a successful or already-absent sandbox deletion, uninstall removes the selected sandbox entry from `sandboxes.json`.
...
If gateway registration cleanup fails, uninstall exits nonzero, preserves the remaining state, and requires a rerun after you resolve the reported error.
