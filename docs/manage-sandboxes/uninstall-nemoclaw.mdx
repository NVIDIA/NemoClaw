...
For the portable experimental profile, this preservation includes the shared Podman resources, current-user selectors, portable configuration, and lifecycle receipts.
...
Before it deletes portable configuration or lifecycle receipts, NemoClaw revalidates the current user's local Podman socket authority and lifecycle generation.
...
NemoClaw uses the pinned local Podman socket to remove only those exact containers.
After it verifies their removal, it unsets a current-user systemd manager selector only when its current value matches NemoClaw's recorded or derived portable projection.
This check covers `CONTAINERS_CONF`, `NETAVARK_FW`, `CONTAINER_HOST`, `CONTAINER_CONNECTION`, and `CONTAINER_SSHKEY`.
NemoClaw preserves unrelated or changed selector values.
It does not remove the user's Podman installation, storage, images, networks, or unrelated containers.
It does not disable a user-managed socket.
NemoClaw then retires the portable lifecycle receipts and removes the portable configuration.
If NemoClaw cannot prove the required portable authority or complete cleanup, uninstall exits nonzero.
It preserves the portable receipts, configuration, gateway state, and other retry evidence, even with `--destroy-user-data`.
