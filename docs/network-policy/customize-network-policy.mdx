...
Network policy rule bodies, credential metadata, internal IP ranges, and binary allowlists are not included — only host stems and category-level summaries appear.
...
2. `missing-approval` — the host **is** allowed by an applied preset and the request is refused with HTTP 401 or 403. The network path is open; credentials or scope are missing.
3. `blocked-by-policy` — the host is **not** allowed by any applied preset and either an existing built-in or custom preset declares it (apply that preset), or the request is refused with a network-block error code (`EHOSTUNREACH`, `ENETUNREACH`, `ENOTFOUND`, `ECONNREFUSED`, `ETIMEDOUT`, `EAI_AGAIN`) or HTTP 403.
4. `unknown` — none of the above apply; the agent should surface the underlying error.
