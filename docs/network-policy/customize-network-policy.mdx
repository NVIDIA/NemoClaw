...
| --- | --- |
| Change every future sandbox for an agent | [Change the Baseline Network Policy](change-baseline-network-policy) |
| Add or remove a maintained preset for one sandbox | [Apply Policy Presets](apply-policy-presets) |
| Add a reviewed endpoint that no maintained preset covers | [Create Custom Policy Presets](create-custom-policy-presets) |

<AgentOnly variant="openclaw,hermes">
  | Allow direct TLS negotiation for an exact endpoint | [Configure Raw TLS
  Passthrough](configure-raw-tls-passthrough) |
</AgentOnly>
| Replace the complete live policy | [Replace the Live Network
Policy](replace-live-network-policy) |
<AgentOnly variant="openclaw,hermes">
...
Static changes modify the policy source that NemoClaw reads during sandbox creation. Follow [Change the Baseline Network Policy](change-baseline-network-policy) to edit the agent policy file, rerun onboarding, and verify the result.
...
Dynamic changes update the OpenShell policy for a running sandbox. Use [Apply Policy Presets](apply-policy-presets) for convenient reviewed additions. NemoClaw reads, modifies, writes, and verifies the live OpenShell policy without recording a second desired-policy state.
...
the Live Network Policy](replace-live-network-policy) only when a preset cannot
express the complete change.

## Policy Presets

Maintained policy presets cover common integrations and package services. Follow [Apply Policy Presets](apply-policy-presets) to preview, apply, reapply, list, or remove them.
...
Custom preset files add reviewed endpoint access without changing the baseline. Follow [Create Custom Policy Presets](create-custom-policy-presets) to author, validate, apply, and remove a custom preset.
...
Some endpoints require direct TLS negotiation and fail through inspected L7 proxying. Follow [Configure Raw TLS Passthrough](configure-raw-tls-passthrough) for the bounded `access: full` and `tls: skip` recipe.
...
OpenShell `policy set` replaces the complete live policy. Follow [Replace the Live Network Policy](replace-live-network-policy) to export the round-trippable base, preserve existing entries, and apply a validated replacement.
