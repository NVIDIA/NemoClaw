...
OpenShell `policy set` replaces the complete live policy. Follow [Replace the Live Network Policy](configure-policies/replace-live-network-policy) to export the parsed base policy, replace every `[STRIPPED_BY_MIGRATION]` marker, preserve existing entries, and apply a validated replacement.
