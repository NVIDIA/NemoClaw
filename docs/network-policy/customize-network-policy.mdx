...
|----------------|-----------------|
| `blocked-by-policy` | The host is not declared by any applied preset, or the request is refused with HTTP 403 / a network-block error code (`EHOSTUNREACH`, `ENETUNREACH`, `ENOTFOUND`, `ECONNREFUSED`, `ETIMEDOUT`, `EAI_AGAIN`). |
| `missing-approval` | The host is allowed by an applied preset, but the request is refused with HTTP 401 or 403 (credentials or scope are missing). |
| `unsupported` | The capability is not offered by NemoClaw or OpenShell. |
| `unknown` | None of the above apply; the agent should surface the underlying error. |
