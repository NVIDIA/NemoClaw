...
Messaging channel policies are channel-owned; edit `src/lib/messaging/channels/<channel>/policy/hermes.yaml` when changing the maintained Hermes preset for Telegram, Discord, Slack, WeChat, WhatsApp, or Microsoft Teams.
...
NemoClaw ships preset policy files for common non-messaging integrations in `nemoclaw-blueprint/policies/presets/` and channel-owned messaging policy files under `src/lib/messaging/channels/<channel>/policy/`.
...
| `teams` | Microsoft Teams bot, OAuth, and webhook endpoints |
