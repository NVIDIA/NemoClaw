...
Start from the current sandbox-authored policy so the presets layered on at onboarding stay in the file you apply.
The `--base` view keeps baseline and preset entries while omitting OpenShell provider-composed entries that should not be round-tripped through `policy set`.
...
<Note>
The `openshell policy set <sandbox-name> --policy <file>` command operates on raw policy files and does not accept the `preset:` metadata block used in preset YAML files.
