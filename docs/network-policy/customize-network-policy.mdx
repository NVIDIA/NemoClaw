...
OpenShell `policy set` replaces the current round-trippable base policy. Follow [Replace the Live Network Policy](configure-policies/replace-live-network-policy) to export that parsed base policy, replace every `[STRIPPED_BY_MIGRATION]` marker, preserve existing entries, and apply a validated replacement. Provider-composed entries remain outside the base document.
