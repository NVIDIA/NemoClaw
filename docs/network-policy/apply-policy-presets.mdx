...
The `discord` binary list must contain `/opt/hermes/.venv/bin/python3` and `/opt/hermes/.venv/bin/python`.
It must not contain `/usr/bin/python3*` or `/usr/local/bin/node`.
