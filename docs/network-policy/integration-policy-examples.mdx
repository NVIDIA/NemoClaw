...
For Gmail or Google Workspace mail tools, first decide whether the tool uses the Gmail REST API, IMAP/SMTP, or another Google API, then create a custom preset with only the endpoints and binaries that workflow needs.
...
Use the standard custom-preset flow:

```bash
$$nemoclaw my-assistant policy-add --from-file ./gmail.yaml --dry-run
$$nemoclaw my-assistant policy-add --from-file ./gmail.yaml --yes
```

After applying the custom preset, run the Gmail workflow with `openshell term` open and inspect any blocked requests.
Add only endpoints you can explain, and prefer method/path-restricted REST rules when the workflow uses HTTPS APIs.
If the tool falls back to IMAP or SMTP, review the security impact separately because those connections cannot be narrowed to HTTP methods and paths.
