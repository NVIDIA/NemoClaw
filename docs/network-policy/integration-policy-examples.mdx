...
After explicitly approving curl for `api.atlassian.com` in OpenShell, it should
return Atlassian's unauthenticated `401` JSON response. That `401` is the
expected success signal for this manual probe: it proves curl reached
Atlassian, but no Jira credentials were supplied.
