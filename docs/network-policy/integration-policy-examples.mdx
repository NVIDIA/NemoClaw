...
description-agent: "Guides users through common post-install integration policy setup for maintained NemoClaw policy presets, including Outlook, messaging channels, GitHub, Jira, Brave, Tavily, and Firecrawl web search, package managers, Hugging Face, local inference, and OpenShell approval workflows."
...
Or apply the `firecrawl` preset for Firecrawl Search.

```bash
$$nemoclaw my-assistant policy-add firecrawl --dry-run
$$nemoclaw my-assistant policy-add firecrawl --yes
```

Rerun onboarding when you enable or disable a web search provider because the Hermes backend and OpenShell credential attachment are part of the sandbox image.
Configure `TAVILY_API_KEY` or `FIRECRAWL_API_KEY` during that onboarding run.
...
The `firecrawl` preset permits only `POST /v2/search` and `POST /v2/scrape` to `api.firecrawl.dev`.
It enables `Authorization` header credential rewriting because Hermes sends its resolver placeholder as a bearer token.
