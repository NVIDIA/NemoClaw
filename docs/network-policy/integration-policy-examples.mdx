...
To add Telegram after onboarding, set the token on the host, add the channel, rebuild so the replacement startup configuration includes the channel, and apply the policy preset:
...
Rerun onboarding when you change providers because the OpenClaw plugin configuration and OpenShell credential attachment are sandbox-creation inputs.
Stock onboarding carries the non-secret selection in the managed startup profile; an explicit custom Dockerfile applies it during its local build.
...
Rerun onboarding when you enable or disable Tavily because the Hermes backend and OpenShell credential attachment are sandbox-creation inputs.
Stock onboarding carries the non-secret selection in the managed startup profile; an explicit custom Dockerfile applies it during its local build.
