...
For an older Balanced sandbox, use the [Homebrew preset replacement procedure](#homebrew-specifics) below.
...
To replace the older built-in Homebrew grant in a Balanced sandbox, remove it before adding the restricted preset:
...
Verify that `policy list` marks `brew-balanced` as applied and `brew` as inactive.
