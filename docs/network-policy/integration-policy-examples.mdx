...
| Preset | Use when |
|--------|----------|
| `tavily` | Agent workflows call the Tavily Search API (`api.tavily.com`) |

During `nemoclaw onboard`, answer **y** to **Enable web search?**, choose **2) Tavily Search**, then paste your API key.
...
When the agent uses web search to answer a question, it is instructed to tell you **Tavily Web Search is used** (Brave uses **Brave Web Search is used**).
