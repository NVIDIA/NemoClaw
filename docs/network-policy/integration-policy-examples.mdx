...
This page covers only integrations that NemoClaw currently ships as maintained policy preset YAML, either under `nemoclaw-blueprint/policies/presets/` or under channel-owned messaging policy directories in `src/lib/messaging/channels/<channel>/policy/`.
...
NemoClaw ships maintained policy presets for common services in `nemoclaw-blueprint/policies/presets/` and channel-owned messaging policy directories.
...
| Microsoft Teams messaging | `teams` |
...
The matching preset for each supported messaging channel is the channel name (`telegram`, `discord`, `slack`, `teams`, `wechat`, or `whatsapp`).
