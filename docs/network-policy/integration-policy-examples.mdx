...
Re-running `policy-add outlook` when the preset is already applied is a safe no-op: the command compares the preset with the live policy, exits zero when nothing changed, and re-applies the preset when its file changed since it was applied.
