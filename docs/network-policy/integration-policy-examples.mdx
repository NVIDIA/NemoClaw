...
Apply the `gmail` preset when the sandbox needs to read or send email through the Gmail API.
The preset uses the Gmail REST API over OAuth2 and does not open SMTP or IMAP ports.

### Prerequisites

- A Google Cloud project with the Gmail API enabled.
- An OAuth2 client ID and client secret, or a service account with domain-wide delegation.
- A `credentials.json` file downloaded from the [Google Cloud Console](https://console.cloud.google.com/apis/credentials).

For App Password authentication (simpler, no OAuth2 flow required):
...
### Upload Credentials to the Sandbox

Upload your credentials file before running any Gmail script:

```bash
openshell sandbox upload /path/to/credentials.json /sandbox/credentials.json
```

For App Password authentication, upload a config file instead:

```bash
openshell sandbox upload /path/to/gmail_config.json /sandbox/gmail_config.json
```

A minimal `gmail_config.json` for App Password access:
...
Connect to the sandbox and run your Gmail script:

```bash
$$nemoclaw my-assistant connect
```

From inside the sandbox, install the Google API client library if needed:

```bash
pip install google-api-python-client google-auth-httplib2 google-auth-oauthlib
```

A minimal script to list inbox messages using the Gmail API:
...
For App Password access using IMAP, install `imaplib` (included in the Python standard library) and read config from the uploaded JSON:

```python
import imaplib
import json

with open("/sandbox/gmail_config.json") as f:
    cfg = json.load(f)
...
print(data[0].split()[-10:])  # last 10 message IDs
```

<Warning>
App Passwords grant full account access.
Store `gmail_config.json` outside the sandbox and upload it only when needed.
Do not commit it to version control.
</Warning>

### Remove the Preset

Remove Gmail access when the task is complete:
