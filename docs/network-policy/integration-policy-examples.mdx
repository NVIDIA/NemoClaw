...
Use OpenShell when you need an editable copy of the sandbox-authored policy:

```bash
openshell policy get my-assistant --base | awk 'found { print } /^---$/ { found = 1 }' > current-policy.yaml
```
