...
To restore the Balanced Homebrew policy, use `$$nemoclaw my-assistant policy add brew-balanced --yes` instead.
...
If Balanced's preset is missing, restore it with `$$nemoclaw my-assistant policy add brew-balanced --yes`.
