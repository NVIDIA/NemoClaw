...
`policy list` also prints the required repair command.
While the journal remains, NemoClaw refuses other policy changes, rebuild, snapshot creation, and snapshot restore.
...
It uses the exact source and target stored in the journal, so the original file and current DNS answers are not required for repair.
...
Inspect the live policy before you retry in that state.
