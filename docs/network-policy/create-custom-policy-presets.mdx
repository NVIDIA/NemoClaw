...
Before it reads the live policy, NemoClaw revalidates the recorded target's structure, endpoint safety, content-bound private-address pin authority, and exclusion from managed MCP ownership.
If validation fails, NemoClaw preserves the journal, does not inspect or mutate the live policy, and neither clears nor commits the journal.
Investigate and repair the registry's trusted state before retrying.
If you did not alter the registry, preserve it for security investigation instead of bypassing the guard.
...
After target validation succeeds, or for an interrupted remove, the retry reads the live policy before it changes or clears the journal.
