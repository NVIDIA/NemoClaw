...
For a remote GPU host and the legacy Brev compatibility flow, refer to [Deploy NemoClaw to a Remote GPU Instance](deploy-to-remote-gpu).
</AgentOnly>
...
Use `$$nemoclaw headless-agent connect` through the SSH session, then run `dcode` inside the sandbox.
...
The hosted installer uses the maintained last-known-good `lkg` reference when `NEMOCLAW_INSTALL_REF` and `NEMOCLAW_INSTALL_TAG` are unset.
Inject provider credentials from your secret manager into the host environment before you run this example.
The example fails before the network install if `NVIDIA_INFERENCE_API_KEY` is missing.
...
Keep every `NEMOCLAW_*` assignment on the `bash` side of the pipeline so the downloaded installer can read it.
...
| `NEMOCLAW_INSTALL_TAG` | Optional, default `lkg` | No | Pins a release tag when the maintained default is not desired. |
| `NEMOCLAW_INSTALL_REF` | Development override only | No | Overrides `NEMOCLAW_INSTALL_TAG` when nonempty. |
...
Retrieve its `API_SERVER_KEY` through the supported command, use it as a bearer token, then clear the shell variable:
...
The installer follows `lkg` by default and requires current backups before it changes an existing managed installation.
