...
When you run it inside an SSH session and the gateway is running, `status` points you to `$$nemoclaw headless-agent dashboard-url` for the SSH port-forward command that reaches the loopback-only dashboard.
