...
Portable Hermes probes can also print currentness, container-inspection, and lifecycle-recovery timing lines.
...
<AgentOnly variant="openclaw">
  | OpenClaw gateway token | Rotated when the container starts with mutable configuration | Not
  captured; a replacement sandbox generates a new token | Rotated for the replacement sandbox |
</AgentOnly>
<AgentOnly variant="hermes">
