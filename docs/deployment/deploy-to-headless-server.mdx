...
It also reports cumulative durations and attempt counts for sandbox identity, policy, inference, gateway health, forward health, and inference-route readiness observations.
`readiness.firstFailedObservation` identifies the first failed observation, or reports `none`.
Use these fields to identify where a slow or failed deployment spent its time.
