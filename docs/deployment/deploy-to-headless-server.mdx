...
Run `openshell forward list`, select the `headless-agent` row whose local port is in that range, and enter that port at the prompt below.
...
Use it as a bearer token, then clear the shell variables:

```bash
printf 'Hermes API port from openshell forward list (8642-8652): '
read -r API_PORT
[[ "$API_PORT" =~ ^(864[2-9]|865[0-2])$ ]] || {
  echo "API_PORT must be an integer from 8642 through 8652" >&2
  exit 1
}
TOKEN=$($$nemoclaw headless-agent gateway-token --quiet)
curl -fsS -H "Authorization: Bearer $TOKEN" \
