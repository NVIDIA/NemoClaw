...
It also reports cumulative duration and attempt count for each launch-readiness observation, including sandbox identity, policy, inference, gateway health, forward health, and inference route checks.
Use the stage and observation values to identify where a slow or failed deployment spent its time.
The line reports lifecycle and forward actions, the first failed observation, readiness decisions, and the failed stage when available.
