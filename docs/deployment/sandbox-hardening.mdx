...
description-agent: "Includes the sandbox container image hardening reference, covering Docker capabilities and process limits. Use when reviewing sandbox image security controls, auditing capability ownership, or looking up the runtime resource limits."
...
OpenShell owns capability enforcement for managed entrypoints, exec processes, and connect shells.
The OpenClaw and Hermes entrypoints do not maintain a second capability drop, verification, sentinel, or fallback layer.
...
When you launch the image directly, make the container runtime the capability owner and drop all Linux capabilities there.
...
Personal shell files `.bashrc` and `.profile` belong to the sandbox user and can be edited.
System-wide shell hooks load runtime proxy settings from `/tmp/nemoclaw-proxy-env.sh`, which retains mode `0444`.
After updating NemoClaw, [rebuild existing sandboxes](../operate-sandboxes/recover-and-rebuild-sandboxes) to apply these profile permissions.
