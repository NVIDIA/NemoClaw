...
In the examples below, `nemoclaw-sandbox` is a placeholder for either the exact managed release image NemoClaw resolves during stock onboarding or an explicitly selected custom image.
Substitute a local image reference from `docker images` when launching it directly.
