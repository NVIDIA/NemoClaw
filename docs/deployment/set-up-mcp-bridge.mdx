...
Current NemoClaw builds require the pinned stable OpenShell `0.0.116` release, which exposes native `protocol: mcp` policy handling and endpoint-bound credential replacement.
Development builds are rejected because they do not match the reviewed credential boundary for the supported managed runtime.
...
| Failure behavior | Provider, policy, and adapter mutations fail closed when source state or live ownership cannot be proven. | Proxy failure could strand a listener, subprocess, or partially persisted secret-bearing launch state. |
| Crash recovery | Agent-native configuration plus live OpenShell resources are re-read under the per-sandbox lifecycle lock; no MCP registry or destroy marker is required. | Recovery would also have to discover orphan host processes and reconstruct their secret-bearing invocation state. |
...
<AgentOnly variant="hermes">
Hermes includes the stable credential handle from the readiness check in this placeholder.
Registration, inspection, rollback, and lifecycle reconciliation preserve that handle while OpenShell still reports it.
</AgentOnly>
...
The MCP integration pins its OpenShell DNS enforcement contract to [`NVIDIA/OpenShell@d1155aa70042d3e2ee49dbfa15346b108b7c1d92`](https://github.com/NVIDIA/OpenShell/tree/d1155aa70042d3e2ee49dbfa15346b108b7c1d92).
...
- [`crates/openshell-supervisor-network/src/proxy.rs:3857-3886`](https://github.com/NVIDIA/OpenShell/blob/d1155aa70042d3e2ee49dbfa15346b108b7c1d92/crates/openshell-supervisor-network/src/proxy.rs#L3857-L3886) produces the list.
- [`crates/openshell-supervisor-network/src/proxy.rs:3911-3951`](https://github.com/NVIDIA/OpenShell/blob/d1155aa70042d3e2ee49dbfa15346b108b7c1d92/crates/openshell-supervisor-network/src/proxy.rs#L3911-L3951) validates every address.
- [`crates/openshell-supervisor-network/src/proxy.rs:3983-4043`](https://github.com/NVIDIA/OpenShell/blob/d1155aa70042d3e2ee49dbfa15346b108b7c1d92/crates/openshell-supervisor-network/src/proxy.rs#L3983-L4043) connects through the validated list.

The CONNECT path passes that validated connector to the direct or operator-configured upstream dial path.
By default, direct connections and corporate-proxy CONNECT requests use the validated address list at [`crates/openshell-supervisor-network/src/proxy.rs:3983-4043`](https://github.com/NVIDIA/OpenShell/blob/d1155aa70042d3e2ee49dbfa15346b108b7c1d92/crates/openshell-supervisor-network/src/proxy.rs#L3983-L4043).
The explicit HTTP-forward path validates the destination before it connects at [`crates/openshell-supervisor-network/src/proxy.rs:5613-5655`](https://github.com/NVIDIA/OpenShell/blob/d1155aa70042d3e2ee49dbfa15346b108b7c1d92/crates/openshell-supervisor-network/src/proxy.rs#L5613-L5655).

The exception is OpenShell's operator-controlled `proxy_connect_by_hostname` option.
When enabled, OpenShell sends the requested hostname to the corporate proxy instead of the validated address, which reopens proxy-side DNS resolution; the upstream source records that residual risk at [`crates/openshell-supervisor-network/src/proxy.rs:3998-4031`](https://github.com/NVIDIA/OpenShell/blob/d1155aa70042d3e2ee49dbfa15346b108b7c1d92/crates/openshell-supervisor-network/src/proxy.rs#L3998-L4031).
...
### Stable OpenShell 0.0.116 Limitations

OpenShell v0.0.116 attributes network policy with `/proc/<pid>/exe` and process ancestors, so script-based adapters require Node or Python interpreter grants rather than immutable package-entrypoint identities.
...
OpenShell v0.0.116 requires a static credential provider to have a complete endpoint binding before the supervisor projects its stable credential handle.
...
It also rejects a second credential-bound definition for the same normalized endpoint URL because OpenShell cannot choose between credentials for otherwise indistinguishable requests.
Keep one managed definition for that URL or use distinct endpoint URLs.
...
OpenShell v0.0.116 updates, attaches, detaches, and deletes providers by mutable name instead of an atomic immutable identity.
...
<AgentOnly variant="openclaw">
OpenClaw writes the native `openclaw.json` `mcp.servers` map in the sandbox.
Use `mcp migrate --apply` to convert older managed Mcporter entries to native configuration.
To discard an entry instead, follow [Remove a Server](manage-mcp-servers#remove-a-server); removal requires proven registry ownership.
</AgentOnly>
<AgentOnly variant="hermes">
...
mcp_servers:
  github:
    ...
    tools:
      ...
      prompts: true
    headers:
      Authorization: Bearer openshell:resolve:env:s0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef_GITHUB_MCP_TOKEN
```

The `s…_` prefix is an illustrative OpenShell stable credential handle.
Hermes records the handle that the readiness check proves for the live provider.
...
Before converting an older managed Mcporter entry, run `nemoclaw <name> mcp migrate` to preview the change.
The preview explains that creating the native entry activates behavior OpenClaw previously did not load.

</AgentOnly>
<AgentOnly variant="deepagents">
The managed image pins Deep Agents Code `0.1.55` and uses its native `/sandbox/.deepagents/.mcp.json` configuration.
...
NemoClaw mutates only the named server and preserves unrelated native entries. Legacy `.nemoclaw-mcp.json` projections require explicit migration and are never layered with native state.
...
External credential values remain in OpenShell provider state, not sandbox files or NemoClaw state.
</AgentOnly>
