...
Current NemoClaw builds default to the pinned stable OpenShell `0.0.82` release, which exposes native `protocol: mcp` policy handling and provider-backed credential replacement.
...
The child-visible compatibility list is pinned to OpenShell `v0.0.82` commit `94cdd697c55aedb571f177ec13cfa54a8e213919` and must be reviewed with every OpenShell version change.
...
The pinned implementation is [`NVIDIA/OpenShell@94cdd697c55aedb571f177ec13cfa54a8e213919`](https://github.com/NVIDIA/OpenShell/tree/94cdd697c55aedb571f177ec13cfa54a8e213919).
In that implementation, [`crates/openshell-supervisor-network/src/proxy.rs:2648-2674`](https://github.com/NVIDIA/OpenShell/blob/94cdd697c55aedb571f177ec13cfa54a8e213919/crates/openshell-supervisor-network/src/proxy.rs#L2648-L2674) resolves the socket-address list, and [`crates/openshell-supervisor-network/src/proxy.rs:2699-2739`](https://github.com/NVIDIA/OpenShell/blob/94cdd697c55aedb571f177ec13cfa54a8e213919/crates/openshell-supervisor-network/src/proxy.rs#L2699-L2739) plus [`crates/openshell-supervisor-network/src/proxy.rs:2794-2803`](https://github.com/NVIDIA/OpenShell/blob/94cdd697c55aedb571f177ec13cfa54a8e213919/crates/openshell-supervisor-network/src/proxy.rs#L2794-L2803) validate and return that same list.
The CONNECT path passes the returned list directly to `TcpStream::connect` at [`crates/openshell-supervisor-network/src/proxy.rs:1079-1081`](https://github.com/NVIDIA/OpenShell/blob/94cdd697c55aedb571f177ec13cfa54a8e213919/crates/openshell-supervisor-network/src/proxy.rs#L1079-L1081).
The explicit HTTP-forward path carries the same returned list from [`crates/openshell-supervisor-network/src/proxy.rs:4093-4100`](https://github.com/NVIDIA/OpenShell/blob/94cdd697c55aedb571f177ec13cfa54a8e213919/crates/openshell-supervisor-network/src/proxy.rs#L4093-L4100) to [`crates/openshell-supervisor-network/src/proxy.rs:4340-4342`](https://github.com/NVIDIA/OpenShell/blob/94cdd697c55aedb571f177ec13cfa54a8e213919/crates/openshell-supervisor-network/src/proxy.rs#L4340-L4342).
A DNS change to a private, special-use, or otherwise unpinned address therefore fails closed instead of widening the route.
The stable `mcp-bridge` live lane verifies this OpenShell contract before any NemoClaw MCP mutation and independently of all three agent adapters.
The development lane first compares its installed moving OpenShell build with the reviewed child-visible credential manifest.
An exact version match runs the same full lifecycle, while any other development version records the expected fail-closed compatibility result and skips managed MCP mutation.
The full lifecycle OpenClaw scenario applies a raw `protocol: mcp` policy with `allowed_ips: [1.1.1.1]`, remaps the hostname to a reachable private runner address, sends a raw MCP `tools/list` request, requires an exact HTTP 403, verifies that the upstream server recorded zero requests, and restores the exact base policy in `finally`.
`restart` resolves the hostname again before updating that policy.
Authenticated MCP rejects `host.openshell.internal`, `host.docker.internal`, and `host.containers.internal` on stable OpenShell `v0.0.82`.
...
### Stable OpenShell 0.0.82 Limitations

OpenShell v0.0.82 attributes network policy with `/proc/<pid>/exe` and process ancestors, so the script-based adapters require Node or Python interpreter grants rather than immutable package-entrypoint identities.
...
OpenShell v0.0.82 attaches static provider credentials at sandbox scope rather than reserving a credential key exclusively for one endpoint.
...
OpenShell v0.0.82 updates, attaches, detaches, and deletes providers by mutable name rather than an atomic immutable identity.
