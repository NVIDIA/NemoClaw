...
`mcp status <server>` therefore also runs a wire-level credential-resolution probe by default: it sends one idempotent MCP `initialize` request from inside the sandbox, wrapped in the adapter runtime so the generated `protocol: mcp` policy applies, carrying the placeholder authorization header exactly as agent traffic does.
The verdict appears as `provider.credentialResolution` in JSON (`ok` true, false, or null with an optional `httpStatus` and redacted `detail`) and as a `credential resolution:` line in text output.
...
An HTTP 2xx verdict proves end-to-end resolution and HTTP 401 or 403 means the upstream rejected the request on the same path agents use; an HTTP 400, endpoint outages, policy denials, timeouts, and unreachable sandboxes report `ok: null` so an endpoint problem is never blamed on the host's credential rewrite.
Because some endpoints answer a literal placeholder bearer with HTTP 400, that verdict's detail names both hypotheses and recommends comparing against a known-good host.
...
If `mcp status <server>` reports `credential resolution: FAILED` while provider presence, attachment, credential readiness, and adapter registration are all healthy, the OpenShell gateway on this host is not rewriting the `openshell:resolve:env:KEY` placeholder on egress, and every agent request to that MCP server receives the same authentication failure.
This is a host-side OpenShell defect rather than a NemoClaw registration problem: verify the OpenShell installation on the host, and compare against a known-good host if one is available (tracked upstream as OpenShell issue 2161).
