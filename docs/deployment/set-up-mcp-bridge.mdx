...
A working rewrite makes the two requests reach the endpoint with different bearers; a dead rewrite forwards both literally, so identical HTTP rejections for both requests indict the host's placeholder rewrite without misblaming an expired or revoked credential, which resolves correctly and is then rejected upstream with a different status than the control.
The verdict appears as `provider.credentialResolution` in JSON (`ok` true, false, or null with the probe and control `httpStatus`/`controlHttpStatus` and a `detail`) and as a `credential resolution:` line in text output.
