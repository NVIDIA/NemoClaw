...
NemoClaw writes `proxy_connect_by_hostname = false` into its managed gateway configuration.
Before an MCP add, restart, or rebuild restoration mutates a policy, provider, or adapter, NemoClaw proves the selected running gateway identity and verifies that its launch-bound configuration keeps this setting disabled.
