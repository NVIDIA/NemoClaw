...
`mcp remove --force` and `destroy --force` report the refusal and continue with the host-side provider and policy cleanup instead of stopping.
A forced destroy leaves the retained volume's adapter entry in place, so onboarding a sandbox with the same name may need to replace that stale config; the exact OpenShell providers are still deleted, so the retained credential placeholder cannot authenticate.
