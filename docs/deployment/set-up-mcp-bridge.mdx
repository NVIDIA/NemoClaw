...
NemoClaw omits `proxy_connect_by_hostname` from its managed gateway configuration, which keeps OpenShell 0.0.106's disabled default.
Before an MCP add, restart, or rebuild restoration mutates a policy, provider, or adapter, NemoClaw proves the selected running gateway identity and verifies that its launch-bound configuration does not enable this setting.
An older gateway launch without launch-bound configuration identity is rejected even when the pinned OpenShell version would default the omitted setting to `false`.
Restart a NemoClaw-managed gateway through onboarding, or restart an externally managed gateway through its declared supervisor, before retrying the MCP mutation.
An enabled, changed, or unproven configuration fails closed before the MCP mutation.
