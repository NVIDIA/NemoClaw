...
Every non-2xx placeholder outcome is inconclusive with the hypotheses named: identical rejections cannot separate "placeholder forwarded verbatim" from "resolved but expired or revoked credential" (or, for HTTP 400, endpoint request validation), and differing rejections cannot either, because an endpoint may reject two different literal bearer strings differently. The identical-rejection warning tells you to verify the stored credential first; for HTTP 401 or 403, a confirmed-valid credential means the host is not rewriting placeholders, while HTTP 400 remains inconclusive because the endpoint may reject the probe request itself.
