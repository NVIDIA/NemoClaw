...
An exact older NemoClaw launch with the setting omitted remains valid because the pinned OpenShell version defaults it to `false`; the next managed gateway restart writes the explicit setting.
