...
`destroy --force` is the exception. It reports the refusal and completes the destruction, leaving the retained volume's adapter entry in place, so onboarding a sandbox with the same name may need to replace that stale config. The exact OpenShell providers are still deleted, so the retained credential placeholder cannot authenticate.
