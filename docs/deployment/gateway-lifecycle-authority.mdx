...
It exports the selected port for onboarding and records it in the selected port's state directory only after onboarding succeeds.
Later NemoClaw commands restore that recorded port when `NEMOCLAW_GATEWAY_PORT` is unset, while an explicit supported value remains authoritative.
