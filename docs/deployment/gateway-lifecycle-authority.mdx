...
Successful installer-driven onboarding promotes that identity to the completed marker; deferred, interrupted, or later direct CLI onboarding keeps the pending identity so a later installer or CLI process can reach the same state.
