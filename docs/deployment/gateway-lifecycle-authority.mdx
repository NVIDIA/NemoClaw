...
It checks that exactly one recognizable process holds the configured port.
The process identity must remain stable while NemoClaw reads `/proc` evidence and confirms the listener a second time.
The process must belong to the unit's cgroup, and its executable must match `execPath`.
