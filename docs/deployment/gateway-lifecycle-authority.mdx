...
It exports the selected port for onboarding and writes a pending port identity before onboarding begins.
Successful onboarding promotes that identity to the completed marker; interrupted onboarding keeps the pending identity so a later installer or CLI process can reach the same recovery state.
