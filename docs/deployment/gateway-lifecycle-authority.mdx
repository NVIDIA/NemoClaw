...
Successful onboarding promotes that identity to the completed marker; deferred or interrupted onboarding keeps the pending identity so a later installer or CLI process can reach the same recovery state.
