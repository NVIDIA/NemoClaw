...
## Version 2 Is Not Yet a Supported Integration

NemoClaw recognizes a version 2 declaration for authenticated interceptor and middleware connections during fresh Linux Docker-driver onboarding.
The shared OpenShell runtime currently pinned by NemoClaw does not provide all required version 2 behavior.
Do not rely on version 2 in an installed release until NemoClaw qualifies it against a released runtime.

Version 2 uses HTTPS endpoints, CA certificates, gateway preparation, and optional interceptor-backed provider profiles.
During preparation, it can create one missing attachable Docker bridge through the selected local Unix-socket connection.
It preserves that bridge after a preparation or startup failure.
It does not support TCP or SSH Docker connections.
It does not change the version 1 declaration or `/v1/activate` protocol.
NemoClaw continues to own generated gateway configuration and gateway lifecycle.
The component supplies policy, and OpenShell validates and enforces the effective sandbox policy.
...
OpenClaw and Hermes start without a configured inference route or model.
They cannot make an inference request until you configure a managed route.
After a managed inference provider is available, use `$$nemoclaw inference set --provider <provider> --model <model> --sandbox component-sandbox`.
Refer to [Switch Inference Providers](../inference/switch-providers) for the route requirements and verification behavior.
