...
During rebuild or warm recreation, the new image remains authoritative for image-owned extension directories: updated directories keep the fresh image copy, removed directories stay removed, and renamed directories do not restore the old directory.

NemoClaw then restores the credential-sanitized `openclaw.json` from the snapshot as a whole file. Saved plugin entries and load paths therefore replace the generated config entries from the replacement image. Verify plugin registration after a rebuild, especially when the image changes a plugin ID or load path.
