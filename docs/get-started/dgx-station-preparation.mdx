...
- A trusted but unrecognized Station software profile through validation-only factory-runtime handling.

NemoClaw confirms Station GB300 hardware from a bounded firmware product, product-family, or board value and an NVIDIA GB300 PCI device.
...
NemoClaw routes a trusted, complete marker with the Station GB300 platform and an unrecognized version or build to validation-only factory-runtime handling.
That handling validates the existing runtime without installing or changing host packages, the NVIDIA driver, or container-runtime configuration.
A malformed or unsafe marker, partial release identity, conflicting DGX platform, or other Station generation stops before host preparation.
...
Do not use the flag for a trusted, complete Station marker with an unrecognized version or build.
NemoClaw handles that case automatically through validation-only factory-runtime handling.
The flag is not a general compatibility bypass.
...
The validation-only path applies to a trusted, complete Station GB300 release marker whose version or build is not in the qualified profile catalog.
It can add the trusted account to the Docker group and cache the digest-pinned visibility-test image.
It does not install packages, repair CDI, register a Docker runtime, restart Docker or containerd, or apply profile-specific failed-service exceptions.
