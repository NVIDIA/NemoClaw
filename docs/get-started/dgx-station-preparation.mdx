...
DGX Station GB300 express install accepts either the generic Ubuntu 24.04 ARM64 image or stock DGX OS `7.2.0`, `7.4.0`, `7.5.0`, or `7.6.x`.
A stock DGX OS image qualifies only when a safe, root-owned `/etc/dgx-release` marker identifies `DGX Server for GALAXY-GB300`.
DGX OS `7.6.x` is recognized by its release metadata and remains subject to the runtime checks below; full Station Express end-to-end qualification for that release family is pending.
Stock DGX OS `7.6.x` can omit the `DGX_OTA_*` fields.
For that release family, NemoClaw requires `DGX_PRETTY_NAME=NVIDIA DGX GB300WS`, a numeric `7.6.x` `DGX_SWBUILD_VERSION`, and `DGX_SWBUILD_DATE` to be present, but does not match the date against an exact build.
...
1. Keep the managed Express recipe. This default leaves the host unchanged and prints the workload stop command and exact Express resume command.
...
For the manual Local vLLM choice, NemoClaw records the exact installer revision until installation succeeds.
...
A resident `packagekitd` process alone does not block stock DGX OS or AI Developer Tools preflight because those paths preserve the factory package stack.
