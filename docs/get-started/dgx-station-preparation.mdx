...
Running containers block initial preparation except during forced factory-runtime validation, and they always block Docker configuration changes.
...
The flag bypasses the `/etc/dgx-release` allowlist and the initial active agent, inference, port `8000`, and running Docker workload checks during forced factory-runtime validation.
This permits the installer to validate and use an existing healthy vLLM server.
It still requires ARM64 Ubuntu 24.04, DGX Station GB300 hardware identity, exactly one healthy GB300 for inference, zero volatile GB300 ECC errors, adequate storage, healthy required system services, and working Docker, Buildx, NVIDIA Container Toolkit, CDI, and container GPU-visibility probes.
If validation requires a CDI, Docker runtime, service, or reboot change, active agent, inference, port `8000`, and Docker workloads block that mutation.
