...
The gateway token is not printed in the install transcript.
If authentication is required, use the `dashboard-url --quiet` command to print a complete URL explicitly.
