...
<AccordionGroup>
  ...
  <Accordion title="Use Docker and supported platforms">
    ...
    Pass `--station-experimental-single-user` to opt in to the named reclaimed-v1/offload90 Station profile within the same managed-vLLM lifecycle; this flag does not change the default Station recipe.
    The experimental profile currently requires its exact qualified runtime image to be preloaded, so a fresh pull remains blocked until a repository-qualified manifest is published and proven.
  ...
  <Accordion title="Installer Behavior and Platform Details">
    ...
    To opt in to the experimental single-user Station profile, run `curl -fsSL https://www.nvidia.com/nemoclaw.sh | bash -s -- --station-experimental-single-user`.
    That profile fails closed unless Docker already has the exact qualified local image ID `sha256:e44830d3f0197850e5a63ca895b13a047f358ff4aea15b1655abf61391bb883e`; it does not substitute another vLLM image.
    ...
    The experimental single-user profile has prior functional qualification evidence, but this change has not completed a fresh, current-PR physical Station qualification.
    </Warning>
