...
<AccordionGroup>
  ...
  <Accordion title="Installer Behavior and Platform Details">
    ...
    After you confirm the interactive express prompt, the installer automates the remaining onboarding steps, allows `sudo` password prompts for required host changes, and selects the managed local inference path for that platform.
