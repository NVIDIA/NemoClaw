...
<AccordionGroup>
  ...
  <Accordion title="Onboarding and Inference Details">
    ...
    If registered sandboxes already exist, the installer first requires any existing OpenShell executable it will use to report a version.
    Only after that validation does it prepare the current NemoClaw CLI without replacing OpenShell and require a fresh backup of every registered sandbox before it changes the gateway.
