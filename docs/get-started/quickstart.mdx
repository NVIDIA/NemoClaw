...
<AccordionGroup>
  ...
  <Accordion title="Onboarding and Inference Details">
    ...
    The shared protected proxy keeps serving its existing backend while any sandbox may depend on that route; reuse the same endpoint or migrate those sandboxes before selecting a different no-authentication backend.
