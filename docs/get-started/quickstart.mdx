...
<AccordionGroup>
  ...
  <Accordion title="Onboarding and Inference Details">
    ...
    The endpoint port must not be reserved for the NemoClaw gateway (including its automatic allocation range, `8990` through `9005`), dashboard, per-sandbox API, auth proxy, or credential adapters.
