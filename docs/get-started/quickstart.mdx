...
<Steps>
  ...
  <Step title="Complete Onboarding">
    ...
    <Note>
      The installer can display `Run express install with these settings? [Y/n]:` before the agent-selection prompt only on supported DGX or Windows WSL hosts.
      ...
      If you accept, skip the rest of this procedure and [continue after express install](#continue-after-express-install).
...
## Continue After Express Install

If you accepted express install mode after running the bare hosted installer, the installer created an OpenClaw sandbox named `my-assistant`.
Skip installation and onboarding, then check the sandbox state.
...
<AccordionGroup>
  ...
  <Accordion title="Use Docker and supported platforms">
    ...
    Press Enter at the express install prompt to switch the remaining onboarding to non-interactive mode and select the managed local inference path for that platform.
