...
<AccordionGroup>
  ...
  <Accordion title="Use Docker and supported platforms">
    ...
    On a qualifying Station with no explicit model or peer, accepting the prompt checks for one already-trusted peer at the deterministic counterpart of two configured private `/30` ConnectX-8 rails. A qualified pair selects the pinned `nemotron-3-ultra-550b-a55b` managed-vLLM recipe aligned with NVIDIA's [dual-Station playbook](https://build.nvidia.com/station/nemoclaw/dual-nodes); otherwise Express retains the single-Station `deepseek-v4-flash` default.
    [Prepare DGX Station to Install NemoClaw](additional-setup/dgx-station-preparation) defines Station qualification, host preparation, trusted-pair and network boundaries, repair limits, and reboot handoff.
  ...
  <Accordion title="Installer Behavior and Platform Details">
    ...
    With no explicit model or peer, DGX Station express install selects `nemotron-3-ultra-550b-a55b` only after a trusted two-Station pair qualifies; otherwise it retains the Station managed-vLLM profile default, `deepseek-v4-flash`.
    NemoClaw does not configure the two private `/30` rails or enroll SSH trust. The operator owns physical isolation, firewalling, SSH trust, and manual reboots; the unauthenticated pair-coordination traffic is suitable only when both Stations and every host that can reach the rails are trusted.
    ...
    One physical DGX OS `7.5.0` GB300 validation completed, but Station remains Deferred pending repeat clean-host qualification, physical two-Station end-to-end validation, authenticated OpenShell validation, and CI coverage.
