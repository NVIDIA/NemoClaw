...
<AccordionGroup>
  ...
  <Accordion title="Installer Behavior and Platform Details">
    ...
    The temporary `--force-station-install` flag has the same terminal requirement and bypasses DGX release-metadata qualification plus the initial active-workload block on genuine Station GB300 hardware.
    This lets forced factory-runtime validation continue and use an existing healthy vLLM server.
    ...
    Active workloads still block any required CDI, Docker runtime, service, or reboot mutation.
