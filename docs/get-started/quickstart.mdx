...
The wizard then asks whether to enable Web Search.
If you accept, the wizard selects Brave Search by default and prompts for a Brave Search API key.
When `NEMOCLAW_EXPERIMENTAL=1` is set, the wizard also offers an unofficial, keyless DuckDuckGo backend; set `NEMOCLAW_WEB_SEARCH_PROVIDER=duckduckgo` to pre-select it and skip the provider prompt in both interactive and non-interactive runs.
...
The default **Balanced** tier includes common development presets such as npm, PyPI, Hugging Face, Homebrew, and either Brave Search or DuckDuckGo (depending on the provider you picked) when the selected agent supports web search.
