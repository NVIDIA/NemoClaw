...
<AccordionGroup>
  ...
  <Accordion title="Onboarding and Inference Details">
    ...
    After backup, it retires the running gateway before replacing OpenShell only when the installed OpenShell version is outside the current release's supported range; an invalid or missing range stops the update without retiring the gateway.
