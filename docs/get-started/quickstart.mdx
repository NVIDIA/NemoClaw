...
<AccordionGroup>
  ...
  <Accordion title="Use Docker and supported platforms">
    ...
    Accepting the Station prompt selects the pinned `nemotron-3-ultra-550b-a55b` managed-vLLM recipe and completes onboarding without more provider, model, policy, or sandbox-name choices.
    Unknown DGX OS versions, malformed or unsafe release markers, NVIDIA BaseOS images, and other Station generations stop before host preparation.
    Set `NEMOCLAW_PROVIDER` or `NEMOCLAW_NO_EXPRESS=1` explicitly to continue on an unqualified Station without host automation.
    On generic Ubuntu, host preparation probes the pinned driver, Docker, Buildx, and NVIDIA Container Toolkit versions, reuses exact matches, installs missing pins, and permits only the reviewed factory `dkms` transition.
    ...
    On stock DGX OS, the installer instead validates the existing GB300 driver and volatile ECC state, local Docker and Buildx, `nvidia-ctk` and CDI, and `nvidia-smi` device visibility through both GPU container launch paths.
    It does not install or replace host packages, enable or restart services, or rewrite the Docker runtime; aside from the private preparation log and separately disclosed `docker`-group membership grant when needed, only the digest-pinned visibility-test image can remain cached.
    The visibility probes do not replace physical acceptance of the pinned managed-vLLM and Nemotron Ultra recipe.
    This automation does not change Station's Deferred support status; product validation on physical DGX Station hardware remains pending.
  ...
  <Accordion title="Installer Behavior and Platform Details">
    ...
    Before onboarding, the Station path accepts either the generic Ubuntu 24.04 ARM64 image or stock DGX OS `7.2.0`, `7.4.0`, or `7.5.0` with a safe, root-owned `/etc/dgx-release` marker for `DGX Server for GALAXY-GB300`.
    Unknown versions, malformed or unsafe release markers, NVIDIA BaseOS images, and other Station generations are outside the automatic preparation boundary.
    Set `NEMOCLAW_PROVIDER` or `NEMOCLAW_NO_EXPRESS=1` to continue without Station host automation.
    On generic Ubuntu, preparation checks for NVIDIA open driver `610.43.02`, Docker CE `29.6.1` with Buildx, and NVIDIA Container Toolkit `1.19.1`.
    It reuses exact versions, installs missing pinned packages, permits only the reviewed `dkms` transition from `3.0.11-1ubuntu13` to `1:3.4.0-1ubuntu1`, and refuses every other mismatched version.
    ...
    On stock DGX OS, the path instead validates the existing GB300 driver and zero-error volatile ECC state, local Docker and Buildx, `nvidia-ctk` and CDI, and `nvidia-smi` device visibility through both CDI and `--gpus all`.
    It does not install or replace host packages, enable or restart services, generate CDI configuration, or rewrite the Docker runtime; aside from the private preparation log and separately disclosed `docker`-group membership grant when needed, only the digest-pinned visibility-test image can remain cached.
    The visibility probes do not replace physical acceptance of the pinned managed-vLLM and Nemotron Ultra recipe.
    This automation does not change Station's Deferred support status; product validation remains pending.
    ...
    Product validation of this express-install boundary is still pending.
    </Warning>
