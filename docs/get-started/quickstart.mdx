...
<Steps>
  ...
  <Step title="Complete Onboarding">
    ...
    <Note>
      ...
      On N1x, enter `n` to continue with the interactive onboarding flow described in this step.
      N1x remains Deferred, and the provider that you select must meet its existing requirements.
      Local NVIDIA NIM remains unavailable on N1x.
    </Note>
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="Use a container runtime and supported platforms">
    ...
    On a qualified Linux host, you can instead select native rootless Podman explicitly with `NEMOCLAW_GATEWAY_RUNTIME=podman`.
    The native Podman gateway path does not require the Docker CLI or daemon.
    Managed local vLLM remains Docker-backed; without Docker, attach an existing local vLLM server instead.
    ...
    When using the Docker path, if a coding agent reports that its execution sandbox blocked Docker, use its command-scoped approval flow, if available.
    ...
    On any express platform, set `NEMOCLAW_NO_EXPRESS=1` to skip the express prompt, or set `NEMOCLAW_PROVIDER` to choose a provider yourself.
    ...
    Accepting the N1x prompt supplies the narrow managed-vLLM preview intent required to pass the pending-validation readiness finding.
    Declining the N1x prompt or setting `NEMOCLAW_NO_EXPRESS=1` continues with ordinary interactive onboarding.
  ...
  <Accordion title="Onboarding and Inference Details">
    ...
    <Note>
    OpenClaw requires device pairing during onboarding. The managed startup profile still carries `NEMOCLAW_DISABLE_DEVICE_AUTH` and its provenance only as legacy compatibility inputs while managed callers transition. OpenClaw 2026.9.1 retired the bypass, so NemoClaw does not emit a device-auth bypass key and changing this input does not skip pairing.
    </Note>
    ...
    After the host upgrade, it runs `nemoclaw upgrade-sandboxes --auto` before generic host admission.
    Confirmed recovery rebuilds stale sandboxes and restores validated backups for registered sandboxes that are not Ready.
    The installer skips generic onboarding when no Station reconciliation remains necessary.
    Required Station reconciliation still passes through host preflight.
    If recovery exits 0 but the installer cannot inspect its output, it reports unconfirmed recovery and skips generic onboarding.
    If a recorded sandbox is not found on its recorded gateway, the installer reports that condition rather than claiming recovery.
    Both conditions finish with `Installation completed with warnings`.
    Refer to [Automatic Recovery Results](../manage-sandboxes/operate-sandboxes/update-sandboxes#interpret-automatic-recovery-results) before removing a stranded record or starting new onboarding.
    ...
    A custom OpenClaw sandbox uses the same validated state backup boundary; OpenClaw's native plugin and package state is restored with the rest of its declared state.
    ...
    For an OpenAI-compatible HTTP endpoint on `localhost`, `127.0.0.1`, or `[::1]`, press Enter to select no authentication when the endpoint uses an explicit port from `1024` through `65535`.
    The endpoint port must not be reserved for the NemoClaw gateway (including its automatic allocation range, `8990` through `9005`), dashboard, per-sandbox API, Model Router, auth proxy, or credential adapters.
    The shared protected proxy stays pinned to its existing backend; reuse that endpoint. Moving or removing a sandbox does not release the host-global binding. To select a different no-authentication backend, back up the sandboxes and remove the final NemoClaw gateway with the uninstaller before reinstalling.
