...
<AccordionGroup>
  ...
  <Accordion title="Installer Behavior and Platform Details">
    ...
    One physical DGX OS `7.5.0` GB300 validation completed, but Station remains Deferred pending repeat clean-host qualification and CI coverage.
    When an existing vLLM conflicts with the selected managed Express recipe, the installer shows the requested model and the running model when available, then offers two choices.
