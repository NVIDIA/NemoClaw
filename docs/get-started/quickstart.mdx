...
When `NEMOCLAW_EXPERIMENTAL=1` is set, the wizard also offers an unofficial, keyless DuckDuckGo backend; set `NEMOCLAW_WEB_SEARCH_PROVIDER=duckduckgo` to pre-select it in non-interactive runs.
