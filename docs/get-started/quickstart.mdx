...
<AccordionGroup>
  ...
  <Accordion title="Installer Behavior and Platform Details">
    ...
    One physical DGX OS `7.5.0` GB300 validation completed, but Station remains Deferred pending repeat clean-host qualification and CI coverage.
    When an existing vLLM conflicts with the selected managed Express recipe, the installer shows the requested model and the model reported by the configured vLLM port when available.
    The default keeps Express, leaves the host unchanged, and prints the manual stop and exact resume steps.
    The second choice leaves vLLM untouched and continues seamlessly through advanced manual Local vLLM setup.
    Without an interactive terminal or a choice, the installer safely selects the default.
    ...
    Active agent and unrelated Docker workloads still block Station preparation.
    Hardware, GPU, ECC, Docker, Buildx, NVIDIA Container Toolkit, CDI, container GPU-visibility, service, and container-inventory preservation checks still apply.
