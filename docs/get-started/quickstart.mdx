...
<Steps>
  ...
  <Step title="Complete Onboarding">
    ...
    <Note>
      The installer can display `Run express install with these settings? [Y/n]:` before the agent-selection prompt only on supported DGX or Windows Subsystem for Linux (WSL) hosts.
      Press Enter to use the recommended preset settings and managed local inference for that platform.
      ...
      Enter `n` to continue with the interactive onboarding flow described in this step.
    </Note>
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="Use Docker and supported platforms">
    ...
    DGX Spark, qualifying DGX Station hosts, and Windows WSL can offer the recommended express install mode after the third-party software notice.

    After you press Enter at the express install prompt, the installer switches the remaining onboarding to non-interactive mode and selects the managed local inference path for that platform.
  ...
  <Accordion title="Onboarding and Inference Details">
    The wizard uses read-only host checks while it collects your credential-free choices.
    It does not change Docker, gateway, provider, policy, or sandbox state until after the review.
    The choices include the agent, inference provider and model, sandbox name, web search, messaging channels, available managed tools, resources, and policy tier.
    When you select a custom resource profile, onboarding collects CPU, then RAM, then the sandbox GPU setting, and the review shows the CPU and RAM overrides.
    Onboarding does not request raw credentials during this stage, store them in the draft, or show them in the review.

    During choice collection, press Enter to accept the default shown in `[brackets]`, type `b` or `back` to return to the previous choice, or type `exit` to quit.
    Backspace continues to edit text in the current prompt.
    You can repeat `back` until you reach the first choice.
    At the review, select **Edit a choice** to jump directly to any available choice.
    Onboarding returns directly to the review when the saved later choices remain compatible.
    If your edit invalidates a dependent choice, onboarding clears it and reopens only the affected prompts before returning to the review.

    After you select **Apply configuration**, NemoClaw requests required credentials, registers inference and optional services, builds and starts the sandbox, sets up OpenClaw, and applies the selected network policy.
    These resource changes are forward-only, so `back` and **Edit a choice** are no longer available.
    Follow the printed repair or configuration command if you need to correct a choice after the changes begin.
    ...
    After choice collection, the wizard shows the complete review before it requests credentials or changes external resources.

    ```text
      Review configuration
        Agent: openclaw
        Inference: custom / openai/openai/gpt-5.5, http://localhost:8000/v1
        Sandbox: my-gpt-claw
        Web search: disabled
        Messaging: none
        Managed tools: none
        Resources: default, GPU: auto
        Policy: balanced
    ...
    **Apply configuration** is the default.
    Select **Edit a choice** to change an earlier answer without restarting onboarding.
    Non-interactive runs continue to print a configuration summary for log clarity but do not show these review actions.
  </Accordion>

  <Accordion title="Web Search Messaging and Network Policies">
    During choice collection, the wizard asks whether to enable web search and offers Brave Search or Tavily Search.
    After you apply the configuration, provide `BRAVE_API_KEY` for Brave Search or `TAVILY_API_KEY` for Tavily Search when prompted.
    ...
    The onboarding flow offers Telegram, Discord, Slack, WeChat, WhatsApp, Microsoft Teams, and Google Chat.
    Press a channel number to toggle it, then press Enter to continue.
    Leave every channel unselected to skip messaging setup.
    After you apply the configuration, NemoClaw requests the selected channel credentials and validates each pasted credential input before it builds the sandbox image.
    ...
    Before the review, NemoClaw asks which network policy tier to apply.
    Web search and messaging selections happen first so the policy suggestions stay aligned.
    After review, NemoClaw applies the reviewed tier's suggestions without reopening the preset picker.
    ...
    After onboarding, run `nemoclaw <sandbox-name> policy list` to inspect the applied presets.
    Run `nemoclaw <sandbox-name> policy add <preset>` to review and add another maintained preset.
