...
<AccordionGroup>
  ...
  <Accordion title="Choose inference and integrations">
    ...
    Web search and messaging are optional managed startup choices.
  ...
  <Accordion title="Onboarding and Inference Details">
    ...
    After confirmation, NemoClaw registers inference, prompts for optional web search and messaging channels, creates and starts the sandbox, sets up OpenClaw, and applies the selected network policy tier and presets.
    ...
    <Note>
    Stock onboarding selects the exact OpenClaw image digest from the installed CLI release's complete all-agent catalog.
    It does not stage or build an OpenClaw Dockerfile.
    NemoClaw applies the managed `NEMOCLAW_DISABLE_DEVICE_AUTH=1` compatibility setting through the transactional startup profile so the dashboard is usable during setup.
    NemoClaw records that this value came from onboarding rather than reporting it as an operator-selected opt-out.
    Setting the value in the host environment after onboarding does not affect an existing sandbox.
    </Note>
  ...
  <Accordion title="Web Search Messaging and Network Policies">
    ...
    When you select a channel, NemoClaw validates the token format before it applies the channel configuration through the managed startup profile.
    ...
    After the managed startup completes and OpenClaw starts, NemoClaw asks which network policy tier to apply.
