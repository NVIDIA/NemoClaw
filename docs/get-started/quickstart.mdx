...
<AccordionGroup>
  ...
  <Accordion title="Web Search Messaging and Network Policies">
    ...
    The choice stage also offers Telegram, Discord, Slack, WeChat, and WhatsApp.
    Press a channel number to toggle it, then press Enter to continue.
    Leave every channel unselected to skip messaging setup.
    After you apply the configuration, NemoClaw requests the selected channel credentials and validates each token format before it builds the sandbox.
