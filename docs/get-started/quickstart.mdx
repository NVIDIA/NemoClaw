...
<AccordionGroup>
  ...
  <Accordion title="Use Docker and supported platforms">
    ...
    Stock onboarding requires the complete immutable managed-image cohort and stops before mutation if it cannot be resolved. An explicit user-supplied `--from <Dockerfile>` remains a separate opt-in path.
