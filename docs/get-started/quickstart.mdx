...
<AccordionGroup>
  ...
  <Accordion title="Use Docker and supported platforms">
    ...
    DGX Spark, qualified Station GB300 hosts, and Windows WSL offer an interactive express-install path that chooses a managed local inference option for the platform.
    On a qualified Station, accepting the prompt selects the pinned `nemotron-3-ultra-550b-a55b` managed-vLLM recipe and completes onboarding without more provider, model, policy, or sandbox-name choices.
    [Prerequisites](prerequisites#dgx-station-express-preparation) defines the Station host qualification, preparation, repair limits, and reboot handoff.
    Station remains Deferred until full onboarding is validated on physical hardware.
  ...
  <Accordion title="Installer Behavior and Platform Details">
    ...
    On DGX Spark, qualified Station GB300 hosts, and Windows WSL, interactive installation offers express install after you accept the third-party software notice.
    ...
    If the host does not meet the [Station prerequisites](prerequisites#dgx-station-express-preparation), the installer stops before host preparation.
    After it changes a prerequisite, reboot, sign in, and run the printed exact-commit command to resume the accepted express recipe.
