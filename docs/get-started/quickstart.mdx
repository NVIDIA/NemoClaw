...
<AccordionGroup>
  ...
  <Accordion title="Dashboard and Terminal Details">
    ...
    NemoClaw waits up to 90 seconds for the dashboard forward to bind under the spawned OpenShell process.
    It then verifies that the spawned process owns the listener and keeps that ownership stable for one post-bind check interval.
    If OpenShell reports that the sandbox is still completing its readiness handoff, NemoClaw waits 5 seconds and retries up to 12 times.
    An exact forward process that exits for another reason receives one retry.
    If the process remains unbound at the deadline, NemoClaw stops that exact process and confirms its exit before retrying.
    The ordinary two-attempt sequence can take about 3 minutes and 15 seconds, including listener stability, cleanup, and retry delays; readiness-handoff retries use a separate one-minute delay budget.
    NemoClaw accepts the forward only when the spawned process owns the listener.
    It does not adopt an unrelated listener or start another forward while it cannot confirm that the prior process stopped.
