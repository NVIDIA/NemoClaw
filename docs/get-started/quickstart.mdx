...
<AccordionGroup>
  <Accordion title="Automate an installation">
    ...
    Set `NEMOCLAW_AGENT` (for example `openclaw`, `hermes`, or `langchain-deepagents-code`) to choose the agent, and set `NEMOCLAW_INSTALL_TAG=<git-ref>` to pin a specific release instead of the default last-known-good tag.
