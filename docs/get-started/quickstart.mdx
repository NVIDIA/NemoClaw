...
<Steps>
  ...
  <Step title="Complete Onboarding">
    ...
    If the installer displays `Run express install with these settings? [Y/n]:`, press Enter to use the recommended preset settings and managed local inference for that platform.
    Enter `n` if you want to choose an inference provider and model, then provide its credential when prompted.
    If you continue interactively, press Enter to accept the suggested `my-assistant` sandbox name.
