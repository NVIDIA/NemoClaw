...
<Steps>
  ...
  <Step title="Complete Onboarding">
    ...
    <Note>
      ...
      On native N1x FASTOS, the installer instead offers a Deferred preview pending a physical NemoClaw Express E2E run and discloses that status before confirmation.
      Press Enter to use the recommended preset settings on a qualified platform or to explicitly opt into the Deferred native N1x FASTOS preview.
      ...
      On native N1x FASTOS, enter `n` to continue with the interactive onboarding flow described in this step.
      Native N1x FASTOS remains Deferred, and the provider that you select must meet its existing requirements.
      On a qualified Windows WSL N1x host, Windows Express instead selects managed llama.cpp with Qwen 3.6; review [Windows preparation](additional-setup/windows-preparation) for its readiness conditions and Ollama fallback.
      Local NVIDIA NIM remains unavailable on either N1x path.
    </Note>
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="Use a container runtime and supported platforms">
    ...
    Native N1x FASTOS can offer a Deferred preview after the notice, but full physical NemoClaw Express E2E validation is pending.
    Native N1x FASTOS remains outside the supported-platform set until that validation passes and the platform matrix status is promoted.
    ...
    Native N1x FASTOS Express selects one-host managed vLLM with `nvidia/Qwen3.6-35B-A3B-NVFP4`.
    Native N1x detection requires Linux `arm64`, a trusted `/etc/fastos-release` marker with `NAME="N1x FASTOS"`, and NVIDIA vendor ID `10de` on a display-class PCI device.
    The native N1x FASTOS path does not use DGX Spark cluster discovery, the fixed catalog path, managed llama.cpp, or NVIDIA NIM.
    Accepting the native N1x FASTOS prompt supplies the narrow managed-vLLM preview intent required to pass the pending-validation readiness finding.
    Declining the native N1x FASTOS prompt or setting `NEMOCLAW_NO_EXPRESS=1` continues with ordinary interactive onboarding.
