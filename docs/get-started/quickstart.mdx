...
<AccordionGroup>
  ...
  <Accordion title="Onboarding and Inference Details">
    ...
    For an OpenAI-compatible endpoint on the exact loopback hosts `localhost`, `127.0.0.1`, or `[::1]`, choose **No authentication** when the upstream does not require a key.
    In non-interactive mode, set `NEMOCLAW_COMPATIBLE_AUTH_MODE=none` explicitly.
    Other compatible endpoint URLs require their credential variable.
