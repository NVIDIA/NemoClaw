...
<AccordionGroup>
  ...
  <Accordion title="Web Search Messaging and Network Policies">
    ...
    Before it prints the final summary, NemoClaw checks that the sandbox gateway, dashboard port forward, and `inference.local` route are reachable.
    ...
    An inference route that is unreachable or returns HTTP 5xx marks the sandbox not ready and makes onboarding exit non-zero.
    Restore the configured endpoint or proxy, then rerun `nemoclaw <sandbox-name> status` to verify the route.
