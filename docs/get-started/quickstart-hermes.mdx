...
<Steps>
  ...
  <Step title="Complete Onboarding">
    ...
    <Note>
      ...
      This express install mode uses preselected settings for that platform and runs onboarding non-interactively.
      Enter `n` to continue with this interactive Hermes flow.
      Refer to [Use Docker and supported platforms](#use-docker-and-supported-platforms) for more information.
    </Note>

    Enter `my-hermes` as the sandbox name, or press Enter to accept the suggested `hermes` name.
...
<AccordionGroup>
  ...
  <Accordion title="Use Docker and supported platforms">
    ...
    DGX Spark, qualifying DGX Station hosts, and Windows WSL can offer an express install mode after the third-party software notice.
    After you accept the express install prompt, the installer switches the remaining onboarding to non-interactive mode and selects the managed local inference path for that platform.
