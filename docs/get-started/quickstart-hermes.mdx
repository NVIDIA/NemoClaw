...
<AccordionGroup>
  ...
  <Accordion title="Use Portable Ollama">
    On Linux x86_64, fresh Hermes Portable onboarding can run the selected Ollama model in a current-user rootless Podman container.
    This path requires the Portable preflight to accept the current user's Podman and NVIDIA GPU authority.

    <Note>
    OpenShell and Podman executables and their parent directories must be owned by root or your current user.
    They must not be writable by the group or other users, even when the group is private.
    A rejected path with mode `0775` needs its group write permission removed.
    If the diagnostic names your own `~/.local/bin` directory, restrict only that directory:

    ```bash
    chmod go-w -- "$HOME/.local/bin"
    stat -c '%a %n' -- "$HOME/.local/bin"
    ```

    A directory that had mode `0775` now has mode `755`.
    This change does not modify files inside the directory or contact any service.
    For a different rejected path, inspect its ownership before changing permissions.
    Ask your administrator about paths you do not own.
    After correcting the reported path, retry the recovery command printed by onboarding without deleting its retained state.
    </Note>
