...
<AccordionGroup>
  ...
  <Accordion title="Use Docker and supported platforms">
    ...
    The first Hermes startup can take several minutes while Docker pulls the exact Hermes image digest for the installed NemoClaw release.
    Stock onboarding does not stage or build a Hermes Dockerfile.
  ...
  <Accordion title="Onboarding and Integration Details">
    ...
    After confirmation, NemoClaw registers inference, prompts for optional Tavily Search and supported messaging channels, creates and starts the sandbox, sets up Hermes, and applies the selected network policy tier and presets.
    ...
    After you select a provider and model, review the summary and confirm sandbox creation.
    Stock onboarding selects the exact Hermes image digest from the installed CLI release's complete all-agent catalog and applies Hermes configuration through the transactional startup profile.
  ...
  <Accordion title="Noninteractive Setup Details">
    ...
    Changing or disabling Tavily requires sandbox recreation because the backend, credential attachment, and policy selection are managed startup inputs.
