...
<AccordionGroup>
  ...
  <Accordion title="Use the dashboard and API remotely">
    ...
    The Hermes dashboard service remains bound to loopback inside the sandbox, but onboarding then starts the host-side dashboard forward on all interfaces (`0.0.0.0`). Other hosts may reach the dashboard at this host's address, subject to the host firewall. [Run Sandboxes](../manage-sandboxes/run-sandboxes) describes how a later forward recovery selects the bind and which commands report it. To keep the dashboard forward on loopback, leave `CHAT_UI_URL` unset and use SSH port forwarding.
