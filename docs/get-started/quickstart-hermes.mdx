...
<AccordionGroup>
  ...
  <Accordion title="Use Portable Ollama">
    ...
    A host Ollama service or leftover Portable container on those published addresses fails inference create before the Portable Ollama runner starts.
  ...
  <Accordion title="Manage a Hermes sandbox">
    Use the `nemohermes` alias for lifecycle, logs, rebuilds, and model changes.
...
- [Persist Sandbox State](../manage-sandboxes/state-and-backups/create-and-restore-snapshots) explains OpenShell persistence and whole-home rebuild transfer.
