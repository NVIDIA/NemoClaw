...
<Steps>
  ...
  <Step title="Complete Onboarding">
    ...
    The install command uses `my-hermes` as the sandbox name.
...
<AccordionGroup>
  ...
  <Accordion title="Onboarding and Integration Details">
    Before review, the wizard collects the agent, inference provider and model, sandbox name, web search and messaging channel choices, and available managed tools.
    It also collects the resource profile, sandbox GPU setting, and policy tier.
    ...
    After you select **Apply configuration**, NemoClaw requests required credentials, registers inference and optional services, builds and starts the sandbox, sets up Hermes, and applies the selected network policy.
    ...
    After choice collection, review all selections and confirm the build.
    Select **Edit a choice** to change one selection directly.
