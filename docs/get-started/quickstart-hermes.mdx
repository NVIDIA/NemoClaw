...
<AccordionGroup>
  ...
  <Accordion title="Use the dashboard and API remotely">
    ...
    For a remote dashboard origin or tunnel, set `CHAT_UI_URL` to an externally reachable HTTPS dashboard origin before onboarding.
    The Hermes dashboard accepts that configured hostname, including WebSocket requests, while it remains loopback-bound.
    The configured hostname does not authenticate proxy clients.
    Protect dashboard ingress with your reverse proxy or other deployment controls.
