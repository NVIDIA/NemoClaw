...
<AccordionGroup>
  ...
  <Accordion title="Use the dashboard and API remotely">
    ...
    Those two ports belong to the first Hermes sandbox on a host.
    For a later Hermes sandbox, NemoClaw allocates the next free dashboard port from `18789` through `18799` and the next free API port from `8642` through `8652`.
  ...
  <Accordion title="Dashboard and API Details">
    ...
    When either port is taken, NemoClaw allocates the next free port in that port's range, so the examples below use the first sandbox's ports.
