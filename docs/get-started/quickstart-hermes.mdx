...
<AccordionGroup>
  ...
  <Accordion title="Use the dashboard and API remotely">
    ...
    The Hermes dashboard service remains bound to loopback inside the sandbox, but the host-side dashboard forward then listens on all interfaces (`0.0.0.0`), at onboarding and at every later forward recovery, so other hosts may reach the dashboard at this host's address, subject to the host firewall. `nemoclaw <name> dashboard-url`, `nemoclaw list`, and `nemoclaw <name> status` report that bind. To keep the dashboard forward on loopback, leave `CHAT_UI_URL` unset and use SSH port forwarding.
