...
<AccordionGroup>
  ...
  <Accordion title="Use Portable Ollama">
    ...
    This experimental path requires both the Podman client and the local rootless Podman engine to be exactly version `5.7.0`.
    Newer and older versions are rejected.
    NemoClaw does not install Podman, and a remote-client-only package does not provide the local engine this profile needs.
    Run `podman version` before onboarding and confirm that both the client and server report `5.7.0`.
