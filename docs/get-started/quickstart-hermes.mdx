...
<AccordionGroup>
  ...
  <Accordion title="Use the dashboard and API remotely">
    ...
    For a remote dashboard origin or tunnel, set `CHAT_UI_URL` to the externally reachable HTTPS dashboard origin before onboarding.
    Do not include credentials in the URL.
    NemoClaw keeps the dashboard process bound to loopback and allows only the configured hostname through Hermes dashboard host validation.
    This hostname check does not authenticate a reverse proxy.
    Protect the proxy or other ingress before you expose the dashboard.
