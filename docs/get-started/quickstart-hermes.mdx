...
<AccordionGroup>
  ...
  <Accordion title="Dashboard and API Details">
    ...
    NemoClaw waits up to 90 seconds for each forward to bind under its spawned OpenShell process.
    It then verifies that each spawned process owns its listener and keeps that ownership stable for one post-bind check interval.
    ...
    The ordinary two-attempt sequence can take about 3 minutes and 15 seconds for each forward, including listener stability, cleanup, and retry delays; readiness-handoff retries use a separate one-minute delay budget.
    NemoClaw accepts each forward only when its spawned process owns the listener.
