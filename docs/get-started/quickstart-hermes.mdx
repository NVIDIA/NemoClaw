...
<AccordionGroup>
  ...
  <Accordion title="Dashboard and API Details">
    ...
    NemoClaw waits up to 90 seconds for each forward to bind and remain owned through OpenShell's first post-bind sandbox check.
    ...
    The ordinary two-attempt sequence can take just over 3 minutes for each forward; readiness-handoff retries use a separate one-minute delay budget.
