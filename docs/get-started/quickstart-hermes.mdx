...
<Steps>
  ...
  <Step title="Complete Onboarding">
    ...
    <Note>
      On supported platforms other than native N1x FASTOS, the installer can display `Run express install with these settings? [Y/n]:` before ordinary onboarding.
      Native N1x FASTOS instead displays `Run the Deferred N1x preview with these settings? [Y/n]:`.
      ...
      On native N1x FASTOS, the selected provider must meet its existing requirements, and local NVIDIA NIM remains unavailable.
...
<AccordionGroup>
  ...
  <Accordion title="Use a container runtime and supported platforms">
    ...
    On native N1x FASTOS, accept the preview prompt to apply those settings as explicit Deferred preview intent.
    Enter `n` to continue with interactive onboarding when you want to select the agent or other settings yourself.
    On native N1x FASTOS, setting `NEMOCLAW_NO_EXPRESS=1` also continues with ordinary onboarding.
    ...
    The native N1x FASTOS preview selects one-host managed vLLM with `nvidia/Qwen3.6-35B-A3B-NVFP4`.
    On a qualified Windows WSL N1x host, Windows Express instead selects managed llama.cpp with Qwen 3.6; review [Windows preparation](additional-setup/windows-preparation) for its readiness conditions and Ollama fallback.
