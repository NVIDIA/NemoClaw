...
<AccordionGroup>
  ...
  <Accordion title="Use Docker and supported platforms">
    ...
    On supported `amd64` Docker hosts, stock Hermes onboarding prefers the exact managed release image and applies current configuration through a secret-free startup profile.
    Other supported Docker paths can use the trusted canonical Dockerfile fallback; its first local build can take several minutes when the base is not already cached.
