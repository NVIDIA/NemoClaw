...
<AccordionGroup>
  ...
  <Accordion title="Use Docker and supported platforms">
    ...
    On supported `amd64` and `arm64` hosts, stock Hermes onboarding requires the exact managed release image and applies current configuration through a secret-free startup profile.
    NemoClaw stops before it creates or replaces the sandbox when the complete release cohort cannot be resolved; gateway startup and provider preparation can already have completed.
    It never silently falls back to a Dockerfile.
    An explicitly selected custom Dockerfile remains a separate local-build workflow and can take several minutes when the base is not already cached.
  ...
  <Accordion title="Onboarding and Integration Details">
    ...
    After confirmation, NemoClaw registers inference, prompts for optional Tavily Search and supported messaging channels, prepares and starts the sandbox, sets up Hermes, and applies the selected network policy tier and presets.
    ...
    After you select a provider and model, review and confirm the configuration.
    NemoClaw materializes Hermes configuration in `/sandbox/.hermes`, routes model traffic through `inference.local`, and starts the Hermes gateway inside the sandbox.
  ...
  <Accordion title="Noninteractive Setup Details">
    ...
    Changing or disabling Tavily requires sandbox recreation because the backend, credential attachment, and policy selection are sandbox-creation and startup inputs.
