...
The Hermes wizard offers Tavily Search and Firecrawl Search as its web search providers.
Hermes does not support the NemoClaw Brave Search path.
If you enable Tavily Search, enter `TAVILY_API_KEY` when prompted; if you enable Firecrawl Search, enter `FIRECRAWL_API_KEY` when prompted.
NemoClaw validates the key, stores it in a sandbox-scoped OpenShell provider, writes `web.backend: tavily` (or `web.backend: firecrawl`) into the Hermes configuration, and writes only an OpenShell resolver placeholder into the generated environment.
...
If you select a bring-your-own-key web search provider (Tavily or Firecrawl) together with the managed Nous web gateway, the selected provider becomes the Hermes web search and extract backend.
...
Set `NEMOCLAW_WEB_SEARCH_PROVIDER=firecrawl` with `FIRECRAWL_API_KEY` to use Firecrawl Search instead of Tavily.
Set `NEMOCLAW_WEB_SEARCH_PROVIDER=none` when you want to disable web search explicitly.
When the selector is unset, Hermes enables Tavily automatically when `TAVILY_API_KEY` is available, or Firecrawl when only `FIRECRAWL_API_KEY` is available, and ignores `BRAVE_API_KEY` because Brave Search is unsupported for Hermes.
Changing or disabling the web search provider requires a sandbox recreation because the backend, credential attachment, and policy selection are build-time inputs.
...
When Firecrawl is enabled, onboarding confirms `web.backend: firecrawl` and sends a real search request through OpenShell's `Authorization` header credential rewrite path.
