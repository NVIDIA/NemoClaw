...
<AccordionGroup>
  ...
  <Accordion title="Use the dashboard and API remotely">
    ...
    The Hermes dashboard service remains bound to loopback inside the sandbox, but onboarding then starts the host-side dashboard forward on all interfaces (`0.0.0.0`), so other hosts may reach the dashboard at this host's address, subject to the host firewall. A later forward recovery, such as `connect` or `recover` re-creating a missing forward, does not read `CHAT_UI_URL`: it re-creates the forward on all interfaces only when that environment sets `NEMOCLAW_DASHBOARD_BIND=0.0.0.0` for a sandbox prepared for remote exposure, or when the host is WSL, and on loopback otherwise. `nemoclaw <name> dashboard-url` and `nemoclaw list` report the bind recorded by the most recent forward launch; `nemoclaw <name> status` omits its SSH port-forward guidance while that recorded bind is `0.0.0.0`. To keep the dashboard forward on loopback, leave `CHAT_UI_URL` unset and use SSH port forwarding.
