...
<Note>
Hermes uses an agent-specific baseline policy that allows the Hermes binary and Python runtime to reach the required Nous Research service endpoints, PyPI, and NVIDIA inference endpoints. Messaging egress is added through the selected channel policy presets.
</Note>
