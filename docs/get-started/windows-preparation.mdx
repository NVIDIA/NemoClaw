...
NemoClaw reuses a Windows-host daemon only when it is bound to `127.0.0.1:11434`, Docker Desktop can reach it, and it returns `403` for an untrusted HTTP `Host` value.
