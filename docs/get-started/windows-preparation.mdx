...
With native Docker Engine inside WSL — or when the Docker runtime is unavailable or cannot be probed — express install now configures WSL-local Ollama instead of aborting.
