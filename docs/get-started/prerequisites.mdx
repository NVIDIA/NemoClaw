...
| Linux on DGX Spark | Docker | Tested | Use the standard installer and `$$nemoclaw onboard`. For an end-to-end walkthrough with local inference, see the [NVIDIA Spark playbook](https://build.nvidia.com/spark/nemoclaw). |
| Linux on DGX Station | Docker | Deferred | Deferred. For evaluation on a qualifying DGX Station GB300, follow [Prepare DGX Station for NemoClaw](prerequisites/dgx-station-preparation) before the Quickstart. |
...
If you are evaluating a qualifying DGX Station GB300, follow [Prepare DGX Station for NemoClaw](prerequisites/dgx-station-preparation) before the Quickstart.

### Windows Preparation

If you are using Windows, follow [Prepare Windows for NemoClaw](prerequisites/windows-preparation) before the Quickstart.
