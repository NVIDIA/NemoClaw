...
| DGX OS (Station) | Docker | Tested with limitations | Tested with limitations on DGX Station GB300. Qualified image profiles use their reviewed preparation behavior. A trusted, complete Station marker with an unrecognized version or build uses validation-only factory-runtime handling. See [Additional Setup for DGX Station](additional-setup/dgx-station-preparation) for hardware identity, runtime gates, and current dual-Station and dedicated CI limitations. |
