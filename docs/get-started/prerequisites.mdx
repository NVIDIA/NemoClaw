...
| DGX OS (Station) | Docker | Tested with limitations | Tested with limitations on DGX Station GB300. Qualified image profiles use their reviewed preparation behavior. Hardware identity is reported separately when an unrecognized software profile remains blocked. See [Additional Setup for DGX Station](additional-setup/dgx-station-preparation) for hardware identity, runtime gates, and current dual-Station and dedicated CI limitations. |
