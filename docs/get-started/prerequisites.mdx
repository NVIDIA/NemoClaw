...
NemoClaw uses the native filesystem support installed with its Node.js package for migration snapshot sanitization and deletion.
The sanitizer fails closed if that platform package is missing or cannot load.
You do not need a host Python installation for snapshot operations.
