...
| DGX OS (Station) | Docker | Tested with limitations | Tested with limitations across qualified profiles on one physical DGX Station GB300. DGX OS `7.6.x` is recognized only for the no-OTA NVIDIA DGX GB300WS profile and remains runtime-gated. Full Station Express end-to-end qualification for that profile is pending. Dual-Station configurations are not yet validated, and dedicated CI coverage is not available. See [Additional Setup for DGX Station](additional-setup/dgx-station-preparation) before the Quickstart. |
