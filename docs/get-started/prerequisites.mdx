...
If you use Colima through Homebrew, install both Colima and the Docker CLI before starting the runtime:
...
| macOS (Apple Silicon) | Colima, Docker Desktop | Tested with limitations | Start the container runtime (Colima or Docker Desktop) before running the installer. Homebrew Colima users must install the Docker CLI separately (`brew install colima docker`) before `docker info` can reach the Colima daemon. Xcode Command Line Tools (`xcode-select --install`) are typically required for Node native modules during install. NemoClaw recommends them but does not enforce them during preflight. |
