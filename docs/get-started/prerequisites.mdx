...
| npm        | 10 or later |
| Python     | Python 3, required for Model Router onboarding and direct blueprint-runner `snapshots prune` and `snapshots delete` operations on POSIX hosts |
...
Model Router onboarding requires a qualifying host Python interpreter.
