...
For a machine-readable headless result, add `--json` and set explicit execution bounds.

```bash
dcode -n "Summarize this repository" --json --max-turns 1 --timeout 120
```

Managed `dcode -n --json` writes exactly one UTF-8 JSON object to `stdout`.
Progress, tool activity, and diagnostics use `stderr`.
The complete JSON object cannot exceed `1048576` bytes.
If the response or an unexpected `stdout` write would break that limit or framing, the command returns a bounded failure envelope with exit code `1`.
...
Use `response` only when `status` is `succeeded` and `exit.classification` is `success`.
For other results, `response` is empty and `completion.responseComplete` is `false`.

| `status` | `exit.code` | `exit.classification` | Meaning |
|---|---:|---|---|
| `succeeded` | `0` | `success` | The final assistant response is complete. |
| `failed` | `1` | `agent_failure` | The managed agent or its model request failed. |
| `failed` | `1` | `process_failure` | The runtime, framing, encoding, or output limit failed. |
| `timed_out` | `124` | `timeout` | The `--timeout` or `--max-turns` bound ended the run. |
| `cancelled` | `130` | `cancelled` | The caller interrupted the run. |

The `completion` object reports the thread ID, elapsed milliseconds, observed response bytes, completion state, and envelope limit.
`failure` is `null` on success.
For all other results, `failure.reason` is the exit classification or a specific framing, encoding, or output-limit reason.
