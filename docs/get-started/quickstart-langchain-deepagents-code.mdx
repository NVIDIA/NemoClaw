...
The release managed image installs hash-locked Deep Agents Code `0.1.34` with NVIDIA provider support.
Stock onboarding selects the exact Deep Agents Code image digest from the installed CLI release's complete all-agent catalog and creates the sandbox without staging or building a Dockerfile.
...
When onboarding records a reasoning effort on a `compatible-endpoint` route that uses `openai-completions`, the transactional startup profile writes that value to a root-owned file and Deep Agents Code model requests carry it as an `extra_body.reasoning_effort` request parameter.
...
Before a managed Deep Agents Code rebuild changes the sandbox, NemoClaw selects its recorded OpenShell gateway, tests the recorded inference route through `https://inference.local`, verifies the provider-bound workload, and resolves the exact current-release image through the complete all-agent catalog.
Initial failures stop before backup.
After backup, NemoClaw rechecks the target, route, managed image, and startup profile before changing MCP state, then checks again after MCP preparation and before stopping inference or deleting the old sandbox.
