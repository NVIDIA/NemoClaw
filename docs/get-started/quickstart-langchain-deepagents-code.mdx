...
description-agent: "Creates and operates a NemoClaw sandbox that runs LangChain Deep Agents Code with managed inference and host-side OTLP or LangSmith trace export. Use when installing or testing dcode, enabling observability, or configuring a local OpenTelemetry collector for LangSmith."
...
LangGraph node scopes export only bounded node names and success or error status.
