...
During managed re-onboarding, NemoClaw restores only explicitly allowlisted, validated `config.toml` UI and thread-display preference keys while keeping the freshly generated model routing, update settings, and provider metadata authoritative.
It does not restore behavior-bearing, unknown, or security-sensitive tables and keys from the backup.
