...
The managed wrapper launches Deep Agents Code with `HOME=/sandbox`, update checks disabled, manual update/install commands blocked, remote Deep Agents sandbox providers disabled, MCP auto-loading disabled, shell allow-list overrides blocked, and LangSmith tracing disabled.
...
NemoClaw intentionally does not preserve `.env` or `.mcp.json` because users may put Tavily, LangSmith, MCP service, or provider credentials there, and this managed harness disables MCP at runtime. If upstream Deep Agents Code credentials are stored in `.deepagents/.state/auth.json`, the managed wrapper refuses to launch until they are removed so they cannot bypass NemoClaw credential handling.
...
The managed wrapper also refuses direct runtime, `.env`, or upstream `/auth` credential storage for this harness; use NemoClaw-managed credential paths when support is available instead of storing service keys inside Deep Agents Code state.
...
The managed wrapper forces `LANGSMITH_TRACING=false` and `DEEPAGENTS_CODE_LANGSMITH_TRACING=false`, and no policy preset opens `api.smith.langchain.com`.
Treat tracing as unsupported until NemoClaw ships a maintained `langsmith` preset and credential flow.
