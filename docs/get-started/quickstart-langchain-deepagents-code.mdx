...
Enable the capability during initial onboarding when you want individual threads to be able to opt in.

```bash
nemo-deepagents onboard --dcode-auto-approval thread-opt-in
```

Changing an existing sandbox requires a transactional rebuild because NemoClaw bakes the capability into the managed image.
