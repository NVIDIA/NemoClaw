...
`start.sh` persists the non-secret `LANGSMITH_TRACING` toggle for login shells, but it does not persist `LANGSMITH_PROJECT`, `DEEPAGENTS_CODE_LANGSMITH_PROJECT`, or `LANGSMITH_API_KEY` in the shared shell environment.
