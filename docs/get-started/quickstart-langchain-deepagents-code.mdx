...
The image includes `ripgrep`, and ordinary sessions suppress the optional Tavily warning unless web search is configured or invoked.
  ...
  <Accordion title="Run the managed runtime">
    ...
    Continue with [Run Deep Agents Code](../manage-sandboxes/operate-sandboxes/run-deep-agents-code) for sandbox selection, interactive and headless tasks, the JSON output contract, runtime restrictions, headless validation profiles, thread auto-approval, and identity checks.
  </Accordion>
