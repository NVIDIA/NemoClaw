...
Any file creation, write, metadata change, rename, or deletion during verification or execution terminates the command and returns `source_identity_mismatch`; the runtime also recomputes the identity after exit before it can report success.
