...
After upgrading from a release that persisted LangSmith environment values, rebuild existing Deep Agents Code sandboxes because the corrected `start.sh` is baked into the image.
