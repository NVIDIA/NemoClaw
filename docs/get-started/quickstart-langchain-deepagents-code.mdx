...
Use a stable task identifier for `taskIdentity` and a `sha256:` source identifier for `sourceIdentity`.
...
The managed runtime admits only an exact profiled argument vector and executes it directly with `shell=False`.
...
- Profile, source, argument, stdout, and stderr digests.
