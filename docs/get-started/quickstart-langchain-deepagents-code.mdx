...
If upstream Deep Agents Code credentials are stored in `.deepagents/.state/auth.json`, the managed wrapper refuses to launch until they are removed so they cannot bypass NemoClaw credential handling.
