...
Rebuild also preserves the standalone Deep Agents Code `tavily` preset, the recorded observability choice, and recorded custom policies from their exact stored content.
...
Start a collector that accepts protobuf trace requests on a host interface reachable through `host.openshell.internal:4318`, then opt in while onboarding the Deep Agents Code sandbox.

```bash
nemo-deepagents onboard --observability
```

Observability is off when you omit `--observability`.
NemoClaw records the opt-in with the onboarding session and sandbox, so resume and rebuild operations preserve it without requiring the flag again.
The `restricted` policy tier suppresses the required egress preset even when the opt-in is recorded.
In non-interactive onboarding, `NEMOCLAW_POLICY_MODE=skip` still skips all policy changes; on a new sandbox that leaves the collector unreachable, and the exporter continues fail-open without delivering traces.

The sandbox always sends headerless OTLP/HTTP protobuf traces to `http://host.openshell.internal:4318/v1/traces`.
The managed exporter reports `service.name=nemoclaw-langchain-deepagents-code` for routing convenience.
It cannot select a remote endpoint, add exporter headers, or receive a backend credential.
The host collector owns backend selection, TLS, authentication, batching, and any additional filtering.
You can route the same receiver to Jaeger, Phoenix, LangSmith, or another OTLP-compatible backend without rebuilding or reconfiguring the sandbox.
Exporter initialization, delivery, and flush failures do not stop Deep Agents Code work.

The source-side metadata-only profile retains operation, model, and tool names plus success or error status.
It excludes prompts, model outputs, tool arguments, tool results, callback metadata, exception text, and human-in-the-loop interrupt values.
Native LangSmith tracing remains disabled.
The managed launch paths continue to force LangSmith and LangChain tracing flags to `false`, force `OTEL_ENABLED=false`, reject ambient OTLP endpoints and headers, and remove those values from the LangGraph child process.
Do not put `LANGSMITH_API_KEY` or other backend credentials in the sandbox.

The collector must satisfy this receiver contract:

- Listen for OTLP/HTTP on a host interface reachable from the sandbox as `host.openshell.internal:4318`.
- Accept only `POST /v1/traces` for this pipeline.
- Accept requests without authentication headers from the sandbox.
- Validate and redact the received spans before forwarding them to a remote backend.
- Keep all remote endpoints and credentials on the host.

<Warning title="Treat the receiver as a trust boundary">
...
Sandbox Python can forge trace attributes and service identity, so the collector must not treat `service.name` or other span fields as authenticated tenant identity.
The metadata-only exclusions describe the managed `dcode` instrumentation, not arbitrary or compromised Python in the sandbox.
Such a process can encode data in span names, status descriptions, events, links, or allowed attribute values even when the sample attribute allowlist is present.
Restrict the receiver to the sandbox bridge and trusted host clients, enforce an attribute allowlist or equivalent redaction before remote export, and do not use this path as a multi-tenant identity boundary.
Treat the sample allowlist as protection against accidental leakage, not as a data-loss-prevention or confidentiality boundary.
</Warning>

### Route Traces to LangSmith

LangSmith is one possible host-side exporter for the backend-neutral receiver.
The following OpenTelemetry Collector fragment keeps backend credentials on the host and applies a defensive span-attribute allowlist before forwarding traces.
Use a collector distribution that includes the transform processor.
...
receivers:
  otlp:
    protocols:
      http:
        endpoint: "${env:NEMOCLAW_OTLP_LISTEN_ADDRESS}:4318"

processors:
  transform/metadata_allowlist:
    error_mode: propagate
    trace_statements:
      - context: resource
        statements:
          - keep_keys(attributes, ["service.name"])
          - set(attributes["service.name"], "nemoclaw-langchain-deepagents-code")
      - context: span
        statements:
          - keep_keys(attributes, ["openinference.span.kind", "llm.model_name", "tool.name", "otel.status_code", "nemo_relay.uuid", "nemo_relay.parent_uuid", "nemo_relay.mark.uuid", "nemo_relay.mark.parent_uuid"])
  batch: {}
...
service:
  pipelines:
    traces:
      receivers: [otlp]
      processors: [transform/metadata_allowlist, batch]
      exporters: [otlphttp/langsmith]
```

Set `NEMOCLAW_OTLP_LISTEN_ADDRESS` to a host address that is reachable through `host.openshell.internal` but not exposed on untrusted host interfaces, and restrict inbound TCP port `4318` with the host firewall or an equivalent ACL.
Set `LANGSMITH_API_KEY` and `LANGSMITH_PROJECT` only in the host environment that runs the collector.
Review the collector's effective configuration and exported spans before relying on it for sensitive workloads.
For regional, self-hosted, and additional redaction settings, refer to the [LangSmith OpenTelemetry gateway documentation](https://docs.langchain.com/langsmith/otel-gateway-trace-redaction).
