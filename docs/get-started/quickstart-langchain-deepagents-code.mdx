...
Before every execution, the managed runtime recomputes `sourceIdentity` from the working directory's Git `HEAD`, rejects tracked changes and untracked files that Git does not ignore, and records the verified identity separately in the receipt.
