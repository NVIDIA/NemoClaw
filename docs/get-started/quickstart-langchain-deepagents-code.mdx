...
During managed re-onboarding, NemoClaw restores only explicitly allowlisted, validated `config.toml` preference keys while keeping the freshly generated model routing, update settings, and provider metadata authoritative.
Tables and keys that are unknown or security-sensitive are not restored.
