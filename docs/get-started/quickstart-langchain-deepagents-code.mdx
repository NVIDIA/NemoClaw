...
Deep Agents Code state lives in its native home under `/sandbox`. OpenShell keeps that storage across stop, start, and reconnect without NemoClaw copying selected files. When a rebuild must create a replacement sandbox, NemoClaw transfers the complete native home and workspace as an opaque unit, including unknown files, configuration, skills, hooks, histories, packages, and plugins. NemoClaw does not merge `config.toml` keys or discard paths that are absent from an agent manifest. OpenShell-owned credentials remain outside the native home and are not included in the transfer. Finish active `dcode` tasks before rebuilding, and make an independent host copy when you need one.
