...
For this model, the managed image applies the Nemotron 3 Ultra harness profile to the OpenAI-compatible route, including model-specific tool-calling, filesystem, retry, context, and final-answer safeguards.
Rebuild existing Deep Agents Code sandboxes after upgrading so their image includes this profile.
