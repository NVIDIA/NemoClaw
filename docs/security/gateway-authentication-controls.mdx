...
Direct managed containers keep authoritative gateway authentication state in a gateway-owned, sandbox-readable database. NemoClaw does not import the former sandbox-writable database into that trust boundary; the recognized local client re-pairs through the bounded auto-pair flow on the first protected start.
