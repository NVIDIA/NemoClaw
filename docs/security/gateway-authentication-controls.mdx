...
Direct managed containers keep authoritative gateway authentication state in a gateway-owned, owner-only database. A separate gateway-written observer exposes only bounded, credential-free fields for an initial pending CLI pairing request; sandbox processes cannot read the database or its SQLite sidecars. NemoClaw does not import the former sandbox-writable database into that trust boundary, and the recognized local client re-pairs through the bounded auto-pair flow on the first protected start.
