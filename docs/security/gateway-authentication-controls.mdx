...
description: "Review runtime-specific gateway access, shared secret redaction, and persistent-memory guidance."
description-agent: "Explains runtime-specific gateway access and shared secret-handling controls. Use when reviewing dashboard exposure, device pairing, secret redaction, or persistent memory."
...
CLI secret redaction and runtime-specific persistent-memory guidance apply across guide variants.
...
NemoClaw does not scan or block OpenClaw tool writes to persistent memory and workspace files.
The former plugin hook provided only best-effort pattern matching and has been retired.

| Aspect | Detail |
|---|---|
| Default | Agent writes are not inspected by a NemoClaw memory-scanner hook. |
| What remains | Inference credentials stay behind OpenShell routing. Host CLI redaction and backup sanitization still apply, but neither control prevents an agent from writing a secret into persistent state. |
| What you can change | This is not a user-facing setting. |
| Risk | A secret included in a conversation or tool result can be written into memory or workspace files that survive across sessions. Snapshot sanitization processes copied JSON, YAML, and `.env` files, not arbitrary workspace content. |
| Recommendation | Keep provider credentials in OpenShell providers. Supply a host environment variable only during a documented credential registration or rotation flow. If a secret reaches persistent state, remove it and rotate the credential. |
...
NemoClaw does not scan or block Hermes memory writes.
Keep provider credentials in OpenShell providers, and use host environment variables only as transient input to documented credential flows.
Avoid writing raw credentials to Hermes state files or workspace content.
...
NemoClaw does not scan or block Deep Agents memory writes.
