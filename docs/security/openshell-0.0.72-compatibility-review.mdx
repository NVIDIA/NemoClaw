...
Review this fallback at every stable OpenShell bump and remove it in the same NemoClaw release that raises every supported Linux host to OpenShell's native glibc floor and passes the gateway-authentication and gateway-upgrade matrix for the release candidate commit without the flag.
...
- `removalCondition`: Remove the fallback when every supported Linux host meets OpenShell's native glibc 2.28-or-newer floor and the gateway-authentication and gateway-upgrade matrix passes for the release candidate commit without the flag.
