...
An OpenClaw custom Dockerfile must also declare `ARG NEMOCLAW_MANAGED_IMAGE_RUNTIME_USER=sandbox` and set `USER ${NEMOCLAW_MANAGED_IMAGE_RUNTIME_USER}` before `ENTRYPOINT`.
...
Automatic fallback and host-store sources are a no-op when a custom Dockerfile omits either required argument.
The explicit `NEMOCLAW_CORPORATE_CA_BUNDLE` path fails onboarding when either argument is absent.
