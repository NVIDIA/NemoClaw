...
When onboarding supplies a corporate CA, the build sets `NODE_EXTRA_CA_CERTS` before Node.js dependency verification, including `npm audit signatures` requests that cross a TLS-inspecting proxy.
