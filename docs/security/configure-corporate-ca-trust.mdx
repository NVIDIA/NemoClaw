...
In the final image stage, an OpenClaw custom Dockerfile must declare exactly one `ARG NEMOCLAW_MANAGED_IMAGE_RUNTIME_USER=sandbox`.
...
Automatic fallback and host-store sources are a no-op when a custom Dockerfile does not meet these requirements.
The explicit `NEMOCLAW_CORPORATE_CA_BUNDLE` path fails onboarding instead.
