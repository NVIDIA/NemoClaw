...
The Hermes package installer sets `SSL_CERT_FILE` and `REQUESTS_CA_BUNDLE` for its build-time `uv pip install` commands only when the decoded CA exists.
If the file does not exist, uv and Python keep their default trust configuration.
