...
When onboarding supplies a corporate CA, the build sets `NODE_EXTRA_CA_CERTS` before build-time Node.js dependency verification, including `npm audit signatures` requests that cross a TLS-inspecting proxy.
...
When the decoded CA file exists, each later Node.js registry remediation sets `NODE_EXTRA_CA_CERTS` and `CURL_CA_BUNDLE` before its npm operation.
