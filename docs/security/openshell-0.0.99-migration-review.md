...
| `OS99-21` | Sandbox SSH aliases include the workspace | NemoClaw uses the v0.0.99 default-workspace alias `openshell-<sandbox>.default` for CLI transports, session discovery, diagnostics, file transfer, and E2E access. It no longer constructs the unqualified `openshell-<sandbox>` SSH alias. |
| `OS99-22` | Sandbox activation requires an effective non-root process identity | The pinned Ubuntu image used by the rootless Podman proof has no OCI `Config.User`. The proof supplies `run_as_user: "1000"` and `run_as_group: "1000"` in its reviewed policy so v0.0.99 activation does not depend on mutable image metadata. |
...
- Default-workspace SSH access uses `openshell-<sandbox>.default`. Privileged Docker lifecycle
  routing accepts `openshell-default--<sandbox>-<id>` only when its labels and owner agree.
- The rootless Podman proof supplies an explicit non-root process identity for its pinned Ubuntu
  image and activates all registered agent identities.
