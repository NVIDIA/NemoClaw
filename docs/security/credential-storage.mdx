...
OpenClaw supports `BRAVE_API_KEY` and `TAVILY_API_KEY`, while Hermes supports `TAVILY_API_KEY` only.
...
For ordinary rebuilds that use a non-local upstream provider, the matching OpenShell provider entry must still exist.
...
The installer's validated prepared-backup recovery is the only exception after a gateway replacement.
It can recreate a missing canonical remote provider only when the provider's exact canonical credential environment variable contains a nonempty value in the current host process.
