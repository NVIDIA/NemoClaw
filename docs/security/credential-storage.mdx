...
OpenClaw supports `BRAVE_API_KEY` and `TAVILY_API_KEY`, while Hermes supports `TAVILY_API_KEY` and `FIRECRAWL_API_KEY`.
NemoClaw registers the selected key in a sandbox-scoped provider named `<sandbox>-brave-search`, `<sandbox>-tavily-search`, or `<sandbox>-firecrawl-search` and writes `openshell:resolve:env:<KEY>` into the agent configuration.
...
Hermes sends its Tavily placeholder in the JSON `api_key` field, and its Firecrawl placeholder in the `Authorization` header as a bearer token.
The `tavily` policy preset enables request-body credential rewriting so OpenShell replaces that body value at egress without exposing the raw key to Hermes; the `firecrawl` policy preset enables the equivalent `Authorization` header rewrite.

Use a dedicated low-scope search key and keep the matching `brave`, `tavily`, or `firecrawl` policy preset applied only while the sandbox needs web search.
...
If the sandbox registry points at a provider that is missing from OpenShell, `$$nemoclaw <name> rebuild` stops before backup or delete even when you export the matching credential environment variable.
Rerun `$$nemoclaw onboard` or re-register the provider first.
...
For the broader sandbox security model and operational trade-offs, refer to [Security Best Practices](best-practices), [Architecture](../reference/architecture), and [Set Up MCP Servers](../manage-sandboxes/set-up-mcp-servers).
