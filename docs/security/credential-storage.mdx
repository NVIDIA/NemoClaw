...
<AgentOnly variant="deepagents">
NemoClaw does not support LangSmith or OpenTelemetry tracing for the managed Deep Agents harness.
No supported path injects `LANGSMITH_API_KEY`, OTLP exporter headers, or tracing endpoint credentials into `dcode`; adding egress endpoints alone does not enable tracing.
</AgentOnly>
