...
After messaging choices are checkpointed and before resource selection, NemoClaw creates any missing sandbox-named OpenShell providers for validated web search and messaging credentials.
An existing provider is left unchanged at this early checkpoint; a replacement value is applied later during sandbox materialization.
If onboarding is interrupted afterward, `--resume` reuses a provider only when the same session recorded its creation and its name, type, and credential keys still match the saved choice.
