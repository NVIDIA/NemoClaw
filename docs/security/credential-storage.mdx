...
When Shields are up, NemoClaw sets every present `credentials` root to `root:sandbox` with mode `0710`, whether it is empty or non-empty.
This search-only access lets OpenClaw confirm that optional credential files are absent during start.
The sandbox group cannot list, create, or remove entries in the root.
Every descendant remains `root:root` with no group or world permission bits, so the sandbox group cannot read credential files.
</AgentOnly>
...
The onboard path also removes `~/.nemoclaw/credentials.json` when the file carries no values at all, such as an empty `{}` left behind by an older release.
A file that still holds any value is kept, including values under keys NemoClaw does not recognize, because NemoClaw never read or migrated them.
