...
- **DAC permissions (default).**
  When the existing process checks verify that the agent and gateway share the sandbox identity, `/sandbox/.openclaw` uses native mode `0700` and `openclaw.json` uses `0600`.
  Separate gateway identities and unverified layouts retain `2770` (setgid `sandbox:sandbox`) and `0660` so existing shared access continues.
  Rebuild existing sandboxes after updating NemoClaw to apply these defaults.
- **Native config ownership.**
  OpenClaw owns `openclaw.json` after first launch.
  NemoClaw does not maintain a config hash, seal, last-good copy, recovery anchor, baseline, or config guard for this file.
  Use `openclaw configure`, `openclaw config set`, or `openclaw config unset` inside the sandbox.
...
| Aspect | Detail |
|---|---|
| Default | The sandbox user owns writable config and state, with private permissions for verified same-user launches. The entrypoint places `OPENCLAW_GATEWAY_TOKEN` in `/tmp/nemoclaw-proxy-env.sh` for interactive shells. The gateway process does not receive the token through its environment or process arguments. |
| What you can change | Use native OpenClaw commands for agent configuration. Use NemoClaw and OpenShell commands for inference routes, credential bindings, network policy, and sandbox lifecycle. |
...
The native `mcp_servers` map in that snapshot is authoritative, including direct edits; there is no separate host-side MCP registry comparison.

| Aspect | Detail |
|---|---|
| Default | The Hermes config tree contains NemoClaw-generated config plus mutable runtime state. |
| What you can change | Hermes and the sandbox user can change mutable runtime config. Use host-side NemoClaw commands when you also need OpenShell policy and provider updates. |
| Risk of direct edits | Invalid config can prevent Hermes from starting. A direct MCP edit cannot by itself create the matching OpenShell policy or credential binding. |
...
Credential-bearing files such as `.deepagents/.env` remain excluded from NemoClaw snapshots. The native `.deepagents/.mcp.json` must contain only supported URLs and OpenShell resolver placeholders for managed entries.

The agent-native `.deepagents/.mcp.json` contains OpenShell placeholders. NemoClaw reads it directly and carries managed entries through bounded lifecycle handoffs rather than reconstructing it from host-side registry state.
...
| Aspect | Detail |
|---|---|
| Default | The Deep Agents config tree contains NemoClaw-generated `config.toml`, agent-native MCP configuration, and mutable Deep Agents memory and skill state. |
...
| Risk of direct edits | Direct MCP edits remain authoritative but cannot widen OpenShell policy. Storing credentials in `.deepagents/.env` or literal values in `.mcp.json` moves them outside the managed credential boundary and is rejected by managed lifecycle operations. |
