...
OpenShell-owned credential stores and host-only secrets remain outside the native home/workspace transfer. The native `.deepagents/.mcp.json` must contain only supported URLs and OpenShell resolver placeholders for managed entries.
