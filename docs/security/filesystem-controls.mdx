...
- **DAC permissions (default).**
  Sandbox-user images create `/sandbox/.openclaw` with native mode `0700` and `openclaw.json` with `0600`.
  Root-mode images use `2770` (setgid `sandbox:sandbox`) and `0660` for the separate gateway identity.
