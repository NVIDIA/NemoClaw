...
For managed `inference.local` routes, the entrypoint removes the exact legacy NemoClaw-generated profile and clears its credential environment aliases before launching sandbox processes. Direct custom-image routes and user-managed profiles remain in `auth-profiles.json`; the entrypoint and migration flows enforce `chmod 600` on those files under `~/.openclaw`.
