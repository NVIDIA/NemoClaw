...
It removes only the exact legacy NemoClaw-generated profile and preserves other profiles.
Direct inference routes outside `inference.local` retain their credentials and create or replace `auth-profiles.json` with the generated direct-route profile.
