...
- Filesystem transitions use the installed helper for recovery-capable images and historical images that include it.
  Agents without an image recovery artifact, and historical images that predate both artifacts, receive the host helper and manifest-derived plan through authenticated root execution without writing them to mutable sandbox state.
- A plan-aware image with a missing helper, or an installed plan that differs from the current agent manifest, fails closed.
