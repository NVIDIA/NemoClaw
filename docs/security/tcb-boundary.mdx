...
- Privileged Hermes config and cron-restore operations use root-owned installed helpers.
...
- Hermes config transactions use descriptor-safe path checks, bounded input, atomic replacement, and post-write verification.
...
Production Hermes Python guards bind privileged helpers and parser dependencies to fixed installed paths.
...
The direct-container root process belongs to the `sandbox` supplementary group so initial OpenClaw bootstrap writes and Hermes config transactions can access their mutable trees.
...
Reviewers must re-check this page when a lifecycle helper, installed mode, process proof, or test seam changes.
