...
It excludes an empty-command-line process only when two descriptor-pinned captures prove the same zombie state; an empty command line for any live state remains a terminal discovery refusal.
