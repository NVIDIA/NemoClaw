...
For `provider-state-mutation-v2`, the coordinator first recovers any retained target, then acquires and repeatedly asserts the durable provider fence around publication and fresh Hermes activation.
...
The Docker health fallback reads the same two-field record, verifies the process state and start identity before and after reading its command line, and accepts only the installed gateway arguments for the configured port or a known rewritten gateway process title.
...
The authorization records authenticated host intent for the exit rather than proving that the host signal was its sole cause; a mismatched or orphaned authorization counts as an unexpected exit.
...
- Re-review the qualified Docker authority, durable provider ledger, privileged execution lease, provider controller, Hermes publisher, and startup gate together when any target field, phase, mode, process proof, release condition, or retry boundary changes.
