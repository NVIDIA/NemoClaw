...
The PR is not approval-ready until the pinned final image and required live E2E gates pass for the PR SHA.
...
Progressive disclosure and the managed MCP bridge therefore require runtime proof for the PR SHA rather than inference from the image build.
...
| --- | --- | --- | --- |
| `HERMES-1` | High | Pin and test | The verified target tag, commit, source SHA-256, CalVer-to-semver mapping, registry cross-check, and producer runs are recorded, while final source-pin coherence still needs a test for the PR SHA. |
...
The remaining gates for the PR SHA are repository CI, automated review, documentation review, security review, and protected Hermes E2E.
...
Before merge, these checks must pass for the PR SHA:
...
- documentation-writer and security-review receipts tied to the PR SHA.
