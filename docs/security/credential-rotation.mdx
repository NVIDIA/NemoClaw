...
Web search provider configuration and credential attachment are sandbox-creation inputs.
Stock onboarding carries the non-secret configuration in the managed startup profile, while an explicit custom Dockerfile must apply it during its local build.
