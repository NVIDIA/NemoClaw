...
```bash
NVIDIA_INFERENCE_API_KEY=nvapi-new-value \
...
DISCORD_BOT_TOKEN=new-token \
  $$nemoclaw <sandbox> channels add discord
$$nemoclaw <sandbox> rebuild --yes
```
...
BRAVE_API_KEY=new-key \
  $$nemoclaw onboard --name <sandbox> --recreate-sandbox \
  --non-interactive --yes --yes-i-accept-third-party-software
```

Use `NEMOCLAW_WEB_SEARCH_PROVIDER=tavily` with `TAVILY_API_KEY` to rotate a Tavily key instead.
