...
Each Hermes sandbox has its own API port.
Run `openshell forward list` to read the sandbox's API port before you forward it.
The commands below use `8642`, which belongs to the first Hermes sandbox on a host.
