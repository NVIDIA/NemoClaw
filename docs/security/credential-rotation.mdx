...
For an active Telegram, Discord, Slack, WeChat, or Microsoft Teams channel, onboarding runs the configured checks for a supplied replacement credential before it transfers the complete native home/workspace, recreates the sandbox, and restores the transfer.
...
The transfer preserves everything beneath the resolved native home/workspace root; it does not copy OpenShell-owned host state outside that root.
...
It then transfers the complete native home/workspace, destroys and recreates the sandbox, and restores the transfer.
The transfer preserves everything beneath the resolved native home/workspace root; it does not copy OpenShell-owned host state outside that root.
...
Rebuild recreates the sandbox and transfers the complete native home and workspace while OpenShell keeps credentials outside that state.
Preserve required state before resetting the provider, and keep the replacement key only in the host environment.
</Warning>

1. Keep an independent, reviewed host copy for every sandbox that uses the provider when you need an additional recovery point before removing it.
