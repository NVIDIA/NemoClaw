...
Run `channels add` from a terminal and omit `NEMOCLAW_NON_INTERACTIVE=1` and the token variables if you want it to prompt for replacement values and offer the rebuild interactively.
A session without a terminal on stdin fails fast on a missing token instead of prompting for it.
When the token variables are set, that session queues the change for a manual rebuild.
