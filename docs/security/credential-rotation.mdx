...
Use this procedure when an existing Hermes sandbox already has Tavily enabled but its credential was registered with the generic `tavily` profile.
...
1. Create a snapshot and confirm that it succeeds before removing the provider:

   ```bash
   $$nemoclaw <sandbox> snapshot create
   ```

2. Read the replacement key silently in the host shell:
...
5. Rebuild the affected Hermes sandbox with the key still exported, then remove it from the host shell:
   ...
   Rebuild every other detached sandbox that should use this profile with its required credentials available.
   If rebuild fails, retain its backup and follow the reported recovery instructions.
...
The managed `.env` retains an OpenShell placeholder; that is expected and does not by itself indicate failed substitution.
</AgentOnly>
