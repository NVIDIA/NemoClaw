...
- Select the Restricted tier during onboarding. Onboarding defaults to the Balanced tier, which applies the `npm`, `pypi`, `huggingface`, and `brew` presets plus the selected `brave` or `tavily` web search preset.
- Keep the remaining defaults and do not add presets.
