...
| Aspect | Detail |
|---|---|
| Default | The stock OpenClaw image declares `USER sandbox`. The OpenShell-managed topology runs the gateway and agent under the same sandbox UID because `no-new-privileges` prevents the nonroot entrypoint from changing users. |
| What you can change | A direct container runtime can override the image user to `root` to let the entrypoint use the separate gateway UID. OpenShell requires the image default to remain nonroot. |
...
| Aspect | Detail |
|---|---|
| Default | `run_as_user: sandbox`, `run_as_group: sandbox`. OpenShell runs the entrypoint, gateway, and agent under the sandbox UID. |
| What you can change | Change the `process` section in the policy file to run as a different user. Direct container runtimes can override the image user to `root` when separate gateway and agent UIDs are required. |
...
| Recommendation | Keep agent commands under the `sandbox` user. Use `root` only as the direct-runtime entrypoint supervisor when separate gateway and agent UIDs are required; do not run agent commands as `root` or make `root` the OpenShell image default. |
