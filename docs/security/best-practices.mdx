...
Hermes rebuild restore blocks new gateway dispatch and waits for active messaging, API, and cron work to drain before replacing cron scripts or job definitions.
It keeps that drain in place until enabled script references validate or the prior state is rolled back.
