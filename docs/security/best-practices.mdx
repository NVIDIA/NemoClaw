...
The guard sets each confidentiality root to `root:sandbox 710`.
It sets every nested directory and file to `root:root` and removes all group and world permission bits.
The sandbox user and gateway cannot list a confidentiality root, traverse its nested directories, or read its protected files.
