...
An operator can select `thread-opt-in` during onboarding or a named transactional rebuild, but each thread still requires an explicit TUI choice or `dcode -y` invocation before tool calls run without further confirmation.
...
| What you can change | Use `--dcode-auto-approval thread-opt-in` during managed Deep Agents onboarding or rebuild. Use `--dcode-auto-approval disabled` in another rebuild to revoke the capability. |
