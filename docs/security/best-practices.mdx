...
- **Content seal under shields up.** When `nemoclaw <name> shields up` runs, it captures a SHA-256 seal of `openclaw.json` and any other locked files into the host-side shields state file. Every `shields status` call recomputes the hash inside the sandbox and surfaces drift on any mismatch, so a host-root tamper that flips perms back to `444 root:root` after rewriting the file is still flagged. `shields up` refuses to re-seal a tampered baseline; restore the original file or rebuild the sandbox before re-running.
