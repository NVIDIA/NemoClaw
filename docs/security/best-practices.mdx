...
Shields-up exempts runtime-mutable subtrees (`sessions/`, `memories/`, `logs/`, `cache/`, `plans/`, plus `openclaw-weixin/`, which the image build regenerates) so the agent can keep operating.
