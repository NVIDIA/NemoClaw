...
| Aspect | Detail |
|---|---|
| Default | Startup auto-pairing and general `connect`-time approval share one policy. NemoClaw approves devices with `clientId` set to `openclaw-control-ui` or `clientMode` set to `webchat` or `cli`, and only for `operator.pairing`, `operator.read`, and `operator.write` scopes. Agent message recovery uses the narrower policy described below. All other clients or scopes are rejected and logged. |
...
Connect-shell OpenClaw agent turns and direct `$$nemoclaw <sandbox> exec -- openclaw agent -m ...` commands use a narrower recovery path.
It runs only for message-bearing agent commands.
It approves only CLI operator pending requests with a safe `requestId`, non-empty scopes limited to `operator.pairing`, `operator.read`, and `operator.write`, and no paired-device identity mismatch.
It leaves webchat, Control UI, `operator.admin`, empty or missing scope, malformed request ID, and identity-mismatch requests unapproved for manual review.

Cron and exec commands can request `operator.admin`.
NemoClaw does not auto-approve `operator.admin`, so expected admin-scope requests must be approved manually from a connect shell.
Use the plural `devices` command:
