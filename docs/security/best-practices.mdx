...
The same workflow also locks the secret-bearing directories (`credentials`, `identity`, `pairing`): each directory itself becomes `root:sandbox 710`, and every nested directory and file inside it is locked to `root:root` with no group or world permission bits.
Neither the sandbox user nor the gateway can read those secrets or list the directory contents while the lock is active.
They can observe the metadata of an entry directly under a locked directory when they already know its name.
A probe for a missing name directly under a locked directory, such as the legacy `credentials/oauth.json`, resolves as missing instead of failing with a permission error.
