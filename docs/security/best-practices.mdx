...
Review runtime-specific gateway access, dashboard exposure where applicable, secret redaction, and persistent-memory handling in [Understand Gateway and Secret Controls](./security-controls/gateway-authentication-controls).
...
| Persistent files are not scanned before writes | NemoClaw does not inspect or block agent writes to memory or workspace files. Snapshot sanitization processes copied JSON, YAML, and `.env` files, not arbitrary workspace content. | Keep provider credentials in OpenShell providers. If a secret reaches persistent state, remove it and rotate the credential. |
