...
OpenClaw's security audit keeps NemoClaw-managed loopback `allowInsecureAuth` and compatibility-driven `dangerouslyDisableDeviceAuth` findings visible as accepted findings instead of counting them as unexplained active findings.
For a non-loopback HTTP dashboard, the insecure-auth finding remains active until you switch `CHAT_UI_URL` to HTTPS.
