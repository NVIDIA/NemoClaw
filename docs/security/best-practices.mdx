...
For additional protection, pass `--cap-drop=ALL` with `docker run` or Compose (see [Sandbox Hardening](../deployment/sandbox-hardening.mdx)).
...
- [Sandbox Hardening](../deployment/sandbox-hardening.mdx) for container-level security measures.
