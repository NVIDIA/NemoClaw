...
| `firecrawl` | Firecrawl Search API. | Agent can submit search and scrape targets to Firecrawl. The preset allows only `POST /v2/search` and `POST /v2/scrape` to `api.firecrawl.dev` from the maintained Hermes runtime and enables `Authorization` header credential rewriting. |
...
Hermes sends the Firecrawl placeholder in the `Authorization` header as a bearer token, so the `firecrawl` policy preset enables header credential rewriting for `api.firecrawl.dev`.
OpenShell replaces these placeholders only when the request reaches the matching egress policy path.
The raw `BRAVE_API_KEY`, `TAVILY_API_KEY`, or `FIRECRAWL_API_KEY` is not written into the sandbox configuration.
