...
| Gateway Authentication | Unauthorized devices or clients reaching the OpenShell gateway or dashboard surfaces when present. | OpenShell gateway | No. Set in managed startup configuration or during an explicit custom image build. |
...
The managed image build removes compilers and network probes from the runtime image.

| Aspect | Detail |
|---|---|
| Default | The managed image Dockerfile purges `gcc`, `gcc-12`, `g++`, `g++-12`, `cpp`, `cpp-12`, `make`, `netcat-openbsd`, `netcat-traditional`, and `ncat` from the sandbox image. |
| What you can change | Use an explicit custom `--from <Dockerfile>` workflow to keep these tools, or install them at runtime if package manager access is allowed. Stock onboarding does not rebuild the managed image. |
...
Stock onboarding resolves the selected OpenClaw, Hermes, or Deep Agents Code managed release image to an immutable digest instead of starting a mutable tag such as `:latest`.
NemoClaw accepts a release catalog only when the same publication cohort contains all three supported agent images.
The checked-in blueprint also references its default sandbox image by `@sha256:`.
A registry compromise or accidental force-push cannot silently swap the sandbox image.

| Aspect | Detail |
|---|---|
| Default | Managed-image catalog entries and `nemoclaw-blueprint/blueprint.yaml` pin images by digest. CI and release validation block mutable managed-image launch references. |
| What you can change | Contributors publishing managed images must update the complete release catalog cohort; contributors changing the blueprint default must update its digest. Release tooling performs these rewrites. |
...
The immutable managed image provides the hardened runtime defaults, and the managed startup profile applies the sandbox-specific authentication setting.
...
| What you can change | Managed onboarding records `NEMOCLAW_DISABLE_DEVICE_AUTH=1` in the secret-free startup profile when the compatibility opt-out applies. An explicit custom Dockerfile must apply it deliberately during its local build. The setting and its managed-vs-operator provenance are materialized in `openclaw.json` audit metadata, and the config is verified by hash at startup. |
...
| Aspect | Detail |
|---|---|
| Default | Derived from the `CHAT_UI_URL` scheme when NemoClaw applies the sandbox configuration. When the URL uses `http://` (local development), insecure auth is allowed. When it uses `https://` (remote or production), insecure auth is blocked. |
...
For an explicit remote bind with a loopback `CHAT_UI_URL`, NemoClaw disables device auth and enables OpenClaw's Host-header origin fallback because the browser's remote origin is not known when the sandbox configuration is applied. Both settings expand access and must remain explicit; use HTTPS or an SSH local forward when possible.
