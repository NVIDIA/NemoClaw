...
| Filesystem | System binary tampering, credential theft, config manipulation. | Landlock LSM + container mounts | Landlock layout: no. Requires sandbox re-creation. Configuration ownership depends on the agent runtime. Refer to [Understand Filesystem Controls](./security-controls/filesystem-controls). |
