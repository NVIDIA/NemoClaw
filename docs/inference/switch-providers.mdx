...
Onboarding a sandbox with a different provider or model warns immediately before it re-points the shared route when any same-name provider endpoint, API family, and credential identity match.
Onboarding refuses to replace those provider-global fields while another registered sandbox depends on them.
The warning names every affected registered sandbox and its recorded provider and model.
Those sandboxes use the new live route until a later onboarding run or, when endpoint, API-family, and credential identity already match, `$$nemoclaw <name> connect` warns and re-points its provider and model again.
`connect` refuses a different custom endpoint, API family, or credential identity because its provider/model-only route change cannot safely reconstruct that metadata.
