...
For an HTTPS IP-literal or DNS-pinned HTTP endpoint, OpenShell can report the provider as absent on the first route attempt.
NemoClaw then registers and verifies the provider and retries the switch.
If that retry fails, NemoClaw removes the new provider and leaves the inference selection unchanged.
