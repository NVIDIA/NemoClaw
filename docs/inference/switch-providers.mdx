...
To point a direct compatible provider at a different custom endpoint, re-run onboarding with the new endpoint.
`inference set` refuses to replace an existing direct binding because OpenShell does not expose the previous endpoint required for rollback.
