...
Onboarding a sandbox with a different valid route warns immediately before it re-points the shared route.
The warning names every affected registered sandbox and its recorded provider and model.
Those sandboxes use the new live route until a later onboarding run or, when custom endpoint and API-family identity already match, `$$nemoclaw <name> connect` warns and re-points its provider and model again.
`connect` refuses a different custom endpoint or API family because its provider/model-only route change cannot safely reconstruct that metadata; re-run onboarding to apply the complete custom route.
...
Incomplete custom-route metadata and invalid gateway bindings remain hard errors because NemoClaw cannot identify those routes safely.
