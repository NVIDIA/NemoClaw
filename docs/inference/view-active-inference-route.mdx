...
For `compatible-endpoint` and `compatible-anthropic-endpoint`, the text output adds `Endpoint` when every matching published registry row records the live provider and model plus the same canonical, credential-free, reusable HTTP(S) URL.
...
NemoClaw omits the endpoint when participating registry metadata is absent, conflicting, invalid, too long, or contains userinfo, a query, a fragment, control characters, or recognized credential material in any URL component.
...
To switch only the model for an existing adapter route, run `$$nemoclaw inference set` without `--endpoint-url` so NemoClaw reuses the recorded route.
