...
The existing schema-version `1` `provider` and `model` fields remain live-first for compatibility with automation written before the explicit route fields existed.
When `routeDrift.canConnect` is `false`, the recorded provider-global identity conflicts with another registered sandbox and `connect` is not offered as remediation.
