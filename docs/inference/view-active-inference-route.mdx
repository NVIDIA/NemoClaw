...
For `compatible-endpoint` and `compatible-anthropic-endpoint`, the text output adds `Endpoint` when every matching registry row for the selected gateway records the same bounded, credential-free HTTP(S) endpoint identity.
