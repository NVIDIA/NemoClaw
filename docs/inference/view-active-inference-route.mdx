...
The command exits non-zero with `OpenShell inference route is not configured for gateway '<gateway-name>'.` when the selected gateway has no registered inference route.
Run `$$nemoclaw onboard` to configure one.
For other lookup failures, the error names the selected gateway and reports a timeout, a nonzero exit status, or the absence of an exit status without rendering command output.
