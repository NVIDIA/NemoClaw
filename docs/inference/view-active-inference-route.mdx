...
For `compatible-endpoint` and `compatible-anthropic-endpoint`, the text output adds `Endpoint` when every matching published registry row records the live provider and model plus the same canonical credential-free HTTP(S) URL.
