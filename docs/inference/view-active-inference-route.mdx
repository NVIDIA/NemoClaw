...
For lookup failures, the error names the gateway and reports a timeout, a nonzero exit status, or the absence of an exit status without rendering command output.
