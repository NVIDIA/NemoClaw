...
For `compatible-endpoint` and `compatible-anthropic-endpoint`, the text output adds `Endpoint` when every matching registry row for the selected gateway records the same credential-free HTTP(S) URL.
...
Text output reports `Endpoint: unavailable` with a non-secret reason, up to five affected sandbox names, and a recovery action.
JSON output adds `endpointDiagnostic` with `reason`, `affectedSandboxNames`, `additionalAffectedSandboxCount`, and `recovery`; it never includes the rejected endpoint or gateway binding.
The reason is `missing-endpoint`, `invalid-endpoint`, `conflicting-endpoints`, or `invalid-gateway-binding`.
