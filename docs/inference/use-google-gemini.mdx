...
- `Gemini model catalog pagination exceeded <count> pages`
  ...
  Retry the request, then inspect the Google service or proxy response if the error continues.
- `Unexpected model catalog response: expected a top-level data array`
  The OpenAI-compatible endpoint returned JSON with the wrong shape.
  Retry the request, then inspect the endpoint or proxy response if the error continues.
