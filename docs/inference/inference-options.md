...
| Hermes Provider | Routes Hermes Agent through the host OpenShell provider registered by NemoClaw. Available only when onboarding Hermes Agent. | Curated Hermes Provider models such as `moonshotai/kimi-k2.6`, `openai/gpt-5.4-mini`, and `z-ai/glm-5.1`. |
