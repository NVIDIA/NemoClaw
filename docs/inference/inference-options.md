...
The **Ollama** option opens a second setup menu for existing local Ollama, installing Ollama when supported, or connecting to an existing remote Ollama URL.
...
| Ollama | Opens a setup menu for local Ollama, supported install/start actions, or a self-hosted remote Ollama daemon. Remote setup accepts an OpenAI-compatible base URL such as `http://192.168.1.50:11434/v1`, normalizes common Ollama paths, lists models from `/api/tags` when available, and forces `/v1/chat/completions`. Set `NEMOCLAW_REMOTE_OLLAMA_TOKEN` only if your reverse proxy requires a bearer token. | Selected during onboarding. |
