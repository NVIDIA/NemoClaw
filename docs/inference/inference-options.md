...
| Ollama | Opens a setup menu for local Ollama, supported install/start actions, or a self-hosted remote Ollama daemon. Remote setup accepts a root URL such as `http://192.168.1.50:11434`, normalizes it to `/v1`, lists models from `/api/tags` when available, and forces `/v1/chat/completions`. Set `NEMOCLAW_REMOTE_OLLAMA_TOKEN` only if your reverse proxy requires a bearer token. | Selected during onboarding. |
