...
The adapter receives OpenShell's normal `Authorization: Bearer <OPENROUTER_API_KEY>` header, forwards requests to `https://openrouter.ai/api/v1`, and injects the OpenRouter attribution headers `HTTP-Referer` and `X-OpenRouter-Title`.
