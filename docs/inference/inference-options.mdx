...
| Anthropic-compatible | Tries `/v1/messages` with a non-streaming request, then repeats the request with `stream: true` and validates the SSE event sequence. Set `NEMOCLAW_REASONING=true` to skip the streaming check for reasoning-only endpoints. |
...
Refer to [Onboarding fails with "Anthropic Messages streaming on this endpoint emits duplicate message_start"](../reference/troubleshooting#onboarding-fails-with-anthropic-messages-streaming-on-this-endpoint-emits-duplicate-message_start) if the streaming check fails.
