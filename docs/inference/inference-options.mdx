...
Onboarding also accepts `1`, `yes`, `y`, and `on` as true aliases, then stores the normalized `true` or `false` value.
...
| `NEMOCLAW_REASONING` | Set to `true` for a reasoning-only compatible endpoint model. Defaults to `false`. |
