...
| `NEMOCLAW_REASONING` | Set to `true` for a reasoning-only compatible endpoint model. Onboarding accepts `true`, `1`, `yes`, `y`, and `on` case-insensitively, then stores `true`; the default is `false`. |
