...
NemoClaw writes the router process output to `~/.nemoclaw/state/model-router.log` and creates that file with owner-only permissions.
A nondefault gateway port writes the same file under `~/.nemoclaw/gateways/<port>/state/model-router.log`.
If the router does not pass its health check in time, the startup error reports that log path and the first unhealthy-endpoint error that the router returned.
Review the log before you share it, because it contains unredacted router output.
...
If you configure a pool that NemoClaw cannot prove uses only NVIDIA HTTPS endpoints, set `OPENAI_API_KEY` for that pool.
NemoClaw stops before it launches Model Router when that key is absent.
