...
If onboarding owns the lock, the session identity changed, or a same-name session uses another router port, `destroy` leaves the current session unchanged.
...
When `destroy` warns about the remaining listener:
