...
NemoClaw sends one semantic health request at a time so slow upstream checks do not overlap.
Each request uses only the time that remains in the 10-minute startup limit.
If NemoClaw made a health request before startup failed, the error identifies the last request outcome, HTTP status, duration, and captured body size.
When a completed response returns an unhealthy-endpoint error, the startup error includes a redacted excerpt.
...
NemoClaw clears the matching Model Router process and credential recovery identity only after the stop succeeds or a complete process scan and liveness probe confirm that the router is absent.
...
If the process inventory is unavailable, the completed scan finds no matching process while the port remains responsive, or the stop fails, `destroy` still completes and keeps the recovery identity.
