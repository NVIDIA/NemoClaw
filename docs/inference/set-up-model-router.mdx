...
NemoClaw stores a router recovery identity that covers the routed credential and parsed pool configuration.
During reconciliation, NemoClaw restarts a healthy recorded router when either identity input changes, but only if that process owns the selected port.
If the recorded process does not own the port, NemoClaw replaces the listener only when process inspection confirms that it is Model Router.
NemoClaw reports an unknown healthy listener and leaves it unchanged.
If the pool file is unreadable, incompatible with the checkpoint, or rejected by Model Router validation, NemoClaw preserves the running router and its recovery identity, then reports the file to repair.
...
NemoClaw clears the matching Model Router process and router recovery identity only after the stop succeeds or a complete process scan and health probe confirm that the router is absent.
...
If the captured session still names the destroyed sandbox but uses another router port, `destroy` clears only the sandbox association and preserves the router process and router recovery identity.
...
Keep each model `name` aligned with a logical model represented by the configured checkpoint.
Pointing a checkpoint model name at an unrelated replacement endpoint does not retrain the router and produces invalid quality predictions.
NemoClaw refuses to start the shipped prefill checkpoint when its pool contains an unscored model.
It also rejects logical model names in its maintained retirement list, even when the checkpoint can still score them.
