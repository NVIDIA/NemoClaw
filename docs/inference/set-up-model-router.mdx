...
Routed onboarding holds a host-wide lock for the selected Model Router port through router setup and sandbox registry publication.
Model Router destruction holds the same port lock through its peer check, process stop, and session update.
