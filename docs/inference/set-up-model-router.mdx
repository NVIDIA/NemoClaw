...
When NemoClaw replaces an environment it owns, the existing environment's size counts as reclaimable capacity.
...
Set `NEMOCLAW_MODEL_ROUTER_VENV` to place the environment on another filesystem.
NemoClaw resolves a relative value from the current working directory before checking capacity.
