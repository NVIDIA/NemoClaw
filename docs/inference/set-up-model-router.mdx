...
When `destroy` removes the last registered Model Router sandbox, it also stops the Model Router process and frees its port.
If the stop fails, destroy still completes and prints a warning with the manual stop command.
While another registered Model Router sandbox remains, destroying one Model Router sandbox keeps the Model Router process running.
