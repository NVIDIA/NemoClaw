...
When a destroy removes the last registered Model Router sandbox, it also stops the Model Router process and frees its port.
A destroy of one Model Router sandbox keeps the router running while another registered Model Router sandbox remains.
