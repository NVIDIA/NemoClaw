...
| Local llama.cpp (already running) | Experimental | Local OpenAI-compatible | Attaches only to an authenticated operator-managed server on loopback port `8081` that passes cooperative native llama.cpp fingerprinting. Onboarding also probes the sandbox route at `host.openshell.internal:8081` and refuses the attachment when that TCP check fails; an unavailable probe stays non-fatal. NemoClaw does not own its server, model, or lifecycle. |
...
| Local llama.cpp already running | You explicitly select an authenticated server on loopback port `8081` that the sandbox can reach at `host.openshell.internal:8081`. | [Set Up llama.cpp](../local-inference/set-up-llama-cpp) |
