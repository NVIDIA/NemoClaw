...
| Local llama.cpp managed install | A qualified DGX Spark host, qualified Linux x86_64 host with at least one NVIDIA GPU, or Windows WSL N1x host satisfies the declarative recipe requirements. Docker uses the effective local `default` context. Export `DOCKER_CONTEXT=default`. If `DOCKER_CONTEXT` is unset, unset `DOCKER_HOST` and select the `default` context. | [Set Up llama.cpp](../local-inference/set-up-llama-cpp) |
