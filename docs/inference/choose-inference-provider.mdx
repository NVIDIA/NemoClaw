...
| Local llama.cpp (managed) | Experimental | Local OpenAI-compatible | Uses repository-owned YAML recipes and exact GGUF verification. The selected runtime-provider boundary owns lifecycle; all currently qualified profiles require the local `default` Docker context with `DOCKER_HOST` unset. Qualified DGX Spark and Linux x86_64 hosts with one NVIDIA GPU offer NVIDIA Nemotron 3 Nano 30B-A3B. Qualifying Windows WSL N1x hosts automatically use managed llama.cpp during Windows Express or non-interactive onboarding when no provider is requested or recorded and `NEMOCLAW_MODEL` is unset. An unset recipe selects Qwen 3.6 35B-A3B; a compatible explicit recipe selects that managed recipe. The N1x recipe remains pending physical agent qualification. |
...
| Local llama.cpp managed install | A qualified DGX Spark, Linux x86_64 host with one NVIDIA GPU, or Windows WSL N1x host satisfies the declarative recipe requirements. Docker uses the local `default` context and `DOCKER_HOST` is unset. | [Set Up llama.cpp](../local-inference/set-up-llama-cpp) |
