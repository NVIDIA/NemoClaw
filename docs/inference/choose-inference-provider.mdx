...
<Note>
DGX Station also has an explicitly opt-in managed-vLLM profile selected by the installer flag `--station-experimental-single-user`.
It serves `nemotron-ultra` through `vllm-local` with a `262144` context window and leaves the canonical Station recipe unchanged when the flag is absent.
This profile is experimental and currently requires the exact qualified local Docker image ID to be preloaded; a fresh pull is blocked until a repository-qualified manifest is published and proven.
Refer to [Set Up vLLM](../local-inference/set-up-vllm#opt-in-to-the-experimental-single-user-station-profile) for the runtime contract and current qualification boundary.
</Note>
