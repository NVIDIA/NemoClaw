...
NemoClaw probes the endpoint again and records the selected API path in the replacement sandbox's managed startup profile.
An explicit custom Dockerfile applies the selected path during its local build.
...
<Note>
`NEMOCLAW_INFERENCE_API_OVERRIDE` changes the current container-startup configuration but does not update the API path recorded by onboarding.
If you later recreate the sandbox without the override, the managed startup profile returns to the recorded API path.
Rerun onboarding to persist the API choice in both the session and replacement startup profile.
</Note>
