...
It reports `reachable` only when the probe completes without a transport error and the route returns HTTP `200` through `499`.
...
When the route is unreachable, the status detail distinguishes DNS, connection, timeout, and TLS verification failures without printing certificate material.
Status and doctor keep the configured sandbox CA authoritative.
A validated managed recovery can compare the root-owned OpenShell CA after a TLS verification failure, but only that recovery path can accept the result.
This comparison does not change stored trust or cause an ordinary status check to report the route as reachable.
