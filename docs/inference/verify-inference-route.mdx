...
Before dashboard-based onboarding prints its final summary, NemoClaw runs the same route-reachability probe from inside the sandbox.
An unreachable route or an HTTP 5xx response marks the sandbox not ready and makes onboarding exit non-zero.
Restore the configured endpoint or proxy, then rerun the status command.
