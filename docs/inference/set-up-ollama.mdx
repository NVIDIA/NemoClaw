...
For managed `inference.local` chat-completions routes using the exact model ID `qwen3.5:9b` or `qwen35-9b-q6:latest`, NemoClaw marks the model as reasoning-capable and disables Tool Search so OpenClaw receives direct tools.
It keeps thinking off to preserve visible replies after mutating tool calls and sends Ollama's supported OpenAI-compatible `reasoning.effort: "none"` request field.
</AgentOnly>
