...
With WSL mirrored networking, the same daemon can answer on `127.0.0.1`; NemoClaw treats it as Windows-host Ollama only when Windows installation and Docker reachability checks also match, so it does not launch the Linux installer or modify a WSL service.
