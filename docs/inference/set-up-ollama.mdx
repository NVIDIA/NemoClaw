...
Among installed models that fit, NemoClaw keeps the default starter model when it is available.
Otherwise, it ranks registry-known installed models by approximate VRAM requirement, then places unregistered tags after them.
The highest-ranked model becomes the default.
Equal requirements and an unregistered-only list retain Ollama's list order.
