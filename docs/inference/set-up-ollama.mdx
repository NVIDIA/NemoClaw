...
- Export the same `NEMOCLAW_OLLAMA_PROXY_PORT` for every gateway port on the host, because all of them manage that one proxy.
...
Onboarding another gateway port restarts that shared proxy with the token already stored on the host, so sandboxes on the gateway you onboarded first keep reaching Ollama after the restart.
If the restart fails, onboarding reports the port owner or the startup error, and no sandbox on any gateway port can reach Ollama until you resolve it and rerun onboarding.
