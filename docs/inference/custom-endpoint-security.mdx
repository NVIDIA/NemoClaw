...
If no other sandbox references it, NemoClaw attempts revocation.
NemoClaw makes this attempt after the new inference route commits or after sandbox deletion succeeds.
If the reference check or revocation fails, NemoClaw does not roll back the committed inference route or the completed sandbox deletion.
NemoClaw prints a warning because the upstream credential may remain in adapter memory.
...
If the affected sandbox was destroyed or revocation continues to fail, remove all remaining sandboxes before adapter cleanup.
A scoped uninstall that leaves sibling gateways in place preserves the shared adapter and its remaining routes.
After all sandboxes are removed, uninstall NemoClaw to stop the adapter and purge its in-memory credentials.
