...
Explicit endpoint URLs saved through Hermes Provider setup, `inference set`, Hermes host-side `config set`, or a direct blueprint run must pass host-side server-side request forgery validation.
...
This allowlist does not relax direct blueprint, Hermes host-side `config set`, or unrelated persisted-URL validation.
...
NemoClaw rejects DNS-backed HTTPS URLs for Hermes Provider setup, Hermes host-side `config set`, and a direct blueprint run because it cannot pin the downstream peer address while preserving TLS Server Name Indication and host validation across the OpenShell runtime boundary.
...
Hermes Provider setup, Hermes host-side `config set`, and a direct blueprint run still reject DNS-backed HTTPS URLs as described above.
...
This narrow exception supports NemoClaw-managed sandbox-to-host inference routes and Hermes host-side `config set` writes to supported provider `baseUrl` fields that already use the OpenShell host bridge.
Other Hermes `config set` keys continue to reject the bridge host.
