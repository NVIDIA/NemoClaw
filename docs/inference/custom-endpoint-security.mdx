...
For an HTTP endpoint on `localhost`, `127.0.0.1`, or `[::1]`, you can select no authentication on an explicit unprivileged port from `1024` through `65535` that is not reserved for the NemoClaw gateway (including its automatic allocation range, `8990` through `9005`), dashboard, per-sandbox API, Model Router, auth proxy, or credential adapters.
