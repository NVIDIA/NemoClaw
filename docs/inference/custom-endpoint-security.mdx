...
<Warning title="Download State Before Cleanup">
Sandbox deletion is an OpenShell lifecycle operation and removes the sandbox's native storage.
Before you destroy a remaining sandbox, download any state that you must keep to an independently managed location.
Refer to [Transfer State Manually](../../manage-sandboxes/state-and-backups/transfer-state-manually) for instructions.
</Warning>
