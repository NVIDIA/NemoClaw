...
NemoClaw configures OpenClaw's native Tool Search in structured mode, exposing `tool_search`, `tool_describe`, and `tool_call` instead of the JavaScript-based `tool_search_code` helper.
For the `nvidia/nemotron-3-super-120b-a12b` managed inference route, this avoids asking the model to generate the invalid JavaScript that produced `[tools] tool_search_code failed` noise.
Progressive-discovery trajectory evidence is still required before marking that model capability verified; final calls continue through OpenClaw's normal policy and approval path.
