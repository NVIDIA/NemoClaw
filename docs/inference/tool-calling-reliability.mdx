...
| Deep Agents Code | NemoClaw `ProgressiveToolDisclosureMiddleware` and the `search_tools` model tool | All matches | All matches |
...
It initially exposes `search_tools` and the core filesystem, shell, user-input, and todo tools; a case-insensitive name or description match makes discovered tools visible for the rest of that graph thread.
