...
Prepare and qualify both hosts before you start managed vLLM setup.

1. Follow [Prepare DGX Station to Install NemoClaw](../../get-started/additional-setup/dgx-station-preparation) on the local host and peer.
2. Complete NVIDIA's [two-Station CX8 fabric playbook](https://build.nvidia.com/station/connect-two-stations/instructions).
3. Configure SSH host-key trust and non-interactive authentication for the selected peer account.
4. Confirm that the selected peer account has passwordless `sudo` for remote preparation.

NemoClaw does not configure physical rails, enroll SSH trust, or reboot either host.
Refer to [Platform Support](../../reference/platform-support) for the current qualification and direct GPU policy boundaries.
...
- [Prepare DGX Station to Install NemoClaw](../../get-started/additional-setup/dgx-station-preparation) for host and pair qualification.
