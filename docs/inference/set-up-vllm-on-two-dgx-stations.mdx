...
- Allow the configured `NEMOCLAW_VLLM_PORT` (`8000` by default) from the OpenShell Docker subnet only to the selected rank-0 rail address.
- Deny that port on management and LAN interfaces.
...
The single-Station fallback uses the bridge-networked managed-inference topology and publishes the configured host port through Docker to container port `8000`.
