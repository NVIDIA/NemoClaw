...
- Use a qualified DGX Spark Arm64 host, a Linux x86_64 host with at least one NVIDIA GPU, or a qualifying Windows WSL N1x host.
- Use NVIDIA driver version `580.65.06` or later on each managed host profile.
- Confirm that Docker is operational on the host.
- Use Docker's effective local `default` context. Set `DOCKER_CONTEXT=default`. If `DOCKER_CONTEXT` is unset, unset `DOCKER_HOST` and select the `default` context.
