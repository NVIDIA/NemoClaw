...
- Use a qualified DGX Spark Arm64 host, a qualified Linux x86_64 host with at least one NVIDIA GPU, or a qualifying Windows WSL N1x host.
- Use NVIDIA driver version `580.65.06` or later on each managed host profile.
- Confirm that Docker is operational on the host.
- Use Docker's effective local `default` context. Export `DOCKER_CONTEXT=default`. If `DOCKER_CONTEXT` is unset, unset `DOCKER_HOST` and select the `default` context.
...
- On Linux x86_64, provide at least 48,000 MiB per NVIDIA GPU.
...
If an existing managed runtime used a non-default Docker context or `DOCKER_HOST`, NemoClaw refuses to resume or rebuild it.
Restore the Docker selector used during onboarding, then run `$$nemoclaw <sandbox-name> destroy`.
Cleanup proceeds only when the selected Docker authority matches the recorded receipt.
After cleanup succeeds, select the local `default` context before you run onboarding again.
