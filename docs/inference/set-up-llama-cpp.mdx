...
- Use Arm64 WSL with one GPU that reports the bounded `NVIDIA RTX Spark N1X` identity.
...
During Windows Express or non-interactive onboarding with no requested or recorded provider, a qualifying Windows WSL N1x host selects **Managed llama.cpp: Qwen 3.6 35B-A3B on N1x WSL (recommended)** without a provider or recipe prompt.
...
For Windows Express or non-interactive onboarding on a qualifying Windows WSL N1x host, leave `NEMOCLAW_PROVIDER` and `NEMOCLAW_LLAMACPP_RECIPE` unset for automatic selection.
Set `NEMOCLAW_LLAMACPP_RECIPE=llama-cpp.qwen3-6-35b-a3b.n1x-wsl.v1` only when automation must make the selected recipe explicit.
