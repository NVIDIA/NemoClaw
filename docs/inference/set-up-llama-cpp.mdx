...
- Remain reachable from the sandbox at `host.openshell.internal:8081`.
  The host and sandbox routes must remain reachable at the same time.
  Onboarding probes that hop after the host fingerprint.
  A failed TCP reachability check blocks attachment and requires a sandbox-reachable listener.
  An unavailable probe is non-fatal and leaves sandbox reachability unverified.
  On a native Docker-bridge route, a loopback-only publish (`-p 127.0.0.1:8081:8081`) or a `127.0.0.1`-only host bind can fail the TCP check.
  In that case, if the server runs in Docker, publish the port on the Docker gateway IP as well (`-p <gateway-ip>:8081:8081`).
  If you run `llama-server` on the host, keep `127.0.0.1:8081` reachable.
  Before you add a non-loopback listener or forwarder, restrict TCP port `8081` ingress to the sandbox subnet.
  On a native Docker-bridge route, add the listener or forwarder on the Docker gateway IP.
  On native Podman, add the listener or forwarder on the sandbox host address instead of a Docker gateway mapping.
  A firewall rule alone cannot make a loopback-only listener reachable from the sandbox.
...
After validation, NemoClaw uses the selected model's `meta.n_ctx` value from the authenticated `/v1/models` response as the served context window.
It does not use `meta.n_ctx_train`, and it does not adopt missing, malformed, or out-of-range context metadata.
A valid `NEMOCLAW_CONTEXT_WINDOW` value takes precedence over the detected value.
Refer to [Configure Model Limits](../manage-inference/configure-model-limits) for context-window behavior and overrides.
...
NemoClaw lists compatible managed llama.cpp profiles with automatic selection in descending YAML `priority` order.
During interactive onboarding without an explicit provider request, the menu ignores `NEMOCLAW_LLAMACPP_RECIPE` and `NEMOCLAW_SERVING_PRESET` and marks the unique highest-priority compatible profile as `(recommended)`.
...
The explicit-only Meta Muse Glimmer profile does not appear in the menu.
...
Use `--profile` to select a profile by its stable ID, including the explicit-only Meta Muse Glimmer profile.
Confirm that `$$nemoclaw profiles list` reports the profile as compatible first:
...
The profile selects the managed llama.cpp provider and installs the recipe that the profile declares.
Do not combine `--profile` with `NEMOCLAW_PROVIDER`, `NEMOCLAW_MODEL`, or `NEMOCLAW_LLAMACPP_RECIPE`.

For non-interactive onboarding without `--profile`, select the repository-owned recipe explicitly:
...
On Docker Desktop in WSL, NemoClaw runs the loopback health probe from the WSL CLI process because a host-network container cannot reach the WSL loopback listener.
...
If resumed onboarding reports that it cannot reserve host-local inference, run `$$nemoclaw <sandbox-name> doctor`.
Correct the reported runtime or gateway-authority condition before you rerun the same provider and recipe selection.
