...
- Remain reachable from the OpenShell Docker bridge at `host.openshell.internal:8081`.
  A loopback-only Docker publish (`-p 127.0.0.1:8081:8081`) or a `127.0.0.1`-only host bind is not enough.
  Publish the port on the Docker gateway IP as well (`-p <gateway-ip>:8081:8081`).
  Onboarding probes the sandbox hop after the host fingerprint and stops when the TCP reachability check fails.
