...
When `NEMOCLAW_LLAMACPP_RECIPE` is unset and the highest-priority compatible profile is unique, the menu marks it as `(recommended)`.
The shipped DGX Spark profile appears as **Managed llama.cpp: NVIDIA Nemotron 3 Nano 30B-A3B on one DGX Spark (recommended)**.
...
When `NEMOCLAW_LLAMACPP_RECIPE` is unset, NemoClaw selects the unique highest-priority compatible automatic profile.
...
Set `NEMOCLAW_LLAMACPP_RECIPE` to an exact recipe ID to select a compatible lower-priority profile, or to make the selection visible in automation.
...
The embedded-template contract rejects both `chatTemplateFile` and `reasoning`; recipes cannot supply arbitrary template or reasoning flags.
