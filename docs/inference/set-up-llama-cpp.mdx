...
During Windows Express or non-interactive onboarding with no requested or recorded provider, `NEMOCLAW_MODEL`, or managed-recipe override, a qualifying Windows WSL N1x host selects **Managed llama.cpp: Qwen 3.6 35B-A3B on N1x WSL (recommended)** without a provider or recipe prompt.
...
For Windows Express or non-interactive onboarding on a qualifying Windows WSL N1x host, leave `NEMOCLAW_PROVIDER`, `NEMOCLAW_MODEL`, and `NEMOCLAW_LLAMACPP_RECIPE` unset for automatic selection.
