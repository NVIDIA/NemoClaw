...
- Remain reachable from the sandbox at `host.openshell.internal:8081`.
  ...
  If you run `llama-server` on the host, bind it on the Docker gateway IP or allow the sandbox subnet; a `127.0.0.1`-only bind is not enough.
  On native Podman, bind or publish the listener on the sandbox host address instead of a Docker gateway mapping.
