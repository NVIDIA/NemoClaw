...
Onboarding automatically selects the Qwen 3.6 35B-A3B recipe after the N1x readiness requirements pass.
...
On a qualifying Windows WSL N1x host, onboarding selects **Managed llama.cpp: Qwen 3.6 35B-A3B on N1x WSL (recommended)** without a provider or recipe prompt.
...
On a qualifying Windows WSL N1x host, leave `NEMOCLAW_PROVIDER` and `NEMOCLAW_LLAMACPP_RECIPE` unset for automatic selection.
