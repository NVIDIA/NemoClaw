...
Set `NEMOCLAW_CONTEXT_WINDOW` to override the probed value.
The resolved window is written as `model.context_length` in the generated `config.yaml`; when it remains unset, Hermes auto-detects from the endpoint.
Invalid values are ignored and leave `model.context_length` unset.
