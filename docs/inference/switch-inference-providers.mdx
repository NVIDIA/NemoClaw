...
To change only the model on a sandbox that is already on this provider, omit `--endpoint-url`. `inference set` reuses the endpoint that onboarding established and does not repoint the gateway route, so re-supplying the original URL is unnecessary — and if that URL resolves to an internal address, the host-side SSRF guard blocks it. When that happens the command explains that dropping `--endpoint-url` performs the model-only switch. To point the sandbox at a genuinely different endpoint, re-run `$$nemoclaw onboard` with the new endpoint (rebuild reuses the recorded endpoint and cannot change it).
