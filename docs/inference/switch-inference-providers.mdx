...
For Deep Agents, prefer `nemo-deepagents onboard --resume --recreate-sandbox` when you need to switch providers or models.
...
Pass `--name <sandbox>` when you do not want to recreate the selected default sandbox.
...
Use the recreate path for Deep Agents provider or model changes:

```bash
nemo-deepagents onboard --resume --recreate-sandbox
```

The wizard reuses the recorded sandbox context where possible, prompts for the provider and model choices that need confirmation, and rewrites the managed `dcode` config for the new route.
If you need a fully fresh set of choices, use `nemo-deepagents onboard --fresh` instead.
...
`NEMOCLAW_AGENT_TIMEOUT` controls the request timeout baked into the Deep Agents sandbox image.
Increase it for slow local inference, such as CPU-only Ollama or vLLM on modest hardware.
Direct edits to `/sandbox/.deepagents/config.toml` are not the supported or durable way to change NemoClaw-managed defaults.
Recreate the sandbox with `nemo-deepagents onboard --resume --recreate-sandbox` to apply a new value.
...
- The host keeps provider credentials.
- The sandbox continues to use `inference.local`.
- Use `nemo-deepagents onboard --resume --recreate-sandbox` for provider, model, context window, max tokens, reasoning mode, timeout, or image-content changes.
