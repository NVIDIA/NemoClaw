...
description: "Apply secondary OpenClaw agents to a NemoClaw sandbox from a checked-in agents.yaml manifest, including per-agent models and OpenClaw-native sub-agent delegation."
...
NemoClaw can apply a multi-agent OpenClaw layout from a single checked-in manifest.
Supply the manifest at onboard time with `--agents <path>`.
Stock onboarding records the resulting `agents.list` entries, per-agent overrides, and `agents.defaults.subagents` block in the managed startup profile and materializes them in `openclaw.json` when the sandbox starts.
An explicit custom Dockerfile applies the same manifest during its local build.
...
For a single primary agent on the configured inference route, no manifest is required because NemoClaw always materializes the canonical `main` agent as the default.
...
NemoClaw reads the manifest on the host and transports its validated, secret-free representation in the managed startup profile.
The trusted config generator in `scripts/generate-openclaw-config.mts` remains the single source of truth for structured schema errors.
A malformed manifest fails configuration application before the agent process starts.
For an explicit custom Dockerfile, the same generator runs during the local image build.
...
| `description` | no | Human-readable. Applied verbatim. |
...
When a secondary agent declares its own `model` or `subagents.model`, NemoClaw adds each unique `provider/model` reference to the generated `models.providers[<onboard-provider>].models[]` array.
...
This manifest produces the following `openclaw.json` configuration:
...
The runtime startup script provisions them on first boot rather than including their contents in the managed profile or image.
...
Re-run `nemoclaw onboard --agents <file> --recreate-sandbox` to apply them through the replacement startup configuration.
