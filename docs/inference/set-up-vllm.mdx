...
Before model downloads and immediately before vLLM launch, the current N1x recipe requires Linux `MemAvailable` of at least `64,000,000,000` bytes (about `59.6 GiB`).
Missing or insufficient host-memory telemetry stops onboarding; a host that cannot expose that amount cannot safely start the current N1x recipe.
...
For a qualified N1x profile, if the selected GPU has a valid index and UUID but both memory values are exactly `[N/A]`, NemoClaw compares Linux `MemAvailable` against the recipe minimum before downloads and again before launch.
Missing or insufficient host-memory telemetry stops onboarding before the vLLM container starts.
For a qualified DGX Spark profile, the same exact `[N/A]` result remains a warning: NemoClaw cannot pre-validate utilization and continues without inferring available memory.
