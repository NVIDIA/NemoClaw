...
A quantized checkpoint refuses to load on a GPU older than the compute capability it was built for.
Before the image pull and the model download, NemoClaw reads the compute capability of each GPU that `nvidia-smi` reports and compares the lowest one against the selected model's minimum.

On a mismatch, onboarding stops within seconds and names both values:
...
Models that serve in the image's default precision have no minimum and skip this check.
When `nvidia-smi` reports no compute capability, NemoClaw continues and leaves the decision to vLLM.

The managed container uses Docker's `--restart unless-stopped` policy, so a container that cannot serve its model restarts instead of exiting.
While NemoClaw waits for the API, it also watches the container's restart count and stops the install once the container has restarted repeatedly, rather than waiting out the full load timeout.
The failure prints the container log tail.
