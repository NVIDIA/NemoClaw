...
Known container markers and mount namespaces distinguishable from visible PID 1 are rejected immediately.
For namespace-local procfs and other ambiguous cases, NemoClaw asks the pinned cached vLLM image to read a random mode-`0600` sentinel through a bounded, read-only Docker bind mount before it trusts client-side capacity.
