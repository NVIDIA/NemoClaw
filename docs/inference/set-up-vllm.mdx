...
After onboarding, `status` and `doctor` probe the host port recorded in the sandbox registry, and `connect` reads that recorded port only when it repairs the route.
None of these commands need `NEMOCLAW_VLLM_PORT` in their environment.
...
`$$nemoclaw <name> destroy` keeps the container while another registered sandbox in any gateway state root uses Local vLLM.
When the destroyed sandbox was the last one, `destroy` removes the NemoClaw-managed container and frees its GPU memory.
Pass `--keep-vllm` or set `NEMOCLAW_KEEP_VLLM=1` to keep that container running for a later sandbox.
...
Except for the interrupted managed install described below, an occupied single-host port stops installation before NemoClaw persists a bearer key or records the selected model.
For an interrupted install, it can remove and replace only a running `nemoclaw-vllm` container that has the NemoClaw ownership label and publishes container port `8000` to the requested host port on `127.0.0.1` or `0.0.0.0` through the selected local Docker connection.
It refuses an unlabeled, stopped, distributed, ambiguous, differently mapped, or differently addressed container.
For every other port holder, NemoClaw also stops before storage prompts, cache creation, image pull, or container start.
...
- The registry records `endpointSource` as `onboard`, or records `deferredN1xManagedVllmAccepted: true`. The latter accepts either a `null` endpoint or the canonical local endpoint.
- A legacy record with both `endpointUrl` and `endpointSource` set to `null` can instead use the one-time `NEMOCLAW_PROVIDER=install-vllm` recovery decision below.
