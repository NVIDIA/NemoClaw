...
`NEMOCLAW_VLLM_PORT` may select another host listener.
...
Before large downloads, NemoClaw requires valid `nvidia-smi` total/free-memory telemetry for the GPU selected by the managed Docker recipe.
It repeats the same check with fresh telemetry immediately before launch so another workload cannot consume the required reservation unnoticed during the download.
