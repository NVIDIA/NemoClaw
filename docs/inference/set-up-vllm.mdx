...
On hosts other than N1x, when vLLM is already running on `localhost:8000`, NemoClaw detects it and queries `/v1/models` for the loaded model.
...
On hosts other than N1x, if another vLLM server already occupies port `8000`, NemoClaw selects the existing-server path and applies the model-identity checks described above.
...
After successful preview onboarding, `rebuild` reuses the recorded `vllm-local` provider and managed-vLLM receipt as explicit preview intent.
You do not need to set `NEMOCLAW_PROVIDER` again for that rebuild.
If the provider or receipt is missing, malformed, or does not describe managed vLLM, readiness preflight stops before NemoClaw deletes the existing sandbox.
