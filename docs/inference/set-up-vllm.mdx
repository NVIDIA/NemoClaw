...
The managed inference catalog is also the source for `NEMOCLAW_VLLM_MODEL` slugs, model labels, immutable revisions, images, serve arguments, and platform defaults.
A host-local model that uses a registered vLLM execution contract requires recipe and preset YAML, not a model-specific CLI code path.
The catalog compiler rejects missing runtime fields, duplicate variants, and ambiguous automatic defaults.

NemoClaw evaluates matching runtime variants from general to specific.
A generic Linux recipe is the baseline for a compatible Linux appliance when no exact device recipe exists.
An exact DGX Spark, DGX Station, or N1x recipe replaces that baseline.
Architecture, GPU compute capability, readiness, and support-state checks still reject incompatible combinations.
...
### Change the Single-Host Port

Set `NEMOCLAW_VLLM_PORT` to an unused non-privileged port for either an existing local server or a single-host managed install.
For a managed container, this changes the loopback and OpenShell bridge publications; vLLM continues to listen on container port `8000`.
