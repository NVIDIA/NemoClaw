...
The Station path accepts a clean generic Ubuntu 24.04 ARM64 image or stock DGX OS `7.2.0`, `7.4.0`, or `7.5.0` when its release marker and factory GPU runtime pass validation.
For a direct GPU sandbox, NemoClaw adds `/sys` to the OpenShell filesystem policy as read-only so CUDA can inspect GPU topology and device metadata during `cuInit(0)` without granting sysfs write access.
