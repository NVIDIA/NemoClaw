...
It also rejects ambient `NEMOCLAW_VLLM_PORT` values other than `8000` and `NEMOCLAW_CONTEXT_WINDOW` values other than `262144`, so host readiness, gateway routing, and the dashboard cannot drift from the qualified contract.
