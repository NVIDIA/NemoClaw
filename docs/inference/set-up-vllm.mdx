...
- DGX Spark and DGX Station use the `linux/arm64` digest `sha256:9204569b17ee4c0eff75194b8e6e458479c8aee18953b5ab9cf359fcdac659e2` with a compressed layer size of `10.67 GB` under `nvcr.io/nvidia/vllm:26.05.post1-py3`.
...
For the DGX Station default, that is approximately `391.526 GB`: `10.670 GB` compressed image layers, `27.659 GB` unpacked image layers, `352.381 GB` model files, and `0.816 GB` writable allowance.
...
When the check is inconclusive, NemoClaw prints the reason, approximate requirement, and diagnostic commands, then continues without claiming that capacity is sufficient.
...
Pressing **Enter**, entering `n` or `no`, or entering any other response stops before the guarded download starts.

<Warning>
`--yes` and `NEMOCLAW_YES=1` do not accept an interactive verified insufficient-storage warning.
Express and other non-interactive setup paths print the warning and continue for now.
Inconclusive checks continue automatically.
</Warning>
