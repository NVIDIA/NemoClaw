...
After onboarding, `status`, `doctor`, and `connect` probe the host port recorded in the sandbox registry, so those commands do not need `NEMOCLAW_VLLM_PORT` in their environment.
