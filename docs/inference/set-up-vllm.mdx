...
- The registry records `endpointSource` as `onboard`, records both route fields as `null` with `deferredN1xManagedVllmAccepted: true`, or the current process supplies the one-time `NEMOCLAW_PROVIDER=install-vllm` recovery decision below.
