...
On DGX Station, when no model is selected explicitly, accepting the installer express prompt leaves `NEMOCLAW_VLLM_MODEL` unset while it checks for a trusted reciprocal pair.
...
After the Stations pass reciprocal GPU, rail, MAC, route, neighbor, and jumbo-frame checks across both rails, the installer prepares the peer with the same reviewed helper, exports the qualified peer, and automatically selects `nemotron-3-ultra-550b-a55b`.
...
With both model and peer unset, if no trusted pair qualifies, it retains the single-Station `deepseek-v4-flash` profile default.
...
Pass `--station-deepseek` to select the existing `deepseek-v4-flash` recipe explicitly while retaining the same one-confirmation express flow.
...
If either host requires a reboot after pinned prerequisite changes, the installer stops with status `10` and resumes only after the manual reboot with owner-only state tied to the printed exact revision and reciprocal identity.
...
Stock DGX OS `7.2.0`, `7.4.0`, or `7.5.0` requires the supported release marker before NemoClaw validates the factory GPU runtime in place; the exact April 2026 NVIDIA Colossus BaseOS and June 2026 NVIDIA AI Developer Tools profiles use their bounded factory-image paths.
For the explicit temporary metadata override, follow [Prepare DGX Station to Install NemoClaw](../../get-started/additional-setup/dgx-station-preparation).
...
Physical validation covers generic Ubuntu 24.04 ARM64, stock DGX OS `7.5.0`, the April 2026 NVIDIA Colossus BaseOS profile, and the June 2026 NVIDIA AI Developer Tools profile on one DGX Station GB300.
...
When `NEMOCLAW_PROVIDER=install-vllm` is instead supplied to the shell installer, the installer enters the same Station host-preparation and trusted-pair discovery boundary used by express setup and selects Nemotron 3 Ultra only after a reciprocal pair qualifies.
