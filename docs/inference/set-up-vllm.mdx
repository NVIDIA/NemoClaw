...
Running the Docker client inside a container or a distinct mount namespace is also unverifiable, even when the default socket is mounted at its canonical path.
