...
- The registry records `endpointSource` as `onboard`, or route normalization records both `endpointUrl` and `endpointSource` as `null`.
...
NemoClaw v0.0.119 can record both `endpointUrl` and `endpointSource` as `null` for this exact provider and model.
Matching sandboxes from either release remain eligible only when every condition above passes.
