...
The model and previous runtime image were validated on one physical DGX Spark for a direct chat request, separated reasoning, a required structured tool call, and an OpenClaw TUI chat turn without fallback.
Physical single-DGX Spark qualification of the replacement runtime image is pending.
