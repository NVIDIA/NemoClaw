...
- The DGX Spark Muse Glimmer recipe uses the candidate `linux/arm64` digest `sha256:ab0f5fc3bb81b9257a9aee801abcb0eeb94bb0523b57b2bb79349dc61e7c1e25` with a compressed layer size of `10.51 GB` under `vllm/vllm-openai`.
...
The candidate tuple was validated on one DGX Spark for text, separated reasoning, structured tool calls, and an OpenClaw chat turn without fallback.
...
<Note title="Candidate vLLM Runtime">
Muse Glimmer is not yet included in a stable upstream vLLM release.
NemoClaw pins the candidate runtime image by digest and does not select a mutable image tag.
</Note>
...
| `muse-glimmer-30b` | `Inferact/Muse-Glimmer-30B-NVFP4-W4A4` | DGX Spark | `12.1` | Candidate single-DGX Spark choice configured for text, reasoning, and structured tool calls. |
