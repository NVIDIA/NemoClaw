...
The profile retains `--gpu-memory-utilization 0.4`, a `262144`-token context window, four concurrent sequences, an `8192`-token batch limit, chunked prefill, prefix caching, and its registered parsers and acceleration backends.
...
To bound resource use while investigating long-context workflows on a single DGX Spark, select the Qwen profile and apply lower context, concurrency, and batch limits.
