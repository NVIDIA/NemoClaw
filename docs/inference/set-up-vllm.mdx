...
NemoClaw creates `~/.cache/huggingface` as the host user with owner-only access, and runs the one-shot download container with the host UID and GID, avoiding root-owned cache paths.
The explicit mode keeps the host umask from leaving the cache group-writable, which the ownership checks on that cache reject.
