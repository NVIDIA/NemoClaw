...
Existing-server and single-host managed-vLLM paths use the host port selected by `NEMOCLAW_VLLM_PORT` (`8000` when unset) on loopback for validation and on the OpenShell Docker bridge for sandbox traffic.
Use a host firewall with default-deny inbound rules.
Allow TCP on the configured port only from the OpenShell Docker subnet to its gateway address, keep loopback access, and deny the port on every other interface.
Do not expose the port to your LAN or the internet.

An authenticated fixed profile publishes container port `8000` through the configured host port on exactly two host addresses: `127.0.0.1` and the exact private IPv4 gateway of the `openshell-docker` bridge.
...
The managed Muse Glimmer and Nemotron 3.5 Lightning profiles use the same bearer-protected listener and two-address publication boundary.
...
On hosts other than N1x, when vLLM is already running on the configured local port, NemoClaw detects it and queries `/v1/models` for the loaded model.
...
Each entry includes a stable ID, display name, backend, model, topology, selection mode, support state, validation level and evidence, estimated downloads, and an incompatibility reason when applicable.
Add `--json` when a script needs the same fields as structured output.

`nemoclaw profiles list` lists hardware-bound presets, not a separate flat model registry.
The absence of an appliance-specific preset does not block a model when a compatible general Linux preset exists.
NemoClaw resolves an explicit `NEMOCLAW_VLLM_MODEL` request through those presets and recipes, then rejects missing images, incompatible CPU architecture, insufficient GPU memory, unsupported compute capability, or an ambiguous result before runtime changes.

The managed inference catalog separates reusable model identity and capabilities, runnable recipes, and hardware presets.
It is the source for `NEMOCLAW_VLLM_MODEL` slugs, model labels, immutable revisions, images, serve arguments, platform defaults, and validation evidence.
A host-local model that uses a registered vLLM execution contract requires model, recipe, and preset YAML, not a model-specific CLI code path.
The catalog compiler rejects missing references and runtime fields, duplicate aliases and variants, unverified support claims, and ambiguous automatic defaults.

NemoClaw first removes incompatible runtime variants, then ranks the remaining candidates from general to specific.
A generic Linux recipe is the baseline for a compatible Linux appliance when no exact device recipe exists.
A higher-priority DGX Spark, DGX Station, N1x, or other device-specific preset selects optimized settings when its requirements match.
Architecture, GPU compute capability, GPU memory capacity, readiness, and support-state checks still reject incompatible combinations.
An image built only for `linux/arm64` cannot run on an amd64 Brev host, so adding a platform allowlist entry is not sufficient; the catalog also needs a compatible amd64 image and recipe.
...
The single-host path pulls the vLLM image, downloads model weights into `~/.cache/huggingface`, and starts the `nemoclaw-vllm` container on `localhost:<configured-port>`.
...
On hosts other than N1x, if another vLLM server already occupies the configured port, NemoClaw selects the existing-server path and applies the model-identity checks described above.
...
### Change the vLLM Port

Set `NEMOCLAW_VLLM_PORT` to an unused non-privileged port for an existing local server or any managed vLLM topology.
For a single-host managed container, this changes the loopback and OpenShell bridge publications; vLLM continues to listen on container port `8000`.
For host-networked managed clusters, it changes the API listener on the head node while leaving the distributed rendezvous port unchanged.
NemoClaw records the actual managed-container binding, so later recovery continues to use the selected port even when the variable is no longer set.
...
NemoClaw rejects invalid values and conflicts with its gateway, dashboard, or other configured local-inference ports before starting the runtime.
...
- The Linux `amd64` Muse Glimmer recipe uses digest `sha256:7eb4028507367e69cb0abfa213042d1814c27c1b499af45fbffec8f16d9cbc6f` with a compressed layer size of approximately `8.63 GB` under `vllm/vllm-openai`.
  It pins the tokenizer to the same local Hugging Face snapshot as the model so the offline runtime never resolves an unpinned `main` revision.
- The Linux `amd64` Nemotron 3.5 Lightning recipe uses vLLM `0.27.1` digest `sha256:c2f3b1b964e47809b722b5e75b61b1e7b39a50f70388cf2bf2418f16a9f31da2` with a compressed layer size of approximately `9.11 GB` under `vllm/vllm-openai`.
  It pins the same public model revision as the DGX Spark recipe.
...
NVIDIA Nemotron 3.5 Lightning is an additional explicit-only profile for one DGX Spark or a compatible Linux `amd64` NVIDIA GPU host and does not change the platform default.
...
# Linux x86_64:
$$nemoclaw onboard --profile vllm.linux-amd64-nvidia.single.nemotron-3.5-lightning-30b-a3b-nvfp4
```

The profile serves the public [`nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4`](https://huggingface.co/nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4) checkpoint through an architecture-specific pinned vLLM image.
Both profiles use one concurrent sequence, a 65,536-token context window, and a 4,096-token batch limit.
The DGX Spark profile uses 0.65 GPU-memory utilization, FP8 KV cache, one-token MTP speculative decoding, and the `step3p5` parsers.
It uses vLLM's Python frontend because the pinned runtime's Rust frontend does not register those parsers.
The Linux `amd64` baseline uses 0.75 GPU-memory utilization, the FlashInfer Mamba backend, the `nemotron_v3` reasoning parser, and the `qwen3_coder` tool-call parser.
...
Muse Glimmer is an additional explicit-only choice for one DGX Spark or a compatible Linux `amd64` NVIDIA GPU host and does not change the platform default.
Select its stable catalog ID after confirming that `$$nemoclaw profiles list` reports it as compatible.
...
These profiles pin architecture-specific post-merge vLLM nightly images and remain Experimental.
...
- If the registry records `endpointUrl` as `null`, NemoClaw derives the local vLLM endpoint using the configured host port (`8000` when unset); otherwise, `endpointUrl` must equal that URL.
...
- `hostLocalInferenceReceipt` is absent or `null`, or contains a canonical vLLM receipt for the configured host port and the same model.
...
When you select option 2, the serving catalog supplies the pinned model, runtime image, and serve arguments.
The installer rejects provider and model overrides for that fixed profile, and the dedicated onboarder rejects vLLM model and serve-argument overrides before it starts the managed container.
`NEMOCLAW_VLLM_PORT` may override only the host listener.
...
| `muse-glimmer-30b` | `Inferact/Muse-Glimmer-30B-NVFP4-W4A4` | DGX Spark, compatible Linux `amd64` NVIDIA GPU | `12.0` on Linux, `12.1` on DGX Spark | Experimental explicit-only choice for text, reasoning, and structured tool calls. NemoClaw does not enable or support DFlash for this profile. |
| `nemotron-3.5-lightning-30b` | `nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4` | DGX Spark, compatible Linux `amd64` NVIDIA GPU | `8.0` on Linux, `12.1` on DGX Spark | Experimental explicit-only choice with architecture-specific recipes. |
