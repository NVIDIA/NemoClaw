...
- The registry records `endpointSource` as `onboard`, or records both route fields as `null` with `deferredN1xManagedVllmAccepted: true`.
...
A successful rebuild records the receipt, so later rebuilds do not require the environment variable.
