...
Its qualified single-user boundary is a 65,536-token context window, 1 concurrent sequence, a 4,096-token batch limit, 0.7 GPU-memory utilization, FP8 KV cache, and one-token MTP speculative decoding.
...
This profile is an explicit opt-in for one DGX Spark and remains Experimental while its pinned frontend and parser combination is qualified for broader support.
