...
When the selected model ID changes, NemoClaw removes `contextWindow` and `maxTokens` values inherited from the previous model entry.
OpenClaw then uses the new model's catalog values or defaults.
