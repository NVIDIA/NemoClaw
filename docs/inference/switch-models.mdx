...
Connecting to a sandbox later verifies that any recorded custom endpoint and API family match, then warns and re-points the route to that sandbox's recorded provider and model.
A different custom endpoint or API family remains fail-closed because `connect` cannot restore that provider metadata with a provider/model-only route change; re-run onboarding to apply the complete custom route.
