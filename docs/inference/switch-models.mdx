...
OpenClaw then uses the selected model's catalog values or defaults.
To keep explicit limits for the new model, recreate the sandbox after you set `NEMOCLAW_CONTEXT_WINDOW` and `NEMOCLAW_MAX_TOKENS` to values supported by that model.
