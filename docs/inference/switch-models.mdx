...
During custom-image startup, NemoClaw reconciles a stale model ID with the host-selected route only when the sandbox user can write `/sandbox/.openclaw/openclaw.json`.
That reconciliation removes both inherited limits when the model ID changes.
If the configuration is not writable, NemoClaw reports that reconciliation was skipped and preserves the existing configuration.
That preserved configuration can still contain the image's inherited model ID and limits.
Make the configuration writable by the sandbox user in the custom image, then recreate the sandbox.
