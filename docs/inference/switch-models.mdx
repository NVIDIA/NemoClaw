...
Connecting to a sandbox later verifies that any recorded provider-global endpoint, API family, and credential identity match, then warns and re-points the route to that sandbox's recorded provider and model.
A different custom endpoint, API family, or credential identity remains fail-closed because `connect` cannot restore that provider metadata with a provider/model-only route change.
