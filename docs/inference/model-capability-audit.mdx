...
- `docs/inference/fix-tool-calling-failures` explains the local inference tool-call failure mode that audit rows should classify separately from provider connectivity.
...
- [Fix Tool-Calling Failures](fix-tool-calling-failures) for separating provider connectivity from model tool-use behavior.
</AgentOnly>
