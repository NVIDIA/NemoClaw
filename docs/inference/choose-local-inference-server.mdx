...
A qualifying Windows WSL N1x host instead selects the managed Qwen llama.cpp recipe automatically during onboarding.
</Warning>
...
| Managed llama.cpp | You want NemoClaw to acquire a verified GGUF file and manage an authenticated llama.cpp container. | Experimental. Lists compatible profiles on qualified DGX Spark hosts. Qualifying Windows WSL N1x hosts automatically select the Qwen 3.6 recipe. | `/v1/chat/completions`. |
