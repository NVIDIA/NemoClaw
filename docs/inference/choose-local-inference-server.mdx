...
description-agent: "Compares NemoClaw local inference options. Use when choosing Ollama, vLLM, llama.cpp, llmman, NVIDIA NIM, or a fixed vLLM profile."
keywords: ["nemoclaw local inference", "ollama vllm llama.cpp llmman nim", "local inference server"]
content:
  type: "concept"
---
...
| Existing llmman | You already operate an authenticated `llmman serve` daemon on loopback port `17434` and want to serve models pulled from OCI registries or Hugging Face. | Experimental. Always available for explicit selection and attaches only after the daemon identifies itself as llmman and requires an API key. | `/v1/chat/completions`. |
...
Choose llmman when you already run `llmman serve` and want to serve models pulled directly from Hugging Face or any OCI registry, such as Docker Hub, GHCR, or a self-hosted mirror.
llmman speaks the Ollama, OpenAI, and Anthropic wire formats from one daemon, so NemoClaw identifies it through `/api/version` and lists its stored models through `/api/tags`, then routes sandbox traffic through Chat Completions.
The daemon must require an API key; NemoClaw owns only provider registration and routing and does not install, start, or manage llmman.

Refer to [Set Up llmman](set-up-llmman).
