...
| Fixed vLLM profile | You need the fixed catalog-selected vLLM model and serving configuration. | DGX Spark Express selects this profile. Direct installation can use the dedicated installer flag. The profile does not appear in the provider menu. | `/v1/chat/completions`. |
