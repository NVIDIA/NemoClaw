...
A qualifying Windows WSL N1x host instead selects the managed Qwen llama.cpp recipe automatically during Windows Express or non-interactive onboarding with no requested or recorded provider, `NEMOCLAW_MODEL`, or managed-recipe override.
</Warning>
...
| Managed llama.cpp | You want NemoClaw to acquire a verified GGUF file and manage an authenticated llama.cpp container. | Experimental. Lists compatible profiles on qualified DGX Spark hosts. Qualifying Windows WSL N1x hosts automatically select the Qwen 3.6 recipe during Windows Express or non-interactive onboarding with no requested or recorded provider, `NEMOCLAW_MODEL`, or managed-recipe override. | `/v1/chat/completions`. |
...
NVIDIA Nemotron is the recommended managed profile on DGX Spark and Linux x86_64. Windows WSL N1x uses the managed Qwen 3.6 35B-A3B profile during Windows Express or non-interactive onboarding with no requested or recorded provider, `NEMOCLAW_MODEL`, or managed-recipe override. Meta Muse Glimmer remains available on DGX Spark only through explicit selection.
