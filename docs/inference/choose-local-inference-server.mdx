...
Before installation, ensure no other process owns `127.0.0.1:${NEMOCLAW_VLLM_PORT:-8000}`.
...
docker container inspect \
  --format "{{json (index .NetworkSettings.Ports \"${VLLM_PORT}/tcp\")}}" \
  nemoclaw-vllm
```

The bridge result must contain exactly one private IPv4 `Gateway` value.
The container result must contain exactly two bindings with `HostPort` set to `${NEMOCLAW_VLLM_PORT:-8000}`: one `HostIp` value of `127.0.0.1` and one that exactly matches the private bridge gateway.
