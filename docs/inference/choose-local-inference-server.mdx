...
| Managed llama.cpp | You want NemoClaw to acquire a verified GGUF file and manage an authenticated llama.cpp container. | Experimental. Lists compatible profiles on qualified DGX Spark hosts and Linux x86_64 hosts with at least one NVIDIA GPU. Docker must use the effective local `default` context. Set `DOCKER_CONTEXT=default`. If `DOCKER_CONTEXT` is unset, unset `DOCKER_HOST` and select the `default` context. Qualifying Windows WSL N1x hosts automatically select managed llama.cpp during Windows Express or non-interactive onboarding when no provider is requested or recorded and `NEMOCLAW_MODEL` is unset. An unset recipe selects Qwen 3.6; a compatible explicit recipe selects that managed recipe. | `/v1/chat/completions`. |
