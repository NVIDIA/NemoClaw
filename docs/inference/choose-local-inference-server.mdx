...
| Existing llama.cpp | You already operate an authenticated llama.cpp server on loopback port `8081` that the sandbox can reach at `host.openshell.internal:8081`. | Experimental. Always available for explicit selection and attaches only after cooperative fingerprinting succeeds. A failed sandbox TCP reachability check blocks the attachment; an unavailable probe stays non-fatal. See [Set Up llama.cpp](set-up-llama-cpp). | `/v1/chat/completions`. |
...
Loopback port `8081` is not enough: the sandbox must also reach `host.openshell.internal:8081`.
