...
| Fixed local model profile | You need one catalog-selected model and serving runtime combination. | DGX Spark Express selects the vLLM profile. Direct installation can use the dedicated installer flag. The profile does not appear in the provider menu. | `/v1/chat/completions`. |
...
DGX Spark Express enables this path and selects vLLM automatically.
...
DGX Spark Express enables the same profile gate and selects the vLLM runtime.
