...
During onboarding, NemoClaw applies the variable only to the `compatible-endpoint` provider.
Onboarding with another provider clears it, even when that provider's route uses `openai-completions`.
...
Onboarding with another provider clears the variable without reading it, so an invalid value neither applies nor fails.
