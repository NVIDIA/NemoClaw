...
Set `NEMOCLAW_REASONING_EFFORT` to `low`, `medium`, `high`, or `default` for a compatible OpenAI endpoint that accepts an explicit reasoning budget.
...
NemoClaw applies the variable only to the `compatible-endpoint` provider.
Another provider clears it, even when that provider's route uses `openai-completions`.
On the `compatible-endpoint` provider, a value outside `low`, `medium`, `high`, and `default` fails before NemoClaw touches the provider, policies, the sandbox, or the registry.
Another provider clears the variable without reading it, so an invalid value neither applies nor fails.

`default` names the unset state, which is useful to clear an effort that a sandbox already records.

NemoClaw writes `params.extra_body.reasoning_effort` on the model entry in the built OpenClaw config, and OpenClaw merges it into the request body.
A model on another API family gets no such entry, and NemoClaw omits the value from the built configuration.
...
When neither is set, the sandbox keeps the effort it already records.
