...
Declare model capabilities before onboarding so NemoClaw can apply them to the OpenClaw configuration through the managed startup profile.
...
NemoClaw writes `params.extra_body.reasoning_effort` on the model entry in the generated OpenClaw config.
...
For stock onboarding, these settings are managed startup-profile inputs.
...
An ordinary sandbox restart preserves this runtime value or endpoint-default state; startup does not reapply the original onboarding value over the recorded runtime update.
