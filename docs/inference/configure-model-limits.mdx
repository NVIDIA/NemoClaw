...
description: "Set the context-window and output-token limits applied to a NemoClaw-managed sandbox."
...
content:
  type: "how_to"
---
Configure model limits before onboarding so NemoClaw can include them in the sandbox's managed startup profile.
Changing a startup model limit on an existing sandbox requires fresh recreation.
...
Onboarding stops before creating the sandbox when Ollama reports a smaller, missing, or malformed value and shows the required `OLLAMA_CONTEXT_LENGTH` value for restarting Ollama.
...
For stock onboarding, model limits are managed startup-profile settings.
An explicit custom Dockerfile applies them during its local build.
