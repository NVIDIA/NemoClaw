...
For a custom image supplied with `--from`, startup keeps valid explicit overrides.
When the selected model differs from the inherited image model, startup removes inherited `contextWindow` and `maxTokens` values if no valid explicit overrides exist.
