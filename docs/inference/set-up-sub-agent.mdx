...
OpenClaw validates and writes the whole patch atomically.
If parsing, validation, or writing fails, the native config remains unchanged.
Correct the reported error, then rerun this section before you restart the gateway.
...
The private directory and restrictive umask keep the credential readable only by your host account until
you install it in the sandbox. The installation creates the destination with mode `0600` before an atomic
rename, so there is no interval where another sandbox user can read the key.

```bash
docker exec --user root "$SANDBOX_CTR" \
  install -d -m 700 -o sandbox -g sandbox /sandbox/.openclaw/agents/vision-operator/agent
docker exec --user root -i "$SANDBOX_CTR" sh -ceu '
target=$1
temporary="${target}.nemoclaw.$$"
trap '\''rm -f -- "$temporary"'\'' 0
umask 077
cat > "$temporary"
chown sandbox:sandbox "$temporary"
chmod 600 "$temporary"
...
Removing only one copy does not retire the credential.
Rotate the provider key if either persistent file may have been exposed.
