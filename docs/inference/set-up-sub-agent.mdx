...
Patch it with your auxiliary provider and `agents.list` changes, then apply those values with native OpenClaw config commands.
...
Read each changed value from the prepared config and pass it to `openclaw config set` as the sandbox user.
OpenClaw validates each value before it writes its native configuration.
...
' /tmp/openclaw.updated.json "$@"
  ...
  read_config_value models providers "$AUX_PROVIDER_ID" |
    docker exec --user sandbox -i "$SANDBOX_CTR" sh -c '
      value=$(cat) || exit 1
      exec openclaw config set "models.providers.$1" "$value" --strict-json --merge
    ' sh "$AUX_PROVIDER_ID"

  if subagent_defaults=$(read_config_value agents defaults subagents 2>/dev/null); then
    printf '%s' "$subagent_defaults" |
      docker exec --user sandbox -i "$SANDBOX_CTR" sh -c '
      value=$(cat) || exit 1
      exec openclaw config set agents.defaults.subagents "$value" --strict-json --merge
    '
  fi

  read_config_value agents defaults timeoutSeconds |
    docker exec --user sandbox -i "$SANDBOX_CTR" sh -c '
      value=$(cat) || exit 1
      exec openclaw config set agents.defaults.timeoutSeconds "$value" --strict-json
    '

  read_config_value agents list |
    docker exec --user sandbox -i "$SANDBOX_CTR" sh -c '
      value=$(cat) || exit 1
      exec openclaw config set agents.list "$value" --strict-json --replace
    '
)
```
...
The subshell stops after the first failed write.
Values written by earlier successful commands remain in the valid OpenClaw config.
Correct the reported value or schema error, then rerun this section before you restart the gateway.

After the writes succeed, restart the managed gateway.
