...
| `/sandbox/.openclaw/.config-hash` | Hash for `openclaw.json`. Keep it in sync after manual config edits so OpenClaw can detect the updated config. If it goes missing, the next `shields up` from the mutable state on a current sandbox synthesizes it from `openclaw.json`. |
...
A stale hash never blocks shields — locking regenerates the record from the current config — and on a current sandbox even a deleted `.config-hash` is synthesized again on the next `shields up`, but keep the refresh step so OpenClaw detects the update immediately.
