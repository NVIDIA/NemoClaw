...
<Note>
Before you run the `docker exec` commands in this section, finish the [Quickstart](../get-started/quickstart) and start the target sandbox on this host.
The commands discover the sandbox container from the `openshell.ai/sandbox-name` Docker label, so the sandbox container must exist before `SANDBOX_CTR` can resolve.
</Note>
