...
Pass the patch to OpenClaw as the sandbox user.
OpenClaw validates and writes its native configuration.

```bash
docker exec --user sandbox -i "$SANDBOX_CTR" sh -c '
  tmp=$(mktemp /tmp/openclaw-config-patch.XXXXXX) || exit 1
  trap '\''rm -f "$tmp"'\'' EXIT
  cat > "$tmp" || exit 1
...
After applying the patch, restart the managed gateway.
