...
NemoClaw does not surface this degraded inspection result during onboarding, so verify that the backend listens only on loopback before continuing.
...
For an HTTP URL using the exact host `localhost`, `127.0.0.1`, or `[::1]` and port `8000` or `11434`, the API key prompt says that pressing Enter selects no authentication.
