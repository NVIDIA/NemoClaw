...
<Warning>
For the no-authentication path on an explicit unprivileged port (`1024` through `65535`), bind the backend to loopback only.
...
```bash
backend_port=12500 # Replace with the explicit port from NEMOCLAW_ENDPOINT_URL.
...
The protected no-authentication proxy is a separate route and can front an endpoint on an explicit unprivileged port (`1024` through `65535`) that is not reserved for the NemoClaw gateway (including its automatic allocation range, `8990` through `9005`), dashboard, per-sandbox API, Model Router, auth proxy, or credential adapters; keep that server bound to loopback.
...
The shared proxy stays pinned to its existing backend; reuse that endpoint. Moving or removing a sandbox does not release the host-global binding. To select a different no-authentication backend, back up the sandboxes and remove the final NemoClaw gateway with the uninstaller before reinstalling.
...
For an HTTP URL using the host `localhost`, `127.0.0.1`, or `[::1]` and an explicit port from `1024` through `65535` that is not a protected NemoClaw host port, the API key prompt says that pressing Enter selects no authentication.
...
| `NEMOCLAW_COMPATIBLE_AUTH_MODE` | Set to `none` to explicitly select no authentication for an HTTP endpoint using `localhost`, `127.0.0.1`, or `[::1]` and an explicit port from `1024` through `65535` that is not reserved for the NemoClaw gateway (including its automatic allocation range, `8990` through `9005`), dashboard, per-sandbox API, Model Router, auth proxy, or credential adapters. |
