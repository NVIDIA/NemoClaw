...
An exact trusted host may return both public and supported private addresses, and every answer remains pinned.
...
Among private answers, only RFC1918, carrier-grade network address translation (CGNAT), and IPv6 unique local address (ULA) destinations can be admitted; link-local metadata and other reserved ranges remain blocked.
