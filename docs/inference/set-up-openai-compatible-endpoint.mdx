...
For an exact loopback URL, the API key prompt says that pressing Enter selects no authentication.
...
| `NEMOCLAW_COMPATIBLE_AUTH_MODE` | Set to `none` to explicitly select no authentication for an exact loopback endpoint. |
