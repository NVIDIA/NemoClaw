...
The API key is optional when the URL uses the exact loopback host `localhost`, `127.0.0.1`, or `[::1]`.
Press Enter at the API key prompt when the loopback server does not require authentication.
Set `COMPATIBLE_API_KEY` when the loopback server requires authentication.
...
| `COMPATIBLE_API_KEY` | Endpoint API key. Optional for exact loopback URLs and required for other URLs. |
