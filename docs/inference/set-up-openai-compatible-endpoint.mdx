...
When the verified `nemoclaw-portable-registry` is the only connected container, onboarding prints the registry removal first, then the network removal. Run them in that order:

```bash
podman rm -f <registry-container-id>
podman network rm <network-id>
```

Onboarding prints no removal command in these cases, and you resolve them yourself before rerunning:

- Another container is connected to the network. Disconnect or remove that container first. Onboarding does not name it, because NemoClaw does not own it.
- A container named `nemoclaw-portable-registry` is connected but does not carry the NemoClaw ownership label. Treat it as a container NemoClaw does not own.
...
Rerun Portable onboarding after the network is gone.
</Warning>
