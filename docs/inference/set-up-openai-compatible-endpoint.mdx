...
The protected no-authentication proxy is a separate route and can front an endpoint on any explicit unprivileged port (`1024` through `65535`); keep that server bound to loopback.
...
The backend port must differ from `NEMOCLAW_OLLAMA_PROXY_PORT` (`11435` by default); configure a different free proxy port before onboarding when they conflict.
...
The endpoint also supports this mode on the configured proxy port when you set `NEMOCLAW_OLLAMA_PROXY_PORT` to a different free port before onboarding.
...
| `NEMOCLAW_COMPATIBLE_AUTH_MODE` | Set to `none` to explicitly select no authentication for an HTTP endpoint using `localhost`, `127.0.0.1`, or `[::1]` and an explicit port from `1024` through `65535`. The endpoint port must differ from `NEMOCLAW_OLLAMA_PROXY_PORT` (`11435` by default). |
