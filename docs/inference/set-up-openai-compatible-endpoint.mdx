...
NemoClaw places a token-protected proxy in front of the endpoint. At startup, NemoClaw normally inspects the backend listeners and rejects an observed non-loopback listener because it would bypass the token check.
If listener inspection is unavailable, the proxy warns and starts without verifying the backend bind.
...
For no-authentication endpoints on port `8000` or `11434`, keep the server bound to loopback.
...
For an HTTP URL using the exact host `localhost`, `127.0.0.1`, or `[::1]` and port `8000` or `11434`, the API key prompt says that pressing Enter selects no authentication. Port `11435` also supports this mode when `NEMOCLAW_OLLAMA_PROXY_PORT` is configured to use a different free port before onboarding.
...
| `NEMOCLAW_COMPATIBLE_AUTH_MODE` | Set to `none` to explicitly select no authentication for an HTTP endpoint using `localhost`, `127.0.0.1`, or `[::1]` and port `8000` or `11434`. Port `11435` requires `NEMOCLAW_OLLAMA_PROXY_PORT` to use a different free port. |
