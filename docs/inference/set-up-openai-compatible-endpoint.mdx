...
<Warning>
The no-authentication path on bundled host-gateway ports requires the backend to listen on loopback only.
NemoClaw places a token-protected proxy in front of the endpoint and rejects a backend reachable on a non-loopback interface because that would bypass the token check.
Endpoints configured with `COMPATIBLE_API_KEY` use a different authenticated path and are not subject to this loopback-only requirement.
</Warning>
