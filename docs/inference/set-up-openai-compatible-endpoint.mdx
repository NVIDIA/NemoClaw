...
| `NEMOCLAW_REASONING_EFFORT` | OpenClaw only. Writes `params.extra_body.reasoning_effort` for the model in the built OpenClaw config when the route uses `openai-completions`. Accepts `low`, `medium`, or `high`; unset leaves the endpoint's own default in place. |
