...
If listener inspection is unavailable, the proxy warns and starts without verifying the backend bind. Setting `NEMOCLAW_OLLAMA_PROXY_SKIP_BIND_PROBE=1` also disables this check and degrades the security posture.
...
For no-authentication endpoints on a bundled host-gateway port, keep the server bound to loopback.
NemoClaw's protected route makes the loopback service reachable from the sandbox without exposing the backend on other host interfaces.
