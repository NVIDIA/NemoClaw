...
<Warning>
The example above listens on every IPv4 interface.
That exposure is needed only for a direct API-key route.
For that route, use a host firewall with default-deny inbound rules, allow TCP port `8000` only from the OpenShell Docker subnet, and deny the port on every other interface.
For **No authentication**, bind the upstream to loopback, such as `127.0.0.1`, when the server supports it.
</Warning>
...
For an API-key route, automatic rewriting requires an HTTP URL with the exact loopback host `localhost`, `127.0.0.1`, or `[::1]` and a bundled host-gateway port: `8000`, `11434`, or `11435`.
NemoClaw validates the entered URL from the host and registers the direct OpenShell gateway route through `host.openshell.internal:<port>` for sandbox traffic.
Make sure the upstream is reachable from containers, such as by listening on `0.0.0.0`, and apply the firewall guidance above.
...
For **No authentication**, the upstream can remain bound to loopback.
NemoClaw exposes a protected local proxy to OpenShell, authenticates the internal route, and removes that internal `Authorization` header before forwarding to the upstream.
The protected proxy supports one loopback upstream origin at a time.
If an existing local inference route uses the proxy, NemoClaw preserves that route and rejects a different no-auth origin.
...
When the URL uses the exact loopback host `localhost`, `127.0.0.1`, or `[::1]`, the wizard offers **API key** and **No authentication**.
Choose **No authentication** only when the loopback server does not require authentication.
Choose **API key** and set `COMPATIBLE_API_KEY` when the loopback server requires authentication.
Other endpoint URLs require `COMPATIBLE_API_KEY`.
...
| `COMPATIBLE_API_KEY` | Endpoint API key. Required unless an exact loopback URL explicitly uses no-auth mode. |
| `NEMOCLAW_COMPATIBLE_AUTH_MODE` | Authentication selection for non-interactive onboarding. Use `api-key` or `none`; `none` is accepted only for exact loopback URLs. |
