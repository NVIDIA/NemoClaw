...
The host-side endpoint probe is skipped for the sandbox-internal `host.openshell.internal` alias.
Use a host-routable endpoint when you want onboarding to verify the API, tool-calling, and streaming paths before sandbox creation.
Otherwise, verify the runtime route after onboarding.
...
NemoClaw validates the endpoint with a real inference request before continuing.
...
- [Verify the Inference Route](../validate-and-troubleshoot/verify-inference-route) after setup.
