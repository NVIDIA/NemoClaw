...
Onboarding stops if that required sandbox route probe cannot reach the endpoint through the managed bridge.
The probe accepts HTTP `200` through `499`, so it proves route access but does not prove that the credentials or selected model work.
After onboarding, use `$$nemoclaw <name> status` to check ongoing route availability.
...
OpenClaw still runs the required sandbox route probe for a manually entered alias, but use a short agent request after onboarding to verify a model response.
