...
The protected no-authentication proxy is a separate route and can front an endpoint on an explicit unprivileged port (`1024` through `65535`) that is not reserved for the NemoClaw gateway (including its automatic allocation range, `8990` through `9005`), dashboard, per-sandbox API, auth proxy, or credential adapters; keep that server bound to loopback.
...
For an HTTP URL using the host `localhost`, `127.0.0.1`, or `[::1]` and an explicit port from `1024` through `65535`, the API key prompt says that pressing Enter selects no authentication.
...
| `NEMOCLAW_COMPATIBLE_AUTH_MODE` | Set to `none` to explicitly select no authentication for an HTTP endpoint using `localhost`, `127.0.0.1`, or `[::1]` and an explicit port from `1024` through `65535` that is not reserved for the NemoClaw gateway (including its automatic allocation range, `8990` through `9005`), dashboard, per-sandbox API, auth proxy, or credential adapters. |
