...
When a saved NVIDIA Endpoints route uses a retired model, interactive onboarding asks you to choose a replacement. Non-interactive onboarding uses `NEMOCLAW_MODEL` when configured, otherwise it uses the agent's default model. This retirement recovery applies only to NVIDIA Endpoints; OpenRouter routes retain models allowed by OpenRouter.
