...
OpenRouter uses the same catalog-backed picker flow and featured model list with its own provider route and credential check, but it does not apply NVIDIA Endpoints retirement filters.
...
When a saved NVIDIA Endpoints route uses a retired model, interactive onboarding asks you to choose a replacement. Non-interactive onboarding uses `NEMOCLAW_MODEL` when configured, otherwise it uses the agent's default model. This retirement recovery applies only to NVIDIA Endpoints. OpenRouter recovery does not apply NVIDIA Endpoints retirement filters.
