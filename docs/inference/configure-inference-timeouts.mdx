...
| `NEMOCLAW_SANDBOX_READY_TIMEOUT` | Image build, gateway upload, and in-sandbox boot after creation; OpenShell command re-registration after policy application; OpenShell readiness after OpenClaw or Hermes managed recovery | `180` seconds |
...
The same budget applies when `start` or `recover` launches a missing supervisor in a legacy keepalive sandbox and proves OpenShell readiness.
...
For an existing sandbox, export the variable before the `start` or `recover` command that performs the launch.
