...
| `NEMOCLAW_ONBOARD_VALIDATION_TIMEOUT_SECONDS` | Non-streaming OpenAI-compatible provider validation during onboarding | Probe-specific; WSL2 uses a `20`-second connection and `30`-second total floor |
...
`NEMOCLAW_ONBOARD_VALIDATION_TIMEOUT_SECONDS` accepts positive finite seconds and rounds fractional values up. It only raises the connection and total deadlines; an unset, invalid, or lower value preserves the probe defaults.
...
This setting only raises applicable non-streaming provider-validation connection and total deadlines. It does not extend the fixed five-second streaming-event probe.
