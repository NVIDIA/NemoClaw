...
| `NEMOCLAW_SANDBOX_READY_TIMEOUT` | OpenShell readiness checks after the sandbox creation stream ends | `180` seconds |
...
Raise `NEMOCLAW_SANDBOX_READY_TIMEOUT` when the sandbox creation stream ends but post-create readiness exceeds 180 seconds.
Image build and gateway upload finish before this budget starts, so their duration does not consume it.
Slow in-sandbox boot or OpenShell registration can consume the budget.

<AgentOnly variant="openclaw,hermes">

Post-`Ready` identity and command checks can also consume this budget for OpenClaw and Hermes sandboxes.
...
An ordinary post-create readiness failure prints a `created_but_not_ready` lifecycle receipt.
The receipt records `readiness_gate`, `readiness_reason`, `create_stream_status`, `timeout_seconds`, and `terminal_resolution`.
NemoClaw prints the retry command only after it deletes the sandbox or confirms that the sandbox is already absent.
If deletion fails, the receipt marks the sandbox as retained and the output gives a manual cleanup command without a retry command.
Docker GPU compatibility cleanup records a deferred terminal resolution and omits the generic retry command.
Exact recreated-sandbox paths preserve the sandbox for inspection when their identity contracts require it.

<AgentOnly variant="openclaw">

Portable OpenClaw onboarding also preserves the sandbox when NemoClaw cannot verify its exact runtime identity.
