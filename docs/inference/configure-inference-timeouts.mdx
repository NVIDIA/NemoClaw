...
During legacy keepalive recovery, NemoClaw restores state and verifies the gateway before the final OpenShell handoff.
The additional OpenShell readiness wait runs after that handoff, when the previous container has already been removed.
If this wait fails, automatic rollback is unavailable and the primary dashboard or API host forward remains stopped.
