...
It does not replace an explicit credential environment-variable name.
...
NemoClaw checks the shared route again immediately before deletion, so later registry drift leaves the original sandbox intact.
