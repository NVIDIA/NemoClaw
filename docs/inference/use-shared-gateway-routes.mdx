...
- For incomplete legacy custom-route metadata, back up and re-onboard the affected sandbox with an explicit endpoint and API family.
- For an invalid gateway binding, restore known-good `gatewayName` and `gatewayPort` metadata from a trusted backup, or back up and re-onboard the sandbox.
