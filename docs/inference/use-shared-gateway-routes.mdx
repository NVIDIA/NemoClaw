...
For the compatibility check, NemoClaw derives the peer's missing credential environment-variable name from the provider's canonical configuration without changing the peer's registry entry.
