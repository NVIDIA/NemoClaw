...
When non-interactive Linux onboarding finds an existing systemd Ollama service but cannot use passwordless sudo, it inspects the active listener before continuing.
If every Ollama listener is already loopback-only, onboarding continues without rewriting the systemd drop-in; otherwise it exits before configuring the proxy and asks you to rerun with `NEMOCLAW_NON_INTERACTIVE_SUDO_MODE=prompt` from a terminal or configure passwordless sudo.
