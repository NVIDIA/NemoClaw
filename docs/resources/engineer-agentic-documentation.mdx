...
description-agent: "Explains how to engineer agent-ready documentation with one canonical source, Markdown and MCP delivery, thin routing skills, workflow automation, review receipts, accuracy assurance, adoption metrics, variants, governance, and deterministic quality gates. Use when designing agentic documentation, docs engineering systems, AI documentation workflows, documentation accuracy tests, or docs-as-code automation."
keywords: ["agentic documentation", "agent-ready docs", "documentation engineering", "AI documentation workflow", "documentation accuracy tests", "documentation review receipt", "docs as code", "llms.txt", "docs MCP server"]
content:
  type: "how_to"
---
...
Agent-authored prose must match current product behavior and accepted product scope.
...
| Evidence-based review and sampling | Explanations, tradeoffs, support claims, and other prose that does not have a stable machine-readable contract. | Compare each changed claim with checked-in source and tests, verify support against an accepted product decision, run an independent documentation writer review, and sample published outcomes. |
...
It generates platform support tables from `ci/platform-matrix.json` and tests guide variant selection against shared source pages.
...
NemoClaw also performs the judgment-based layer during development, not as a separate post-publication audit.
On hosts that support subagents, the repository-wide `AGENTS.md` tells the implementation agent to start a documentation authoring subagent alongside user-visible code work.
Before final handoff, an independent documentation writer subagent reviews the completed change for documentation impact and checks changed explanatory text for accuracy, grounding, completeness, usability, and style.
The pull request then records the review against the current commit and instruction version.
...
| Repository-wide documentation contract and development-time review | `AGENTS.md`. |
...
- User-visible code changes start documentation authoring during implementation, and every completed change receives an independent documentation writer review before handoff.
