...
| Review docs | A post-merge candidate changes a public procedure or reference. | Product implementation, tests, candidate patch, and style policy. | Independent review approves the candidate. |
...
When a source appears in fewer guides than the established OpenClaw, Hermes, and Deep Agents shared set, or in some but not all configured guides that include Pi, its frontmatter declares the subset:
...
The authoring agent determines documentation impact, and a separate documentation writer reviews the candidate.
...
| Repository-wide post-merge documentation contract | `docs/AUTOMATION.md`. |
