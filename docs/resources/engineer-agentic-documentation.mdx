...
Each push to `main` starts `Docs / Post-Merge Catch-Up` for the merged implementation, tests, and documentation from the latest reachable semver tag through the pushed commit.

1. It authors documentation changes in a credential-free OpenShell sandbox.
2. A separate read-only sandbox reviews the exact documentation candidate.
3. For an approved nonempty patch, a publisher with no model credential creates a GitHub-verified commit and one SHA-specific draft PR.

An approved empty patch creates no PR; repeat after each nonempty draft merges until a later cumulative exact-SHA run approves an empty patch.
Code merges continue while their documentation changes remain pending.
...
- Post-merge publication binds the approved patch digest to the repository and exact `main` commit.
- Required checks run `npm run docs` after a maintainer starts any approval-required runs.
- Release tagging requires the exact `main` run's no-change publisher job to succeed, with no managed documentation PR open and no branch for that candidate.
