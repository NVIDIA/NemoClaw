...
- Docker-driver post-reboot recovery now waits for the labeled container to reach Docker readiness.
  ...
  When the local pairing pass cannot list or approve the clone request, restored snapshot clones retry pairing once only if the authenticated verifier reports a pending scope upgrade.
  The retry preserves the local-device and single-request bounds.
