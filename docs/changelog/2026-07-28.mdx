...
- Docker-driver post-reboot recovery now waits for the labeled container to reach Docker readiness.
  ...
  Restored snapshot clones now remove the shared gateway URL, port, and token before listing the clone request, then select pairing-scoped stored-device authentication only after an exact local repair preflight.
