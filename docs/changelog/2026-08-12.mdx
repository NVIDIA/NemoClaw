...
- On Linux and Windows Subsystem for Linux 2 (WSL2), `nemoclaw onboard --host-mount <host:/sandbox/path>` can now expose an existing host directory read-only beneath `/sandbox`.
  ...
  It persists accepted mounts for `nemoclaw <sandbox> rebuild` and interrupted `nemoclaw onboard --resume`.
  The `destroy` command removes their declaration, and sandbox status reports them.
