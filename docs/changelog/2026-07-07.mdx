...
- Installer upgrades require a fresh backup of every registered sandbox before replacing the gateway.
  Pre-fingerprint OpenClaw and Hermes sandboxes can recover only after managed-image name confirmation, recorded custom-image cases remain blocked, and successful recovery restores the recorded provider and route without starting generic onboarding.
