...
- Current NemoClaw-managed Hermes images on the Docker driver now use the durable runtime provider state mutation contract for Shields transitions.
  NemoClaw validates the request and receipt, holds Hermes gateway startup behind the current fence, and preserves rollback and recovery across controller restarts.
