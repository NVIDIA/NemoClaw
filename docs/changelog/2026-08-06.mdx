...
- DGX Spark users can now select an opt-in fixed vLLM serving profile through the hosted installer or the CLI.
  `profiles list` reports compatible catalog entries without changing resources, while `onboard --profile` records the exact catalog, recipe, and runtime provenance and rejects silent definition drift during resume.
...
- Portable experimental-profile recovery can now restart a receipt-owned user-local Ollama executable after a semantic `/api/tags` health check.
  Recovery requires explicit re-enrollment for installs that predate ownership receipts, rejects symbolic links and unrelated daemons, and verifies both the authenticated proxy and the sandbox `inference.local` route before reporting success.
