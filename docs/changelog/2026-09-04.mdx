...
- NemoClaw now uses detached `openshell forward service` processes for managed dashboard, messaging, and MCP host forwards instead of owning SSH forwarding processes and receipt state.
  These service-managed forwards follow sandbox and gateway availability, refuse an occupied host port, verify reachability for an existing forward, verify ownership and startup stability for a newly launched forward, and relaunch through normal recovery.
