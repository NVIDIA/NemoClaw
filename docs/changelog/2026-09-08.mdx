...
- Hermes rebuild now preserves supported operator-managed configuration set through `nemohermes <name> config set`.
  Rebuild excludes managed route fields and credential material, verifies the restored values, and lists restored and dropped keys.
  A capture, restore, or verification failure leaves rebuild recovery state intact and exits nonzero.
  Related change: [PR #10780](https://github.com/NVIDIA/NemoClaw/pull/10780).
