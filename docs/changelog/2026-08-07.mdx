...
- OpenClaw, Hermes, and Deep Agents Code sandboxes now allow read-only `dpkg-query` access to installed package metadata while package-management writes remain denied.
  An interactive `policy restore` refusal now reports `Cancelled.`, and a non-interactive refusal prints the complete usage line without changing the policy.
