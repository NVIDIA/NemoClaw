...
Use [shared-model.yaml](shared-model.yaml) to run separate OpenClaw and Pi sandboxes against one managed Qwen3-4B vLLM service on the same Station configuration.
These examples cover their exact combinations and do not qualify other DGX Station models, GPU counts, agents, or container engines.

| File | Agents | What it demonstrates |
|---|---|---|
| [vllm.yaml](vllm.yaml) | OpenClaw | One agent and one managed Qwen3-4B service |
| [shared-model.yaml](shared-model.yaml) | OpenClaw and Pi | Two sandboxes reuse one managed Qwen3-4B service and provider |
...
Confirm that the selected example's gateway and inference ports are unused and that its gateway CIDR does not overlap a host route or Docker network.
...
# The shared-model example also requires the Pi image.
IMAGE_PREFIX=nc-fabric AGENT_PLATFORM=linux/arm64 docker buildx bake pi --load
cargo run -p nemoclaw-build -- bundle
```

The runtime build reports its immutable image reference.
Use `docker image inspect` on `nc-fabric:openclaw` and `nc-fabric:pi` to identify the loaded image digests required by your selected example.
...
cp examples/station/shared-model.yaml .local/station-demo/deployment.yaml
```

Edit the copy before running NemoClaw:

1. Assign a fresh `metadata.uid`.
2. Replace each zero image digest with the immutable runtime and agent image references built from the same checkout.
...
Applying downloads the pinned model snapshot, starts managed vLLM and the declared agent runtimes, and may take several minutes.
Apply verifies readiness but does not request an agent response.
Use the [headless OpenClaw request](../../docs/agents.md#run-one-headless-openclaw-request) and [Pi request](../../docs/agents.md#run-one-deep-agents-or-pi-request) procedures for the agents in your selected example.
...
The retained OpenClaw run based on v1 commit `ba2e41fbb093f764d07774bb5a920e99e55daedf` exercised the single-agent configuration and full lifecycle on 2026-09-19.
The retained shared-model run based on v1 commit `524f0f260c12d494dc94c45b7f82af3b2aa2ae4c` exercised both agent requests and the full lifecycle on 2026-09-20.
