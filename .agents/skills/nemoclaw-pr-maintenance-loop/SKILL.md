...
Follow the detailed action order in [references/action-order.md](references/action-order.md): integrate base, fix CI, address human review feedback, address CodeRabbit feedback, trigger missing E2E recommendations, and report merge-ready PRs.

Apply at most one safe fix per loop pass, then update state and return to gate checks.
## Step 6: Update State
