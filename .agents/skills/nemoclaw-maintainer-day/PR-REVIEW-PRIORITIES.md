...
3. **Evening** (`/nemoclaw-maintainer-evening`) — check what shipped, record open labeled PRs as carry-forward work, generate a QA-focused summary, cut the tag, and prepare release notes for posting.
...
Version labels activate release work; they are not readiness claims. Open labeled PRs keep their existing label as carry-forward work until a maintainer explicitly removes it.
