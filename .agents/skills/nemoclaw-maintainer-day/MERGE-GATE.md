...
Use the protected-environment path when the required check reports `E2E reviewer authorization required to run fork E2E`.
...
The controller also checks the PR number, head repository, PR SHA, base SHA, plan, pending required check, compatible `main`, and PR state.
Immediately before dispatch, it confirms that the PR is open and that the PR SHA, base SHA, and required-check identity still match.
It also performs a bounded direct check read and requires the required check to remain in the exact expected phase, regardless of a matching, stale, or missing list result.
...
It also rejects a missing required check, an identity mismatch, or an incompatible controller commit.
