...
- GitHub usually associates the run with the PR.
  If that association is empty, require the controller path, branch, repository, and run time to match the required check.
...
Use the protected-environment path when the required check reports `Maintainer approval required to skip credentialed E2E`.
...
Immediately before it writes success, the controller confirms that the PR is open and that the PR SHA, base SHA, and required-check identity still match.
