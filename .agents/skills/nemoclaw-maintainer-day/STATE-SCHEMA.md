...
- `gates.autoApprove` applies only when the current actor has not contributed to that PR and every pre-approval gate passes.
- `gates.autoPushSmallFixes` allows pushing narrow fixes to contributor branches. After using it on a PR, the same actor cannot use `autoApprove` for that PR.
