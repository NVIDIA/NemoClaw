...
5. Fetch trusted `origin/main`. Read `.github/pr-limits.yaml` with `git show origin/main:.github/pr-limits.yaml`. Confirm the complete policy passes `parsePrLimits` from `.github/actions/pr-limit/index.mjs`. Then grep the validated policy with `grep -E "^(${login}|default):"` after shell-escaping the lowercase GitHub login; use the username entry when present, otherwise use `default`. Treat `unlimited` as no limit. Do not create a claim that would exceed the resolved numeric limit.
