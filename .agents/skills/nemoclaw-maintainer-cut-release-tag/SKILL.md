...
Accepted release evidence requires `allow_jetson_dispatch=false` and `allow_dgx_spark_runner_queue=false`.
...
- the trusted dispatch receipt to prove empty selectors, `include_staging_brev_launchable=true`, `allowJetsonDispatch: false`, `allowJetsonRunnerQueue: false`, and `allowDgxSparkRunnerQueue: false`;
