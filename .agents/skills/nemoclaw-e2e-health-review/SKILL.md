...
The NemoClaw repo has a rich E2E testing infrastructure, but workflow/job inventories drift. Discover the current workflow set at runtime instead of relying on embedded snapshots.

```bash
find .github/workflows -maxdepth 1 -type f \( -name '*e2e*.yaml' -o -name '*e2e*.yml' -o -name 'main.yaml' -o -name 'main.yml' \) | sort
```

### Nightly E2E Workflow (`.github/workflows/nightly-e2e.yaml`)

Read `nightly-e2e.yaml` directly for the current schedule, dispatch inputs, concurrency policy, and job matrix before interpreting results:

```bash
grep -nE '^(on:|concurrency:|jobs:|  [A-Za-z0-9_-]+:)' .github/workflows/nightly-e2e.yaml
```

Known interpretation guidance:

- `schedule` runs represent unattended nightly health.
- `workflow_dispatch` runs are useful supplemental evidence when schedule runs are missing or cancelled.
- `cancel-in-progress` on a shared concurrency group can make dispatch runs cancel scheduled runs; report this as a reliability/system issue when observed.
- `notify-on-failure`-style jobs are notification plumbing, not product E2E coverage.

### Other E2E Workflows

Discover other E2E-related workflows from `.github/workflows/*e2e*.yaml` and inspect each file's `on:` block to determine whether it runs on PRs, pushes, workflow calls, or manual dispatch.

### Main CI Gate (`.github/workflows/main.yaml`)

Read `main.yaml` directly to identify the current main-branch gate jobs. Failures in required main-branch jobs mean a merge broke main.

### E2E Test Scripts (`test/e2e/`)

Shell-based test scripts generally follow: prerequisites, pre-cleanup/install, test cases, and final cleanup. Compare scripts to workflow references at runtime:

```bash
find test/e2e -maxdepth 2 -type f \( -name '*.sh' -o -name '*.ts' \) | sort
grep -R "test/e2e" .github/workflows/*.y*ml
```
