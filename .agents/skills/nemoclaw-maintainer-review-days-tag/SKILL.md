...
description: Review every open PR carrying today's version-target label (the "day's tag"). Produces a single prioritized review queue showing CI status, CodeRabbit state, mergeability, reviewer decision, and recommended next action per PR. Use when a maintainer wants to sweep the whole day's release target in one pass. Trigger keywords - review days tag, review todays tag, review days PRs, sweep version label, review release target, days tag review, review all targeted PRs, todays release review.
user_invocable: true
...
By default, exclude PRs authored by the person running the skill. Maintainers should not review or approve their own PRs; use `--include-mine` only when the user explicitly asks for whole-tag visibility.
...
```bash
SCRIPT=".agents/skills/nemoclaw-maintainer-review-days-tag/scripts/review-days-tag.ts"
...
(Optional flags: `--repo OWNER/REPO`, `--json` for raw output, `--include-mine` to include the authenticated user's own PRs, `--viewer LOGIN` for tests/debugging.)

The script:

1. Detects the authenticated GitHub user with `gh api user --jq .login` equivalent (`gh api user` in the script)
2. Lists every open PR with the `<version>` label
3. Filters out PRs where `author.login` matches the authenticated user unless `--include-mine` was explicitly passed
4. For each remaining PR, gathers: mergeability, status checks, CodeRabbit unresolved threads, reviewer decision, draft state, author, age, size
5. Classifies each PR into a **next-action bucket**:
...
```markdown
### Day's tag review — v0.0.8 (N open PRs, excluding @current-user)
...
- **Filtered own PRs:** if any were excluded, mention the count and the authenticated GitHub login
