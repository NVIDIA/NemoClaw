 ...
 *     <version> [--repo OWNER/REPO] [--json] [--include-mine] [--viewer LOGIN]
 */
...
function renderTable(
  ...
  const lines: string[] = [];
  const exclusion = viewerLogin ? `, excluding @${viewerLogin}` : "";
  lines.push(`### Day's tag review — ${version} (${rows.length} open PR${rows.length === 1 ? "" : "s"}${exclusion})`);
  lines.push("");
  if (filteredOwnCount > 0 && viewerLogin) {
    lines.push(`_Filtered out ${filteredOwnCount} PR${filteredOwnCount === 1 ? "" : "s"} authored by @${viewerLogin}; pass \`--include-mine\` to include them._`);
    lines.push("");
  }
  if (rows.length === 0) {
...
function main(): void {
  ...
  const includeMine = args.includes("--include-mine");
  const repo = parseStringArg(args, "--repo", "NVIDIA/NemoClaw");
  const viewerOverride = parseStringArg(args, "--viewer", "");

  const positional = args.filter((a, i) =>
    !a.startsWith("--") && args[i - 1] !== "--repo" && args[i - 1] !== "--viewer"
  );
  ...
  const prs = raw ?? [];
  const viewer = viewerOverride || ((ghJson(["api", "user"]) as { login?: string } | null)?.login ?? "");
  const viewerLogin = viewer || null;
  const filteredPrs = includeMine || !viewerLogin
    ? prs
    : prs.filter((pr) => pr.author?.login !== viewerLogin);
  const filteredOwnCount = prs.length - filteredPrs.length;
  const summaries = filteredPrs.map(summarize);
  ...
  console.log(renderTable(version, summaries, includeMine ? null : viewerLogin, filteredOwnCount));
}
