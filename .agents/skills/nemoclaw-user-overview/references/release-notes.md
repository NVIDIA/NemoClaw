...
- Gateway failures now fail faster and explain more. `nemoclaw status` classifies gateway probe failures by layer, including unreachable gateways, unhealthy responses, and sandbox mismatches, and `nemoclaw <name> connect` exits early with recovery guidance when the OpenShell gateway is down.
...
- OpenClaw compatibility is more resilient across runtime changes. Kimi mixed tool calls are normalized more consistently, compatible OpenClaw JSON envelope changes are tolerated in tests, runtime dependency handling is more robust across filesystem boundaries, and OpenClaw patch drift is easier to classify during image builds.
- Messaging channel removal now keeps the sandbox registry and onboard session policy preset state in sync, so removed channel presets do not return during later `onboard --resume` or rebuild flows.
...
- Source-checkout installs bootstrap OpenShell when needed before running preflight, so `git clone` based installs can reach the same managed OpenShell setup path as packaged installs.
...
- Sandbox image builds skip broad permission repair where the image already owns the relevant paths, reducing build time while preserving writable sandbox state.
