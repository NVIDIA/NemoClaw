...
| `nemoclaw-maintainer-cut-release-tag` | Verify the scheduled 4 PM signed tag independent of E2E, use manual recovery when needed, wait for workflow-managed `latest`, carry remaining open items forward, and delete the released label; `lkg` stays manual. |
