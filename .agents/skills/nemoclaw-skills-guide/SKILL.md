...
Covers the daily maintainer cadence, trusted E2E dispatch, a continuous loop that fixes E2E failures, workflow policy, documentation refactors, releases, review selection, comparison, triage, security review, and stale bug verification.
...
| `nemoclaw-maintainer-fix-e2e-failures` | Run a persistent loop that fixes automatic `main` E2E failures by root cause, coordinates peer approvals, merges eligible PRs, and keeps monitoring. |
