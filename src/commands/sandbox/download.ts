...
export default class SandboxDownloadCommand extends NemoClawCommand {
  ...
  static description =
    "Host-side wrapper around `openshell sandbox download`. Confirms that the sandbox is live and the source is a regular file or directory; symbolic links and other special source types are refused. If the source type cannot be confirmed, the command exits without downloading. Otherwise, it downloads to a fresh private temporary directory, verifies that OpenShell wrote an artifact, publishes the artifact to the requested destination, and removes the temporary directory. Existing destination directories resolve to their canonical paths; symbolic-link file destinations and new destinations below symbolic-link parents are rejected. Relative host destinations resolve against the caller's working directory. Absolute host destinations do not use caller-working-directory resolution.";
