...
const GATEWAY_OWNER_MODES = new Set(["nemoclaw-managed", "externally-supervised"]);
const GATEWAY_OWNER_SOURCES = new Set(["declared", "packaged-service", "standalone"]);
const GATEWAY_SUPERVISOR_KINDS = new Set(["systemd-system", "systemd-user"]);
...
function parseGatewayOwner(value: unknown): CheckpointGatewayOwner | null {
  if (!isObjectRecord(value)) return null;
  ...
  if (!mode || !GATEWAY_OWNER_MODES.has(mode)) return null;
  if (!source || !GATEWAY_OWNER_SOURCES.has(source)) return null;
  if (endpoint === null && value.endpoint !== null) return null;
  if (stateDir === null && value.stateDir !== null) return null;
  if (!requiredCapabilities) return null;

  let supervisor: CheckpointGatewayOwner["supervisor"] = null;
  if (value.supervisor !== null) {
    if (!isObjectRecord(value.supervisor)) return null;
    const kind = readString(value.supervisor.kind);
    const serviceName = readString(value.supervisor.serviceName);
    const execPath = readString(value.supervisor.execPath);
    if (!kind || !GATEWAY_SUPERVISOR_KINDS.has(kind) || !serviceName || !execPath) return null;
    supervisor = {
      kind: kind as NonNullable<CheckpointGatewayOwner["supervisor"]>["kind"],
      serviceName,
      execPath,
    };
  }
  ...
  return {
    mode: mode as CheckpointGatewayOwner["mode"],
    source: source as CheckpointGatewayOwner["source"],
    endpoint,
    stateDir,
    supervisor,
...
function parseCurrentSchema(value: Record<string, unknown>): OnboardCheckpoint | null {
  ...
  // Older v1 checkpoints predate this field. Preserve their loadability while
  // retaining null as an explicit "no owner proof" value that resume rejects.
  const gatewayOwner = value.gatewayOwner == null ? null : parseGatewayOwner(value.gatewayOwner);
  ...
  if (value.gatewayOwner != null && !gatewayOwner) return null;
...
export function serializeCheckpoint(checkpoint: OnboardCheckpoint): Record<string, unknown> {
  return {
    ...
    gatewayOwner: checkpoint.gatewayOwner,
