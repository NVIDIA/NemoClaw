...
describe("checkpoint schema inspection", () => {
  ...
  it("loads the secret-free gateway owner identity", () => {
    const checkpoint = baseCheckpoint({
      gatewayOwner: {
  ...
  it("loads an older v1 checkpoint without owner proof as an explicit null binding", () => {
    const checkpoint = serializeCheckpoint(baseCheckpoint());
    delete checkpoint.gatewayOwner;

    expect(inspectCheckpoint(checkpoint)).toMatchObject({
      status: "loaded",
      checkpoint: { gatewayOwner: null },
    });
  });

  it("rejects a malformed gateway owner instead of trusting partial identity", () => {
    const checkpoint = serializeCheckpoint(baseCheckpoint());
    checkpoint.gatewayOwner = {
      mode: "externally-supervised",
      source: "declared",
      endpoint: "http://127.0.0.1:8080",
      stateDir: "/var/lib/openshell/gateway",
