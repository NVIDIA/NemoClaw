...
const PROFILE_SHA256 = createHash("sha256")
  .update(ENCODED_PROFILE, "utf8")
  .digest("hex");
...
describe("sandbox rebuild authority", () => {
  ...
  it.each([
    [
      "driver",
      (source: SandboxEntry) => ({ ...source, openshellDriver: "mxc" }),
    ],
    [
      "generation",
  ...
  it("publishes a replacement by CAS without changing unrelated registry state", () => {
    ...
    const swapped = swapSandboxRebuildAuthorityInRegistry(
      before,
      authority,
      replacement(),
    );
  ...
  it.each([
    [
      "sandbox name",
      (candidate: SandboxEntry) => ({ ...candidate, name: "other" }),
    ],
    [
      "provider",
      (candidate: SandboxEntry) => ({ ...candidate, openshellDriver: "mxc" }),
    ],
    [
