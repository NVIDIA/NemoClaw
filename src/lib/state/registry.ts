...
function assertPendingPolicyVerificationMatchesRegistration(
  ...
  const commonChecks = [
    ...
    ["reservation session", recordedEntry?.reservationSessionId === reservation.authority.sessionId],
    ...
    ["recorded lifecycle generation", recordedEntry?.lifecycleGeneration === checkpoint.lifecycleGeneration],
    ["recorded lifecycle identity", recordedEntry?.lifecycleLiveIdentityFingerprint === checkpoint.sandboxIdentityFingerprint],
    ...
    ["requested lifecycle generation", checkpoint.lifecycleGeneration === requestedEntry.lifecycleGeneration],
    ["requested lifecycle identity", checkpoint.sandboxIdentityFingerprint === requestedEntry.lifecycleLiveIdentityFingerprint],
    ...
    ["recorded inference route", isDeepStrictEqual(normalizeSandboxInferenceRouteSelection(normalizeInferenceSelection(recordedEntry)), normalizeSandboxInferenceRouteSelection(reservation.authority.selection))],
    ["requested inference route", isDeepStrictEqual(normalizeSandboxInferenceRouteSelection(normalizeInferenceSelection(requestedEntry)), normalizeSandboxInferenceRouteSelection(reservation.authority.selection))],
  ] as const;
  ...
  return withLock(() => {
    ...
    if (existing?.pendingPolicyVerification) {
      const sameReservation =
        existing.pendingRouteReservation === true &&
