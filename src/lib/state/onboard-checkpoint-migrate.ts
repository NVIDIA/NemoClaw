...
export function deriveCheckpointFromSession(session: Session): OnboardCheckpoint {
  return {
    ...
    // A legacy session cannot prove which lifecycle authority was selected.
    // The entrypoint may backfill this only while ownership remains managed;
    // it never adopts an external declaration without durable owner proof.
    gatewayOwner: null,
    effectGroups: {},
    bindings: {
