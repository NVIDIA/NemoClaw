...
import { readShieldsTimerTakeoverToken } from "./mcp-lifecycle-lock/shields-timer-authority";
import {
...
function ensurePermanentContainmentForStaleGenerationSync(
  lockPath: string,
  sandboxName: string,
  stateDir: string,
...
function isPermanentContainmentError(error: unknown): boolean {
  return (
    error instanceof Error &&
    (error as Error & { code?: string }).code === "NEMOCLAW_PERMANENT_CONTAINMENT"
  );
}
