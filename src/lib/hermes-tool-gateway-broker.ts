...
function persistHermesToolGatewayProviderState(sandboxName, refreshToken) {
...
function registerHermesToolGatewayRefreshProvider(sandboxName, refreshToken, runOpenshell) {
  ...
  persistHermesToolGatewayProviderState(sandboxName, normalized);
...
function isHermesToolGatewayBrokerProcess(pid) {
  ...
  return Boolean(cmdline && cmdline.includes("tool-gateway-broker.js"));
}
...
function spawnHermesToolGatewayBroker() {
  ensurePrivateDir(HERMES_TOOL_GATEWAY_STATE_DIR);
