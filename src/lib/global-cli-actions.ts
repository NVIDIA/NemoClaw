...
export function runSetupJetsonAction(): void {
  if (process.platform !== "linux") {
    console.error(
      "setup-jetson is for NVIDIA Jetson (Linux). Use 'nemoclaw onboard' on macOS.",
    );
    process.exit(1);
  }
  const scriptPath = path.join(SCRIPTS, "setup-jetson.sh");
  const result = spawnSync("sudo", ["bash", scriptPath], { stdio: "inherit" });
