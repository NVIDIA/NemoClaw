 ...
 * managed route probe uses, without a login shell (#8624). A login shell runs
 * sandbox-user startup files, which can emit output into the probe's evidence
 * and create persistent side effects before the managed command runs. Every
 * other terminal agent keeps the login shell, which their smoke commands rely
 * on for profile-provided PATH entries.
 */
