...
describe("privileged sandbox exec routing", () => {
  ...
  it("uses the OpenShell 0.0.106 managed label during portable create discovery", () => {
    const previousProfile = process.env.NEMOCLAW_EXPERIMENTAL_PROFILE;
    ...
    try {
      withPrivilegedExecMocks(
        {
          ...
          dockerCapture: (args) => {
            ...
            return "immutable-alpha-id\topenshell-default--alpha-sandbox-id\n";
          },
        },
        ({ privilegedSandboxExecArgv }) => {
          expect(privilegedSandboxExecArgv("alpha", ["id"])).toEqual([
      ...
      if (previousProfile === undefined) delete process.env.NEMOCLAW_EXPERIMENTAL_PROFILE;
      else process.env.NEMOCLAW_EXPERIMENTAL_PROFILE = previousProfile;
    }

    expect(discoveryCalls[0]).toContain("label=openshell.managed=true");
    expect(discoveryCalls[0]).not.toContain("label=openshell.ai/managed-by=openshell");
  });
