...
function hermesSessionProbeOutput(gatewaySessionCreds: boolean): string {
  return [
    "NEMOCLAW_HERMES_WHATSAPP_SESSION_V1",
    `GATEWAY_SESSION=${gatewaySessionCreds ? "present" : "missing"}`,
  ].join("\n");
}
...
describe("whatsapp.statusHealth openclaw CLI probe", () => {
  ...
  it("reports an unpaired Hermes sandbox from the canonical session path", () => {
    const exec = makeExec({
      status: 0,
      stdout: hermesSessionProbeOutput(false),
      stderr: "",
    });
    ...
    expect(report?.signals.find((s) => s.label === "Pairing / session")?.hint).toContain(
      "hermes whatsapp",
    );
    expect(command).toContain("/sandbox/.hermes/platforms/whatsapp/session/creds.json");
    expect(command).not.toContain("profiles/dashboard-home");
    expect(command).not.toContain("platforms.whatsapp.extra.session_path");
  ...
  it("does not treat a Hermes gateway session file as live inbound health", () => {
    const exec = makeExec({
      status: 0,
      stdout: hermesSessionProbeOutput(true),
      stderr: "",
    });
    const result = createWhatsappStatusHealthHook({ executeSandboxCommand: exec })(
      context({ ...BASE_INPUTS, agent: "hermes" }),
  ...
  it.each([
    {
      ...
      stdout: "GATEWAY_SESSION=missing",
    },
    {
      label: "missing gateway session line",
      stdout: "NEMOCLAW_HERMES_WHATSAPP_SESSION_V1",
    },
    {
      label: "invalid gateway session value",
      stdout: [
        "NEMOCLAW_HERMES_WHATSAPP_SESSION_V1",
        "GATEWAY_SESSION=yes",
      ].join("\n"),
    ...
    expect(stringifyReport(result)).not.toContain("profiles/dashboard-home");
  });
