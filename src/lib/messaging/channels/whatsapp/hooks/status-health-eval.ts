...
export type WhatsappProbeInput = {
  ...
  // Hermes dashboard pairing and gateway startup share one canonical session
  // path. This probe records only whether its credential file exists.
...
function sessionLocationSignal(input: WhatsappProbeInput): DiagnosticSignal | null {
  ...
  if (gateway === false) {
    return {
      label: "Session location",
      severity: "info",
      detail: "the Hermes gateway session path has no WhatsApp credentials",
    };
  }
  return {
    label: "Session location",
    severity: "info",
    detail: "the Hermes gateway session path could not be inspected",
  };
}
