...
function requireSandboxName(value: string, label: string): string {
  ...
  if (!normalized || normalized !== value) fail(`${label} sandbox name is invalid`);
  return normalized;
}
...
export function rebindSandboxMessagingPlanForClone(
  ...
  const targetIntent = {
    ...
    // provider. Preserve "unknown-token" overlap semantics until that exact
    // destination credential is durably proven; never copy a stale hash.
    credentialBindings,
  ...
  return secretProvenanceNeutralRebound;
}
