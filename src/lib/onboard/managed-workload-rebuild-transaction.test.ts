...
function workloadReceipt(
  ...
  const image = contract(agent, generation);
...
function previousEntry(
  ...
  const workload = workloadReceipt(agent, "old");
  return {
    name: `rebuild-${agent}`,
    agent,
...
function handoff(
  ...
  const previousContract = contract(agent, "old");
  const replacementContract = contract(agent, "new");
  const previousProfile = managedStartupE2eProfile(agent);
  return {
    schemaVersion: 1,
    providerId,
    agent,
    previousReceipt: workloadReceipt(agent, "old"),
    previousContract,
    previousProfile,
    replacement: {
      source: {
...
function transactionHarness(
  ...
  const events: string[] = [];
  const oldEntry = previousEntry(agent, providerId);
  ...
  const commitAuthority = (
    ...
    currentEntry =
      swapped.result.status === "committed"
        ? structuredClone(swapped.result.entry)
        : currentEntry;
      ...
      runManagedWorkloadRebuildTransaction(
        {
          ...
          handoff: handoff(agent, providerId),
          operations,
...
describe("managed workload rebuild transaction", () => {
  it.each(
    AGENTS.flatMap((agent) => PROVIDERS.map((provider) => [agent, provider] as const)),
  )("atomically rebuilds %s through the socket-free %s provider contract", async (agent, provider) => {
    const harness = transactionHarness(agent, provider);
  ...
  it.each([
    ...
    expect(
      harness.events.some((event) => event === "retire:runtime-old-exact"),
    ).toBe(false);
    expect(
      harness.events.some((event) => event === "rollback:runtime-new-staged-exact"),
    ).toBe(expectsRollback);
  });
  ...
  it("fails before create when durable authority drifts during preparation", async () => {
    const events: string[] = [];
    const oldEntry = previousEntry("openclaw", "mxc");
    let currentEntry = structuredClone(oldEntry);
    const operations = operationsHarness("mxc", events);
    const prepare = operations.prepare;
    operations.prepare = vi.fn(async (...args) => {
      const prepared = await prepare(...args);
      currentEntry = {
        ...currentEntry,
        lifecycleGeneration: "generation-concurrent",
      };
      return prepared;
    });
  ...
  it("coalesces repeated rollback calls onto one exact-handle operation", async () => {
    ...
    const plan = {
      ...
      previousAuthority: captureSandboxRebuildAuthority(
