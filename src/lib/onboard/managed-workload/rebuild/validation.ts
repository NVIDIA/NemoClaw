...
export function validatePreparedReplacement(
  ...
  const previousRuntimeHandle = boundedText(
    value.previousRuntimeHandle,
    "previous runtime handle",
  );
...
export function validateReboundReplacement(
  ...
  return Object.freeze({
    ...restored,
    providerRebindReceipt: boundedText(
      value.providerRebindReceipt,
      "provider rebind receipt",
    ),
  });
}
