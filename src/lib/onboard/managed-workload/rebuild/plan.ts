...
export function createManagedWorkloadRebuildPlan(input: {
  ...
  if (
    ...
    input.handoff.replacement.source.contract.platform !==
      input.handoff.previousContract.platform
  ) {
    throw new ManagedWorkloadRebuildTransactionError(
      "prepare",
  ...
  const previousAuthority = captureSandboxRebuildAuthority(
    input.previousEntry,
    providerId,
  );
  const replacementReceipt = buildManagedWorkloadRebuildReceipt(
    input.handoff,
    input.provider,
  );
