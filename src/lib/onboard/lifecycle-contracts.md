...
After onboarding, OpenShell owns standard sandbox start, stop, deletion, containment, and provider reconciliation. Public NemoClaw start and stop commands delegate through the typed OpenShell SDK, validate the returned immutable identity, and wait for the same sandbox to reach its requested phase. Recovery uses that same boundary and never falls back to `docker start`, `docker stop`, `podman start`, or `podman stop`.
