...
| **Provider, model, or credential-binding change** — `runInferenceSet` | CLI intent plus registry/session metadata. Target resolution and OpenShell preparation occur before locking. The target is re-resolved under the sandbox lifecycle lock; that phase validates provider/model syntax, selected agent, and local reachability before the first write. | First mutation is the gateway route, then a minimal registry write, API-family/config resolution, registry refresh, best-effort config/hash sync, and audit. Every changed OpenClaw config sync then restarts the managed gateway while the sandbox lock remains held. No sandbox deletion. | The registry stores logical provider/model/credential-environment metadata, while agent-native config stores the applied route and API family. Audit records the action, sandbox, and reason rather than credentials; raw values remain gateway-bound. | Forward-only; no rollback. `rebuild` is the repair path for degraded state. Covered by `inference-set*.test.ts`. Gap: several stores can diverge after a mid-sequence failure. |
| **Credential rotation** — `configRotateToken` in `src/lib/sandbox/config.ts`; `rotateSandboxToken` in `src/lib/sandbox/config-rotate-token.ts` | A named sandbox registry route selects the provider and credential binding whenever that registry row exists; an incomplete route fails closed. Only a sandbox with no registry row falls back to the onboard session, where a non-null different `sandboxName` is rejected but a legacy/null session name is accepted. The new value comes from a named environment variable, stdin, or a secret prompt; it is trimmed, then rejected when empty or still containing internal whitespace. | OpenShell provider update runs first, with provider create as a fallback using the provider type owned by onboarding metadata and the registered gateway-reachable endpoint; an endpoint-backed provider without that metadata fails closed instead of being recreated incompletely. After provider rotation succeeds, `saveCredential` stages the value in the current process and audit runs last. No sandbox deletion. | The logical binding is unchanged, so session and registry are not rewritten. The raw value exists only in process memory/environment and the gateway provider; audit records action/sandbox/reason without the value. | No rollback follows a successful provider update; a later credential-staging or audit failure can report failure after the credential is already active. Covered by `test/security/config-rotate-token.test.ts` and the rotate-token case in `test/security/config-set-nested-ssrf.test.ts`. Gap: a null-name legacy session is not strongly bound to the requested sandbox. |
