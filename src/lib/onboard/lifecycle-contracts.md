...
| **Managed clone handoff and provider transaction (internal and dormant)** — `prepareManagedWorkloadCloneHandoff`; `prepareManagedCloneProviderTransaction` | The current source registry row owns mutable operator intent; an integrity-bound backup authority owns immutable managed-workload and provider-runtime history. The handoff validates the provider, current registry generation, live-identity fingerprint, workload equivalence, runtime generation, and backup digest before it prepares secret-free startup, messaging, dashboard, and provider contributions. | No production command consumes this dormant transaction. Its materializer can create only bindings proven absent at preflight, never updates or deletes a destination-owned provider, and revalidates source and destination authority immediately before each create. | Both plans are deeply frozen and secret-free. A successful create produces an exact process-local ownership receipt; a non-zero create reconciled to an exact provider remains ambiguous and unowned. Raw credentials exist only in the explicit apply environment and one OpenShell child environment. | Failure rolls back only providers confirmed created by the exact in-process receipt and preserves collisions or ambiguous creates. `src/lib/onboard/managed-workload-clone-handoff.test.ts`, `src/lib/onboard/managed-startup-clone-rebinder.test.ts`, and provider transaction tests cover the dormant fail-closed boundary. Epic [#7744](https://github.com/NVIDIA/NemoClaw/issues/7744) tracks any future activation. |
...
## Managed backup and rebuild restore authority

Managed-image backups use one provider-neutral authority path for `backup-all`, stopped-sandbox backup retries, and rebuild backups.
...
Rebuild restore uses this same boundary.
...
The provider-neutral receipt can represent another provider, including an MXC-style implementation, without adding provider switches to backup or rebuild orchestration.
