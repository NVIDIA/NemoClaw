...
After onboarding, OpenShell owns ordinary standard sandbox start, stop, deletion, containment, and provider reconciliation. Public NemoClaw start and stop commands delegate through the typed OpenShell SDK, validate the returned immutable identity, and wait for the same sandbox to reach its requested phase. The retained post-reboot Docker reconciliation path can start or unpause only one exact label- and identity-bound Docker-driver container after a healthy OpenShell gateway reports the registered sandbox missing; it is not a general lifecycle fallback. Native Podman has no equivalent direct recovery path.
