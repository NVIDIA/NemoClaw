...
After the sandbox name and web search are checkpointed, a validated web search credential may be registered in OpenShell before messaging.
After messaging is checkpointed, validated messaging credentials may be registered before the resource prompt.
Each successful registration saves a secret-free provider-name receipt.
...
fresh OpenClaw onboard
  ...
  -> create or update exact web-search provider + save name receipt*
  -> checkpoint messaging prompts* -> create or update exact messaging providers + save name receipts*
...
| Progress and recovery | `lastStepStarted`, `lastCompletedStep`, `failure`, `steps`, `machine`, `sandboxPromptProgress`, `stagedCredentialProviders`, `checkpoint` | Step helpers own step status. `OnboardRuntime` owns normal machine transitions, revisions, terminal state, and machine events. Explicit session recovery and the process-exit failure backstop own their narrow snapshot changes. The OpenClaw sandbox handler owns prompt-group completion markers. `stagedCredentialProviders` contains only names registered before sandbox setup. Provider-effect replay requires the receipt's exact provider-name fingerprint and live name, type, and credential-key bindings. A marker is trusted only when its matching persisted value is present and valid, including an explicit `null` where supported. `checkpoint` is the dedicated versioned resume contract: a secret-free tri-state decision record plus durable sandbox identity, effect-group receipts, and logical web-search and messaging provider bindings, serialized alongside the session under its own `schemaVersion` with fail-closed handling of an unknown future version. The primary inference provider binding remains owned and revalidated by the provider and inference phases instead of entering this checkpoint ledger. |
...
| Messaging intent | `messagingPlan`, `telegramConfig`, `wechatConfig` | `messaging-plan-authority.ts` selects the compact registry plan for an existing sandbox. A staged plan, then a matching session plan, supplies new or pending targets. `telegramConfig` and `wechatConfig` are legacy fallback/preserved fields. Tokens stay outside the session. |
...
The registry is separately owned by `src/lib/state/registry.ts`; backup/recovery manifests are owned by their rebuild and recreate modules. Step helpers own only step status and safe step updates. They cannot move the machine snapshot. Explicit state results pass through `OnboardRuntime` validation.

## Duplicated decision points

1. **Messaging intent source:** `messaging-plan-authority.ts` now owns source precedence. The registry plan is authoritative for an existing sandbox. Staged and matching-session plans are fallbacks for new or pending targets. Conflict and persistence policy remain separate work.
...
5. **Registry lifecycle:** create registers post-ready; rebuild records removals and restores retry metadata through `rebuild-registry-rollback.ts`; some resume decisions remove the row before backup/create preparation and have no receipt-based compensation. Recommended owner: `sandbox-registration.ts` plus the existing durable pre-create identity.
6. **Replacement validation:** generic rebuild retains and fingerprints a successfully built context; normal live DCode rebuild adds route and managed-context proofs; re-onboard still stages the replacement after delete. Health-before-delete and atomic swap remain unresolved. Closed issue #5801 records the original gap; #6835 fixed only the printed recovery path.
