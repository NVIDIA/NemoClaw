...
The CLI action can reconcile an absent failed N1x managed-vLLM attempt before entry admission,
including the default `my-assistant` name. It requires one retained failed-create record and its
pending registry route on the current gateway port and default workspace. Under the lifecycle lock,
it restores the recorded gateway, requires OpenShell to confirm absence, and asks for explicit cleanup
consent. The existing destroy path rechecks authority and removes verified residual resources.
Only confirmed retirement of both the recovery record and registry row permits fresh onboarding
under the same name. A live sandbox, an inconclusive lookup, changed authority, or declined cleanup
keeps recovery unresolved. Installer auto-yes does not authorize this cleanup.
