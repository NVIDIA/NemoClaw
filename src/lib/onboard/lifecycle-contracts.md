...
fresh onboard
  ...
  -> gateway/provider effects -> checkpoint completed sandbox prompts* -> resolve complete sandbox-create intent
...
| Progress and recovery | `lastStepStarted`, `lastCompletedStep`, `failure`, `steps`, `machine`, `sandboxPromptProgress` | Step-mutation helpers and `OnboardRuntime` own whole-step progress; the sandbox handler owns prompt-group completion markers. A marker is trusted only when its matching persisted value is present and valid, including an explicit `null` where supported. |
