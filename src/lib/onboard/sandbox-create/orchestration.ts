...
export function cleanupRecreatedSourceHermesStateVolume(
  ...
  if (cleanup.status === "failed") {
    throw new Error(
      `Sandbox '${input.sandboxName}' is gone, but its managed Hermes state volume '${cleanup.volumeName}' could not be removed: ${deps.redact(cleanup.detail)}. The sandbox registry entry was preserved so exact cleanup can be retried.`,
    );
  }
