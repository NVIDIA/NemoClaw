...
describe("dockerfile patch helpers", () => {
  ...
  it("bakes the host corporate CA into NEMOCLAW_CORPORATE_CA_B64 (#6210)", () => {
    const caDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-corp-ca-arg-"));
    tmpRoots.push(caDir);
    const caFile = path.join(caDir, "corp-ca.pem");
    const caPem = "-----BEGIN CERTIFICATE-----\nMIIBcorp\n-----END CERTIFICATE-----\n";
    fs.writeFileSync(caFile, caPem, { mode: 0o644 });
    process.env.NEMOCLAW_CORPORATE_CA_BUNDLE = caFile;
    try {
      ...
      expect(Buffer.from(encoded, "base64").toString("utf8")).toBe(caPem);
    } finally {
