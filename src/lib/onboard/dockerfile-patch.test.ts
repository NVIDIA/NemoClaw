...
describe("dockerfile patch helpers", () => {
  ...
  it("patches the staged Dockerfile with DuckDuckGo provider when selected", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-onboard-dockerfile-ddg-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
    ...
    try {
      patchStagedDockerfile(
        ...
        "openai-api",
        null,
        { fetchEnabled: true, provider: "duckduckgo" },
      );
      const patched = fs.readFileSync(dockerfilePath, "utf8");
      assert.match(patched, /^ARG NEMOCLAW_WEB_SEARCH_ENABLED=1$/m);
      assert.match(patched, /^ARG NEMOCLAW_WEB_SEARCH_PROVIDER=duckduckgo$/m);
      assert.doesNotMatch(patched, /BRAVE_API_KEY/);
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
