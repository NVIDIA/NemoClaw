...
describe("dockerfile patch helpers", () => {
  ...
  it("#4560: bakes NEMOCLAW_EXTRA_AGENTS_JSON env into the staged Dockerfile", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-onboard-dockerfile-extra-agents-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
    ...
    process.env.NEMOCLAW_EXTRA_AGENTS_JSON = JSON.stringify(extras);
    try {
      patchStagedDockerfile(
        dockerfilePath,
        "gpt-5.4",
      ...
      const line = patched
        .split("\n")
        .find((l) => l.startsWith("ARG NEMOCLAW_EXTRA_AGENTS_JSON_B64="));
      assert.ok(line, "expected extra agents build arg");
      const encoded = line.split("=")[1];
  ...
  it("#4560: preserves the empty default when NEMOCLAW_EXTRA_AGENTS_JSON is unset or invalid", () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "nemoclaw-onboard-dockerfile-extra-agents-bad-"),
    );
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    const baseDockerfile = [
      "ARG NEMOCLAW_MODEL=nvidia/nemotron-3-super-120b-a12b",
