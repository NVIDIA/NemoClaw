...
const CA_DISABLED_ENVIRONMENT: NodeJS.ProcessEnv = {
...
function openClawInput(
  ...
  return {
    ...
    environment: CA_DISABLED_ENVIRONMENT,
    ...overrides,
  };
}
...
describe("buildManagedStartupOnboardProfile", () => {
  ...
  ] as const)("keeps credential-bearing proxy aliases out of the %s secret-free profile", (agent, input) => {
    const built = buildManagedStartupOnboardProfile(
      input({
        environment: {
          ...CA_DISABLED_ENVIRONMENT,
