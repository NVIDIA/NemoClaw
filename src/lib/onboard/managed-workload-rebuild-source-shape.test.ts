...
describe("managed workload rebuild source shape", () => {
  it.each(CENTRAL_REBUILD_MODULES)(
    ...
    (file) => {
      ...
      expect(text).not.toMatch(
        /switch\s*\(\s*(?:providerId|[^)]*[.]openshellDriver)\s*\)/iu,
      );
    },
  );

  it.each(CENTRAL_REBUILD_MODULES)(
    "keeps %s free of name-only sandbox deletion",
    (file) => {
      const text = source(file);
  ...
  it("requires exact provider handles for rollback and retirement", () => {
    ...
    expect(contract).toContain("A sandbox name is intentionally insufficient authority");
  });
