...
/** Privileged control for one authenticated live ID on an already qualified engine. */
export function createLiveDockerPrivilegedSandboxControl(input: {
  readonly engine: import("../../adapters/container-engine").ContainerEngine;
  ...
  if (input.engine.engineId !== "docker" || input.engine.operation !== "sandbox-lifecycle") {
    throw new Error("Live privileged control requires a qualified Docker lifecycle engine.");
  }
  const labels = {
    [OPENSHELL_MANAGED_BY_LABEL]: OPENSHELL_MANAGED_BY_VALUE,
    [OPENSHELL_SANDBOX_NAME_LABEL]: input.sandboxName,
    "openshell.ai/sandbox-id": input.sandboxId,
    "openshell.ai/sandbox-workspace": input.workspace,
    "openshell.ai/sandbox-namespace": input.sandboxNamespace,
  };
  const observe = (): string => {
    input.assertCurrent();
    const listed = input.engine.capture(
      [
        "ps",
        "--all",
        "--no-trunc",
        ...
        `label=${OPENSHELL_SANDBOX_NAME_LABEL}=${input.sandboxName}`,
        "--filter",
        `label=openshell.ai/sandbox-namespace=${input.sandboxNamespace}`,
        "--format",
        "{{.ID}}",
      ],
      5000,
    ...
    if (
      ...
      listed.error ||
      ids.length !== 1 ||
      !/^[0-9a-f]{64}$/u.test(ids[0] ?? "")
    ) {
      throw new DirectSandboxFallbackUnavailableError(
        "The live MCP sandbox has no unambiguous runtime identity.",
      );
    }
    const observed = input.engine.capture(
      [
        "inspect",
        "--format",
        "{{json .Id}}\t{{json .State.Running}}\t{{json .State.Paused}}\t{{json .Config.Labels}}",
        ids[0],
      ],
      5000,
    );
    ...
    try {
      ...
      /* Fail closed below without printing runtime metadata. */
    }
    const actualLabels = actual[3];
    if (
      observed.status !== 0 ||
      observed.error ||
      actual.length !== 4 ||
      ...
      Object.entries(labels).some(
        ([key, value]) => (actualLabels as Record<string, unknown>)[key] !== value,
      )
    ) {
      throw new DirectSandboxFallbackUnavailableError(
        "The live MCP runtime does not match its authenticated sandbox identity.",
      );
    }
    input.assertCurrent();
    return ids[0];
  };
  const resourceHandle = observe();
  const resolveTarget: RuntimeProviderPrivilegedSandboxControl["resolveTarget"] = (target) => {
    if (
      target.sandboxName !== input.sandboxName ||
      target.sandbox.name !== input.sandboxName ||
      observe() !== resourceHandle
    ) {
      throw new PinnedSandboxResourceIdentityChangedError(input.sandboxName);
    }
    return Object.freeze({ providerId: "docker", resourceHandle });
  };
  return Object.freeze({
    resolveTarget,
    execute(command: RuntimeProviderPrivilegedSandboxCommandInput) {
      const target = resolveTarget(command);
      if (
        command.expectedResourceHandle !== undefined &&
        command.expectedResourceHandle !== target.resourceHandle
      ) {
        throw new PinnedSandboxResourceIdentityChangedError(input.sandboxName);
      }
      const result = input.captureCommand(
        [
          "exec",
          ...(command.input ? ["-i"] : []),
