...
describe("Synchronous Docker operation capture", () => {
  it.each([1024 * 1024 + 129, 16 * 1024 * 1024])(
    "passes a valid %i-byte config through the fixed authority",
    (bytes) => {
      const executableRoot = fakeDockerScript("/usr/bin/wc -c");
      const authority = createDockerOperationAuthority("sandbox-lifecycle", {
        PATH: executableRoot,
        HOME: "/tmp/nemoclaw-home",
        DOCKER_HOST: "unix:///tmp/nemoclaw-owned.sock",
      });
      const payload = Buffer.from(JSON.stringify({ padding: "x".repeat(bytes - 15) }) + "\n");
      expect(payload.length).toBe(bytes);
      const result = captureDockerOperationCommand(authority, ["exec", "owned", "cat"], {
        input: payload,
        timeoutMs: 10_000,
        maxOutputBytes: 1024,
      });
      ...
      expect(Number(result.stdout.toString("utf8").trim())).toBe(bytes);
    },
  );

  it("retains endpoint, executable, and environment across ambient drift", () => {
    const executableRoot = fakeDockerScript(
      `printf '%s\\n' "$PATH" "\${NVIDIA_API_KEY-unset}" "$@"`,
    );
    const authority = createDockerOperationAuthority("sandbox-lifecycle", {
      PATH: executableRoot,
      HOME: "/tmp/nemoclaw-home",
    ...
    vi.stubEnv("PATH", fakeDocker("wrong-executable"));
    vi.stubEnv("DOCKER_HOST", "unix:///tmp/wrong.sock");
    vi.stubEnv("NVIDIA_API_KEY", "secret-must-not-leak");
    const result = captureDockerOperationCommand(authority, ["version"], { timeoutMs: 10_000 });
    expect(result.stdout.toString("utf8")).toBe(
      `${executableRoot}\nunset\n--config\n/tmp/nemoclaw-home/.docker\n--host\nunix:///tmp/nemoclaw-owned.sock\nversion\n`,
    );
  });

  it("refuses a changed endpoint before synchronous execution", () => {
    const capture = createDriftingContextCapture();
    const authority = createDockerOperationAuthority(
      "sandbox-lifecycle",
      { HOME: "/tmp/nemoclaw-home", DOCKER_CONTEXT: "spark" },
    ...
    expect(() => captureDockerOperationCommand(authority, ["info"], { timeoutMs: 1000 })).toThrow(
      "Docker context endpoint changed after qualification",
    );
    expect(capture.mock.calls.some(([, args]) => args.at(-1) === "info")).toBe(false);
  });

  it("refuses executable replacement and a forged authority before running", () => {
    const executableRoot = fakeDocker("first");
    const authority = createDockerOperationAuthority("sandbox-lifecycle", {
      PATH: executableRoot,
      HOME: "/tmp/nemoclaw-home",
    ...
    writeFakeExecutable(executableRoot, "docker", "printf 'replacement\n'");
    expect(() =>
      captureDockerOperationCommand(authority, ["version"], { timeoutMs: 1000 }),
    ).toThrow("changed after qualification");
    expect(() =>
      captureDockerOperationCommand({ ...authority }, ["version"], { timeoutMs: 1000 }),
    ).toThrow("owned authority");
  });

  it("enforces the caller timeout for synchronous execution", () => {
    const executableRoot = fakeDockerScript("exec /bin/sleep 5");
    const authority = createDockerOperationAuthority("sandbox-lifecycle", {
      PATH: executableRoot,
      HOME: "/tmp/nemoclaw-home",
      DOCKER_HOST: "unix:///tmp/nemoclaw-owned.sock",
    });
    const result = captureDockerOperationCommand(authority, ["version"], { timeoutMs: 10 });
    expect((result.error as NodeJS.ErrnoException | undefined)?.code).toBe("ETIMEDOUT");
    expect(result.status).not.toBe(0);
  });

  it("refuses oversized stdin and bounds output without raising the generic engine cap", () => {
    const executableRoot = fakeDockerScript("/usr/bin/head -c 4096 /dev/zero");
    const authority = createDockerOperationAuthority("sandbox-lifecycle", {
      PATH: executableRoot,
      HOME: "/tmp/nemoclaw-home",
      DOCKER_HOST: "unix:///tmp/nemoclaw-owned.sock",
    ...
    expect(() =>
      captureDockerOperationCommand(authority, ["exec", "owned", "cat"], {
        input: Buffer.alloc(16 * 1024 * 1024 + 1),
        timeoutMs: 1000,
      }),
    ).toThrow("bounded input");
    expect(() =>
      authority.engine.capture(["exec", "owned", "cat"], 1000, Buffer.alloc(1024 * 1024 + 1)),
    ).toThrow("input is invalid or exceeds its byte bound");
    const result = captureDockerOperationCommand(authority, ["version"], {
      timeoutMs: 1000,
      maxOutputBytes: 128,
    });
    expect((result.error as NodeJS.ErrnoException | undefined)?.code).toBe("ENOBUFS");
    expect(() =>
      captureDockerOperationCommand(authority, ["version"], {
        timeoutMs: 1000,
