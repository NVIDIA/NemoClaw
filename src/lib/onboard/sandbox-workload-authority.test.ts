...
describe("managed workload authority", () => {
  ...
  it("rejects missing explicit OCI platform", () => {
    const { platform: _platform, ...withoutPlatform } = managedReceipt(
      "openclaw",
      "linux/amd64",
    );
    expect(() =>
      readManagedWorkloadAuthority(
        managedEntry(
          "openclaw",
          "linux/amd64",
          withoutPlatform as SandboxWorkloadReceipt,
        ),
      ),
  ...
  it("rejects cross-agent image authority", () => {
    ...
    expect(() =>
      readManagedWorkloadAuthority(
        managedEntry("hermes", "linux/amd64", openClawReceipt),
      ),
    ).toThrow(/does not belong to 'hermes'/u);
  });

  it("rejects cross-agent startup profile authority", () => {
    const hermesReceiptWithOpenClawProfile = managedReceipt(
      "hermes",
      "linux/amd64",
      "openclaw",
    );
    ...
    ).toThrow(/startup profile belongs to 'openclaw'/u);
  });
