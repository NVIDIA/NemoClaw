...
describe("handleFinalizationState", () => {
  ...
  // Scenario A (#4504): the auto-pair scope-approval
  // sweep run against the freshly-recovered gateway — strictly after process
  // recovery (which can restart the gateway, #3573), and strictly before
  // deployment verification.
  it("runs the auto-pair scope-approval sweep before verify (#4504)", async () => {
    const { deps, calls } = createDeps();
