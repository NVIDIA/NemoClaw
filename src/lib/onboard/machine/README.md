...
- Session step helpers record only step status (`pending`, `in_progress`, `complete`, `failed`, `skipped`) and session fields accepted by `filterSafeUpdates`. They cannot change the machine snapshot.
