...
describe("ensureOpenshellForOnboard", () => {
  ...
  it("applies the 0.0.82 floor during final validation when the blueprint omits a minimum", () => {
    const deps = makeDeps({
    ...
    expect(deps.error).toHaveBeenCalledWith("    blueprint.yaml min_openshell_version: 0.0.82");
  });
});
