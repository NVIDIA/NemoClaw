...
function rejectedCorporateCaError(error: unknown): CorporateCaValidationError | null {
...
export async function prepareSandboxWorkloadSource(
  ...
  try {
    ...
    const rejectedCa = rejectedCorporateCaError(error);
    if (rejectedCa) {
      throw new SandboxWorkloadPreparationError(
        `${CORPORATE_CA_EXPLICIT_ENV} was rejected: ${rejectedCa.message}`,
        { cause: error },
      );
    }
