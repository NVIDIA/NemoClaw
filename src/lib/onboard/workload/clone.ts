...
function requireSandboxName(value: string, label: string): string {
  if (!/^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/u.test(value)) {
    fail(`${label} sandbox name is invalid`);
  }
  return value;
}
...
function requireProviderName(value: string, label: string): string {
  if (!/^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/u.test(value)) {
    fail(`${label} provider name is invalid`);
  }
  return value;
}
...
function reboundWorkload(
  ...
  if (!provider.workload.acceptsReceipt(normalized)) {
...
function reboundMessaging(
  ...
  return Object.freeze({ schemaVersion: 1, plan });
}
...
export function prepareManagedWorkloadCloneHandoff(
  ...
  return Object.freeze({
    ...
    runtimeSnapshot: Object.freeze(runtimeSnapshot),
    rebound,
    workload,
    ...(messaging === undefined ? {} : { messaging: Object.freeze(messaging) }),
    registryFields: Object.freeze(registryFields(rebound.profile, input.source)),
  });
}
