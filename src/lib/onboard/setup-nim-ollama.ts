...
export function createSetupNimOllamaHandlers(deps: SetupNimOllamaDeps): {
  ...
  async function handleWindowsHostOllamaSelection(
    ...
    if (isSwitch) {
      state.verifyLivePolicyRequirements?.("switch to the Windows Ollama runtime");
      deps.switchToWindowsOllamaHost();
    } else if (isInstall) {
      state.verifyLivePolicyRequirements?.("install the Windows Ollama runtime");
      ...
      if (!deps.awaitWindowsOllamaReady()) {
        ...
        state.verifyLivePolicyRequirements?.("start the Windows Ollama runtime");
  ...
  async function handleRunningOllamaSelection(
    ...
    if (!isWindowsHostOllama) {
      state.verifyLivePolicyRequirements?.("configure the local Ollama runtime");
    ...
    state.verifyLivePolicyRequirements?.("start the local Ollama runtime");
  ...
  async function handleInstallOllamaSelection(
    ...
    state.verifyLivePolicyRequirements?.("install the local Ollama runtime");
    const installResult =
