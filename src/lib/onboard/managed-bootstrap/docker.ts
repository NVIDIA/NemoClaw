...
function reconstructDockerBootstrapTransaction(
  ...
  assertDockerBootstrapTransactionAuthority(transaction, handle, snapshot, replacement);
  return transaction;
}
...
export function createDockerManagedBootstrapAdapter(
  ...
  const rollbackBootstrapNow = ({
    ...
    if (!snapshot) {
      if (journal) {
        throw new ManagedBootstrapCommitStateIndeterminateError({
          ...
          detail: "a durable Docker transaction exists without its observed snapshot",
        });
      }
    ...
    assertDockerBootstrapTransactionAuthority(journal, handle, snapshot, replacement);
  ...
  return {
    ...
    async replaceForBootstrap({ handle, snapshot, request, replacementOptions }) {
      ...
      if (existingJournal) {
        ...
        if (existingJournal.phase !== "cutover") {
          throw new ManagedBootstrapCommitStateIndeterminateError({
            bootstrapIdentity: existingJournal.bootstrapIdentity,
            runtimeId: existingJournal.replacementRuntimeId,
            detail: `replacement retry requires rollback or commit from durable phase ${existingJournal.phase}`,
          });
        }
        assertCompletedCutoverRuntimeState(existingJournal, deps);
        const resumedReplacement = inspectExact(existingJournal.replacementRuntimeId, deps);
        if (
          assertReplacementMatchesIntent(
            snapshot.specCanonicalJson,
            resumedReplacement,
            originalName,
            plan,
            openshellSandboxCommandEnvValue(handle.intendedWorkloadArgv) as string,
          ) !== existingJournal.replacementSpecHash
        ) {
          throw new Error("Managed bootstrap Docker retry changed its replacement plan.");
        }
        return Object.freeze({
          schemaVersion: MANAGED_BOOTSTRAP_SCHEMA_VERSION,
          sandbox: handle.sandbox,
          bootstrapIdentity: handle.bootstrapIdentity,
          originalRuntimeId: existingJournal.originalRuntimeId,
          replacementRuntimeId: existingJournal.replacementRuntimeId,
          image: snapshot.image,
          runtimeImageContentId: existingJournal.runtimeImageContentId,
          originalSpecHash: existingJournal.originalSpecHash,
          replacementSpecHash: existingJournal.replacementSpecHash,
          profileFingerprint: existingJournal.profileFingerprint,
        });
      }
      ...
      try {
        ...
        if (dockerContainerName(replacementBeforeJournal) !== stagingName) {
        ...
        let journal = createDockerBootstrapJournalDurably(stagedAuthority, deps);
        const originalAtFence = inspectExact(snapshot.runtimeId, deps);
        const replacementAtFence = inspectExact(replacementRuntimeId, deps);
        assertTransactionOriginal(journal, originalAtFence);
        assertTransactionReplacement(journal, replacementAtFence);
        if (
          dockerContainerName(originalAtFence) !== originalName ||
          originalAtFence.State?.Running !== true ||
          dockerContainerName(replacementAtFence) !== stagingName
        ) {
          throw new Error(
        ...
        if (dockerContainerName(afterStop) !== originalName) {
          throw new Error(
        ...
        const renamedOriginal = deps.dockerRename(snapshot.runtimeId, backupContainerName, options);
        const afterOriginalRename = inspectExact(snapshot.runtimeId, deps);
        assertTransactionOriginal(journal, afterOriginalRename);
        if (dockerContainerName(afterOriginalRename) !== backupContainerName) {
          throw new Error(
        ...
        const renamedReplacement = deps.dockerRename(replacementRuntimeId, originalName, options);
        const afterReplacementRename = inspectExact(replacementRuntimeId, deps);
        assertTransactionReplacement(journal, afterReplacementRename);
        if (dockerContainerName(afterReplacementRename) !== originalName) {
          throw new Error(
        ...
        const started = deps.dockerStart(replacementRuntimeId, options);
        const running = inspectExact(replacementRuntimeId, deps);
        assertTransactionReplacement(journal, running);
        if (
          dockerContainerName(running) !== originalName ||
        ...
        const runningSpecHash = assertReplacementMatchesIntent(
          snapshot.specCanonicalJson,
          running,
          originalName,
          plan,
        ...
        if (runningSpecHash !== replacementSpecHash) {
        ...
        if (dockerContainerName(preservedOriginal) !== backupContainerName) {
        ...
        try {
          ...
          if (!durable && stagedAuthority) {
            const original = inspectExact(stagedAuthority.originalRuntimeId, deps);
            assertTransactionOriginal(stagedAuthority, original);
            if (
              dockerContainerName(original) !== stagedAuthority.originalName ||
              original.State?.Running !== true
            ) {
              throw new Error(
            ...
            const staged = inspectTransactionRuntime(
              stagedAuthority,
              stagedAuthority.replacementRuntimeId,
              deps,
            );
            if (staged) removeExactReplacement(stagedAuthority, staged, deps);
          }
        } catch (cleanupError) {
