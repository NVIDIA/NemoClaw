...
consume persisted engine recovery, add GPU and local inference, qualify the
installer, and enable supported activation.

The dormant persisted-engine authority records only an operation-scoped
provider, engine, endpoint authority, and non-secret binding digest. A caller
may match that record only with a freshly provider-qualified injected engine;
the record never persists an executable, endpoint, environment value, or
credential and cannot select or activate a provider. Its contract remains
provider-neutral, and tests prove that an MXC-style engine uses the same
authority without introducing a Podman switch.

Snapshot creation, clone, rebuild, backup, restore, and recovery can prepare an
inert lifecycle ledger against that exact injected authority. Each record binds
the action, source and target runtime handles, and a digest of opaque runtime
and acceleration state before publishing mutation authority. After restart,
the caller must reconstruct the same provider, engine, endpoint authority,
binding, runtime handles, and state digest. A durable process-owned execution
lease excludes competing work, can replace an abandoned owner only after the
recorded PID is no longer live, and is revalidated around every command and
completion transition. Exact runtime argument positions prevent a mutable name
from becoming destructive authority. Completion receipts and retirement
tombstones make recovery independent of process-local state while keeping the
provider dormant.
