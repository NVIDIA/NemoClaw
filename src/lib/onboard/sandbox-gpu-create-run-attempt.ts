...
function resolveCreateAttemptNonce(
  ...
  if (
    ...
    input.createAttemptNonce !== undefined &&
    input.createAttemptNonce !== resumedCreateAttemptNonce
  ) {
    throw new Error(
      "Sandbox create-attempt nonce does not match the verified resume checkpoint; refusing continuation.",
    );
  }
  if (!input.resumeVerifiedCreate) {
    ...
    if (
      nonce &&
      (nonce.length !== NEMOCLAW_CREATE_ATTEMPT_NONCE_HEX_LENGTH || !/^[0-9a-f]+$/u.test(nonce))
    ) {
      throw new Error("Sandbox creation has an invalid create-attempt nonce.");
    }
    return nonce;
  }
  if (
