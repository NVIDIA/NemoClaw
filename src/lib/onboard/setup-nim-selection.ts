...
export function credentialMutationGuardFor(
  ...
  return state.verifyLivePolicyRequirements;
}

export function withCredentialMutationGuard<T extends object>(
  state: SetupNimSelectionState,
  options: T,
): T & { verifyLivePolicyRequirements?: (operation: string) => void } {
  const guard = credentialMutationGuardFor(state);
  return guard ? { ...options, verifyLivePolicyRequirements: guard } : options;
}
