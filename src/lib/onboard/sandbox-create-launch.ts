...
export function buildSandboxRuntimeEnvArgs(input: SandboxRuntimeEnvArgsInput): {
  ...
  // Every sandbox needs to know its own name at runtime, not just the
  // deepagents image: OpenShell exports OPENSHELL_SANDBOX as the boolean "1" to
  // the processes it spawns inside the sandbox, so this injection is the only
  // in-container source of the name. nemoclaw-start.sh bakes it into the
