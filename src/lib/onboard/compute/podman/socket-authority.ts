...
function directoryChain(
  ...
  const immediateDirectory = path.dirname(socketPath);
  const directories: string[] = [];
  for (let directory = immediateDirectory; ; directory = path.dirname(directory)) {
...
export function capturePodmanSocketAuthority(
  ...
  return Object.freeze({
    directoryChain: directoryChain(normalized, uid, lstat),
