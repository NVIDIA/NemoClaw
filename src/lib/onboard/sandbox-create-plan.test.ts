...
describe("prepareSandboxCreatePlan", () => {
  ...
  it("drops managed web-search extra providers when onboard disables web search (#6501)", () => {
    const result = prepareSandboxCreatePlan({
      basePolicyPath: "/repo/policy.yaml",
      buildCtx: "/tmp/nemoclaw-build-1",
      sandboxName: "sandbox",
    ...
    const providerArgs = result.createArgs
      .map((arg, index) => (arg === "--provider" ? result.createArgs[index + 1] : null))
      .filter((value): value is string => value !== null);
    expect(providerArgs).toEqual(["custom-provider"]);
  });

  it.each([
    ["brave", ["brave-search", "custom-provider"]],
    ["tavily", ["tavily-search", "custom-provider"]],
  ] as const)("keeps only selected %s web search and unrelated custom extra providers (#6501)", (provider, expectedProviders) => {
    const result = prepareSandboxCreatePlan({
      basePolicyPath: "/repo/policy.yaml",
      buildCtx: "/tmp/nemoclaw-build-1",
      sandboxName: "sandbox",
    ...
    const providerArgs = result.createArgs
      .map((arg, index) => (arg === "--provider" ? result.createArgs[index + 1] : null))
      .filter((value): value is string => value !== null);
    expect(providerArgs).toEqual(expectedProviders);
  });
