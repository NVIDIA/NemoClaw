...
describe("Hermes portable lifecycle", () => {
  ...
  it("reuses settled schema-8 authority under the explicit GFN gate", async () => {
    const receipt = activeReceipt(stateDir);
    publishSuccessor();
    const fixture = lifecycleDeps(receipt);
    Object.assign(fixture.deps.env!, { GFN_HERMES_TRUST_DURABLE_AUTHORITY: "1" });

    const requalified = await withPortableHostFence(stateDir, () =>
      withMcpLifecycleLockSync(
        SANDBOX,
        () => requalifyHermesPortableSandboxAuthority(SANDBOX, lifecycleContext(), fixture.deps),
        { stateDir: path.join(stateDir, "state") },
