...
import {
  createActiveLifecycleTestReceipt,
...
function activeReceipt(homeDir = "/home/test"): HermesPortableConfiguredReceipt {
  return createActiveLifecycleTestReceipt({
    stateDir,
    policyPath,
    homeDir,
    sandboxName: SANDBOX,
...
describe("Hermes portable lifecycle", () => {
  ...
  it("uses one entry and final qualification when the timing callback fails (#10423)", async () => {
    ...
    expect(operations.filter((operation) => operation === "sandbox:list")).toHaveLength(5);
    expect(operations.filter((operation) => operation === "sandbox:get")).toHaveLength(5);
  ...
  it("uses post-start authority to roll back an Error-origin OpenShell sandbox", async () => {
    const receipt = activeReceipt();
    await publishSuccessor();
    const fixture = lifecycleDeps(receipt, false, {
  ...
  it("does not stop an already-running container after a health failure (#9203)", async () => {
    const receipt = activeReceipt();
