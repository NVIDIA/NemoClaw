...
export interface BuiltManagedStartupProfile {
  ...
  /** Digest of the canonical secret-free profile transport. */
  readonly startupProfileSha256: string;
  /** Exact normalized PEM bytes, encoded separately from the secret-free profile. */
...
/**
 * Build the one driver-neutral startup profile used by Docker, Podman, and
 * future OpenShell compute drivers.
 */
export function buildManagedStartupProfile(
  ...
  try {
    ...
    const encodedProfile = encodeManagedStartupProfile(built.profile);
