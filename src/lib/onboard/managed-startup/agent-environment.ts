...
function applicationRuntimePlan(
  ...
  const unsetEnvironment = MANAGED_STARTUP_RUNTIME_CLEANUP_OBLIGATIONS.filter(({ emittedFor }) =>
    emittedFor.includes(profile.agent),
  )
    .map(({ input }) => input)
    .sort();
  return Object.freeze({
    exportEnvironment: sortedEnvironment(exportEnvironment),
    unsetEnvironment: Object.freeze(unsetEnvironment),
  });
}
