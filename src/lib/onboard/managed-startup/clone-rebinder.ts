 ...
 * immutable image affordances (proxy route, tuning, CA, OpenClaw bootstrap
 * knobs); mutable operator intent is re-read from this state before a clone can
  ...
  return {
    ...
    compatibility:
      profile.agent === "openclaw"
        ...
        : null,
    // Input-modality detection is a stock-image affordance with no durable
    // post-onboard mutator. Keep the receipt-owned, already validated value.
...
function currentAgentConfig(
  ...
  return {
    ...
    autoApprovalMode,
    // Registry persistence represents false as absence for this flag.
  ...
  if (dashboard.agent === "hermes") {
    if (dashboard.mode === "disabled") {
      if (destinationDashboardPort === null) {
        return {
          ...dashboard,
          // Disabled Hermes carries CHAT_UI_URL as a stock-image input only.
          // Remove the source sandbox's host-port identity even though no
          // destination forward is allocated.
          url: "http://127.0.0.1/",
        };
      }
      return {
