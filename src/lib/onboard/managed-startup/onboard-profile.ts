...
export interface ManagedStartupOnboardProfileInput {
  ...
  /** Validated durable CA material retained by an authoritative managed rebuild. */
...
export function buildManagedStartupOnboardProfile(
  ...
  const corporateCa =
    input.corporateCaOverride === undefined
      ? resolveCorporateCa(input.environment)
      : input.corporateCaOverride;
