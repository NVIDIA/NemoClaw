...
function releaseSupersededOllamaModel(
  ...
  try {
    ...
    withOwnershipLock(() => {
      ...
      try {
        verifyLivePolicyRequirements?.("release the superseded Ollama model");
      } catch (error) {
...
export function createSetupInference(
  ...
  return async function setupInferenceWithDeps(
    ...
    const verifyLivePolicyRequirements = sandboxName
    ...
    const mutateGatewayRoute = (): Promise<SetupInferenceResult> =>
      ...
      withInferenceMutationLocks(async () => {
        verifyLivePolicyRequirements?.("change the inference provider route");
        if (
        ...
        if (
          ...
          verifyLivePolicyRequirements?.("change the inference provider route after DNS validation");
        }
        ...
        const runGatewayOpenshell: typeof runExactGatewayOpenshell = (...args) => {
          verifyLivePolicyRequirements?.("change the OpenShell inference provider route");
        ...
        const reserveRoute = (name: string, selectedProvider: string, selectedModel: string) => {
          ...
          verifyLivePolicyRequirements?.("reserve the sandbox inference route");
        ...
        const profiledUpsertProvider = bindOpenAiProviderProfile(
          (...args) => {
            verifyLivePolicyRequirements?.("register the inference provider");
        ...
        if (options.hostLocalInference) {
          try {
            verifyLivePolicyRequirements?.("prepare the host-local inference runtime");
            ...
            verifyLivePolicyRequirements?.("prepare the host-local inference provider route");
        ...
        try {
          ...
          if (hostLocalRoute) {
            ...
            verifyLivePolicyRequirements?.("commit the host-local inference provider route");
            ...
            verifyLivePolicyRequirements?.("publish the host-local inference provider receipt");
          ...
          verifyLivePolicyRequirements?.("report successful inference provider setup");
