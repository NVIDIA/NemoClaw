...
export type CuaTaskAdapterResult = CuaTargetAttachment | CuaTaskResult | CuaFailure;
  ...
  execute(request: CuaTaskAdapterRequest): CuaTaskAdapterResult {
    ...
    try {
      snapshot = snapshotBoundedExecutable(this.executable, {
        label: "the CUA task adapter",
        minBytes: 1,
        maxBytes: MAX_ADAPTER_BYTES,
        temporaryDirectoryPrefix: "nemoclaw-cua-task-adapter-",
        ...(this.expectedDigest === undefined ? {} : { expectedDigest: this.expectedDigest }),
      });
    } catch {
      this.#executableDigest = null;
      throw new CuaTaskAdapterInvocationError(
        "the CUA task adapter is unavailable or does not match its expected digest",
