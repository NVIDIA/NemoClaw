...
describe("CLI OpenShell sandbox command executor", () => {
  ...
  it("sanitizes the default environment for direct buffered CLI calls", async () => {
    ...
    const result = await runCliOpenShellBufferedCommand(process.execPath,
      ["-e", "process.stdout.write(JSON.stringify({ secret: process.env.NVIDIA_INFERENCE_API_KEY, path: process.env.PATH }))"],
