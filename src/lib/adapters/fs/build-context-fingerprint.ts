...
export function fingerprintBuildContext(buildCtx: string): string {
  ...
  const updateEntry = (kind: string, relativePath: string, stat: EntrySnapshot): void => {
    ...
    // ordinary permissions. Include the complete portable Unix mode surface
    // so a post-preflight special-bit mutation cannot reuse this fingerprint.
    hash.update(`${kind}\0${relativePath}\0${String(stat.mode & 0o7777n)}\0${String(stat.size)}\0`);
  };
