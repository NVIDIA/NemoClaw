...
/** Invocation-local proof of one live sandbox, issued by a source-aware command owner. */
export interface PolicySourceAuthority {
  readonly sandboxName: string;
  readonly runtimeSelection: OpenShellRuntimeSelection;
  assertCurrent(): void;
}

export function assertPolicySourceAuthority(
  sandboxName: string,
  gatewayName: string | undefined,
  runtimeSelection: OpenShellRuntimeSelection | undefined,
  authority: PolicySourceAuthority | undefined,
): void {
  if (!authority) return;
  if (
    authority.sandboxName !== sandboxName ||
    !runtimeSelection ||
    !isDeepStrictEqual(authority.runtimeSelection, runtimeSelection) ||
    (gatewayName !== undefined && gatewayName !== runtimeSelection.gatewayName) ||
    typeof authority.assertCurrent !== "function"
  ) {
    throw new PolicyObservationError(
      "The live policy authority does not match the requested sandbox and runtime selection.",
    );
  }
  authority.assertCurrent();
}

/** Keep registry failure classification and explicit live-source selection at the gateway boundary. */
export function resolvePolicySourceGateway<T>(options: {
  sandboxName: string;
  operation: string;
  requestedGatewayName?: string;
  runtimeSelection?: OpenShellRuntimeSelection;
  sourceAuthority?: PolicySourceAuthority;
  readRegisteredSandbox(): T | null;
  resolveRegisteredGateway(sandbox: T): string;
}): string {
  let sandbox: T | null = null;
  try {
    sandbox = options.readRegisteredSandbox();
  } catch (error) {
    if (!options.sourceAuthority || !(error instanceof ConfigCorruptError)) {
      throw new PolicyObservationError(
        `Refusing to ${options.operation}: sandbox '${options.sandboxName}' policy state is unavailable.`,
      );
    }
  }
  if (!sandbox && !options.sourceAuthority) {
    throw new PolicyObservationError(
      `Refusing to ${options.operation}: sandbox '${options.sandboxName}' policy state is unavailable.`,
    );
  }
  ...
  try {
    gatewayName = sandbox
      ? options.resolveRegisteredGateway(sandbox)
      : options.sourceAuthority!.runtimeSelection.gatewayName;
  } catch {
    throw new PolicyObservationError(
      `Refusing to ${options.operation}: the recorded sandbox gateway is unavailable or invalid.`,
    );
  }
  if (options.requestedGatewayName && options.requestedGatewayName !== gatewayName) {
    throw new PolicyObservationError(
      `Refusing to ${options.operation}: the requested gateway does not match the recorded sandbox gateway.`,
    );
  }
  assertPolicySourceAuthority(
    options.sandboxName,
    gatewayName,
