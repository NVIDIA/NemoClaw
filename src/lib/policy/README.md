...
NemoClaw requires the preset entries before it reports success.
...
entries, which may include credential bindings. Apply the artifact as the policy through the external authority; do not reconstruct it from the digest
or key list. Then rerun `nemoclaw <sandbox> shields up`. NemoClaw verifies the
effective policy and locks configuration. If policy changes during the
