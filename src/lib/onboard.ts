...
function stageRemoteOllamaCredential(credentialEnv: string): string {
  ...
  // check-direct-credential-env-ignore -- intentional: preserve an explicit dedicated remote Ollama env before applying provider-key fallback
  const explicitCredential = normalizeCredentialValue(process.env[credentialEnv] ?? "");
  const existing = getCredential(credentialEnv) || explicitCredential;
  if (existing) {
    return existing;
  }
  const providerKeyHint = normalizeCredentialValue(process.env.NEMOCLAW_PROVIDER_KEY ?? "");
  const token = providerKeyHint || "ollama";
