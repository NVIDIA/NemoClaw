...
async function preflight(): Promise<ReturnType<typeof nim.detectGpu>> {
  ...
  if (gpu && gpu.type === "nvidia" && gpu.jetson) {
    // Jetson uses unified memory and cannot run local NIM containers — surface
    // the right framing up-front instead of falling into the generic
    // "VRAM too small" message below.
...
function getGatewayImageTag() {
  const openshellVersion = runCapture(["openshell", "--version"], { ignoreError: true }) || "";
...
function patchGatewayImageForJetson() {
  ...
  const inspectOut = (
    docker.dockerInspectFormat(
      '{{index .Config.Labels "io.nemoclaw.jetson-patched"}}',
      image,
      { ignoreError: true },
    ) || ""
  ).trim();
  ...
  try {
    ...
    docker.dockerRun(["build", "--quiet", "-t", image, tmpDir], {
      ignoreError: false,
    });
