...
export function ensureSandboxPortForwardForPort(
  ...
  // OpenShell v0.0.82 removes the forward PID file shortly after SIGTERM,
  ...
  // NemoClaw must compensate while the supported OpenShell 0.0.82
  ...
  const startResult = runOpenshell(
    ...
    {
      ignoreError: true,
      // OpenShell 0.0.82 leaves the background SSH forward attached to the
