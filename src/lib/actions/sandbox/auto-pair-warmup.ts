 ...
 * discarded. The leaf converts host execution failures to one fixed result,
 * and onboarding separately observes the canonical pairing state before it
 * reports success. On re-onboard where `operator.write` is already paired the
 * call succeeds and the approval pass is a no-op.
 *
...
export type SandboxScopeWarmupResult = "executed" | "exec-failed";
...
function runSandboxWarmupScript(
  ...
  try {
    ...
    if (!openshellBinary) return "exec-failed";
    ...
    return !result.error && result.status === 0 ? "executed" : "exec-failed";
  } catch {
    return "exec-failed";
  }
}
 ...
 * `openshell sandbox exec`. Host execution failures return one fixed result;
 * command output remains discarded. The finalization settlement gate decides
 * readiness from a later canonical observation.
 */
export function runSandboxScopeWarmupRun(
  sandboxName: string,
  gatewayName: string,
): SandboxScopeWarmupResult {
  return runSandboxWarmupScript(sandboxName, gatewayName, WARMUP_SCRIPT);
}
 ...
 */
export function runRestoredSandboxScopeWarmupRun(
  sandboxName: string,
): SandboxScopeWarmupResult {
  return runSandboxWarmupScript(sandboxName, undefined, RESTORED_CLONE_WARMUP_SCRIPT);
}
