...
const mocks = vi.hoisted(() => ({
  ...
  revalidateContainingMcpPolicyAuthority: vi.fn(),
...
vi.mock("./mcp-bridge-policy", async (importOriginal) => {
  ...
  return {
    ...
    qualifyMcpPolicyAuthorityReceipt: (options: object) => ({
      ...options,
      authority: "nemoclaw-managed" as const,
    }),
    removeGeneratedPolicy: mocks.removeGeneratedPolicy,
    revalidateContainingMcpPolicyAuthority: mocks.revalidateContainingMcpPolicyAuthority,
...
describe("MCP adapter teardown rollback", () => {
  beforeEach(() => {
    ...
    mocks.preflightMcpEntryTargets.mockReset().mockResolvedValue(
      new Map([[entry.server, { addresses: ["8.8.8.8"] }]]),
    );
    mocks.revalidateContainingMcpPolicyAuthority.mockReset().mockResolvedValue(undefined);
  ...
  it.each([
    ...
    async (_lifecycle, prepare) => {
      ...
      expect(mocks.restoreExistingMcpBridgeRuntime).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ lifecyclePhase: "teardown-rollback" }),
      );
