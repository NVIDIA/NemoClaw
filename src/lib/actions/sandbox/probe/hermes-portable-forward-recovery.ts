...
function parseStrictForwardList(
  ...
  const lines = stripAnsi(output)
    ...
    .map((line) => line.trim())
    .filter(Boolean);
  if (lines.length === 1 && /^No active forwards\.$/iu.test(lines[0])) return [];
...
function strictForwardListCommandOutput(result: CommandResult): string | null | undefined {
  ...
  return streams.length === 1 ? streams[0] : result.output;
}
...
function captureForwardEntries(
  ...
  const entries = parseStrictForwardList(
    strictForwardListCommandOutput(result),
    new Set(input.ports),
  );
...
function observeForwards(
  ...
  for (const port of input.ports) {
    ...
    if (entry) {
      ...
      // OpenShell 0.0.106+ `forward service` owns the production forward but
      // intentionally does not publish it through the legacy `forward list`
      // registry. A live unlisted listener is therefore the expected direct
      // service state while the sandbox and gateway authorities remain current.
      // The launcher still refuses a pre-existing listener before starting a
      // missing service; this branch never adopts or signals its process.
      states.set(port, portReachable ? "healthy" : "absent");
    }
  }
