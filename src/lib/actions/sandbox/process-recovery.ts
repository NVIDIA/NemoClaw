...
async function recoverSandboxProcesses(
  ...
  if (
    ...
    if (persistedSandbox.openshellDriver?.trim()) {
      return {
        kind: "unsupported-provider",
        failureDetail: "The registered runtime provider does not support sandbox process recovery.",
      };
    }
  }
