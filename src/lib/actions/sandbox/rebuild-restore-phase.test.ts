...
describe("rebuild policy restore fidelity", () => {
  ...
  it("stops before filesystem restore when replacement authority changed (#9833)", async () => {
    const restoreState = vi.spyOn(
      snapshotRestore,
    ...
    const validatePolicyAuthority = vi
      .fn<() => Promise<void>>()
      .mockRejectedValue(new Error("replacement authority changed"));
    ...
    ).rejects.toThrow("replacement authority changed");

    expect(restoreState).not.toHaveBeenCalled();
    expect(applyPreset).not.toHaveBeenCalled();
  });

  it("stops policy replay when authority changes during filesystem restore (#9833)", async () => {
    ...
    const validatePolicyAuthority = vi
      ...
      .mockRejectedValueOnce(new Error("authority changed during restore"));
    ...
    ).rejects.toThrow("authority changed during restore");
