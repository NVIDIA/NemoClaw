...
describe("MCP tool discovery host boundary (#6901)", () => {
  ...
  it("rejects a successful structured result when the runtime exits nonzero (#10944)", () => {
  ...
  it("keeps the process failure authoritative when the structured result also fails (#10944)", () => {
    const result = classifyMcpToolDiscoveryResult(
      {
        ...framedResult({
          protocol: MCP_TOOL_DISCOVERY_RESULT_PROTOCOL,
