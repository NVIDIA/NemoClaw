    ...
    return {
      ...
      detail: sanitizedRecoveryDetail(
        "forwardRecoveryFailureDetail" in result
          ...
          : "a required host forward could not be restored",
      ),
    };
  }
...
export async function startSandbox(
  ...
  if (resolved.bundle.identity.id === "docker" && !expectedContainerId) {
    return {
      ...
      message:
        ...
        "return its immutable container identity. The existing container was preserved; " +
  ...
  await resolved.lifecycle.verifyStarted(input, async (name) => {
    ...
    const callerRecoveryFailure = deps.startupRecovery?.onRecoveryFailure;
    const processRecovery: SandboxStartupRecoveryOptions = {
      ...deps.startupRecovery,
      expectedContainerId,
      onRecoveryFailure: (failure) => {
        recoveryFailureState.current = failure;
        callerRecoveryFailure?.(failure);
      },
    };
    ...
    await (deps.verifyGateway ?? verifyGateway)(name, {
      ...
      requirePinnedManagedGatewayProof:
