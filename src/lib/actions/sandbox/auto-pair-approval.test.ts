...
describe("auto-pair approval pass behaviour (#4616)", () => {
  itWithPython(
    "approves allowlisted local-state requests, skips unknown clients, and reports the count",
    () => {
      ...
      try {
        ...
        fs.writeFileSync(
          ...
          JSON.stringify({
            ...
            "ok-webchat-cli-upgrade": {
              requestId: "ok-webchat-cli-upgrade",
              deviceId: "web-1",
              publicKey: "web-key",
              clientId: "cli",
              clientMode: "cli",
        ...
        expect(paired["web-1"].approvedScopes).toEqual(["operator.pairing", "operator.write"]);
  ...
  itWithPython(
    "removes stale CLI pairing-only devices so user agent commands can first-pair with write",
    () => {
      const policy = readAutoPairApprovalPolicyModule();
      expect(policy).toBeTruthy();
      const policyB64 = Buffer.from(policy as string, "utf-8").toString("base64");
  ...
  itWithPython("does not approve first-time CLI requests from NemoClaw's background pass", () => {
    const policy = readAutoPairApprovalPolicyModule();
    expect(policy).toBeTruthy();
    const policyB64 = Buffer.from(policy as string, "utf-8").toString("base64");
    const script = buildAutoPairApprovalScript(policyB64, { emitSummary: true });
