...
const SUBPROCESS_TEST_TIMEOUT_MS = HARNESS_TIMEOUT_MS + 5_000;
...
`;
  ...
  const failureDetail = [
    ...
    .filter(Boolean)
    .join("\n");
  expect(result.status, `harness failed: ${failureDetail}`).toBe(0);
  return { status: result.status, stdout: result.stdout };
}

describe("MCP status wire-level credential-resolution probe", () => {
  it(
    "probes by default for a single named server and surfaces the wire failure (#6379)",
    () => {
...
`,
  ...
  it(
    "skips status probe traffic until exact policy and provider readiness are verified (#6379)",
    () => {
...
`,
  ...
  it(
    "renders the identical-rejection probe in the human-readable status output (#6379)",
    () => {
...
`,
  ...
  it(
    "keeps the status warning for identical 400 explicitly inconclusive (#6379)",
    () => {
...
`,
  ...
  it(
    "never probes from bare status or list so multi-server views stay fast (#6379)",
    () => {
...
`,
  ...
  it(
    "honors --no-probe on a named server and --probe on the multi-server form (#6379)",
    () => {
...
`,
  ...
  it(
    "rejects combining --probe with --no-probe (#6379)",
    () => {
...
describe("MCP add post-add credential-resolution probe", () => {
  it(
    "warns loudly on an identical-rejection probe without failing the committed add (#6379)",
    () => {
...
`,
      ...
      expect(payload.probed).toBe(true);
      expect(payload.logLines.some((line) => line.includes("MCP server 'github' added"))).toBe(
        true,
      );
      expect(
  ...
  it(
    "skips post-add probe traffic when policy verification is absent, drifted, or unknown (#6379)",
    () => {
...
`,
  ...
  it(
    "keeps the post-add warning for identical 400 explicitly inconclusive (#6379)",
    () => {
...
`,
  ...
  it(
    "skips the post-add probe when --no-probe is passed (#6379)",
    () => {
