...
// installer's validators up-front so model/profile conflicts, malformed extra
// args, or a gated model with no `HF_TOKEN` fail fast on the host — before any
// sandbox readiness probe, inference-route reset, or SSH attach — instead of
...
export function preflightVllmModelEnvOrExit(): void {
  ...
  console.error(
    `  Hint: NEMOCLAW_VLLM_MODEL, NEMOCLAW_VLLM_PROFILE, and NEMOCLAW_VLLM_EXTRA_ARGS_JSON are consumed by the managed-vLLM install path; they are not used by \`${CLI_NAME} <name> connect\`.`,
  );
  console.error(
