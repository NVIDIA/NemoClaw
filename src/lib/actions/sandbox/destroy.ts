...
async function destroySandboxUnlocked(
  ...
  if (deleteSucceededOrAlreadyGone && sandbox) {
    const stateVolumeCleanup = abortPreparedCleanupOnError(() =>
      removeManagedHermesStateVolume({
        ...
        createAttemptNonce: sandbox.createAttemptNonce,
      }),
    );
  ...
  if (deleteSucceededOrAlreadyGone && retainedRecoveryAuthority) {
    if (!sandbox) {
      for (const retainedVolume of retainedRecoveryAuthority.resources.managedDockerVolumes ?? []) {
        const cleanup = abortPreparedCleanupOnError(() =>
          removeRetainedManagedHermesStateVolume(sandboxName, retainedVolume),
        );
        if (cleanup.status === "failed" || cleanup.status === "not-owned") {
          console.error(
            `  Sandbox '${sandboxName}' is gone, but retained Docker volume '${cleanup.volumeName}' could not be safely removed: ${redactDestroyError(cleanup.detail)}`,
          );
          console.error("  The retained recovery record was preserved so exact cleanup can be retried.");
          requestSandboxDestroyExit(1);
        }
        if (cleanup.status === "removed") {
          console.log(`  Removed retained managed Hermes state volume for '${sandboxName}'.`);
        }
      }
    }
