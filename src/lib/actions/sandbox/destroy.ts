...
async function destroySandboxUnlocked(
  ...
  // Reap the sandbox's Hermes forward watcher as soon as the delete is
  // confirmed, before any later step in this function can exit early: the
  // managed Hermes state-volume cleanup below calls
  // `requestSandboxDestroyExit(1)` on failure, which throws and unwinds past
  // the later `cleanupSandboxServices()` call, leaving the watcher live and
  // restarting a forward for a sandbox that OpenShell has already deleted
  // (#10385). `cleanupSandboxServices()` still reaps its own gated copy for
  // any caller that reaches it independently of this function.
  if (deleteSucceededOrAlreadyGone) {
    stopHermesForwardWatchersForDestroyedSandbox(sandboxName);
  }
