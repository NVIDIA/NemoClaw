...
function rejectUnsupportedOpenShellMcpHostAlias(hostname: string): void {
  ...
  // whyNotSourceFix: v0.0.82 exposes no attested driver gateway address.
  ...
  throw new McpBridgeError(
    `Authenticated MCP OpenShell host alias '${hostname}' is unavailable with OpenShell v0.0.82 because that release does not expose an attested driver gateway address for exact policy pinning. Use a normal HTTPS DNS endpoint with public address records.`,
    2,
  );
}
...
export function normalizeMcpServerUrl(rawUrl: string): string {
  ...
  if (parsed.hostname.startsWith("[") && parsed.hostname.endsWith("]")) {
    ...
    // whyNotSourceFix: v0.0.82 does not support this target form.
...
export async function validateMcpServerUrlResolvedTarget(parsed: URL): Promise<string[]> {
  ...
  // OpenShell v0.0.82 resolves, validates every answer against allowed_ips, and
