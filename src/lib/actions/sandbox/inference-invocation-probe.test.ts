...
describe("sandbox inference invocation probe", () => {
  ...
  it("keeps curl --max-time safely inside the outer exec timeout so a slow endpoint is not killed (#11162)", () => {
    ...
    expect(
      resolveInferenceInvocationMaxTimeSeconds(READINESS_INFERENCE_INVOCATION_TIMEOUT_MS),
    ).toBe(readinessMaxTime);
  });

  it("surfaces a probe timeout when the transport reports the subprocess was killed (#11162)", async () => {
    const execute = vi.fn(
      async (
        _sandbox: string,
        _command: string,
    ...
    await expect(probeSandboxInferenceInvocation(input, { execute }, 30_000)).resolves.toEqual({
      ok: false,
  ...
  it("surfaces a subprocess error reason when the transport fails to spawn (#11162)", async () => {
    const execute = vi.fn(
      async (
        _sandbox: string,
        _command: string,
  ...
  it("keeps the generic unavailable detail when the transport reports no reason (#11162)", async () => {
  ...
  it("reports a curl-level endpoint timeout (exit 28) as a timeout, not a generic exit (#11162)", async () => {
    const execute = vi.fn(async () => ({ status: 28, stdout: "curl-error:28", stderr: "" }));
  ...
  it("reports a non-timeout curl exit accurately (#11162)", async () => {
    const execute = vi.fn(async () => ({ status: 7, stdout: "curl-error:7", stderr: "" }));
  ...
  it("surfaces a probe timeout when the Deep Agents Code launcher is killed by the outer timeout (#11162)", async () => {
    const runBuffered = vi.fn(async () => ({
      outcome: {
  ...
  it("surfaces a subprocess error when the Deep Agents Code launcher fails (#11162)", async () => {
    const runBuffered = vi.fn(async () => ({
      outcome: {
        kind: "failed" as const,
    ...
    await expect(
      ...
      ok: false,
      detail: "sandbox inference invocation probe subprocess failed (invocation)",
      httpStatus: null,
      endpoint: "https://inference.local/v1/chat/completions",
    });
  });
  ...
  it.each([
    ...
    // curl's --max-time is derived from the outer timeout so it finishes before
    // the outer kill instead of being SIGTERM-collapsed into "unavailable" (#11162).
    expect(argv[argv.indexOf("--max-time") + 1]).toBe(
      String(resolveInferenceInvocationMaxTimeSeconds(REBUILD_INFERENCE_INVOCATION_TIMEOUT_MS)),
    );
