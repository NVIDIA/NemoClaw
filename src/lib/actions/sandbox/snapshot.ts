...
function formatSnapshotVersion(b: unknown) {
	const snapshotVersion =
		(b as { snapshotVersion?: number }).snapshotVersion ?? 0;
...
function resolveSrcPodImage(
	...
	const registeredImage = (srcEntry as { imageTag?: string | null } | undefined)
		?.imageTag;
	const registeredDriver = (
		srcEntry as { openshellDriver?: string | null } | undefined
	)?.openshellDriver;
...
async function autoCreateSandboxFromSource(
	...
	const createResult = await sandboxCreateStream.streamSandboxCreate(
		command,
		process.env,
		{
	...
	if (createResult.status !== 0 && !createResult.forcedReady) {
		console.error(
			`  Failed to create sandbox '${dstName}' (exit ${createResult.status}).`,
		);
	...
	if (verify.status !== 0 || !isSandboxReady(verify.output || "", dstName)) {
		console.error(
			`  Sandbox '${dstName}' did not reach Ready state after create.`,
		);
		snapshotExit(1);
	}
	...
	const srcDriver = (srcEntry as { openshellDriver?: string | null })
		.openshellDriver;
...
function listLiveSandboxesOnSandboxGateway(
	sandboxName: string,
): Set<string> | null {
...
function requireLiveSandboxesOnSandboxGateway(
	sandboxName: string,
	error: string,
): Set<string> {
...
function verifyRestoreDestinationOnOwnGateway(targetSandbox: string): void {
	...
	if (!liveNames.has(targetSandbox)) {
		...
		console.error(
			"  Aborting restore before deleting or overwriting local sandbox metadata.",
		);
		snapshotExit(1);
	}
}
...
function runSnapshotCreate(
	...
	if (!liveNames.has(sandboxName)) {
		console.error(
			`  Sandbox '${sandboxName}' is not running. Cannot create snapshot.`,
		);
		snapshotExit(1);
	}
	if (!isSnapshotCreationAllowedByShields(sandboxName)) {
		...
		console.error(
			`  Run \`${CLI_NAME} ${sandboxName} shields down\` first, then retry.`,
		);
		snapshotExit(1);
	}
	...
	if (result.success) {
		...
		const entry =
			sandboxState.findBackup(sandboxName, manifest.timestamp).match ??
			manifest;
...
function reconcileSnapshotPolicyPresets(
	...
	const toRemove = currentPresets.filter(
		(p: string) => !snapshotPresets.includes(p),
	);
	const toAdd = snapshotPresets.filter(
		(p: string) => !currentPresets.includes(p),
	);
	...
	if (failed.length > 0) {
		console.warn(
			`  Warning: could not reconcile preset(s): ${failed.join("; ")}`,
		);
	}
}
...
function reconcileSnapshotCustomPolicies(
	...
	const snapshotByName = new Map(
		snapshotCustom.map((entry) => [entry.name, entry]),
	);
	const currentByName = new Map(
		currentCustom.map((entry) => [entry.name, entry]),
	);
	...
	const summary: string[] = [];
	if (toAdd.length > 0)
		summary.push(`add ${toAdd.map((c) => c.name).join(", ")}`);
	if (toRemove.length > 0)
		summary.push(`remove ${toRemove.map((c) => c.name).join(", ")}`);
	console.log(
		`  Reconciling custom policies on '${targetSandbox}': ${summary.join("; ")}`,
	);
	...
	if (failed.length > 0) {
		console.warn(
			`  Warning: could not reconcile custom policy(ies): ${failed.join("; ")}`,
		);
	}
}
...
async function runSnapshotRestore(
	...
	const targetSandbox =
		target === sandboxName
			? sandboxName
			: validateName(target, "target sandbox name");
	const sourceLiveNames = requireLiveSandboxesOnSandboxGateway(
		sandboxName,
		"  Failed to query live sandbox state from OpenShell.",
	);
	const isCrossSandboxRestore = targetSandbox !== sandboxName;
	const targetEntry = isCrossSandboxRestore
		? registry.getSandbox(targetSandbox)
		: null;
	const targetExists =
		sourceLiveNames.has(targetSandbox) || Boolean(targetEntry);
	...
	if (selector) {
		...
		if (!match) {
			console.error(
				`  No snapshot matching '${selector}' found for '${sandboxName}'.`,
			);
			console.error(
				"  Selector must be an exact version (v<N>), name, or timestamp.",
			);
		...
		console.log(
			`  Using latest snapshot ${v}${nameSuffix} (${latest.timestamp})`,
		);
	}
	...
	if (!isCrossSandboxRestore) {
		...
		if (!targetExists) {
			console.error(
				`  Sandbox '${targetSandbox}' is not running. Cannot restore snapshot.`,
			);
			snapshotExit(1);
		}
	} else {
		...
		if (!sourceLiveNames.has(sandboxName)) {
			if (targetExists) {
				...
				console.error(
					`  Create '${targetSandbox}' manually with '${CLI_NAME} onboard'.`,
				);
			}
			snapshotExit(1);
		}
		...
		if (!fromImage) {
			console.error(
				`  Cannot resolve image for source sandbox '${sandboxName}' — aborting before ` +
					(targetExists
						? `deleting '${targetSandbox}'.`
						: `creating '${targetSandbox}'.`),
			);
			snapshotExit(1);
		}
		if (targetExists) {
	...
	if (targetSandbox !== sandboxName) {
		console.log(
			`  Restoring snapshot from '${sandboxName}' into '${targetSandbox}'...`,
		);
	} else {
...
export async function runSandboxSnapshot(
	...
	switch (request.kind) {
		...
		case "list": {
			...
			console.log(
				`    ${CLI_NAME} ${sandboxName} snapshot restore [version|name|timestamp]`,
			);
			break;
		}
		case "restore": {
		...
		default:
			console.log(`  Usage:`);
			console.log(
				`    ${CLI_NAME} ${sandboxName} snapshot create [--name <name>]`,
			);
			console.log(
