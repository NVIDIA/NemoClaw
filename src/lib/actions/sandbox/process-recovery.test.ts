...
describe("probeSandboxInferenceGatewayHealth gateway-chain subprobe (#3265)", () => {
  ...
  it("reports reachable on a 2xx response (full chain up)", async () => {
  ...
  it("treats a 5xx response as unhealthy, not reachable (#6192)", async () => {
    const result = await probeSandboxInferenceGatewayHealth("my-sandbox", {
      execImpl: makeExec("503"),
    });
  ...
  it("reports broken when curl returns 000 (DNS or connection refused)", async () => {
    const result = await probeSandboxInferenceGatewayHealth("my-sandbox", {
  ...
  it("fails closed (unavailable) when the sandbox exec itself fails (#6192)", async () => {
    const result = await probeSandboxInferenceGatewayHealth("my-sandbox", {
      execImpl: async () => null,
    });
    expect(result.ok).toBe(false);
  ...
  it("fails closed (unavailable) when exec returns a non-zero status (#6192)", async () => {
    const result = await probeSandboxInferenceGatewayHealth("my-sandbox", {
