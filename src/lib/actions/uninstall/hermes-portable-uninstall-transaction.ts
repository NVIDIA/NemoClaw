...
export function runHermesPortableUninstallTransaction(
  ...
  let journal = readJournalFile(stateDir);
  if (!journal) {
    const authority = normalizeAuthority(deps.prepare());
  ...
  for (;;) {
    ...
    if (phase === "prepared") {
      ...
      journal = replaceJournal(stateDir, journal, "sandboxes-retired");
      continue;
    }
    if (phase === "sandboxes-retired") {
      ...
      deps.afterPhaseAction?.(phase);
      journal = replaceJournal(stateDir, journal, "providers-retired");
      continue;
    }
    if (phase === "providers-retired") {
      ...
      deps.afterPhaseAction?.(phase);
      journal = replaceJournal(stateDir, journal, "inference-retired");
      continue;
    }
    if (phase === "inference-retired") {
      ...
      deps.afterPhaseAction?.(phase);
      journal = replaceJournal(stateDir, journal, "resources-absent");
      continue;
    }
    if (phase === "resources-absent") {
      deps.retireRegistry(journal.authority);
      deps.afterPhaseAction?.(phase);
      journal = replaceJournal(stateDir, journal, "registry-retired");
      continue;
    }
    if (phase === "registry-retired") {
      deps.retireLifecycleReceipts(journal.authority);
      deps.afterPhaseAction?.(phase);
      journal = replaceJournal(stateDir, journal, "receipts-retired");
      continue;
    }
    deps.retirePrivateInferenceState(journal.authority);
    deps.afterPhaseAction?.(phase);
    journal = replaceJournal(stateDir, journal, "completed");
  }
}
