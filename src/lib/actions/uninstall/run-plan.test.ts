...
describe("uninstall run plan", () => {
  ...
  it("tears down the portable runtime on a full uninstall and fails when it cannot (#9189)", () => {
    ...
    // A failed portable teardown must not let uninstall report success.
    expect(result.exitCode).toBe(1);
  });

  it("runs the default portable teardown without docker when the portable profile is active", () => {
    const logs: string[] = [];
    const run = vi.fn((command: string, args: string[]) => {
      if (args[0] === "-c") return ok("/fake/bin/tool\n");
      if (args[0] === "-f") return ok("");
    ...
    expect(result.exitCode).toBe(0);
    // The default teardown probes podman via the injected run (read-only);
    // with no receipts and no selectors it is a no-op that stays green.
    expect(
      run.mock.calls.some(([command, args]) => command === "podman" && args[0] === "ps"),
    ).toBe(true);
    expect(run.mock.calls.some(([command, args]) => command === "podman" && args[0] === "rm")).toBe(
      false,
    );
  });
