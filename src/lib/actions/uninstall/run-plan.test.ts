...
describe("uninstall run plan", () => {
  ...
  it("removes agent-alias wrapper shims via binName-aware fd-read classification (#6098)", async () => {
  ...
  it("uses the 'already removed' wording for gateway remove no-ops, sub-bug 4 (#3456)", async () => {
    const warnings: string[] = [];
    const logs: string[] = [];
    const result = await runUninstallPlan(
      { assumeYes: true, deleteModels: false, keepOpenShell: true },
