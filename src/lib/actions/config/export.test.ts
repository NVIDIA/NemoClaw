...
vi.mock("../../config/v1alpha1-export", () => ({
...
function dependencies(): ConfigExportDependencies {
  ...
  mocks.validateV1Alpha1Export.mockReset().mockReturnValue(config);
...
describe("runConfigExport", () => {
  ...
  it("publishes a file and returns the versioned result", async () => {
    ...
    expect(mocks.validateV1Alpha1Export).toHaveBeenCalledWith({ kind: "NemoClawConfig" });
