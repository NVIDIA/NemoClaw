 ...
 */
export function waitUntil(conditionFn: () => boolean, timeoutSeconds = 10, pollIntervalMs = 250): boolean {
...
export function waitForHttp(url: string, timeoutSeconds = 5): boolean {
  ...
  return waitUntil(() => {
    try {
      const result = spawnSync("curl", ["-sf", "--connect-timeout", "1", "--max-time", "1", url], { stdio: "ignore" });
      return result.status === 0;
    } catch {
      return false;
    }
