...
const SERVICE_NAMES = ["telegram-bridge", "slack-bridge", "cloudflared"] as const;
...
export function stopAll(opts: ServiceOptions = {}): void {
  ...
  stopService(pidDir, "slack-bridge");
...
export async function startAll(opts: ServiceOptions = {}): Promise<void> {
  ...
  if (!process.env.TELEGRAM_BOT_TOKEN && (!process.env.SLACK_BOT_TOKEN || !process.env.SLACK_APP_TOKEN)) {
  ...
  // Telegram bridge (only if token is set)
  if (process.env.TELEGRAM_BOT_TOKEN) {
    const sandboxName =
      opts.sandboxName ?? process.env.NEMOCLAW_SANDBOX ?? process.env.SANDBOX_NAME ?? "default";
    startService(
  ...
  // Slack bridge (only if both tokens are set)
  if (process.env.SLACK_BOT_TOKEN && process.env.SLACK_APP_TOKEN) {
    const sandboxName =
      opts.sandboxName ?? process.env.NEMOCLAW_SANDBOX ?? process.env.SANDBOX_NAME ?? "default";
    startService(
      ...
      "slack-bridge",
      "node",
      [join(repoDir, "scripts", "slack-bridge.js")],
      { SANDBOX_NAME: sandboxName },
    );
  }
  ...
  if (isRunning(pidDir, "slack-bridge")) {
    console.log("  │  Slack:       bridge running                        │");
  } else {
    console.log("  │  Slack:       not started (missing tokens)          │");
  }
