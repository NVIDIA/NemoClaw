...
const { appendAuditEntry, redactShieldsDiagnostic } = require("./audit");
const {
  resolveAgentConfig,
...
type ShieldsPolicyAuthority = PolicyMutationAuthority & {
  shieldsPolicySnapshotPath?: string;
  shieldsRequiredPolicy?: Record<string, unknown>;
};
...
function assertShieldsPolicyMutationAuthority(
  sandboxName: string,
  operation: string,
  recorded?: ShieldsPolicyAuthority,
): ShieldsPolicyAuthority {
  if (recorded?.authority === "externally-managed" && recorded.shieldsRequiredPolicy) {
    return resolveShieldsPolicySnapshotRestoreAuthority(
      sandboxName,
      recorded.shieldsRequiredPolicy,
      recorded,
    ).authority;
  }
  if (recorded?.authority === "externally-managed" && recorded.shieldsPolicySnapshotPath) {
    return resolveShieldsPolicySnapshotRestoreAuthority(
      sandboxName,
      recorded.shieldsPolicySnapshotPath,
      recorded,
    ).authority;
  }
...
function canonicalPolicyValue(value: unknown): unknown {
  ...
  return Object.fromEntries(
    Object.keys(value)
      .sort()
      .map((key) => [key, canonicalPolicyValue(value[key])]),
  );
}

function requiredPolicyReference(requiredPolicy: Record<string, unknown>): string {
  const digest = createHash("sha256")
    .update(JSON.stringify(canonicalPolicyValue(requiredPolicy)), "utf8")
    .digest("hex");
  const policyKeys = isObjectRecord(requiredPolicy.network_policies)
    ? Object.keys(requiredPolicy.network_policies).sort()
    : [];
  return `canonical JSON SHA-256 ${digest}; network policy keys: ${
    policyKeys.length > 0 ? policyKeys.join(", ") : "(none)"
  }`;
}
...
class ExternalShieldsPolicyRecoveryError extends Error {
  constructor(readonly reason: ExternalPolicyRecoveryReason, message: string) {
    super(message);
...
function externalPolicyRecoveryHandoff(
  ...
  const cause =
    reason === "authority-drift"
      ? `Policy authority changed while NemoClaw verified recovery for sandbox '${sandboxName}'.`
      : `The effective policy for sandbox '${sandboxName}' does not match the required restrictive policy.`;
  return (
    `${cause} Required effective policy reference: ${requiredPolicyReference(requiredPolicy)}. ` +
    "The external policy authority must make the effective policy for this named sandbox match the saved restrictive snapshot and current managed MCP entries without changing policy authority. " +
    `Then run \`${CLI_NAME} ${sandboxName} shields up\`. NemoClaw will verify the exact effective policy and lock configuration.`
  );
}
...
function externalPolicyVerifiedHandoff(
  ...
  return (
    ...
    `Run \`${CLI_NAME} ${sandboxName} shields up\` to finish the config lock without changing policy authority.`
  );
}

type ShieldsPolicySnapshotRestoreAuthority = {
  authority: ShieldsPolicyAuthority;
...
function resolveShieldsPolicySnapshotRestoreAuthority(
  sandboxName: string,
  requiredPolicyOrSnapshotPath: Record<string, unknown> | string,
  recorded?: ShieldsPolicyAuthority,
): ShieldsPolicySnapshotRestoreAuthority {
  const snapshotPath =
    typeof requiredPolicyOrSnapshotPath === "string"
      ? requiredPolicyOrSnapshotPath
      : recorded?.shieldsPolicySnapshotPath;
  const requiredPolicy =
    typeof requiredPolicyOrSnapshotPath === "string"
      ? readShieldsPolicySnapshot(requiredPolicyOrSnapshotPath)
      : requiredPolicyOrSnapshotPath;
  if (
    ...
    throw new ExternalShieldsPolicyRecoveryError(
      "policy-mismatch",
      externalPolicyRecoveryHandoff(sandboxName, requiredPolicy, "policy-mismatch"),
    );
  }
  ...
  if (recorded && authority.authority !== recorded.authority) {
    throw new ExternalShieldsPolicyRecoveryError(
      "authority-drift",
      externalPolicyRecoveryHandoff(sandboxName, requiredPolicy, "authority-drift"),
    );
  }
  ...
  return {
    authority: {
      ...revalidated,
      ...(snapshotPath ? { shieldsPolicySnapshotPath: snapshotPath } : {}),
      shieldsRequiredPolicy: structuredClone(requiredPolicy),
    },
...
function inspectShieldsPolicyRecovery(sandboxName: string): ShieldsPolicyRecoveryInspection {
  ...
  try {
    ...
    return {
      status: "external",
      handoff: externalPolicyVerifiedHandoff(sandboxName, result.externalRequiredPolicy),
    };
  } catch (error) {
...
function runHermesProviderProtectionTransition(
  ...
  if (targetPosture === "mutable") {
    assertShieldsPolicyMutationAuthority(sandboxName, "change Shields provider protection");
  }
  runHermesRuntimeProviderStateMutation({
...
// OpenShell policy authority does not own the NemoClaw-managed config lock.
// After a policy refusal, restore and verify only the restrictive config posture.
function relockRestrictiveConfigUnderMutationLock(
  sandboxName: string,
  rawTarget: AgentConfigTarget,
  allowLegacyHermesProtocol = false,
  cachedProtocol?: HermesShieldsProtocol,
): ReturnType<typeof lockAgentConfigUnderMutationLock> {
  const target = ensureConfigHashSensitiveFile(rawTarget);
  const protocol = resolveHermesShieldsProtocol(
    sandboxName,
    target,
  ...
  if (!relock.ok || !relock.lastResult) {
    throw new Error(
      relock.error ?? "Restrictive config lock did not re-confirm after the settle window",
    );
  }
  return relock.lastResult;
}
...
function rethrowPolicyAuthorityRefusalAfterConfigRelock(
  ...
  try {
    ...
    const message = relockError instanceof Error ? relockError.message : String(relockError);
    console.error(
      `  CRITICAL: Policy authority changed, and the restrictive config lock could not be re-confirmed. The recorded Shields recovery remains authoritative. ${redactShieldsDiagnostic(message)}`,
    );
  }
  throw error;
}
...
function relockAgentConfigAfterPolicyAuthorityRefusal(
  sandboxName: string,
  rawTarget: AgentConfigTarget,
  processToken: string,
  allowLegacyHermesProtocol = false,
  ...
  validateName(sandboxName, "sandbox name");
  if (!/^[0-9a-f]{32}$/.test(processToken)) {
    throw new Error("Invalid policy-authority config recovery token");
  }
  return withShieldsTransitionLock(
    sandboxName,
    "restore restrictive config after policy-authority refusal",
    () =>
      relockRestrictiveConfigUnderMutationLock(sandboxName, rawTarget, allowLegacyHermesProtocol),
    { takeoverToken: processToken },
  );
}

/**
 * Rebind an existing locked Shields posture to a replacement sandbox's files.
 *
 * Rebuild owns the surrounding lifecycle lock and has already proved that the
 * old sandbox was deleted. This transition changes config permissions and the
 * persisted file seal only; it never reads or writes an OpenShell policy.
 */
function rebindReplacementConfigLock(sandboxName: string, allowLegacyHermesProtocol = false): void {
  validateName(sandboxName, "sandbox name");
  withShieldsTransitionLock(sandboxName, "rebind replacement config lock", () => {
    const state = loadShieldsState(sandboxName);
    if (state._isCorrupt) {
      throw new Error("Cannot rebind a replacement config lock from corrupt Shields state");
    }
    if (state.shieldsDown !== false) {
      throw new Error("Cannot rebind a replacement config lock without a recorded lockdown");
    }
    if (readTimerMarker(sandboxName)) {
      throw new Error("Cannot rebind a replacement config lock while a Shields timer is active");
    }
...
function applyShieldsPolicySnapshot(
  ...
  if (options.deadlineAuthoritative) {
    ...
    const managedMcpPolicies = resolveExactManagedMcpPolicies(
      ...
      policyMutationAllowed && !options.externalVerificationOnly
        ? undefined
        : YAML.stringify(policyAuthority.inspection.effectivePolicy),
      policyAuthority.gatewayName,
    );
  ...
  try {
    ...
    if (options.externalVerificationOnly && policyMutationAllowed) {
      throw new ExternalShieldsPolicyRecoveryError(
        "authority-drift",
        externalPolicyRecoveryHandoff(sandboxName, externalRequiredPolicy, "authority-drift"),
      );
    }
...
function rollbackShieldsDown(
  ...
  if (rollbackResult?.status === 0) {
    if (initialMode === "mutable_default" && target.agentName === "openclaw") {
      try {
        assertShieldsPolicyMutationAuthority(sandboxName, "restore mutable Shields config");
        unlockAgentConfigUnderMutationLock(sandboxName, target, false, protocol);
        assertShieldsPolicyMutationAuthority(sandboxName, "continue Shields rollback");
        ...
        assertShieldsPolicyMutationAuthority(sandboxName, "complete Shields rollback");
  ...
  if (rollbackChattrApplied !== null && rollbackFileHashes !== null) {
    ...
    assertShieldsPolicyMutationAuthority(sandboxName, "record the Shields rollback state");
    saveShieldsState(sandboxName, {
      shieldsDown: false,
      shieldsDownAt: null,
      shieldsDownTimeout: null,
...
interface LockdownActivationResult {
  ...
  fileHashes?: { [path: string]: string };
  managedMcpOmissions?: ManagedMcpPolicyOmission[];
  policyAuthority?: ShieldsPolicyAuthority;
}
...
function activateLockdownFromSnapshot(
  ...
  let restoreAuthority: ShieldsPolicyAuthority;
  try {
    restoreAuthority = resolveShieldsPolicySnapshotRestoreAuthority(
      sandboxName,
      restoreResult.externalRequiredPolicy ?? snapshotPath,
    ).authority;
  } catch (error) {
    if (isPolicyAuthorityRefusalError(error)) throw error;
    return {
      ok: false,
      error: `policy verification before config lock failed: ${
        error instanceof Error ? error.message : String(error)
      }`,
    };
  }
  let completedAuthority = restoreAuthority;
  assertShieldsPolicyMutationAuthority(
    sandboxName,
    "continue restoring Shields after the policy change",
    restoreAuthority,
  );
  ...
  assertShieldsPolicyMutationAuthority(
    sandboxName,
    "restore the Shields config lock",
    restoreAuthority,
  );
  let lastLockResult: ReturnType<typeof lockAgentConfig> | undefined;
  let relock: ReturnType<typeof relockAndReconfirm>;
  try {
    relock = relockAndReconfirm(() => {
      assertShieldsPolicyMutationAuthority(
        sandboxName,
        "restore the Shields config lock",
        restoreAuthority,
      );
      lastLockResult = lockAgentConfig(
        sandboxName,
        target,
        false,
        allowLegacyHermesProtocol,
    ...
    if (!lastLockResult) throw error;
    return {
      ok: false,
      error: `policy verification after config lock failed: ${
        error instanceof Error ? error.message : String(error)
      }`,
      chattrApplied: lastLockResult.chattrApplied,
      fileHashes: lastLockResult.fileHashes,
    };
  }
  if (!relock.ok || !relock.lastResult) {
    if (lastLockResult) {
      return {
        ok: false,
        error: `policy verification after config lock failed: ${
          relock.error ?? "policy authority changed during config lock verification"
        }`,
        chattrApplied: lastLockResult.chattrApplied,
        fileHashes: lastLockResult.fileHashes,
      };
  ...
  try {
    assertShieldsPolicyMutationAuthority(
      sandboxName,
      "complete the Shields config lock",
      restoreAuthority,
    );
    if (restoreResult.externalPolicyVerified) {
      const verifiedRestore = applyShieldsPolicySnapshot(sandboxName, snapshotPath, {
        ...restoreOptions,
        externalVerificationOnly: true,
      });
      if (!verifiedRestore.externalRequiredPolicy) {
        throw new Error("External Shields policy verification did not return its required policy");
      }
      completedAuthority = resolveShieldsPolicySnapshotRestoreAuthority(
        sandboxName,
        verifiedRestore.externalRequiredPolicy,
      ).authority;
    } else {
      completedAuthority = resolveShieldsPolicySnapshotRestoreAuthority(
        sandboxName,
  ...
  return {
    ...
    chattrApplied: relock.lastResult.chattrApplied,
    fileHashes: relock.lastResult.fileHashes,
    policyAuthority: completedAuthority,
...
function recoverExpiredAutoRestoreInline(
  ...
  assertShieldsPolicyMutationAuthority(sandboxName, "record inline Shields recovery");
  saveShieldsState(sandboxName, {
    shieldsDown: false,
    shieldsDownAt: null,
    shieldsDownTimeout: null,
  ...
  assertShieldsPolicyMutationAuthority(sandboxName, "continue inline Shields recovery");
  if (marker?.processToken && /^[0-9a-f]{32}$/.test(marker.processToken)) {
    clearShieldsDownTransition(sandboxName, marker.processToken);
    assertShieldsPolicyMutationAuthority(sandboxName, "continue inline Shields recovery");
  }
  ...
  assertShieldsPolicyMutationAuthority(
    sandboxName,
    "write the inline Shields recovery audit entry",
  );
  appendAuditEntry({
  ...
  assertShieldsPolicyMutationAuthority(sandboxName, "complete inline Shields recovery");
...
function applyRecoveredShieldsDownForwardPolicy(
  ...
  assertShieldsPolicyMutationAuthority(
    sandboxName,
    "continue the interrupted Shields down transition",
    policyAuthority,
  );
  if (result.status !== 0) {
  ...
  assertShieldsPolicyMutationAuthority(
    sandboxName,
    "complete the interrupted Shields down policy change",
    policyAuthority,
  );
}
...
function finishRecoveredHermesShieldsDown(
  ...
  assertShieldsPolicyMutationAuthority(sandboxName, "complete interrupted Shields down");
  if (completion.authority) {
    assertRecoveredShieldsDownAuthority(sandboxName, completion, completion.authority.phase);
  }
  const convergence = waitForHermesInferenceRouteConvergence(sandboxName, { run });
  assertShieldsPolicyMutationAuthority(sandboxName, "continue interrupted Shields down");
  if (!convergence.ok) {
  ...
  if (completion.transition) {
    assertShieldsPolicyMutationAuthority(
      sandboxName,
      "publish the interrupted Shields down transition",
    );
    writeShieldsDownTransition({ ...completion.transition, phase: "active" }, "preparing");
    assertShieldsPolicyMutationAuthority(
      sandboxName,
      "complete the interrupted Shields down transition",
    );
  }
  if (completion.authority) {
...
function completeInterruptedShieldsDown(
  ...
  assertShieldsPolicyMutationAuthority(sandboxName, "recover interrupted Shields down");
  ...
  try {
    ...
    assertShieldsPolicyMutationAuthority(sandboxName, "unlock interrupted Shields config");
  ...
  if (!completion.alreadyCommitted) {
    ...
    assertShieldsPolicyMutationAuthority(
      sandboxName,
      "write the interrupted Shields down audit entry",
    );
    appendAuditEntry(completion.audit);
    assertShieldsPolicyMutationAuthority(sandboxName, "complete interrupted Shields down");
  }
  assertShieldsPolicyMutationAuthority(sandboxName, "publish interrupted Shields down");
...
function shieldsDownWithoutHostLock(
  ...
  // OpenShell accepted the weaker policy. Recheck authority before making
  // config mutable; a refusal here deliberately leaves the timer, snapshot,
  // and provisional state available to restore the restrictive posture.
  assertShieldsPolicyMutationAuthority(
    sandboxName,
    "continue Shields down after the policy change",
    policyAuthority,
  );
  ...
  try {
    ...
    assertShieldsPolicyMutationAuthority(sandboxName, "unlock the Shields config", policyAuthority);
    if (
      target.agentName === "hermes" &&
    ...
    assertShieldsPolicyMutationAuthority(sandboxName, "complete Shields down", policyAuthority);
  } catch (err) {
    if (isPolicyAuthorityRefusalError(err)) {
      rethrowPolicyAuthorityRefusalAfterConfigRelock(
        err,
        sandboxName,
        target,
  ...
  if (transition) {
    try {
      ...
      assertShieldsPolicyMutationAuthority(
        sandboxName,
        "publish the Shields down transition",
        policyAuthority,
      );
      transition = { ...transition, phase: "active" };
      writeShieldsDownTransition(transition, "preparing");
  ...
  let policySnapshotRecovery: ShieldsPolicySnapshotRecovery | undefined;
  try {
    assertShieldsPolicyMutationAuthority(
      sandboxName,
      "publish the Shields down result",
      policyAuthority,
    );
    if (opts.issuePolicySnapshotRecovery) {
      assertShieldsPolicyMutationAuthority(
        sandboxName,
        "issue the Shields down recovery receipt",
        policyAuthority,
      );
      policySnapshotRecovery = issueShieldsPolicySnapshotRecovery(
        sandboxName,
        transition,
        snapshotPolicy,
        policyYaml,
  ...
  assertShieldsPolicyMutationAuthority(
    sandboxName,
    "write the Shields down audit entry",
    policyAuthority,
  );
  appendAuditEntry({
    action: "shields_down",
...
function shieldsUpWithoutHostLock(sandboxName: string, opts: ShieldsUpOpts = {}): void {
  ...
  let policyAuthority: ShieldsPolicyAuthority;
  try {
    policyAuthority =
      state.shieldsDown && state.shieldsPolicySnapshotPath
        ? inspectShieldsPolicySnapshotRestoreAuthority(sandboxName)
        : assertShieldsPolicyMutationAuthority(sandboxName, "raise Shields");
  } catch (error) {
    if (isExternalPolicyAuthorityRefusalError(error) && error instanceof Error) {
      console.error(`  ${error.message}`);
    }
    throw error;
  }
  if (recoveredProviderTarget) {
    assertShieldsPolicyMutationAuthority(sandboxName, "continue raising Shields", policyAuthority);
  }
  ...
  if (state.shieldsDown === false) {
    ...
    if (issues.length === 0) {
      ...
      assertShieldsPolicyMutationAuthority(sandboxName, "complete Shields up", policyAuthority);
      clearTimerMarker(sandboxName);
      assertShieldsPolicyMutationAuthority(
        sandboxName,
        "publish the Shields up result",
        policyAuthority,
      );
    ...
    const relock = relockAndReconfirm(() => {
      assertShieldsPolicyMutationAuthority(
        sandboxName,
        "continue restoring the Shields config lock",
        policyAuthority,
      );
    ...
    assertShieldsPolicyMutationAuthority(
      sandboxName,
      "complete the Shields up state",
      policyAuthority,
    );
    clearTimerMarker(sandboxName);
    assertShieldsPolicyMutationAuthority(
      sandboxName,
      "write the Shields up audit entry",
      policyAuthority,
    );
    appendAuditEntry({
      action: "shields_up",
  ...
  if (snapshotPath) {
    ...
    policyAuthority = activation.policyAuthority ?? policyAuthority;
    assertShieldsPolicyMutationAuthority(sandboxName, "continue raising Shields", policyAuthority);
    ...
    assertShieldsPolicyMutationAuthority(
      sandboxName,
      "record the Shields config lock",
      policyAuthority,
    );
    saveShieldsState(sandboxName, {
      chattrApplied: lockResult.chattrApplied,
    ...
    assertShieldsPolicyMutationAuthority(
      sandboxName,
      "complete the Shields config lock state",
      policyAuthority,
    );
  }
  ...
  assertShieldsPolicyMutationAuthority(sandboxName, "record the Shields up state", policyAuthority);
  saveShieldsState(sandboxName, {
    shieldsDown: false,
    shieldsDownAt: null,
    shieldsDownTimeout: null,
  ...
  assertShieldsPolicyMutationAuthority(
    sandboxName,
    "continue publishing Shields up",
    policyAuthority,
  );
...
export {
  ...
  relockAgentConfigAfterPolicyAuthorityRefusal,
