...
// Decide mint vs skip without mutating /sandbox or the config directory.
export const DEEP_AGENTS_SHIELDS_DOWN_HASH_MINT_GATE_SCRIPT = String.raw`
import errno
import os
import stat
import sys
...
config_name = config_child_name(config_dir, sys.argv[2])
if len(sys.argv) != 3:
    die("unsupported shields-down hash mint gate arguments")
...
try:
    dir_fd = os.open(config_dir, dir_flags)
except OSError as exc:
    if exc.errno in (errno.ENOENT, errno.ELOOP, getattr(errno, "ENOTDIR", errno.EINVAL)):
        sys.stdout.write("skip\n")
        raise SystemExit(0)
    die("open failed for %s: %s" % (config_dir, exc))
...
try:
    ...
    try:
        config_fd = os.open(config_name, file_flags, dir_fd=dir_fd)
    except OSError as exc:
        if exc.errno in (errno.ENOENT, errno.ELOOP):
            sys.stdout.write("skip\n")
            raise SystemExit(0)
        die("open failed for %s: %s" % (config_name, exc))
    try:
...
export function buildDeepAgentsShieldsDownHashMintGateCommand(
  configDir: string,
  configPath: string,
): string[] {
  return [
