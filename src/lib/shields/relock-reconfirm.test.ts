...
import {
  ...
  relockAndReconfirm,
  resolveSettleMs,
} from "./relock-reconfirm";

const SUBMITTED_POLICY_HASH = "d71616b0333b";

function timedOutPolicySet(overrides: { stdout?: string; stderr?: string } = {}) {
  return {
    status: 1,
    stdout: overrides.stdout ?? `Policy version 11 submitted (hash: ${SUBMITTED_POLICY_HASH})\n`,
    stderr: overrides.stderr ?? "Timeout waiting for policy version 11 to load\n",
  };
}

function policyReceipt(hash = SUBMITTED_POLICY_HASH, version = "11", status = "pending") {
  return {
    status: 0,
...
describe("deferred permissive policy confirmation", () => {
  it("accepts an exact committed receipt only after proving no sandbox container runs (#8304)", () => {
    const proveNoRunningDirectSandbox = vi.fn(() => true);
    const readPolicyReceipt = vi.fn(() => policyReceipt());

    expect(
      confirmDeferredPermissivePolicyForStoppedSandbox(timedOutPolicySet(), {
        proveNoRunningDirectSandbox,
        readPolicyReceipt,
        parsePolicy: () => "version: 1\nnetwork_policies: {}",
      }),
    ).toBe(true);
    expect(proveNoRunningDirectSandbox).toHaveBeenCalledOnce();
    expect(readPolicyReceipt).toHaveBeenCalledOnce();
  });
  ...
  it("rejects a receipt whose policy body is invalid (#8304)", () => {
    expect(
      confirmDeferredPermissivePolicyForStoppedSandbox(timedOutPolicySet(), {
        proveNoRunningDirectSandbox: () => true,
        readPolicyReceipt: () => policyReceipt(),
        parsePolicy: () => "",
      }),
    ).toBe(false);
  });
