...
function getSandboxInferenceConfig(model, provider = null, preferredInferenceApi = null) {
  ...
  switch (provider) {
    ...
    case "ollama-local":
    case "vllm-local":
      // Local-inference providers (used on Jetson and other GPU-light hosts):
      // point the sandbox at the host's local endpoint instead of the gateway
      // proxy URL so model resolution doesn't require a round-trip through
      // inference.local.
      providerKey = "inference";
      primaryModelRef = `inference/${model}`;
