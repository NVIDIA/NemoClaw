...
const ONBOARD_BASE_ARGS = ["--non-interactive", "--resume", "--fresh", "--recreate-sandbox", "--gpu"];

function onboardUsageLines(noticeAcceptFlag: string): string[] {
  const name = CLI_NAME;
  return [
    `  Usage: ${name} onboard [--non-interactive] [--resume | --fresh] [--recreate-sandbox] [--gpu] [--from <Dockerfile>] [--name <sandbox>] [--agent <name>] [--control-ui-port <N>] [${noticeAcceptFlag}]`,
    "",
