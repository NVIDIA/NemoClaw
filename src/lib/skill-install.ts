...
export function buildCanonicalSkillAddCommand(
  ...
  return [
    ...
    [
      ...
      'destination="$name"',
      'if [ -e "$destination" ] || [ -L "$destination" ]; then [ -d "$destination" ] && [ ! -L "$destination" ] && [ "$(realpath -e -- "$destination")" = "$destination" ]; fi',
...
export function buildCanonicalSkillRemoveCommand(
  ...
  return [
    ...
    [
      ...
      '[ -d "$destination" ] && [ ! -L "$destination" ] && [ "$(realpath -e -- "$destination")" = "$destination" ]',
      'rm -rf -- "$destination"',
