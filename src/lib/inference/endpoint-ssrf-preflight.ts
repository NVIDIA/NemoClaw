...
export async function assertEndpointResolvesPublic(
  ...
  try {
    ...
    // Inference retains its established local routes, except hosted-only
    // onboarding explicitly disables host loopback (portable profile).
    if (
      (isLoopbackHostname(hostname) && options.allowExplicitLoopback !== false) ||
      isOpenShellManagedHost(hostname)
    ) {
