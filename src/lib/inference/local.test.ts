...
describe("local inference helpers", () => {
  ...
  it("normalizes remote Ollama URLs into API root and OpenAI-compatible base URLs", () => {
    ...
    expect(normalizeRemoteOllamaBaseUrl("https://lan-box.example.com/ollama")).toEqual({
      rootUrl: "https://lan-box.example.com/ollama",
      openAiBaseUrl: "https://lan-box.example.com/ollama/v1",
      tagsUrl: "https://lan-box.example.com/ollama/api/tags",
    });
  });

  it("rejects remote Ollama URLs without an HTTP(S) origin", () => {
    expect(normalizeRemoteOllamaBaseUrl("lan-box:11434")).toBeNull();
    expect(normalizeRemoteOllamaBaseUrl("ftp://lan-box:11434")).toBeNull();
    expect(normalizeRemoteOllamaBaseUrl("mailto:ollama@example.com")).toBeNull();
  });
