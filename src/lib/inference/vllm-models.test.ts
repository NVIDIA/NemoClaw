...
describe("vllm model registry", () => {
  ...
  it("keeps the qualified single-user recipe explicit, exact, and separate from the canonical Ultra recipe", () => {
    const ultra = VLLM_MODELS.find((m) => m.envValue === "nemotron-3-ultra-550b-a55b");
    expect(ultra).toBeDefined();

    const canonical = resolveVllmModelProfile(ultra!, "station", {} as NodeJS.ProcessEnv);
    expect(canonical).toEqual({ model: ultra, profile: null });
    expect(canonical.model).toBe(ultra);

    const resolved = resolveVllmModelProfile(ultra!, "station", {
      [VLLM_PROFILE_ENV]: EXPERIMENTAL_SINGLE_USER_PROFILE,
    } as NodeJS.ProcessEnv);
    const profile = resolved.profile;
    expect(profile).toBeDefined();
    expect(profile).toEqual({
      id: "experimental-single-user",
      ...
      runtime: {
        ...
        expectedImageId: "sha256:e44830d3f0197850e5a63ca895b13a047f358ff4aea15b1655abf61391bb883e",
        qualification: {
    ...
    expect(resolved.model.revision).toBe("183968f87ae4cedce3039313cac1fd43d112c578");
    expect(resolved.model.servedModelId).toBe("nemotron-ultra");
    expect(resolved.model.serveModelPath).toMatch(
      /\/snapshots\/183968f87ae4cedce3039313cac1fd43d112c578$/,
    );
    expect(resolved.model.readiness).toEqual({
      healthEndpoint: true,
      servedModelId: "nemotron-ultra",
      maxModelLen: 262144,
    });
    expect(resolved.model.serveEnv).toBeUndefined();

    const cmd = buildVllmServeCommand(resolved.model, {} as NodeJS.ProcessEnv);
    expect(cmd).toBe(
      [
        "vllm serve",
        "/root/.cache/huggingface/hub/models--nvidia--NVIDIA-Nemotron-3-Ultra-550B-A55B-NVFP4/snapshots/183968f87ae4cedce3039313cac1fd43d112c578",
    ...
    const driftedCanonical = { ...ultra!, revision: "future-canonical-revision" };
    const independentlyPinned = resolveVllmModelProfile(driftedCanonical, "station", {
      [VLLM_PROFILE_ENV]: EXPERIMENTAL_SINGLE_USER_PROFILE,
    } as NodeJS.ProcessEnv).model;
    expect(independentlyPinned.revision).toBe("183968f87ae4cedce3039313cac1fd43d112c578");
    expect(independentlyPinned.serveModelPath).toMatch(
      /\/snapshots\/183968f87ae4cedce3039313cac1fd43d112c578$/,
    );
  });

  it("rejects invalid or incompatible explicit model profiles before runtime work", () => {
    const ultra = VLLM_MODELS.find((m) => m.envValue === "nemotron-3-ultra-550b-a55b");
    ...
    expect(() =>
      selectVllmModelProfileFromEnv({
        [VLLM_PROFILE_ENV]: "unknown-profile",
      } as NodeJS.ProcessEnv),
    ).toThrow(/Unknown NEMOCLAW_VLLM_PROFILE='unknown-profile'/);
    expect(() =>
      resolveVllmModelProfile(qwen!, "station", {
        [VLLM_PROFILE_ENV]: EXPERIMENTAL_SINGLE_USER_PROFILE,
      } as NodeJS.ProcessEnv),
    ).toThrow(/requires NEMOCLAW_VLLM_MODEL='nemotron-3-ultra-550b-a55b'/);
    expect(() =>
      resolveVllmModelProfile(ultra!, "linux", {
        [VLLM_PROFILE_ENV]: EXPERIMENTAL_SINGLE_USER_PROFILE,
      } as NodeJS.ProcessEnv),
    ).toThrow(/is not supported on linux/);
    expect(() =>
      resolveVllmModelProfile(ultra!, "station", {
        [VLLM_PROFILE_ENV]: EXPERIMENTAL_SINGLE_USER_PROFILE,
        [VLLM_EXTRA_ARGS_ENV]: '["--enable-prefix-caching"]',
      } as NodeJS.ProcessEnv),
    ).toThrow(/cannot be combined with NEMOCLAW_VLLM_EXTRA_ARGS_JSON/);
  });
...
describe("preflightVllmModelEnv", () => {
  ...
  it("succeeds for the explicit qualified Station profile and matching Ultra model", () => {
    expect(
      preflightVllmModelEnv({
  ...
  it("rejects invalid or incompatible profile input", () => {
    expect(
      preflightVllmModelEnv({
        [VLLM_PROFILE_ENV]: "unknown-profile",
      } as NodeJS.ProcessEnv),
      ...
      ok: false,
      message: expect.stringMatching(/Unknown NEMOCLAW_VLLM_PROFILE='unknown-profile'/),
    });
    expect(
      preflightVllmModelEnv({
        [VLLM_PROFILE_ENV]: EXPERIMENTAL_SINGLE_USER_PROFILE,
      ...
      ok: false,
      message:
        "NEMOCLAW_VLLM_PROFILE='experimental-single-user' requires " +
        "NEMOCLAW_VLLM_MODEL='nemotron-3-ultra-550b-a55b'.",
    });
    expect(
      preflightVllmModelEnv({
