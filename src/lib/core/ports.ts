...
/** Default Hermes OpenAI-compatible API port (manifest `forward_ports[1]` / start.sh `PUBLIC_PORT`). */
 ...
 * OpenAI-compatible API on its own allocated port so two sandboxes can serve
 * inference on one host. Every port in that range is therefore unavailable as a
